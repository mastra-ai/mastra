...
export function CardItems({ titles, items }: CardItemsProps) {
  const [activeTab, setActiveTab] = useState(sluggify(titles[0]));
      ...
      <div className="mt-6 w-full gap-3 grid md:grid-cols-2 lg:grid-cols-3">
        {currentItems.map((item) => (
          <Link
            ...
            className="flex-1 flex text-center bg-(--mastra-surface-3) mb-0 border-[0.5px] rounded-md dark:border-[#343434] border-(--border) items-center group justify-center p-2 px-4 text-sm hover:opacity-80 transition-opacity"
          >
            {item.title}
          </Link>
        ))}
