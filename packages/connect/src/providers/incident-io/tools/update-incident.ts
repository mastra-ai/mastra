        ...
        incident: z
          .object({
            ...
            incident_timestamp_values: z
              .array(z.object({ incident_timestamp_id: z.string(), value: z.string().optional() }).passthrough())
              .optional(),
          ...
          .passthrough(),
        notify_incident_channel: z.boolean(),
      })
      .passthrough(),
  })
  .passthrough();
...
export const updateIncidentOutputSchema = ProviderResponseSchema;

export function updateIncidentTool(proxy: PlatformProxy) {
  return createTool({
    id: 'incident_io_update_incident',
    description: 'Update incident in incident.io.',
    inputSchema: updateIncidentInputSchema,
    outputSchema: updateIncidentOutputSchema,
    execute: async (input, { requestContext }): Promise<z.infer<typeof updateIncidentOutputSchema>> => {
      const platformProxy = proxy.withRequestContext(requestContext);
      const config: PlatformProxyRequest = {
        // https://api.incident.io/v1/openapiV3.json,
        endpoint: `/v2/incidents/${encodeURIComponent(input['id'])}/actions/edit`,
        retries: 0,
        data: input.body,
      };
