...
export const getIncidentInputSchema = z.object({ id: z.string() });
...
export const getIncidentOutputSchema = ProviderResponseSchema;

export function getIncidentTool(proxy: PlatformProxy) {
  return createTool({
    id: 'incident_io_get_incident',
    description: 'Get incident in incident.io.',
    inputSchema: getIncidentInputSchema,
    outputSchema: getIncidentOutputSchema,
    execute: async (input, { requestContext }): Promise<z.infer<typeof getIncidentOutputSchema>> => {
