...
export const listEndpointsInputSchema = z.object({ project_id: z.string().regex(new RegExp('^[a-z0-9-]{1,60}$')) });
...
export const listEndpointsOutputSchema = ProviderResponseSchema;

export function listEndpointsTool(proxy: PlatformProxy) {
  return createTool({
    id: 'neon_list_endpoints',
    description: 'List endpoints in Neon.',
    inputSchema: listEndpointsInputSchema,
    outputSchema: listEndpointsOutputSchema,
    execute: async (input, { requestContext }): Promise<z.infer<typeof listEndpointsOutputSchema>> => {
