...
describe('MastraMCPClient - MCP Tool Tracing', () => {
  let testServer: {
    httpServer: HttpServer;
    mcpServer: McpServer;
    serverTransport: StreamableHTTPServerTransport;
  ...
  function createMockTracingContext() {
    ...
    const mockCreateChildSpan = vi.fn(() => ({
      end: mockEnd,
      error: mockError,
  ...
  it('should create MCP_TOOL_CALL span on successful tool execution', async () => {
    const { tracingContext, mockCreateChildSpan, mockEnd, mockError } = createMockTracingContext();
    const tools = await client.tools();

    const result = await tools.greet?.execute?.({ name: 'Traced' }, { tracingContext } as any);

    expect(result).toEqual({ content: [{ type: 'text', text: 'Hello, Traced!' }] });
    expect(mockCreateChildSpan).toHaveBeenCalledTimes(1);
    expect(mockCreateChildSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        type: 'mcp_tool_call',
        name: expect.stringContaining('greet'),
        entityId: 'tracing-test-client_greet',
        entityName: 'greet',
        attributes: expect.objectContaining({
          mcpServer: 'tracing-test-client',
        }),
      }),
    );
  ...
  it('should include input data in span', async () => {
    const { tracingContext, mockCreateChildSpan } = createMockTracingContext();
    const tools = await client.tools();
    const input = { name: 'InputTest' };

    await tools.greet?.execute?.(input, { tracingContext } as any);
  ...
  it('should not fail when tracingContext is not provided', async () => {
    const tools = await client.tools();
    const result = await tools.greet?.execute?.({ name: 'NoTrace' });
    expect(result).toEqual({ content: [{ type: 'text', text: 'Hello, NoTrace!' }] });
  });

  it('should not fail when tracingContext has no currentSpan', async () => {
    const tools = await client.tools();
    const result = await tools.greet?.execute?.({ name: 'NoSpan' }, { tracingContext: {} } as any);
    expect(result).toEqual({ content: [{ type: 'text', text: 'Hello, NoSpan!' }] });
  });

  it('should error span when tool execution fails', async () => {
    const { tracingContext, mockEnd, mockError } = createMockTracingContext();

    // Get tools while server is still running
    const tools = await client.tools();

    // Now shut down the server to cause the callTool to fail
    await testServer.mcpServer.close();
    await testServer.serverTransport.close();
    testServer.httpServer.close();

    try {
      await tools.greet?.execute?.({ name: 'Fail' }, { tracingContext } as any);
    } catch {
      // Expected to throw
    }
  ...
  it('should set correct span name with server and tool names', async () => {
    const { tracingContext, mockCreateChildSpan } = createMockTracingContext();
    const tools = await client.tools();

    await tools.greet?.execute?.({ name: 'NameTest' }, { tracingContext } as any);

    expect(mockCreateChildSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        name: "mcp_tool: 'greet' on 'tracing-test-client'",
      }),
    );
  });
});
