...
for (const name of ["startSSE", "startHonoSSE", "connectSSE", "handleServerlessRequest", "sendLoggingMessage", "getServer"]) assert.ok(!(name in proto), `MCPServer.${name} must not exist`);
