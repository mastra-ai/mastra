...
The runner checks the packed export inventory (removed surfaces absent, v2 ones present), typechecks, then runs an isolated HTTP server on an assigned port and a stdio server as a child process. It closes its own clients, servers and processes.
