...
- 85436ef: Remove deprecated `mcpClient.getResources()`, `MCPConfiguration` class and `MCPConfigurationOptions`

### Minor Changes

- 844ea5d: Update peer dependencies to match core package version bump (1.0.0)
- 9d819d5: Update peer dependencies to match core package version bump (0.22.2)

### Patch Changes

- e314bb8: Add timeout configuration to mcp server config
- 8dcf9f9: add flag to skip sessions and streaming in serverless mcp
...
- 9d819d5: Breaking change to move mcp related tool execute arguments nested under an `mcp` argument that is only populated if the tool is passed to an MCPServer. This simpliflies tool definitions and gives you the correct types when working with tools meant for MCP servers.
- d70e742: Fix MCP auth context propagation to tools called by agents
