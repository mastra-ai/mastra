...
describe('CompletenessScorer', () => {
  ...
  describe('basic functionality', () => {
    it('should return high score for identical text', async () => {
      ...
      const result = await scorer.measure({ input: text, output: text });
    ...
    it('should return lower score for simplified text missing elements', async () => {
      ...
      const result = await scorer.measure({ input: original, output: simplified });
  ...
  describe('edge cases', () => {
    ...
    it('should handle empty original string', async () => {
      const result = await scorer.measure({ input: '', output: 'some text' });
      expect(result.score).toBe(0);
    });

    it('should handle whitespace-only strings', async () => {
      const result = await scorer.measure({ input: '   \n  ', output: '  \n  ' });
      expect(result.score).toBe(1);
      const { input, output } = result.metrics?.elementCounts as { input: number; output: number };
      expect(input).toBe(0);
      expect(output).toBe(0);
    ...
    it('should handle null and undefined inputs', async () => {
      ...
      await expect(scorer.measure({ input: null, output: '' })).rejects.toThrow();
      // @ts-expect-error Testing invalid input
      await expect(scorer.measure({ input: '', output: undefined })).rejects.toThrow();
    });
  });

  describe('special cases', () => {
    it('should handle lists and enumerations', async () => {
      const result = await scorer.measure({ input: 'apples, oranges, and bananas', output: 'apples and bananas' });
      expect(result.score).toBeLessThan(0.8);
      expect(result.metrics?.missingElements).toContain('oranges');
    });

    it('should handle repeated elements', async () => {
      const result = await scorer.measure({ input: 'cat cat cat cats', output: 'cat cats' });
