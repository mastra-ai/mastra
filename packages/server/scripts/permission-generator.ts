...
/**
 * Permission values kept for backwards compatibility even when no current
 * server route derives them directly.
 */
const COMPATIBILITY_PERMISSIONS: string[] = [];
...
export function derivePermissionData(): PermissionData {
  ...
  for (const permission of COMPATIBILITY_PERMISSIONS) {
 ...
 *
 * Source of truth: SERVER_ROUTES plus explicit compatibility permissions.
 */

/**
 * All known API resources.
 * Derived from SERVER_ROUTES paths and explicit compatibility permissions.
 */
