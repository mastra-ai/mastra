      ...
      if (selectedToolkits || selectedTools) {
        try {
          ...
          if (updatedIntegration.provider === 'mcp' || updatedIntegration.provider === 'smithery') {
            ...
            console.info('MCP update - transport:', isHttpTransport ? 'http' : isStdioTransport ? 'stdio' : 'unknown');
            console.info('MCP update - metadata:', JSON.stringify(mcpMetadata, null, 2));
            ...
            console.info('MCP update - creating provider with config:', JSON.stringify(mcpConfig, null, 2));
            const mcpProvider = createMCPProvider(mcpConfig);

            try {
              console.info('MCP update - selectedTools:', selectedTools);
              console.info('MCP update - connecting to MCP server...');

              const toolsResponse = await mcpProvider.listTools({ limit: 1000 });
              console.info('MCP update - got tools from server:', toolsResponse.tools.length);

              // Filter by selectedTools if provided
              const toolsToCache = selectedTools
                ? toolsResponse.tools.filter(tool => selectedTools.includes(tool.slug))
                : toolsResponse.tools;
              console.info('MCP update - tools to cache after filter:', toolsToCache.length);
              ...
              console.info('MCP update - deleted old cached tools');
              ...
              if (cachedTools.length > 0) {
                await integrationsStore.cacheTools({ tools: cachedTools });
                console.info('MCP update - cached tools:', cachedTools.length);
              } else {
                console.info('MCP update - no tools to cache!');
              }
            } finally {
              await mcpProvider.disconnect();
            }
