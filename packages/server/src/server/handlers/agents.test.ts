...
describe('Agent Routes Authorization', () => {
  ...
  describe('SIGNAL_ROUTES', () => {
    ...
    describe('suspended tool-call data exposure', () => {
      const suspendedRun = {
        runId: 'run-suspended',
        status: 'suspended',
        threadId: 'thread-123',
        ...
        suspendedAt: new Date('2026-01-15T12:00:00.000Z'),
        toolCalls: [
          {
            toolCallId: 'tool-call-1',
      ...
      const redactedSuspendedRun = {
        ...suspendedRun,
        toolCalls: [{ toolCallId: 'tool-call-1', toolName: 'findUserTool', requiresApproval: true }],
      };
      let getServerSpy: ReturnType<typeof vi.spyOn>;

      beforeEach(() => {
        getServerSpy = vi.spyOn(mastra, 'getServer').mockReturnValue({ auth: { authenticateToken: vi.fn() } } as any);
        (mockAgent as any).listRuns = vi.fn(async () => ({ runs: [runningRun, suspendedRun], total: 2 }));
        (mockAgent as any).listSuspendedRuns = vi.fn(async () => ({ runs: [suspendedRun], total: 1 }));
      });
      ...
      it('should redact tool args and suspend payloads from unscoped agent-run listings', async () => {
        const result = await LIST_AGENT_RUNS_ROUTE.handler({
          mastra,
          agentId: 'test-agent',
          requestContext: new RequestContext(),
        ...
        expect(result).toEqual({ runs: [runningRun, redactedSuspendedRun], total: 2 });
      });

      it('should redact tool args and suspend payloads from unscoped suspended-run listings', async () => {
        ...
        expect(result).toEqual({ runs: [redactedSuspendedRun], total: 1 });
      });

      it('should not treat a client-supplied resourceId as a trusted scope', async () => {
        const result = await LIST_AGENT_RUNS_ROUTE.handler({
          mastra,
          agentId: 'test-agent',
          requestContext: new RequestContext(),
      ...
      it('should return full tool-call data when the listing is scoped by a server-derived resourceId', async () => {
        const result = await LIST_AGENT_RUNS_ROUTE.handler({
          mastra,
          agentId: 'test-agent',
          requestContext: createContextWithReservedKeys({ resourceId: 'user-b' }),
        } as any);

        expect(result).toEqual({ runs: [runningRun, suspendedRun], total: 2 });
      });

      it('should return full tool-call data when the listing is scoped by an authorized thread', async () => {
        await mockMemory.createThread({ threadId: 'thread-123', resourceId: 'user-b', title: 'Thread 123' });
        ...
        expect(result).toEqual({ runs: [suspendedRun], total: 1 });
      });

      it('should return full tool-call data when the server has no auth configured', async () => {
        getServerSpy.mockReturnValue(undefined as any);
