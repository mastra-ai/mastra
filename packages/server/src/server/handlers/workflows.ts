...
// ============================================================================
// Exported Handler Functions (used by agent-builder and route handlers)
// ============================================================================

export async function listWorkflowsHandler({ mastra }: Pick<WorkflowContext, 'mastra'>) {
...
export async function getWorkflowByIdHandler({
  mastra,
  workflowId,
}: Pick<WorkflowContext, 'mastra' | 'workflowId'>): Promise<WorkflowInfo> {
  if (!workflowId) {
    throw new HTTPException(400, { message: 'Workflow ID is required' });
  }
...
export const streamVNextWorkflowHandler = streamWorkflowHandler;
...
export const observeStreamVNextWorkflowHandler = observeStreamWorkflowHandler;
...
export async function listWorkflowRunsHandler({
  ...
  offset,
  resourceId,
}: Pick<WorkflowContext, 'mastra' | 'workflowId'> & {
  ...
  if (
    ...
    throw new HTTPException(400, { message: 'limit must be a positive integer or false' });
  }
...
export const LIST_WORKFLOWS_ROUTE = createRoute({
  ...
  handler: async ({ mastra }) => {
    try {
      const result = await listWorkflowsHandler({ mastra });
...
export const LIST_WORKFLOW_RUNS_ROUTE = createRoute({
  ...
  tags: ['Workflows'],
  handler: async ({ mastra, workflowId, fromDate, toDate, limit, offset, resourceId }) => {
    try {
      const result = await listWorkflowRunsHandler({ mastra, workflowId, fromDate, toDate, limit, offset, resourceId });
