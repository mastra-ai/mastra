...
describe('QUERY_TRACES', () => {
  ...
  it('distinguishes malformed cursors from changed-query conflicts', async () => {
    ...
    expect(await malformed.getResponse().json()).toMatchObject({ code: 'TRACE_QUERY_CURSOR_MALFORMED' });
    ...
    expect(await conflict.getResponse().json()).toMatchObject({ code: 'TRACE_QUERY_CURSOR_CONFLICT' });
  ...
  it('publishes strict runtime and OpenAPI schemas with the observability read permission', () => {
    ...
    expect(Object.keys(QUERY_TRACES.openapi?.responses ?? {})).toEqual(['200', '400', '409', '422', '501']);
    ...
    expect(validation).toMatchObject({
      status: 422,
      body: { code: 'TRACE_QUERY_INVALID', issues: [{ code: 'invalid_request' }] },
    });
  });
});
