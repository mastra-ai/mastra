...
export const SCORE_TRACES_ROUTE = createRoute({
  ...
  handler: async ({ mastra, ...params }) => {
    try {
      ...
      const { scorerId, targets } = params;

      const scorer = mastra.getScorerById(scorerId);
      if (!scorer) {
        throw new HTTPException(404, { message: `Scorer '${scorerId}' not found` });
      }
...
export const LIST_LOGS_ROUTE = createRoute({
  ...
  responseType: 'json',
  queryParamSchema: wrapSchemaForQueryParams(
    logsFilterSchema.merge(paginationArgsSchema).merge(logsOrderBySchema).partial(),
  ),
...
export const LIST_SCORES_ROUTE = createRoute({
  ...
  responseType: 'json',
  queryParamSchema: wrapSchemaForQueryParams(
    scoresFilterSchema.merge(paginationArgsSchema).merge(scoresOrderBySchema).partial(),
  ),
...
export const CREATE_SCORE_ROUTE = createRoute({
  ...
  path: '/observability/scores',
  responseType: 'json',
  bodySchema: z.object({ score: createScoreRecordSchema }),
  ...
  handler: async ({ mastra, score }) => {
    try {
      const observabilityStore = await getObservabilityStore(mastra);
      await observabilityStore.createScore({ score });
      return { success: true };
    } catch (error) {
...
export const LIST_FEEDBACK_ROUTE = createRoute({
  ...
  responseType: 'json',
  queryParamSchema: wrapSchemaForQueryParams(
    feedbackFilterSchema.merge(paginationArgsSchema).merge(feedbackOrderBySchema).partial(),
  ),
...
export const CREATE_FEEDBACK_ROUTE = createRoute({
  ...
  path: '/observability/feedback',
  responseType: 'json',
  bodySchema: z.object({ feedback: createFeedbackRecordSchema }),
  ...
  handler: async ({ mastra, feedback }) => {
    try {
      const observabilityStore = await getObservabilityStore(mastra);
      await observabilityStore.createFeedback({ feedback });
      return { success: true };
    } catch (error) {
