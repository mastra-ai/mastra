...
async function formatAgentList({
  ...
  const tools = await agent.getTools({ requestContext });
  const llm = await agent.getLLM({ requestContext });
  const defaultGenerateOptions = await agent.getDefaultGenerateOptions({ requestContext });
  const defaultStreamOptions = await agent.getDefaultStreamOptions({ requestContext });
  ...
  if ('getWorkflows' in agent) {
    ...
    try {
      const workflows = await agent.getWorkflows({ requestContext });
...
async function formatAgent({
  ...
  const llm = await agent.getLLM({ requestContext });
  const defaultGenerateOptions = await agent.getDefaultGenerateOptions({ requestContext: proxyRequestContext });
  const defaultStreamOptions = await agent.getDefaultStreamOptions({ requestContext: proxyRequestContext });
...
export async function getEvalsByAgentIdHandler({
  ...
  requestContext,
  agentId,
}: Context & { requestContext: RequestContext; agentId: string }) {
  try {
    const agent = await getAgentFromSystem({ mastra, agentId });
