...
interface ResumeCall {
  resumeData: unknown;
  options: { runId?: string; toolCallId?: string; abortSignal?: AbortSignal; model?: string };
}
...
describe('Session — suspend capture on message()', () => {
  ...
  it('forwards the captured model when resuming a suspended run', async () => {
    const { harness, agent } = setup();
    agent.enqueueRun({
      finishReason: 'suspended',
      runId: 'run-model',
      suspendPayload: { toolCallId: 'tc-model', toolName: 'shell', args: { cmd: 'echo ok' } },
    });
    agent.enqueueRun({ finishReason: 'stop', text: 'approved' });
    const session = await harness.session({ resourceId: 'u', threadId: { fresh: true }, modelId: 'gpt-5' });

    await session.message({ content: 'do it' });
    await session.respondToToolApproval({ toolCallId: 'tc-model', approved: true });

    expect(agent.resumeCalls[0]!.options.model).toBe('gpt-5');
  });
