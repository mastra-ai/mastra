...
describe('harness.threads — CRUD', () => {
  ...
  it('rejects a duplicate caller-supplied threadId for the same resource', async () => {
    const { harness } = setupHarness();
    await harness.threads.create({
      resourceId: 'r1',
      threadId: 'thread-explicit-duplicate',
      title: 'original',
      metadata: { color: 'red' },
    });
    ...
    await expect(
      harness.threads.get({ resourceId: 'r1', threadId: 'thread-explicit-duplicate' }),
