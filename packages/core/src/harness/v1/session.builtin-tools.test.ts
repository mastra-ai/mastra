...
describe('builtin askUser → Harness round-trip', () => {
  it('produces a suspend payload that Session classifies as "question"', async () => {
    ...
    // The askUser tool registers the question through requestContext, then
    // suspends with an empty payload. The harness classifier reads the tool
    // name + model-surfaced args from the agent output below.
