...
import { HarnessEventSerializationError } from './errors';
import { EventEmitter, assertJsonSerializable, parseHarnessEventId } from './events';
...
describe('EventEmitter', () => {
  it('uses a delivery snapshot when subscribers unsubscribe during dispatch', () => {
    ...
    offSecond = emitter.subscribe(() => {
      seen.push('second');
    });

    emitter.emit({ type: 'org.test', payload: { ok: true } });

    expect(seen).toEqual(['first', 'second']);
  });

  it('rejects non-finite numbers as non-JSON event payloads', () => {
    for (const value of [Number.NaN, Infinity, -Infinity]) {
      try {
        assertJsonSerializable('org.test', 'session-1', { payload: { value } });
        throw new Error('expected serialization error');
      } catch (err) {
        expect(err).toBeInstanceOf(HarnessEventSerializationError);
        expect(err).toMatchObject({
          eventType: 'org.test',
          path: 'event.payload.value',
