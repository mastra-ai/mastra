...
function agentEndReason(full: FullOutput<unknown>): AgentEndEvent['reason'] {
...
export class Session {
  ...
  private _currentTurnCallerAbort?: { signal: AbortSignal; listener: () => void };
  /**
  ...
  private _beginTurn(callerSignal: AbortSignal | undefined): AbortController {
    ...
    if (callerSignal) {
      if (callerSignal.aborted) {
        ...
        const listener = () => controller.abort((callerSignal as { reason?: unknown }).reason);
        this._currentTurnCallerAbort = { signal: callerSignal, listener };
  ...
  private _endTurn(controller: AbortController): void {
    if (this._currentTurnAbortController === controller) {
      this._cleanupCurrentTurnCallerAbort();
  ...
  private _cleanupCurrentTurnCallerAbort(): void {
    const current = this._currentTurnCallerAbort;
    if (!current) return;
    current.signal.removeEventListener('abort', current.listener);
    this._currentTurnCallerAbort = undefined;
  }
  ...
  private _recordTurnCompletion(full: FullOutput<unknown>): void {
    ...
    if (usage && typeof usage === 'object') {
      ...
      if (typeof u.totalTokens === 'number') {
        ...
        this._tokenUsage.totalTokens += prompt + completion;
      }
    }
  }
  ...
  getQueueDepth(): number {
    return this._visibleQueueDepth();
  }

  private _visibleQueueDepth(): number {
    const queue = this._record.pendingQueue ?? [];
    const activeHead =
      this._currentQueuedItemId !== undefined && queue.some(item => item.id === this._currentQueuedItemId) ? 1 : 0;
    return Math.max(0, queue.length - activeHead);
  }
  ...
  async message(opts: MessageOptions): Promise<AgentResult | AgentStream | unknown> {
    ...
    if (opts.output !== undefined && opts.sync === true && this.isRunning()) {
      const err = new HarnessConfigError(
        'message().output',
        'cannot run structured output while another turn is active',
      );
    ...
    if (opts.output === undefined && this.isRunning()) {
      const subscriptionWaiter = this._createActiveTurnWaiter();
      void subscriptionWaiter.promise.catch(() => {});
      try {
        const sub = await Promise.race([this._ensureThreadSubscription(agent), subscriptionWaiter.promise]);
        this._assertLive('message()');
        activeRunId = sub.activeRunId() ?? this._currentRunId ?? null;
      } catch (err) {
        admissionStart?.reject(err);
        if (opts.admissionId !== undefined) this._messageAdmissionStarts.delete(opts.admissionId);
        throw err;
    ...
    if (activeRunId !== null) {
      ...
      if (effectiveModelId !== this._record.modelId) {
        const err = new HarnessOverrideConflictError(
          this.id,
          'model',
        ...
        try {
          ...
          if (!reservation.created) {
            ...
            if (existing) {
              ...
              return this._returnDuplicateMessageResult(existing, opts);
            }
      ...
      if (admissionStart !== undefined && admissionIdentity !== undefined && admissionHash !== undefined) {
        const now = Date.now();
      ...
      if (opts.stream === true) {
        const output = agent.getRunOutput(signal.runId) as AgentStream | undefined;
        if (output) return output;
        throw new HarnessValidationError('message()', 'active message stream is no longer live');
      }
      ...
      try {
        ...
        if (admissionIdentity !== undefined && admissionHash !== undefined) {
          await this._writeMessageResultEvidenceBestEffort({
            ...
            signalId: signal.signal.id,
            runId: signal.runId,
            result: cloneForStorageEvidence(full),
            admissionId: opts.admissionId!,
            admissionHash,
          });
        }
    ...
    this._currentRunId = signal.runId;
    ...
    let streamStarted = agent.getRunOutput(signal.runId) !== undefined;
            ...
            if (runId && this._completedRuns.has(runId)) {
              ...
              if (cached?.ok && evidence.admissionId !== undefined && evidence.admissionHash !== undefined) {
                await this._writeMessageResultEvidenceBestEffort({
                  ...
                  runId,
                  result: cloneForStorageEvidence(cached.full),
    ...
    if (!willInterleave) {
      ...
      try {
        ...
        const baseExecOptions: AgentExecutionOptionsBase<unknown> = {
          ...
          abortSignal: turnAbortSignal,
          requestContext,
          ...(this._record.modelId ? { model: this._record.modelId as never } : {}),
          ...(toolsets ? { toolsets } : {}),
          ...(mode.instructions ? { instructions: mode.instructions } : {}),
        };
  ...
  async getState<TState = unknown>(): Promise<TState> {
    ...
    return immutableSnapshot((this._record.state ?? {}) as TState);
  }
  ...
  getDisplayState(): SessionDisplayState {
    ...
    const snapshot: SessionDisplayState = {
      ...
      // Queue
      queueDepth: this._visibleQueueDepth(),
    };
  ...
  async updateJudgeDefaults(opts: { judgeModelId?: string; maxTurns?: number }): Promise<GoalState | undefined> {
    ...
    if (opts.judgeModelId !== undefined && opts.judgeModelId.trim().length === 0) {
      throw new HarnessValidationError('judgeModelId', 'judgeModelId must be a non-empty string');
    }
  ...
  private async _runGoalJudge(turn: FullOutput<unknown>, wasGoalDriven: boolean): Promise<void> {
    ...
    let updated: GoalState | undefined;
    await this._flushUpdate(prev => {
      const current = prev.goal;
      if (!current || current.id !== evaluatedGoalId || current.status !== 'active') return prev;
      const turnsUsed = decision.decision === 'waiting' ? current.turnsUsed : current.turnsUsed + 1;
      updated = { ...current, turnsUsed, lastDecision: decision };
    ...
    if (decision.decision === 'done') {
      ...
      this._emit({ type: 'goal_done', goalId: evaluatedGoalId, reason: decision.reason, turnsUsed: updated.turnsUsed });
      return;
    }
  ...
  private async _callJudge(goal: GoalState, turn: FullOutput<unknown>): Promise<GoalJudgeDecision> {
    ...
    const judgeThreadId = this._judgeThreadId(goal);
  ...
  /** @internal — deterministic memory thread id used by the goal judge. */
  _judgeThreadId(goal: Pick<GoalState, 'id'>): string {
    return `${this._record.id}-${goal.id}`;
  }
  ...
  private async _resume(
    ...
    try {
      ...
      const resumeStream = agent.resumeStream(resumeData, {
        ...
        ...(resumeRuntimeDependencies.modelId ? { model: resumeRuntimeDependencies.modelId as never } : {}),
      });
      ...
      try {
        await Promise.race([
          this._rollbackAcceptedResumeAfterFailure({
            responseId,
            pending,
            pendingQueuedItemId,
            resumedAt,
  ...
  private async _rollbackAcceptedResumeAfterFailure(input: {
    responseId?: string;
    pending: PendingResume;
    ...
    await this._flushUpdate(prev => {
      const next: SessionRecord = { ...prev };
      const receipt =
        input.responseId !== undefined ? getOwnRecordValue(prev.inboxResponseReceipts, input.responseId) : undefined;
      if (receipt && receipt.status !== 'applied' && receipt.status !== 'failed' && receipt.status !== 'dead') {
        next.inboxResponseReceipts = {
          ...(prev.inboxResponseReceipts ?? {}),
          [receipt.responseId]: {
            ...receipt,
            status: 'failed',
            error: projectHarnessPublicError(input.err),
            retryable: false,
            failedAt: receipt.failedAt ?? failedAt,
            updatedAt: failedAt,
          },
      ...
      const current = prev.pendingResume;
      const currentItemId = current ? (current.itemId ?? current.toolCallId) : undefined;
      const pendingItemId = input.pending.itemId ?? input.pending.toolCallId;
      const matchesPending =
        current !== undefined &&
        current.runId === input.pending.runId &&
        current.toolCallId === input.pending.toolCallId &&
        currentItemId === pendingItemId &&
        current.resumedAt === input.resumedAt;
      if (!matchesPending) return next;

      delete next.pendingResume;
      if (input.pendingQueuedItemId !== undefined) {
        queueFailed = true;
        next.pendingQueue = (prev.pendingQueue ?? []).filter(item => item.id !== input.pendingQueuedItemId);
        const queueReceipt = prev.queueAdmissionReceipts?.[input.pendingQueuedItemId];
        if (queueReceipt && queueReceipt.status !== 'completed') {
          next.queueAdmissionReceipts = {
            ...(prev.queueAdmissionReceipts ?? {}),
            [input.pendingQueuedItemId]: {
              ...queueReceipt,
              status: 'failed',
              error: projectHarnessPublicError(input.err),
              failedAt: queueReceipt.failedAt ?? failedAt,
              updatedAt: failedAt,
            },
          };
        }
    ...
    if (!queueFailed || input.pendingQueuedItemId === undefined) return;
    this._currentQueuedItemId = undefined;
    this._currentQueuedItemSource = undefined;
    const resolver = this._queueResolvers.get(input.pendingQueuedItemId);
    if (resolver) {
      this._queueResolvers.delete(input.pendingQueuedItemId);
      resolver.reject(input.err);
    }
    this._notifyMaybeIdle();
    void this._maybeDrainQueue();
  }
  ...
  private async _runQueuedTurn(item: QueuedItem): Promise<FullOutput<unknown>> {
    ...
    try {
      const effectiveModelId = this._modelIdForQueuedItem(item.id);
      const requestContext = await Promise.race([
        this._buildRequestContext({
          modeId: effectiveModeId,
          modelId: effectiveModelId,
                ...
                [itemId]:
                  receipt.status === 'completed'
                    ...
                    : {
                        ...receipt,
                        status: 'completed',
                        result: cloneForStorageEvidence(result),
                        completedAt: receipt.completedAt ?? now,
                        updatedAt: now,
                      },
              },
  ...
  private async _buildRequestContext(turn: {
    ...
    const session = this;
    const stateSnapshot = immutableSnapshot((this._record.state ?? {}) as unknown);
    ...
    const harnessSlot: HarnessRequestContext<unknown> = {
      ...
      state: stateSnapshot,
      getState: () => immutableSnapshot((session._record.state ?? {}) as unknown),
...
function immutableSnapshot<T>(value: T): T {
  return deepFreezeValue(cloneForStorageEvidence(value));
}

function cloneForStorageEvidence<T>(value: T): T {
  try {
    return structuredClone(value);
  } catch {
    return makeStorageCloneable(value, new WeakMap<object, unknown>()) as T;
  }
}

function makeStorageCloneable(value: unknown, seen: WeakMap<object, unknown>): unknown {
  ...
  if (Array.isArray(value)) {
    ...
    seen.set(value, out);
    for (let index = 0; index < value.length; index += 1) {
      out[index] = makeStorageCloneable(value[index], seen);
    }
    return out;
  }
  if (value instanceof Map) {
    ...
    for (const [key, entry] of value.entries()) {
      out.push([makeStorageCloneable(key, seen), makeStorageCloneable(entry, seen)]);
    }
    return out;
  }
  if (value instanceof Set) {
    ...
    for (const entry of value.values()) {
      out.push(makeStorageCloneable(entry, seen));
    }
    return out;
  }
  if (typeof value === 'object') {
    ...
    for (const [key, entry] of Object.entries(value as Record<string, unknown>)) {
      const cloned = makeStorageCloneable(entry, seen);
...
function deepFreezeValue<T>(value: T, seen = new WeakSet<object>()): T {
  ...
  for (const key of Reflect.ownKeys(objectValue)) {
    deepFreezeValue((objectValue as Record<PropertyKey, unknown>)[key], seen);
  }
