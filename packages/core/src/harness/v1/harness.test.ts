...
describe('Harness v1 — construction', () => {
  ...
  it('clamps outbox claim renewal when dispatch lowers the claim ttl below channel config', async () => {
    vi.useFakeTimers();
    ...
    try {
      ...
      let resolveDelivery!: () => void;
      const delivery = new Promise<{ providerMessageId: string }>(resolve => {
        resolveDelivery = () => resolve({ providerMessageId: 'provider-slow-clamped' });
      });
      const harness = new Harness({
        modes: [{ id: 'default', agentId: 'default' }],
        defaultModeId: 'default',
      ...
      await expect(
        harness.channels.dispatchOutbox({ channelId: 'support', claimId: 'claim-clamped-2', claimTtlMs: 100 }),
      ).resolves.toMatchObject({ claimed: 0, sent: 0, failed: 0, dead: 0 });
  ...
  it('does not call the outbox provider or mutate after pre-delivery claim ownership is lost', async () => {
    ...
    const renewSpy = vi.spyOn(storage, 'renewChannelOutboxClaim').mockImplementationOnce(async input => {
      const claimedEntry = [...db.harnessChannelOutbox.entries()].find(([, item]) => item.id === outboxItemId);
      const claimed = claimedEntry?.[1];
      expect(claimed).toMatchObject({ status: 'claimed', claimId: 'claim-lost' });
      db.harnessChannelOutbox.set(claimedEntry![0], {
        ...claimed!,
    ...
    const retainedByCompetingClaim = [...db.harnessChannelOutbox.values()].find(item => item.id === outboxItemId);
  ...
  it('throws HarnessConfigError for non-integer positive configuration caps', () => {
    expect(
      () =>
        new Harness({
          agents: { default: makeAgent() },
          modes: [{ id: 'default', agentId: 'default' }],
          defaultModeId: 'default',
          sessions: { storage: makeStorage(), maxQueueDepth: 1.5 },
        }),
    ).toThrow(HarnessConfigError);
...
describe('Harness v1 — lease + write concurrency', () => {
  ...
  it('evicts stale session handles during shutdown', async () => {
    const harness = makeHarness();
    const session = await harness.session({ threadId: 't1', resourceId: 'r1' });

    await harness.shutdown();

    await expect(session.setState({ stale: true })).rejects.toThrow(HarnessSessionClosedError);
    await expect(session.message({ content: 'stale' })).rejects.toThrow(HarnessSessionClosedError);
  });
