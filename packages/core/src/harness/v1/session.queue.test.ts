 ...
 *     and persist on the queued item
...
describe('Session.queue() — FIFO + per-turn overrides', () => {
  ...
  it('forwards queued model overrides to the agent execution options', async () => {
    const { harness, agent } = setupHarness();
    agent.enqueueRun({ finishReason: 'stop', text: 'queued reply' });
    const session = await harness.session({ resourceId: 'u', threadId: { fresh: true }, modelId: 'default-model' });

    await session.queue({ content: 'go', model: 'queued-model' });

    expect(agent.streamCalls[0]!.options.model).toBe('queued-model');
    await session.close();
  });
});
