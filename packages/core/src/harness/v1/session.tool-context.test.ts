...
describe('HarnessRequestContext — state reads/writes', () => {
  ...
  it('returns immutable state snapshots from session and tool context reads', async () => {
    const { harness, agent } = setupHarness();
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true } });
    await session.setState<{ nested: { count: number } }>({ nested: { count: 1 } });
    await session.message({ content: 'hi' });

    const direct = await session.getState<{ nested: { count: number } }>();
    const slot = getHarnessSlot(agent.streamCalls);

    expect(Object.isFrozen(direct)).toBe(true);
    expect(Object.isFrozen(direct.nested)).toBe(true);
    expect(Object.isFrozen(slot.state)).toBe(true);
    expect(Object.isFrozen(slot.getState())).toBe(true);
  });
...
describe('HarnessRequestContext — abort plumbing', () => {
  it('aborts the slot signal when the caller-supplied signal aborts', async () => {
    ...
    agent.enqueueRun({ finishReason: 'stop', text: 'held', holdUntil });
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true } });
    const controller = new AbortController();
    const message = session.message({ content: 'hi', abortSignal: controller.signal });
    await vi.waitFor(() => {
