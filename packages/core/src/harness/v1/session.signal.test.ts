...
describe('Session.signal()', () => {
  ...
  it('idle dispatch wakes a fresh run and the stream call carries that runId', async () => {
    ...
    const message = Array.isArray(call.messages)
      ? (call.messages.at(-1) as { __isCreatedSignal?: boolean; type?: string; contents?: unknown })
      : (call.messages as { __isCreatedSignal?: boolean; type?: string; contents?: unknown });
    expect(message.__isCreatedSignal).toBe(true);
    expect(message.type).toBe('user-message');
    expect(message.contents).toBe('hi');
  });

  it('idle dispatch forwards the selected session model to the agent execution options', async () => {
    const { harness, agent } = setupHarness();
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true } });
    ...
    expect(agent.streamCalls[0]!.options.model).toBe('gpt-5');
  });
