...
describe('Session.getDisplayState — shape', () => {
  ...
  it('treats missing legacy pendingQueue as an empty queue', async () => {
    const { harness } = setupHarness();
    const session = await harness.session({ resourceId: 'u', threadId: { fresh: true } });
    delete (session.getRecord() as unknown as { pendingQueue?: unknown }).pendingQueue;

    expect(session.getDisplayState().queueDepth).toBe(0);
  });
});
