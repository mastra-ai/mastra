...
function freezeChannelConfig(entry: HarnessChannelConfig): HarnessChannelConfig {
  const clone = {
    ...entry,
    adapter: { ...entry.adapter },
    ingress: { ...entry.ingress },
    ...(entry.inbox ? { inbox: { ...entry.inbox } } : {}),
    ...(entry.actions ? { actions: { ...entry.actions } } : {}),
    ...(entry.outbox ? { outbox: { ...entry.outbox } } : {}),
  };
  return deepFreeze(clone) as HarnessChannelConfig;
}
...
export class HarnessChannelRegistry {
  ...
  constructor(config: Record<string, HarnessChannelConfig> | undefined) {
    ...
    for (const [channelId, entry] of Object.entries(config)) {
      ...
      const config = freezeChannelConfig(entry);
      this.pending.set(channelId, { channelId, providerId, platform: entry.platform, config });
    }
  }
