...
export class Harness {
  ...
  constructor(config: HarnessConfig) {
    ...
    if (config.subagents) {
      ...
      if (!Number.isInteger(this._subagentMaxDepth) || this._subagentMaxDepth < 1) {
    ...
    if (
      goalsCfg?.defaultMaxTurns !== undefined &&
      (!Number.isInteger(goalsCfg.defaultMaxTurns) || goalsCfg.defaultMaxTurns < 1)
    ) {
  ...
  private async _dispatchClaimedChannelOutboxItem(
    ...
    const configuredClaimRenewMs = config?.outbox?.claimRenewMs ?? Math.max(1, Math.floor(claimTtlMs / 2));
    const claimRenewMs =
      configuredClaimRenewMs >= claimTtlMs && claimTtlMs > 1
        ? Math.max(1, Math.floor(claimTtlMs / 2))
        : configuredClaimRenewMs;
  ...
  threads = {
    create: async (opts: ThreadCreateOptions): Promise<ThreadRecord> => {
      ...
      const saveThreadIfVisibleOwner = async (fence?: { assertActive(): Promise<void> }) => {
        ...
        if (existing && existing.metadata?.[EXTERNAL_SESSION_STORAGE_OWNER_METADATA_KEY] !== true) {
          throw new HarnessConfigError(
            'threads.create().threadId',
            `thread "${threadId}" already exists for resource "${opts.resourceId}"; use threads.selectOrCreate() for idempotent selection`,
          );
        }
