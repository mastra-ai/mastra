...
describe('Session.message() — default path', () => {
  ...
  it('forwards the selected model to the agent execution options', async () => {
    const { harness, agent } = setup();
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true } });
    ...
    expect(agent.calls[0]!.options.model).toBe('gpt-5');
  });

  it('forwards a per-turn model override to the agent execution options', async () => {
    const { harness, agent } = setup();
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true }, modelId: 'gpt-5' });

    await session.message({ content: 'hi', model: 'gpt-5-mini' });

    expect(agent.calls[0]!.options.model).toBe('gpt-5-mini');
  });
  ...
  it('aborting the caller signal aborts the per-turn signal handed to the agent', async () => {
    ...
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true } });
    const ac = new AbortController();
    const message = session.message({ content: 'hi', abortSignal: ac.signal });
    await vi.waitFor(() => {
      expect(agent.calls).toHaveLength(1);
    });
    const turnSignal = agent.calls[0]!.options.abortSignal as AbortSignal;
  ...
  it('removes the caller abort listener when the turn completes normally', async () => {
    const { harness } = setup();
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true } });
    const ac = new AbortController();
    const removeListener = vi.spyOn(ac.signal, 'removeEventListener');
  ...
  it('does not take ownership of turn state for active-delivery messages', async () => {
    const { harness, agent } = setup();
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true } });
    const activeOutput = buildFakeOutput({
      runId: 'active-run',
      fullOutput: { ...agent.fullOutput, runId: 'active-run' },
    });
    (agent as any)._internalRegisterStreamRun(activeOutput, {});
    const ownerController = new AbortController();
    (session as any)._currentRunId = 'active-run';
    (session as any)._currentTurnAbortController = ownerController;

    const stream = await session.message({ content: 'second', stream: true });

    expect((stream as any).runId).toBeTypeOf('string');
    expect((session as any)._currentTurnAbortController).toBe(ownerController);
    expect(session.isRunning()).toBe(true);
  });

  it('rejects ignored per-turn overrides for active-delivery messages', async () => {
    const agent = new LiveStreamFakeAgent('default');
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    const harness = new Harness({
      agents: { default: agent } as any,
    ...
    await expect(
      session.message({ content: 'second', additionalTools: { extra: { id: 'extra' } as any } }),
  ...
  it('stores serializable admission evidence when the live agent result carries non-cloneable fields', async () => {
    const { harness, agent } = setup();
    agent.fullOutput = {
      ...agent.fullOutput,
      toolResults: [{ toolName: 'debug', result: { value: 1, helper: () => 'not persisted' } }],
    };
    const session = await harness.session({ resourceId: 'u1', threadId: { fresh: true } });

    const first = await session.message({ content: 'hi', admissionId: 'admission-noncloneable' });
    const second = await session.message({ content: 'hi', admissionId: 'admission-noncloneable' });

    expect((first.toolResults[0] as any).result.helper()).toBe('not persisted');
    expect(((second.toolResults[0] as any).result as Record<string, unknown>).helper).toBeUndefined();
    expect(second.text).toBe('hello back');
    expect(agent.calls).toHaveLength(1);
  });
