...
interface StubWorkspace {
  id: string;
  name: string;
  status: 'pending' | 'not-initialized' | 'initializing' | 'ready' | 'destroyed' | 'error';
...
describe('HarnessRequestContext.workspace — runtime plumbing', () => {
  it('does not initialize an already-ready shared workspace again', async () => {
    const ws = makeWorkspace('shared-ready');
    ...
    expect((ws as unknown as StubWorkspace).init).not.toHaveBeenCalled();
  });

  it('destroys a shared workspace that resolves while shutdown is waiting', async () => {
    const ws = makeWorkspace('shared-pending');
    let resolveWorkspace!: (workspace: Workspace) => void;
    const { harness } = setupHarness({
      workspace: {
        kind: 'shared',
        workspace: () =>
    ...
    resolveWorkspace(ws);
    await Promise.all([acquiring, shutdown]);

    expect((ws as unknown as StubWorkspace).destroy).toHaveBeenCalledTimes(1);
  });
...
describe('Workspace lifecycle on harness.shutdown()', () => {
  ...
  it('shared: shutdown() destroys an eager workspace even when no session storage is bound', async () => {
    const ws = makeWorkspace('shared') as unknown as StubWorkspace;
    const mastra = new Mastra({ agents: { default: makeAgent('default') } });
    const harness = new Harness({
      mastra,
      modes: [{ id: 'default', agentId: 'default' }],
      defaultModeId: 'default',
      workspace: { kind: 'shared', workspace: ws as unknown as Workspace, eager: true },
    });
...
describe('Provider failures surface as HarnessWorkspaceProvisioningError', () => {
  ...
  it('destroys a per-resource provider workspace when initialization fails', async () => {
    const ws = makeWorkspace('per-resource-init-failure') as unknown as StubWorkspace;
    ws.status = 'not-initialized';
    ws.init = vi.fn(async () => {
      ws.status = 'error';
    ...
    const provider = {
      providerId: 'broken-init',
      resumable: false,
      create: async () => ws as unknown as Workspace,
      destroy,
    ...
    expect(ws.init).toHaveBeenCalledTimes(1);
    expect(destroy).toHaveBeenCalledTimes(1);
  });

  it('destroys a per-session provider workspace when initialization fails', async () => {
    const ws = makeWorkspace('per-session-init-failure') as unknown as StubWorkspace;
    ws.status = 'not-initialized';
    ws.init = vi.fn(async () => {
      ws.status = 'error';
      throw new Error('init boom');
