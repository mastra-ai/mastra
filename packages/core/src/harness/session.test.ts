  ...
  it('emits permission approval events and resolves pending tool approvals', async () => {
    ...
    const result = await sendTarget.ifIdle.streamOptions.toolsets.harness.guarded!.execute(
      { command: 'ship' },
      {
        ...
        workspace: undefined,
        agent: { toolCallId: 'tool-call-1', suspend },
      } as never,
    );
    ...
    expect(suspend).toHaveBeenCalledWith(
      expect.objectContaining({
        pendingItemId: expect.any(String),
        permission: expect.objectContaining({ decision: 'pendingApproval' }),
      }),
      expect.objectContaining({ requireToolApproval: true }),
    );
    const pendingItemId = (suspend.mock.calls[0]![0] as { pendingItemId: string }).pendingItemId;
...
describe('Harness.ownerId', () => {
  ...
  it('exposes message and queue APIs', async () => {
    const { harness } = createHarness(createMemory());
    const session = await harness.session({ threadId: 'thread-1', resourceId: 'resource-1' });

    expect(typeof session.sendMessage).toBe('function');
    expect(typeof session.queueMessage).toBe('function');
  });
