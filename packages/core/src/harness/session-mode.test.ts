/**
 * Session harness — listModes() / getMode() coverage.
 *
...
describe('Harness.listModes()', () => {
  it('returns every registered mode in declaration order', () => {
    const modes: HarnessMode[] = [
      { id: 'build', defaultModelId: 'test-model' },
      { id: 'plan', defaultModelId: 'test-model' },
      { id: 'fast', defaultModelId: 'test-model' },
    ];
...
describe('Harness.getMode()', () => {
  it('returns the mode object for a known id', () => {
    ...
    expect(harness.getMode('plan')).toMatchObject({ id: 'plan', defaultModelId: 'test-model' });
  });
