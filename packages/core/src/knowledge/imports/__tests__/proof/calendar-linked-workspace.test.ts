...
describe(`calendar importer linked-workspace proof (${adapter})`, () => {
  it('refuses an existing workspace without altering its files', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'knowledge-calendar-existing-'));
    temporaryDirectories.push(directory);
    const workspace = join(directory, 'workspace');
    await mkdir(workspace);
    const sentinel = join(workspace, 'sentinel.txt');
    await writeFile(sentinel, 'preserve existing content');
    await expect(run('pnpm', ['exec', 'tsx', creator, '--out', workspace], root)).rejects.toThrow('EEXIST');
    expect(await readFile(sentinel, 'utf8')).toBe('preserve existing content');
  });
