...
export class AISDKV5OutputStream {
    ...
    get response() {
        return transformResponse({ response: this.#modelOutput.response, isMessages: true, runId: this.#modelOutput.runId });
    }
    ...
    get content() {
        return transformResponse({ response: this.response, isMessages: true, runId: this.#modelOutput.runId }).messages?.flatMap((message: any) => message.content) ?? [];
    }
