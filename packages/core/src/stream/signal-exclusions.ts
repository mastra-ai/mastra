...
export function isSignalChunkExcluded(chunk: unknown, excludeSignals: readonly AgentSignalType[] | undefined): boolean {
  if (!excludeSignals?.length || !chunk || typeof chunk !== 'object') return false;
  ...
  return type !== undefined && excludeSignals.some(excluded => canonicalSignalType(excluded) === type);
}
