...
for (const engine of ['legacy', 'workflow'] as const) {
  describe(`BackgroundTaskManager [engine=${engine}]`, () => {
    ...
    describe('concurrency', () => {
      ...
      (engine === 'workflow' ? it.skip : it)('backpressure reject throws on limit', async () => {
      ...
      (engine === 'workflow' ? it.skip : it)('backpressure fallback-sync returns signal', async () => {
        const isolatedPubsub = new EventEmitterPubSub();
    ...
    describe('retry', () => {
      (engine === 'workflow' ? it.skip : it)('retries a failed task up to maxRetries', async () => {
        let callCount = 0;
      ...
      (engine === 'workflow' ? it.skip : it)('fails after exhausting retries', async () => {
    ...
    describe('callbacks', () => {
      (engine === 'workflow' ? it.skip : it)('invokes onTaskComplete callback', async () => {
        const onComplete = vi.fn();
        const isolatedPubsub = new EventEmitterPubSub();
        const mgr = new BackgroundTaskManager({ enabled: true, engine, onTaskComplete: onComplete });
      ...
      (engine === 'workflow' ? it.skip : it)('invokes onTaskFailed callback', async () => {
        const onFailed = vi.fn();
        const isolatedPubsub = new EventEmitterPubSub();
        const mgr = new BackgroundTaskManager({ enabled: true, engine, onTaskFailed: onFailed });
        mgr.__registerMastra(mastra);
        await mgr.init(isolatedPubsub);
    ...
    // Recovery tests target the legacy nack-redelivery + cross-process handoff
    // model. Step 4 of the workflow-engine migration replaces these with
    // workflow-side recovery semantics; for now, skip under the workflow engine.
    (engine === 'workflow' ? describe.skip : describe)('recovery on startup', () => {
      it('fails stale running tasks from a previous process', async () => {
        const isolatedPubsub = new EventEmitterPubSub();

        const mgr1 = new BackgroundTaskManager({ enabled: true, engine: 'legacy' });
        ...
        const mgr2 = new BackgroundTaskManager({ enabled: true, engine: 'legacy' });
    ...
    describe('stream', () => {
      ...
      it('emits every progress output chunk by default', async () => {
        ...
        const executeFn = vi
          .fn()
          .mockImplementation(async (_args: any, opts: { onProgress?: (chunk: any) => void }) => {
      ...
      (engine === 'workflow' ? it.skip : it)(
        'throttles progress output chunks while still emitting completion',
        async () => {
          const isolatedPubsub = new EventEmitterPubSub();
          const mgr = new BackgroundTaskManager({ enabled: true, engine, progressThrottleMs: 100 });
          mgr.__registerMastra(mastra);
          await mgr.init(isolatedPubsub);
