...
describe('Workflow', () => {
  ...
  describe('foreach', () => {
    ...
    it('should not leak data between concurrent workflow runs with foreach', async () => {
      ...
      // Create a CHILD WORKFLOW (not just a step) - this matches the issue reproduction
      ...
      childWorkflow.then(processItemStep).commit();
      ...
      const parentWorkflow = createWorkflow({
        ...
        steps: [prepareItemsStep, childWorkflow, collectResultsStep],
        options: { validateInputs: false },
      });

      // Use nested workflow in foreach - this is the pattern from the issue
      parentWorkflow
        .then(prepareItemsStep)
        .foreach(childWorkflow, { concurrency: 10 })
      ...
      new Mastra({
        logger: false,
        storage: testStorage,
        workflows: { 'parent-workflow': parentWorkflow, 'child-workflow': childWorkflow },
      });
