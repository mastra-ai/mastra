...
describe('Workflow', () => {
  describe('Basic Workflow Execution', () => {
    ...
    it('should execute multiple steps in parallel', async () => {
      const step1Action = vi.fn().mockImplementation(async ({ context: _context }: { context: any }) => {
        return { value: 'step1' };
      });
      const step2Action = vi.fn().mockImplementation(async ({ context: _context }: { context: any }) => {
        return { value: 'step2' };
      });
