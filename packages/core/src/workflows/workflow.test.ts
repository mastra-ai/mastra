...
import type { ChunkType, StepFailure, StreamEvent, WatchEvent, WorkflowStreamEvent } from './types';
...
describe('Workflow', () => {
  ...
  describe('Basic Workflow Execution', () => {
    ...
    it('should update state after each concurrent batch in foreach step', async () => {
      ...
      const run = await workflows.createRunAsync();
      const result = await run.start({
        inputData: [2, 1],
        ...
        .foreach(
          createStep({
            ...
            execute: async ctx => {
              ...
              ctx.setState({ ...ctx.state, output: ctx.inputData });
              return ctx.inputData;
            },
          }),
        )
  ...
  describe('Schema Validation', () => {
    ...
    it('should allow a steps input schema to be a subset of the previous step output schema', async () => {
      ...
      const run2 = await missingRequiredKeyWorkflow.createRunAsync();
      ...
      const run3 = await missingOptionalKeyWorkflow.createRunAsync();
      ...
      const run4 = await extraOptionalKeyWorkflow.createRunAsync();
      ...
      const errorRun = await errWorkflow.createRunAsync();
      ...
      expect((errorResult.steps['extra-required-key-step'] as StepFailure<any, any, any>).error).toContain(
        'Error: Step input validation failed: \n- c: Required',
      );
      ...
      const errorRun2 = await errWorkflow2.createRunAsync();
      ...
      expect((errorResult2.steps['distinct-type-step'] as StepFailure<any, any, any>).error).toContain(
  ...
  describe('Restart', () => {
    ...
    it('should restart a workflow execution that was previously active and has nested workflows', async () => {
      ...
      const run2 = await workflow.createRunAsync({ runId: runId2 });
    ...
    it('should successfully suspend and resume a restarted workflow execution', async () => {
      ...
      const run = await promptEvalWorkflow.createRunAsync({ runId });
    ...
    it('should restart workflow execution for a do-while workflow', async () => {
      ...
      const run = await dowhileWorkflow.createRunAsync({ runId });
    ...
    it('should restart workflow execution for workflow with parallel steps', async () => {
      ...
      const run = await testParallelWorkflow.createRunAsync({ runId });
