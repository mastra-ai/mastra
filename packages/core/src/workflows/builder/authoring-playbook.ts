...
| \`classifier\`  | The complete previous output. Insert a \`mapping\` step first when the classifier needs a reshaped input. | \`{ answers, usage }\`. Route with \`answers.<question>.choice\`, \`answers.<question>.score\`, or \`answers.<question>.probability\`. The complete answer object includes distributions when provided. |
...
- \`list-available-classifiers\` → every configured classifier you may put in \`{ type: "classifier", classifierId }\`. Each row carries the id, configured question metadata, and a routing path for each question. Copy question keys and choice literals exactly. A classifier returns \`answers\` and \`usage\`; use a following \`conditional\` entry to route on \`answers.<question>.choice\`, \`.score\`, or \`.probability\`. There is no classifier-specific switch entry.
