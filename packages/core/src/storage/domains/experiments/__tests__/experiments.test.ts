...
describe('ExperimentsInMemory', () => {
  ...
  describe('delete boolean return', () => {
    it('deleteExperiment returns true when the row was deleted', async () => {
      const created = await storage.createExperiment({
        datasetId: 'ds-1',
        datasetVersion: 1,
        targetType: 'agent',
      ...
      const deleted = await storage.deleteExperiment({ id: created.id });
      expect(deleted).toBe(true);
    });

    it('deleteExperiment returns false when no row matched', async () => {
      const deleted = await storage.deleteExperiment({ id: 'does-not-exist' });
      expect(deleted).toBe(false);
    });

    it('deleteExperiment returns false on tenancy mismatch and preserves the row', async () => {
      const created = await storage.createExperiment({
        datasetId: 'ds-1',
        datasetVersion: 1,
        targetType: 'agent',
      ...
      const deleted = await storage.deleteExperiment({
        id: created.id,
    ...
    it('deleteExperimentResults returns false on parent tenancy mismatch', async () => {
      const experiment = await storage.createExperiment({
        datasetId: 'ds-1',
        datasetVersion: 1,
        targetType: 'agent',
      ...
      const deleted = await storage.deleteExperimentResults({
        experimentId: experiment.id,
        filters: { organizationId: 'org-b', projectId: 'proj-a' },
      });
      expect(deleted).toBe(false);
