...
describe('InMemoryHarness channel inbox ledger', () => {
  ...
  it('rejects same-harness duplicate ids and save-time idempotency collisions', async () => {
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    await storage.createOrLoadChannelInboxItem(sampleChannelInbox());
  ...
  it('isolates inbox ids by harness namespace', async () => {
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    await storage.createOrLoadChannelInboxItem(sampleChannelInbox());
    ...
    await expect(
      storage.saveChannelInboxItem(
        sampleChannelInbox({
          harnessName: 'third',
          channelId: 'third-support',
          idempotencyKey: 'third-provider-event',
          admissionId: 'third-inbox-1',
        }),
      ),
...
describe('InMemoryHarness wakeup ledger', () => {
  ...
  it('isolates wakeup ids by harness namespace', async () => {
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    await storage.createOrLoadHarnessWakeupItem(sampleWakeup());

    await expect(storage.createOrLoadHarnessWakeupItem(sampleWakeup({ harnessName: 'other' }))).resolves.toMatchObject({
      duplicate: false,
      conflict: false,
    });
  });
...
describe('InMemoryHarness channel action ledger', () => {
  ...
  it('isolates action receipt ids by harness namespace', async () => {
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    await storage.createOrLoadChannelActionReceipt(sampleChannelActionReceipt());

    await expect(
      storage.createOrLoadChannelActionReceipt(sampleChannelActionReceipt({ harnessName: 'other' })),
    ).resolves.toMatchObject({
      duplicate: false,
      conflict: false,
    });
...
describe('InMemoryHarness channel outbox ledger', () => {
  ...
  it('isolates outbox ids by harness namespace', async () => {
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    await storage.enqueueChannelOutbox(sampleChannelOutbox());

    await expect(storage.enqueueChannelOutbox(sampleChannelOutbox({ harnessName: 'other' }))).resolves.toEqual({
      outboxItemId: 'outbox-1',
      duplicate: false,
      conflict: false,
    });
