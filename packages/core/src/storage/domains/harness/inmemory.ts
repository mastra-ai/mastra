...
export class InMemoryHarness extends HarnessStorage {
  ...
  async getAttachmentRecord({
    ...
    const record = this.db.harnessAttachmentRecords.get(
  ...
  async saveChannelInboxItem(record: ChannelInboxItem): Promise<void> {
    ...
    const existing = this.findChannelInboxById(namespaced.harnessName, namespaced.id);
    if (existing) {
  ...
  async createOrLoadChannelInboxItem(
    ...
    const existingById = this.findChannelInboxById(namespace, incoming.id);
    if (existingById) {
      throw new HarnessStorageChannelInboxTransitionError(
        incoming.id,
        existingById.status,
  ...
  async renewChannelInboxClaim({
    ...
    const current = this.findChannelInboxById(resolveHarnessName(harnessName, this.harnessName), inboxItemId);
    if (
      !current ||
  ...
  async updateChannelInboxItem(
    record: ChannelInboxItem,
    opts: { claimId: string; harnessName?: string },
  ): Promise<void> {
    const namespace = resolveHarnessName(opts.harnessName ?? record.harnessName, this.harnessName);
  ...
  async saveChannelActionReceipt(record: ChannelActionReceipt): Promise<void> {
    ...
    const existing = this.findChannelActionReceiptById(receipt.harnessName, receipt.id);
    if (existing) {
  ...
  async createOrLoadChannelActionReceipt(
    ...
    const existingById = this.findChannelActionReceiptById(namespace, incoming.id);
    if (existingById) {
      throw new HarnessStorageChannelActionReceiptTransitionError(
        incoming.id,
        existingById.status,
  ...
  async renewChannelActionReceiptClaim({
    ...
    const current = this.findChannelActionReceiptById(resolveHarnessName(harnessName, this.harnessName), receiptId);
    if (
      !current ||
      current.claimId !== claimId ||
      current.claimExpiresAt === undefined ||
  ...
  async updateChannelActionReceipt(
    record: ChannelActionReceipt,
    opts: { claimId: string; harnessName?: string },
  ): Promise<void> {
    const namespace = resolveHarnessName(opts.harnessName ?? record.harnessName, this.harnessName);
  ...
  async enqueueChannelOutbox(record: ChannelOutboxItem): Promise<{
    ...
    const existingById = this.findChannelOutboxById(item.harnessName, item.id);
    if (existingById) {
  ...
  async renewChannelOutboxClaim({
    ...
    const current = this.findChannelOutboxById(resolveHarnessName(harnessName, this.harnessName), outboxItemId);
    if (
      !current ||
  ...
  async markChannelOutboxSent({
    ...
    const current = this.claimedChannelOutboxItem(outboxItemId, claimId, harnessName);
    const storageNow = Date.now();
    const next: ChannelOutboxItem = {
      ...current,
      status: 'sent',
  ...
  async createOrLoadHarnessWakeupItem(
    ...
    const existingById = this.findHarnessWakeupById(namespace, incoming.id);
    if (existingById) {
      throw new HarnessStorageWakeupTransitionError(
        incoming.id,
        existingById.status,
  ...
  async renewHarnessWakeupClaim({
    ...
    const current = this.findHarnessWakeupById(resolveHarnessName(harnessName, this.harnessName), wakeupItemId);
    if (
      !current ||
      current.claimId !== claimId ||
      current.claimExpiresAt === undefined ||
  ...
  private findChannelActionReceiptById(
    harnessName: string | undefined,
    receiptId: string,
  ): ChannelActionReceipt | null {
    if (harnessName !== undefined) {
      const receipt = this.db.harnessChannelActionReceipts.get(channelActionReceiptKey(harnessName, receiptId));
      return receipt ? cloneJson(receipt) : null;
    }
  ...
  private findChannelInboxById(harnessName: string | undefined, inboxItemId: string): ChannelInboxItem | null {
    if (harnessName !== undefined) {
      const item = this.db.harnessChannelInbox.get(channelInboxKey(harnessName, inboxItemId));
      return item ? cloneJson(item) : null;
    }
    for (const item of this.db.harnessChannelInbox.values()) {
  ...
  private findChannelOutboxById(harnessName: string | undefined, outboxItemId: string): ChannelOutboxItem | null {
    if (harnessName !== undefined) {
      const item = this.db.harnessChannelOutbox.get(channelOutboxKey(harnessName, outboxItemId));
      return item ? cloneJson(item) : null;
    }
  ...
  private claimedChannelOutboxItem(outboxItemId: string, claimId: string, harnessName?: string): ChannelOutboxItem {
    const current = this.findChannelOutboxById(resolveHarnessName(harnessName, this.harnessName), outboxItemId);
    const storageNow = Date.now();
    if (
      !current ||
  ...
  private findHarnessWakeupById(harnessName: string | undefined, wakeupItemId: string): HarnessWakeupItem | null {
    if (harnessName !== undefined) {
      const item = this.db.harnessWakeupItems.get(harnessWakeupKey(harnessName, wakeupItemId));
      return item ? cloneJson(item) : null;
    }
    for (const item of this.db.harnessWakeupItems.values()) {
...
function channelActionReceiptKey(harnessName: string, receiptId: string): string {
  return `${harnessName}\u0000${receiptId}`;
}

function channelOutboxKey(harnessName: string, outboxItemId: string): string {
  return `${harnessName}\u0000${outboxItemId}`;
}

function harnessWakeupKey(harnessName: string, wakeupItemId: string): string {
  return `${harnessName}\u0000${wakeupItemId}`;
}
