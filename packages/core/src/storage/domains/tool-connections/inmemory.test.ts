...
import { InMemoryToolConnectionsStorage } from './inmemory';

describe('InMemoryToolConnectionsStorage', () => {
  let db: InMemoryDB;
  let store: InMemoryToolConnectionsStorage;

  beforeEach(() => {
    db = new InMemoryDB();
    store = new InMemoryToolConnectionsStorage({ db });
  });
