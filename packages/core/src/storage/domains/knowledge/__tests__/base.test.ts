...
describe('InMemoryKnowledgeStorage canonical model', () => {
  ...
  it.each(['second-page', 'replacement-outbox'])('restores all state after %s failure', async stage => {
    const db = new InMemoryDB();
    const subject = new InMemoryKnowledgeStorage({ db });
    await subject.createNode({ id: PROJECT_SCOPE_ID, name: 'Scope', isScope: true, scopeIds: [] });
    const node = await subject.createNode({ name: 'Document', scopeIds: [PROJECT_SCOPE_ID] });
    await subject.setNodeAddress({ source: 'importer', address: 'document:1', nodeId: node.id });
    for (let index = 0; index < 105; index++)
      await subject.createRecord({
    ...
    const snapshot = () =>
      structuredClone(Object.fromEntries(Object.entries(db).filter(([name]) => name.startsWith('knowledge'))));
    const before = snapshot();
    const set = db.knowledgeSemanticOutbox.set.bind(db.knowledgeSemanticOutbox);
    const spy = vi.spyOn(db.knowledgeSemanticOutbox, 'set').mockImplementation((key, entry) => {
      if (
        (stage === 'second-page' &&
          entry.documentId === 'knowledge:record:prior-100' &&
          entry.operation === 'delete') ||
        (stage === 'replacement-outbox' && entry.documentId === 'knowledge:record:replacement')
      )
