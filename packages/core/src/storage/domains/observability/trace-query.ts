  ...
  z.union([
    ...
    z
      .object({
        ...
        set: z.array(literalSchema).min(1),
      })
      .strict(),
...
export function planTraceQuery(
  ...
  const state: PlannerState = { nodes: 0, issues };
  ...
  if (depth > TRACE_QUERY_MAX_DEPTH || state.nodes > TRACE_QUERY_MAX_NODES) {
    if (!state.issues.some(issue => issue.code === 'predicate_too_complex')) {
      state.issues.push({
        ...
        path,
        message: `Predicates are limited to ${TRACE_QUERY_MAX_NODES} nodes and ${TRACE_QUERY_MAX_DEPTH} levels`,
      });
    }
    return undefined;
  }
