...
import { SKILL_FILE_NAME, SKILL_SUBDIRS, SKILL_ERROR_CODES, DEFAULT_LOAD_OPTIONS } from './constants';
...
export async function loadSkill(skillPath: string, options: LoadSkillOptions = {}): Promise<LoadedSkill> {
  ...
  if (!isValidSkillDirectory(skillPath)) {
    throw new SkillError(`Invalid skill path: "${skillPath}"`, SKILL_ERROR_CODES.DIRECTORY_NOT_FOUND, skillPath);
  }
