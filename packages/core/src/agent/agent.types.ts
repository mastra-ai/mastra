...
export type AgentExecutionOptionsBase<OUTPUT> = {
  ...
  /**
   * When true, any non-empty assistant text already streamed to the client is
   * persisted to thread history even if the stream is aborted before the model
   * finishes. Defaults to false (partial output is discarded on abort).
   *
   * @example
   * ```typescript
