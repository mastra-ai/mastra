...
/** Completion checks and scoring configuration shared with network execution. */
...
export interface DelegationStartContext {
  ...
  params: {
    /** Thread identifier supplied for the delegated run. */
    threadId?: string;
    /** Resource identifier supplied for the delegated run. */
    resourceId?: string;
    /** Instructions supplied to the delegated agent. */
    instructions?: string;
    /** Maximum generation steps requested for the delegated agent. */
    maxSteps?: number;
  };
  /** Current iteration number (1-based) */
  iteration: number;
...
export type OnDelegationStartHandler = (
  /** Delegation request and parent-run context inspected before execution. */
...
export interface DelegationCompleteContext {
  ...
  result: {
    /** Text returned by the delegated primitive. */
    text: string;
    /** Thread used by the delegated agent, when available. */
    subAgentThreadId?: string;
    /** Resource used by the delegated agent, when available. */
    ...
    subAgentToolResults?: Array<{
      /** Name of the tool executed by the delegated agent. */
      toolName: string;
      /** Identifier of the delegated agent's tool call. */
      toolCallId: string;
    ...
    usage?: {
      /** Input tokens consumed by the delegated run. */
      inputTokens: number;
      /** Output tokens generated by the delegated run. */
      outputTokens: number;
      /** Total tokens reported for the delegated run. */
      totalTokens: number;
      /** Reasoning tokens reported for the delegated run. */
      reasoningTokens?: number;
      /** Cached input tokens reported for the delegated run. */
...
export type OnDelegationCompleteHandler = (
  /** Delegation outcome and parent-run context. */
...
export interface IterationCompleteContext {
  ...
  toolResults: Array<{
    ...
    /** Output returned by the tool. */
    result: unknown;
    /** Tool execution error, when present. */
    error?: Error;
  }>;
...
export type OnIterationCompleteHandler = (
  /** Completed iteration's messages, tool calls, results and run identity. */
...
export interface DelegationConfig {
   ...
   */
  messageFilter?: (
    /** Parent messages and delegation context used to select forwarded messages. */
...
export type AgentExecutionOptionsBase<OUTPUT> = {
   ...
   * @param chunk - Stream chunk emitted by the execution.
   */
  ...
  scorers?:
    ...
    | Record<
        string,
        {
          /** Name of a registered scorer to execute. */
          scorer: MastraScorer['name'];
          /** Sampling policy applied after eligibility checks. */
          sampling?: ScoringSamplingConfig;
          /** Predicate restricting which runs are eligible for scoring. */
  ...
  untilIdle?:
    | boolean
    | {
        /** Maximum time in milliseconds to wait while idle for background-task completions. */
        maxIdleMs?: number;
      };
