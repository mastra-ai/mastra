...
> extends Agent<TAgentId, TTools, TOutput> {
   ...
   * Abort a run by id.
   *
   * Same gap as {@link abortThreadStream}: the base implementation reaches a
   * durable run's controller through neither of its paths. A run that has not
   * started yet is left to the base implementation, which records the intent
   * the durable entry points consult when the run starts.
   */
