...
type ConsumedDuringPreparation =
  ...
  | 'requestContext'
  // Fork (PF-348): signal echoes are replayed into the first in-process LLM
  // iteration via the prepare-stream run scope; durable cold resumes rebuild
  // pending signals from storage instead, so the key is deliberately not
  // serialized into the durable envelope.
  | '_initialSignalEchoes'
