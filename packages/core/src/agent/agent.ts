...
import { OpenAIReasoningSchemaCompatLayer, OpenAISchemaCompatLayer, type ModelInformation } from '@mastra/schema-compat';
...
export class Agent<TAgentId extends string = string, TTools extends ToolsInput = ToolsInput> extends MastraBase {
  ...
  if ("structuredOutput" in options && options.structuredOutput && options.structuredOutput.schema) {    
    ...
    if (options.structuredOutput?.model) {
      structuredOutputModel = await this.resolveModelConfig(options.structuredOutput?.model, requestContext) as MastraLanguageModelV2
    }
