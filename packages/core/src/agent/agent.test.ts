...
const openaiV5 = createOpenAIv5({ apiKey: process.env.OPENAI_API_KEY });

const dummyModelElection = new MockLanguageModelV1({
  doGenerate: async () => ({
...
const dummyModelElectionV5 = new MockLanguageModelV2({
  doGenerate: async () => ({
    rawCall: { rawPrompt: null, rawSettings: {} },
    finishReason: 'stop',
    usage: { inputTokens: 10, outputTokens: 20, totalTokens: 30 },
    content: [{ type: 'text', text: `Donald Trump won the 2016 U.S. presidential election, defeating Hillary Clinton.` }],
    warnings: [],
  }),
})
...
const models = {
    ...
    v2: new MockLanguageModelV2({
      doGenerate: async ({ prompt }) => {
        ...
        return {
          text: `processed: ${textContent}`,
          content: [
            { type: 'text', text: `processed: ${textContent}` },
          ],
          warnings: [],
          finishReason: 'stop',
  ...
  dummyModelGenerateElection: {
    v1: dummyModelElection,
    v2: dummyModelElectionV5,
  },
  obama: {
    v1: new MockLanguageModelV1({
    ...
    v2: new MockLanguageModelV2({
      doGenerate: async () => ({
        ...
        finishReason: 'stop',
        usage: { inputTokens: 10, outputTokens: 20, totalTokens: 30 },
        content: [{ type: 'text', text: `{"elements":[{"winner":"Barack Obama","year":"2012"},{"winner":"Donald Trump","year":"2016"}]}` }],
        warnings: [],
      }),
    })
  },
  dummyResponseModel: {
    v1: new MockLanguageModelV1({
      ...
      doStream: async () => ({
        stream: simulateReadableStream({
          chunks: [{ id: '1', type: 'text-delta', textDelta: 'Dummy response' }],
        }),
        rawCall: { rawPrompt: null, rawSettings: {} },
      }),
    }),
    v2: new MockLanguageModelV2({
      ...
      doStream: async () => ({
        stream: simulateReadableStream({
          chunks: [
            { id: '1', type: 'text-delta', delta: 'Dummy response' },
            { id: '2', type: 'finish', finishReason: 'stop', usage: { inputTokens: 10, outputTokens: 20, totalTokens: 30 } }
          ],
        }),
        rawCall: { rawPrompt: null, rawSettings: {} },
      }),
...
function agentStructuredOutputTests(model) {
  describe.only(`structuredOutput - version-${model.specificationVersion}`, () => {
...
agentStructuredOutputTests(models.dummyStructuredOutput.v1);
agentStructuredOutputTests(models.dummyStructuredOutput.v2);

function agentGenerateTests(model) {
  describe.only(`generate - version-${model.specificationVersion}`, () => {
...
agentGenerateTests(models.dummyModelGenerateElection.v1);
agentGenerateTests(models.dummyModelGenerateElection.v2);

function messagePersistenceTests(model) {
  describe.skip(`messagePersistence - version-${model.specificationVersion}`, () => {
    it('stream - should persist the full message after a successful run', async () => {
    ...
    it('generate - should persist the full message after a successful run', async () => {
      const mockMemory = new MockMemory();
      const agent = new Agent({
    ...
    it('vNext - should persist the full message after a successful run', async () => {
      const mockMemory = new MockMemory();
      const agent = new Agent({
        name: 'test-agent',
        instructions: 'test',
...
messagePersistenceTests(models.dummyResponseModel.v1);
messagePersistenceTests(models.dummyResponseModel.v2);

function agentStreamTests(model) {
  describe.only(`stream - version-${model.specificationVersion}`, () => {
    it('stream -should rescue partial messages (including tool calls) if stream is aborted/interrupted', async () => {
      const mockMemory = new MockMemory();
      let saveCallCount = 0;
    ...
    it('generate - should rescue partial messages (including tool calls) if generate is aborted/interrupted', async () => {
      const mockMemory = new MockMemory();
      let saveCallCount = 0;
      let savedMessages: any[] = [];
      mockMemory.saveMessages = async function (...args) {
  ...
  describe.only('client side tools', () => {
...
agentStreamTests(models.openai.v1);
agentStreamTests(models.openai.v2);


function emptyResponseModelTests(model) {
  describe.only(`emptyResponseModel - version-${model.specificationVersion}`, () => {
    it('stream - should only call saveMessages for the user message when no assistant parts are generated', async () => {
      const mockMemory = new MockMemory();
      let saveCallCount = 0;
    ...
    it('generate - should only call saveMessages for the user message when no assistant parts are generated', async () => {
      const mockMemory = new MockMemory();
      let saveCallCount = 0;
    ...
    it(`vNext - should only call saveMessages for the user message when no assistant parts are generated`, async () => {
      const mockMemory = new MockMemory();
      let saveCallCount = 0;
...
emptyResponseModelTests(models.emptyResponseModel.v1);
emptyResponseModelTests(models.emptyResponseModel.v2);

function errorResponseModelTests(model) {
  describe.only(`errorResponseModel - version-${model.specificationVersion}`, () => {
    it('generate - should not save any message if interrupted before any part is emitted', async () => {
      const mockMemory = new MockMemory();
      let saveCallCount = 0;
    ...
    it('stream -should not save any message if interrupted before any part is emitted', async () => {
      const mockMemory = new MockMemory();
      let saveCallCount = 0;
    ...
    it('stream - should not save thread if error occurs after starting response but before completion', async () => {
      const mockMemory = new MockMemory();
    ...
    it('generate - should not save thread if error occurs after starting response but before completion', async () => {
      const mockMemory = new MockMemory();
      const saveThreadSpy = vi.spyOn(mockMemory, 'saveThread');
...
errorResponseModelTests(models.errorResponseModel.v1);
errorResponseModelTests(models.errorResponseModel.v2);


function zodSchemaTests(model) {
  describe.only(`zodSchema - version-${model.specificationVersion}`, () => {
...
zodSchemaTests(models.obama.v1);
zodSchemaTests(models.obama.v2);
...
describe('voice capabilities', () => {
  ...
  beforeEach(() => {
    voiceAgent = new Agent({
      ...
      model: models.dummyResponseModel.v1,
...
function agentWithMemoryTests(model) {
  describe.only('agent memory with metadata', () => {
...
agentWithMemoryTests(models.dummyResponseModel.v1);
agentWithMemoryTests(models.dummyResponseModel.v2);
...
function dynamicMemoryConfigTests(model) {
  describe.only('dynamic memory configuration', () => {
...
dynamicMemoryConfigTests(models.dummyResponseModel.v1);
dynamicMemoryConfigTests(models.dummyResponseModel.v2);

function inputProcessorsTests(model) {
  describe.only(`inputProcessors - version-${model.specificationVersion}`, () => {
...
inputProcessorsTests(models.inputProcessors.v1);

inputProcessorsTests(models.inputProcessors.v2);


function dynamicInstructionsTests(model) {
...
dynamicInstructionsTests(models.dummyResponseModel.v1);
dynamicInstructionsTests(models.dummyResponseModel.v2);

const uiMessgageDummyModelv1 = new MockLanguageModelV1({
  doGenerate: async () => ({
    rawCall: { rawPrompt: null, rawSettings: {} },
    finishReason: 'stop',
    usage: { completionTokens: 10, promptTokens: 3 },
...
const uiMessgageDummyModelv2 = new MockLanguageModelV2({
  doGenerate: async () => ({
    rawCall: { rawPrompt: null, rawSettings: {} },
    finishReason: 'stop',
    warnings: [],
    content: [
      { type: 'text', text: 'Response acknowledging metadata' },
    ],
    usage: { inputTokens: 10, outputTokens: 10, totalTokens: 20 },
  }),
  doStream: async () => ({
    stream: simulateReadableStream({
      chunks: [
        ...
        { id: '2', type: 'text-delta', delta: ' acknowledging' },
        { id: '3', type: 'text-delta', delta: ' metadata' },
        { id: '4', type: 'finish', finishReason: 'stop', logprobs: undefined, usage: { inputTokens: 10, outputTokens: 3, totalTokens: 13 } },
      ],
    }),
    warnings: [],
    content: [
...
function uiMessageWithMetadataTests(model) {
  describe.only('UIMessageWithMetadata support', () => {
...
uiMessageWithMetadataTests(uiMessgageDummyModelv1);
uiMessageWithMetadataTests(uiMessgageDummyModelv2);