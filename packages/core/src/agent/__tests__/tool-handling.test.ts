...
function toolhandlingTests(version: 'v1' | 'v2' | 'v3') {
  ...
  describe(`${version} - agent tool handling`, () => {
    ...
    it('should NOT sanitize tool names with dots (regression test for issue #11781)', async () => {
      // Dots are valid in tool names and should be preserved
      // Users reported tools like "fs.readdir", "weather.tool" stopped working
      const toolName = 'fs.readdir';
      ...
      const userAgent = new Agent({
        id: 'user-agent',
        name: 'User agent',
        instructions: 'Test tool with dot in name.',
        model: testModel,
        tools: {
          [toolName]: {
            id: toolName,
            description: 'Tool with dot in name',
            inputSchema: z.object({}),
            execute: async () => {},
          },
        },
      ...
      // The tool name should be preserved with the dot
      expect(Object.keys(tools)).toContain(toolName);
      // Should NOT be converted to underscore
      expect(Object.keys(tools)).not.toContain('fs_readdir');
    });
  });
