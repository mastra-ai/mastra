...
function modelListTests(version: 'v1' | 'v2') {
  describe(
    ...
    () => {
      ...
      it('should fallback to next model when primary model returns 401 authentication error', async () => {
        let usedModelName = '';

        const authErrorModel = new MockLanguageModelV2({
          doGenerate: async () => {
            usedModelName = 'auth-error';
        ...
        const agent = new Agent({
          id: 'test-auth-error-fallback',
          name: 'Test Auth Error Fallback',
          instructions: 'test agent',
          model: [
            {
      ...
      it('should fallback to next model when primary model returns 403 forbidden error', async () => {
        let usedModelName = '';

        const forbiddenModel = new MockLanguageModelV2({
          doGenerate: async () => {
            usedModelName = 'forbidden';
            throw new APICallError({
              ...
              isRetryable: false,
              responseBody:
                '{"error":{"message":"You do not have access to this model","type":"invalid_request_error"}}',
            });
          },
          doStream: async () => {
            usedModelName = 'forbidden';
        ...
        const agent = new Agent({
          id: 'test-forbidden-fallback',
          name: 'Test Forbidden Fallback',
          instructions: 'test agent',
          model: [
            {
