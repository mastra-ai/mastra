 ...
 * result can ever arrive for it. `reconcileAbandonedProviderToolCalls` rewrites
 * such `state:'call'`/`'partial-call'` parts to a terminal `output-error` state
 * so they are no longer indistinguishable from a live pending tool.
 *
...
describe('MessageList.reconcileAbandonedProviderToolCalls', () => {
  it('rewrites an abandoned provider call to output-error, preserving args', () => {
    ...
    expect(list.reconcileAbandonedProviderToolCalls(id)).toBe(true);
  ...
  it('leaves client-executed (non-provider) calls untouched', () => {
    ...
    expect(list.reconcileAbandonedProviderToolCalls(id)).toBe(false);
  ...
  it('reconciles only the abandoned call in a multi-tool message', () => {
    const parts: MastraDBMessage['content']['parts'] = [
      {
        type: 'tool-invocation',
        toolInvocation: {
  ...
  it('returns false for an unknown message id', () => {
    ...
    expect(list.reconcileAbandonedProviderToolCalls('does-not-exist')).toBe(false);
  });
});
