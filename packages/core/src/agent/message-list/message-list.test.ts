...
describe('MessageList modification methods', () => {
  ...
  beforeEach(() => {
    ...
    messageList.add('Hello', 'user');
    messageList.add('Hi there!', 'user');
    messageList.add(
      [
        {
          role: 'assistant',
    ...
    messageList.add('I need help', 'user');
  });
  ...
  describe('removeById', () => {
    it('should remove a message by ID', () => {
      ...
      const result = messageList.removeById(firstMessageId);

      expect(result).toBe(true);
      expect(messageList.get.all.v2()).toHaveLength(3);
      expect(messageList.get.all.v2().find(m => m.id === firstMessageId)).toBeUndefined();
    });

    it('should return false for non-existent message ID', () => {
      const result = messageList.removeById('non-existent-id');

      expect(result).toBe(false);
      expect(messageList.get.all.v2()).toHaveLength(4);
    });
  });

  describe('removeWhere', () => {
    it('should remove messages matching predicate', () => {
      const removedCount = messageList.removeWhere(message => message.role === 'user');

      expect(removedCount).toBe(3); // 3 user messages
      expect(messageList.get.all.v2()).toHaveLength(1);
      expect(messageList.get.all.v2()[0].role).toBe('assistant');
    });

    it('should return 0 when no messages match', () => {
      const removedCount = messageList.removeWhere(message => message.role === ('system' as any));
  ...
  describe('removeByRole', () => {
    it('should remove all user messages', () => {
      const removedCount = messageList.removeByRole('user');
    ...
    it('should remove all assistant messages', () => {
      const removedCount = messageList.removeByRole('assistant');

      expect(removedCount).toBe(1);
      expect(messageList.get.all.v2()).toHaveLength(3);
      expect(messageList.get.all.v2().every(m => m.role === 'user')).toBe(true);
    });
  });

  describe('removeBySource', () => {
    it('should remove all user messages by source', () => {
      const removedCount = messageList.removeBySource('user');

      expect(removedCount).toBe(3); // 3 user messages
      expect(messageList.get.all.v2()).toHaveLength(1);
    });

    it('should remove all response messages by source', () => {
      const removedCount = messageList.removeBySource('response');

      expect(removedCount).toBe(1); // 1 response message
      expect(messageList.get.all.v2()).toHaveLength(3);
    });
  });

  describe('modifyById', () => {
    it('should modify a message by ID', () => {
      const messages = messageList.get.all.v2();
      const firstMessageId = messages[0].id;

      const result = messageList.modifyById(firstMessageId, message => ({
        ...message,
        content: {
          ...message.content,
      ...
      expect(result).toBe(true);
      const modifiedMessage = messageList.get.all.v2().find(m => m.id === firstMessageId);
      expect(modifiedMessage?.content.content).toBe('Modified content');
    });

    it('should return false for non-existent message ID', () => {
      const result = messageList.modifyById('non-existent-id', message => message);

      expect(result).toBe(false);
    });

    it('should preserve message ID even if updater tries to change it', () => {
      const messages = messageList.get.all.v2();
      const firstMessageId = messages[0].id;

      messageList.modifyById(firstMessageId, message => ({
        ...message,
        id: 'different-id',
      }));
      ...
      expect(modifiedMessage?.id).toBe(firstMessageId);
    });
  });

  describe('modifyWhere', () => {
    it('should modify all messages matching predicate', () => {
      const modifiedCount = messageList.modifyWhere(
      ...
      const userMessages = messageList.get.all.v2().filter(m => m.role === 'user');
      expect(userMessages.every(m => m.content.content === 'Modified user message')).toBe(true);
    });
  });

  describe('clear', () => {
    it('should remove all messages', () => {
      const clearedCount = messageList.clear();

      expect(clearedCount).toBe(4);
      expect(messageList.get.all.v2()).toHaveLength(0);
      expect(messageList.getMessageCounts().total).toBe(0);
    });
  });

  describe('getMessageCounts', () => {
    it('should return correct message counts', () => {
      const counts = messageList.getMessageCounts();
  ...
  describe('filter', () => {
    it('should return a new MessageList with filtered messages', () => {
      const filteredList = messageList.filter(message => message.role === 'user');

      expect(filteredList.get.all.v2()).toHaveLength(3);
      expect(filteredList.get.all.v2().every(m => m.role === 'user')).toBe(true);

      // Original list should be unchanged
      expect(messageList.get.all.v2()).toHaveLength(4);
    });

    it('should preserve message source tracking in filtered list', () => {
      const filteredList = messageList.filter(message => message.role === 'user');

      expect(filteredList.getMessageCounts().user).toBe(3);
      expect(filteredList.getMessageCounts().response).toBe(0);
    });
  });
});
