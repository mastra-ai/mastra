...
describe('OpenAI reasoning — orphaned message-item replay (#24052)', () => {
  ...
  it('strips an orphaned message itemId on the prompt-with-suspended path (filterIncompleteToolCalls: false)', () => {
    // filterIncompleteToolCalls: false routes the prompt through the `prompt-with-suspended`
    // conversion branch, which must apply the same orphaned-reasoning sanitizer.
