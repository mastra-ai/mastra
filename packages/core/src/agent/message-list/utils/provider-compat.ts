...
/**
 * Reads an OpenAI Responses itemId from a model-message part.
 *
 * On model messages (as opposed to UI messages) provider fields live under
 * `providerOptions`, so this inspects `providerOptions.openai.itemId`.
 */
function getModelPartOpenAIItemId(part: unknown): string | undefined {
  if (!part || typeof part !== 'object') return undefined;
  const providerOptions = (part as Record<string, unknown>).providerOptions as Record<string, unknown> | undefined;
...
/**
 * Strips orphaned OpenAI reasoning/message-item references before the prompt is sent to the
 * Responses API.
 *
 * OpenAI reasoning models tag each emitted item with `providerOptions.openai.itemId`
 * (`rs_*` for reasoning, `msg_*` for text). When `store=true` the SDK replays these by
 * `item_reference`, and the Responses API requires a `message` item's reasoning partner to be
 * present. Recall windows, or a turn whose reasoning was never persisted (e.g. reasoning was
 * not surfaced to the client), can leave a `message` item whose `rs_*` reasoning is gone,
 * which OpenAI rejects with:
 *   "Item 'msg_*' of type 'message' was provided without its required 'reasoning' item".
 *
 * The repair mirrors the client-stream fix (#23323): drop just the orphaned `itemId` so the
 * message is sent by value as a fresh item instead of an unresolvable reference.
 *
 * Pairing is evaluated per assistant "turn group" — a run of consecutive assistant/tool
 * messages delimited by user/system messages — because a text-only assistant message can be
 * validly paired with a reasoning item carried by an earlier message of the same turn.
 *
 * @see https://github.com/mastra-ai/mastra/issues/24052
 * @see https://github.com/mastra-ai/mastra/issues/23323
 */
export function sanitizeOrphanedReasoningItems(messages: ModelMessage[]): ModelMessage[] {
  const result: ModelMessage[] = [];

  let groupStart = 0;
  while (groupStart < messages.length) {
    const current = messages[groupStart]!;
    ...
    // Collect the maximal run of non-user/non-system messages (assistant + tool).
    let groupEnd = groupStart;
    while (groupEnd < messages.length) {
      const role = messages[groupEnd]!.role;
    ...
    const groupHasReasoningItemId = group.some(
      message =>
        message.role === 'assistant' &&
        Array.isArray(message.content) &&
        message.content.some(part => part.type === 'reasoning' && getModelPartOpenAIItemId(part) !== undefined),
    );

    for (const message of group) {
      if (groupHasReasoningItemId || message.role !== 'assistant' || !Array.isArray(message.content)) {
        result.push(message);
        continue;
      }
      ...
      const content = message.content.map(part => {
        if (part.type !== 'text' || getModelPartOpenAIItemId(part) === undefined) return part;
        changed = true;
        ...
        const { itemId: _itemId, ...openaiRest } = openai;
        return { ...part, providerOptions: { ...providerOptions, openai: openaiRest } };
      });

      result.push(changed ? ({ ...message, content } as ModelMessage) : message);
    }
