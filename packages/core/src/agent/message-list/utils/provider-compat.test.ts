...
describe('dropCrossProviderSignedReasoning', () => {
  const KIMI = 'kimi-for-coding';
  const ANTHROPIC = 'anthropic.messages';

  /**
   * A stored assistant turn with signed thinking, stamped with the provider that produced it.
   * `k3` is served through `@ai-sdk/anthropic` against api.kimi.com, so a Kimi turn carries the
   * same `anthropic` provider key as a real Anthropic turn — only the stamp tells them apart.
   */
  const dbAssistant = (options: {
    provider?: string;
    signature?: string;
    ...
    if (options.signature !== undefined || options.redactedData !== undefined) {
      providerMetadata.anthropic = {
        ...(options.signature !== undefined ? { signature: options.signature } : {}),
        ...(options.redactedData !== undefined ? { redactedData: options.redactedData } : {}),
      };
    }
    return {
      id: `db-${options.signature ?? options.redactedData ?? options.provider ?? 'assistant'}`,
      role: 'assistant',
      createdAt: new Date(0),
      threadId: 'thread-1',
      resourceId: 'resource-1',
      content: {
        ...
        parts: [
          {
            type: 'reasoning',
            reasoning: options.text ?? 'thinking',
            providerMetadata,
          },
        ],
      },
  ...
  const reasoningPart = (options: { signature?: string; redactedData?: string; text?: string }): ModelMessage =>
    ({
      role: 'assistant',
      content: [
        {
          ...
          ...(options.signature !== undefined || options.redactedData !== undefined
            ? {
                providerOptions: {
                  anthropic: {
                    ...(options.signature !== undefined ? { signature: options.signature } : {}),
  ...
  const reasoningSignatures = (messages: ModelMessage[]): Array<string | undefined> =>
  ...
  it('drops a Kimi-stamped signed reasoning part for an Anthropic target', () => {
    const messages = [reasoningPart({ signature: 'kimi-sig' })];
    const result = dropCrossProviderSignedReasoning(
      messages,
      [dbAssistant({ provider: KIMI, signature: 'kimi-sig' })],
      ANTHROPIC,
    );
  ...
  it('drops an Anthropic-stamped signed reasoning part for a Kimi target (symmetry)', () => {
    const messages = [reasoningPart({ signature: 'anthropic-sig' })];
    const result = dropCrossProviderSignedReasoning(
      messages,
      [dbAssistant({ provider: ANTHROPIC, signature: 'anthropic-sig' })],
      KIMI,
    );
  ...
  it('keeps signed reasoning stamped with the target provider', () => {
    const messages = [reasoningPart({ signature: 'anthropic-sig' })];
    const result = dropCrossProviderSignedReasoning(
      messages,
      [dbAssistant({ provider: ANTHROPIC, signature: 'anthropic-sig' })],
  ...
  it('keeps unsigned reasoning and text on the same foreign turn', () => {
    const messages: ModelMessage[] = [
      {
        role: 'assistant',
        content: [
          { type: 'reasoning', text: 'unsigned foreign thinking' },
          { type: 'text', text: 'answer remains' },
        ],
      } as ModelMessage,
    ];
    const result = dropCrossProviderSignedReasoning(
      messages,
      [dbAssistant({ provider: KIMI, text: 'unsigned foreign thinking' })],
      ANTHROPIC,
    );
  ...
  it('drops a redacted thinking block the same way', () => {
    const messages = [reasoningPart({ redactedData: 'kimi-redacted' })];
    const result = dropCrossProviderSignedReasoning(
      messages,
      [dbAssistant({ provider: KIMI, redactedData: 'kimi-redacted' })],
      ANTHROPIC,
    );
  ...
  it('keeps text alongside a dropped signed reasoning part', () => {
    const messages: ModelMessage[] = [
      {
        role: 'assistant',
        content: [
          { type: 'reasoning', text: 'kimi thinking', providerOptions: { anthropic: { signature: 'kimi-sig' } } },
          { type: 'text', text: 'Kimi assistant answer remains.' },
        ],
      } as ModelMessage,
    ];
    const result = dropCrossProviderSignedReasoning(
    ...
    expect(result).toEqual([
      { role: 'assistant', content: [{ type: 'text', text: 'Kimi assistant answer remains.' }] },
    ]);
  });
  ...
  it('returns the input array by identity when the origin message is unstamped', () => {
    const messages = [reasoningPart({ signature: 'kimi-sig' })];
    const result = dropCrossProviderSignedReasoning(messages, [dbAssistant({ signature: 'kimi-sig' })], ANTHROPIC);

    expect(result).toBe(messages);
  });

  it('returns the input array by identity when nothing matches', () => {
    const messages = [reasoningPart({ signature: 'kimi-sig' })];
    const result = dropCrossProviderSignedReasoning(
      messages,
      [dbAssistant({ provider: 'openai.responses', signature: 'openai-sig' })],
      ANTHROPIC,
    );
  ...
  it('drops only the foreign turn when Kimi and Anthropic turns are interleaved', () => {
    const messages = [reasoningPart({ signature: 'kimi-sig' }), reasoningPart({ signature: 'anthropic-sig' })];
    const result = dropCrossProviderSignedReasoning(
      messages,
      [
        dbAssistant({ provider: KIMI, signature: 'kimi-sig' }),
    ...
    expect(reasoningSignatures(result)).toEqual(['anthropic-sig']);
  });

  it('drops foreign signed reasoning on the trailing assistant turn too', () => {
    const messages: ModelMessage[] = [
    ...
    expect(reasoningSignatures(result)).toEqual([]);
    // The tool continuation survives; only the foreign signed thinking is removed.
    expect(result[1]).toMatchObject({ role: 'assistant', content: [{ type: 'tool-call', toolCallId: 'call-1' }] });
  });

  it('warns only for signed blocks present in this prompt, not for every stored foreign turn', () => {
    const warn = vi.fn();
    const messages = [reasoningPart({ signature: 'kimi-present' })];
    dropCrossProviderSignedReasoning(
      messages,
      [
        dbAssistant({ provider: KIMI, signature: 'kimi-present' }),
        // Older history that recall did not select for this prompt.
        dbAssistant({ provider: KIMI, signature: 'kimi-trimmed' }),
      ],
      ANTHROPIC,
      { warn } as unknown as IMastraLogger,
    );
    ...
    expect(warn).toHaveBeenCalledTimes(1);
    expect(warn.mock.calls[0]?.[0]).toContain('kimi-for-coding');
  });

  it('does not mutate the messages it was given', () => {
    const messages = [reasoningPart({ signature: 'kimi-sig' })];
    const snapshot = structuredClone(messages);
    dropCrossProviderSignedReasoning(messages, [dbAssistant({ provider: KIMI, signature: 'kimi-sig' })], ANTHROPIC);
