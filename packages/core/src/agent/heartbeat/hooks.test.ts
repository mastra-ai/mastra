...
function makeAgent(
  ...
  return {
    sendSignal: opts.sendSignal ?? vi.fn(() => ({ accepted: true, runId: 'run-x', signal: {}, action: 'wake' })),
...
describe('HeartbeatWorker — lifecycle hooks', () => {
  it('prepare returning overrides changes effective values used by sendSignal', async () => {
    const sendSignal: any = vi.fn(() => ({ accepted: true, runId: 'r1', signal: {}, action: 'wake' }));
  ...
  it('prepare returning undefined uses row defaults', async () => {
    const sendSignal: any = vi.fn(() => ({ accepted: true, runId: 'r2', signal: {}, action: 'wake' }));
  ...
  it('threaded succeeded path emits onFinish(outcome: succeeded)', async () => {
    const onFinish = vi.fn();
    const sendSignal: any = vi.fn(() => ({ accepted: true, runId: 'r3', signal: {}, action: 'wake' }));
  ...
  it('threaded delivered path emits onFinish(outcome: delivered, joinedExistingRun: true)', async () => {
    const onFinish = vi.fn();
    const sendSignal: any = vi.fn(() => ({ accepted: true, runId: 'r4', signal: {}, action: 'delivered' }));
    const agent = makeAgent({ hooks: { onFinish }, sendSignal });
    const mastra = makeMastra({ agent });
  ...
  it('threaded persisted path emits onFinish(outcome: persisted)', async () => {
    ...
    expect(onFinish).toHaveBeenCalledWith(expect.objectContaining({ outcome: 'persisted', runId: 'r5' }));
  });

  it('threaded discarded path emits onFinish(outcome: discarded)', async () => {
    const onFinish = vi.fn();
    const sendSignal: any = vi.fn(() => ({ accepted: true, runId: 'r6', signal: {}, action: 'discarded' }));
    const agent = makeAgent({ hooks: { onFinish }, sendSignal });
    const mastra = makeMastra({ agent });
    ...
    expect(onFinish).toHaveBeenCalledWith(expect.objectContaining({ outcome: 'discarded', runId: 'r6' }));
  });
  ...
  it('hook exceptions are logged but never recurse or re-route', async () => {
    ...
    const sendSignal: any = vi.fn(() => ({ accepted: true, runId: 'r7', signal: {}, action: 'wake' }));
    const agent = makeAgent({ hooks: { onFinish }, sendSignal });
    const mastra = makeMastra({ agent });
  ...
  it('no hooks configured → execution still works (regression guard)', async () => {
    const sendSignal: any = vi.fn(() => ({ accepted: true, runId: 'r8', signal: {}, action: 'wake' }));
    const agent = {
      sendSignal,
      generate: vi.fn(),
  ...
  it('trigger info carries kind=manual when ctx.triggerKind is manual', async () => {
    const prepare = vi.fn(() => undefined);
    const onFinish = vi.fn();
    const sendSignal: any = vi.fn(() => ({ accepted: true, runId: 'r9', signal: {}, action: 'wake' }));
    const agent = makeAgent({ hooks: { prepare, onFinish }, sendSignal });
    const mastra = makeMastra({ agent });
