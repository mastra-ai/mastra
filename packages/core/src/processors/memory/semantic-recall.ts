...
export interface SemanticRecallOptions {
  ...
  /**
   * Embedder for generating query embeddings. Optional only when `vector` embeds server-side,
...
export class SemanticRecall implements Processor {
   ...
   */
  private get storeEmbedsText(): boolean {
    return !this.embedder && this.vector.embedsServerSide;
  }
  ...
  private async performSemanticSearch({
    ...
    // On a server-embedding store there is nothing to embed and no dimension to validate:
    // the query text goes to the database as text.
    const { embeddings, dimension } = this.storeEmbedsText
    ...
    if (this.storeEmbedsText) {
      // `queryText` is not on the shared QueryVectorParams; only stores that embed server-side
      // accept it, and they are the only ones reached here.
  ...
  private getDefaultIndexName(): string {
    const model = this.embedder?.modelId || (this.vector.embedsServerSide ? 'server' : 'default');
  ...
  private async ensureVectorIndex(indexName: string, dimension?: number): Promise<void> {
    if (dimension === undefined) {
      // A server-embedding store derives the dimension from its own model, so there is nothing
  ...
  async processOutputResult(
    ...
    if (!this.vector || !this.storage || (!this.embedder && !this.storeEmbedsText)) {
    ...
    try {
      ...
      if (this.storeEmbedsText) {
        ...
        // shared UpsertVectorParams; only stores that embed server-side accept it.
