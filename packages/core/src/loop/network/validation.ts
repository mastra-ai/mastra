...
/**
 * Result of running completion scorers.
 * 
 * Note: `finalResult` is only populated when using the default LLM completion check.
 * Custom scorers evaluate pass/fail - the primitive result is used as the final result.
 */
export interface CompletionRunResult {
  /** Whether the task is complete (based on strategy) */
  complete: boolean;
  /** 
   * Final result to return (only from default LLM check).
   * When using custom scorers, the primitive result is used instead.
   */
  finalResult?: string;
...
/**
 * Result from the default LLM completion check.
 * Unlike regular scorers, this includes the LLM-generated finalResult.
 */
export interface DefaultCompletionResult extends ScorerResult {
  /** The final result generated by the LLM (only for default completion) */
  finalResult?: string;
}

/**
 * Runs the default LLM completion check.
 * Unlike scorers, this returns structured output including the finalResult.
 * 
...
export async function runDefaultCompletionCheck(
  agent: Agent,
  context: CompletionContext,
): Promise<DefaultCompletionResult> {
  const start = Date.now();
