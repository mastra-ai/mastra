...
>({
  ...
  return createStep({
    ...
    execute: async ({ inputData, suspend, resumeData, requestContext }) => {
      ...
      // Use dynamic tools from prepareStep if available, otherwise fall back to original tools
      const dynamicTools = (inputData as any).dynamicTools as Tools | undefined;
      const effectiveTools = dynamicTools || tools;
      const tool =
        effectiveTools?.[inputData.toolName] ||
        Object.values(effectiveTools || {})?.find(
          (t): t is Tools[keyof Tools] => `id` in t && (t as any).id === inputData.toolName,
        );
