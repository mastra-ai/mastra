...
describe('createLLMMappingStep toModelOutput', () => {
  ...
  it('should auto-detect multimodal content in tool results without toModelOutput (MCP image content)', async () => {
    const llmMappingStep = createLLMMappingStep(
      {
        models: {} as any,
        controller,
    ...
    const calls = (messageList.updateToolInvocation as Mock).mock.calls;
    const screenshotPart = calls.find(([p]: [any]) => p.toolInvocation.toolName === 'screenshot')?.[0];

    expect(screenshotPart.providerMetadata?.mastra?.modelOutput).toEqual({
      type: 'content',
      value: [
        { type: 'text', text: 'Screenshot captured' },
  ...
  it('should NOT auto-detect when tool result has content array but no image/audio parts', async () => {
    const llmMappingStep = createLLMMappingStep(
      {
        models: {} as any,
        controller,
    ...
    const calls = (messageList.updateToolInvocation as Mock).mock.calls;
    const textPart = calls.find(([p]: [any]) => p.toolInvocation.toolName === 'textTool')?.[0];

    expect(textPart.providerMetadata).toBeUndefined();
  });

  it('should auto-detect audio content in tool results', async () => {
    const llmMappingStep = createLLMMappingStep(
      {
        models: {} as any,
        controller,
        ...
        tools: {
          audioTool: {
            execute: async () => ({
              content: [{ type: 'audio', data: 'audiobase64', mimeType: 'audio/mp3' }],
            }),
            inputSchema: z.object({}),
          },
        },
    ...
    const calls = (messageList.updateToolInvocation as Mock).mock.calls;
    const audioPart = calls.find(([p]: [any]) => p.toolInvocation.toolName === 'audioTool')?.[0];

    expect(audioPart.providerMetadata?.mastra?.modelOutput).toEqual({
      type: 'content',
      value: [{ type: 'file-data', data: 'audiobase64', mediaType: 'audio/mp3' }],
    });
  });
});
