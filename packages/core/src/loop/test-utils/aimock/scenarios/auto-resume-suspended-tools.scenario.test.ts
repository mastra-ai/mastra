...
describeForAllEngines(
  ...
  engine => {
    ...
    // Durable: tool-invocation metadata (suspended state) is not persisted to memory
    // across runs, so the auto-resume system prompt injection doesn't detect suspended tools.
    it.skipIf(engine === 'durable')(
      'auto-resumes a suspended tool when the user sends a follow-up message on the same thread',
      async () => {
