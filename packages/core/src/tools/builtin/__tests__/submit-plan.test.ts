...
type SubmitPlanInput = { title?: string; plan: string };
type SubmitPlanCtx = ToolExecutionContext<SubmitPlanInput, SubmitPlanResume>;

function makeAgentCtx(opts: {
  resumeData?: SubmitPlanResume;
  suspend?: (p: SubmitPlanInput) => Promise<void>;
}): SubmitPlanCtx {
  return {
...
describe('submitPlan tool (standalone)', () => {
  ...
  it('suspends when no resumeData is provided', async () => {
    let suspended: SubmitPlanInput | undefined;
    await expect(
      submitPlan.execute!(
    ...
    expect(suspended).toEqual({ title: 'Add darkmode', plan: '# Overview\nSteps...' });
  });
