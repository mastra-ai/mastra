...
describe('taskWrite tool (standalone)', () => {
  ...
  it('reports missing thread without creating a phantom thread', async () => {
    const memory = (await store.getStore('memory'))!;

    const result = await runTaskWrite({ tasks: sampleTasks }, makeCtx({ threadId: 'missing-thread', storage: store }));

    expect(result.written).toBe(3);
    expect(result.summary).toMatch(/thread not found/);
    await expect(memory.getThreadById({ threadId: 'missing-thread' })).resolves.toBeNull();
  });
});
