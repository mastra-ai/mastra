  ...
  private createExecute(tool: ToolToConvert, options: ToolOptions, logType?: 'tool' | 'toolset' | 'client-tool') {
    ...
    return async (args: unknown, execOptions?: MastraToolInvocationOptions) => {
      ...
      try {
        ...
        const hasResumeData =
          !!execOptions?.resumeData ||
          !!(
            originalArgs &&
            typeof originalArgs === 'object' &&
            !Array.isArray(originalArgs) &&
            (originalArgs as Record<string, unknown>)?.resumeData
          );
        ...
        if (processedInputSchema) {
          ...
          processedInputSchema = {
            ...
            ...('jsonSchema' in processedInputSchema
              ? {
                  jsonSchema: injectToolInputFields(
                    processedInputSchema.jsonSchema as Record<string, any>,
                    this.isBackgroundEligible,
                    this.isResumableTool,
                  ),
                }
