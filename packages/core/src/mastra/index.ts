...
function hasHarnessSessionStorage(storage: MastraCompositeStore | undefined): boolean {
  return storage?.stores?.harness !== undefined;
}

function harnessHasSessionStorage(harness: HarnessV1 | undefined): boolean {
  try {
    return harness?._internalGetSessionStorage() !== undefined;
  } catch {
    return false;
  }
}
...
> {
  ...
  constructor(
    ...
    if (workersOption === false) {
      ...
      const hasHarnessWakeupStorage =
        hasHarnessSessionStorage(config?.storage) || Object.values(configuredHarnesses).some(harnessHasSessionStorage);
    ...
    */
    ...
    const hasHarnessWakeupStorage =
      hasHarnessSessionStorage(this.#storage) || Object.values(this.#harnesses).some(harnessHasSessionStorage);
  ...
  async shutdown(): Promise<void> {
    ...
    let schedulerShutdownError: unknown;
    let hasSchedulerShutdownError = false;
    if (this.#scheduler) {
      try {
        await this.#scheduler.stop();
      } catch (error) {
        schedulerShutdownError = error;
        hasSchedulerShutdownError = true;
