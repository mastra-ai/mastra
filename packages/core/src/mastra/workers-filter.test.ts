...
describe('Mastra workers filter (MASTRA_WORKERS env)', () => {
  ...
  it('propagates workflow scheduler stop failures during shutdown', async () => {
    ...
    vi.spyOn(mastra.scheduler!, 'stop').mockRejectedValueOnce(failure);

    await expect(mastra.shutdown()).rejects.toBe(failure);
    expect(logger.error).toHaveBeenCalledWith('Failed to stop workflow scheduler', failure);
  });
  ...
  it('does not register harness wakeup worker for storage without a harness domain', () => {
    const storage = new MockStore();
    delete (storage.stores as Record<string, unknown>).harness;
