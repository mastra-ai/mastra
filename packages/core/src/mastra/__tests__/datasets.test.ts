...
describe('Mastra Dataset Methods', () => {
  ...
  // --------------------------------------------------------------------------
  // captureSpanToDataset
  // --------------------------------------------------------------------------
  describe('captureSpanToDataset', () => {
    it('creates dataset item from span data', async () => {
      const store = mastra.getDatasetsStore()!;

      // Create dataset first
      const dataset = await store.createDataset({ name: 'capture-test' });

      // Capture span as dataset item
      ...
      expect(item.sourceSpanId).toBe('span-123');
      expect(item.sourceTraceId).toBe('trace-456');
    });

    it('applies transform function to span data', async () => {
      const store = mastra.getDatasetsStore()!;
      const dataset = await store.createDataset({ name: 'transform-test' });
      ...
      const item = await mastra.captureSpanToDataset({
        ...
        transform: ({ input, output }) => ({
          input: { transformed: true, original: input },
          expectedOutput: { modified: output },
          metadata: { wasTransformed: true },
        }),
      });

      expect(item.input).toEqual({ transformed: true, original: { raw: 'input-data' } });
      expect(item.expectedOutput).toEqual({ modified: { raw: 'output-data' } });
      expect(item.metadata).toEqual({ wasTransformed: true });
    });
    ...
    it('throws error when storage is not configured', async () => {
      ...
      await expect(
        mastraNoStorage.captureSpanToDataset({
          span: { spanId: 's1', traceId: 't1' },
          datasetId: 'ds1',
        }),
