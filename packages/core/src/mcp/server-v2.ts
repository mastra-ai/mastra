...
/** Either MCP server family Mastra can register. */
export type MCPServerRegistryEntry = MCPServerBase | MCPServerBaseV2;

/**
 * Outcome of `MCPServerBaseV2.executeTool`. A tool that calls
 * `context.mcpv2.suspend(payload)` is reported as `suspended` together with the
 * payload and the tool's `resumeSchema` (as JSON Schema) so the caller can ask
 * for exactly that input; otherwise the tool's output is returned as `completed`.
 */
...
/**
 * Abstract base class for MCP 2026-07-28 server implementations.
 *
 * This mirrors `MCPServerBase` (same config, ids, metadata, tool conversion and
 * Mastra registration) and differs only where the protocol does: there is no
 * standalone SSE transport, and `executeTool` reports a tool that suspended for
 * input instead of a bare result. `mcpVersion` distinguishes the two families in
 * a `MCPServerRegistryEntry` without requiring anything of 1.x instances.
 */
export abstract class MCPServerBaseV2<TId extends string = string> extends MastraBase {
  /** Discriminates this family from `MCPServerBase` in a `MCPServerRegistryEntry`. */
  ...
  /**
   * Constructor for the MCPServerBaseV2.
   ...
   * @param options Options for the HTTP transport. The legacy transport `options` bag
   * has no meaning on a 2026-07-28 server and is ignored.
   */
   ...
   */
  public abstract getToolListInfo(requestContext?: RequestContext): ReturnType<MCPServerBase['getToolListInfo']>;
   ...
   */
  public abstract getToolInfo(toolId: string): ReturnType<MCPServerBase['getToolInfo']>;

  /**
   * Executes a specific tool provided by this MCP server.
   * @param toolId The ID/name of the tool to execute.
   * @param args The arguments to pass to the tool's execute function.
   * @param executionContext Optional context for the tool execution. `mcpv2` carries the
   * protocol request facilities and, on a continuation, the previous round's
   * `resumeData` and `suspendPayload`; the implementation supplies `suspend`.
   * @returns A promise that resolves to the tool's output, or to the suspension when the tool asked for input.
...
/** Narrows a registry entry to the 2026-07-28 family. 1.x instances have no `mcpVersion`. */
