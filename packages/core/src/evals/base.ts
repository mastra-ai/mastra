 ...
 *
 * A run is either scored (`score` is set, `notScorable` is absent) or was
 * declared not scorable by a step that returned `notScorable()` (`notScorable`
 * is set, `score` is absent). Narrow on `notScorable` before reading `score`.
 */
