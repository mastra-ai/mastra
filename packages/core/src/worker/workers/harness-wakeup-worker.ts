...
export class HarnessWakeupWorker extends MastraWorker {
  ...
  async #markWakeupQueued(
    ...
    try {
      await storage.updateHarnessWakeupItem(
        ...
        { claimId, harnessName: item.harnessName },
      );
    } catch (error) {
      ...
      throw Object.assign(new Error('HarnessWakeupWorker: failed to mark admitted wakeup queued'), {
        cause: error,
        wakeupItemId: item.id,
        queuedItemId,
      });
  ...
  async #markWakeupFailedOrDead(
    ...
    try {
      await storage.updateHarnessWakeupItem(next, { claimId, harnessName: item.harnessName });
    } catch (updateError) {
