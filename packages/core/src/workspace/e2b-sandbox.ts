...
import type { WorkspaceFilesystem, FilesystemMountConfig } from './filesystem';
 ...
 * import { E2BSandbox } from '@mastra/core/workspace/e2b-sandbox';
 *
 ...
 * import { Workspace } from '@mastra/core/workspace';
 * import { E2BSandbox } from '@mastra/core/workspace/e2b-sandbox';
 * import { S3Filesystem } from '@mastra/workspace-fs-s3';
 *
...
export class E2BSandbox implements WorkspaceSandbox {
  ...
  private async mountS3(mountPath: string, config: S3MountConfig): Promise<void> {
    ...
    if (checkResult.stdout.includes('not found')) {
      await this._sandbox.commands.run('apt-get update && apt-get install -y s3fs');
    }
    ...
    // Build mount command
    let mountCmd = `s3fs ${config.bucket} ${mountPath} -o passwd_file=/tmp/.passwd-s3fs`;

    if (config.endpoint) {
      // For S3-compatible storage (MinIO, etc.)
      mountCmd += ` -o url=${config.endpoint} -o use_path_request_style`;
    }

    const result = await this._sandbox.commands.run(mountCmd);
    if (result.exitCode !== 0) {
      throw new Error(`Failed to mount S3 bucket: ${result.stderr}`);
    }
  }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      const e2bModule = await (Function('return import("@e2b/code-interpreter")')() as Promise<{
