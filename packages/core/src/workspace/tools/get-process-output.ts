...
Use this after starting a background command with execute_command (background: true) to check if the process is still running and read its output.`,
  ...
  inputSchema: z.object({
    ...
    tail: z
      ...
      .optional()
      .describe(
        `Number of lines to return, similar to tail -n. Positive or negative returns last N lines from end. Omit to keep the first ${DEFAULT_HEAD_LINES} and last ${DEFAULT_TAIL_LINES} lines. Use 0 for no limit.`,
      ),
    wait: z
      .boolean()
      .optional()
