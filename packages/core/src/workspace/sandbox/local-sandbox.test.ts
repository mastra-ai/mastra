...
import { LocalSandbox, getMarkerDir, MASTRA_GENERATED_HEADER } from './local-sandbox';
import {
  detectIsolation,
  ...
  describe('seatbelt isolation (macOS)', () => {
    ...
    it('should not overwrite custom seatbelt profile in memory on mount/unmount', async () => {
      ...
      try {
        ...
        // Verify the in-memory profile is STILL our custom content and wasn't overwritten on mount
        expect((seatbeltSandbox as any)._seatbeltProfile).toBe(customContent);

        // Trigger an unmount operation (which calls removeMountIsolationForPath)
        await seatbeltSandbox.unmount('/my-mount');

        // Verify the in-memory profile is STILL our custom content and wasn't overwritten on unmount
        expect((seatbeltSandbox as any)._seatbeltProfile).toBe(customContent);
      } finally {
        await seatbeltSandbox._destroy();
      }
    ...
    it('should regenerate seatbelt profile in memory on mount/unmount if custom profile file did not exist initially', async () => {
      if (os.platform() !== 'darwin') {
        return;
      }

      // Define a path that doesn't exist yet
      const customProfilePath = path.join(tempDir, 'non-existent-profile.sb');
      ...
      try {
        ...
        // Verify that the file was generated by Mastra
        const fileGenerated = await fs
          .access(customProfilePath)
          .then(() => true)
          .catch(() => false);
        expect(fileGenerated).toBe(true);

        const initialProfile = (seatbeltSandbox as any)._seatbeltProfile;
        expect(initialProfile).toBeDefined();

        const sourcePath = path.join(tempDir, 'src-dir-2');
        ...
        await seatbeltSandbox.mount(mockFs, '/my-mount-2');

        // Verify the in-memory profile has been regenerated to include the canonical source-path write rule
        const regeneratedProfile = (seatbeltSandbox as any)._seatbeltProfile;
        expect(regeneratedProfile).not.toBe(initialProfile);
        expect(regeneratedProfile).toContain(expectedWriteRule);

        // Trigger an unmount operation (which calls removeMountIsolationForPath)
        await seatbeltSandbox.unmount('/my-mount-2');

        // Verify the in-memory profile is regenerated again and the write rule is removed
        const unmountedProfile = (seatbeltSandbox as any)._seatbeltProfile;
        expect(unmountedProfile).not.toBe(regeneratedProfile);
        expect(unmountedProfile).not.toContain(expectedWriteRule);
      } finally {
        await seatbeltSandbox._destroy();
      }
    });
    ...
    it('should migrate and regenerate legacy headerless seatbelt profiles generated by Mastra', async () => {
      if (os.platform() !== 'darwin') {
        return;
      }

      const legacyProfilePath = path.join(tempDir, 'legacy-generated-profile.sb');

      // Create a temporary sandbox configuration to generate legacy profile format
      const sandboxConfig = {
        workingDirectory: tempDir,
        isolation: 'seatbelt' as const,
        nativeSandbox: {
          seatbeltProfilePath: legacyProfilePath,
        },
      };

      // Generate raw seatbelt profile (no header) and write to path to simulate pre-existing legacy profile
      const rawLegacyProfile = generateSeatbeltProfile(tempDir, sandboxConfig.nativeSandbox);
      await fs.mkdir(path.dirname(legacyProfilePath), { recursive: true });
      await fs.writeFile(legacyProfilePath, rawLegacyProfile, 'utf-8');

      // Start new sandbox pointing to the legacy profile
      const sandbox = new LocalSandbox(sandboxConfig);
      try {
        await sandbox._start();

        // Verify the file on disk has been migrated to include the header prefix
        const migratedContent = await fs.readFile(legacyProfilePath, 'utf-8');
        expect(migratedContent.startsWith(MASTRA_GENERATED_HEADER)).toBe(true);

        const sourcePath = path.join(tempDir, 'src-dir-legacy');
        await fs.mkdir(sourcePath, { recursive: true });
        const canonicalSourcePath = await fs.realpath(sourcePath);
        const expectedWriteRule = `(allow file-write* (subpath "${canonicalSourcePath}"))`;

        // Mount should regenerate the profile and include the mount path write rule
        const mockFs = {
          id: 'test-fs',
          name: 'MockFs',
          provider: 'local',
        ...
        await sandbox.mount(mockFs, '/my-mount-legacy');
