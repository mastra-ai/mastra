...
function createMockFGAProvider(authorized = true): IFGAProvider {
  return {
    ...
    require: authorized
      ...
      : vi
          .fn()
          .mockRejectedValue(new FGADeniedError({ id: 'test' }, { type: 'agent', id: 'agent-1' }, 'agents:execute')),
...
describe('FGADeniedError', () => {
  it('should include user, resource, and permission in error', () => {
    const error = new FGADeniedError({ id: 'user-1' }, { type: 'agent', id: 'agent-1' }, 'agents:execute');
    ...
    expect(error.permission).toBe('agents:execute');
