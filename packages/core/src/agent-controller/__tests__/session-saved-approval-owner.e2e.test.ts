...
const fixture = fileURLToPath(new URL('./fixtures/saved-approval-owner-process.ts', import.meta.url));
...
describe('Session saved approval through an explicit owner after a mode switch', () => {
  it.each(['approve', 'decline'])(
    'restores the subscribed owner and handles %s without automatic execution',
    async decision => {
