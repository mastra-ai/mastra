...
describe('AgentController signal messages', () => {
  ...
  it('ignores trailing chunks from an aborted subscription run', async () => {
    ...
    const subscribeToThread = vi.spyOn(agent, 'subscribeToThread');
    subscribeToThread.mockResolvedValueOnce({
      stream: (async function* () {
        yield { type: 'start', runId: 'run-1' };
        await abortReleased;
    ...
    subscribeToThread.mockResolvedValue({
      stream: (async function* () {
        await new Promise(() => {});
      })(),
      unsubscribe: vi.fn(),
    ...
    expect(subscribeToThread).toHaveBeenCalledTimes(2);
