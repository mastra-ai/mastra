...
The deep merge can add or override call-time keys, but it can't remove them. Set \`providerOptionsMode: 'replace'\` on an entry to send only that entry's \`providerOptions\`. Use it when call-time options are tuned for the primary model, such as a provider routing order, and a fallback on another vendor would reject them.

\`\`\`typescript title="src/mastra/agents/cross-vendor-agent.ts"
import { Agent } from '@mastra/core/agent';

const agent = new Agent({
  id: 'cross-vendor',
  name: 'Cross Vendor Agent',
  instructions: 'You are a helpful assistant.',
  model: [
    { model: '__GATEWAY_OPENAI_MODEL__', maxRetries: 2 },
    {
      model: '__GATEWAY_ANTHROPIC_MODEL_SONNET__',
      maxRetries: 2,
...
// The OpenAI entry receives these options. The Anthropic entry receives only its own.
