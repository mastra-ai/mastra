...
- 0491e7c: **BREAKING CHANGE**: Scorers for Agents will now use `MastraDBMessage` instead of `UIMessage`
...
- 9e1911d: - Removes modelSettings.abortSignal in favour of top-level abortSignal only. Also removes the deprecated output field - use structuredOutput.schema instead.
...
- 00f4921: The `@mastra/core` package no longer allows top-level imports except for `Mastra` and `type Config`. You must use subpath imports for all other imports.
...
- 96d35f6: Removed MastraMessageV3 intermediary format, now we go from MastraDBMessage->aiv5 formats and back directly
...
- 351a11f: Use tool's outputSchema to validate results and return an error object if schema does not match output results.
...
- 173c535: Removes deprecated input-processor type and processors.
...
- 39c9743: Add exponential backoff to model retry logic to prevent cascading failures
...
- f743dbb: Update provider registry and model documentation with latest models and providers
- 0e8ed46: Fix agent onChunk callback receiving wrapped chunk instead of direct chunk
...
- 910db9e: Set correct peer dependency range for `@mastra/observability`
- 2f897df: Add requestContext column if it does not exist
- d629361: Only handle download image asset transformation if needed
- 08c31c1: Add serializedStepGraph to runExecutionResult response
- 1521d71: Track usage in workflow and agent network
- 8940859: Allow resuming nested workflow step with chained id
...
- e629310: Fix OpenAI schema validation errors in processors
...
- fd3d338: Fix tool outputSchema validation to allow unsupported Zod types like ZodTuple. The outputSchema is only used for internal validation and never sent to the LLM, so model compatibility checks are not needed.
- 71c8d6c: Ensure model_generation spans end before agent_run spans.
- 6179a9b: Fix workflow input property preservation after resume from snapshot
...
- c30400a: Fix vector definition to fix pinecone
- 69e0a87: fix resumeStream type to use resumeSchema
- 01f8878: Add type bailed to workflowRunStatus
- 5df9cce: Add tool call approval
- 4c77209: default validate inputs to true in Workflow execute
...
- bc94344: Fix a bug where streaming didn't output the final chunk
- 57d157f: Don't call `os.homedir()` at top level (but lazy invoke it) to accommodate sandboxed environments
- 903f67d: Fix: Don't download unsupported media
- d827d08: Add support for doGenerate in LanguageModelV2. This change fixes issues with OpenAI stream permissions.
  - Added new abstraction over LanguageModelV2

- 2a90c55: Detect thenable objects returned by AI model providers
- 23c10a1: Fix input tool validation when no inputSchema is provided
...
- c710c16: Bug fix: Use input processors that are passed in generate or stream agent options rather than always defaulting to the processors set on the Agent class.
- cfae733: Fix tool input validation to use schema-compat transformed schemas
...
- e3dfda7: Add import for WriteableStream in execution-engine and dedupe llm.getModel in agent.ts
- 993ad98: Fix generate toolResults and mismatch in provider tool names
- 676ccc7: Adds ability to create custom `MastraModelGateway`'s that can be added to the `Mastra` class instance under the `gateways` property. Giving you typescript autocompletion in any model picker string.
...
- c10398d: Add an additional check to determine whether the model natively supports specific file types. Only download the file if the model does not support it natively.
- 7b763e5: Added support for .streamVNext and .stream that uses it in the inngest execution engine
- d36cfbb: pass writableStream parameter to workflow execution
- 3697853: Remove tools passed to the Routing Agent in .network()
- b2e45ec: Fix agent network iteration counter bug causing infinite loops
...
- 53d927c: Add ability to pass agent options when wrapping an agent with createStep. This allows configuring agent execution settings when using agents as workflow steps.
...
- 3f2faf2: Fix MCP server registration
- 22f64bc: Fix network loop iteration counter and usage promise handling:
...
- bda6370: Workflow validation zod v4 support
-
- 735d8c1: Use memory mock in server tests
- acf322e: Fix network routing agent smoothstreaming
...
- cc34739: Fix agent network working memory tool routing. Memory tools are now included in routing agent instructions but excluded from its direct tool calls, allowing the routing agent to properly route to tool execution steps for memory updates.
- 4d59f58: Fixes assets not being downloaded when available
- 2b8893c: Fix creating system messages from inside processors using processInput.
- 8e5c75b: Fix usage tracking with agent network
- e1bb9c9: Remove unused dependencies
- e59e0d3: Fix message conversion for incomplete client-side tool calls
...
- fa8409b: Add `initialState` and `outputOptions` to run.stream() call.
