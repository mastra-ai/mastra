
                    import { EventHandler } from '@arkw/core';
                    import { SharingAgreementResponseFields } from '../constants';
                    import { ZendeskIntegration } from '..';

                    export const ShowSharingAgreement: EventHandler<ZendeskIntegration> = ({
  eventKey,
  integrationInstance: { name, dataLayer, getApiClient },
  makeWebhookUrl,
}) => ({
                        id: `${name}-sync-SharingAgreementResponse`,
                        event: eventKey,
                        executor: async ({ event, step }: any) => {
                            const {  sharing_agreement_id,  } = event.data;
                            const { referenceId } = event.user;
                            const proxy = await getApiClient({ referenceId })


                            const response = await proxy['/api/v2/sharing_agreements/{sharing_agreement_id}'].get({
                                
                                params: {sharing_agreement_id,} })

                            if (!response.ok) {
                            return
                            }

                            const d = await response.json()

                            const records = d?.data?.map(({ _externalId, ...d2 }) => ({
                                externalId: _externalId,
                                data: d2,
                                entityType: `SharingAgreementResponse`,
                            }));

                            await dataLayer?.syncData({
                                name,
                                referenceId,
                                data: records,
                                type: `SharingAgreementResponse`,
                                properties: SharingAgreementResponseFields,
                            });
                        },
                })
                