...
import type { ScorerVersionResponse } from '@mastra/client-js';
import { useScorerVersions, useScorerVersionMutations } from '../../hooks/use-stored-scorers';
import { SaveScorerVersionDialog } from './save-scorer-version-dialog';
import { ScorerVersionCompareDialog } from './scorer-version-compare-dialog';
import { toast } from '@/lib/toast';

interface ScorerVersionsListProps {
  scorerId: string;
  activeVersionId?: string;
}

interface VersionListItemProps {
  version: ScorerVersionResponse;
  isActive: boolean;
  onActivate: () => void;
  onDelete: () => void;
}
...
export function ScorerVersionsList({ scorerId, activeVersionId }: ScorerVersionsListProps) {
  const [isSaveDialogOpen, setIsSaveDialogOpen] = useState(false);
  const [isCompareDialogOpen, setIsCompareDialogOpen] = useState(false);
  const [page, setPage] = useState(0);

  const { data, isLoading } = useScorerVersions(scorerId, { page, perPage: 10 });
  const { activateScorerVersion, deleteScorerVersion } = useScorerVersionMutations(scorerId);
  ...
  const handleActivate = (versionId: string) => {
    activateScorerVersion.mutate(versionId, {
      onSuccess: () => {
  ...
  const handleDelete = (versionId: string) => {
    if (confirm('Are you sure you want to delete this version?')) {
      deleteScorerVersion.mutate(versionId, {
        onSuccess: () => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {totalPages > 1 && (
        <div className="p-4 border-t border-border1 flex items-center justify-between">
          <Button
            ...
            disabled={page === 0 || deleteScorerVersion.isPending}
          >
            Previous
          </Button>
          ...
          <Button
            ...
            size="md"
            onClick={() => setPage(p => p + 1)}
            disabled={page >= totalPages - 1 || deleteScorerVersion.isPending}
          >
            Next
          </Button>
        </div>
      ...
      <SaveScorerVersionDialog scorerId={scorerId} open={isSaveDialogOpen} onOpenChange={setIsSaveDialogOpen} />
      <ScorerVersionCompareDialog
        scorerId={scorerId}
