...
describe('TracesListView — rows', () => {
  ...
  it('features the selected trace and leaves the others plain', () => {
    ...
    expect(rows[0]?.classList.contains('bg-gray-alpha-3!')).toBe(false);
    expect(rows[1]?.classList.contains('bg-gray-alpha-3!')).toBe(true);
  });
...
describe('TracesListView — featuring by trace alone', () => {
  it('features a branch row by its trace when no span was named', () => {
    ...
    const rows = screen.getAllByRole('button');
    expect(rows[0]?.classList.contains('bg-gray-alpha-3!')).toBe(true);
    expect(rows[1]?.classList.contains('bg-gray-alpha-3!')).toBe(false);
  });
});
