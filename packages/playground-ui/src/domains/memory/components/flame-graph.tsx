...
const MSG_COLOR = 'var(--green-7)';
const OBS_COLOR = 'var(--blue-7)';
const REFLECT_COLOR = 'var(--orange-7)';

function TimeAxis({ domain }: { domain: TDomain }) {
  const ticks = [0, 0.25, 0.5, 0.75, 1];
  return (
    <div className="grid grid-cols-[6rem_1fr] items-center">
      <p className="text-icon3 border-gray-alpha-3/50 text-ui-xs flex items-center self-stretch border-r pl-3 font-medium">
        Time
      </p>
...
export function FlameTooltip({
  ...
  if (showValue) {
    return (
      <div className="border-gray-alpha-3 bg-gray-1 text-ui-xs flex flex-col gap-0.5 rounded border px-2 py-1.5 font-mono shadow">
        {time && (
  ...
  return (
    <div className="border-gray-alpha-3 bg-gray-1 text-ui-xs rounded border px-2 py-1 font-mono shadow">
      {time && <span className="text-gray-10">{time}</span>}
    </div>
  );
}
...
function AreaRow({ label, data, dataKey, color, gradientId, domain, zoomDomain, threshold }: AreaRowProps) {
  ...
  return (
    <div className="border-gray-alpha-3/50 relative grid grid-cols-[6rem_1fr] items-center border-b hover:z-10">
      <p className="text-icon3 border-gray-alpha-3/50 text-ui-xs flex items-center self-stretch border-r pl-3 font-medium">
        {label}
      </p>
      <div>
        <ResponsiveContainer width="100%" height={32}>
          <AreaChart data={data} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
            ...
            <Tooltip content={<FlameTooltip domain={domain} showValue />} cursor={{ stroke: 'var(--gray-alpha-3)' }} />
            <Area
              type="linear"
              dataKey={dataKey}
              stroke={color}
        ...
        <ResponsiveContainer width="100%" height={height}>
          ...
          >
            ...
            <Area
              ...
              fill={`url(#${gradientId})`}
              isAnimationActive={false}
              activeDot={{ r: 5, stroke: color, strokeWidth: 2, fill: 'var(--background-1)' }}
...
function ZoomTrack({
  ...
  return (
    <div className="border-gray-alpha-3/50 grid grid-cols-[6rem_1fr] items-center border-b">
      <div className="border-gray-alpha-3/50 flex items-center gap-1 self-stretch border-r pl-3">
      ...
      >
        <div
          ...
          className="bg-background-2/60 absolute inset-y-0 left-0"
          style={{ width: `${leftPercent}%` }}
        />
        <div
          data-zoom-part="band"
          className="border-gray-alpha-3/30 bg-gray-alpha-1 absolute inset-y-0 border-y"
          style={{ left: `${leftPercent}%`, right: `${100 - rightPercent}%` }}
        />
        <div
          data-zoom-part="after"
          className="bg-background-2/60 absolute inset-y-0 right-0"
          style={{ width: `${100 - rightPercent}%` }}
        />
        <div
          data-zoom-handle="left"
          className="bg-gray-alpha-10 hover:bg-gray-10 absolute inset-y-0 w-1 cursor-col-resize"
