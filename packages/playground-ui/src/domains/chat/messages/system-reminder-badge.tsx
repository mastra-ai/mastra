    ...
    <div className="border-border1 bg-surface2 overflow-hidden rounded-lg border">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <p className="text-ui-sm text-foreground font-medium">System reminder</p>
          <p className="text-ui-xs text-muted-foreground mt-1 break-all">{title}</p>
        </div>
        {isExpanded ? (
      ...
      {isExpanded && reminder.body && (
        <div className="border-border1 bg-surface1 border-t px-4 py-3">
          <pre className="text-ui-xs text-foreground font-mono break-words whitespace-pre-wrap">{reminder.body}</pre>
        </div>
      )}
    </div>
  );
