...
export const AgentNetworkNode = memo(function AgentNetworkNode({
  id,
  data,
  selected,
}: NodeProps<Node<AgentNetworkNodeData, 'agent-network'>>) {
      ...
      <div className="p-3">
        ...
        {data.networkId ? (
          <div className="space-y-1">
            <div className="text-xs text-icon4 bg-surface2 rounded px-2 py-1 font-mono truncate">{data.networkId}</div>
            <div className="flex items-center gap-4 text-xs text-icon4">
              ...
              <div className="flex items-center gap-1">
                ...
                <span>{ROUTING_LABELS[data.routingStrategy] || data.routingStrategy}</span>
              </div>
            </div>
            {/* Mini agent avatars */}
            {agentCount > 0 && (
              <div className="flex items-center gap-1 mt-1">
                {data.agents.slice(0, 4).map(agentId => (
                  <div
                    key={agentId}
