...
export const ForeachNode = memo(function ForeachNode({ id, data, selected }: NodeProps) {
  const nodeData = data as unknown as ForeachNodeData;
        ...
        <div className="space-y-1 text-xs">
          {nodeData.collection ? (
            <div className="text-icon4 bg-surface2 rounded px-2 py-1 font-mono truncate">
              {nodeData.collection.$ref || 'collection'}
            </div>
          ) : (
          ...
          <div className="flex items-center gap-4 text-icon4">
            <div className="flex items-center gap-1">
              ...
              <span className="font-mono">{nodeData.itemVariable || 'item'}</span>
            </div>
            {nodeData.concurrency && nodeData.concurrency > 1 && (
              <div className="flex items-center gap-1">
                <span className="text-icon3">Parallel:</span>
                <span>{nodeData.concurrency}</span>
              </div>
            )}
          </div>
        </div>
