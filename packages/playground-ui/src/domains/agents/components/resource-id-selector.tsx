...
export function ResourceIdSelector({
  ...
  const options = React.useMemo(() => {
    const uniqueIds = new Set([agentId, ...availableResourceIds]);
    return Array.from(uniqueIds);
  }, [agentId, availableResourceIds]);
    ...
    <Popover.Root open={open} onOpenChange={setOpen}>
      ...
      <Popover.Portal>
        <Popover.Positioner align="start" sideOffset={4} className="z-50">
          <Popover.Popup className="w-[240px] rounded-md border border-border1 bg-surface1 p-1 shadow-md">
