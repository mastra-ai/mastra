...
export const AgentTabActions = ({ agentId }: AgentTabActionsProps) => {
  ...
  return (
    <div className="ml-auto flex items-center gap-2" data-testid="agent-tab-actions">
      <IconButton tooltip="Copy Agent ID" size="sm" variant="outline" onClick={handleCopy}>
        <CopyIcon />
      </IconButton>
      ...
      {canWriteAgents && (
        <IconButton tooltip="Clone agent" size="sm" variant="outline" onClick={handleClone} disabled={isCloning}>
          <CopyPlus />
        </IconButton>
      )}

      <IconButton tooltip="Copy session URL" size="sm" variant="outline" onClick={handleShareLink}>
