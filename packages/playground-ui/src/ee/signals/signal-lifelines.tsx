...
export function SignalLifelines({
  ...
  return (
    <section aria-label={`${label} lifelines`} className="min-w-0">
      <h3 className="font-mono text-ui-sm font-semibold tracking-widest uppercase" style={{ color: nodeColor(hue) }}>
        <Tooltip>
          <TooltipTrigger
            render={
              <button
      ...
      {isCollapsed ? undefined : rows.length === 0 ? (
        <p className="text-neutral3 mt-2 text-ui-sm">No themes in these landmarks.</p>
      ) : (
