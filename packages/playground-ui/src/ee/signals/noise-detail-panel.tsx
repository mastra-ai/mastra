          ...
          {insightTraceId === undefined && (
            <>
              <section aria-labelledby="noise-summary-heading">
                <h2 id="noise-summary-heading" className="text-ui-sm text-neutral3 font-mono tracking-wider uppercase">
                  Summary
                </h2>
                <p className="text-ui-md text-neutral5 mt-3">
                  Noise contains trace signal summaries that did not consistently match a recurring theme in this
                  snapshot.
                </p>
                {noiseQuery.isPending && <p className="text-ui-md text-neutral3 mt-4">Loading noise details…</p>}
                {noiseQuery.isError && <p className="text-ui-md mt-4 text-red-500">Unable to load noise details.</p>}
                {noiseQuery.data && (
                  <p className="text-ui-md text-neutral5 mt-4 font-mono tabular-nums">
                    {shareSentence(
              ...
              <section aria-labelledby="noise-examples-heading">
                <h2 id="noise-examples-heading" className="text-ui-sm text-neutral3 font-mono tracking-wider uppercase">
                  Example summaries
                </h2>
                {examplesQuery.isPending && <p className="text-ui-md text-neutral3 mt-3">Loading examples…</p>}
                ...
                {examplesQuery.data && (
                  <>
                    {examplesQuery.data.examples.length === 0 ? (
                      <p className="text-ui-md text-neutral3 mt-3">No noise examples in this snapshot.</p>
                    ) : (
                        ...
                        {examplesQuery.data.examples.map(example => (
                          <li key={example.traceId}>
                            <button
                              ...
                              className="border-border1 bg-surface3 text-ui-md text-neutral5 hover:bg-surface5 w-full cursor-pointer rounded-md border p-3 text-left"
