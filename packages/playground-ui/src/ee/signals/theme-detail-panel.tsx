      ...
      <DrawerContent>
        <DrawerHeader className="border-border1 border-b">
          {signalName !== undefined && (
            <span
              className="font-mono text-ui-sm font-semibold tracking-widest"
            ...
            <>
              ...
              {detailQuery.data && !detailQuery.data.theme && (
                <section>
                  ...
                  <p className="text-neutral3 mt-2 text-ui-md">This theme has no data in the selected snapshot.</p>
                </section>
              )}
              {detailQuery.data?.theme && (
                <>
                  <section aria-labelledby="theme-summary-heading">
                    <h2 id="theme-summary-heading" className="text-neutral3 font-mono text-ui-sm tracking-wider uppercase">
                      Summary
                    </h2>
                    <p className="text-neutral5 mt-3 text-ui-md">
                      {detailQuery.data.theme.description ?? 'No description available.'}
                    </p>
                    <p className="text-neutral5 mt-3 font-mono text-ui-md tabular-nums">
                      {shareSentence(
                  ...
                  <section aria-labelledby="theme-examples-heading">
                    <h2
                      ...
                      className="text-neutral3 font-mono text-ui-sm tracking-wider uppercase"
                    >
                      Examples
                    </h2>
                    ...
                    {examplesQuery.data && (
                      <>
                        {examplesQuery.data.examples.length === 0 ? (
                          <p className="text-neutral3 mt-3 text-ui-md">No examples in this snapshot.</p>
                        ) : (
                  ...
                  {snapshotTotal > 1 && (
                    <section aria-labelledby="theme-trend-heading">
                      <h2 id="theme-trend-heading" className="text-neutral3 font-mono text-ui-sm tracking-wider uppercase">
                        Trend
                      </h2>
                      {historyQuery.isPending && <p className="text-neutral3 mt-3 text-ui-md">Loading trend…</p>}
                      {historyQuery.isError && <p className="mt-3 text-ui-md text-red-500">Unable to load the trend.</p>}
