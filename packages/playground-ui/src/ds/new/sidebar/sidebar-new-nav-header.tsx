      ...
      {showTitle ? (
        <header
          ...
          className={cn('max-w-full min-w-0 truncate pl-3 text-ui-sm font-medium', {
            'text-mastra-foreground': isActive,
            'text-mastra-muted-foreground': !isActive,
          })}
        >
          {href ? (
            <Link
              ...
              className={cn('block min-w-0 truncate transition-colors duration-normal', {
                'hover:text-mastra-foreground': !isActive,
                'text-mastra-foreground': isActive,
              })}
            >
              {children}
        ...
        <>
          ...
          <div aria-hidden="true" className="bg-mastra-border mx-3 h-px flex-1" />
        </>
      )}
    </div>
  );
