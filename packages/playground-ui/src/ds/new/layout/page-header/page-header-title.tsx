...
export function PageHeaderTitle({ children, className, isLoading, size = 'md', ...props }: PageHeaderTitleProps) {
  return (
    <h1
      ...
      className={cn(
        'col-start-[title] row-start-1 flex min-w-0 items-center gap-2 self-start font-medium text-foreground [&>svg]:size-[1.25em] [&>svg]:opacity-50',
        titleSizeClasses[size],
        isLoading && 'w-60 max-w-[50%] animate-pulse rounded-md bg-muted',
        className,
      )}
      {...props}
    >
