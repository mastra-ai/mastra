...
export type TabListProps = Omit<TabListVariantsProps, 'variant'> & {
   ...
   * CSS variable, e.g. `style={{ '--tab-indicator-color': 'var(--purple-9)' } as React.CSSProperties}`.
   */
    ...
    <div className={cn('w-full overflow-x-auto', sticky && 'sticky top-0 z-10 bg-surface-secondary')}>
      ...
      >
        ...
        {(resolvedVariant === 'pill' || resolvedVariant === 'pill-ghost') && (
          <BaseTabs.Indicator
            className={cn(
              'absolute top-1/2 left-0 z-0 rounded-full bg-[var(--tab-indicator-color,var(--surface-hover))]',
