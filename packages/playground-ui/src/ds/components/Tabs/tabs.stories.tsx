...
function ContainedExample({
  ...
  const closeHandler = (tab: string) =>
    closableTabs && visibleTabs.length > 1
      ...
      : undefined;
  return (
    <main className="new-theme bg-background min-h-screen p-4 sm:p-10">
      <div className="mx-auto w-full max-w-5xl">
        <Tabs defaultTab="activity" value={activeTab} onValueChange={setActiveTab} appearance="contained" frame={frame}>
          ...
          <TabContent value="traces">
            <p className="text-ui-md text-foreground/90">Trace content</p>
          </TabContent>
          <TabContent value="settings">
            <p className="text-ui-md text-foreground/90">Settings content</p>
          </TabContent>
          {moreTabs &&
            extraTabs
              .filter(label => visibleTabs.includes(label))
              .map(label => (
                <TabContent key={label} value={label}>
                  <p className="text-ui-md text-foreground/90">{label} content</p>
                </TabContent>
              ))}
        </Tabs>
      </div>
