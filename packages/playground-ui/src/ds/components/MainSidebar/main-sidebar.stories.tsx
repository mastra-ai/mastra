          ...
          <MainSidebar.NavLink asChild state={state} link={{ name: 'Search', url: '#', icon: <Search /> }}>
            ...
            >
              ...
              {state !== 'collapsed' && (
                <kbd
                  aria-hidden="true"
                  className="border-border1 bg-surface4 text-ui-xs text-neutral3 ml-auto rounded border px-1.5 py-0.5 font-mono"
                >
                  ⌘K
                </kbd>
              )}
      ...
      <MainSidebar.Bottom className="pb-3">
        ...
        {state !== 'collapsed' && (
          <>
            ...
            <span className="bg-sidebar-nav-active text-ui-xs dark:text-neutral6 ml-3 inline-flex h-5 items-center rounded-full px-2.5 font-sans font-semibold text-black/80">
              v0.0.0
            </span>
          </>
        )}
