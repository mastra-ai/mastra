    ...
    return (
      <DropdownMenu.Sub key={cat.id} onOpenChange={resetSubSearch}>
        <DropdownMenu.SubTrigger>
          ...
          {selectedCount > 0 && <span className={cn('text-caption text-info-fg')}>{selectedCount}</span>}
        </DropdownMenu.SubTrigger>
  ...
  return (
    <DropdownMenu modal={false}>
      <DropdownMenu.Trigger asChild>
        <Button disabled={disabled} size="md" icon={<FilterIcon />}>
          {label}
          {activeFilterCount > 0 && (
            <span
              className={cn(
                'ml-0.5 inline-flex size-5 items-center justify-center rounded-full bg-info-bg text-caption text-foreground',
              )}
            >
              {activeFilterCount}
            </span>
