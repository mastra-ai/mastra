...
export const InContext: Story = {
  render: () => (
    <div className="bg-surface-hover flex items-center gap-2 rounded-md p-3">
      <code className="font-mono text-sm text-(--text-primary)">npm install @mastra/core</code>
...
export const CodeBlock: Story = {
  render: () => (
    <div className="w-dropdown-max-height bg-surface-hover relative rounded-md p-4">
      <CopyButton content="const agent = new Agent()" className="absolute top-2 right-2" />
      <pre className="font-mono text-sm text-(--text-primary)">const agent = new Agent()</pre>
    </div>
  ),
};
