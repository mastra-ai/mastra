...
function MetricsColumn({ title }: { title: string }) {
  return (
    <Column className="p-5" withRightSeparator>
      <Column.Toolbar>
        <h2 className="text-gray-10 text-lg">{title}</h2>
      ...
      <Column.Content className="gap-3">
        {columnContent.map(([label, value]) => (
          <Card key={label} appearance="surface">
            ...
            <CardContent density="compact" className="text-ui-sm text-gray-9">
              {label}
            </CardContent>
          </Card>
        ))}
...
export const ResponsiveGrid: Story = {
  render: () => (
    <div className="bg-background-1 h-144 p-4">
...
export const HorizontallyScrollable: Story = {
  render: () => (
    <div className="bg-background-1 h-128 w-full max-w-3xl p-4">
