...
export function ItemListRow({ isSelected, children, columns }: ItemListRowProps) {
  return (
    <li
      className={cn(
        'flex overflow-hidden rounded-lg border border-transparent border-t-border py-1 pb-0.5 text-body text-foreground first:border-t-transparent',
        '[&:last-child>button]:rounded-b-lg',
        '[&.selected-row]:border [&.selected-row]:border-dashed [&.selected-row]:border-white/50 [&.selected-row]:pr-1 [&.selected-row]:first:border-t',
