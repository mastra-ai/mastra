...
export function comboboxTriggerClass({
  ...
  return cn(
    ...
    error && 'border-red-9 hover:border-red-9 focus-visible:border-red-9',
    className,
  );
}
...
export const comboboxItemClass = cva(
  cn(
    ...
    'min-h-8 py-1.5',
    'text-ui-smd leading-ui-sm text-gray-10',
...
export const comboboxStyles = {
  ...
  popup: cn(
    'w-max max-w-(--available-width) min-w-(--anchor-width) rounded-xl border border-gray-alpha-3 bg-gray-1 text-gray-10',
    'shadow-dialog',
  ...
  searchContainer: cn('flex items-center border-b border-gray-alpha-3 px-2.5 py-1.5', transitions.colors),

  /** Search icon */
  searchIcon: cn('mr-2 size-3.5 shrink-0 text-gray-9', transitions.colors),

  /** Search input */
  searchInput: cn(
    'flex h-7 w-full rounded-md bg-transparent py-1 text-ui-smd leading-ui-sm text-gray-10',
  ...
  /** Empty state */
  empty: 'not-empty:block hidden py-4 text-center text-ui-smd text-gray-9',
  ...
  checkContainer: 'flex h-4 w-4 shrink-0 items-center justify-center text-gray-10',
  ...
  /** Option description */
  optionDescription: 'text-ui-sm text-gray-9 truncate',
