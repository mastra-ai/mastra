...
export function ComposerSendButton({ appearance = 'round', ...props }: ActionProps) {
  return (
    <Button
      type="submit"
      variant={appearance === 'outline' ? 'outline' : 'default'}
      size={appearance === 'outline' ? 'icon-sm' : 'icon-md'}
      className={appearance === 'round' ? 'border-border1 bg-surface5 rounded-full border' : undefined}
      {...props}
    >
      <ArrowUp
