...
const stubNodeBuiltinsPlugin: Plugin = {
  ...
  load(id) {
    if (id.startsWith('\0node-stub:')) {
      ...
      // AsyncLocalStorage needs a real constructor with run/getStore methods.
      if (mod === 'async_hooks') {
        return [
          'export class AsyncLocalStorage { run(_, fn) { return fn(); } getStore() { return undefined; } }',
          'export default { AsyncLocalStorage };',
        ].join('\n');
      }

      // stream/web classes exist natively in the browser — re-export the globals
      // so that code extending them (e.g. MastraReadableStream) works.
      if (mod === 'stream/web') {
        return [
          ...
          'export default { ReadableStream: RS, WritableStream: WS, TransformStream: TS };',
        ].join('\n');
      }
