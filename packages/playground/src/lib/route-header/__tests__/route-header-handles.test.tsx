...
describe('route header handles', () => {
  ...
  it('review queue route yields a single Review Queue crumb (it is a top-level sidebar entry)', () => {
    const handles = collectRouteHandles(getAppRoutes());
    ...
    expect(crumbs.map(c => c.id)).toEqual(['nav:/experiments/review-queue']);
    expect(crumbs[0]).toMatchObject({ label: 'Review Queue' });
  });
