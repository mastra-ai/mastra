...
export function useRouteSidePanel() {
  ...
  if (!ctx) {
    return { hasPanel: false, isCollapsed: true, setIsCollapsed: noop, panelHandle: detachedHandle, toggle: noop };
  }
  const { activeOwner, isCollapsed, setIsCollapsed, panelHandle, toggle } = ctx;
  return { hasPanel: activeOwner !== null, isCollapsed, setIsCollapsed, panelHandle, toggle };
}
