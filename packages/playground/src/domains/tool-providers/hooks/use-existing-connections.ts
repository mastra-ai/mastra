 ...
 */
export const useExistingConnections = (providerId: string | null | undefined, toolkit: string | null | undefined) => {
  const client = useMastraClient();

  return useQuery({
    queryKey: ['tool-integration-connections', providerId, toolkit],
    queryFn: () => client.getToolProvider(providerId!).listConnections({ toolkit: toolkit! }),
