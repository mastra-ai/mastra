        ...
        <div className="flex flex-col gap-3">
          {health.integrations.map(integration => (
            <IntegrationRow
              ...
              onReauthorize={async (toolkit, connectionId) => {
                ...
                if (result.status === 'completed') {
                  await health.invalidateIntegration(integration.providerId);
                }
              }}
...
interface IntegrationRowProps {
  ...
  onReauthorize: (toolkit: string, connectionId: string) => Promise<void> | void;
  isPending: boolean;
}

const IntegrationRow = ({ integration, onReauthorize, isPending }: IntegrationRowProps) => {
  return (
    <div className="flex flex-col gap-1.5" data-testid={`health-integration-${integration.providerId}`}>
      ...
      <ul className="flex flex-col gap-1 pl-5">
        {integration.byToolkit.map(service => {
          ...
          return (
            <li
              ...
              data-testid={`health-service-${integration.providerId}-${service.toolkit}`}
            >
              <div className="flex items-center gap-2">
                {stateGlyph(state)}
              ...
              {service.disconnectedConnections.map(conn => (
                <div
                  ...
                  data-testid={`health-disconnected-${integration.providerId}-${service.toolkit}-${conn.connectionId}`}
                >
                  ...
                  <Button
                    ...
                    variant="ghost"
                    disabled={isPending}
                    onClick={() => onReauthorize(service.toolkit, conn.connectionId)}
                    aria-label={`Reauthorize ${conn.label}`}
                    data-testid={`health-reauthorize-${integration.providerId}-${service.toolkit}-${conn.connectionId}`}
                  >
