    ...
    server.use(
      ...
      http.get(`${BASE_URL}/api/tool-providers/${INTEGRATION_ID}/connections`, ({ request }) => {
        ...
        return HttpResponse.json({
          items: [{ connectionId: 'c1', toolkit: TOOL_SERVICE, status: 'active', label: 'Work', authorId: 'admin-1' }],
        });
      }),
    );
    renderPicker({ initial: [] });
    ...
    server.use(
      ...
      http.get(`${BASE_URL}/api/tool-providers/${INTEGRATION_ID}/connections`, ({ request }) => {
        ...
        if (!authorId) {
          return HttpResponse.json({
            items: [
              { connectionId: 'c1', toolkit: TOOL_SERVICE, status: 'active', label: 'Work', authorId: 'admin-1' },
              { connectionId: 'c2', toolkit: TOOL_SERVICE, status: 'active', label: 'Bob', authorId: 'user-bob' },
            ],
          });
        }
        return HttpResponse.json({
