...
describe('when Studio users publish prompt block versions', () => {
  it('loads prompt block versions and forwards version route params', async () => {
    ...
    const detail = renderHook(
      () => usePromptBlockVersion({ blockId: PROMPT_BLOCK_ID, versionId: PROMPT_BLOCK_VERSION_ID }),
      { wrapper },
    );
  ...
  it('creates, activates, restores, and deletes prompt block versions with cache invalidation', async () => {
    ...
    server.use(
      http.post(`${TEST_BASE_URL}/api/stored/prompt-blocks/${PROMPT_BLOCK_ID}/versions`, () =>
        HttpResponse.json(newVersion),
      ),
      http.post(
        `${TEST_BASE_URL}/api/stored/prompt-blocks/${PROMPT_BLOCK_ID}/versions/prompt-block-version-2/activate`,
        () => HttpResponse.json(activatedPromptBlockVersion),
      ),
      http.post(
        `${TEST_BASE_URL}/api/stored/prompt-blocks/${PROMPT_BLOCK_ID}/versions/${PROMPT_BLOCK_VERSION_ID}/restore`,
        () => HttpResponse.json(newVersion),
      ),
      http.delete(
        `${TEST_BASE_URL}/api/stored/prompt-blocks/${PROMPT_BLOCK_ID}/versions/${PROMPT_BLOCK_VERSION_ID}`,
    ...
    queryClient.setQueryData(
      ['prompt-block-versions', PROMPT_BLOCK_ID],
      makePromptBlockVersionsList([makePromptBlockVersion()]),
    );
