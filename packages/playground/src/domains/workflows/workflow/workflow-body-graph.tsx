...
export function WorkflowBodyGraph({
  ...
  return (
    <section aria-label={isForEach ? 'Loop body' : 'Nested workflow'} className="flex h-full min-h-0 flex-col">
      {isForEach && (
        <div className="flex shrink-0 items-start gap-2 px-4 py-3 text-neutral3">
          <Repeat2 aria-hidden className="mt-0.5 size-4 shrink-0 text-neutral3" />
          <Txt variant="ui-xs">Runs for every item. All items finish before the workflow continues.</Txt>
        </div>
      )}
      <div className="min-h-0 flex-1 overflow-hidden">
        <ReactFlowProvider>
          <WorkflowNestedGraph stepGraph={stepGraph} embedded open workflowName={workflowName} isForEach={isForEach} />
        </ReactFlowProvider>
      </div>
    </section>
  );
