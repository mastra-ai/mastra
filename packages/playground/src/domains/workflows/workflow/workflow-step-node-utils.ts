...
type BaseResolvedWorkflowStep = {
  id: string;
  flow: SerializedStepFlowEntry;
};

export type ResolvedWorkflowStep =
  | (BaseResolvedWorkflowStep & {
      kind: 'step' | 'map-step' | 'foreach-step' | 'nested-workflow-step' | 'loop-step';
      step: Extract<SerializedStepFlowEntry, { step: unknown }>['step'];
    })
  | (BaseResolvedWorkflowStep & { kind: 'parallel-step' | 'conditional' | 'sleep-step' | 'sleep-until-step' });
