  ...
  return (
    <div style={depth > 0 ? { paddingLeft: depth * 12 } : undefined}>
      <div className="flex items-center gap-2 py-1">
        ...
        {prop.description && <span className="text-neutral3 truncate text-ui-sm italic">— {prop.description}</span>}
      </div>
      {hasChildren && (
...
function InstructionsDiffView({ previousBlocks, currentBlocks }: { previousBlocks: unknown; currentBlocks: unknown }) {
  ...
  if (prevBlocksArr.length <= 1 && currBlocksArr.length <= 1) {
    ...
    return (
      <div className="border-border1 relative overflow-hidden rounded-md border font-mono text-ui-md">
        {block && (
    ...
    <div className="flex flex-col gap-2">
      {Array.from({ length: maxLen }, (_, idx) => {
        ...
        if (!prevBlock && currBlock) {
          return (
            <div key={idx} className="rounded-md border border-green-900/30 bg-green-950/10 p-3 font-mono text-ui-md">
        ...
        if (prevBlock && !currBlock) {
          return (
            <div key={idx} className="relative rounded-md border border-red-900/30 bg-red-950/10 p-3 font-mono text-ui-md">
              <div className="absolute top-2 right-2">
        ...
        const diffLines = computeLineDiff(oldStr, newStr);
        return (
          <div key={idx} className="border-border1 relative overflow-hidden rounded-md border font-mono text-ui-md">
            {prevBlock && (
              <div className="absolute top-2 right-2 z-10">
                <BlockCopyButton block={prevBlock} />
              </div>
