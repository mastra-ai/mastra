...
export function MemorySidebar({
  ...
  return (
    <SidebarPanel>
      {hasMemory ? (
        <div
          data-testid="memory-sidebar-panel"
          className={cn(
            'h-full min-h-0 min-w-0',
            showMemoryDetail
              ? 'grid grid-cols-[minmax(280px,320px)_minmax(360px,1fr)]'
        ...
        >
          ...
          {showMemoryDetail && <MemoryDetailView agentId={agentId} threadId={threadId} />}
        </div>
      ) : (
        <EmptyState
          iconSlot={null}
