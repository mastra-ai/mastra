    ...
    <div className="bg-surface4 rounded-lg">
            ...
            {availableModes.map(m => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => setMode(m)}
                  className={`inline-flex items-center gap-1.5 rounded border px-2.5 py-1 text-ui-sm font-medium transition-colors ${isActive ? config.color : 'bg-surface2 text-neutral4 hover:bg-surface3 border-transparent'} `}
                >
                  {config.icon}
                  {config.label}
                </button>
      ...
      {searchResults && (
        <div className="border-border1 border-t">
          <div className="flex items-center justify-between px-4 py-2 text-ui-sm">
          ...
          {searchResults.results.length === 0 ? (
            <div className="text-neutral4 px-4 py-8 text-center text-ui-md">No results found. Try a different query.</div>
          ) : (
  ...
  return (
    <li className="border-border1 border-t first:border-t-0">
      <button onClick={onClick} className="hover:bg-surface5 flex w-full gap-3 px-4 py-3 text-left transition-colors">
        <span className="text-neutral3 w-4 shrink-0 text-ui-sm tabular-nums">{rank}</span>
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            <FolderOpen className="text-neutral4 h-3.5 w-3.5 shrink-0" />
            <span className="text-neutral6 truncate font-mono text-ui-md">{fileId}</span>
          ...
          <p className="text-neutral4 line-clamp-2 text-ui-sm">{result.content}</p>
          {result.lineRange && (
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={handleSearch} className="space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="bg-surface3 border-border1 text-neutral6 placeholder:text-neutral3 focus:ring-accent1 w-full rounded-lg border py-2 pr-4 pl-10 text-ui-md focus:ring-2 focus:outline-hidden"
            />
          </div>
        ...
        <div className="flex items-center gap-4 text-ui-md">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            ...
            <span className="text-neutral3 ml-auto text-ui-sm">Score: {result.score.toFixed(3)}</span>
          </div>
          <p className="text-neutral4 line-clamp-3 text-ui-md whitespace-pre-wrap">
            {result.content.slice(0, 300)}
            {result.content.length > 300 && '...'}
          </p>
          {result.lineRange && (
            <p className="text-neutral3 mt-2 text-ui-sm">
              Lines {result.lineRange.start}–{result.lineRange.end}
            </p>
          )}
        </div>
