...
function AgentEvalsProbe() {
  const { pathname, search } = useLocation();
  return <div data-testid="agent-evals-probe">{`${pathname}${search}`}</div>;
}
      ...
      <QueryClientProvider client={queryClient}>
        <MemoryRouter initialEntries={[initialEntry]}>
          <Routes>
            ...
            <Route path="/agents/:agentId/evaluate" element={<AgentEvalsProbe />} />
          </Routes>
        </MemoryRouter>
      </QueryClientProvider>
    </MastraReactProvider>,
...
describe('CreateDatasetPage', () => {
  ...
  it('returns to the agent Evals > Datasets tab when opened with ?agentId', async () => {
    server.use(
    ...
    renderPage('/datasets/new?targetType=agent&targetIds=chef-agent&agentId=chef-agent');
    fireEvent.change(screen.getByPlaceholderText('Enter dataset name'), { target: { value: 'My DS' } });
    fireEvent.click(screen.getByRole('button', { name: /create dataset/i }));

    expect((await screen.findByTestId('agent-evals-probe')).textContent).toBe(
      '/agents/chef-agent/evaluate?tab=datasets',
    );
  });
});
