    ...
    <PageLayout className={showSkillsEmptyState ? 'flex min-h-full flex-col' : undefined}>
      ...
      <PageLayout.MainArea className={showSkillsEmptyState ? 'flex flex-1 flex-col gap-6' : 'grid content-start gap-6'}>
        ...
        {workspaces.length > 1 && (
          <div className="relative">
            <button
              ...
              className="border-border1 bg-surface2 hover:bg-surface3 flex w-full max-w-md items-center gap-2 rounded-lg border px-3 py-2 text-ui-md transition-colors"
            >
                ...
                {workspaces.map(workspace => (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="text-neutral6 truncate text-ui-md font-medium">{workspace.name}</div>
                      <div className="text-neutral4 truncate text-ui-sm">
                        {workspace.source === 'agent' ? `Agent: ${workspace.agentName}` : 'Global workspace'}
                      </div>
                    </div>
                    <div className="flex shrink-0 gap-1">
                      {workspace.safety?.readOnly && (
                        <span className="rounded bg-amber-500/20 px-1.5 py-0.5 text-ui-xs text-amber-400">
                          Read-only
                        </span>
                      )}
                      {workspace.capabilities.hasFilesystem && (
                        <span className="bg-surface4 text-neutral4 rounded px-1.5 py-0.5 text-ui-xs">FS</span>
                      )}
                      {workspace.capabilities.hasSandbox && (
                        <span className="bg-surface4 text-neutral4 rounded px-1.5 py-0.5 text-ui-xs">Sandbox</span>
                      )}
                      {workspace.capabilities.hasSkills && (
                        <span className="bg-surface4 text-neutral4 rounded px-1.5 py-0.5 text-ui-xs">Skills</span>
                      )}
                    </div>
                  </button>
                ))}
        ...
        {workspaces.length === 1 && selectedWorkspace && (
          <div className="text-neutral4 flex items-center gap-2 text-ui-md">
            {selectedWorkspace.source === 'agent' ? (
              <Bot className="text-accent1 h-4 w-4" />
            ) : (
            ...
            {isReadOnly && (
              <span className="rounded bg-amber-500/20 px-1.5 py-0.5 text-ui-xs text-amber-400">Read-only</span>
            )}
          </div>
        )}
              ...
              {hasSkills && (
                <Tab value="skills">
                  ...
                  {isSkillsConfigured && skills.length > 0 && (
                    <span className="bg-surface4 text-neutral4 rounded px-1.5 py-0.5 text-ui-sm">{skills.length}</span>
                  )}
                </Tab>
              )}
            </TabList>
...
function WorkspaceSearchPanel({
  ...
  return (
    <div className="border-border1 bg-surface2 space-y-4 rounded-lg border p-4">
      {canSearchFiles && (
        <div>
          <h3 className="text-neutral5 mb-3 flex items-center gap-2 text-ui-md font-medium">
            <FileText className="h-4 w-4" />
            Search Indexed Files
          </h3>
          {showInitWarning && (
            <p className="mb-3 text-ui-sm text-amber-400">
