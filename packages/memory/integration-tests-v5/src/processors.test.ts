...
function v2ToCoreMessages(messages: MastraMessageV2[] | UIMessageWithMetadata[]): CoreMessage[] {
...
describe('Memory with Processors', () => {
  ...
  it('should apply multiple processors without duplicating messages', async () => {
    ...
    expect(remembered.uiMessages.filter(m => m.role === 'user').length).toBe(2);
    expect(remembered.uiMessages.length).toBe(4); // 2 user, 2 assistant. These wont be filtered because they come from memory.query() directly
  });
