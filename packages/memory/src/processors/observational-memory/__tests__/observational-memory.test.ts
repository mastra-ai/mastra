...
import { registerOp, unregisterOp } from '../operation-registry';
import {
...
async function createReplayStorage() {
...
describe('Full Async Buffering Flow', () => {
  ...
  it('should back off removals when projection drifts and final remaining tokens would fall below the floor', async () => {
    ...
    try {
      ...
      const remainingMessages = messageListAfterStep1.get.all.db();
      const remainingTokens = new TokenCounter().countMessages(remainingMessages);
      expect(remainingTokens).toBeGreaterThanOrEqual(expectedFloor);
    } finally {
...
describe('Per-step save deduplication', () => {
  ...
  // Repro capture workflow docs:
  // ../__fixtures__/repro-captures/README.md
  const decodeCapturedState = (value: unknown): unknown => {
    if (!value || typeof value !== 'object') {
      return value;
    }

    if (Array.isArray(value)) {
      return value.map(item => decodeCapturedState(item));
    }

    const record = value as Record<string, unknown>;
    if (record.__type === 'Set' && Array.isArray(record.values)) {
      return new Set(record.values.map(item => decodeCapturedState(item)));
    }

    if (record.__type === 'Map' && Array.isArray(record.entries)) {
      return new Map(
        record.entries.map(entry => {
          if (!Array.isArray(entry) || entry.length !== 2) return [entry, entry];
          return [decodeCapturedState(entry[0]), decodeCapturedState(entry[1])];
        }),
      );
    }

    return Object.fromEntries(Object.entries(record).map(([key, current]) => [key, decodeCapturedState(current)]));
  };

  const buildReplayEstimateKey = (kind: string, payload: string) => {
    const payloadHash = createHash('sha1').update(payload).digest('hex');
    return `${kind}:${payloadHash}`;
  };

  const normalizeReplayTokenEstimateCache = (target: any, kind: string, payload: string) => {
    const estimate = target?.providerMetadata?.mastra?.tokenEstimate ?? target?.metadata?.mastra?.tokenEstimate;
    if (!estimate || !Number.isFinite(estimate.tokens)) {
      return;
    }

    estimate.v = 1;
    estimate.source = typeof estimate.source === 'string' ? estimate.source : 'v1:o200k_base';
    estimate.key = buildReplayEstimateKey(kind, payload);
  };

  const normalizeReplayMessageTokenEstimates = (message: any) => {
    if (!message?.content || typeof message.content !== 'object') {
      return;
    }

    if (typeof message.content.content === 'string' && !Array.isArray(message.content.parts)) {
      normalizeReplayTokenEstimateCache(message.content, 'content-content', message.content.content);
      normalizeReplayTokenEstimateCache(message, 'message-content', message.content.content);
      return;
    }
    ...
    for (const part of message.content.parts) {
      ...
      if (part.type === 'text' && typeof part.text === 'string') {
        normalizeReplayTokenEstimateCache(part, 'text', part.text);
        continue;
      }
      ...
      if (invocation.state === 'result') {
        const useStoredModelOutput = Boolean(
          part.providerMetadata?.mastra && 'modelOutput' in part.providerMetadata.mastra,
        );
        const resultForCounting = useStoredModelOutput ? part.providerMetadata.mastra.modelOutput : invocation.result;
        if (resultForCounting === undefined) {
          continue;
        }

        if (typeof resultForCounting === 'string') {
          normalizeReplayTokenEstimateCache(
            part,
            useStoredModelOutput ? 'tool-result-model-output' : 'tool-result',
            resultForCounting,
          );
        } else {
          normalizeReplayTokenEstimateCache(
            part,
            useStoredModelOutput ? 'tool-result-model-output-json' : 'tool-result-json',
            JSON.stringify(resultForCounting),
          );
        }
      ...
      const existingKey = String(part.providerMetadata?.mastra?.tokenEstimate?.key ?? '');
      if (existingKey.includes('-name') && typeof invocation.toolName === 'string') {
        normalizeReplayTokenEstimateCache(part, `tool-${invocation.state}-name`, invocation.toolName);
        continue;
      }

      if (existingKey.includes('-args') && invocation.args !== undefined) {
        if (typeof invocation.args === 'string') {
          normalizeReplayTokenEstimateCache(part, `tool-${invocation.state}-args`, invocation.args);
        } else {
          normalizeReplayTokenEstimateCache(
            part,
  ...
  const runReproStepReplay = async (
    fixtureDir: string,
    options?: {
      requireCleanupObserved?: boolean;
      expectedMinRemaining?: number;
      requireThresholdReached?: boolean;
    },
  ) => {
    const { MessageList } = await import('@mastra/core/agent');
    const { RequestContext } = await import('@mastra/core/di');

    const input = JSON.parse(readFileSync(join(fixtureDir, 'input.json'), 'utf8'));
    const preState = JSON.parse(readFileSync(join(fixtureDir, 'pre-state.json'), 'utf8'));
    const capturedOutput = JSON.parse(readFileSync(join(fixtureDir, 'output.json'), 'utf8'));

    const storage = await createReplayStorage();
    await storage.saveThread({
      thread: {
        id: input.threadId,
    ...
    const preMessages = (preState.messages ?? []).map((message: any) => ({
      ...message,
      createdAt: message.createdAt ? new Date(message.createdAt) : undefined,
    }));
    preMessages.forEach(normalizeReplayMessageTokenEstimates);
    if (Array.isArray((preState as any).messageList?.messages)) {
      (preState as any).messageList.messages.forEach(normalizeReplayMessageTokenEstimates);
    }
    ...
    const bufferedChunks = Array.isArray(preState.bufferedChunks)
      ? preState.bufferedChunks.map((chunk: any) => ({
          ...chunk,
          createdAt: chunk.createdAt ? new Date(chunk.createdAt) : new Date(),
          lastObservedAt: chunk.lastObservedAt ? new Date(chunk.lastObservedAt) : undefined,
        }))
      : [];

    // Repro captures can be taken while async buffering is still in-flight. In those
    // cases pre-state may contain only a subset of chunk coverage even though activation
    // later succeeds with a larger observed ID set. Hydrate missing chunk coverage from
    // captured cleanup IDs so replay follows the same threshold/activation branch.
    const capturedCleanupObservedIds = Array.isArray(capturedOutput?.details?.thresholdCleanup?.observedIds)
      ? (capturedOutput.details.thresholdCleanup.observedIds as string[])
      : [];
    const bufferedObservedSet = new Set(
      bufferedChunks.flatMap((chunk: any) => (Array.isArray(chunk?.messageIds) ? chunk.messageIds : [])),
    );
    const missingChunkObservedIds = capturedCleanupObservedIds.filter(id => !bufferedObservedSet.has(id));

    if (missingChunkObservedIds.length > 0) {
      const messagesById = new Map(
        preMessages.filter((message: any) => !!message?.id).map((message: any) => [message.id, message]),
      );
      const hydratedMessages = missingChunkObservedIds
        .map(id => messagesById.get(id))
        .filter((message): message is any => Boolean(message));

      if (hydratedMessages.length > 0) {
        const chunkTokenCounter = new TokenCounter();
        const hydratedMessageTokens = chunkTokenCounter.countMessages(hydratedMessages);
        const lastObservedAt = hydratedMessages.reduce<Date | undefined>((latest, message) => {
        ...
        bufferedChunks.push({
          id: `replay-hydrated-${randomUUID()}`,
          cycleId: bufferedChunks[bufferedChunks.length - 1]?.cycleId ?? 'replay-hydrated-cycle',
          observations: bufferedChunks[bufferedChunks.length - 1]?.observations ?? 'replay-hydrated-observations',
          tokenCount:
            bufferedChunks[bufferedChunks.length - 1]?.tokenCount ??
            Math.max(1, Math.floor(hydratedMessageTokens * 0.05)),
          messageIds: hydratedMessages.map(message => message.id),
          messageTokens: hydratedMessageTokens,
          lastObservedAt,
          createdAt: preState.record?.updatedAt ? new Date(preState.record.updatedAt) : new Date(),
        });
      }
    }

    const decodedObservedMessageIds = decodeCapturedState(preState.record?.observedMessageIds);
    const normalizedObservedMessageIds = Array.isArray(decodedObservedMessageIds)
      ? decodedObservedMessageIds
      : decodedObservedMessageIds instanceof Set
        ? Array.from(decodedObservedMessageIds.values())
        : [];

    await storage.insertObservationalMemoryRecord({
      ...preState.record,
      observedMessageIds: normalizedObservedMessageIds,
      threadId: input.threadId,
      resourceId: input.resourceId,
      createdAt: preState.record?.createdAt ? new Date(preState.record.createdAt) : new Date(),
      updatedAt: preState.record?.updatedAt ? new Date(preState.record.updatedAt) : new Date(),
      lastObservedAt: preState.record?.lastObservedAt ? new Date(preState.record.lastObservedAt) : undefined,
      lastBufferedAtTime: preState.record?.lastBufferedAtTime ? new Date(preState.record.lastBufferedAtTime) : null,
    ...
    const om = new ObservationalMemory({
      ...
      scope: 'thread',
      model: mockModel as any,
      observation: preState.record?.config?.observation,
      reflection: preState.record?.config?.reflection,
    });

    const messageList = new MessageList({ threadId: input.threadId, resourceId: input.resourceId });
    messageList.deserialize(preState.messageList);

    const sealedIdsFromMessages = new Set(
      preMessages
        .filter((message: any) => message?.content?.metadata?.mastra?.sealed)
        .map((message: any) => message.id)
        .filter(Boolean),
    );

    const rawInputState = input.state && typeof input.state === 'object' ? decodeCapturedState(input.state) : {};
    const restoredSealedIds =
      rawInputState && typeof rawInputState === 'object' && (rawInputState as any).sealedIds instanceof Set
        ? new Set((rawInputState as any).sealedIds as Set<string>)
        : sealedIdsFromMessages;

    const state: Record<string, unknown> = {
      ...(rawInputState as Record<string, unknown>),
      initialSetupDone: (rawInputState as any).initialSetupDone ?? true,
      sealedIds: restoredSealedIds,
    };

    const lockKey = (om as any).getLockKey(input.threadId, input.resourceId);
    const observationBufferKey = (om as any).getObservationBufferKey(lockKey);
    const reflectionBufferKey = (om as any).getReflectionBufferKey(lockKey);

    (ObservationalMemory as any).asyncBufferingOps.delete(observationBufferKey);
    (ObservationalMemory as any).asyncBufferingOps.delete(reflectionBufferKey);
    (ObservationalMemory as any).lastBufferedBoundary.delete(observationBufferKey);
    (ObservationalMemory as any).lastBufferedBoundary.delete(reflectionBufferKey);
    (ObservationalMemory as any).lastBufferedAtTime.delete(observationBufferKey);
    (ObservationalMemory as any).lastBufferedAtTime.delete(reflectionBufferKey);
    (ObservationalMemory as any).reflectionBufferCycleIds.delete(reflectionBufferKey);

    if (typeof preState.record?.lastBufferedAtTokens === 'number') {
      (ObservationalMemory as any).lastBufferedBoundary.set(observationBufferKey, preState.record.lastBufferedAtTokens);
      (ObservationalMemory as any).lastBufferedBoundary.set(reflectionBufferKey, preState.record.lastBufferedAtTokens);
    }
    if (preState.record?.lastBufferedAtTime) {
      const restoredBufferedAt = new Date(preState.record.lastBufferedAtTime);
      (ObservationalMemory as any).lastBufferedAtTime.set(observationBufferKey, restoredBufferedAt);
      (ObservationalMemory as any).lastBufferedAtTime.set(reflectionBufferKey, restoredBufferedAt);
    }
    (ObservationalMemory as any).sealedMessageIds.set(input.threadId, new Set(restoredSealedIds));

    const registeredOps: Array<'bufferingObservation' | 'bufferingReflection' | 'observing' | 'reflecting'> = [];
    const recordId = preState.record?.id;
    if (typeof recordId === 'string') {
      if (preState.record?.isBufferingObservation) {
        registerOp(recordId, 'bufferingObservation');
        registeredOps.push('bufferingObservation');
      }
      if (preState.record?.isBufferingReflection) {
        registerOp(recordId, 'bufferingReflection');
        registeredOps.push('bufferingReflection');
      }
      if (preState.record?.isObserving) {
        registerOp(recordId, 'observing');
        registeredOps.push('observing');
      }
      if (preState.record?.isReflecting) {
        registerOp(recordId, 'reflecting');
        registeredOps.push('reflecting');
      }
    }

    const requestContext = new RequestContext();
    requestContext.set('MastraMemory', { thread: { id: input.threadId }, resourceId: input.resourceId });
    ...
    let capturedObservedIds: string[] = [];
    let capturedMinRemaining: number | undefined;
    const originalCleanup = (om as any).cleanupAfterObservation.bind(om);

    (om as any).cleanupAfterObservation = async (...args: any[]) => {
      cleanupCallCount += 1;
      capturedObservedIds = Array.isArray(args[5]) ? args[5] : [];
      capturedMinRemaining = typeof args[6] === 'number' ? args[6] : undefined;
      return originalCleanup(...args);
    };

    let thresholdReachedInReplay = false;
    let thresholdHandleCallCount = 0;
    let lastThresholdResult: any;
    const originalHandleThresholdReached = (om as any).handleThresholdReached.bind(om);
    (om as any).handleThresholdReached = async (...args: any[]) => {
      thresholdHandleCallCount += 1;
      thresholdReachedInReplay = true;
      const result = await originalHandleThresholdReached(...args);
      lastThresholdResult = result;
      return result;
    };

    let lastThresholdSnapshot:
      | {
          totalPendingTokens: number;
          threshold: number;
          allMessageCount: number;
          unobservedMessageCount: number;
        }
      | undefined;
    const originalCalculateObservationThresholds = (om as any).calculateObservationThresholds.bind(om);
    (om as any).calculateObservationThresholds = (...args: any[]) => {
      const result = originalCalculateObservationThresholds(...args);
      lastThresholdSnapshot = {
        totalPendingTokens: result?.totalPendingTokens ?? 0,
        threshold: result?.threshold ?? 0,
        allMessageCount: Array.isArray(args[0]) ? args[0].length : 0,
        unobservedMessageCount: Array.isArray(args[1]) ? args[1].length : 0,
      };
      return result;
    };

    try {
      const rawCapturedArgs = input.args && typeof input.args === 'object' ? decodeCapturedState(input.args) : {};
      const capturedArgs = (rawCapturedArgs ?? {}) as Record<string, unknown>;
      const capturedSteps = Array.isArray(capturedArgs.steps) ? (capturedArgs.steps as any[]) : [];
      const capturedStepIds = capturedSteps.map(step =>
        step && typeof step === 'object' ? (step as any).id : undefined,
      );
      const fixtureStepFromDir = Number((fixtureDir.match(/-step-(\d+)-/) ?? [])[1] ?? NaN);
      const preRunTokens = (om as any).tokenCounter.countMessages(messageList.get.all.db() as any);
      const recomputedFixtureMessageTokens = (om as any).tokenCounter.countMessages(preMessages as any);
      const recomputedSerializedMessageListTokens = Array.isArray((preState as any).messageList?.messages)
        ? (om as any).tokenCounter.countMessages((preState as any).messageList.messages as any)
        : undefined;

      expect(input.stepNumber).toBe(fixtureStepFromDir);
      if (
        typeof preState.contextTokenCount === 'number' ||
        typeof recomputedSerializedMessageListTokens === 'number' ||
        typeof recomputedFixtureMessageTokens === 'number'
      ) {
        const expectedPreRunTokens =
          typeof recomputedSerializedMessageListTokens === 'number'
            ? recomputedSerializedMessageListTokens
            : typeof preState.contextTokenCount === 'number' &&
                preState.contextTokenCount === recomputedFixtureMessageTokens
              ? preState.contextTokenCount
              : recomputedFixtureMessageTokens;
        expect(preRunTokens).toBe(expectedPreRunTokens);
      }
      if (Array.isArray(input.messageIds) && input.messageIds.length > 0) {
        expect(input.messageIds.every((id: unknown) => typeof id === 'string' && id.length > 0)).toBe(true);
      }
      if (capturedStepIds.length > 0) {
        expect(capturedStepIds.every(Boolean)).toBe(true);
      }

      await om.processInputStep({
        messageList,
        messages: Array.isArray(capturedArgs.messages) ? (capturedArgs.messages as any[]) : [],
        requestContext,
        stepNumber: input.stepNumber,
        state,
        steps: capturedSteps,
        systemMessages: Array.isArray(capturedArgs.systemMessages) ? (capturedArgs.systemMessages as any[]) : [],
        model: mockModel as any,
        tools: capturedArgs.tools as any,
        toolChoice: capturedArgs.toolChoice as any,
        activeTools: Array.isArray(capturedArgs.activeTools) ? (capturedArgs.activeTools as string[]) : undefined,
        providerOptions: capturedArgs.providerOptions as any,
        modelSettings: capturedArgs.modelSettings as any,
        structuredOutput: capturedArgs.structuredOutput as any,
        retryCount: Number.isFinite(capturedArgs.retryCount) ? Number(capturedArgs.retryCount) : 0,
        abort: (() => {
          throw new Error('aborted');
        }) as any,
      });
      ...
      (om as any).cleanupAfterObservation = originalCleanup;
      (om as any).handleThresholdReached = originalHandleThresholdReached;
      (om as any).calculateObservationThresholds = originalCalculateObservationThresholds;
      if (typeof recordId === 'string') {
        for (const op of registeredOps) {
          unregisterOp(recordId, op);
        }
      }
    }

    if (options?.requireThresholdReached) {
      const capturedThresholdReached = Boolean(capturedOutput?.details?.thresholdReached);
      expect(capturedThresholdReached).toBe(true);
      if (!thresholdReachedInReplay) {
        throw new Error(
          `threshold replay divergence (in-memory): ${JSON.stringify({
            thresholdReachedInReplay,
            thresholdHandleCallCount,
            lastThresholdResult,
            lastThresholdSnapshot,
            capturedThreshold: capturedOutput?.details?.threshold,
            capturedPendingTokens: capturedOutput?.details?.totalPendingTokens,
            capturedContextMessageCount: capturedOutput?.details?.contextMessageCount,
            capturedObservedIdsCount: capturedOutput?.details?.thresholdCleanup?.observedIdsCount,
            preStateContextTokenCount: preState?.contextTokenCount,
          })}`,
        );
      }
    }

    const postMessages = messageList.get.all.db();
    const postIds = new Set(postMessages.map(m => m.id).filter(Boolean));
    const removedIds = [...preIds].filter(id => !postIds.has(id));
    const observedSet = new Set(capturedObservedIds);

    const extraRemoved = removedIds.filter(id => !observedSet.has(id));
    const missingObservedStillPresent = capturedObservedIds.filter(id => postIds.has(id));

    if (options?.requireCleanupObserved) {
      if (cleanupCallCount <= 0 || capturedObservedIds.length <= 0 || removedIds.length <= 0) {
        throw new Error(
          `cleanup replay divergence (in-memory): ${JSON.stringify({
            cleanupCallCount,
            capturedObservedIdsCount: capturedObservedIds.length,
            removedIdsCount: removedIds.length,
            thresholdHandleCallCount,
            thresholdReachedInReplay,
            lastThresholdResult,
            lastThresholdSnapshot,
            capturedThresholdCleanup: capturedOutput?.details?.thresholdCleanup,
          })}`,
        );
      }
    }

    if (typeof options?.expectedMinRemaining === 'number') {
      expect(capturedMinRemaining).toBe(options.expectedMinRemaining);
    }

    expect(extraRemoved).toEqual([]);
    expect(missingObservedStillPresent).toEqual([]);

    if (typeof capturedMinRemaining === 'number') {
      const postTokens = (om as any).tokenCounter.countMessages(postMessages as any);
      expect(postTokens).toBeGreaterThanOrEqual(capturedMinRemaining);
    }
  };

  it('should replay latest-cwd activation step against live logic without observed-ID drift', async () => {
    const fixtureDir = join(
      __dirname,
      '..',
      '__fixtures__',
      'repro-captures',
      'latest-cwd-activation-1772669801367',
      '1772669801367-step-5-2b157b7d-c666-43ba-bb44-d2c98c173875',
    );

    await runReproStepReplay(fixtureDir);
  });

  it('should replay collapse-to-0p3k step fixture against live logic', async () => {
    const fixtureDir = join(
      __dirname,
      '..',
      '__fixtures__',
      ...
      '1772668823377-step-23-97fbc5ab-b157-4eba-9834-3bd9dbeaf7a5',
    );

    await runReproStepReplay(fixtureDir);
  });

  it('should replay drift-collapse step-11 fixture against live logic', async () => {
    const fixtureDir = join(
      __dirname,
      '..',
      '__fixtures__',
      'repro-captures',
      'drift-collapse-step11-1772672962337',
      '1772672996421-step-11-e405d0fc-da5e-4657-8fc8-1c910b1db326',
    );

    await runReproStepReplay(fixtureDir, {
      requireThresholdReached: true,
      requireCleanupObserved: true,
      expectedMinRemaining: 2000,
    });
  });
});
