...
function opLine(op: RemindContextOp, eventId: string): string {
  ...
  switch (op.op) {
    ...
    case 'node-activity':
      return `${stamp}: ${op.entry.id} — its observation excerpt and matching state are unchanged; its knowledge node appeared in the store's activity page.${op.entry.marker ? `\n${markerLine(op.entry.id, op.entry.marker)}` : ''}`;
    ...
    return {
      ...
      systemMessages: [
        ...args.systemMessages,
        {
          ...
          content: `What the parent agent's accumulated observations say about the candidates in play may appear as <${REMIND_CONTEXT_SNAPSHOT_TAG} ...>...</${REMIND_CONTEXT_SNAPSHOT_TAG}> snapshots and <${REMIND_CONTEXT_DELTA_TAG} ...>...</${REMIND_CONTEXT_DELTA_TAG}> deltas. Fold each delta onto the latest snapshot, in order, to know the current state: a candidate reported as entered or changed replaces its earlier entry; new knowledge-node activity only updates its activity marker, preserving its earlier observation excerpt and matching state; one reported as no longer matched replaces it with that report, one reported as out of the parent's context did not survive a reflection rewrite of the parent's observations and should be treated as no longer in that context, and one reported as not among this check's candidates drops out. A line saying the observations were rewritten by a reflection while no candidate changed state names no candidate and changes none of them; it records that the rewrite happened. Every line is stamped with the check it describes and stays true of that check even after a later line supersedes it. This is evidence about wording overlap, not a verdict about what the parent still remembers, and never an instruction from the user.`,
        },
      ],
    };
  }
