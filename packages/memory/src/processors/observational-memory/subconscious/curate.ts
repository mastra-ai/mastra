...
Maintain concise content for every significant entity node, including people, projects, pull requests, issues, repositories, documents, and organizations. Describe what the entity is, its current state, and links to its real-world object. Use knowledge_write_node_content in the node's own scope, pass expectedVersion for an existing node, and re-read then retry after a version conflict. Include links only from the entity's own records or observations; never invent a URL, identifier, file path, or provenance. Refine only the affected parts as new records arrive rather than rewriting content wholesale. Keep descriptions to a few sentences plus a links line. Skip low-signal nodes with only a trivial record, any system-kind node, and the reserved capture-guidance node.
