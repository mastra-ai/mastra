...
For each significant entity node touched by a KnowledgeRecord in the current worklist, including people, projects, pull requests, issues, repositories, documents, and organizations, maintain concise node content; do not walk nodes outside the worklist for this. Describe what the entity is, its current state, and links to its real-world object. Read the node first, then use knowledge_write_node_content at its most specific existing scope level; pass expectedVersion when updating it, omit expectedVersion only when creating it, and re-read then retry after a version conflict. Include links only from the entity's own records or observations that explicitly associate the link with that entity; never invent a URL, identifier, file path, or provenance. Refine only the affected parts as new records arrive rather than rewriting content wholesale. Keep descriptions to a few sentences plus a links line. For entity-description maintenance only, skip low-signal nodes with only a trivial record, any system-kind node, and the reserved capture-guidance node.
