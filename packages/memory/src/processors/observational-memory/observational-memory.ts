    ...
    // ════════════════════════════════════════════════════════════════════════
    {
      ...
      if (reproCaptureEnabled && preRecordSnapshot && preMessagesSnapshot && preSerializedMessageList) {
        writeProcessInputStepReproCapture({
          ...
          preBufferedChunks: getBufferedChunks(preRecordSnapshot),
          preContextTokenCount: this.tokenCounter.countMessages(preMessagesSnapshot),
          preSerializedMessageList,
          postBufferedChunks: getBufferedChunks(freshRecord),
...
${formattedMessages}
  ...
  private async tryActivateBufferedObservations(
    ...
    const refreshedChunks = messageList
      ? this.refreshBufferedChunkMessageTokens(freshChunks, messageList)
      : freshChunks;
    ...
    omDebug(
      `[OM:tryActivate] swapping: freshChunks=${refreshedChunks.length}, bufferActivation=${bufferActivation}, activationRatio=${activationRatio}, forceMax=${forceMaxActivation}, totalChunkTokens=${refreshedChunks.reduce((s, c) => s + (c.tokenCount ?? 0), 0)}`,
    );
    const activationResult = await this.storage.swapBufferedToActive({
      ...
      forceMaxActivation,
      bufferedChunks: refreshedChunks,
    });
    omDebug(
