...
import { formatToolResultForTokenCounting, resolveToolResultValue } from './tool-result-helpers';
...
export class TokenCounter {
  ...
  private countMultimodalToolResultContent(part: CacheablePart, toolResult: unknown): number | undefined {
    ...
    const countJsonContentPart = (contentPart: Record<string, unknown>) => {
      const formatted = formatToolResultForTokenCounting(contentPart);
      ...
      if (state === 'result') {
        ...
        if (resultForCounting !== undefined) {
          ...
          if (contentTokens !== undefined) {
            ...
            const formattedResult = formatToolResultForTokenCounting(resultForCounting);
