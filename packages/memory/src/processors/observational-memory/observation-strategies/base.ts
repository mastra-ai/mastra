...
export abstract class ObservationStrategy {
  ...
  protected async streamMarker(marker: { type: string; data: unknown }): Promise<void> {
    // Prefer sendDataPartSignal (reliable even when idle), fall back to writer.custom().
    if (this.opts.sendDataPartSignal) {
      await this.opts.sendDataPartSignal(marker as { type: `data-${string}`; data: unknown }).catch(() => {});
