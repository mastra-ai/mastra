...
import type { AgentConfig, ConversationExchange } from './types';
import { cleanMessagesForEncoding } from './utils';

// ============================================================================
// Observer System Prompt
// ============================================================================

export const OBSERVER_INSTRUCTIONS = `You are the memory consciousness of an AI assistant. Your observations will be the ONLY information the assistant has about past interactions with this user.
...
export const DEFAULT_OBSERVER_MODEL = 'google/gemini-2.5-flash';

export const DEFAULT_OBSERVER_MODEL_SETTINGS = {
  temperature: 0.3,
  maxOutputTokens: 100_000,
};

export const DEFAULT_OBSERVER_PROVIDER_OPTIONS = {
  google: {
    thinkingConfig: {
      thinkingBudget: 215,
 ...
 */
export function buildObserverUserPrompt(exchange: ConversationExchange, existingObservations: string): string {
  let prompt = ``;
  ...
  const hasAgent = exchange.relevantMessages.some(m => m.role === `assistant`);

  if (hasUser && hasAgent) {
    prompt +=
      'Make new observations about the following conversational exchange between the user and assistant. Make sure you make observations about the user message AND the assistant response:\n\n';
  } else if (hasUser) {
    prompt +=
      'Make new observations about the following user messages. Be aware that no assistant messages are present:\n\n';
  } else if (hasAgent) {
    prompt +=
      'Make new observations about the following assistant messages. Be aware that no user messages are present:\n\n';
  }

  // Encode messages, removing provider metadata to reduce tokens
  const cleanedMessages = cleanMessagesForEncoding(exchange.relevantMessages);
  const toonHistory = encode(cleanedMessages);
  prompt += `MESSAGE_HISTORY: ${toonHistory.replaceAll(`role: user`, `role: user (IMPORTANT)`)}\n\n`;

  if (hasUser) {
    const lastUserMessage = [...exchange.relevantMessages].reverse().find(m => m.role === `user`);
    prompt += `Note that the most recent user message is always important and should have special consideration in regards to the conversation and observations. The user calls the shots so we need to make sure we keep on track with what they're saying specifically, above all else.\nHere is a copy of the most recent message to make it clear what to do: ${encode(lastUserMessage)}\n\n`;
  }
...
// ============================================================================
// Observer Agent Factory
// ============================================================================

/**
 * Create an observer agent with the given configuration
 */
export function createObserverAgent(config?: AgentConfig): Agent {
  return new Agent({
    id: 'observational-memory-observer',
    name: 'Observer Agent',
    instructions: OBSERVER_INSTRUCTIONS,
    model: config?.model || DEFAULT_OBSERVER_MODEL,
  });
}

/**
 * Get the model settings for the observer agent
 */
export function getObserverModelSettings(config?: AgentConfig): { temperature: number; maxOutputTokens: number } {
  return {
    temperature: config?.modelSettings?.temperature ?? DEFAULT_OBSERVER_MODEL_SETTINGS.temperature,
    maxOutputTokens: config?.modelSettings?.maxOutputTokens ?? DEFAULT_OBSERVER_MODEL_SETTINGS.maxOutputTokens,
  };
}

/**
 * Get the provider options for the observer agent
 */
export function getObserverProviderOptions(config?: AgentConfig): Record<string, unknown> {
  return config?.providerOptions ?? DEFAULT_OBSERVER_PROVIDER_OPTIONS;
}
