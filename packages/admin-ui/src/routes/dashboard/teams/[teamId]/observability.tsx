...
export function TeamObservability() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-neutral9">Observability</h1>
      </div>

      <p className="text-neutral6 mb-6">Select a project to view its observability data.</p>
      ...
      {projects?.data && projects.data.length > 0 ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {projects.data.map(project => (
            <Link
              key={project.id}
              to={`/projects/${project.id}/observability`}
            ...
            >
              ...
              <p className="text-sm text-neutral6 mt-1">View traces, logs, and metrics</p>
            </Link>
          ))}
        </div>
      ) : (
        <div className="p-6 bg-surface2 rounded-lg border border-border text-center">
          <p className="text-neutral6">No projects yet. Create a project first to view observability data.</p>
          <Link
