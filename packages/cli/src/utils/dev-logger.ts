...
export class DevLogger {
  ...
  ready(host: string, port: number, basePath: string, startTime?: number, https?: HTTPSOptions): void {
    let protocol = 'http';
    ...
    console.info('');
    console.info(`${pc.dim('│')} ${pc.bold('Playground:')} ${pc.cyan(`${protocol}://${host}:${port}${basePath}`)}`);
