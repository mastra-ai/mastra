...
import { dependencyVersionSpec, generateScaffold } from '../../../scripts/generate-factory-scaffold.mjs';
...
function createFixture({
  ...
  coreVersion = '2.0.0',
  mastraVersion = '3.0.0',
}: { cliVersion?: string; coreVersion?: string; mastraVersion?: string } = {}) {
  ...
  writeJson(path.join(packageRoot, 'package.json'), { name: 'mastra', version: cliVersion });
...
describe('generateScaffold', () => {
  it('copies authoritative source files byte-for-byte and generates deterministic stable dependencies', () => {
    ...
    expect(JSON.stringify(manifest)).not.toMatch(/(?:alpha|beta|rc)\./);
    expect(first['.env.example']).toContain('# FOO=');
    expect(first.npmrc).toBeUndefined();
  });

  it('uses exact matching-channel versions when workspace packages are prereleases', () => {
    const fixture = createFixture({
  ...
  it('rejects prerelease dependencies in a stable CLI release', () => {
