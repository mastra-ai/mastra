...
index 54cbad7183ef2c548a55d4d424891321ea8de18b..f1963616a2b71950b7a15fa8793f0516e2fcda90 100755
--- a/dist/index.js
+++ b/dist/index.js
@@ -1524,12 +1524,35 @@ async function start(options = {}) {
     commands.push("index.mjs");
     const server = spawn("node", commands, {
       cwd: outputPath,
-      stdio: "inherit",
...
+        if (stderrBuffer.includes("ERR_MODULE_NOT_FOUND")) {
+          const packageNameMatch = stderrBuffer.match(/Cannot find package '([^']+)'/);
+          const filePathMatch = stderrBuffer.match(/imported from (.+\.mjs)/);
+          
+          const packageName = packageNameMatch ? packageNameMatch[1] : null;
+          const filePath = filePathMatch ? filePathMatch[1] : null;
+          
+          logger.error("Module not found error - this is a custom error message placeholder\n\nDEBUG: packageName = " + packageName + "\nDEBUG: filePath = " + filePath + "\n\nOriginal error:\n" + stderrBuffer.trim());
+        } else {
+          logger.error(stderrBuffer.trim());
+        }
+        process.exit(code);
