...
export async function listSkillsCommand(dir?: string) {
  ...
  const targetDir = dir ? (isAbsolute(dir) ? dir : join(cwd, dir)) : join(cwd, '.mastra/skills');
  ...
  try {
    ...
    const skillDirs = entries.filter(entry => entry.isDirectory()).map(entry => join(targetDir, entry.name));
