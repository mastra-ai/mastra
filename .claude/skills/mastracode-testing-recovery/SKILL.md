...
- Drive runtime behavior through the TUI whenever the behavior is user-visible or user-triggered: terminal input, slash commands, keyboard navigation, AIMock-backed model/tool calls, and sanitized DB/config seeding before launch. Do not use Harness internals like `harness.emit()`, `getDisplayState()` mutation, or runtime thread APIs (`createThread()`, `getCurrentThreadId()`) as shortcuts for behavior a user can trigger through `/new`, `/threads`, or normal startup. Notification/state signal scenarios are the exception because those events originate outside terminal input; use the public agent signal APIs (`sendNotificationSignal`, `sendStateSignal`) there.
- For model-driven tool behavior, use AIMock's built-in `response.toolCalls` fixture shape (`{ name, arguments, id? }`) and `match.hasToolResult: true` follow-ups instead of manually emitting Harness tool/task events.
