...
- A `font-mono` class anywhere outside `packages/playground-ui/src/ds` (lint rejects it): use `<Txt font="mono">` for identifiers and timestamps, `InlineCode` or `CodeBlock` for code, `tabular-nums` for numbers. `<pre>` and `<code>` are already mono
- Mono on text a person wrote or reads as language (labels, headings, status, prose): mono is for machine identifiers (IDs, hashes, log lines) and timestamps (dates and times of day). Numbers such as durations, counts, and costs use the body face with `tabular-nums`. See the Typography section of `packages/playground-ui/README.md`
