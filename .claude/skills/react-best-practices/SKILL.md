...
- Use PascalCase components for JSX-returning helpers; keep lowercase helpers for non-JSX values. Runtime tags are fine, but carry a heading level in context and let the component pick the tag — a tag read off `useContext` is reported as a component created during render (`structure-component-naming`)
- Derive props/params instead of accepting a value computable from another arg (`structure-derive-dont-duplicate`)
- Extract complex derived logic into named locals plus predicates or pure helpers with early returns: oversized conditions, nested ternaries, ternaries that compute instead of picking (multi-line branches, or an `as` cast re-asserting what the condition tested), fallback chains, hook calls buried inside the expression consuming them, and `let`-based render prep are code smells, in render prep and in hook options, request builders, config maps, and reducers alike (`structure-complex-derived-logic`)
