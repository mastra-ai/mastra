...
- Extract complex derived logic into named locals plus predicates or pure helpers with early returns: oversized conditions, nested ternaries, ternary branches that span several lines, fallback chains, and `let`-based render prep are code smells, in render prep and in hook options, request builders, and reducers alike (`structure-complex-derived-logic`)
