...
| `structure-complex-derived-logic`        | Extract Complex Derived Logic                         | MEDIUM-HIGH | Treat oversized conditions, nested ternaries, multi-line ternary branches, fallback chains, and `let`-based render prep as smells; move them into named locals and helper returns. | `references/rules/structure-complex-derived-logic.md`        |
