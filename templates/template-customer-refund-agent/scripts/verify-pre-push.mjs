...
} from "./distributable-snapshot.mjs";
...
function run(command, args, cwd, env) {
  ...
  if (result.status !== 0)
    throw new Error(`${command} ${args.join(" ")} failed in clean snapshot.`);
}
...
try {
  ...
  env.LOCAL_AUTH_SIGNING_KEY =
    "pre-push-synthetic-signing-key-at-least-32-characters";
