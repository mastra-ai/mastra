...
async function runtime() {
  ...
  vi.stubGlobal(
    ...
    async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (
        path === "/articles" &&
        new URL(request.url).searchParams.get("starting_after") ===
          "published-first"
      )
        return Response.json({
          data: [
      ...
      if (path.startsWith("/articles/")) {
        ...
        return Response.json({
          ...
          updated_at:
            (id === "published-second" ? 1_700_000_002 : 1_700_000_000) +
            articleRevision,
        });
      }
...
afterEach(async () => {
  ...
  await Promise.all(
    databases.splice(0).map((file) => rm(file, { force: true })),
  );
...
describe("Phase 005 Intercom registered lifecycle", () => {
  it("drives signed create/follow-up/replay through the registered HTTP and ingest workflows, preserving owner and ordered external effects", async () => {
    ...
    await recoverLocalWorkflows(
      (await import("../../src/mastra/index")).mastra,
      10,
      caseStore,
    );
    ...
    expect((supportCase.metadata as { ownerId?: string }).ownerId).toBe(
      "intercom:local-demo:contact:lead-a",
    );
    expect(await caseStore.turns(caseId)).toHaveLength(2);
    ...
    expect(outbox.rows.map((row) => row.operation)).toEqual([
      "reply",
      "note",
      "status",
      "ticket",
    ...
    await knowledgePublicationStore.rollback(
      knowledgeBinding,
      publication.generationId!,
    );
    expect(
      await knowledgePublicationStore.activeGeneration(knowledgeBinding),
    ).toBe(publication.generationId);
    serveEmptyArticles();
    await expect(publishKnowledge(knowledgeBinding)).rejects.toThrow(
      "Knowledge candidate has no documents.",
    );
    expect(
      await knowledgePublicationStore.activeGeneration(knowledgeBinding),
    ).toBe(publication.generationId);
