...
it.each([
  ...
  "leaves database files unchanged for %s",
  async (_name, overrides, filename) => {
    const { directory, env } = await fixture();
    ...
    Object.assign(
      env,
      typeof overrides === "function" ? overrides(directory) : overrides,
      { LOCAL_DEMO_DATABASE_URL: `file:${database}` },
    );
    if (_name === "external mode")
      env.LOCAL_DEMO_CLIENT_DATABASE_URL = `file:${join(directory, "client.db")}`;
    ...
    ).resolves.toEqual(artifacts.map(() => "unchanged"));
  },
);
...
it("refuses a non-SQLite selected main without deleting either database pair", async () => {
  ...
  await expect(
    Promise.all(artifacts.map((path) => readFile(path))),
