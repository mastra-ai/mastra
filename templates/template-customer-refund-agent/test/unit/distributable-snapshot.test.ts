...
describe("distributable snapshot", () => {
  it("copies only the allowlisted distributable files without requiring Git", async () => {
    ...
    await expect(
      readFile(join(snapshot.destination, "README.md"), "utf8"),
    ).resolves.toBe("fixture\n");
    await expect(access(join(snapshot.destination, ".env"))).rejects.toThrow();
    await expect(
      access(join(snapshot.destination, "src", ".env.local")),
    ).rejects.toThrow();
    await expect(
      access(join(snapshot.destination, "test", "private.db-wal")),
    ).rejects.toThrow();
  });
  ...
  it("fingerprints whitespace changes and rejects them before the clean runner starts", async () => {
    ...
    await expect(assertNoDistributedWhitespace(root)).rejects.toThrow(
  ...
  it("fails before copying an allowed symlink", async () => {
    ...
    await expect(createDistributableSnapshot(root)).rejects.toThrow(
      "rejects symlink",
    );
  });
});
