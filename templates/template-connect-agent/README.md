...
- **Scope the toolset** — pass per-integration options to `connect()` in `src/mastra/tools/connect.ts`, e.g. `integrations: { linear: { allowTools: ['linear_list_issues', 'linear_create_issue'] } }` to restrict what the agent can do.
