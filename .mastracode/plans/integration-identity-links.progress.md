...
- Source (b) API rosters were initially deferred out of Phase 3, then landed in a later iteration on this branch: each identity-capable integration (standalone and platform variants) now discovers candidates from its provider member API (GitHub org members, Linear `users` GraphQL, Jira user search, IncidentIO users, GitLab members, Slack `users.list`) and merges them with observed comment authors via `mergeCandidates`. The capability contract is unchanged.
...
**Landed**:
- `mastracode/factory-ui/src/ui/domains/settings/components/IdentityClaimsSection.tsx` — initially a per-integration expander; reworked in a later iteration on this branch into a single consolidated searchable multi-select across all identity-capable integrations, driven by the merged `/web/identity` feed.
...
- ~~**Source (b) rosters** — deferred from Phase 3.~~ Landed in a later iteration on this branch: every identity-capable integration now discovers candidates from its provider member API behind `mergeCandidates` (see Phase 3 deviations and the Source (b) inventory below).
...
Source (a) is the shared `work_item_comments.author_external` scan for every integration. Source (b) provider rosters landed in a later iteration on this branch.

| Integration | Source (a) observed field | Source (b) status | Notes |
| --- | --- | --- | --- |
| GitHub (standalone) | `work_item_comments.author_external.platform = 'github'` | implemented | `GET /orgs/{org}/members` via installation tokens. |
| GitHub (platform) | same as standalone | implemented | Installations discovered from Platform; members deduped by login. |
| Linear (standalone) | `work_item_comments.author_external.platform = 'linear'` | implemented | GraphQL `users` via `loadConnection` access token. |
| Linear (platform) | same as standalone | implemented | Proxied through Platform. |
| Jira (standalone) | `work_item_comments.author_external.platform = 'jira'` | implemented | `GET /rest/api/3/users/search` via deployment `JIRA_BASIC_AUTH`. |
| Jira (platform) | same as standalone | implemented | Per-connection contexts proxied through Platform. |
| IncidentIO (standalone) | `work_item_comments.author_external.platform = 'incidentio'` | implemented | `GET /v2/users` via `DIRECT_CONNECTION_TOKEN`. |
| IncidentIO (platform) | same as standalone | implemented | Proxied through Platform. |
| GitLab | `work_item_comments.author_external.platform = 'gitlab'` | implemented | Member rosters via the GitLab API client. |
| Slack | `work_item_comments.author_external.platform = 'slack'` | implemented | `users.list` via existing bot token. |
