...
export class MastraRBACGoogle implements IRBACProvider<GoogleUser> {
  ...
  constructor(options: MastraRBACGoogleOptions) {
    ...
    if (!options.accessToken && !options.getAccessToken && !options.serviceAccount) {
      throw new Error(
        'Google Workspace Directory authentication is required. Provide accessToken, getAccessToken, or serviceAccount.',
      );
    }
