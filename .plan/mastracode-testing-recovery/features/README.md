...
| TUI | Quiet mode | #13556, #16771, #16807, #16839 | settings preference + TUI render state + rollout prompt + compact tool grouping/previews + quiet task summaries + continuation labels/glyph contrast | Partial | High | [page](./tui/quiet-mode.md) |
...
| Goals | Persistent `/goal` mode | #16065, #16322, #16340, #16654, #16690, #16676 | thread goal metadata + GoalManager + active pursuit timer + judge agent/defaults + waiting/retry/resume judge decisions + TUI judge/input-lock/status-line state + approved-plan goal start/return-to-plan ordering | Partial | High | [page](./goals/persistent-goals.md) |
