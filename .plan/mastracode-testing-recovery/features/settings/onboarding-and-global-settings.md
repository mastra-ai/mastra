...
- `mastracode/src/onboarding/settings.ts` — `settings.json` schema, migrations, defaults, OM threshold/caveman defaults, shell passthrough defaults/parsing, pack resolution, and custom provider parsing.
...
- `mastracode/src/onboarding/__tests__/settings.test.ts` — settings parsing, migrations, pack resolution, thread active pack inference, quiet-mode defaults/rollout, shell passthrough parsing, and preview-line normalization.
