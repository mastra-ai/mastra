...
- Direct `/api-keys` provider ordering and multi-provider delete isolation are covered by `api-key-multi-provider-delete`; add/storage plus stored-key delete with real-env preservation are covered by real PTY e2e. Remaining `/api-keys` settings-submenu breadth is navigation coverage rather than provider storage semantics.
