...
- `/models` share/targeted-edit flow through real TUI overlay, not only helper functions; activation and persisted custom-pack defaults are now covered by `models-pack-activation-persistence`, import collision/overwrite is covered by `custom-pack-import-overwrite`, and provider deletion persistence is covered by `custom-provider-delete`.
