...
| Signal transport settings | Global settings `signals` block (`unixSocketPubSub`, `experimentalGithubSignals`) plus optional `MastraCodeConfig` overrides | `createMastraCode()` signal PubSub selection, cross-process PubSub/thread-lock behavior, GitHub signal polling |
