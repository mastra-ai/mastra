...
- Headless startup with Memory Gateway base URL/env values is covered by `mastracode/src/__tests__/index.test.ts`; active custom model pack startup restoration is covered by `settings-startup-model-restore`, and TUI startup restore for enabled AgentBrowser/CDP settings is covered by `browser-startup-restore`. Remaining startup breadth is headless browser-settings projection plus Browserbase wizard variants.
- Direct `/browser` wizard AgentBrowser/CDP save plus storage-state export is covered by `browser-wizard-export`; quick-command profile/CDP mutual exclusion, custom executable persistence, and profile clear cleanup are covered by `browser-settings-persistence`, and startup attach/status/tool injection is covered by `browser-startup-restore`. Remaining browser settings breadth is Browserbase variants.
