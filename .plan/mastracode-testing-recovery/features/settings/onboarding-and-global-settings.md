...
- Must preserve: skipped/completed onboarding state, active model/OM pack IDs, custom packs, provider access detection, stored-key env precedence, YOLO preference, browser settings, OM observe-attachments/threshold/caveman settings, masked sensitive settings prompts, removed Claude Max warning state, and settings migrations.
...
- `mastracode/src/onboarding/settings.ts` — `settings.json` schema, migrations, defaults, OM threshold/caveman defaults, pack resolution, and custom provider parsing.
...
- `mastracode/src/tui/mastra-tui.ts` — startup onboarding trigger, runtime provider access refresh, auto-update prompt checks, modal setup rendering, and `applyOnboardingResult()` runtime persistence.
...
- `mastracode/src/__tests__/index.test.ts` — startup settings plumbing, stored API-key env loading, caveman startup restore, and `observeAttachments` default/restore behavior are partially mocked.
