...
- First-run onboarding wizard breadth still missing for login refresh; built-in OpenAI mode/OM pack completion, custom-pack completion, YOLO persistence, skipped-state cleanup, and persisted setup settings are covered by `setup-completion-persistence` and `setup-custom-pack-completion`.
- Reload after `/setup`: active custom pack startup footer restoration is covered by `settings-startup-model-restore`; `/om` threshold startup restore is covered by `om-threshold-persistence`; thread metadata, subagent defaults, and OM model defaults still need reload parity.
- Direct Mastra Code `/om` observer/reflector model override regression remains; threshold restore/persistence is covered by `om-threshold-persistence`, and caveman/observe-attachments global/thread persistence is covered by real PTY e2e.
...
- Headless startup with browser settings and Memory Gateway base URL/env values; active custom model pack startup restoration is covered by `settings-startup-model-restore`.
