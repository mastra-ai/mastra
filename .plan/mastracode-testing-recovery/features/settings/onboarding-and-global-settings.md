...
- Reload after `/setup`: active custom pack startup footer restoration is covered by `settings-startup-model-restore`; `/om` threshold startup restore and role-specific observer/reflector model override reload are covered by `om-threshold-persistence` and `om-model-override-reload`; subagent defaults and remaining OM model-pack defaults still need reload parity.
