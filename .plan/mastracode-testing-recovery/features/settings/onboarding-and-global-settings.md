...
- `mastracode/src/__tests__/index.test.ts` — startup settings plumbing, stored API-key env loading, Memory Gateway stored-key/base-URL env hydration plus gateway-backed provider auth checks, registry provider API-key access with multi-env `apiKeyEnvVar` entries, caveman startup restore, `observeAttachments` default/restore behavior, and GitHubSignals processor wiring are partially mocked.
...
- `mastracode/scripts/mc-e2e/scenarios/api-key-multi-provider-delete.ts` — partial real PTY coverage for `/api-keys` provider ordering and multi-provider delete isolation: seeds stored `302ai` and Anthropic keys, proves the sorted provider list shows `302ai` before `anthropic`, deletes only the selected `302ai` stored key, and verifies Anthropic auth/env projection remains intact while the deleted provider is cleared.
- `mastracode/scripts/mc-e2e/scenarios/settings-api-keys-navigation.ts` — partial real PTY coverage for the `/settings` API Keys entry: opens Settings, selects the API Keys row, and proves the Settings submenu hands off to the same API-key management overlay.
...
- `mastracode/scripts/mc-e2e/scenarios/login-dialog-masked-input.ts` — partial real PTY coverage for provider login prompts: drives `/login` into the Anthropic login dialog, verifies the authorization-code prompt masks typed text, and proves the raw code still reaches OAuth storage.
...
- `mastracode/scripts/mc-e2e/scenarios/browser-settings-persistence.ts` — partial real PTY coverage for `/browser set`/`clear`: sets CDP URL, switches to a profile and proves CDP is cleared while profile preservation is enabled, sets a custom executable path, clears the profile, then runs clear-all and proves `settings.json` resets to the default disabled Stagehand browser block.
...
- `mastracode/scripts/mc-e2e/scenarios/browser-active-pending-status.ts` — partial real PTY settings/status coverage for active-vs-pending browser configuration: starts with an enabled AgentBrowser/CDP runtime, changes saved CDP settings without `/browser on`, and proves `/browser status` renders active runtime settings separately from pending file settings plus apply guidance.
- `mastracode/scripts/mc-e2e/scenarios/browser-wizard-export.ts` — partial real PTY settings coverage for the interactive `/browser` wizard: selects AgentBrowser/CDP through the modal flow, proves saved settings/status projection, and verifies `/browser export storageState` writes the requested storage-state file.
- `mastracode/scripts/mc-e2e/scenarios/browser-wizard-browserbase.ts` — partial real PTY settings coverage for the interactive `/browser` wizard Browserbase path: selects Stagehand Browserbase, verifies credential guidance, skips local launch/profile prompts, clears stale CDP/profile/executable settings, and proves Browserbase settings persist without credentials written to settings.
- `mastracode/scripts/mc-e2e/scenarios/browser-profile-provider-mismatch.ts` — partial real PTY settings coverage for profile/provider mismatch: seeds a Stagehand-marked profile, verifies AgentBrowser reuse prompts for confirmation, proves `No` preserves settings/marker, then proves `Yes` persists AgentBrowser settings and rewrites the profile marker.
...
- `mastracode/scripts/mc-e2e/scenarios/om-pack-startup-restore.ts` — partial real PTY startup/reload coverage for built-in OM pack defaults: seeds `activeOmPackId: "openai"` before launch, verifies `/om` restores both observer and reflector models to the built-in OpenAI Mini model, and proves persisted settings remain pack-backed rather than role-overridden.
...
- Reload after `/setup`: active custom pack startup footer restoration is covered by `settings-startup-model-restore`; model-pack-backed Explore subagent default restoration is covered by `subagent-model-startup-restore`; `/om` threshold startup restore, built-in OM pack default reload, and role-specific observer/reflector model override reload are covered by `om-threshold-persistence`, `om-pack-startup-restore`, and `om-model-override-reload`. Remaining reload breadth is browser/settings wizard variants and deeper custom-pack edge cases.
...
- Direct MaskedInput regression for storage connection strings is covered by strengthened `storage-settings`, proving render output is masked while the submitted value remains raw; `/api-keys` masked input is covered by real PTY e2e, and login-dialog masked authorization-code prompts are covered by `login-dialog-masked-input`.
- Headless startup with Memory Gateway base URL/env values is covered by `mastracode/src/__tests__/index.test.ts`; active custom model pack startup restoration is covered by `settings-startup-model-restore`, and TUI startup restore for enabled AgentBrowser/CDP settings is covered by `browser-startup-restore`. Remaining startup breadth is headless browser-settings projection plus Browserbase startup variants.
- Direct `/browser` wizard AgentBrowser/CDP save plus storage-state export is covered by `browser-wizard-export`; Browserbase wizard persistence and stale local launch-option cleanup are covered by `browser-wizard-browserbase`; quick-command profile/CDP mutual exclusion, custom executable persistence, and profile clear cleanup are covered by `browser-settings-persistence`, and startup attach/status/tool injection is covered by `browser-startup-restore`. Active-vs-pending projection is covered by `browser-active-pending-status`; remaining browser settings breadth is startup variants.
- Direct `/api-keys` provider ordering and multi-provider delete isolation are covered by `api-key-multi-provider-delete`; add/storage plus stored-key delete with real-env preservation are covered by real PTY e2e, and `/settings` submenu handoff is covered by `settings-api-keys-navigation`.
