...
- Success looks like: selected pack updates current run, thread metadata, global settings, subagent defaults, OM models, and status line; API-key-only providers, including stored keys and non-hardcoded registry providers, do not get blocked at the auth step.
- Must preserve: skipped/completed onboarding state, active model/OM pack IDs, custom packs, provider access detection, stored-key env precedence, YOLO preference, OM observe-attachments setting, and settings migrations.
...
| Provider access | Shared AuthStorage + env/API-key detection + stored `apikey:<provider>` entries + provider registry `apiKeyEnvVar` | Onboarding auth gate, pack filtering, model prompts, provider-specific OAuth modules |
...
- `mastracode/src/auth/storage.ts` — provider post-login default model IDs plus stored API-key helpers.
