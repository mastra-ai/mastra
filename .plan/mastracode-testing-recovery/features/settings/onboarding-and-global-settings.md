...
- Direct MaskedInput regression for storage connection strings is covered by strengthened `storage-settings`, proving render output is masked while the submitted value remains raw; `/api-keys` masked input is covered by real PTY e2e. Remaining MaskedInput breadth is login-dialog-specific.
