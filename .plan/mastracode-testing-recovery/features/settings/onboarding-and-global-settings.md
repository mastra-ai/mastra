...
- `/models` share/import-cancel/custom-pack completion breadth through real TUI overlay, not only helper functions; activation and persisted custom-pack defaults are covered by `models-pack-activation-persistence`, import collision/overwrite is covered by `custom-pack-import-overwrite`, targeted active-pack rename/edit is covered by `custom-pack-rename-active`, and provider deletion persistence is covered by `custom-provider-delete`.
