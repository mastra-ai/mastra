...
- Direct `/browser` wizard AgentBrowser/CDP save plus storage-state export is covered by `browser-wizard-export`; Browserbase wizard persistence and stale local launch-option cleanup are covered by `browser-wizard-browserbase`; quick-command profile/CDP mutual exclusion, custom executable persistence, and profile clear cleanup are covered by `browser-settings-persistence`, and startup attach/status/tool injection is covered by `browser-startup-restore`. Remaining browser settings breadth is active-vs-pending projection and startup variants.
