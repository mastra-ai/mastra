...
- `mastracode/src/tui/commands/__tests__/browser.test.ts` — direct `/browser on` shield proving enabled settings create a browser, attach it to static and state-derived mode agents, record `activeBrowserSettings`, persist profile provider metadata, and project active-vs-pending `/browser status` profile/executable/storage-state drift including storage-state-only config drift.
...
- `mastracode/scripts/mc-e2e/scenarios/browserbase-startup-restore.ts` — real PTY/TUI e2e coverage for enabled Stagehand Browserbase settings restored during startup, including `/browser status` Browserbase projection and active-vs-pending drift after saving a CDP URL without `/browser on`.
- `mastracode/scripts/mc-e2e/scenarios/browser-active-pending-status.ts` — real PTY/TUI e2e coverage for `/browser status` config drift: after changing saved CDP settings without applying them, status renders `Browser (active)`, `Pending changes (not yet applied)`, both active/pending CDP URLs, and `/browser on` apply guidance.
...
- Startup restore for enabled AgentBrowser/CDP settings is covered by `browser-startup-restore`, and Stagehand Browserbase startup restore plus drift projection is covered by `browserbase-startup-restore`; quick-setting profile/executable persistence and clear-all reset are covered by `browser-settings-persistence`; wizard AgentBrowser/CDP save/export is covered by `browser-wizard-export`, and Browserbase wizard persistence is covered by `browser-wizard-browserbase`. Richer active-state projection for profile/executable/storage-state fields, including storage-state-only drift detection, is covered by `browser.test.ts`. Optional live external-provider smoke remains deferred as non-hermetic breadth.
- Profile provider mismatch warnings/cancel/proceed are covered by `browser-profile-provider-mismatch`; active-vs-pending status/config-drift rendering is covered by `browser-active-pending-status`. Baseline `/browser status` disabled/enabled transcript is covered by `integration-commands`.
- Browser reload/history parity beyond saved-settings startup restore and optional live external-provider smoke remain explicit non-blocking integration breadth; deterministic CDP attach/tool injection and active/pending status projections are covered.
