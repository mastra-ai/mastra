...
- `mastracode/src/index.ts` — exports `resolveModel`, accepts typed `MastraCodeConfig` overrides including `memory` and `configDir`, and passes either the override or default dynamic memory into Harness from `createMastraCode()`.
