...
- `mastracode/src/agents/memory.ts` — dynamic OM memory factory and Mastra Code defaults; current source reads persisted thresholds from harness state, sets observation `bufferActivation` to `2000` for thread scope, enables `threadTitle: true` for MC, disables async buffering for resource scope, passes `remapForCodexOAuth: true` for observer/reflector model resolution, and instructs OM to ignore `dynamic-agents-md` system reminders.
...
- `mastracode/src/tui/commands/om.ts` — `/om` settings modal wiring, including threshold/thread/global persistence and observe-attachments persistence.
...
- [Onboarding and global settings](../settings/onboarding-and-global-settings.md) — first-run setup selects default OM pack/model and settings store persisted OM threshold defaults.
...
- State is split across harness state, settings, thread settings, storage, and display state; threshold writes must keep thread overrides and global defaults intentionally separate.
