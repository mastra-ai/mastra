...
| Idle/provider-change activation | OM config `activateAfterIdle` (`number`, duration string, `auto`, or `false`) + `activateOnProviderChange` + provider/model cache-retention heuristics from `activation-ttl.ts`; Mastra Code defaults to `auto` and provider-change activation for observations | Core observation/reflection activation, Harness `om_activation` events, TUI OM markers |
...
- `mastracode/src/agents/memory.ts` — dynamic OM memory factory and Mastra Code defaults; current source reads persisted thresholds/caveman mode from harness state, sets observation `bufferActivation` to `2000` for thread scope, defaults `activateAfterIdle: 'auto'` and `activateOnProviderChange: true`, enables `threadTitle: true` for MC, disables async buffering for resource scope, passes `remapForCodexOAuth: true` for observer/reflector model resolution, instructs OM to ignore `dynamic-agents-md` system reminders, and conditionally adds caveman compression instructions for observers/reflections.
- `packages/memory/src/processors/observational-memory/thresholds.ts`, `activation-ttl.ts`, and `model-context.ts` — activation retention floor, `blockAfter` resolution, chunk-boundary safeguards, duration-string parsing, provider/model-aware automatic idle TTL resolution, and normalized provider-change comparison for formatted vs bare model IDs.
...
- `mastracode/src/tui/handlers/om.ts`, `event-dispatch.ts`, `state.ts`, and `components/om-marker.ts` — OM event rendering, including thread-title-updated markers/status refresh, TTL/provider-change activation attribution, provider-change marker state, formatted idle durations, and reflection activation labels.
...
- `packages/memory/src/processors/observational-memory/__tests__/activation-ttl.test.ts`, `idle-buffering.test.ts`, and `memory-config.test.ts` — provider-aware `activateAfterIdle` TTL resolution, duration-string serialization, idle observation/reflection activation, provider-change activation attribution, and non-expiry coverage.
...
- Mastra Code-specific test that `getDynamicMemory()` wires intended OM activation defaults, including `activateAfterIdle: 'auto'` and `activateOnProviderChange: true`, into core memory.
...
- Storage adapters must keep activation-boundary math in sync; in-memory, LibSQL, MongoDB, and PG each implement `swapBufferedToActive()`, while idle activation relies on accurate message/part timestamps and provider-change activation relies on durable step-start model metadata before that swap.
