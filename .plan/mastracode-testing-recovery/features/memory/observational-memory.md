...
| Idle activation TTL | OM config `activateAfterIdle` (`number`, duration string, `auto`, or `false`) + provider/model cache-retention heuristics from `activation-ttl.ts`; Mastra Code defaults to `auto` | Core observation/reflection activation, Harness `om_activation` events, TUI OM markers |
...
| Buffer activation math | Core OM thresholds + storage `swapBufferedToActive()` + idle/provider-change activation priority | Runtime context trimming, async buffering, blockAfter fallback, prompt-cache TTL expiry |
...
- `packages/memory/src/processors/observational-memory/thresholds.ts` and `activation-ttl.ts` — activation retention floor, `blockAfter` resolution, chunk-boundary safeguards, duration-string parsing, and provider/model-aware automatic idle TTL resolution.
...
- `mastracode/src/tui/handlers/om.ts`, `event-dispatch.ts`, and `components/om-marker.ts` — OM event rendering, including thread-title-updated markers/status refresh plus TTL/provider-change activation attribution and formatted idle durations.
...
- `packages/memory/src/processors/observational-memory/__tests__/activation-ttl.test.ts`, `idle-buffering.test.ts`, and `memory-config.test.ts` — provider-aware `activateAfterIdle` TTL resolution, duration-string serialization, idle observation/reflection activation, and non-expiry coverage.
...
- Mastra Code-specific test that `getDynamicMemory()` wires intended OM activation defaults, including `activateAfterIdle: 'auto'`, into core memory.
...
- Memory factory cache key must include every setting that changes behavior, including persisted threshold defaults, attachment-observation mode, and idle-activation defaults.
...
- Storage adapters must keep activation-boundary math in sync; in-memory, LibSQL, MongoDB, and PG each implement `swapBufferedToActive()`, while idle activation relies on accurate message/part timestamps before that swap.
