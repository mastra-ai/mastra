...
| OM custom model overrides | `settings.models.activeOmPackId`, `omModelOverride`, `observerModelOverride`, and `reflectorModelOverride`; `/om` snapshots the other role's current model when leaving a built-in OM pack for `custom` | Observer/reflector model functions, OM settings modal, global/thread settings persistence |
...
- `mastracode/src/providers/claude-max.ts`, `openai-codex.ts`, `github-copilot.ts`, and `auth/providers/openai-codex.ts` — provider-specific OAuth fetch/storage integration, OpenAI Codex callback port fallback, and current provider-header handling used by the shared auth path.
- `mastracode/src/auth/storage.ts` — credential persistence, stored provider API-key helpers, env loading, and refresh.
- `mastracode/src/tui/prompt-api-key.ts`, `components/api-key-dialog.ts`, `components/masked-input.ts`, and `tui/commands/api-keys.ts` — missing-key prompt, masked key entry dialog used by model selectors, and `/api-keys` stored-key management command.
...
- `mastracode/src/onboarding/packs.ts` — provider-filtered built-in model/OM packs and current OpenAI/Anthropic default IDs, including OpenAI build/plan `openai/gpt-5.5` and fast/OM `openai/gpt-5.4-mini`.
...
- `mastracode/src/agents/__tests__/model.test.ts` — provider/model resolution, custom provider routing, Anthropic/OpenAI API-key fallback, provider stored-key fallback, env fallback, OAuth priority, explicit `mastra/` gateway routing, gateway base URL/key selection, harness header forwarding, provider-header current behavior, and generic `authStorage.reload()` assertion.
...
- `mastracode/src/onboarding/__tests__/packs.test.ts`, `settings.test.ts` — built-in pack defaults (including current OpenAI and Anthropic defaults), settings resolution, and stale-field migrations.
...
- `mastracode/src/auth/providers/openai-codex.test.ts` — OpenAI Codex OAuth callback port preference/fallback, authorization URL redirect URI, originator/scope, account ID extraction, refresh, and device OAuth behavior.
...
- `packages/core/src/llm/model/registry-generator.test.ts`, `provider-registry.test.ts`, and `mastracode/src/utils/__tests__/gateway-sync.test.ts` — generated provider-key quoting, corrupt cache validation/deletion, refresh-time skip logic, and the MastraCode gateway-sync wrapper delegation.
