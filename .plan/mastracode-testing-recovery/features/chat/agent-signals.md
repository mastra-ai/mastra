...
- Success looks like: active-run text does not wait for `agent_end`; idle sends still start a normal stream; persisted signal messages carry `delivery="while-active"` or `delivery="message"`; state/reactive/notification signal variants stream through `data-signal` without masquerading as system reminders; cross-runtime subscribers receive run registration, stream parts, completion/abort/suspend, and signal-enqueued events over PubSub or per-thread Unix socket PubSub; unsupported signal transports fall back to legacy `streamUntilIdle`; Playground shows pending signal previews and keeps the send button available when the model/thread supports signals.
...
| --- | --- | --- |
| Signal object | `packages/core/src/agent/signals.ts` narrowed `AgentSignalContents`, delivery-attribute resolution, state/reactive/notification signal wire variants, and legacy content decoder | Agent DB/LLM/data-part conversion, Harness send APIs, React SDK, server schemas, history rendering |
| State signal tracking | `packages/core/src/agent/state-signals.ts` + thread `metadata.mastra.stateSignals` | processor state snapshots/deltas, external `sendStateSignal()`, cache-key dedupe/versioning |
| Notification signal tracking | notifications storage records + `createNotificationSignal()` / `createNotificationSummarySignal()` | `sendNotificationSignal()`, notification dispatch, `notification_inbox`, TUI notification cards |
| GitHub reactive signal tracking | `metadata.mastra.githubSignals` subscription metadata + GitHub reactive signal tags | GitHub subscribe/unsubscribe/status handling, PR polling, notification generation |
...
- `packages/core/src/agent/state-signals.ts` — state-signal snapshot/delta parsing, cache-key dedupe, thread metadata tracking, and signal persistence.
- `packages/core/src/notifications/*` — notification records, delivery policy, inbox tool, and notification/summary signal creation.
- `mastracode/src/github-signals/index.ts` — GitHub subscribe/unsubscribe/status reactive signals and PR-activity notification production.
...
- [Processor state signals](./processor-state-signals.md) — state/reactive signal variants share the signal wire path but represent processor state instead of user follow-ups.
- [Notification inbox signals](./notification-inbox-signals.md) — notification variants share the signal wire path but are stored/managed through an inbox.
- [GitHub signal subscriptions](../git/github-signal-subscriptions.md) — GitHub reactive signals and polling produce notification records.
