...
| --- | --- | --- |
| Signal object | `packages/core/src/agent/signals.ts` narrowed `AgentSignalContents` + legacy content decoder | Agent DB/LLM/data-part conversion, Harness send APIs, React SDK, server schemas |
...
| Unix socket signal transport | `mastracode/src/utils/signals-pubsub.ts` + core `UnixSocketPubSub` | per-thread socket routing under `/tmp/mc/<resourceId>/<threadId>.sock`, cross-process TUI/headless signal sharing, broker election/backpressure |
...
- `packages/core/src/agent/signals.ts` — signal creation, XML/LLM wrapping, DB-message conversion, data-part conversion, canonical text/file parts, providerOptions threading, and legacy content normalization.
...
- `packages/core/src/events/__tests__/unix-socket-pubsub.test.ts` and `mastracode/src/utils/__tests__/signals-pubsub.test.ts` — broker election, fan-out, backpressure, duplicate subscribe suppression, stale socket recovery, and per-thread socket path routing.
