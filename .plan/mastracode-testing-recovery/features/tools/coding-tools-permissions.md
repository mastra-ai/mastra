...
| --- | --- | --- |
| Tool definitions | Workspace + `createDynamicTools()` + `McpManager` interface + config `extraTools` (static record or request-context function) + provider web-search fallback | Runtime, prompt guidance |
...
| Tool rendering | Harness events/history, with dedicated renderers for view/edit/shell/task/web-search shapes; local shell passthrough uses its own live component | TUI projections |
...
- `mastracode/src/tui/components/tool-execution-enhanced.ts` — TUI render cases for tools, including view/edit/shell/list/task/web-search compact renderers.
