...
- Success looks like: risky tools ask unless policy/session grants/YOLO allow them; configured extra tools appear when allowed and denied tools disappear everywhere; tool validation failures show the concrete missing/invalid parameter message instead of a generic placeholder.
- Must preserve: visible tool list, runtime tool availability, prompt guidance, approval UI, bounded tool output, accurate schema serialization, and actionable validation errors stay aligned.
...
| --- | --- | --- |
| Tool definitions | Workspace + `createDynamicTools()` + `McpManager` interface + config `extraTools` (static record or request-context function) + provider web-search fallback + schema compatibility wrappers | Runtime, prompt guidance, provider tool-call schemas |
...
| Stable tool categories | `MC_TOOLS` constants after workspace name remap | Permission rules, denied-tool filtering, subagent allowlists, TUI render cases |
...
| Tool rendering/errors | Harness events/history, with dedicated renderers for view/edit/shell/task/web-search shapes; local shell passthrough uses its own live component; `showFormattedError()` / tool renderers preserve validation text | TUI projections |
...
- `mastracode/src/agents/prompts/tool-guidance.ts` — tool usage guidance, including gitignore-aware `find_files` / `search_content` instructions and denied-tool filtering.
...
- `mastracode/src/tui/components/tool-execution-enhanced.ts` — TUI render cases for tools, including view/edit/shell/list/task/web-search compact renderers and validation error details.
...
- `mastracode/src/tui/components/__tests__/tool-execution-enhanced.test.ts` — quiet validation error rendering coverage.
