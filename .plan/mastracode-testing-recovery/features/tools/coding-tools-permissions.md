...
- Commands / shortcuts / flags: agent tool calls, `/permissions`, `/yolo`, Ctrl+Y, approval dialogs (`y` approve, `n` decline, `a` always allow category, `Y` YOLO).
...
| Stable tool categories | `MC_TOOLS` constants after workspace name remap, including `LSP_INSPECT` as read-category code intelligence and `NOTIFICATION_INBOX` as edit-category inbox mutation | Permission rules, denied-tool filtering, subagent allowlists, TUI render cases, notification guidance |
| Permission policies | Harness state / permission rules | Approval engine, `/permissions`, dynamic tool filtering |
| Approval shortcut decoding | `decodePrintableShortcut()` normalizes raw bytes, Kitty CSI-u, and xterm modifyOtherKeys printable input; `ToolApprovalDialogComponent` maps decoded `y`/`n`/`a`/`Y` to approval actions | Tool approval keyboard UX across Apple Terminal, iTerm2, Ghostty, WezTerm, kitty, and other terminal protocols |
...
| Tool output budget | Core workspace output helpers + MC web-tool wrappers + `tokenx` estimate/slice helpers | Model context, TUI/history renderers, TokenLimiterProcessor |
...
- `mastracode/src/agents/tools.ts` — dynamic tools, Tavily/native provider web search, MCP merge, notification inbox registration, config `extraTools` (record or function), denied-tool filtering, hooks.
- `mastracode/src/agents/prompts/tool-guidance.ts` — tool usage guidance, including gitignore-aware `find_files` / `search_content`, `lsp_inspect` marker usage, `notification_inbox` summary handling, and denied-tool filtering.
...
- `mastracode/src/tools/web-search.ts` and `mastracode/src/utils/token-estimator.ts` — MC web-search/web-extract string formatting and `tokenx`-estimated 2k token truncation.
- `packages/core/src/workspace/tools/output-helpers.ts` and `packages/core/src/processors/processors/token-limiter.ts` — core workspace output truncation default (`DEFAULT_MAX_OUTPUT_TOKENS = 2_000`) and `tokenx`-based processor token limits.
...
- `mastracode/src/tui/components/tool-approval-dialog.ts`, `tui/key-input.ts`, `tool-validation-error.ts`, `ansi.ts`, `subagent-execution.ts`, and `tui/handlers/tool.ts` — TUI result/approval/error renderers using safe stringification for object payloads, terminal-protocol-aware approval shortcut decoding, and bounded validation-error plus ANSI/OSC parsing.
...
- `packages/core/src/notifications/tool.ts` — core `notification_inbox` tool actions and status mutations.
...
- [Notification inbox signals](../chat/notification-inbox-signals.md) — `notification_inbox` is a status-mutating tool that delivers unread notification details.
...
- `mastracode/src/agents/tools.test.ts`, `extra-tools.test.ts` — dynamic tools, including notification inbox registration/guidance, request-context-aware extraTools functions, and pre/post tool hook wrapping.
- `packages/core/src/notifications/notifications.test.ts` — `notification_inbox` list/read/markSeen/dismiss/archive/search actions and status mutations.
...
- `mastracode/src/tui/__tests__/key-input.test.ts` and `components/__tests__/tool-approval-dialog.test.ts` — raw-byte, Kitty CSI-u, xterm modifyOtherKeys, Shift+letter, modifier rejection, and one-shot approval shortcut handling.
...
- `packages/core/src/processors/processors/token-limiter*.test.ts` — core TokenLimiterProcessor token-budget behavior and heuristic token-accuracy margins after the `tokenx` migration.
...
- End-to-end PTY coverage that approval shortcuts work under a real terminal with Kitty keyboard protocol enabled; current coverage is component-level decoding and dialog handling.
- Plan-mode runtime tools and prompt guidance both hide write tools.
- End-to-end `notification_inbox read` flow after a notification summary, proving permission/category handling and signal delivery work in the same run.
