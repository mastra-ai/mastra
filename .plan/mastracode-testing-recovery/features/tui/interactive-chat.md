...
| Streaming components | TUI transient projection; trailing content after inline boundaries including state/reactive signals; local shell passthrough output component | Chat container |
...
- `mastracode/src/tui/handlers/message.ts` — assistant streaming partitioning, inline state/reactive signal rendering, and text-preservation guard after tool updates.
...
- `mastracode/src/tui/render-messages.ts` — history reconstruction, persisted user/state/reactive signal rendering, hidden GitHub operation-signal filtering, and optimistic/signal message reconciliation.
