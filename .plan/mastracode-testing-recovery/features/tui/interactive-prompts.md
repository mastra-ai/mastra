...
| Prompt label/answer wrapping, multi-select, and custom free text | `AskQuestionBorderedBox` wraps long option labels and free-text answers with `wrapTextWithAnsi()` plus continuation indentation; `WrappingSelectList` wraps active picker labels with `↳` continuation rows, checkbox multi-select state, and item-based navigation; ask-question components' `Custom response...` sentinel switches select mode to input mode for single-select prompts only | Streaming/answered/cancelled inline question rendering, `ask_user` option prompts |
