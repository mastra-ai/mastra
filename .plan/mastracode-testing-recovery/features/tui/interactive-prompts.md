...
- What the user can do: answer multiple interactive tool prompts sequentially when the agent triggers them in parallel, write multiline free-text `ask_user` answers with wrapping/newlines, choose a predefined option even when its label is long, choose `Custom response...` to type an answer not present in a single-select option list, or enter sensitive values without echoing them in cleartext, including access requests for paths outside the project root.
...
| Prompt label/answer wrapping and custom free text | `AskQuestionBorderedBox` wraps long option labels and free-text answers with `wrapTextWithAnsi()` plus continuation indentation; ask-question components' `Custom response...` sentinel switches select mode to input mode | Streaming/answered/cancelled inline question rendering, `ask_user` option prompts |
