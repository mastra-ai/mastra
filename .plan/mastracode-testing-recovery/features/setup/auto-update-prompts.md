...
- Later changes: current source also has an `/update` command; queue row #13787 should verify and map that slash-command path separately.
...
- Success looks like: startup remains usable when the registry/changelog is unavailable, update prompts show concise changelog entries, and choosing No persists the dismissed version.
- Must preserve: no blocking startup on network failure, no repeated prompt spam for a dismissed version, and package-manager-specific install commands.
...
| --- | --- | --- |
| Current version | `getCurrentVersion()` build define / package fallback | TUI prompt, analytics, `/update` |
...
- `mastracode/src/utils/update-check.ts` — package-manager detection, latest-version fetch, semver comparison, changelog fetch/parse, and update execution helpers.
