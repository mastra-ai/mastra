...
- Proposed scenarios:
  ...
  - Completed: `custom-provider-edit-share-import` edits a provider name/URL/API key through `/custom-providers`, shares a saved custom pack to clipboard, cancels a colliding shared-pack import through `/models`, and proves settings only persist the completed provider edit. Remaining breadth: custom-pack completion, import-rename, invalid provider URL/duplicate-name branches, remove-model, and selector persistence.
