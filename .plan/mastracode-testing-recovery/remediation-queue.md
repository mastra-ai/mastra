...
- Proposed scenarios:
  ...
  - Completed: `file-attachment-history-reload` seeds persisted user signal history with projected text-file content, an image file part, and a binary file part, then proves `/threads` loaded history renders `[1 image] [1 file]` plus the text-file body without leaking raw base64. Remaining breadth: clear-after-send/preserve semantics and OM observation.
