...
- Proposed scenarios:
  ...
  - Completed: `setup-custom-pack-completion` walks `/setup` through Custom mode-pack creation, selects env-backed synthetic plan/build/fast models plus a custom OM model, disables YOLO, and proves custom pack/default/OM/subagent-cleanup persistence in `settings.json`. Remaining `onboarding-full-wizard` breadth: login refresh.
