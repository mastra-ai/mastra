...
export function createObservabilityTests(config: ObservabilityTestConfig = {}) {
  ...
  describe(suiteName, () => {
    ...
    describe('listScoresBySpan', () => {
      it('should list scores for a specific span', async () => {
        // Find a trace/span that has scores seeded in beforeAll
        const scoresResponse = await client.listScores({
          filters: { scorerId: 'e2e-test-scorer' },
          pagination: { page: 0, perPage: 1 },
        });

        if (scoresResponse.scores.length === 0) {
          throw new Error('No seeded scores found — score seeding in beforeAll likely failed');
        }

        const { traceId, spanId } = scoresResponse.scores[0];
        if (!spanId) {
          throw new Error('Seeded score has no spanId — score was created without a spanId');
        }

        const response = await client.listScoresBySpan({ traceId, spanId });
