...
export default async function setup(project: TestProject) {
  ...
  const storage = new LibSQLStore({
    id: 'client-sdk-e2e-storage-zod-v4',
    url: ':memory:',
  });
  ...
  const mastra = new Mastra({
    ...
    observability: new Observability({
      configs: {
        default: {
          serviceName: 'client-sdk-e2e-zod-v4',
          exporters: [
  ...
  console.log(`[Setup] Test server (zod v4) started on ${baseUrl}`);
  ...
  return async () => {
    console.log('[Teardown] Stopping test server (zod v4)');
    if (server) {
      server.close();
    }
  };
