...
sidebar_position: 1
sidebar_label: "With Vercel AI SDK"
...
When setting up a [custom API route](/server-db/custom-api-routes), use the `chatRoute()` utility to create a route handler that automatically formats the agent stream into an AI SDK-compatible format.
...
Here's an example: For a [nested agent stream within a tool](/streaming/tool-streaming#tool-using-an-agent), `data-tool-agent` UI message parts will be emitted and can be leveraged on the client as documented below:
...
For more information about tool streaming see [Tool streaming documentation](/streaming/tool-streaming)
...
> When using `useChat()` with agent memory, refer to the [Agent Memory section](/agents/agent-memory#usechat) for key implementation details.
...
> You can then access this data in your tools via the `runtimeContext` parameter. See the [Runtime Context documentation](/server-db/runtime-context) for more details.
