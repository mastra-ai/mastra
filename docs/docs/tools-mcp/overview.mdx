...
To make tools available to an agent, you configure them in the agent's definition. Mentioning available tools and their general purpose in the agent's system prompt can also improve tool usage. For detailed steps and examples, see the guide on [Using Tools and MCP with Agents](/agents/using-tools-and-mcp#add-tools-to-an-agent).

## Using `RuntimeContext`

Use [RuntimeContext](/server-db/runtime-context) to access request-specific values. This lets you conditionally adjust behavior based on the context of the request.
...
> See [Runtime Context](/server-db/runtime-context) for more information.

## Testing with Mastra Playground

Use the Mastra [Playground](/server-db/local-dev-playground) to test tools with different inputs, inspect execution results, and verify tool behavior.
