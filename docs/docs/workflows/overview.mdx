...
> See [Running Workflows](/examples/workflows/running-workflows) for more information.
...
Use the Mastra [Playground](/server-db/local-dev-playground) to easily run workflows with different inputs, visualize the execution lifecycle, see the inputs and outputs for each step, and inspect each part of the workflow in more detail.
