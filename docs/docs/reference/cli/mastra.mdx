...
Starts a server which exposes a [local dev playground](/server-db/studio) and REST endpoints for your agents, tools, and workflows. You can visit [http://localhost:4111/swagger-ui](http://localhost:4111/swagger-ui) for an overview of all available endpoints once `mastra dev` is running.

You can also [configure the server](/server-db/studio#configuration).
...
Enable local HTTPS support. [Learn more](/server-db/studio#local-https).
...
If you're deploying to a [serverless platform](/deployment/serverless-platforms) you need to install the correct deployer in order to receive the correct output in `.mastra`.
...
Starts a local server to serve your built Mastra application in production mode. By default, [OTEL Tracing](/observability/otel-tracing) is enabled.
...
Disable the [OTEL Tracing](/observability/otel-tracing).
...
Read the [Scorers overview](/scorers/overview) to learn more.
