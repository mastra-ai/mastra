...
または、[Playground](/docs/v0/getting-started/studio) の UI でモデルを参照・テストできます。
...
モデルは単なる文字列にすぎないため、[request context](/docs/v0/server-db/request-context)や変数、その他のロジックに応じて動的に選択できます。
...
* ユーザーが選べるモデル - アプリ内でユーザーが好みのモデルを選択できるようにする。
* マルチテナントアプリケーション - 各顧客が自分のAPIキーとモデル設定を持ち込める。
...
`"provider/model"` という文字列を受け付けるあらゆる場所で、AI SDK のモデル（例: `groq('gemma2-9b-it')`）を使用できます。これは、モデルルーターのフォールバックや [scorers](/docs/v0/scorers/overview) 内でも同様です。
