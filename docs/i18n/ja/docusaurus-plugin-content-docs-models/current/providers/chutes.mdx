---
title: "Chutes | モデル | Mastra"
description: "Mastra で Chutes のモデルを利用。全33モデルに対応。"
---
...
Mastra のモデルルーター経由で 33 種類の Chutes モデルにアクセスできます。認証は `CHUTES_API_KEY` 環境変数によって自動的に行われます。

詳しくは [Chutes のドキュメント](https://llm.chutes.ai) をご覧ください。
...
Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用します。プロバイダー固有の機能の一部は利用できない場合があります。詳しくは [Chutes のドキュメント](https://llm.chutes.ai) をご確認ください。
