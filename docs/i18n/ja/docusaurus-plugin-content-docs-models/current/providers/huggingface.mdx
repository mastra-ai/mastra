---
title: "Hugging Face | モデル | Mastra"
description: "Mastra で Hugging Face のモデルを利用できます。14 件のモデルに対応。"
---
...
Mastraのモデルルーター経由で、Hugging Faceの14のモデルにアクセスできます。認証は `HF_TOKEN` 環境変数により自動で行われます。

詳しくは [Hugging Face のドキュメント](https://huggingface.co)をご覧ください。
...
Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用します。プロバイダー固有の機能の一部は使用できない場合があります。詳しくは [Hugging Face のドキュメント](https://huggingface.co)をご確認ください。
