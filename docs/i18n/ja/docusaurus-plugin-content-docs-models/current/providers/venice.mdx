...
description: "Mastra で Venice AI のモデルを利用できます。利用可能なモデルは13個。"
---
...
Mastraのモデルルーター経由で、13種類のVenice AIモデルにアクセスできます。認証は`VENICE_API_KEY`環境変数を用いて自動的に行われます。
...
Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用します。プロバイダー固有の機能の一部は利用できない場合があります。詳細は [Venice AI のドキュメント](https://docs.venice.ai) をご確認ください。
