...
description: "Mastra で FastRouter モデルを利用できます。現在、14 種類のモデルに対応。"
---
...
Mastraのモデルルーター経由で14種類のFastRouterモデルにアクセスできます。認証は `FASTROUTER_API_KEY` 環境変数により自動的に行われます。
...
Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用しています。プロバイダー固有の機能の一部は利用できない場合があります。詳細は [FastRouter のドキュメント](https://fastrouter.ai/models) をご確認ください。
