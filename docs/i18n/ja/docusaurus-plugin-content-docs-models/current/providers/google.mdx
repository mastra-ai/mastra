---
title: "Google | モデル | Mastra"
description: "Mastra で Google のモデルを利用。利用可能なモデルは全24件。"
---
...
Mastra のモデルルーター経由で 24 個の Google モデルにアクセスできます。認証は `GOOGLE_GENERATIVE_AI_API_KEY` 環境変数によって自動的に行われます。
...
Google は、`providerOptions` パラメーターで次のプロバイダー固有のオプションをサポートします。

```typescript
const response = await agent.generate("Hello!", {
  providerOptions: {
    google: {
      // 利用可能なオプションについては下記の表を参照してください
    }
  }
});
```
...
このプロバイダーは、スタンドアロンのパッケージとして直接インストールでき、Mastra のモデルルーター用の文字列の代わりに使用できます。詳しくは[パッケージのドキュメント](https://www.npmjs.com/package/@ai-sdk/google)をご覧ください。

```bash npm2yarn copy
npm install @ai-sdk/google
```

各プロバイダー向けの詳細なドキュメントは、[AI SDK の Google プロバイダーに関するドキュメント](https://ai-sdk.dev/providers/ai-sdk-providers/google)をご覧ください。
