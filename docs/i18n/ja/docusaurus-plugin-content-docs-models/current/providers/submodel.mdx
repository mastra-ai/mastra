...
description: "Mastra で submodel を利用する。利用可能なモデルは 9 件。"
---
...
Mastra のモデルルーター経由で 9 つの submodel モデルにアクセスできます。認証は `SUBMODEL_INSTAGEN_ACCESS_KEY` 環境変数により自動的に行われます。
...
Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用します。プロバイダー固有の機能の一部は利用できない場合があります。詳細は [submodel のドキュメント](https://submodel.gitbook.io)をご確認ください。
