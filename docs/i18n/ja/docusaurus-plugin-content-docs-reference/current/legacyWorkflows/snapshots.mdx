---
title: "リファレンス: スナップショット | レガシー ワークフロー | Mastra ドキュメント"
description: "Mastra におけるスナップショットの技術リファレンス ― 一時停止と再開を可能にする、シリアライズされたワークフロー状態"
---
...
<div id="the-role-of-snapshots-in-suspend-and-resume">
...
<div id="how-snapshots-are-saved-and-retrieved">
  ## スナップショットの保存と取得方法
</div>

Mastra は、設定したストレージシステムにスナップショットを永続化します。既定では LibSQL データベースに保存されますが、Upstash など他のストレージプロバイダーを使用するように設定することも可能です。
libsql を使用する場合、スナップショットは `workflow_snapshots` テーブルに保存され、対応する実行の `run_id` によって一意に識別されます。
永続化レイヤーを活用することで、スナップショットをワークフローの複数回の実行にわたって保持でき、人間の介入を伴う高度な機能を実現できます。

[libsql storage](../storage/libsql) および [upstash storage](../storage/upstash) の詳細はこちらをご覧ください。

<div id="saving-snapshots">
  ### スナップショットの保存
</div>
...
<div id="storage-options-for-snapshots">
  ## スナップショットのストレージオプション
</div>
...
<div id="best-practices-for-working-with-snapshots">
...
<div id="advanced-snapshot-patterns">
  ## さらに進んだスナップショットのパターン
</div>

<div id="custom-snapshot-metadata">
  ### カスタムスナップショットのメタデータ
</div>
...
- [suspend と resume のガイド](/docs/v0/workflows-legacy/suspend-and-resume)