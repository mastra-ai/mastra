---
title: "リファレンス: ToneConsistencyMetric | Evals | Mastra ドキュメント"
description: Mastra における Tone Consistency Metric のドキュメント。テキストの感情トーンとセンチメントの一貫性を評価します。
---

<div id="toneconsistencymetric">
...
`ToneConsistencyMetric` クラスは、テキストの感情的なトーンとその一貫性を評価します。入力と出力のペア間でトーンを比較するモードと、単一のテキスト内でトーンの安定性を分析するモードの2つで動作します。
...
<div id="info-object-tone-comparison">
  ### info オブジェクト（トーン比較）
</div>
...
<div id="info-object-tone-stability">
  ### info オブジェクト（トーンの安定性）
</div>
...
<div id="example-with-both-modes">
  ## 両方のモードを使う例
</div>
