---
title: "リファレンス: HallucinationMetric | Evals | Mastra ドキュメント"
description: Mastra の Hallucination Metric に関するドキュメント。提供されたコンテキストとの矛盾を検出することで、LLM の出力が事実に即しているかを評価します。
---

<div id="hallucinationmetric">
  # HallucinationMetric
</div>
...
`HallucinationMetric` は、提供されたコンテキストとモデル出力を比較し、LLM が事実に基づく正確な情報を生成しているかを評価します。この指標は、コンテキストと出力の間に存在する明確な矛盾を特定することで、ハルシネーションを測定します。
...
<div id="hallucinationmetricoptions">
...
3. ハルシネーションスコアを算出:
   - ハルシネーションと判断された記述（矛盾と根拠のない主張）をカウント
   - 総記述数で割る
   - 設定された範囲にスケーリング
