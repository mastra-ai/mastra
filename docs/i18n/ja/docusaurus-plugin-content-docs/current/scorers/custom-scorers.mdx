...
<div id="functions-vs-prompt-objects">
  ### 関数 vs プロンプトオブジェクト
</div>
...
<div id="agent-type-for-agent-evaluation">
...
<div id="step-by-step-breakdown">
  ### 手順ごとの詳細解説
</div>

<div id="preprocess-step-optional">
...
<div id="generatescore-step-required">
...
**プロンプトオブジェクト:** `generateScore` でプロンプトオブジェクトを使用する方法の詳細は、[`createScorer`](/reference/v0/scorers/create-scorer) の API リファレンスを参照してください。必須の `calculateScore` 関数についても説明しています。

**データフロー:** スコアは `generateReason` で `score` パラメータとして参照できます


<div id="generatereason-step-optional">
...
<div id="example-create-a-custom-scorer">
  ## 例: カスタムスコアラーの作成
</div>
...
> すべての API と設定オプションについては [createScorer](/reference/v0/scorers/create-scorer) を参照してください。
...
<div id="high-gluten-free-output">
  ### 高グルテン不使用の高出力
</div>
...
<div id="low-gluten-free-example">
  ## 低グルテンの例
</div>
...
* [createScorer API リファレンス](/reference/v0/scorers/create-scorer) - 技術仕様の完全版ドキュメント
