...
- Node.js `v20.0` 以上
- サポートされている[モデルプロバイダー](/models/v0)の API キー

<div id="adding-mastra">
  ## Mastra の追加
</div>

まず、エージェントを実行するために必要な Mastra の依存関係をインストールします。本ガイドではモデルとして OpenAI を使用しますが、サポートされている任意の[モデルプロバイダー](/models/v0)を利用できます。
...
`src/mastra/index.ts` ファイルをインポートすると、[`.getAgent()`](/reference/v0/core/getAgent) などのメソッドでプログラムからアクセスできます。さらに [`.generate()`](/reference/v0/agents/generate) を使うと、該当するエージェントとやり取りできます。

:::note

詳しくは [Agent リファレンス](/reference/v0/agents/agent) を参照してください。
...
本番環境では、Express サーバーと並行して Mastra を動かす必要はありません。一方、開発時には、Mastra が提供する [ローカル開発環境](/docs/v0/getting-started/studio) を使ってエージェントの改善やデバッグができます。
