...
> 詳細は、[Mastra での AI SDK の使用](/models/v0#use-ai-sdk-with-mastra) をご覧ください。
...
Mastra と AI SDK を併用する推奨方法は、`@mastra/ai-sdk` パッケージをインストールすることです。`@mastra/ai-sdk` は、AI SDK 互換の形式で Mastra エージェントをストリーミングするためのカスタム API ルートとユーティリティを提供します。チャット、ワークフロー、ネットワークのルートハンドラーに加え、UI 統合向けのユーティリティやエクスポート型も含まれます。
...
[カスタム API ルート](/docs/v0/server-db/custom-api-routes) を設定する際は、`chatRoute()` ユーティリティを使って、エージェントのストリームを AI SDK 互換形式に自動整形するルートハンドラーを作成します。
...
例: [ツール内のネストされたエージェントストリーム](/docs/v0/streaming/tool-streaming#tool-using-an-agent) の場合、`data-tool-agent` の UI メッセージパーツが出力され、以下のとおりクライアント側で利用できます。
...
ツール ストリーミングの詳細は、[ツール ストリーミングのドキュメント](/docs/v0/streaming/tool-streaming)をご覧ください
...
Mastra は、HTTP ストリーミングを使ってフロントエンドコンポーネントをエージェントに直接接続できる AI SDK の UI フックをサポートしています。
...
> `useChat()` をエージェントのメモリと併用する場合は、実装の要点については [Agent Memory セクション](/docs/v0/agents/agent-memory) を参照してください。
...
> その後、ツールで `runtimeContext` パラメータを通じてこのデータにアクセスできます。詳しくは [Runtime Context のドキュメント](/docs/v0/server-db/runtime-context)をご覧ください。
