...
`NetlifyDeployer` クラスは、スタンドアロンの Mastra アプリケーションを Netlify にデプロイします。設定およびデプロイの管理を行い、Netlify 固有の機能を追加してベースの [Deployer](/reference/v0/deployer/) クラスを拡張します。
...
> 利用可能な設定オプションの一覧は、[NetlifyDeployer](/reference/v0/deployer/netlify) の API リファレンスをご覧ください。
