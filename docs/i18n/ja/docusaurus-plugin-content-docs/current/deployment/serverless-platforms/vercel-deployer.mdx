...
`VercelDeployer` クラスは、スタンドアロンの Mastra アプリケーションを Vercel にデプロイする役割を担います。設定やデプロイの管理を行い、Vercel 固有の機能でベースとなる [Deployer](/reference/v0/deployer/) クラスを拡張します。
...
> 利用可能なすべての設定オプションについては、[VercelDeployer](/reference/v0/deployer/vercel) の API リファレンスを参照してください。
