...
[はじめにガイド](/docs/v0/getting-started/installation)では、すぐに開発を始められるよう、妥当なデフォルト設定を備えたプロジェクトのひな型を作成します。デフォルトでは、CLI はアプリケーションのファイルを `src/mastra/` ディレクトリ配下に整理し、次のような構成になります。
