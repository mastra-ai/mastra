...
基本的なリトリーバル手法の使い方については、[Retrieve Results](/examples/v0/rag/query/retrieve-results) の例をご覧ください。
...
利用可能な演算子や構文の詳細は、[メタデータフィルタ リファレンス](/reference/v0/rag/metadata-filters)を参照してください。
...
メタデータフィルタリングの使用例は、[Hybrid Vector Search](/examples/v0/rag/query/hybrid-vector-search) を参照してください。
...
詳細な設定オプションや高度な活用方法については、[Vector Query Tool リファレンス](/reference/v0/tools/vector-query-tool)を参照してください。
...
再ランキングの詳細は、[rerank()](/reference/v0/rag/rerankWithScorer) メソッドを参照してください。
...
グラフベースのリトリーバルの詳細については、[GraphRAG](/reference/v0/rag/graph-rag) クラスと [createGraphQueryTool()](/reference/v0/tools/graph-rag-tool) 関数を参照してください。

グラフベースのリトリーバル手法の利用例については、[Graph-based Retrieval](/examples/v0/rag/usage/graph-rag) の例を参照してください。
