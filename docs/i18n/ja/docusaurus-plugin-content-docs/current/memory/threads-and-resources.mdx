...
1. **`thread`**: 会話を表すグローバルに一意の ID（例: `support_123`）。すべてのリソースにわたって一意である必要があります。
2. **`resource`**: スレッドの所有者であるユーザーまたはエンティティ（例: `user_123`, `org_456`）。

`resource` は特に [resource-scoped memory](./working-memory#resource-scoped-memory) において重要で、同一のユーザーまたはエンティティに紐づくすべてのスレッド間でメモリを保持できます。
...
> Mastra Playground は `thread` と `resource` の ID を自動で設定します。独自のアプリケーションでは、各 `.generate()` または `.stream()` の呼び出しごとに、それらを手動で指定する必要があります。
...
Mastraは、ユーザーの最初のメッセージに基づいて、わかりやすいスレッドタイトルを自動生成できます。`generateTitle` を `true` に設定して有効にしてください。これにより整理しやすくなり、UIで会話を表示するのも簡単になります。
...
> タイトル生成は、エージェントの応答後に非同期で実行され、応答時間には影響しません。詳細と例については、[完全な構成リファレンス](/reference/v0/memory/memory-class)を参照してください。
