...
すべてのメモリタイプはデフォルトで[リソーススコープ](./working-memory#resource-scoped-memory-default)であり、同一ユーザーまたはエンティティの全会話にわたって保持されます。[スレッドスコープ](./working-memory#thread-scoped-memory)の設定にすると、メモリは単一の会話スレッド内に限定されます。
...
利用可能なオプションには、[LibSQL](/docs/memory/storage/memory-with-libsql)、[MongoDB](/examples/memory/memory-with-mongodb)、[Postgres](/docs/memory/storage/memory-with-pg)、[Upstash](/docs/memory/storage/memory-with-upstash) があります。
