...
Semantic recall は、メッセージとその埋め込みを保存するために、[ストレージとベクターデータベース](/reference/v0/memory/memory-class)に依存します。
...
* [LibSQL](/docs/v0/memory/storage/memory-with-libsql)
* [MongoDB](/docs/v0/memory/storage/memory-with-mongodb)
* [Postgres](/docs/v0/memory/storage/memory-with-pg)
* [Upstash](/docs/v0/memory/storage/memory-with-upstash)
...
Semantic recall は、メッセージをベクトル化するために[埋め込みモデル](/reference/v0/memory/memory-class)に依存します。Mastra は、`provider/model` という文字列を用いるモデルルーター経由で埋め込みモデルを扱えるほか、AI SDK と互換性のある任意の[埋め込みモデル](https://sdk.vercel.ai/docs/ai-sdk-core/embeddings)も利用できます。
...
インデックスの設定オプションやパフォーマンス調整の詳細については、[PgVector 設定ガイド](/reference/v0/vectors/pg#index-configuration-guide)をご覧ください。
