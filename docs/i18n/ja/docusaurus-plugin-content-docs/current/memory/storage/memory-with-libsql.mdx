...
エージェントに LibSQL のメモリを追加するには、`Memory` クラスを使用し、`LibSQLStore` を使って新しい `storage` キーを作成します。`url` にはリモートの場所またはローカルのファイルシステム上のリソースを指定できます。
...
export const libsqlAgent = new Agent({
  name: "libsql-agent",
  instructions:
    "あなたは過去のやり取りの記憶を自動的に呼び出すことができるAIエージェントです。",
  model: openai("gpt-4o"),
  memory: new Memory({
...
埋め込みは、memory の `semanticRecall` がキーワードではなく意味にもとづいて関連メッセージを検索するために用いる数値ベクトルです。このセットアップでは、ベクトル埋め込みを生成するために `@mastra/fastembed` を使用します。
