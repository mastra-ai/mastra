...
Mastra は 600 以上のモデルに対応しています。[完全な一覧](/models/v0)から選択してください。
...
> 詳細は[Agent リファレンス ドキュメント](/reference/v0/agents/agent)をご覧ください。
...
詳しくは [Calling agents](/examples/v0/agents/calling-agents) をご覧ください。
...
> 詳しくは [.generate()](/reference/v0/agents/generate) または [.stream()](/reference/v0/streaming/agents/stream) を参照してください。
...
> 詳しくは、[Runtime Context](/docs/v0/server-db/runtime-context)をご覧ください。
...
Mastra の [Playground](/docs/v0/getting-started/studio) を使って、さまざまなメッセージでエージェントをテストし、ツールの呼び出しと応答を確認し、エージェントの動作をデバッグします。
...
- [ランタイムコンテキスト](/examples/v0/agents/runtime-context)
