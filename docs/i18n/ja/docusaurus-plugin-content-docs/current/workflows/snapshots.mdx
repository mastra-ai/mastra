---
title: "スナップショット | ワークフロー | Mastra ドキュメント"
description: "Mastra のスナップショットでワークフローの実行状態を保存・再開する方法を学びましょう"
---
...
各スナップショットには、`runId`、入力、ステップのステータス（`success`、`suspended` など）、サスペンド時および再開時のペイロード、最終的な出力が含まれます。これにより、実行を再開する際に完全なコンテキストを利用できます。
...
スナップショットは、設定済みのストレージシステムに保存されます。既定では LibSQL を使用しますが、Upstash や PostgreSQL を利用するように設定することも可能です。各スナップショットは `workflow_snapshots` テーブルに保存され、ワークフローの `runId` によって識別されます。
...
4. スナップショットの状態を基にワークフロー実行を再構成する
5. 中断されたステップを再開し、実行を継続する
...
<div id="postgres-mastrapg">
  ### Postgres `@mastra/pg`
</div>

この例では、PostgreSQL でスナップショットを利用する方法を紹介します。
...
`suspendSchema` を定義すると、ワークフローを一時停止する際にカスタムメタデータを添付できます。このメタデータはスナップショットに保存され、ワークフローの再開時に利用できます。
...
<div id="providing-resume-data">
  ### レジュメデータの提供
</div>

一時停止中のステップを再開する際に、構造化された入力を渡すには `resumeData` を使用します。これはステップの `resumeSchema` に適合している必要があります。
...
- [ヒューマン・イン・ザ・ループの例](/examples/v0/workflows_legacy/human-in-the-loop)
- [WorkflowRun.watch()](/reference/v0/workflows/run-methods/watch)