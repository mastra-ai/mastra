...
> 詳細は、[ワークフローの一時停止と再開の例](/examples/v0/workflows_legacy/suspend-and-resume)をご覧ください。
...
> 詳細は [Runtime Context](/docs/v0/server-db/runtime-context) をご覧ください。
...
- [`.sleep()`](/reference/v0/workflows/workflow-methods/sleep): 指定したミリ秒数だけ一時停止する
- [`.sleepUntil()`](/reference/v0/workflows/workflow-methods/sleepUntil): 特定の日付まで一時停止する
- [`.waitForEvent()`](/reference/v0/workflows/workflow-methods/waitForEvent): 外部イベントを受信するまで一時停止する
- [`.sendEvent()`](/reference/v0/workflows/workflow-methods/sendEvent): 待機中のワークフローを再開するイベントを送信する