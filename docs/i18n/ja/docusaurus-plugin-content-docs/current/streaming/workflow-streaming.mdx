---
title: "ワークフローのストリーミング | ストリーミング | Mastra ドキュメント"
description: "Mastra でのワークフローのストリーミングの使い方を学びます。ワークフロー実行イベントの処理、ステップのストリーミング、エージェントやツールとの統合について解説します."
---

<div id="workflow-streaming">
  # ワークフローのストリーミング
</div>
...
`writer` は `streamVNext` を使用している場合にのみ利用可能です。

:::

`writer` 引数はワークフローのステップの `execute` 関数に渡され、アクティブなストリームへカスタムイベントやデータ、値を送出するために使用できます。これにより、実行中であっても、各ステップが中間結果や進捗状況の更新を提供できるようになります。
...
<div id="inspecting-workflow-stream-payloads">
  ### ワークフローのストリーム・ペイロードの確認
</div>

ストリームに書き込まれたイベントは、出力されるチャンクに含まれます。これらのチャンクを確認すると、イベントタイプ、中間値、ステップ固有のデータといったカスタムフィールドにアクセスできます。
...
<div id="resuming-an-interrupted-workflow-stream">
...
<div id="workflow-using-an-agent">
  ## エージェントを使ったワークフロー
</div>

エージェントの `textStream` をワークフローステップの `writer` にパイプします。これにより部分的な出力がストリーミングされ、Mastra がエージェントの使用状況をワークフローの実行に自動的に集約します。
