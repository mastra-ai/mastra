...
<div id="building-an-ai-recruiter">
  # AIリクルーターを構築する
</div>
...
- サポート対象の[モデルプロバイダー](/models/v0)の API キー
- 既存の Mastra プロジェクト（新規プロジェクトをセットアップする場合は[インストールガイド](/docs/v0/getting-started/installation)に従ってください）

<div id="building-the-workflow">
  ## ワークフローの構築
</div>
...
開発サーバーを起動して、Mastraの[playground](/docs/v0/getting-started/studio)内でワークフローをテストできます。
...
[`.createRunAsync()`](/reference/v0/workflows/workflow-methods/create-run) と [`.start()`](/reference/v0/workflows/run-methods/start) を呼び出して、プログラムからワークフローをテストすることもできます。新しいファイル `src/test-workflow.ts` を作成し、次のコードを追加します。
