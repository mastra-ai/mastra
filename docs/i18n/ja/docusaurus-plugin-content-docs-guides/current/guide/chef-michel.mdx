...
description: 利用可能な食材で作れる料理を支援するための、Mastraでのシェフアシスタントエージェント作成ガイド。
---
...
- サポート対象の [Model Provider](/models/v0) の API キー
- 既存の Mastra プロジェクト（新規プロジェクトのセットアップは [インストールガイド](/docs/v0/getting-started/installation) を参照）
...
エージェントの応答を人に見せるのではなく、コードの別の部分に渡したい場合があります。こうしたケースでは、エージェントは[構造化出力](/docs/v0/agents/overview#structured-output)を返す必要があります。
...
これにより、登録済みエージェントと対話するためのエンドポイントを公開するサーバーが起動します。[playground](/docs/v0/getting-started/studio) では、UI 上でエージェントをテストできます。
