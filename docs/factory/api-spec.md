...
- **Integration-specific routes** (GitHub/GitLab/Linear/Slack/WorkOS webhooks and OAuth callbacks) are not part of `FACTORY_ROUTE_CONTRACTS` — each `FactoryIntegration` contributes its own `ApiRoute[]` via `IntegrationContext`, mounted alongside the core contract routes.
