...
| (archive/restore) | `POST .../attention/:kind/:sourceId/:occurrence/archive` \| `.../restore` | Same path shape as `attentionRead` |

## Supervisor / diagnostics

| Contract | Method & Path | Notes |
|---|---|---|
| (health check) | `GET /web/factory/projects/:id/supervisor/health` | Returns `checkedAt`, `findings[]`, `counts` |

## Documents

| Contract | Method & Path | Notes |
|---|---|---|
| `documentList` | `GET /web/factory/projects/:id/documents` | Returns `docsRoot`, `manifestPath`, the full 15-kind `catalog` (kind/group/label/defaultPath/purpose), synced `documents[]` (bodies omitted), and `sync` state (`sourceRef`, `sourceSha`, `manifestStatus`, `syncedAt`) or `null` if never synced |
...
- **Responses are typed by the same Zod schema on both sides** — `surface.ts` validates outgoing payloads against `responseSchema`, and `factory-ui`/`sdk` consumers can derive request/response types from the same contract object.
