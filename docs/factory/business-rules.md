...
3. **Externally-authored items fail closed.** `externallyAuthored(item)` is true for any forge-authored item — `github-pr`, `github-issue`, `gitlab-mr`, `gitlab-issue` (`PROVIDER_AUTHORED_SOURCES`) — unless `metadata.factoryAuthored === true` or `metadata.authorTrusted === true`. A missing trust stamp (not yet reconciled) is treated as untrusted, not as trusted-by-default. `manual` and `linear-issue` cards are created by someone who already holds Factory access, so they carry no outside author to gate on and the predicate returns false. GitLab never stamps `authorTrusted` — its webhook payload carries no write-access signal and its connection is org-wide rather than scoped to the acting user — so every non-Factory GitLab card stays gated.
4. **`knownExternalAuthor`** additionally requires the provider to have explicitly answered `authorTrusted: false` — it does not fire on a merely-missing stamp, only a confirmed outside contribution. Only GitHub answers this today, so it never fires for GitLab.
