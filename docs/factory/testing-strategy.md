...
- **Storage domains** (`storage/domains/*/*.test.ts`): each domain's CRUD, schema round-tripping (`toFoo`/DB row), and domain-specific invariants (e.g. `documents/documents.test.ts` for missing/oversize handling, `work-items/*.test.ts` for stage transitions and revision conflicts) against an in-memory or test storage backend.
