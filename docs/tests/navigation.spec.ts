...
function sidebarCategory(sidebar: Locator, label: string) {
  return sidebar.getByRole('link', { name: label, exact: true }).locator('xpath=ancestor::li[1]')
}

test.describe('Collapsible sidebar', () => {
  test('keeps only one sibling category expanded', async ({ page, isMobile }) => {
    const getErrors = trackJsErrors(page)
    ...
    const sidebar = isMobile ? page.locator('.navbar-sidebar') : page.getByRole('navigation', { name: 'Docs sidebar' })
    const agents = sidebarCategory(sidebar, 'Agents')
    const workflows = sidebarCategory(sidebar, 'Workflows')
    const agentsCaret = agents.locator(':scope > .menu__list-item-collapsible > button.menu__caret')
    const workflowsCaret = workflows.locator(':scope > .menu__list-item-collapsible > button.menu__caret')

    await expect(agents).toHaveClass(/menu__list-item--collapsed/)
    await expect(workflows).toHaveClass(/menu__list-item--collapsed/)

    await agentsCaret.click()
    await expect(agents).not.toHaveClass(/menu__list-item--collapsed/)
    await expect(agents.getByRole('link', { name: 'Tools', exact: true })).toBeVisible()

    await workflowsCaret.click()
    await expect(workflows).not.toHaveClass(/menu__list-item--collapsed/)
    await expect(workflows.getByRole('link', { name: 'Workflow State', exact: true })).toBeVisible()
    await expect(agents).toHaveClass(/menu__list-item--collapsed/)

    const agentsLink = agents.getByRole('link', { name: 'Agents', exact: true })
    const agentsHref = await agentsLink.getAttribute('href')
    expect(agentsHref).toBeTruthy()
    await agentsLink.click()
    await expect(page).toHaveURL(agentsHref!)

    if (isMobile) {
      await page.getByRole('button', { name: 'Toggle navigation bar' }).click()
    }

    const navigatedSidebar = isMobile
      ? page.locator('.navbar-sidebar')
      : page.getByRole('navigation', { name: 'Docs sidebar' })
    await expect(sidebarCategory(navigatedSidebar, 'Agents')).not.toHaveClass(/menu__list-item--collapsed/)
    await expect(sidebarCategory(navigatedSidebar, 'Workflows')).toHaveClass(/menu__list-item--collapsed/)
