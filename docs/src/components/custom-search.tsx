...
export const CustomSearch: FC<SearchProps> = ({
  ...
  const virtualizer = useVirtualizer({
    ...
    estimateSize: () => (isMobile ? 90 : 95), // Smaller size for mobile screens
    overscan: 5,
  });
  ...
  return (
    <div className="overflow-hidden w-full max-h-[600px]">
      <div
        className={cn(
          className,
          "flex items-center p-2 py-1 w-full md:p-4 md:py-[10px] gap-[14px]",
        )}
      >
        ...
        <input
          ...
          className={cn(
            ...
            "outline-none caret-[var(--light-green-accent-2)]  dark:caret-accent-green dark:text-icons-6 text-[var(--light-color-text-4)] focus:outline-none w-full placeholder:text-icons-4 dark:placeholder:text-icons-4 placeholder:text-small md:placeholder:text-base placeholder:font-medium",
          )}
          autoComplete="off"
          type="search"
          autoFocus
      ...
      <div
        className={cn(
          ...
          !search || !results.length ? "h-fit" : "h-[500px]",
        )}
      >
        <div
                    ...
                    >
                      <div className="mt-3">
                        <div className="border-t-[0.5px] border-[var(--light-border-code)] dark:border-borders-1 pt-3">
                          <div
                            className={cn(
                              "flex gap-2 justify-between items-center px-2 py-2 mb-2 w-full text-base font-medium rounded-md cursor-pointer md:px-4",
                              isSelected
                          ...
                          >
                            <div className="flex gap-2 items-center">
                              <span className="dark:text-accent-green text-accent-green-2 shrink-0">
                                <JarvisIcon className="w-4.5 h-4.5 md:w-6 md:h-6 shrink-0" />
                              </span>
                              <span className="flex flex-col font-medium text-left text-small md:text-base text-icons-5">
                                <span
                                  id="use-ai-text"
                  ...
                  >
                    ...
                    >
                      <div className="flex gap-2 md:gap-[14px] items-center">
                        ...
                        <span className="text-small font-medium truncate md:text-base dark:text-icons-6 text-[var(--light-color-text-4)]">
                          {subResult.title}
                        </span>
                      </div>
...
function EmptyState({ setSearch }: { setSearch: (search: string) => void }) {
  ...
  return (
    <div className="flex flex-col pt-4">
      <p className="px-2 mb-2 text-sm font-medium text-icons-2 md:px-3">
        Top searches
      </p>
      <ul className="flex flex-col w-full">
        {searches.map((search) => (
          <Button
            ...
            className={cn(
              "justify-start p-2 w-full h-auto text-left rounded-md cursor-pointer md:p-3",
