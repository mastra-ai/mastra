...
The Firecrawl MCP server runs locally via `npx`. Once the client is configured, call `.listTools()` to retrieve all available tools from it. Tool names are namespaced by server name to prevent conflicts.
...
> Park page info: Confirm parkId first. For park hours, use firecrawl_firecrawl_extract on `https://queue-times.com/en-US/parks/{parkId}/queue_times`. For crowd forecast and ride stats, use firecrawl_firecrawl_extract on `https://queue-times.com/en-US/parks/{parkId}`.
...
> Weather: Use weatherTool only if the user asks about weather, or if weather would clearly affect ride recommendations (heavy rain, extreme heat, lightning risk). When calling weatherTool, pass only the city name.
...
- [Firecrawl MCP](https://docs.firecrawl.dev/mcp): available tools and setup
