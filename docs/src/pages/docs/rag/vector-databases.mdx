...
<Tabs.Tab>
  ...
  const store = new DefaultVectorDB(process.env.DATABASE_URL)
  await store.createIndex("my-collection", 1536);
  await store.upsert(
    "my-collection",
    embeddings,
...
- Metadata is stored as schemaless JSON - this flexibility requires careful planning
- Design and document your schema upfront to ensure team-wide consistency
- Be strict about field naming - inconsistencies like 'category' vs 'Category' will affect queries
- Only store fields you plan to filter or sort by
- Use clear, unambiguous names (e.g., prefer 'createdAt' over 'date' or 'timestamp')
- Track content freshness with timestamps (e.g., 'createdAt', 'lastUpdated') for filtering outdated information
- Include schema version to handle metadata structure changes over time (e.g., 'schemaVersion: "2.0"')
- Structure metadata to match your most common query patterns
