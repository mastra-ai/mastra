...
You can use conditions with nested workflows:

```typescript
parentWorkflow.step(nestedWorkflowA).then(nestedWorkflowB, {
  when: async ({ context }) => {
    // Condition based on nestedWorkflowA's results
    const resultA = context.getStepResult("nested-workflow-a");
    return resultA.someValue === true;
  },
});
```
