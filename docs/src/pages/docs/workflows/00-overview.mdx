...
```ts showLineNumbers filename="src/mastra/workflow/index.ts" copy
...
// 3. Add and link steps in your workflow
myWorkflow
  .step(stepOne)
  .then(stepTwo)
  .commit();
...
To create a workflow in Mastra, you use the Workflow class and define its properties:
...
To create a step in Mastra, you use the Step class and define its properties:
...
Create a sequential flow where `stepOne` runs first and is followed by `stepTwo`.
