  ...
  content={[
    {
      ...
      isOptional: true,
      defaultValue: "{ name: 'PlayDialog', apiKey: process.env.PLAYAI_API_KEY, userId: process.env.PLAYAI_USER_ID }",
    },
    {
      name: "speaker",
      type: "string",
...
```typescript
async getSpeakers(): Promise<Array<PlayAIVoiceInfo>>
```

#### Returns

Returns an array of voice information objects with the following properties:
