# Murf Voice Reference

The Murf voice implementation in Mastra provides high-quality text-to-speech (TTS) capabilities using Murf's AI voice service. It supports multiple voices across different languages and offers extensive audio customization options.
  ...
  content={[
    {
      ...
      isOptional: true,
      defaultValue: "{ name: 'GEN2', apiKey: process.env.MURF_API_KEY }",
    },
    {
      name: "speaker",
      type: "string",
      ...
      isOptional: true,
      defaultValue: "First voice from MURF_VOICES",
    },
  ]}
/>
...
Murf provides a wide range of voices across multiple languages. Here are some examples:
...
5. Custom pronunciation dictionaries can be provided for better text-to-speech accuracy.
6. Speech-to-text functionality is not supported.
