---
title: "LibSQL Vector Store | Vector Databases | RAG | Mastra Docs"
...
The LibSQLVector class provides vector search using [LibSQL](https://github.com/tursodatabase/libsql) and [Turso](https://turso.tech/) with vector extensions, offering a lightweight and efficient vector database solution.
The LibSQLVector class provides vector search using LibSQL, a fork of SQLite with vector extensions. It's part of the `@mastra/core` package and offers efficient vector similarity search with metadata filtering.

## Installation

LibSQL vector store is included in the core package:
  ...
  content={[
    {
      name: "connectionUrl",
      type: "string",
      description: "LibSQL database URL. Use ':memory:' for in-memory database, 'file:local.db' for local file, or a LibSQL-compatible connection string like 'libsql://your-database.turso.io'.",
    },
    {
      name: "authToken",
      type: "string",
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      defaultValue: "cosine",
      description: "Distance metric for similarity search. Note: Currently only cosine similarity is supported by LibSQL.",
    },
  ]}
/>
