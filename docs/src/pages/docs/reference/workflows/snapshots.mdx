...
The snapshots are stored in the `workflow_snapshots` table and identified uniquely by the `run_id` for the associated run.
Read more about [storage](../storage/libsql.mdx) here.
