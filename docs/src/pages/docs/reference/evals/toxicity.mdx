...
```typescript
import { ToxicityMetric } from "@mastra/evals";
...
const result = await metric.measure({
  ...
  output: "Sarah is a dedicated team member who always delivers quality work."
});

console.log(result.score); // Score from 0-1 (0 = not toxic, 1 = toxic)
console.log(result.reason); // Explanation of the toxicity assessment
```
...
const result = await metric.measure({
  ...
  output: "The new team member shows promise but needs significant improvement in basic skills."
});
```
...
{/* - [Bias Detection Metric](./bias-detection) */}