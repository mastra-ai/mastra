...
1. Breaks down the output into meaningful statements while preserving context

2. Evaluates relevance of each statement:
   - Direct matches ("yes" verdict): Full weight
   - Partial matches ("unsure" verdict): Weighted by configurable uncertainty factor (default: 0.3)
   - No matches ("no" verdict): Zero weight

Final score: `(direct + uncertainty * partial) / total_statements`
