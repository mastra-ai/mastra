---
title: "Memory Message Processors | Reference | Mastra Docs"
description: Documentation on how to filter and transform messages in Mastra Memory.
---

# Memory Message Processors

Memory message processors allow you to filter or transform messages before they're sent to the LLM. This is particularly useful for:

- Limiting token usage to prevent context overflow
- Filtering out specific message types (e.g., audio files, tool calls)
- Removing messages from specific roles
...
Here are additional examples of custom message processors:
  ...
  process(messages: CoreMessage[]): CoreMessage[] {
    return messages.filter(message => {
      ...
      if (typeof message.content === 'string') {
        return !message.content.includes('data:audio/') && 
...
You can combine multiple processors by passing them in an array:
...
## Using Processors at Runtime

You can also override the default processors when making specific calls to the agent:

```typescript copy showLineNumbers
// Default memory with no processors
const memory = new Memory();

const agent = new Agent({ memory, /* other options */ });

// Apply processors for a specific query
await agent.stream("What was our conversation about?", {
  threadId: "thread_id",
  resourceId: "user_id",
  memoryOptions: {
    // Use different processors just for this call
    processors: [
      new ToolCallFilter(), // Remove all tool calls
      new TokenLimiter(4000) // And limit tokens
    ]
  }
});
```

This flexibility allows you to adapt message processing based on the specific requirements of each interaction.
