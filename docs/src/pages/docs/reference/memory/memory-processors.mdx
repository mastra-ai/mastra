...
const memory = new Memory({
  processors: [
    // Limit message history to approximately 128000 tokens (ex. for gpt-4o)
    new TokenLimiter(128000),
  ],
});
```

The `TokenLimiter` uses `o200k_base` encoding by default, which is used by GPT-4o models. If you need to use a different encoding:
