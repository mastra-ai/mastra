...
const memory = new Memory({
  processors: [
    ...
    new ToolCallFilter({
      exclude: ["audioPlayer", "videoPlayer"],
    }),
  ],
});
```
...
```typescript copy showLineNumbers
import { Memory, MemoryProcessor, CoreMessage } from "@mastra/memory";

// Simple example of implementing the MemoryProcessor interface
class SimpleMessageFilter implements MemoryProcessor {
...
```typescript copy showLineNumbers
class TextOnlyFilter implements MemoryProcessor {
  process(messages: CoreMessage[]): CoreMessage[] {
    return messages
      .map((message) => {
...
```typescript copy showLineNumbers
class AudioMessageFilter implements MemoryProcessor {
  process(messages: CoreMessage[]): CoreMessage[] {
...
```typescript copy showLineNumbers
class ReasoningFilter implements MemoryProcessor {
  process(messages: CoreMessage[]): CoreMessage[] {
    return messages
      .map((message) => {
        if (Array.isArray(message.content)) {
...
const memory = new Memory({
  processors: [
    ...
    new ToolCallFilter({ exclude: ["audioPlayer"] }),
...
The examples in this documentation are just illustrations to help you understand how to implement the `MemoryProcessor` interface. You should create your own custom processors tailored to your specific use cases.
