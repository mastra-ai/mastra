...
- If you define a dependencies schema but don't provide dependencies when calling `generate()`, `stream()`, or `resolveInstructions()`, you'll get runtime validation errors
...
const instructions = await agent.resolveInstructions({
  apiKey: process.env.WEATHER_API_KEY,
  userId: 123,
  temperatureUnit: "celsius",
});
...
await agent.resolveInstructions();
// ⚠️ Will throw: "Dependencies validation failed"
```
