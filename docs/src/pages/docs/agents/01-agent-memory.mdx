...
> **Note**: While commonly used for working memory, `maskStreamTags` can mask any XML-style tags in the response stream. For example, you can use it to hide `<think>` tags from DeepSeek r1 or other model-specific tags.
