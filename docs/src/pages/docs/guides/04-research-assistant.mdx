...
### 3. Load and Process the Paper

This step handles the initial document processing. We:
1. Set up our Mastra instance with the agent and vector store
2. Fetch the research paper from its URL
3. Convert it into a document object
4. Split it into smaller, manageable chunks for better processing
...
// Initialize Mastra and PgVector
...
```typescript copy showLineNumbers{29} filename="src/index.ts"
// Generate embeddings
