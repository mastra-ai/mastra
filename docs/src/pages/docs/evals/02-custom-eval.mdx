...
```typescript copy showLineNumbers filename="src/mastra/evals/recipe-completeness-judge.ts"
...
```typescript copy showLineNumbers filename="src/mastra/evals/recipe-completeness.ts"
import { Metric, type MetricResult } from '@mastra/core/eval';
import { type LanguageModel } from '@mastra/core/llm';
...
You can now use the `RecipeCompletenessMetric` in your project. [See the full example here](/docs/guides/01-chef-michel).
