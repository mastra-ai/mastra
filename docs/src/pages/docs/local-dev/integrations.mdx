...
export const mem0Tool = createTool({
  ...
  inputSchema: z.object({
    question: z.string().describe('Question to answer or a statement to save into memory'),
  }),
  outputSchema: z.object({
...
- We import the `mem0` integration.
- We define a tool called `mem0Tool` that uses the Mem0 API client to create a new memory.
- The tool accepts `question` as an input and returns the memory ID.
