...
The Mastra Engine uses PostgreSQL to store agent memory and vector embeddings. It works locally with Docker for development and scales to production PostgreSQL deployments. Start with PostgreSQL + pgvector, then adapt individual components based on your needs.