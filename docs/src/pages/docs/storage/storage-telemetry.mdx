...
Mastra provides built-in telemetry support for monitoring storage operations. This guide shows you how to set up telemetry and use it to optimize your application.

## Overview

Mastra's telemetry system tracks:

1. **Storage Operations**: Database queries and transactions
2. **Performance Metrics**: Execution time and resource usage
3. **Error Events**: Connection and query failures
4. **Resource Usage**: Connection pools and cache hits
...
// Configure storage with production telemetry
const storage = new DefaultStorage({
  name: 'my-storage',
  config: {
  ...
  telemetry: {
    ...
    propagation: true,   // Enable context propagation
