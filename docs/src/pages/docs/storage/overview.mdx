...
Mastra's storage system provides a unified interface for managing conversation data, workflow states, and evaluation metrics. The system defines a common interface for different database implementations.

## Core Concepts

### 1. Storage Interface

The storage system implements a standard interface for all database types:
