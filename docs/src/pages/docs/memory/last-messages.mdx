...
Conversation History is the core memory feature that saves messages exchanged between a user and an agent within a specific conversation thread.

When a new message is sent, a set number of the most recent messages are retrieved from the thread and are added to the LLM's context window. This allows the agent to understand the immediate conversational context and respond coherently.

When using this feature, you only need to send the latest message in each interaction and won't need to manage the conversation history yourself.
