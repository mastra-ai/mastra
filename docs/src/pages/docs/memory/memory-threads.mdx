...
For more details on how to use working memory effectively, see the [Working Memory documentation](./working-memory.mdx).
...
await agent.stream([{ role: "user", content: "Only the new message" }], {
  resourceId: "user_123",
  threadId: "thread_abc",
});
```
