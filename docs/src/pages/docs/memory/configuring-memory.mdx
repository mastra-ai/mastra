...
Mastra's memory system is configurable, allowing you to adapt it to your specific requirements. This section covers how to configure storage backends, vector databases, and memory settings.
...
LibSQL is included by default and provides a file-based storage solution. See the [LibSQL reference](../reference/storage/libsql.mdx) for configuration details.
...
PostgreSQL provides storage and vector capabilities. See the [PostgreSQL reference](../reference/storage/postgres.mdx) for configuration details.
...
For serverless and edge deployments, Upstash Redis is a managed solution. See the [Upstash Redis reference](../reference/storage/upstash.mdx) for configuration details.
