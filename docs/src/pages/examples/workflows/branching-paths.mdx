...
const stepTwo = new Step({
  id: "stepTwo",
  execute: async ({ context }) => ({
    isDivisibleByFive: context.steps.stepOne.payload.doubledValue % 5 === 0
  })
});
...
const stepThree = new Step({
  id: "stepThree",
  execute: async ({ context }) => ({
    incrementedValue: context.steps.stepOne.payload.doubledValue + 1
  })
});
...
const stepFour = new Step({
  id: "stepFour",
  execute: async ({ context }) => ({
    isDivisibleByThree: context.steps.stepThree.payload.incrementedValue % 3 === 0
  })
});
