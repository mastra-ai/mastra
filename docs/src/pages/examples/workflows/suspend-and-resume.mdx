...
const promptAgent = new Step({
  ...
  execute: async ({ context, suspend }) => {
    ...
    if (initialDraft.confidenceScore < 0.7) {
      ...
      if (!guidance) {
        // Suspend with payload containing the draft and reason
        await suspend();
        return undefined;
      }
...
const improveResponse = new Step({
  ...
  execute: async ({ context, suspend }) => {
    ...
    // If scores are below threshold, suspend for human intervention
...
function calculateToneScore(content: string = '') {
  return 0.7; // Simulate a score that will trigger suspension
}

function calculateCompletenessScore(content: string = '') {
  return 0.9;
}
...
async function runWorkflow() {
  ...
  if (initialResult.results.promptAgent?.status === 'suspended') {
    console.log('Workflow suspended at promptAgent step');
    console.log('Suspension payload:', initialResult?.results.promptAgent?.suspendPayload);
    ...
    let improveResponseStatus = undefined;
    // Check if improveResponse step is suspended
    if (resumeResult1?.results.improveResponse?.status === 'suspended') {
      do {
        console.log('Workflow suspended at improveResponse step');
        console.log('Suspension payload:', resumeResult1?.results.improveResponse?.suspendPayload);
        ...
        // @ts-ignore
        improveResponseResumeAttempts = finalResult?.results.improveResponse?.suspendPayload?.resumeAttempts ?? 0;
        improveResponseStatus = finalResult?.results.improveResponse?.status;
