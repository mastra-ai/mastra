...
description: Example of using a Mastra agent to dynamically construct and apply metadata filters for precise document retrieval.
---
...
This system uses an agent to dynamically construct metadata filters from a user's query to search for relevant chunks in the vector store, reducing the amount of results returned.

## Overview

The system implements an intelligent RAG pipeline using Mastra and OpenAI, where an agent dynamically interprets queries and constructs appropriate metadata filters. Here's what it does:
...
In our instructions we do the following:
- Provide the agent with the metadata structure so that it knows the fields available for filtering.
- Provide a prompt (e.g. PGVECTOR_PROMPT) that provides the agent with filter operations and syntax for the vector store queries.

> Note: Different vector stores have specific prompts available. See [Vector Store Prompts](../../docs/reference/tools/vector-query-tool#vector-store-prompts) for details.
