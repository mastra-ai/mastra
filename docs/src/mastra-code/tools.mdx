...
Mastra Code selects the first available provider in this order:

- **Parallel**: Used when `PARALLEL_API_KEY` is set.
- **Tavily**: Used when `TAVILY_API_KEY` is set and no Parallel key is configured.
- **Anthropic web search**: Built into Anthropic models and used when no provider key is configured.
- **OpenAI web search**: Built into OpenAI models and used when no provider key is configured.

Parallel search accepts one or more `searchQueries` and optional controls such as `objective`, `mode`, `maxResults`, domain filters, date filters, and location. Tavily and model-native providers accept a single `query`; their optional parameters vary by provider.
...
Extract content from one or more web pages. Mastra Code uses Parallel when `PARALLEL_API_KEY` is set, then Tavily when `TAVILY_API_KEY` is set.
