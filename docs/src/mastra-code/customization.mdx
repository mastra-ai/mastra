...
const { controller, mcpManager, authStorage } = await createMastraCode({
	cwd: '/path/to/project',
	initialState: {
		yolo: false,
	},
...
await controller.sendMessage({ content: 'Explain the auth module' })
```

The `controller` object is a standard `AgentController` instance. You can subscribe to events, switch modes, manage threads, and send messages through its API.
...
const { controller, mcpManager, hookManager, authStorage } = createMastraCode()
