...
Mastra Code uses [output processors](/reference/processors/overview) to modify agent behavior after each model step in the agentic loop. The built-in code agent includes a `ToolResultReminderProcessor` that detects when tool arguments reference instruction files such as `AGENTS.md` and streams a `Loaded AGENTS.md` reminder in the same turn.

You can customize or replace this processor by passing an `outputProcessors` array when creating the agent in your own modes configuration:

```typescript title="src/custom-processors.ts"
import { createMastraCode } from 'mastracode'
import { Agent } from '@mastra/core/agent'
...
The reminder appears in the TUI as a system notice during the same turn as the triggering tool usage and is also persisted with the conversation, so it remains visible when reloading threads.
