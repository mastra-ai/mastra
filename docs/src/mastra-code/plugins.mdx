...
`createMastraCodeBehaviorPlugin()` is a thin adapter around `@mastra/behaviors`. Pass a normalized definition or a behavior-directory path:
...
The shared provider contributes stable behavior control tools, intent enforcement, state instructions, model routing, and periodic recovery. Mastra Code resolves state model IDs through the current session. A reload stops the previous provider and scheduler before installing the replacement.
