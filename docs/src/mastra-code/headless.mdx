...
To open the interactive UI with a first message already sent, use `--send-prompt` instead, or `--initial-prompt` to send it only when there is no earlier conversation for the directory to resume.
