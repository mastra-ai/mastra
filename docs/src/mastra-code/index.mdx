...
To skip resuming and start on a new conversation, like running `/new` first, add `--tui-new-thread`. It works with or without a prompt, and a prompt passed with it is always sent:
