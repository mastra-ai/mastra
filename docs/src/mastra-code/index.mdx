...
<Steps>
  ...
  <StepItem>
    ...
    In the first step, log in to Anthropic or OpenAI through OAuth. Alternatively, skip this step and run `/connect` later to add an account or API key. To connect a Cursor subscription, finish onboarding, run `/connect`, and select **Sign in with an account**. Next, choose models for each mode (you can later switch models with `/models`). When you select a model from the model selector that doesn't have an API key configured, Mastra Code prompts you to enter one.
