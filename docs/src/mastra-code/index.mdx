...
<Steps>
  ...
  <StepItem>
    ...
    In the first step, log in to Anthropic or OpenAI through OAuth. Alternatively, skip this step. If you select a model without a configured provider API key, Mastra Code prompts you to enter one. Next, choose models for each mode. You can later change the model for the current mode with `/model` or switch packs with `/models`.
