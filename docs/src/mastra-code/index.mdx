...
Mastra Code requires Node.js 22.19.0 or later.

The optional `@` file autocomplete uses [`fd`](https://github.com/sharkdp/fd), a file finder that respects `.gitignore`. Install `fd` with your package manager to enable filename suggestions:
...
Type `@` followed by part of a filename to search project files and attach a path to your message:
...
Use the arrow keys to select a suggestion, then press Tab to insert it. File suggestions require `fd` or `fdfind`; slash-command autocomplete continues to work without it.

## Dynamic workflows

Describe a workflow in Build mode and ask Mastra Code to build it. Mastra Code discovers the available agents, tools, and workflows, validates the resulting definition, and saves it for later sessions.

Use `/workflows` to manage saved workflows:
...
Run `/workflows help` for the complete command reference.

## Plugins

Use `/plugins` to install and manage trusted local or GitHub plugins. Plugins can add tools, slash commands, skills, signal providers, and system instructions. Plugins execute code inside Mastra Code and can add instructions to the agent prompt, so install them only from sources you trust.
