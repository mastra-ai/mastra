...
<Steps>
  ...
  <StepItem>
    ...
    In the first step, log in to Anthropic or OpenAI through OAuth. Alternatively, skip this step to later authenticate with an API key. Next, choose models for each mode. You can later change the model for the current mode with `/model` or switch packs with `/models`.
...
| `/model` | Change the current mode model and update the active custom pack |
