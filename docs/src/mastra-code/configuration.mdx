...
The database grows as threads, messages, and telemetry accumulate. Deleting rows doesn't shrink the file on its own, because SQLite keeps the freed pages for reuse.

In a session, run `/prune` to delete data older than the retention policies. Run `/prune vacuum` to compact the files afterwards and return that space to the operating system. Add `keep-memory` to keep chat history. Both forms close the session when they finish, because maintenance needs the database to itself.

If a session won't start, run the same maintenance from a shell instead:

```bash
mastracode prune                 # delete rows past the retention policies
mastracode prune --vacuum        # then compact the files to reclaim disk
mastracode prune --keep-memory   # keep chat history
```

`--vacuum` needs enough free disk for the compacted copy before it can swap that copy into place.

The retention defaults are 14 days for traces and logs, 30 days for scorer results and workflow snapshots, and 90 days for chat history.
