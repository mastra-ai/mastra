...
For legacy settings without a saved per-role [observational memory model selection](#om-model-settings), the same chain also covers observational memory. If the active pack's memory-model provider is unavailable, the role falls back to the next pack's memory model in chain order.
...
Custom model packs can carry an optional legacy memory model stored as `models.memory`. For settings created before per-role selections were introduced, Mastra Code checks explicit role overrides, then the active pack and its fallback chain, then the standalone memory setting. Saving either **Auto** or a concrete per-role selection replaces this legacy fallback behavior for that role.
