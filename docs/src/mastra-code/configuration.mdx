...
If you have an Anthropic Claude Max or OpenAI ChatGPT Plus subscription, run `/login` to sign in with an account. Kimi For Coding account login requires an OAuth client registered for the integration. You can also run `/connect` and select **Sign in with an account**.
...
| **Kimi For Coding** | Registered OAuth client | Access to Kimi For Coding models through an approved integration |

Kimi For Coding account login appears only when `KIMI_OAUTH_CLIENT_ID` contains an OAuth client ID registered for the integration. Don't reuse the Kimi CLI client ID. Use `KIMI_API_KEY` when you don't have a registered OAuth client.
...
| `KIMI_OAUTH_CLIENT_ID` | OAuth client ID registered for a Kimi For Coding integration |
