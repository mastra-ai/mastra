...
`/model` changes the model for the current mode. If the selected model needs a provider key, Mastra Code prompts for the key and displays its environment variable name, such as `ANTHROPIC_API_KEY`.
...
- **Cancel the prompt**: Press Escape to keep the current mode model unchanged.

`/models` switches model packs. In the custom pack editor, cancelling a key prompt keeps the selected model in the pack without storing a key. The model remains marked as unavailable.
