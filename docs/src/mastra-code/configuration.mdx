...
`/models` also configures **fallback packs**: select a pack and choose **Set fallback…** from its action menu. When every account serving the active pack's provider is unavailable — all accounts rate-limited or the provider down after retries — the turn hops to the fallback pack's model for that mode. Fallbacks work for built-in and custom packs, and chains are allowed (A → B → C). Cycles are safe: a cascade revisits any one pack at most once (A → B → A), then the error surfaces. Every hop renders a notice in the transcript and is saved in the thread history. After a hop, the thread stays on the landed pack until you switch back with `/models` — there is no automatic recovery.
...
- **Add another account** — runs the normal sign-in flow and appends the account, then returns to the manager. The new account is not activated — select it to make it active.
...
Rotation walks accounts in the order they were added and wraps around; each account is tried at most once per request, so a request never burns the whole pool on one failure. When every account in the pool has been tried, the error surfaces normally.
