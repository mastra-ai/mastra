...
Custom providers are stored in global `config.json`. API keys saved through this flow are stored locally in plaintext, so use it only on a trusted user account and machine.
