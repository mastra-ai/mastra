...
Rotation walks the remaining accounts in the order they were added; each account is tried at most once per request. When every account in the pool has been tried, the error surfaces normally. Other errors — malformed requests (400) and anything unrecognized — never rotate; they surface immediately.
