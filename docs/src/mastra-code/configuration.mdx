...
Mastra Code can connect to external [MCP](https://mastra.ai/guides/mcp/overview) servers and make their tools available to the agent. Configure servers in JSON files:
...
[Observational memory](https://mastra.ai/guides/memory/observational-memory) (OM) uses background agents to maintain a dense log of observations and reflections about the conversation, providing long-term context that persists across sessions.
