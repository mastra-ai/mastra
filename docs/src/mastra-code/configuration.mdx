...
If you have an Anthropic Claude Max, OpenAI ChatGPT Plus, or Cursor subscription, run `/connect` and select **Sign in with an account**. The `/login` command opens the same flow.

| Provider | OAuth flow | What you get |
| - | - | - |
| **Anthropic** | Claude Max subscription | Access to Claude models via your subscription |
| **Cursor** | Cursor subscription | Stored Cursor account credentials |
...
Cursor authentication stores and refreshes credentials. It does not add a Cursor model provider or route models through an OpenAI-compatible endpoint.
