...
Global preferences are stored in `~/.mastracode/config.json`. Durable application state is stored in `state.json` in the operating system's application data directory. The state file contains `onboarding`, `modelUseCounts`, and `updateDismissedVersion`; settings such as `observability.resources` and `memoryGateway` remain in `config.json`.

Mastra Code keeps the previous application-data `settings.json` synchronized as a versioned compatibility mirror for older running versions and downgrades. The mirror can be retired after versions that require `settings.json` are no longer supported. Setting `MASTRA_APP_DATA_DIR` places all three files in that directory. A custom `--settings` path remains a single combined file.
...
This changes the project and global locations used for `config.json`, MCP server configuration, hooks, slash commands, agent instructions, skills, and legacy `database.json` discovery. `configDir` must be a single directory name; absolute paths, `.` and `..`, and names containing `/` or `\` are rejected.
...
Custom providers are stored in global `config.json`. During the legacy compatibility period, `settings.json` contains the same provider data so older Mastra Code versions continue to work. Mastra Code restricts both files to the current operating-system user on macOS and Linux. API keys saved through this flow are stored locally in plaintext, so use it only on a trusted user account and machine.
...
Set the shell in global `config.json` with the `shellPassthrough` block:
...
Mastra Code can read the MCP servers you already configured globally in Claude Code and Codex CLI, so you do not have to copy them into `~/.mastracode/mcp.json`. Both sources are off by default. Turn them on in global `config.json` with the `mcp` block:
...
Credentials and durable application state are stored alongside the database. Global preferences are stored in `~/.mastracode/config.json`. To switch to a remote LibSQL (Turso) or PostgreSQL backend, run `/settings` and select a storage backend. You'll be prompted for a connection URL.
...
1. Persisted setting from `/theme` command (stored in `config.json`)
