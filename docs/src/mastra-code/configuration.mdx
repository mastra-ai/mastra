...
`/models` also configures **fallback packs** via **Set fallback pack…**. When every account serving the active pack's provider is unavailable — all accounts rate-limited or the provider down after retries — the turn hops to the fallback pack's model for that mode. Fallbacks work for built-in and custom packs, and chains are allowed (A → B → C). Cycles are safe: each pack is tried at most twice per cascade (its initial visit plus one revisit), then the error surfaces. Every hop renders a notice in the transcript and is saved in the thread history. After a hop, the thread stays on the landed pack until you switch back with `/models` — there is no automatic recovery.
