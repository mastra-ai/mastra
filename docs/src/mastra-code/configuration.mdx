...
`/model` changes the model for the current mode. `/models` switches model packs. When a custom pack needs a model without a configured key, Mastra Code prompts for the provider key and displays its environment variable name, such as `ANTHROPIC_API_KEY`.
