...
Connect to a remote MCP server via URL. Requires a `url` field. The optional `headers` field lets you pass static headers. Set `auth` to `"oauth"` when the server requires an OAuth flow.
...
Mastra Code stores OAuth tokens automatically for HTTP MCP servers. Static `headers` are still sent with every request, so use them for fixed metadata and `auth: "oauth"` for dynamic authorization.
