...
`/models` switches model packs. Built-in packs keep every mode on one provider. Use `/model` to override the current mode with another model from that provider. To mix providers, create or edit a custom pack with `/models`.
