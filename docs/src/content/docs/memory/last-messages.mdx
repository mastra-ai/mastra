...
flowchart TD
    A[New User Message] --> B[Agent with Memory];

    subgraph ContextWindow [Context Window]
        direction LR
        E1[System Instructions]
        E2[Last 40 Messages]
        E3[Current User Message]
    end

    B -- assembles --> ContextWindow;
    ContextWindow -- sent to --> F[LLM Provider];

    %% Define the side query separately
    E2 -- queries --> D[(Thread Storage)];
    D -- returns last 40 messages --> E2;
