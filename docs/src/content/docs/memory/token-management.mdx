...
The context window of language models is a finite resource that affects both cost and performance. You should always use the `TokenLimiter` processor to prevent API errors when there are more tokens than what your model supports. This processor manages token usage. See the [Memory Processors reference](/docs/reference/memory/memory-processors.mdx) for more details on available processors.
...
The `TokenLimiter` processor acts as a safeguard that manages token usage before messages are sent to the LLM:
...
1. **Always use `TokenLimiter`**: This is a reliable way to prevent context overflow errors
