...
The context window is the information visible to the language model at any given time. Anything outside this window is inaccessible to the model when generating its response.
...
2.  **Relevant Context / History**: Information from the ongoing conversation relevant to the current interaction.
