...
Mastra's memory system is organized around **threads** and **resources**. This architecture provides a way to manage conversations for multiple users while maintaining separate contexts.
...
You can programmatically access and manage user threads, which is useful for building interactive chat applications. See the [getThreadsByResourceId reference](../reference/memory/getThreadsByResourceId.mdx) for more details on retrieving threads.
