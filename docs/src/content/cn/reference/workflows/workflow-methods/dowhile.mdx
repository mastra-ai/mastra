---
title: "参考：Workflow.dowhile() | 工作流"
description: "工作流中 `Workflow.dowhile()` 方法的文档，用于创建循环当条件满足时执行步骤。"
packages:
  - "@mastra/core"
---
...
`.dowhile()` 方法在条件满足时执行一个步骤。它总是在评估条件之前至少运行一次该步骤。第一次评估条件时，`iterationCount` 为 `1`。
