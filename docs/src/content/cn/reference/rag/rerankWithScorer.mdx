...
description: "Mastra 中 rerankWithScorer 函数的文档，该函数为向量搜索结果提供高级重排序功能。"
packages:
  - "@mastra/rag"
---
...
`rerankWithScorer()` 函数通过结合语义相关性、向量相似性和基于位置的评分，为向量搜索结果提供高级重排序功能。
...
`rerankWithScorer` 函数接受来自 @mastra/rag 的任何 `RelevanceScoreProvider`。
