---
title: "Alibaba (中国) | 模型 | Mastra"
description: "在 Mastra 中使用 Alibaba (中国) 模型。共 63 个模型。"
---

{/* 此文件由 generate-model-docs.ts 自动生成 - 请勿手动编辑 */}

# <img src="https://models.dev/logos/alibaba-cn.svg" alt="Alibaba (China) logo" className="inline w-8 h-8 mr-2 align-middle dark:invert dark:brightness-0 dark:contrast-200" />Alibaba (中国)

通过 Mastra 的模型路由器访问 63 个 Alibaba (中国) 模型。身份验证使用 `DASHSCOPE_API_KEY` 环境变量自动处理。

在 [Alibaba (中国) 文档](https://www.alibabacloud.com/help/en/model-studio/models) 中了解更多信息。
...
Mastra 使用 OpenAI 兼容的 `/chat/completions` 端点。某些提供商特定功能可能不可用。查看 [Alibaba (中国) 文档](https://www.alibabacloud.com/help/en/model-studio/models) 了解详情。
