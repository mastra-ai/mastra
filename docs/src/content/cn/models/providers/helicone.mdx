---
title: "Helicone | 模型 | Mastra"
description: "在 Mastra 中使用 Helicone 模型。共 91 个模型。"
---
...
通过 Mastra 的模型路由器访问 91 个 Helicone 模型。身份验证使用 `HELICONE_API_KEY` 环境变量自动处理。

在 [Helicone 文档](https://helicone.ai/models) 中了解更多信息。
...
Mastra 使用 OpenAI 兼容的 `/chat/completions` 端点。某些提供商特定功能可能不可用。查看 [Helicone 文档](https://helicone.ai/models) 了解详情。
