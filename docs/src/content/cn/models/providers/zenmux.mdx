---
title: "ZenMux | 模型 | Mastra"
description: "在 Mastra 中使用 ZenMux 模型。共 60 个模型。"
---
...
通过 Mastra 的模型路由器访问 60 个 ZenMux 模型。身份验证使用 `ZENMUX_API_KEY` 环境变量自动处理。

在 [ZenMux 文档](https://docs.zenmux.ai) 中了解更多信息。
...
Mastra 使用 OpenAI 兼容的 `/chat/completions` 端点。某些提供商特定功能可能不可用。查看 [ZenMux 文档](https://docs.zenmux.ai) 了解详情。
