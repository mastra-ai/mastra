...
description: "Mastra で Cortecs モデルを利用できます。利用可能なモデルは 11 件です。"
---
...
Mastra のモデルルーター経由で 11 種類の Cortecs モデルにアクセスできます。認証は `CORTECS_API_KEY` 環境変数を用いて自動的に行われます。
...
<Callout type="info">
  Mastra は OpenAI 互換の `/chat/completions` エンドポイントを利用しています。プロバイダー固有の機能の一部は使用できない場合があります。詳細は [Cortecs のドキュメント](https://cortecs.ai)をご確認ください。
</Callout>
