...
description: "Mastra で OpenAI のモデルを利用できます。利用可能なモデルは 30 件です。"
---
...
Mastraのモデルルーター経由で30種類のOpenAIモデルにアクセスできます。認証は `OPENAI_API_KEY` 環境変数により自動で行われます。
...
OpenAI では、`providerOptions` パラメーターを通じて、次のプロバイダー固有のオプションをサポートしています。
