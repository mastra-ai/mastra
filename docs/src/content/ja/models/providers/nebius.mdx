...
description: "Mastra で Nebius AI Studio のモデルを利用。利用可能なモデルは15件。"
---
...
Mastra のモデルルーター経由で 15 種類の Nebius AI Studio モデルにアクセスできます。認証は `NEBIUS_API_KEY` 環境変数により自動的に行われます。

詳細は [Nebius AI Studio のドキュメント](https://docs.studio.nebius.com/quickstart) をご覧ください。
...
<Callout type="info">
  Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用します。プロバイダー固有の機能の一部は利用できない場合があります。詳しくは [Nebius AI Studio のドキュメント](https://docs.studio.nebius.com/quickstart)をご確認ください。
</Callout>
