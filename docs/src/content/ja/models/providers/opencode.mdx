...
description: "Mastra で OpenCode Zen のモデルを利用できます。全14モデルに対応。"
---
...
Mastra のモデルルーター経由で、14 の OpenCode Zen モデルにアクセスできます。認証は `OPENCODE_API_KEY` 環境変数を使用して自動的に行われます。

詳しくは [OpenCode Zen ドキュメント](https://opencode.ai/docs/zen) をご覧ください。
...
<Callout type="info">
  Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用しています。プロバイダー固有の機能の一部は利用できない場合があります。詳細は [OpenCode Zen ドキュメント](https://opencode.ai/docs/zen)をご確認ください。
</Callout>
