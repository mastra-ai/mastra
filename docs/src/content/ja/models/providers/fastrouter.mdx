---
title: "FastRouter | モデル | Mastra"
description: "Mastra で FastRouter モデルを利用できます。利用可能なモデルは14件です。"
---
...
Mastra のモデルルーター経由で 14 種類の FastRouter モデルにアクセスできます。認証は `FASTROUTER_API_KEY` 環境変数によって自動的に行われます。

詳しくは [FastRouter のドキュメント](https://fastrouter.ai/models)をご覧ください。
...
<Callout type="info">
  Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用します。プロバイダー固有の機能の一部は利用できない場合があります。詳細は [FastRouter のドキュメント](https://fastrouter.ai/models) をご確認ください。
</Callout>
