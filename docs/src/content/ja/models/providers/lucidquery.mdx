...
description: "Mastra で LucidQuery AI のモデルを利用できます。利用可能なモデルは 2 件です。"
---
...
Mastra のモデルルーターを通じて、2 つの LucidQuery AI モデルにアクセスできます。認証は `LUCIDQUERY_API_KEY` 環境変数を使用して自動的に行われます。
...
<Callout type="info">
  Mastra は OpenAI 互換の `/chat/completions` エンドポイントを使用しています。プロバイダー特有の機能の一部は利用できない場合があります。詳細は [LucidQuery AI のドキュメント](https://lucidquery.com) をご参照ください。
</Callout>
