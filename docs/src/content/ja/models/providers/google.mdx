...
description: "Mastra で Google モデルを利用。利用可能なモデルは24件。"
---
...
Mastra のモデルルーター経由で 24 種類の Google モデルにアクセスできます。認証は `GOOGLE_GENERATIVE_AI_API_KEY` 環境変数により自動的に行われます。

詳細は [Google のドキュメント](https://ai.google.dev/gemini-api/docs/pricing)をご参照ください。
...
Google は `providerOptions` パラメーターで、次のプロバイダー固有のオプションをサポートしています。
