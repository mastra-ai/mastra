...
`requestContext`を通じて提供される動的な値を使用して、為替レートデータを取得するツールを作成します。
...
export const getExchangeRatesTool = createTool({
  ...
  execute: async ({ requestContext }) => {
    const currency = requestContext.get("currency");
...
`set()`を使って`RequestContext`を設定し、`requestContext`を渡して`execute()`を呼び出します。
...
requestContext.set("currency", "USD");
