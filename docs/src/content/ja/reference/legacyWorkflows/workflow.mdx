---
title: "リファレンス: Workflow クラス | ワークフローの構築（レガシー） | Mastra Docs"
description: Mastra の Workflow クラスに関するドキュメント。条件分岐やデータ検証を伴う複雑な処理フローに対して、ステートマシンを構築できるようにします。
---
...
ワークフローに[Step](./step-class.mdx)を追加し、他のステップへの遷移も含めます。チェーン呼び出しのためにワークフローインスタンスを返します。[ステップの詳細](./step-class.mdx)。
...
必要に応じてトリガーデータを指定してワークフローを実行します。[トリガースキーマ](./workflow.mdx#trigger-schemas)に基づいて型付けされます。
