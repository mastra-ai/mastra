---
title: "リファレンス: Workflow.while() | ワークフローのループ処理（レガシー） | Mastra ドキュメント"
description: "Mastra のワークフローにおける `.while()` メソッドのドキュメント。指定した条件が真である間、ステップを繰り返します。"
---
...
- [.until()](./until.mdx) - 条件が満たされるまでループする
