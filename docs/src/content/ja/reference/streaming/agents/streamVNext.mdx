...
すべてのコールバック関数がトップレベルのプロパティとして利用できるようになり、より洗練された API 体験を提供します。

```ts filename="index.ts" showLineNumbers copy
const stream = await agent.streamVNext("物語を聞かせて", {
  onFinish: (result) => {
    console.log('ストリーミングが完了:', result);
  },
  onStepFinish: (step) => {
    console.log('ステップが完了:', step);
  },
  onChunk: (chunk) => {
    console.log('チャンクを受信:', chunk);
  },
  onError: ({ error }) => {
    console.error('ストリーミングのエラー:', error);
  },
  onAbort: (event) => {
    console.log('ストリームが中断されました:', event);
  },
});
