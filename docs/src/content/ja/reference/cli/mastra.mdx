---
title: "リファレンス: CLI コマンド"
description: Mastra CLI を使ってプロジェクトを開発・ビルド・起動するためのドキュメント。
---
...
[ローカル開発用プレイグラウンド](/docs/server-db/local-dev-playground)と、エージェント・ツール・ワークフロー向けの REST エンドポイントを公開するサーバーを起動します。`mastra dev` の実行中は、[http://localhost:4111/swagger-ui](http://localhost:4111/swagger-ui) にアクセスすると、利用可能なすべてのエンドポイントの概要を確認できます。

[サーバーの設定](/docs/server-db/local-dev-playground#configuration)も行えます。

### フラグ

このコマンドは[共通フラグ][common-flags]に加えて、以下の追加フラグを受け付けます。

#### `--https`

ローカルの HTTPS サポートを有効にします。 [詳しくはこちら](/docs/server-db/local-dev-playground#local-https)。
...
本番モードでビルド済みの Mastra アプリケーションを配信するローカルサーバーを起動します。既定で [OTEL Tracing](/docs/observability/otel-tracing) が有効です。
...
[OTEL トレーシング](/docs/observability/otel-tracing)を無効にします。
