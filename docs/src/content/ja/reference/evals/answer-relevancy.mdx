---
title: "リファレンス: Answer Relevancy | メトリクス | Evals | Mastra ドキュメント"
description: Mastra における Answer Relevancy メトリクスのドキュメント。LLM の出力が入力クエリにどれだけ適切に応答しているかを評価します。
---
...
`AnswerRelevancyMetric` クラスは、LLM の出力が入力クエリにどの程度適切に答えているか、あるいは対応しているかを評価します。審査員方式で関連性を判定し、詳細なスコアと理由付けを提供します。
