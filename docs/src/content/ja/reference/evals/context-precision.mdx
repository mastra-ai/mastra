---
title: "リファレンス: Context Precision | メトリクス | Evals | Mastra ドキュメント"
description: Mastra における Context Precision メトリクスのドキュメント。期待される出力を生成するために取得されたコンテキストノードの妥当性と正確性を評価します。
---
...
`ContextPrecisionMetric` クラスは、期待される出力の生成に対して、取得されたコンテキストノードがどれだけ関連性と精度を備えているかを評価します。各コンテキストの寄与度を判定ベースの手法で分析し、位置に応じた重み付きスコアを付与します。
