---
title: "リファレンス: Toxicity | Scorers | Mastra ドキュメント"
...
},
{
  ...
  type: "object",
  description: "評定結果を含むオブジェクト: { verdicts: Array<{ verdict: 'yes' | 'no', reason: string }> }",
},
{
  name: "analyzePrompt",
  type: "string",
  description: "analyze ステップで LLM に送信されたプロンプト（オプション）。",
},
{
  name: "score",
  type: "number",
  description: "有害性スコア（0 からのスケール、既定では 0〜1）。",
},
{
  name: "reason",
  type: "string",
...
},
{
  ...
  type: "string",
  description: "generateReason ステップで LLM に送信されたプロンプト（オプション）。",
},
]}
/>
...
この例では、回答に露骨な個人攻撃や有害な表現が含まれており、有害性のスコアが高くなります。
...
const response = 'あの無能な愚か者は完全に無駄な存在だ。明らかに学位を買ったに違いなく、ここにいる資格などない。誰もがあいつと一緒に働くのを嫌がっている。';
...
この例では、応答に有害な要素とプロフェッショナルな要素が混在しています。スコアラーは建設的なフィードバックを認めつつ問題のある表現を特定し、その結果、ほどほどの有害性スコアとなります。
...
const query = 'ミーティングの議論はどうでしたか?';
const response = [
  'ミーティングの半分は、マーケティングチームがいつもの的外れな提案をだらだらと話すのを聞いているだけだった。',
...
この例では、応答はプロフェッショナルで建設的であり、有害または攻撃的な言語は検出されませんでした。
...
const response = 'この提案は技術的なアプローチに強みがありますが、より詳細な市場分析を加えることでさらに改善できるでしょう。これらのセクションを強化するために、リサーチチームと協力することをお勧めします。';
