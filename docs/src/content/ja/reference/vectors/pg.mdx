---
title: "リファレンス: PG Vector Store | ベクターデータベース | Mastra ドキュメント"
description: Mastra の PgVector クラスに関するドキュメント。pgvector 拡張を用いた PostgreSQL でのベクトル検索を提供します。
---
...
- **m パラメータ**:
  - 8-16: 精度は中程度、メモリ使用量は少ない
  - 16-32: 高精度、メモリ使用量は中程度
  - 32-64: 非常に高精度、メモリ使用量は多い
- **efConstruction**:
  - 32-64: 構築が速く、品質は良好
  - 64-128: 構築はやや遅く、品質はより良い
  - 128-256: 構築が最も遅く、品質は最良
...
`PgVector` クラスは、基盤となる PostgreSQL の接続プールをパブリックフィールドとして公開しています：

```typescript
pgVector.pool // pg.Poolのインスタンス
```

これは、直接SQLクエリの実行、トランザクションの管理、プール状態の監視といった高度な使い方を可能にします。プールを直接使用する場合:

* 使用後はクライアント（`client.release()`）を解放する責任があります。
* `disconnect()` を呼び出した後もプールには引き続きアクセスできますが、新規クエリは失敗します。
* 直接アクセスすると、PgVector のメソッドが提供する検証やトランザクションロジックは適用されません。

この設計は高度な利用を可能にしますが、ユーザーによる慎重なリソース管理が求められます。
