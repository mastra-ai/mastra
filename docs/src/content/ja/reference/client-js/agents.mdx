...
description: client-js SDK を使って、応答の生成、ストリーミングでの対話、エージェントツールの管理など、Mastra の AI エージェントとのやり取り方法を学びます。
---
...
const response = await agent.generate({
  ...
  threadId: "thread-1", // 任意: 会話コンテキスト用のスレッド ID
  resourceid: "resource-1", // 任意: リソース ID
  output: {}, // 任意: 出力設定
});
```
...
リアルタイムのやり取りに向けて、エージェントからの応答をストリーミングします:
...
強化されたメソッドシグネチャを備えた拡張版の VNext API を使い、応答をストリーミングします。この方法は、Mastra のネイティブ形式と AI SDK v5 互換形式の両方をサポートし、より強力な機能と柔軟なフォーマット指定を提供します。
...
強化された VNext API を使用し、改良されたメソッドシグネチャと AI SDK v5 との互換性によりレスポンスを生成します：
