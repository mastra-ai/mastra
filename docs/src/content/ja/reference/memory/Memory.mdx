...
`generateTitle` 機能は、ユーザーの最初のメッセージに基づいて会話スレッドに意味のあるタイトルを自動生成します。これにより、アプリケーション内で会話を整理・識別しやすくなります。
...
#### コスト最適化のためのカスタムモデルの指定

メインの会話に高品質なモデルを使いながら、タイトル生成用に別の（一般的には安価な）モデルを指定することができます。
...
ランタイムのコンテキストに基づいて、モデルを動的に決定する関数を指定することも可能です。
...
<PropertiesTable
  content={[
    ...
    {
      name: "threads",
      type: "{ generateTitle?: boolean | { model: MastraLanguageModel | ((ctx: RuntimeContext) => MastraLanguageModel | Promise<MastraLanguageModel>) } }",
      description:
        "メモリスレッド作成に関する設定。`generateTitle`を設定すると、ユーザーの最初のメッセージからLLMの要約によってthread.titleが生成されます。ブール値を指定するとエージェントが使用しているモデルによる生成を有効／無効にできます。また、オブジェクトを指定してタイトル生成用のカスタムモデルを設定することも可能です（コスト最適化に役立ちます）。例: { generateTitle: { model: openai('gpt-4.1-nano') } }",
      isOptional: true,
