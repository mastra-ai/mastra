...
description: さまざまなベクターストアに保存されたデータに対するベクトル検索結果を、メタデータで精密に絞り込める Mastra のフィルタリング機能のドキュメント。
---
...
3. 論理演算子:
   ...
   - 空にしないこと
   - 適切にネストすること
   - トップレベル、または他の論理演算子内でのみ使用できる
   - フィールドレベルやフィールド内では使用できない
   - 他の演算子の内側では使用できない
   - 有効: `{ "$and": [{ "field": { "$gt": 100 } }] }`
...
5. 演算子のネスト:
   - 論理演算子は、直接演算子を置くのではなく、フィールド条件を含める必要がある
   - 有効: `{ "$and": [{ "field": { "$gt": 100 } }] }`
   - 無効: `{ "$and": [{ "$gt": 100 }] }`
...
* 等価比較の値はプリミティブ（string/number/boolean）である必要があります。`null`/`undefined`、配列、オブジェクト、Date は等価比較では使用できません。範囲演算子は number または Date を受け付けます（Date はエポックミリ秒に正規化されます）。
* `$in`/`$nin` には「空でないプリミティブの配列」が必要です。Date 要素は許可され、エポックミリ秒に正規化されます。配列の等価比較はサポートされていません。
* 暗黙の AND は正規化されます（`{a:1,b:2}` → `{$and:[{a:1},{b:2}]}`）。論理演算子はフィールド条件を含み、空でない配列を使用し、ルートまたは他の論理演算子内にのみ出現し（フィールド値の内部では不可）、これらの条件を満たす必要があります。
* インデックス作成時に `nonFilterableMetadataKeys` に指定したキーは保存されますが、フィルタ不可です。この設定は不変です。
* $exists には boolean 値が必要です。
* undefined/null/空のフィルタは、フィルタなしとして扱われます。
* 各メタデータキー名は最大 63 文字に制限されます。
* ベクターごとのメタデータ合計: 最大 40 KB（フィルタ可能 + フィルタ不可）
* ベクターごとのメタデータキー合計: 最大 10
