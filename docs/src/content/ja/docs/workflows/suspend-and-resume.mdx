...
## suspend/resume における `RequestContext` の使用

`suspend/resume` と `RequestContext` を併用する場合は、インスタンスを自分で作成し、それを `start` と `resume` 関数に渡せます。
`RequestContext` はワークフローの実行間で自動的には共有されません。
