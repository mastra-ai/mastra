...
Mastra のメッセージを AI SDK 形式に変換する必要がある場合は、`@mastra/ai-sdk` の `toAISdkMessages()` 関数を使用してください:

```typescript
import { toAISdkMessages } from '@mastra/ai-sdk'; // AI SDK v5 (デフォルト)
import { toAISdkMessages as toAISdkV4Messages } from '@mastra/ai-sdk/v4'; // AI SDK v4

// Mastra のメッセージを AI SDK v5 UI 形式に変換（デフォルト）
const v5Messages = toAISdkMessages(mastraMessages);

// Mastra のメッセージを AI SDK v4 UI 形式に変換
const v4Messages = toAISdkV4Messages(mastraMessages);

// 高度な変換には、引き続き convertMessages() を使用できます:
