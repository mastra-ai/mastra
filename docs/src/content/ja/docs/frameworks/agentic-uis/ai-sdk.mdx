...
### Node.js でのストリーミング（Express/NestJS）

Express や NestJS などの Node.js 環境では、`pipeUIMessageStreamToResponse()` を使用して、`ServerResponse` オブジェクトに Server-Sent Events 経由で UI メッセージをストリーミングします。

#### Express

```typescript filename="app.ts" showLineNumbers copy
...
app.get('/api/chat', async (req, res) => {
  ...
  stream.pipeUIMessageStreamToResponse({
    response: res,
    uiOptions: {
      sendReasoning: true,
      sendSources: false,
...
```typescript filename="chat.controller.ts" showLineNumbers copy
...
export class ChatController {
  ...
  async stream(@Res() res: Response) {
    const myAgent = this.mastraService.getAgent('weatherAgent');
    const stream = await myAgent.stream([
      { role: 'user', content: 'What is the weather?' }
    ], { format: 'aisdk' });
...
このメソッドは `toUIMessageStreamResponse()` と同じ `uiOptions` を受け取り、さらに `status`、`statusText`、`headers` などの Node.js 固有のパラメータも指定できます。
