---
title: "Mastra と Next.js のはじめ方 | Mastra ガイド"
...
<Tabs items={["App Router", "Pages Router"]}>
  <Tabs.Tab>
    ...
    <Steps>
      ...
      次のコマンドを実行して、適切な初期設定でデフォルトの Weather エージェントを素早くスキャフォールドします。
      ...
      > LLM プロバイダーごとに使用する環境変数が異なります。詳細は [Model Capabilities](/docs/getting-started/model-capability) を参照してください。
      ...
      いつも通りに Next.js アプリを起動できます。

      ## テスト用ディレクトリを作成する

      テスト用の Page、Action、Form を含む新しいディレクトリを作成します。
  ...
  <Tabs.Tab>
    ...
    <Steps>
      ...
      プロジェクトに Mastra を統合するには、次の 2 つの方法があります。

      ### 1. ワンライナーを使う

      次のコマンドを実行して、適切な初期設定済みのデフォルト Weather エージェントを素早くスキャフォルドします。
      ...
      <Callout type="warning">
        既定では、`mastra init` はインストール先として `src` を提案します。プロジェクトのルートで Pages Router（例: `src/pages` ではなく `pages`）を使用している場合は、プロンプトで `.` と入力してください。
      </Callout>
      ...
      > LLM プロバイダーごとに使用する環境変数名が異なります。詳しくは [Model Capabilities](/docs/getting-started/model-capability) を参照してください。
