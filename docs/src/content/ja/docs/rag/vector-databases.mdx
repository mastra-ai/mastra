...
<Tabs items={['Pg Vector', 'Milvus', 'Pinecone', 'Qdrant', 'Chroma', 'Astra', 'LibSQL', 'Upstash', 'Cloudflare', 'MongoDB', 'OpenSearch']}>
  ...
  <Tabs.Tab>
    ...
    Milvusは、AI アプリケーションと類似性検索のために設計された高性能なオープンソースのベクトルデータベースです。優れたスケーラビリティ、高スループット、高度なフィルタリングのサポートを提供します。

    詳細なデプロイメントオプションとベストプラクティスについては、[公式Milvusドキュメント](https://milvus.io/docs)を参照してください。
