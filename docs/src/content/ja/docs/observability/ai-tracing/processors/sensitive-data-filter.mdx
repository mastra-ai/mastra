---
title: "機密データフィルター | プロセッサー | 可観測性 | Mastra ドキュメント"
description: "AIのトレース内の機密情報を自動的に編集して保護する"
---
...
export const mastra = new Mastra({
  observability: {
    default: { enabled: true }, // SensitiveDataFilterを自動的に含みます
  },
...
<Callout>
  フィールドの一致判定は大文字小文字を区別せず、区切り記号を正規化します。たとえば、`api-key`、`api_key`、`Api Key` はいずれも `apikey` として扱われます。
</Callout>
...
どのフィールドをマスキングし、マスキングの表示方法をどうするかをカスタマイズできます。
  ...
  observability: {
    configs: {
      debug: {
        ...
        processors: [], // プロセッサなし（SensitiveDataFilterを含む）
        exporters: [new DefaultExporter()],
      },
    },
  },
...
<Callout type="warning">
  機密データのフィルタリングを無効化するのは、管理された環境でのみ行ってください。トレースを外部サービスや共有ストレージに送信する場合は、絶対に無効化しないでください。
</Callout>
