---
title: "Arize エクスポーター | AI トレーシング | オブザーバビリティ | Mastra ドキュメント"
description: "OpenTelemetry と OpenInference を使って、AI のトレースを Arize Phoenix または Arize AX に送信する"
---
...
1. **Phoenix インスタンス**: Docker でデプロイするか、[Phoenix Cloud](https://app.phoenix.arize.com/login) に登録
2. **エンドポイント**: Phoenix のエンドポイント URL（`/v1/traces` で終わる）
...
<Callout type="info">
  **Docker でのクイックスタート**
  ...
  インメモリの Phoenix インスタンスでローカルにテストするには:
  ...
  `PHOENIX_ENDPOINT=http://localhost:6006/v1/traces` を設定し、Mastra エージェントを実行すると、[localhost:6006](http://localhost:6006) でトレースを確認できます。
</Callout>
...
2. **Space ID**: 組織のスペース ID
3. **API Key**: Arize AX の設定で生成
4. **環境変数**: 資格情報を設定する
...
new ArizeExporter({
  // Phoenix の設定
  endpoint: 'https://your-collector.example.com/v1/traces',  // Phoenix で必須

  // Arize AX の設定
  spaceId: 'your-space-id',  // Arize AX で必須

  // 共有設定
  apiKey: 'your-api-key',  // 認証エンドポイントで必須
