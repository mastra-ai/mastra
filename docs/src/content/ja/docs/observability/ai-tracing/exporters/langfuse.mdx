---
title: "Langfuse Exporter | AI トレーシング | 可観測性 | Mastra ドキュメント"
...
1. **Langfuse アカウント**: [cloud.langfuse.com](https://cloud.langfuse.com) で登録するか、セルフホスト版をデプロイ
...
トレースは即座に Langfuse ダッシュボードに表示され、デバッグに最適です。
...
new LangfuseExporter({
  publicKey: process.env.LANGFUSE_PUBLIC_KEY!,
  secretKey: process.env.LANGFUSE_SECRET_KEY!,
  realtime: false,  // デフォルト - バッチでトレース処理
})
```
