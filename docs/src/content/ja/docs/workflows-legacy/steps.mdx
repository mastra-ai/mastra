---
title: "ステップの作成とワークフローへの追加（レガシー） | Mastra Docs"
description: "Mastra のワークフローにおけるステップは、入力・出力・実行ロジックを定義し、処理を体系的に管理するための枠組みを提供します。"
---
...
`.step()` と `.then()` を使って、ワークフロー内でステップを直接作成できます。次のコードは、2つのステップを順番に定義・連結・実行する方法を示しています。
...
ステップのロジックを別のエンティティで管理したい場合は、ステップを外部で定義してからワークフローに追加できます。次のコードは、ステップを独立して定義し、その後でワークフローに紐づける方法を示しています。
