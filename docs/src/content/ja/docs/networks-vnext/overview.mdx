...
console.log(await network.generate('What are the biggest cities in France?', { requestContext }));
```
...
```typescript
const stream = await network.stream('What are the biggest cities in France?', { requestContext });
