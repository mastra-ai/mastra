...
console.log(await network.generate('What are the biggest cities in France? How are they like?', { requestContext }));
// これは workflow1 を呼び出します。個別の都市を調査する場合、ルーティングエージェントによれば最も適したプリミティブだからです。
console.log(await network.generate('Tell me more about Paris', { requestContext }));
```
