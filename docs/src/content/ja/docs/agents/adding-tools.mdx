...
ツールとワークフローの依存性注入をサポートしています。`generate`または`stream`関数呼び出しに直接requestContextを渡すか、[サーバーミドルウェア](/docs/deployment/server#Middleware)を使用して注入することができます。
...
export const agent = new Agent({
  ...
  tools: {
    weather: createTool({
      ...
      description: "Get the weather in a location",
      inputSchema: z.object({ location: z.string() }),
      execute: async ({ context: { location }, requestContext }) => {
        const scale = requestContext.get("temperature-scale");
...
type MyRequestContext = {"temperature-scale": "celsius" | "fahrenheit"}

const requestContext = new RequestContext<MyRequestContext>();
