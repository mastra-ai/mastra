...
1. 型安全なrequestContextを通じて、実行時の設定変数をエージェントに渡す
  ...
  server: {
    middleware: [
      async (c, next) => {
        ...
        const requestContext = c.get<WeatherRequestContext>("requestContext");
...
ツールはrequestContext変数にアクセスでき、エージェントのrequestContextタイプに準拠する必要があります：
