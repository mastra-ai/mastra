...
For an AI SDK example, see [Using Mastra Memory](/ecosystem/agent-frameworks-and-ui/ai-sdk-ui#using-mastra-memory).
:::
...
Install the Mastra memory module along with a [storage adapter](/guides/storage/overview#supported-providers) for your database. The examples below use `@mastra/libsql`, which stores data locally in a `mastra.db` file.
...
When memory is enabled, [Studio](/guides/studio/overview) uses message history to display past conversations in the chat sidebar.
...
Once you have a thread, use [`recall()`](/reference/memory/recall) to retrieve its messages. It supports pagination, date filtering, and [semantic search](/guides/memory/semantic-recall).
...
Search by meaning (see [Semantic recall](/guides/memory/semantic-recall) for setup):
