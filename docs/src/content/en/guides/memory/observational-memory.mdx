...
To see how it works in practice, open [Studio](/guides/studio/overview) and navigate to an agent with OM enabled. The **Memory** tab displays:
...
- **[Message history](/guides/memory/message-history)**: High-fidelity record of the current conversation
- **[Working memory](/guides/memory/working-memory)**: Small, structured state (JSON or markdown) for user preferences, names, goals
- **[Semantic Recall](/guides/memory/semantic-recall)**: RAG-based retrieval of relevant past messages
- **[Multi-user threads](/guides/memory/multi-user-threads)**: How OM attributes facts to individual users when several people share a single thread
...
- [Message History](/guides/memory/message-history)
