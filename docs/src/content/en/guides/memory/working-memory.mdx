...
While [message history](/guides/memory/message-history) and [semantic recall](./semantic-recall) help agents remember conversations, working memory allows them to maintain persistent information about users across interactions.
...
If you use [Observational Memory](/guides/memory/observational-memory), `observationalMemory.observation.manageWorkingMemory` lets OM update working memory for the agent.
...
By default, working memory reaches the model as part of the system message. You can opt into delivering it as a [state signal](/guides/long-running-agents/signals) instead by setting `useStateSignals: true`:
