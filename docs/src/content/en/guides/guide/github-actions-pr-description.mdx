---
title: "Guide: Automating Multi-Language PR Descriptions in GitHub Actions | Mastra Workflows | Guides"
...
The finished action writes a description with summaries in English, Spanish, and Japanese, each marked with a flag emoji for easy scanning.

![Screenshot of PR description in GitHub UI](/img/guides/github-action-pr-summary.jpg)

## Before you begin

- A Mastra project (see [Quickstart](/docs/quickstart))
...
GitHub Actions workflows live in the `.github/workflows` directory. Create a `.github` directory at the root of your project, then create a `workflows` directory inside it and add a file named `pr-description.yml`.
...
\`\`\``,
  {
    onError: () => {
      const fileCount = execSync("git diff --name-only HEAD~1 | wc -l").toString().trim();
      writeFileSync(
        process.env.PR_DESCRIPTION_FILE!,
        `This PR contains a large diff (${fileCount} files changed) that exceeds the agent's context window limit.`
      );
    },
    onFinish: (result) => {
...
- Give your agent [tools](/docs/tools/overview) to fetch additional context
