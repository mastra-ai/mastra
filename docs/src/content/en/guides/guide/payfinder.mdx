...
1. Knowledge Store/Index
   ...
   - Implementation: We'll use OpenAI's text-embedding-3-small to create embeddings and store them in libSQLVector

2. Retriever
   - Finding relevant content via similarity search
   - Matching query embeddings with stored vectors
   - Implementation: We'll use libSQLVector to perform similarity searches on our stored embeddings
...
```bash
npm init -y
npm install @mastra/core @mastra/rag @ai-sdk/openai ai zod pdf.js-extract
```
...
export const ragAgent = new Agent({
  name: 'RAG Assistant',
  instructions: 
    `You are a helpful rag assistant that analyzes annual meeting statements.
