...
After running the [installation guide](/docs/v0/getting-started/installation) you'll need to install additional dependencies:

```bash copy
npm install @mastra/rag@^0 ai@^4.0.0
```
...
- A [Vector Query Tool](/reference/v0/tools/vector-query-tool) for performing semantic search over the vector store to find relevant content in papers
...
- [Filter RAG](/examples/v0/rag/usage/filter-rag) for filtering results using metadata
- [Cleanup RAG](/examples/v0/rag/usage/cleanup-rag) for optimizing information density
- [Chain of Thought RAG](/examples/v0/rag/usage/cot-rag) for complex reasoning queries using workflows
- [Rerank RAG](/examples/v0/rag/usage/cleanup-rag) for improved result relevance
