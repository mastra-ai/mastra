...
- `onDelegationComplete` logs completion and bails on errors
- `contextFilter` limits context to the last 10 messages for efficiency
...
Update the supervisor agent to include completion scoring in `defaultOptions` in `src/mastra/agents/supervisor-agent.ts`:
...
The scorer checks for substantial content, proper structure, and contextual information. If the task isn't complete, the supervisor will continue iterating. Now all hooks and completion scoring are configured in the agent's `defaultOptions`, making them apply to every call automatically.
