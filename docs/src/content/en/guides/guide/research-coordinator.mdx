...
You'll learn how to set up sub-agents with clear roles, configure a supervisor agent to coordinate them, use delegation hooks to control execution, and validate task completion with scorers.
...
The supervisor pattern requires memory to track conversation history and enable effective coordination. Memory is configured directly on the supervisor agent.
...
The supervisor coordinates research and writing tasks. Its instructions define the delegation strategy - when to use each sub-agent and how to synthesize results. Memory is configured directly on the agent.
...
Delegation hooks give you control over sub-agent execution. Use `onDelegationStart` to modify delegations before they happen, and `onDelegationComplete` to process results.
  ...
  const stream = await supervisorAgent.stream(
    `Research ${topic} and write a comprehensive article about it`,
    {
      ...
      delegation: {
        ...
        // Only pass last 10 messages to sub-agents
...
This configuration logs delegation events and modifies the research agent's prompt to request recent data. The `contextFilter` limits context to the last 10 messages for efficiency.

## Add completion scoring

Completion scorers automatically validate whether the task is complete. They prevent the supervisor from finishing prematurely.
...
export const taskCompleteScorer = createScorer({
  ...
  description:
    "Checks if the research and writing task has been fully completed",
...
Update `src/index.ts` to use the scorer:
...
The scorer checks for substantial content, proper structure, and contextual information. If the task isn't complete, the supervisor will continue iterating.
...
- [Completion Scorers](/docs/evals/scorers)
- [Delegation Hooks](/reference/streaming/agents/stream#delegation-hooks)
