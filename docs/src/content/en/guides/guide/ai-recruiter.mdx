...
You can test your workflow inside [Studio](/docs/v0/getting-started/studio) by starting the development server:
...
You can also test the workflow programmatically by calling [`.createRunAsync()`](/reference/v0/workflows/workflow-methods/create-run) and [`.start()`](/reference/v0/workflows/run-methods/start). Create a new file `src/test-workflow.ts` and add the following:
