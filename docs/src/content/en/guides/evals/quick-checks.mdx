...
For subjective or semantic evaluation, use [LLM-based scorers](/guides/evals/built-in-scorers) instead.
...
Each check is a standard `createScorer()` instance with a `preprocess` step and a `generateScore` step. They follow the same [four-step pipeline](/guides/evals/custom-scorers#the-four-step-pipeline) as any other scorer:
...
- [Built-in Scorers](/guides/evals/built-in-scorers)
