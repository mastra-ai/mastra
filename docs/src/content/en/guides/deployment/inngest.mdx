...
Durable agents wrap a Mastra Agent with Inngest's durable execution engine. The key benefit is that **the entire agentic loop is durable** - if your server crashes mid-way through a multi-step tool-calling conversation, Inngest automatically resumes exactly where it left off.

### Why use durable agents?

- **Crash recovery**: Server restarts mid-conversation? Inngest picks up where it left off
- **Long-running operations**: Multi-step tool calling preserved across failures
- **Built-in retries**: Automatic retry handling for transient failures
- **Observability**: Full visibility into agent execution in the Inngest dashboard
...
export const durableAgent = createInngestAgent({ agent, inngest });
```

### Registering with Mastra

When you add a durable agent to Mastra, the required workflow is automatically registered:

```ts title="src/mastra/index.ts"
import { Mastra } from "@mastra/core";
import { serve } from "@mastra/inngest";
import { inngest } from "./inngest";
import { durableAgent } from "./agents";

export const mastra = new Mastra({
  agents: { 
    myAssistant: durableAgent,  // Workflow auto-registered
  },
  server: {
    apiRoutes: [
      {
...
Monitor the agent's progress with callbacks:
...
const { output, cleanup } = await durableAgent.stream(
  [{ role: "user", content: "Analyze this data and create a report" }],
  {
    ...
    onStepFinish: (step) => {
      // Called after each LLM step (useful for multi-step tool calling)
      console.log("Step completed:", step.stepNumber);
    },
    onFinish: (result) => {
      // Called when the agent completes
      console.log("Done! Tokens used:", result.usage);
    },
    onError: (error) => {
...
Durable agents support memory for persistent conversations:
...
When an agent suspends (e.g., waiting for tool approval), you can resume it:

```ts
// Original execution suspends
const { output, runId, cleanup } = await durableAgent.stream(
  [{ role: "user", content: "Delete all files" }],
  {
    requireToolApproval: true,
    onSuspended: (data) => {
      console.log("Waiting for approval:", data.toolName);
    },
  }
);
...
const resumed = await durableAgent.resume(
  ...
  {
    onFinish: (result) => {
      console.log("Resumed execution completed");
    },
  }
);
```
...
When you call `createInngestAgent()`:

1. A durable workflow is created internally (`durable-agentic-loop`)
2. The `InngestAgent` object implements `DurableAgentLike`, which Mastra recognizes
3. When added to Mastra, the underlying agent is registered so the workflow can access it
4. The workflow is automatically registered with Mastra
5. When `serve()` is called, the workflow is exposed to Inngest

The workflow executes each step of the agentic loop durably - if execution is interrupted, Inngest resumes from the last completed step rather than starting over.

### API reference

For detailed API documentation, see the [createInngestAgent reference](/reference/workflows/createInngestAgent).
