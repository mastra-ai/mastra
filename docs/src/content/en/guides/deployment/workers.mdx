...
Subscribes to workflow events on the [PubSub](/guides/server/pubsub) bus and executes workflow steps. Every `workflow.start`, step transition, and lifecycle event flows through this worker.
...
The scheduler reads declarative `schedule` fields from your workflow definitions automatically. See [Scheduled workflows](/guides/workflows/scheduled-workflows) for how to declare schedules.
...
To run workers separately, configure a distributed [PubSub](/guides/server/pubsub) backend and use the `MASTRA_WORKERS` environment variable to control which workers start in each process.
...
- [PubSub](/guides/server/pubsub): Event delivery backends
- [Scheduled workflows](/guides/workflows/scheduled-workflows): Declare cron schedules on workflows
