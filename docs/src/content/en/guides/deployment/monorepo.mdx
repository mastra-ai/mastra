...
Deploying Mastra in a monorepo follows the same process as a standalone application. This guide covers monorepo-specific considerations. For the core build and deployment steps, see [Deploy a Mastra Server](/guides/deployment/mastra-server).
...
Keep `bundler` as a direct property in the object passed to `new Mastra()`. See [build-time configuration](/guides/deployment/mastra-server#build-time-configuration) for the supported entry-file shape.
...
- [Deploy a Mastra Server](/guides/deployment/mastra-server): Core build and deployment guide
