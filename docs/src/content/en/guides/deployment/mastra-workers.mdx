...
:::warning
The default in-memory PubSub cannot deliver events across processes. You must configure a distributed PubSub backend before splitting workers into separate containers.
:::
...
**Do not scale the scheduler worker.** Run exactly one instance. Multiple schedulers polling the same storage fire duplicate events for the same schedule.
