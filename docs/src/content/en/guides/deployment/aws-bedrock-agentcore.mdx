...
Install the required dependencies. The `bedrock-agentcore` package provides the `BedrockAgentCoreApp` HTTP server that implements the [AgentCore Runtime service contract](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-service-contract.html):

```bash npm2yarn
npm install bedrock-agentcore @opentelemetry/auto-instrumentations-node --legacy-peer-deps
```
...
Run [`mastra init`](/reference/cli/mastra#mastra-init) to set up a new Mastra project. When prompted, choose a provider (e.g. OpenAI) and enter your key:
...
Update your `package.json` file to edit the `main` entry and add scripts:
