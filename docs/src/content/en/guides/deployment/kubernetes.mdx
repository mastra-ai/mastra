...
<Steps>
  ...
  <StepItem>
    Verify the pods are running and the service responds:
...
Multiple clients can observe the same run at once. Each `observe()` call receives the full stream, so a user watching from two devices, or several users following one run, all stay in sync.
...
- When several pods start at once against an uninitialized database, they can race to create the schema and a pod may fail to start. Start with one replica so the schema is created once, then scale up.
