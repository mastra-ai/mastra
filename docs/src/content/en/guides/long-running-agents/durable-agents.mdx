...
A durable agent wraps a regular [`Agent`](/guides/agents/overview) so the agentic loop runs inside a workflow. Events flow through [PubSub](/guides/server/pubsub), which means a client can disconnect and reconnect without missing chunks. The run state is persisted, so it survives process restarts.
...
Visit [Background tasks](/guides/long-running-agents/background-tasks) for the full background task guide, including configuration, subagents, and suspend/resume.
...
- [Worker overview](/guides/deployment/workers)
