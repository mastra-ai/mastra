...
With a `threadId`, the schedule sends a [signal](/guides/long-running-agents/signals) into that thread, so the prompt joins the agent's conversation. Threaded schedules require a `resourceId` alongside the `threadId`.
...
Threaded schedules accept extra fields that control how the signal behaves, including the signal type, XML tag, tag attributes, and active-or-idle delivery behavior. They mirror the options [`agent.sendSignal()`](/guides/long-running-agents/signals) accepts and stay JSON-serializable so they persist with the schedule.
...
Workflow schedules created this way are independent of the declarative `schedule` field on `createWorkflow`, see [scheduled workflows](/guides/workflows/scheduled-workflows) for the declarative form and Studio views.
...
- [Signals](/guides/long-running-agents/signals): the delivery mechanism behind threaded schedules.
- [Scheduled workflows](/guides/workflows/scheduled-workflows): declare a cron schedule on a workflow definition.
