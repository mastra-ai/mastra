...
For a one-off completion check within a single `stream()` call, use [`isTaskComplete`](/guides/agents/supervisor-agents) instead.

## Quickstart

Goals require a configured [storage](/guides/storage/overview) backend and a memory-backed thread. Add a `goal` config to the agent, a judge model is required for the goal to do anything, then set an objective for a thread:
...
- [Supervisor agents](/guides/agents/supervisor-agents): `isTaskComplete` and the rubric scorer
- [Signal providers](/guides/long-running-agents/signal-providers): how the objective is projected into context
- [Memory storage](/guides/storage/overview): the storage backend goals require
