...
Background tasks require a configured [storage](/guides/storage/overview) backend on the Mastra instance. Tasks are persisted so they survive process restarts.
...
When a tool call dispatches as a background task, two streams may surface lifecycle events for it: the agent's own stream and the [`backgroundTaskManager.stream()`](/guides/long-running-agents/background-tasks#subscribe-to-all-task-events) SSE stream. Each stream covers a different set of chunk types:
