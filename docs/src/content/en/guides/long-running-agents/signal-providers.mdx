...
A signal provider monitors an external source, such as GitHub, Slack, continuous integration (CI), or your own API, and pushes [notification signals](/guides/long-running-agents/signals#notification-signals) into subscribed agent threads.
...
A signal provider is the producing side of the [signals](/guides/long-running-agents/signals) system. It brings external events into a thread, while the signal APIs control how the thread consumes them.
...
- [Notification signals](/guides/long-running-agents/signals#notification-signals)
