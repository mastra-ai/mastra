...
- [Slack](/guides/capabilities/channels/slack)
- [Microsoft Teams](/guides/capabilities/channels/teams)
- [Discord](/guides/capabilities/channels/discord)
- [Telegram](/guides/capabilities/channels/telegram)
- [WhatsApp](/guides/capabilities/channels/whatsapp)

[More](/guides/capabilities/channels/other-adapters) lists additional platforms. Mastra channels work with compatible [Chat SDK adapters](https://chat-sdk.dev/adapters) beyond the platforms listed here, and the same Mastra configuration pattern applies across adapters.
...
We recommend configuring [storage](/guides/storage/overview) for channels. Storage lets Mastra persist channel state, thread subscriptions, tool approvals, and memory across restarts:
...
Channels route messages through the agent's [signal pipeline](/guides/long-running-agents/signals), and each run acquires a lease on its thread so one run owns the conversation at a time.
...
Vercel's managed Redis integration and Upstash Redis both work well. For more on when a distributed pub/sub is needed, see the [PubSub guide](/guides/server/pubsub) and the [`RedisStreamsPubSub` reference](/reference/pubsub/redis-streams).

## Related

- [Channels reference](/reference/agents/channels)
- [AgentController channels](/guides/agent-controller/channels)
