...
Observational memory lets the agent learn long-lived facts from past conversations. Storage on the `Mastra` instance is required, see the [Memory overview](/guides/memory/overview) for the prerequisites.
...
If a required adapter is missing, Mastra throws a descriptive error at agent run time. See [Semantic recall](/guides/memory/semantic-recall) for the full list of supported vector stores and embedders.

## Related

- [Memory overview](/guides/memory/overview): Concepts and core configuration.
- [BuilderAgentDefaults reference](/reference/editor/agent-builder/builder-agent-defaults): The full `memory` field schema.
- [Configuration](/guides/agent-builder/configuration): Wire `memory` alongside the rest of the Builder config.
