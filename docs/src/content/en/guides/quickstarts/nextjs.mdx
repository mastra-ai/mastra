...
In ~5 minutes, you'll have a functional AI-powered application ready to extend with your own tools and logic.

<figure>
![cp](https://gist.github.com/user-attachments/assets/23a0fe24-9551-41ec-bd22-c1f25a23d9d3)
<figcaption class="text-sm text-center">You'll build this AI agent that calls a weather tool, shows the JSON result, streams a weather summary in the chat UI, and remembers conversation history.  </figcaption>
</figure>
...
You'll call `weather-agent.ts` from your Next.js route in the next steps.
  ...
  return (
    <div className="w-full p-6 relative size-full h-screen">
      <div className="flex flex-col h-full">
        ...
        <PromptInput onSubmit={handleSubmit} className="mt-top-20">
          <PromptInputBody>
            <PromptInputTextarea
...
This component wires up `useChat()` with `DefaultChatTransport` so prompts are sent to `/api/chat` and responses stream back in real time. 

It renders text with the `<MessageResponse>` component, while any tool invocations are displayed with `<Tool>` .
...
- Run your agent in [Studio](/docs/v1/getting-started/studio)
- Learn more about the [Mastra Project Structure](/docs/v1/getting-started/project-structure)
- Read more about integrating Mastra in your [Next.js project](/docs/v1/frameworks/web-frameworks/next-js) and with Vercel's [AI SDK](/docs/v1/frameworks/agentic-uis/ai-sdk).