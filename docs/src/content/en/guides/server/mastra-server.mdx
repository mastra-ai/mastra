...
This page covers the [`server`](/reference/configuration#server-options) configuration options passed to the `Mastra` constructor. For running Mastra with your own HTTP server (Hono, Express, etc.), visit [Server Adapters](/guides/server/server-adapters).

:::

## Server features

- **[Middleware](/guides/server/middleware)**: Intercept requests for authentication, logging, CORS, or injecting request-specific context.
- **[Custom API Routes](/guides/server/custom-api-routes)**: Extend the server with your own HTTP endpoints that have access to the Mastra instance.
- **[Request Context](/guides/server/request-context)**: Pass request-specific values to agents, tools, and workflows based on runtime conditions.
- **[Server Adapters](/guides/server/server-adapters)**: Run Mastra with Express, Hono, or your own HTTP server instead of the generated server.
- **[Custom Adapters](/guides/server/custom-adapters)**: Build adapters for frameworks not officially supported.
- **[Mastra Client SDK](/guides/server/mastra-client)**: Type-safe client for calling agents, workflows, and tools from browser or server environments.
- **[A2A](/guides/agents/a2a)**: Expose agents through A2A agent cards and task streams, plus push notifications.
- **[Authentication](/guides/server/auth)**: Secure endpoints with JWT, Clerk, Supabase, Firebase, Auth0, or WorkOS.
...
Mastra servers can be deployed to any Node.js-compatible environment. Deploy it to production using the [Mastra platform](/docs/mastra-platform/overview) or your own infrastructure. Visit the [deployment docs](/guides/deployment/overview) to learn how.
...
This behavior is only configurable by using [server adapters](/guides/server/server-adapters#stream-data-redaction). For server adapters, stream data redaction is enabled by default, too.
...
- Add [middleware](/guides/server/middleware)
- Create [custom API routes](/guides/server/custom-api-routes)
- Add [authentication](/guides/server/auth)
- [Deploy](/guides/deployment/overview) your server to production
