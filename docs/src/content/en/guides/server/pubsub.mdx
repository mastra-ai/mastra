...
- **Scheduled workflows**: The scheduler dispatches due runs by publishing to the workflow topic. See [Scheduled workflows](/guides/workflows/scheduled-workflows).
- **Background tasks**: A task manager dispatches work to a worker group and fans task lifecycle updates out to subscribers, which is how background task streams stay live. See [Background task streaming](/guides/long-running-agents/background-tasks#subscribe-to-all-task-events).
...
- [Background task streaming](/guides/long-running-agents/background-tasks#subscribe-to-all-task-events)
- [Scheduled workflows](/guides/workflows/scheduled-workflows)
- [Workers](/guides/deployment/workers): Run workflow orchestration and background tasks in dedicated processes using PubSub
