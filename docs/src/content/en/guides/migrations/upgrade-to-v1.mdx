...
export const weatherTool = createTool({
  ...
  execute: async (input, context) => {
    ...
    return getWeather(input.location, userTier);
  },
});
```
  ...
  execute: async (input, context) => {
    if (context?.mcp) {
      ...
      const result = await context.mcp.elicitation.sendRequest({
        message: `Is it ok to fetch the account balance for account ${input.accountId}?`,
        requestedSchema: {
          type: "object",
          properties: {
      ...
      if (result.action === "accept") {
        return {
          balance: getAccountBalance(input.accountId),
        };
      }
    }
  },
