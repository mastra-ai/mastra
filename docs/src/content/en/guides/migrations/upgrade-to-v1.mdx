...
The `format` parameter has been removed from `agent.stream()`. AI SDK stream transformations are now handled by the `@mastra/ai-sdk` package.
...
The `@mastra/ai-sdk` package provides utilities for seamless integration with Vercel AI SDK. For streaming responses in API routes:
...
Learn more in the [Using Vercel AI SDK documentation](/docs/frameworks/agentic-uis/ai-sdk).
