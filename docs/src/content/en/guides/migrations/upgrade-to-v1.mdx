...
2. Memory methods now return consistent object wrappers
3. `format` parameter removed from all get methods
4. MessageList `.v2()` methods renamed to `.db()`
5. AI SDK format conversion moved to `@mastra/ai-sdk/ui`
...
The `rememberMessages` method now returns an object wrapper for consistency:

```typescript
// Before
const messages = await memory.rememberMessages({ threadId, resourceId });
// messages was MastraDBMessage[]

// After
const result = await memory.rememberMessages({ threadId, resourceId });
const messages = result.messages; // MastraDBMessage[]
```

#### 3. Update MessageList Usage
