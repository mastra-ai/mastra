...
- Start your Mastra app against the new credentials and confirm that an existing thread or workflow run loads as expected in [Studio](/guides/studio/observability).
...
- `MastraStorageExporter` persists observability events to Mastra Storage for [Studio](/guides/studio/observability).
...
See the [observability overview](/guides/observability/overview) for the full configuration, including composite storage with DuckDB for metrics.
...
See [Studio deployment](/guides/studio/deployment#mastra-platform) for details.
...
Update any clients pointing to your old Mastra Cloud URL to the new Server or Studio URL. Verify traces appear in the new platform by checking the [Studio observability dashboard](/guides/studio/observability). Delete your old Mastra Cloud project once everything is confirmed working.
