...
Cloud Store data lives in a Turso-hosted libSQL database. Exporting it requires the database URL and an auth token (both surfaced in the dashboard), then a SQL dump via the [Turso CLI](https://docs.turso.tech/cli).
...
<Steps>
  ...
  <StepItem>
    ...
    Use the credentials from the dashboard (or from support if you requested them) to authenticate the Turso CLI, then dump the database by name:

    ```bash
    turso auth login
    turso db shell <database-name> ".dump" > mastra-cloud-dump.sql
    ```

    Replace `<database-name>` with the name portion of your `MASTRA_STORAGE_URL` (e.g. if the URL is `libsql://my-app-acme.turso.io`, the database name is `my-app-acme`). Authenticating via `turso auth login` keeps credentials out of shell history, process listings, and terminal logs.
