...
Streaming methods now return output classes with promise-based properties. Update code that uses `processDataStream()` or iterates directly over streams:

```diff
  // Agent streaming
  const stream = await agent.stream({ messages: [...] });
- response.processDataStream({ onChunk: async (chunk) => { ... } });
...
+ for await (const chunk of networkStream.fullStream) { ... }

  // Workflow streaming (stream is directly iterable)
  const stream = await run.stream({ inputData: {...} });
  for await (const chunk of stream) { ... }
```

All streaming methods provide `fullStream` for iteration and promise-based properties like `status`, `result`, and `usage`.
