...
### Changed: `RuntimeContext` utilities to `RequestContext`

Client SDK utility functions have been renamed from `RuntimeContext` to `RequestContext`. This change aligns with the global rename for clarity about request-specific context.

To migrate, update utility function imports and calls.

```diff
- import {
-   parseClientRuntimeContext,
-   base64RuntimeContext,
-   runtimeContextQueryString,
...
- const context = parseClientRuntimeContext(data);
- const encoded = base64RuntimeContext(context);
- const qs = runtimeContextQueryString(context);
+ const context = parseClientRequestContext(data);
+ const encoded = base64RequestContext(context);
+ const qs = requestContextQueryString(context);
```

### Changed: Pagination parameters from `offset/limit` to `page/perPage`
...
To migrate, update pagination parameters in all client SDK method calls.
...
### Changed: Client SDK types from `Get*` to `List*`
...
### Changed: `getMemoryThread` parameter structure
...
+ const thread = await client.memory.getMemoryThread({ threadId });
```

## Removed

### Removed: `toAISdkFormat` function
...
### Removed: Network memory methods
...
### Removed: Deprecated stream endpoints
...
### Removed: Network memory API endpoints
...
### Removed: Eval-related client SDK types
