...
### Changed: Pagination from `offset/limit` to `page/perPage`
...
### Changed: `getMessagesPaginated` to `listMessages`
...
### Changed: Storage `getMessages` and `saveMessages` signatures
...
### Changed: Vector store API from positional to named arguments
...
### Changed: Vector store method renames
...
### Changed: PGVector constructor from connection string to object
...
### Changed: PGVector `defineIndex` to `buildIndex`
...
### Changed: PostgresStore `schema` to `schemaName`
...
### Changed: Score storage methods to `listScoresBy*` pattern
...
### Removed: Non-paginated storage functions
...
### Removed: `getTraces` and `getTracesPaginated`
...
### Removed: TABLE_EVALS from MSSQL storage
