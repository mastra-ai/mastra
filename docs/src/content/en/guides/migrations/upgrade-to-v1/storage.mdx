...
The `getThreadsByResourceId()` method has been replaced with a more flexible `listThreads()` method that supports optional filtering by `resourceId` and/or `metadata`. This change provides more powerful filtering capabilities.

To migrate, use the memory store and the new `listThreads()` method with an optional filter object.
