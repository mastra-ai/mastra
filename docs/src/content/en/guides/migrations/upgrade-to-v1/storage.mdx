...
If you're using PostgreSQL or LibSQL storage with evals/scoring features, you need to update your database schema to rename a column in the `mastra_scorers` table.

:::important[Who needs this migration]
Only if you use `@mastra/pg` or `@mastra/libsql` with evals/scoring (the `mastra_scorers` table exists in your database). If you don't use scoring or the table doesn't exist, skip this migration.
:::

:::important[What breaks without it]
New score records won't be saved correctly, and querying scores by request context will fail.
:::

:::important[Deployment order]
Deploy your v1 code first, then run this migration. The v1 code is compatible during the transition.
:::

Run this SQL through your normal migration process (e.g., Prisma Migrate, Drizzle Kit, or your DBA review process):
...
### Workflow snapshot column (TEXT → JSONB)

The `snapshot` column in the `mastra_workflow_snapshot` table has been changed from `TEXT` to `JSONB`.

**PostgreSQL users with existing tables must migrate the column type.** Other databases do not require migration.
...
```sql
ALTER TABLE "your_schema".mastra_workflow_snapshot
ALTER COLUMN snapshot TYPE jsonb
USING snapshot::jsonb;
```
