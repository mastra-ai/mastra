...
### CompositeStorage for mixing storage backends

`CompositeStorage` combines storage domains from different adapters. Use it when you need different databases for different purposes - for example, PostgreSQL for memory and workflows, but a specialized database for observability.
...
See the [CompositeStorage reference](/reference/storage/composite) for more details.
