...
The `getThreadsByResourceId()` method has been replaced with `listThreads()`. `listThreads()` supports filtering by `resourceId` or `metadata` or a combination of both.
...
```shell
npx @mastra/codemod@beta v1/list-threads-by-resource-to-list-threads .
```
