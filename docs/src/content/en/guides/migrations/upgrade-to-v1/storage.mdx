...
The `getThreadsByResourceId()` method has been replaced with `listThreads()`. This new method provides more flexibility by making `resourceId` optional and adding support for metadata filtering.

To migrate, use the new `listThreads()` method with the `filter` parameter.
...
The new `listThreads()` method also supports filtering by metadata:

```typescript
// Filter by metadata only (across all resources)
const threads = await storage.listThreads({
