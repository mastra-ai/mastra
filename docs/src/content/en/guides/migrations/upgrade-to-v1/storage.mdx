...
If you use `@mastra/pg` or `@mastra/libsql` with evals/scoring (the `mastra_scorers` table exists in your database), rename the `runtimeContext` column:
...
**PostgreSQL only.** The `metadata` column in `mastra_threads` changed from TEXT to JSONB:
...
**PostgreSQL only.** The `snapshot` column in `mastra_workflow_snapshot` changed from TEXT to JSONB:
