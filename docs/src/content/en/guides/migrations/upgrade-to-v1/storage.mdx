...
**Who needs this:** Only if you use `@mastra/pg` or `@mastra/libsql` with evals/scoring (the `mastra_scorers` table exists in your database). If you don't use scoring or the table doesn't exist, skip this migration.

**What breaks without it:** New score records won't be saved correctly, and querying scores by request context will fail.

**Deployment order:** Deploy your v1 code first, then run this migration. The v1 code is compatible during the transition.
