...
Storage APIs have been standardized with consistent pagination and naming patterns across all methods. Additionally, storage operations now use a domain-based architecture that enables composite storage configurations.

## Changed

### Domain architecture

Storage operations are now organized into **domains**, each representing a specific type of data. This domain-based architecture enables **composite storage**, where you can use different storage backends for different domains (e.g., PostgreSQL for memory, DynamoDB for workflows).

1. **Storage Domains**: Four distinct domains organize storage operations:
   - `memory` - Threads, messages, and resources
   - `workflows` - Workflow snapshots and runs
   - `evals` - Evaluation scores
   - `observability` - Traces and spans

2. **Domain Stores**: Access domain-specific operations via `getStore(domain)`, which returns a store instance for that domain.

3. **Composite Storage**: Configure different storage backends for different domains, enabling optimized storage strategies per data type.

### Method access

All storage operations must now be accessed through domain stores. The general migration pattern is:

```diff
- // Old: Direct storage access
- const messages = await storage.listMessages({ threadId: 'thread-1' });
- await storage.saveMessages({ messages: [...] });
- const thread = await storage.getThreadById({ threadId: 'thread-1' });
- const runs = await storage.listWorkflowRuns({ workflowId: 'workflow-1' });
- const scores = await storage.getScores({ scorerId: 'scorer-1' });

+ // New: Access via domain stores
+ const memoryStore = await storage.getStore('memory');
+ const messages = await memoryStore.listMessages({ threadId: 'thread-1' });
+ await memoryStore.saveMessages({ messages: [...] });
+ const thread = await memoryStore.getThreadById({ threadId: 'thread-1' });
```
...
To migrate, update all pagination parameters from `offset/limit` to `page/perPage`. Note that `page` is 0-indexed. Also remember to access storage operations through domain stores.
...
To migrate, rename the method and update pagination parameters. You can now use `perPage: false` to fetch all records. Also remember to access storage operations through domain stores.
...
To migrate, rename the method call and update pagination parameters. Also remember to access storage operations through domain stores.
...
### Workflow snapshot methods renamed

Workflow snapshot methods have been renamed and now require accessing through the workflows domain store. The parameter name has also changed from `workflowName` to `workflowId` for consistency.

To migrate, access the workflows domain store and update method names and parameters.
...
To migrate, remove format parameters and update code to work with the consistent return type. Also remember to access storage operations through domain stores.
...
+ const result = await memoryStore.saveMessages({ messages: mastraDBMessages });
...
To migrate, update method names from `getScores` to `listScoresByScorerId` and related variants. Also remember to access storage operations through domain stores.
...
To migrate, use paginated methods. For fetching all records, use `perPage: false`. Also remember to access storage operations through domain stores.
...
The `getTraces()` and `getTracesPaginated()` methods have been removed from direct storage access. Traces are now accessed through the observability domain store. This change provides better separation of concerns between core storage and observability features.

To migrate, use the observability domain store via `getStore('observability')`.

```diff
- const traces = await storage.getTraces({ traceId: 'trace-123' });
- const paginated = await storage.getTracesPaginated({ page: 0, perPage: 20 });

+ // Access traces through observability domain store
...
### TABLE_EVALS from storage

The `TABLE_EVALS` table has been removed from storage implementations. This change reflects the removal of legacy evals functionality.