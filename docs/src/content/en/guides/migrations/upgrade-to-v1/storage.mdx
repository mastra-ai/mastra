...
BEGIN
  IF EXISTS (
    ...
    WHERE table_name = 'mastra_scorers'
    AND column_name = 'runtimeContext'
  ) THEN
...
**LibSQL:**
```sql
-- LibSQL/SQLite doesn't support conditional DDL
-- Check if you have the old column name before running:
-- SELECT * FROM pragma_table_info('mastra_scorers') WHERE name = 'runtimeContext';
...
- PostgreSQL version is safe to run multiple times (checks if column exists)
