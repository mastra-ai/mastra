...
### Changed: `RuntimeContext` to `RequestContext`
...
### Changed: Voice methods moved to `agent.voice` namespace
...
### Changed: `agent.fetchMemory()` to `(await agent.getMemory()).query()`
...
### Changed: Processor method names from `get*` to `list*`
...
To migrate, update processor method names and the parameter from `runtimeContext` to `requestContext`.
...
### Changed: Default options method renames for AI SDK versions
...
### Changed: `modelSettings.abortSignal` to top-level `abortSignal`
...
### Changed: `output` to `structuredOutput.schema`
...
### Removed: `generateVNext` and `streamVNext` methods
...
### Removed: `format` parameter from `stream()` and `generate()`
...
### Removed: `TMetrics` generic parameter from Agent
...
### Removed: `experimental_generateMessageId` option

The `experimental_generateMessageId` option has been removed from agent generate/stream methods. Message IDs are now automatically generated internally.

To migrate, remove the `experimental_generateMessageId` option. Message ID generation is handled automatically by the Memory layer.
