...
The `id` field is now required when creating an Agent. Previously, agents could be created without an explicit ID, but this is no longer supported. This change enables typesafe autocomplete for `mastra.getAgentById()` and ensures consistent agent identification throughout the system.
...
// Retrieve by ID (now with autocomplete!)
