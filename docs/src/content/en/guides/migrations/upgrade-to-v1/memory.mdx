...
For more information on using processors with agents, see the [Processors docs](/guides/agents/processors). For a complete example with memory, see the [TokenLimiter reference](/reference/processors/token-limiter-processor#extended-usage-example).
