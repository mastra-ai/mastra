...
### Changed: Top-level imports to subpath imports
...
To migrate, update all imports from `@mastra/core` to use the appropriate subpath. The main index now only allows `Mastra` and `type Config` - everything else must be imported from specific subpaths.
...
### Changed: Direct property access to getter methods
...
### Changed: `experimental_auth` to `auth`
...
### Removed: `memory` property from Mastra class
