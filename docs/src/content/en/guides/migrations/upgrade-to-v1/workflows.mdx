...
Step suspendPayload is now validated for steps that have a suspend schema defined. This also uses the `validateInputs` flag to determine whether to validate the suspendPayload or not.
