...
### New `resumeStream()` method

A new `resumeStream()` method is now available for resuming suspended workflows with streaming support. This method was previously only available as the experimental `resumeStreamVNext()`.
...
```diff
- const { stream, getWorkflowState } = await run.streamVNext({ inputData });
+ const stream = await run.stream({ inputData });
