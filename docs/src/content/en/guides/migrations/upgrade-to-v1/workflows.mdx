...
### Changed: Workflow step `execute` signature to `(inputData, context)` format

Workflow step `execute` functions now receive `inputData` as the first parameter and `context` as the second parameter. This change aligns with the tool signature changes and provides clearer separation between step inputs and execution context.

To migrate, update step signatures to use `inputData` as the first parameter (typed from `inputSchema`) and access context properties through the `context` parameter.
...
+     // inputData is the first parameter
+     // context contains: { mastra, requestContext, tracingContext, suspend, resumeData, workflow }
+   },
  });
```

### Changed: `RuntimeContext` to `RequestContext` in step context
...
### Changed: `createRunAsync` to `createRun`
...
### Changed: `runCount` to `retryCount` (deprecated)
...
### Removed: Legacy workflows export
...
### Removed: `pipeThrough` and `pipeTo` methods from WorkflowRunOutput
...
### Removed: `waitForEvent` API

The `waitForEvent` API has been removed from workflows. Use the streaming API or events system instead.

To migrate, use workflow streaming or the events API for waiting on workflow execution milestones.

```diff
- await workflow.waitForEvent('step-complete');
