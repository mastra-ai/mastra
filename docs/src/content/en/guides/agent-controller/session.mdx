...
See [Threads and state](/guides/agent-controller/threads-and-state) for the full lifecycle.
...
See [Modes](/guides/agent-controller/modes) for how modes carry their own model.
...
Grants are intentionally in-memory and reset when the session restarts. See [Tool approvals](/guides/agent-controller/tool-approvals) for the approval flow.
...
Queue a follow-up with `agentController.followUp({ content })`. To redirect the agent mid-run instead of waiting, use `agentController.steer({ content })`. Both build on [signals](/guides/long-running-agents/signals).
...
See [Threads and state](/guides/agent-controller/threads-and-state) for defining a schema.
