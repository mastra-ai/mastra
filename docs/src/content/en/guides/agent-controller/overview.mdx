...
- **Resume a conversation exactly where the user left off.** Persistent [threads and state](/guides/agent-controller/threads-and-state) reload the active mode, model, and progress across restarts, so a coding agent or assistant picks up mid-task instead of starting over.
- **Gate destructive actions behind human approval.** [Tool approvals and permission policies](/guides/agent-controller/tool-approvals) let you require confirmation for risky operations like file writes or deployments, while trusted tools run automatically.
- **Move a task through distinct phases without losing context.** [Modes](/guides/agent-controller/modes) switch the agent's instructions, tools, and model on the same thread, so you can build a plan-then-execute coding agent or a research-then-draft assistant.
...
- **Delegate focused work to child agents.** [Subagents](/guides/agent-controller/subagents) run subtasks with constrained tools and can fork the parent conversation, so a research mode can spin off web search or code review without polluting the main thread.
- **Drive a live UI from agent activity.** The [event system](/guides/agent-controller/session) emits typed events and coalesced display snapshots, so your TUI or web app reflects message updates, mode changes, and pending approvals in real time.
...
You could assemble all of this yourself on top of the [Agent class](/guides/agents/overview), which exposes the full agent loop, tools, and memory. The AgentController provides opinionated defaults for an ongoing session where the agent acts as a collaborator rather than a one-shot endpoint. Reach for the Agent class directly when you want full control or a request-response call. Reach for the AgentController when you want the collaborative-session model without building the runtime around it.

## Key capabilities

- **Session**: Per-conversation state: active thread, mode, model, grants, follow-ups, token usage, and the display snapshot: accessed through `agentController.session`. See [Session](/guides/agent-controller/session).
- **Modes**: Define distinct agent personalities (instructions, tools, model) and switch between them without losing conversation context. See [Modes](/guides/agent-controller/modes).
- **Threads and state**: Persist conversations and structured state across sessions, users, and mode switches. See [Threads and state](/guides/agent-controller/threads-and-state).
- **Subagents**: Spawn focused child agents with constrained tools for subtasks, optionally forking the parent conversation. See [Subagents](/guides/agent-controller/subagents).
- **Tool approvals and permissions**: Configure which tools require user confirmation and grant session-wide exceptions, plus handle interactive tool suspension. See [Tool approvals](/guides/agent-controller/tool-approvals).
...
- **Follow-ups and steering**: Queue messages while the agent is running, or inject mid-stream instructions to redirect the agent. Built on [signals](/guides/long-running-agents/signals).
...
- **Observational memory**: Automatic summarization and reflection across threads for long-running agent sessions. See [Observational memory](/guides/memory/observational-memory).
...
- [Session](/guides/agent-controller/session): The per-conversation state on an AgentController
- [Modes](/guides/agent-controller/modes): Define and switch between agent personalities
- [Threads and state](/guides/agent-controller/threads-and-state): Manage persistent conversations
- [Subagents](/guides/agent-controller/subagents): Delegate focused subtasks
- [Tool approvals and permissions](/guides/agent-controller/tool-approvals): Human-in-the-loop gating
