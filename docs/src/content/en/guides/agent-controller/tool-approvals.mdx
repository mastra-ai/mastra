...
The AgentController owns permission _policy_ (which categories require approval). The [`Session`](/guides/agent-controller/session) owns the _grants_ a user makes during a conversation. Grant a category or tool for the rest of the session so it runs without further prompting:
...
- [Session](/guides/agent-controller/session)
- [Subagents](/guides/agent-controller/subagents)
