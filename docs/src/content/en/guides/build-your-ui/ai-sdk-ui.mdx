...
The `withMastra()` function wraps an AI SDK model with Mastra features, allowing you to use [processors](/docs/v1/agents/processors) and [memory](/docs/v1/memory/memory-processors) with AI SDK's `generateText`, `streamText`, `generateObject`, and `streamObject` functions.
