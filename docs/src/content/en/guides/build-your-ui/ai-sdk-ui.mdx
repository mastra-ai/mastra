...
If you have a client-side stream from `agent.stream(...)` and want AI SDK-formatted parts, use the `fullStream` property directly with `toAISdkStream`:
...
const uiMessageStream = createUIMessageStream({
  execute: async ({ writer }) => {
    for await (const part of toAISdkStream(response, { from: "agent" })) {
      writer.write(part);
    }
  },
});
