...
export const approvalWorkflow = createWorkflow({
  ...
  inputSchema: z.object({ requestType: z.string(), details: z.string() }),
  ...
  const { messages, sendMessage, setMessages, status } = useChat({
    transport: new DefaultChatTransport({
      ...
      prepareSendMessagesRequest: ({ messages }) => {
        ...
        return {
          body: { inputData: { requestType: metadata.requestType, details: metadata.details } },
        };
      },
    }),
  });
    ...
    <div>
      {!suspended ? (
        <form onSubmit={(e) => {
          e.preventDefault();
          setMessages([]);
          sendMessage({ text: "Start", metadata: { requestType, details } });
        }}>
          <input value={requestType} onChange={(e) => setRequestType(e.target.value)} placeholder="Request type" />
