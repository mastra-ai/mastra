...
`handleChatStream()` and `handleNetworkStream()` are compatible with AI SDK v6 response helpers such as [`createUIMessageStreamResponse()`](https://ai-sdk.dev/docs/reference/ai-sdk-ui/create-ui-message-stream-response) and [`pipeUIMessageStreamToResponse()`](https://ai-sdk.dev/docs/reference/ai-sdk-ui/pipe-ui-message-stream-to-response). For best TypeScript inference, pass `messages` as `UIMessage[]` from your installed `ai` version.
...
If your app is typed against AI SDK v6, use `toAISdkV6Stream()` instead. `toAISdkStream()` remains the default path for AI SDK v5.
