...
Agents use tools to call APIs, query databases, or run custom functions from your codebase. Tools give agents capabilities beyond language generation by providing structured access to data and performing clearly defined operations. You can also load tools from remote [MCP servers](/guides/mcp/overview) to expand an agent's capabilities.
...
Add subagents through the `agents` configuration to create a [supervisor](/guides/agents/supervisor-agents). Mastra converts each subagent to an `agent-<key>` tool. Include a `description` on each subagent so the supervisor knows when to delegate.
...
Use `hooks` to run custom logic before and after every tool call an agent makes. Hooks apply to all tool sources: assigned tools, memory tools, toolsets, client tools, agent and workflow tools, and [workspace tools](/guides/workspace/overview#tool-hooks). Common uses include logging, auditing, input validation, and blocking specific calls.
...
The task tools manage a structured, durable task list for an agent run. They require [Memory](/guides/memory/overview) so the list is persisted in a thread-scoped store.
...
Only one task can be `in_progress` at a time. The list is stored in the thread-scoped `threadState` storage domain and projected onto the agent's [state-signal](/guides/long-running-agents/signals) lane, so it survives observational-memory truncation. See the [Task tools reference](/reference/tools/task-tools) for full schemas.

The [AgentController](/guides/agent-controller/overview) automatically includes all built-in tools in every mode, you don't need to add them manually. See [Tool approvals](/guides/agent-controller/tool-approvals#built-in-tools) for AgentController-specific behavior.
...
- [Background tasks](/guides/long-running-agents/background-tasks): Run long-running tools without blocking the agent loop
...
- [Tools with structured output](/guides/agents/structured-output#combine-tools-and-structured-output): Model compatibility when combining tools and structured output
- [Agent approval](/guides/agents/agent-approval)
...
- [Request context](/guides/server/request-context)
