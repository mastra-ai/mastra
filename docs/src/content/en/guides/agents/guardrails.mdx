...
For an introduction to processor behavior and custom processors, including how to add them to an agent, see [Processors](/guides/agents/processors).
...
:::note
When streaming responses over HTTP, Mastra redacts sensitive request data (system prompts, tool definitions, API keys) from stream chunks at the server level by default. See [Stream data redaction](/guides/server/mastra-server#stream-data-redaction) for details.
:::
...
For more on how violation callbacks integrate with the processor pipeline, see [Violation callbacks](/guides/agents/processors#violation-callbacks) in the Processors documentation.
...
By default, processors run sequentially. Guardrails that only `block` (and never mutate messages) are independent and can run using a [workflow processor](/guides/agents/processors#use-workflows-as-processors).
...
See [workflows as processors](/guides/agents/processors#use-workflows-as-processors) for more details on `.parallel()` and `.map()`.
...
- [Processors](/guides/agents/processors): Processor behavior and execution order. Also covers custom processors and retry behavior
- [`Processor` Interface](/reference/processors/processor-interface): API reference for the `Processor` interface
- [Memory Processors](/guides/memory/memory-processors): Processors for message history plus semantic recall and working memory
