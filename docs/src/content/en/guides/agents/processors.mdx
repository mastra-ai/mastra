...
With this ordering, an output guardrail that calls `abort()` skips memory processors and prevents messages from being saved. See [Memory Processors](/guides/memory/memory-processors#processor-execution-order) for details.
...
**For security and validation processors**, see the [Guardrails](/guides/agents/guardrails) page for input/output guardrails and moderation processors.
**For memory-specific processors**, see the [Memory Processors](/guides/memory/memory-processors) page for processors that handle message history, semantic recall, and working memory.
...
See the [`ToolCallFilter` reference](/reference/processors/tool-call-filter) for configuration options and the [Memory Processors](/guides/memory/memory-processors#manual-control-and-deduplication) page for pre-memory filtering.
...
:::note
Reactive signals default to `tagName: 'system-reminder'`. Visit [Signals](/guides/long-running-agents/signals#send-processor-context) for more on processor-emitted signals.
:::
...
When an agent is registered with Mastra, processor workflows are automatically registered as workflows, allowing you to view and debug them in the [Studio](/guides/studio/overview).
...
- [Guardrails](/guides/agents/guardrails): Security and validation processors
- [Memory Processors](/guides/memory/memory-processors): Memory-specific processors and automatic integration
