...
As a result, subagents have the context they need without cluttering their memory with the entire supervisor conversation. Visit [memory in multi-agent systems](/guides/memory/overview#memory-in-multi-agent-systems) for more details.
...
Subagent invocations are dispatched as tool calls, so they can run as [background tasks](/guides/long-running-agents/background-tasks). This is useful when one or more delegations are long-running and you don't want them to block the supervisor's response.
...
If a subagent isn't listed on the supervisor but has its own background-eligible tools, the supervisor still dispatches the subagent as a background task and inherits its config. See [Inheriting from the subagent](/guides/long-running-agents/background-tasks#inheriting-from-the-subagent) for details.

## Subagent versioning

When using the [editor](/guides/editor/overview), you can control which stored version of each subagent the supervisor uses at runtime. Set version overrides on the Mastra instance or per invocation:
...
Version overrides propagate automatically through delegation. See [Subagent versioning](/guides/editor/overview#sub-agent-versioning) for details on resolution order and server API usage.

## Related

- [Background tasks](/guides/long-running-agents/background-tasks)
- [Subagent versioning](/guides/editor/overview#sub-agent-versioning)
...
- [Memory in multi-agent systems](/guides/memory/overview#memory-in-multi-agent-systems)
