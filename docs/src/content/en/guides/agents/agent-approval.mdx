...
Agent approval uses snapshots to capture request state. Configure a [storage provider](/guides/storage/overview) on your Mastra instance or you'll see a "snapshot not found" error.

Snapshots for agent runs are minimal resume artifacts: they hold only what's needed to resume the suspended run and are deleted once the run finishes. Use [tracing](/guides/observability/overview) for the execution record and [memory](/guides/memory/overview) for the conversation history.
...
Suspended runs only survive restarts when your Mastra instance is configured with a persistent [storage provider](/guides/storage/overview). The default in-memory store loses snapshots when the process exits.

:::

## Tool approval: Supervisor agents

A [supervisor agent](/guides/agents/supervisor-agents) coordinates multiple subagents using `.stream()` or `.generate()`. When a subagent calls a tool that requires approval, the request propagates up through the delegation chain and surfaces at the supervisor level:
