...
Next, add Tailwind to your project. Install the required dependencies:
...
Edit the `tsconfig.json` to include `compilerOptions`:
...
`cd` into your Vite project and run [`mastra init`](/reference/v1/cli/create-mastra).
...
Lastly, add a `mastra:dev` script to your `package.json` to be able to run the Mastra server in development mode:
...
Open your `src/mastra/index.ts` file and add a [`chatRoute()`](/reference/v1/ai-sdk/chat-route) to your configuration. The `chatRoute()` function creates an API route that your React frontend can call to get AI SDK-compatible chat responses. You'll use it with `useChat()` in the next step.
...
Edit the `src/App.tsx` file to create a chat interface:
