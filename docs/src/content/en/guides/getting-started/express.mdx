...
In this guide, you'll build a tool-calling AI agent using Mastra, then connect it to an Express app by using the [Express server adapter](/reference/v1/server/express-adapter).
...
Inside your Express project directory, run [`mastra init`](/reference/v1/cli/create-mastra).
...
You're passing the `app` instance and the main `mastra` instance to `MastraServer`. Callng `init()` automatically registers Mastra middleware and all available endpoints.
