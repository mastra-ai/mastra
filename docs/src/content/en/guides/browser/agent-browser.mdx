...
This adds `browser_record` and `browser_record_caption` to the agent's toolset. See [Browser recording (alpha)](/guides/browser/recording) for details. See [AgentBrowser reference](/reference/browser/agent-browser) for all configuration options and tool details.
