...
For detailed instructions, follow the [observability instructions](/guides/observability/overview). To get up and running quickly, add the `@mastra/observability` package to your project and configure it with [LibSQL](/reference/storage/libsql) and [DuckDB](/reference/vectors/duckdb) for a local development setup that supports both traces and metrics.
...
Visit the [metrics overview](/guides/observability/metrics/overview) for more details.
...
Tracing filters out low-level framework details so your traces stay focused and readable. Visit the [tracing overview](/guides/observability/tracing/overview) for more details.
...
Log forwarding is enabled by default when you configure observability. See [logging](/guides/observability/logging) for level configuration, query examples, and customization details.
