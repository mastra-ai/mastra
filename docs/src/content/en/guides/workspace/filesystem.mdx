...
- [`S3Filesystem`](/ecosystem/workspace-integrations/s3-filesystem): Stores files in Amazon S3 or S3-compatible storage (R2, MinIO, Tigris)
- [`GCSFilesystem`](/ecosystem/workspace-integrations/gcs-filesystem): Stores files in Google Cloud Storage
- [`PlatformFilesystem`](/reference/workspace/platform-filesystem): Stores files in a Mastra Platform workspace bucket
- [`GoogleDriveFilesystem`](/ecosystem/workspace-integrations/google-drive-filesystem): Stores files inside a Google Drive folder
- [`AzureBlobFilesystem`](/ecosystem/workspace-integrations/azure-blob-filesystem): Stores files in Azure Blob Storage
- [`FilesSDKFilesystem`](/ecosystem/workspace-integrations/files-sdk-filesystem): Stores files in any [FilesSDK](https://files-sdk.dev) adapter (S3, R2, GCS, Azure Blob, Vercel Blob, local filesystem, and more): useful when you want one provider that can target multiple backends
- [`AgentFSFilesystem`](/ecosystem/workspace-integrations/agentfs-filesystem): Stores files in a Turso/SQLite database via AgentFS
- [`MesaFilesystem`](/ecosystem/workspace-integrations/mesa-filesystem): Stores files in versioned Mesa repos
- [`ArchilFilesystem`](/ecosystem/workspace-integrations/archil-filesystem): Stores files on Archil elastic, serverless disks
...
For local development, you typically don't need `mounts`, a `LocalFilesystem` and `LocalSandbox` pointed at the same directory gives you both file tools and command execution on the same files. See [configuration patterns](/guides/workspace/overview#configuration-patterns) for more detail.
...
- [GoogleDriveFilesystem reference](/ecosystem/workspace-integrations/google-drive-filesystem)
- [AzureBlobFilesystem reference](/ecosystem/workspace-integrations/azure-blob-filesystem)
- [FilesSDKFilesystem reference](/ecosystem/workspace-integrations/files-sdk-filesystem)
- [AgentFSFilesystem reference](/ecosystem/workspace-integrations/agentfs-filesystem)
- [MesaFilesystem reference](/ecosystem/workspace-integrations/mesa-filesystem)
