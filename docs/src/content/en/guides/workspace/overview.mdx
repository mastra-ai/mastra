...
- **[Filesystem](/guides/workspace/filesystem)**: File storage (read, write, list, delete, copy, move, grep)
- **[Sandbox](/guides/workspace/sandbox)**: Command execution (shell commands) and background processes
- **[LSP inspection](/guides/workspace/lsp)**: Hover, definition, and implementation queries through language servers
- **[Search](/guides/workspace/search)**: BM25, vector, or hybrid search over indexed content
- **[Skills](/guides/workspace/skills)**: Reusable instructions for agents
...
The `skills` array specifies paths to directories containing skill definitions, see [Skills](/guides/workspace/skills).
...
Static providers are owned by the workspace. Resolver-backed providers are owned by your application because the workspace creates them at request time. See [runtime sandbox lifecycle ownership](/guides/workspace/sandbox#lifecycle-ownership) for the resolver cleanup model.
...
Under the hood, `mounts` creates a [CompositeFilesystem](/guides/workspace/filesystem#mounts-and-compositefilesystem) that routes file tool operations to the correct provider based on path prefix. Commands in the sandbox access the mounted paths directly (e.g., `ls /data`).
...
One workspace instance serves all requests. The resolver runs at tool execution time, so each request gets its own filesystem. See [dynamic filesystem](/guides/workspace/filesystem#dynamic-filesystem) for details.
...
The resolver is incompatible with `mounts` and `lsp: true`, since both require a concrete sandbox instance at construction time. See [dynamic sandbox](/guides/workspace/sandbox#dynamic-sandbox) for details.
...
If the owning agent also defines [tool hooks](/guides/agents/using-tools#run-logic-around-tool-calls), workspace hooks run inside the agent hook wrapper. The order is agent `beforeToolCall`, workspace `beforeToolCall`, the tool, workspace `afterToolCall`, and agent `afterToolCall`.
...
See [LSP inspection](/guides/workspace/lsp) for configuration, examples, and tool name remapping.
...
- `Search` (if configured): Indexes files from `autoIndexPaths`, see [Search and Indexing](/guides/workspace/search)
...
- [LSP inspection](/guides/workspace/lsp)
- [Skills](/guides/workspace/skills)
