...
For agents that dispatch [background tasks](/guides/long-running-agents/background-tasks), use [`Agent.streamUntilIdle()`](/reference/streaming/agents/streamUntilIdle) to keep the stream open until those tasks complete and the agent has had a chance to respond to their results.
