...
In Mastra, implement this pattern by combining [agents](/guides/agents/overview), [workflows](/guides/workflows/overview), and [memory](/guides/memory/overview). A workflow can route the task, but the defining feature of the pattern is that ownership moves to the next agent.
...
In Mastra, [workflows](/guides/workflows/overview) can implement coordination patterns such as handoffs and councils. A workflow is distinct because its control logic lives in the workflow itself, regardless of which agents it calls.
...
Mastra doesn't provide a dedicated council primitive. In Mastra, implement this pattern with [agents](/guides/agents/overview) and [workflows](/guides/workflows/overview): Run multiple agents in parallel and collect their outputs. Then add a final synthesis or review step. Workflow control flow methods such as `.parallel()` provide the structure for this pattern.
...
| Workflows | Execution graph | The path is known in advance | Less adaptive when the task changes | [Workflows](/guides/workflows/overview) |
| Supervisor agents | One lead agent | Delegation must adapt during execution | Results depend on good coordination and clear boundaries | [Supervisor agents](/guides/agents/supervisor-agents) |
