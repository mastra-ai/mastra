...
Variables are passed through the agent's `variables` configuration or through [request context](/guides/server/request-context).
...
Prompt blocks can be managed from code through [`mastra.getEditor().prompt`](/reference/editor/mastra-editor#namespaces). This is useful for seeding a set of starter prompts or syncing blocks between environments, or alternatively generating prompt variants from a script.
...
Prompt blocks follow the same [versioning lifecycle](/guides/editor/overview#versioning) as agents. Each prompt block has a draft that you can edit and publish as a versioned snapshot. As a result, prompt content can be versioned and rolled back independently from the agent that uses it.
...
- [Editor overview](/guides/editor/overview): Setup and versioning.
