...
- [**Prompts**](/guides/editor/prompts): Reusable, versioned instruction templates with template variables and display conditions.
- [**Tools**](/guides/editor/tools): Add tools from integration providers, MCP servers, and override tool descriptions at runtime.
...
For building agents entirely in code, see the [Agents overview](/guides/agents/overview).
...
Once registered, you can manage agents through [Studio](/guides/studio/overview) or programmatically through the server API and Client SDK. See the [MastraEditor reference](/reference/editor/mastra-editor) for all configuration options.
...
The `editor.agent` namespace also exposes `getById`, `list`, `listResolved`, and `delete`. The `editor.prompt` namespace exposes the same CRUD methods for prompt blocks. See [Prompts](/guides/editor/prompts#programmatic-control) for examples.
...
Because stored agents are data, you can build automation loops that tune agents without human involvement. A common pattern is to pair the editor API with [datasets](/guides/evals/datasets/overview) and [experiments](/guides/evals/datasets/running-experiments):
...
| Field | Description |
| - | - |
| Instructions | Replace or extend the agent's system prompt using [prompt blocks](/guides/editor/prompts). |
...
When a [supervisor agent](/guides/agents/supervisor-agents) delegates to sub-agents, version overrides determine which stored version of each sub-agent to use instead of the code-defined default. This lets you iterate on sub-agent prompts and tools through the editor without redeploying the supervisor.
...
- Set up [prompts](/guides/editor/prompts) to build reusable instruction templates.
- Add [tools](/guides/editor/tools) from integration providers and MCP servers.
