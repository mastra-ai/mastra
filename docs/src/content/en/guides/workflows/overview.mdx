...
Workflows let you define complex sequences of tasks using clear, structured steps rather than relying on the reasoning of a single agent. They give you full control over how tasks are broken down and how data moves between them, plus what gets executed when. Workflows run using the built-in execution engine by default, or can be deployed to [workflow runners](/guides/deployment/workflow-runners) like Inngest for managed infrastructure.
...
Workflow steps can also call registered agents or import and execute tools directly, visit the [Using Tools](/guides/agents/using-tools) page for more information.
...
Workflows can be composed using a number of different methods. The method you choose determines how each step's schema should be structured. Visit the [Control Flow](/guides/workflows/control-flow) page for more information.

## Studio

Open [Studio](/guides/studio/overview) and select a workflow from the **Workflows** tab.
...
- [**Time travel**](/guides/workflows/time-travel): After a run completes, replay individual steps to inspect or retry them.
...
Visit [Workflow State](/guides/workflows/workflow-state) for complete documentation on state schemas, initial state, persistence across suspend/resume, and nested workflows.

## Workflows as steps

Use a workflow as a step to reuse its logic within a larger composition. Input and output follow the same schema rules described in [Core principles](/guides/workflows/control-flow).
...
Use [RequestContext](/guides/server/request-context) to access request-specific values. This lets you conditionally adjust behavior based on the context of the request.
...
For type-safe request context schema validation, see [Schema Validation](/guides/server/request-context#schema-validation).
...
- [Workers](/guides/deployment/workers): Offload workflow orchestration to dedicated processes in production
