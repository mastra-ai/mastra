...
When an `@mastra/mcp` 2.x server executes a tool, `context.mcpv2` is an `MCPRequestContextV2` with the per-request facilities: `protocolVersion: '2026-07-28'`, `requestId`, `signal`, the request's `_meta`, and request-scoped `log(level, message, data?)` and `progress({ progress, total?, message? })`. `log` and `progress` keep the `@mastra/mcp` 1.x signatures, so a tool that only uses them works unchanged on either version. The legacy `context.mcp` shape is never present on a 2.x server. The `v2` suffix is temporary: the next major release of `@mastra/core` renames it to `context.mcp`.
