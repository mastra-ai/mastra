...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Configure which [workers](/docs/deployment/workers) run in this Mastra instance. Set to `false` to disable all event processing. Pass an array of `MastraWorker` instances to use specific workers. When omitted, Mastra auto-creates workers based on PubSub and config.',
    isOptional: true,
  },
  {
