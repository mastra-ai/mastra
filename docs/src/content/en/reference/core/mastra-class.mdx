...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Global version overrides for sub-agent delegation. When a supervisor agent delegates to a sub-agent, these overrides determine which stored version of that sub-agent to use instead of the code-defined default. Requires the editor package to be configured. See [Sub-agent versioning](/guides/editor/overview#sub-agent-versioning) for details.',
    isOptional: true,
    properties: [
      {
        type: 'VersionOverrides',
  ...
  },
  {
    ...
    description:
      'Configure [background task](/guides/long-running-agents/background-tasks) execution for agents. See [background tasks configuration reference](/reference/configuration#backgroundtasks) for all options.',
    isOptional: true,
    properties: [
      {
        type: 'BackgroundTaskManagerConfig',
  ...
  },
  {
    ...
    type: 'object',
    description:
      'Configure the [scheduler worker](/guides/deployment/workers#scheduler-worker) for cron-driven workflow triggers. Auto-enables when any workflow declares a `schedule`. See [Scheduled workflows](/guides/workflows/scheduled-workflows).',
    isOptional: true,
    properties: [
      {
        type: 'object',
  ...
  },
  {
    ...
    description:
      'Boot-time recovery behavior for orphaned agent and workflow runs. See [Crash recovery](/guides/long-running-agents/durable-agents#crash-recovery).',
    isOptional: true,
        ...
        parameters: [
          {
            ...
            defaultValue: "'off'",
            description:
              "Set to `'auto'` to automatically re-drive orphaned RUNNING durable agent runs on server boot. Recovery re-issues LLM calls and re-executes tool calls, so tools must be idempotent. See [Crash recovery](/guides/long-running-agents/durable-agents#crash-recovery).",
          },
        ],
      },
    ],
