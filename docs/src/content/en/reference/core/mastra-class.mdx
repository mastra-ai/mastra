        ...
        parameters: [
          {
            ...
            isOptional: true,
            defaultValue: "'off'",
            description: "Set to `'auto'` to automatically re-drive orphaned RUNNING durable agent runs on server boot. Recovery re-issues LLM calls and re-executes tool calls, so tools must be idempotent. See [Crash recovery](/docs/long-running-agents/durable-agents#crash-recovery).",
          },
        ],
      },
    ],
