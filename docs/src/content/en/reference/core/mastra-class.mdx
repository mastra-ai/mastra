...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Global version overrides for sub-agent delegation. When a supervisor agent delegates to a sub-agent, these overrides determine which stored version of that sub-agent to use instead of the code-defined default. Requires the editor package to be configured. See [Sub-agent versioning](/reference/editor/versioning#sub-agent-versioning) for details.',
    isOptional: true,
    properties: [
      {
        type: 'VersionOverrides',
