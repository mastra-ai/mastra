...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'object',
    description:
      'Configure the [scheduler worker](/ecosystem/deployment-patterns/workers#scheduler-worker) for cron-driven workflow triggers. Auto-enables when any workflow declares a `schedule`. See [Scheduled workflows](/docs/workflows/scheduled-workflows).',
    isOptional: true,
    properties: [
      {
        type: 'object',
