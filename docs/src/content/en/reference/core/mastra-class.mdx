...
The `Mastra` class is the central orchestrator of any Mastra application. It manages agents, tools, workflows, storage, logging, telemetry, and more. Most applications create a single `Mastra` instance to coordinate everything.

Think of `Mastra` as a top-level registry:

- When you register **tools**, they become available to all agents and workflows
- When you register **integrations**, they can be accessed by agents, workflows, and tools alike

This centralized design simplifies configuration and promotes reuse across your application.
...
```typescript {6} filename="server.ts" showLineNumbers
...
```typescript {12-13} filename="server.ts" showLineNumbers
import { mastra } from "./mastra";
