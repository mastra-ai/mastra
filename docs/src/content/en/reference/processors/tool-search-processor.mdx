    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            description:
              'Fetch searchable tool IDs and descriptions when discovery or restoration needs them. Each entry resolves its full Tool only when selected. An unloaded text response does not call this function unless injectCatalog is enabled.',
            isOptional: true,
          },
          {
            name: 'deferredFilter',
...
The metadata catalog is read on search, explicit load, or restoration of a previously loaded deferred tool. Setting `injectCatalog: true` also reads metadata during preparation so the model can see the catalog, without resolving full schemas. The catalog is scoped to a model step. Resolving one entry must return its complete tool with the same ID, input schema, approval requirement, and executor. Mastra then uses its existing validation, approval, and execution path.

Use `deferredFilter` for metadata visibility. `filter` keeps its existing full-tool contract and runs after a deferred entry resolves. Search without automatic loading does not resolve schemas; automatic loading resolves only allowed matches.
...
Use a stable `binding` string when one tool name can address different execution targets. `deferStoredToolProviders` supplies it from the original provider and account pin. A deferred executor cannot cross unrelated request contexts. Mastra preserves identity through its own merged and transformed execution views; restored runs rebuild their own tools.
