    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'boolean',
            description:
              'When true, tools passed per-request via processInputStep (for example MCP tools in args.tools) are indexed for search_tools and load_tool and are not injected into the prompt until loaded. When false (default), args.tools remain immediately available but are not searchable via the meta-tools.',
            isOptional: true,
            default: 'false',
          },
        ],
...
If the hook throws or rejects, `ToolSearchProcessor` treats the tool as disallowed for that request. The hook may run for every matching search candidate, so keep async policy checks cheap or cached. The `search_tools` meta-tool is always available; `load_tool` is available unless `search.autoLoad` is enabled. By default, tools passed directly through the agent or `processInputStep` remain available unless you filter them outside `ToolSearchProcessor`. Set `includeResolvedTools: true` to index per-request tools (such as MCP) for search/load without injecting them until loaded.

## Per-request and MCP tools

When MCP or other dynamically resolved tools are passed in `args.tools`, enable `includeResolvedTools` so they follow the same search/load pattern as constructor-time tools:
...
With this option, per-request tools are searchable via `search_tools` and become available only after `load_tool` (or `search.autoLoad`) — they are not sent in the prompt on every turn.
