...
<PropertiesTable
  content={[
  {
    ...
    type: 'MastraCache',
    description:
      'The cache backend. Required. Implement `MastraCache` directly or wire an existing `MastraServerCache` (e.g. `RedisCache` from `@mastra/redis`) using `createMastraCacheFromServerCache`.',
    isOptional: false,
  },
  {
    name: 'ttl',
