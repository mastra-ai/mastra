...
<PropertiesTable
  ...
  {
    name: 'model',
    type: "ClassifierOptions['model']",
    required: false,
    description:
      'The evaluation model used to make the routing decision, with the same type as the Classifier model option. Required with choices, and unrelated to the models being routed to.',
  },
  {
    name: 'instructions',
    type: 'string',
  ...
  {
    name: 'select',
    type: '(answers) => SelectableModel | undefined',
    required: false,
    description:
      'Maps the classifier answers to a model. Receives the full typed answers and returns a model, or undefined to abstain. Required with classifier.',
  },
  {
    name: 'minProbability',
    type: 'number',
...
`onDecision` fires once per request with the decision, including when the router abstains. Use it to log which model served a request and how confident the choice was:
...
- **Set**: require a probability at or above the threshold. If the model returns no distribution at all, the router **abstains**, because a threshold that can't be evaluated must not pass.

If your evaluation model doesn't report choice distributions, the router abstains on every request once `minProbability` is set. Log `onDecision` to check: an `abstained` value of `below-threshold` with no `probability` means the distribution is missing rather than the confidence being low.
...
Before enabling routing, run a sample of your traffic with and without the router and compare the total cost per completed task, not the price per token. Include cache reads and writes, classifier calls, extra steps, and follow-up requests, and check the answers on requests the router downgraded. The [token usage metrics](/reference/observability/metrics/automatic-metrics#token-usage-metrics) report cache read and write tokens for each model call.
...
- Only downgrade. Keep the capable model as the agent's configured default and use the router to move down, so an incorrect decision costs quality on that request. With the default `scope: 'run'`, a bad downgrade applies to every step of the run, so a run that turns out harder than its first message stays on the cheaper model.
