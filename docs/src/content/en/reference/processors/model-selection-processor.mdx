...
The decision carries the model that was applied, the criterion that was chosen, the confidence in that choice, and, when no model was applied, why. With `select`, the decision reports only `model`, which is absent when `select` returns `undefined`:
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    required: false,
    description: 'The choice the classifier selected. Absent when the classifier failed and with select.',
  },
  {
    name: 'probability',
    type: 'number',
  ...
  },
  {
    ...
    required: false,
    description: 'Why no model was applied. Absent on a successful routing decision and with select.',
  },
]}
/>
