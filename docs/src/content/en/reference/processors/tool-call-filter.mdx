...
Filtering happens in the `processLLMRequest` hook, which runs after the message list is converted to a model prompt. Changes are transient: they affect only what is sent to the model on that call. Stored messages, memory, and UI history keep their original tool calls and results.
