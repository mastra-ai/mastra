...
description: "The PrefillErrorHandler is an error processor that handles assistant-response prefill errors."
packages:
  - "@mastra/core"
---
...
Mastra adds this processor to every agent's `errorProcessors` by default, after `ProviderHistoryCompat` and before `StreamErrorRetryProcessor`. It only acts on the first retry attempt, so it needs no configuration. Supply your own instance to tune it — anything already in your list is kept and the default is only added when its `id` is absent.
...
To tune or explicitly scope prefill recovery, supply your own instance — the default `PrefillErrorHandler` is not added when you do:
