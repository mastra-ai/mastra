...
Mastra adds this processor to every agent's `errorProcessors` by default, after `StreamErrorRetryProcessor`. It only acts on the first retry attempt, so it needs no configuration. Supply your own instance to tune it — anything already in your list is kept and the default is only added when its `id` is absent.
