...
Mastra adds this processor to every agent's `errorProcessors` by default, last — after the `ProviderHistoryCompat` and `PrefillErrorHandler` defaults, so the processors that repair a request get their chance before this one retries it unchanged. The default instance uses `maxRetries: 2` with a bad-request matcher and a connection-reset matcher. Supply your own instance in `errorProcessors` to change its policy — because a default is only added when its `id` is missing from your list, your instance is used and the default is not added alongside it.

## Usage example

Configure `StreamErrorRetryProcessor` — supplying the same `id` means the default is not added:
...
To run with no error processors at all, pass `errorProcessors: []` — an explicitly empty list means the agent adds none of the defaults. Supplying your own processor for all three default ids has the same effect while keeping your own implementations.
