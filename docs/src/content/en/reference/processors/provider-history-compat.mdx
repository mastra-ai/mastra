...
Add `ProviderHistoryCompat` to `errorProcessors` when you want all built-in compatibility rules available for an agent:
...
- **Reactive rules need `errorProcessors`.** `processAPIError` runs from any of the three lists, so an input-processor registration does repair the message list — but the retry it asks for is only honored when the agent has a retry budget, and that budget stays at zero unless the agent configures `errorProcessors` or sets `maxProcessorRetries` explicitly. Registered only as an input processor, a reactive rule repairs the history and the request still fails.
- **Preemptive rules need `inputProcessors`.** `processLLMRequest` runs over the input processors only, so prompt rewrites do not happen from the error list.

An agent that wants both registers the processor in both lists. The rules are keyed to the failure they repair, so the error hook re-running over history the input pass already rewrote asks for no further retry.
