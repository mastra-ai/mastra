...
Mastra adds this processor to every agent's `errorProcessors` by default when your list does not already contain that `id`. Among the added defaults it comes first, so its outbound prompt rules run before the retry processors. Supply your own instance to add custom rules or position it yourself:
