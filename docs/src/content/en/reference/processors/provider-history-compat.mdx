...
Because nothing is persisted, a caller that reuses the same message list in a later turn sees the original IDs again, and the repair runs again on that request. If you need the sanitized IDs written back to storage, supply a custom rule that implements `fix` instead.

### Deprecated reactive recovery

The rule's `errorPatterns` and `fix` hooks are deprecated. They remain as a dormant fallback for placements where the prompt lane can't see the request (for example a `CompatRule` applied outside the agent's prompt path), and they're the only repair available once a provider has already rejected the call. They'll be removed in a future major release.

`fix` on the `CompatRule` interface itself isn't deprecated. Custom rules legitimately use the reactive hook, and the two hooks can be combined on one rule.
