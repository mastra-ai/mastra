...
| `anthropic-strip-foreign-signed-reasoning` | Anthropic-compatible | Reactive API error recovery | Strips the signature / redacted payload off the persisted turn that provoked a replayed-signature rejection (``Invalid `signature` in `thinking` block``), so the retry succeeds. Only the offending turn is touched; other turns' signatures are kept. Does not recover the trailing `tool_use`-first continuation shape. |
