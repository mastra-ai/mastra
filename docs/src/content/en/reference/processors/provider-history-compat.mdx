...
An agent that wants both registers the processor in both lists, as the example above does. Double registration cannot cause a double retry: the runner stops at the first processor that asks for one, and each reactive rule bails once `retryCount` is above zero, so a repaired turn is attempted exactly once more.
