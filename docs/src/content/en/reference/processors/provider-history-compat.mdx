...
| `anthropic-strip-foreign-signed-reasoning` | Anthropic-compatible | Preemptive prompt rewrite + reactive API error recovery | Drops signed thinking from the outbound prompt when its origin turn was stamped with a different provider, and if a provider still rejects a replayed signature (``Invalid `signature` in `thinking` block``), strips the signature / redacted payload off the persisted turn that provoked the rejection so the retry succeeds. Only the offending turn is touched reactively and other turns' signatures are kept. Turns emptied of all content by the drop are removed from the prompt. The trailing `tool_use`-first continuation shape is declined rather than repaired, since a strip can't put a thinking block back. |
