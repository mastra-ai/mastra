...
When you attach `SkillSearchProcessor` to an agent with a skill-enabled `Workspace`, the agent treats skills as on-demand:

- It does not auto-add the default eager `SkillsProcessor`.
- It exposes `search_skills` and `load_skill` for discovery and instruction loading.
- It does not expose the overlapping `skill` and `skill_search` tools.
- It still exposes `skill_read` so the agent can read supporting skill files such as references, scripts, and assets.
