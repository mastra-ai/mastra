...
The processor methods run at different points in the agent execution lifecycle:
...
<PropertiesTable
  ...
  },
  {
    name: 'messageList',
    type: 'MessageList',
    description: 'MessageList instance for managing messages. Call `updateToolInvocation` to replace the tool result with a redacted or transformed value.',
    isOptional: false,
  },
  {
    name: 'stepNumber',
  ...
  },
  {
    name: 'result',
    type: 'unknown',
    description: 'Value returned by the tool. For client-executed tools this is the output of `tool.execute()` after it has passed through `ensureSerializable`. For provider-executed tools (e.g. Anthropic `web_search`) it is the raw result from the provider stream, which is not run through `ensureSerializable`.',
    isOptional: false,
  },
  {
    name: 'providerExecuted',
    type: 'boolean',
    description: 'Whether this result came from a provider-executed tool such as Anthropic web_search. Defaults to undefined for client-executed tools.',
    isOptional: true,
  },
  {
    name: 'systemMessages',
  ...
  },
  {
    name: 'abort',
    type: '(reason?: string, options?: { retry?: boolean; metadata?: unknown }) => never',
    description: 'Function to abort the run. Pass `retry: true` to request the LLM retry the step with the abort reason as feedback.',
    isOptional: false,
  },
  {
    name: 'retryCount',
