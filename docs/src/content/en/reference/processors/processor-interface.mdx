...
): Promise<LanguageModelV2Prompt | undefined | void> | LanguageModelV2Prompt | undefined | void;
```
...
`processLLMRequest` returns `LanguageModelV2Prompt | undefined | void`.

- Return a `LanguageModelV2Prompt` to replace the outbound prompt for the current provider call.
