...
| `processAPIError` | When an LLM API call fails with a non-retryable error | Handle API rejections, modify messages and retry |
...
<PropertiesTable
  ...
  },
  {
    name: 'feedback',
    type: 'string',
    description: 'Optional feedback message to include as system context on retry.',
    isOptional: true,
  },
]}
/>
  ...
  processAPIError({
    ...
    if (APICallError.isInstance(error) && error.message.includes('context length exceeded')) {
      ...
      if (messages.length > 4) {
        ...
        return { retry: true, feedback: 'Trimmed message history to fit context window.' }
      }
    }
  }
}
