...
Several conditions can share one `onResult`, or use separate processors when they need different classifiers:
...
When using a registered classifier by string, answer types are not inferred from the instance. Pass the question map as a type argument to narrow them: `new ClassifierProcessor<typeof safety.questions>({ ... })`.
