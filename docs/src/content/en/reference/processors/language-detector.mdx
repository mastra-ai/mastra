    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            description:
              'Deprecated and has no effect. The value is accepted but never reaches the detection agent, so it changes neither the prompt nor any request setting. Use `providerOptions` to trade speed for accuracy.',
            isOptional: true,
          },
          {
