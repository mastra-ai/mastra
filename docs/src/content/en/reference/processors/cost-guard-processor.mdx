...
- **Run scope** (default): Tracks cost within a single agent run via trace ID
- **Resource scope**: Tracks cumulative cost per `resourceId` across runs
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "Scope for cost tracking. 'run' tracks cost within the current agent run via trace ID. 'resource' tracks cumulative cost per resourceId across runs. 'thread' tracks cumulative cost per threadId across runs. All scopes require observability storage with getMetricAggregate support.",
    isOptional: true,
    defaultValue: "'run'",
  },
  {
