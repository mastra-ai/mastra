...
export const agent = new Agent({
  ...
  outputProcessors: [
    ...
    new ModerationProcessor({
      ...
      threshold: 0.7,
      strategy: "filter",
      instructions: "Detect and filter inappropriate content in responses",
      includeScores: true,
      chunkWindow: 1
    })
  ]
...
> **Performance tip:** Without `BatchPartsProcessor`, the moderation processor would make an LLM call for every streaming chunk (potentially hundreds of calls per response). By batching chunks first, you significantly reduce API calls and improve response times. The `chunkWindow` parameter can still be used to provide context from previous batches.
