    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            description:
              "Controls how older messages are trimmed when exceeding the token limit: 'best-fit' walks from newest to oldest and skips messages that don't fit (may create gaps), 'contiguous' stops at the first message that doesn't fit, ensuring a continuous suffix of conversation history, 'memory-only' only trims messages loaded from memory",
            isOptional: true,
            default: "'best-fit'",
          },
          {
...
During `processInputStep`, messages produced by the current run (tool calls, tool results, and partial answers) are never trimmed. Removing them mid-run would hide tool data from the next step, so the model would call the same tool again until `maxSteps` runs out. Only older history (memory and earlier input) is trimmed to make room.
...
- **Current run exceeds limit**: If the current run's messages alone exceed the remaining budget, `processInputStep` throws a TripWire instead of dropping tool data. Set `maxToolResultTokens` or raise `limit`.
