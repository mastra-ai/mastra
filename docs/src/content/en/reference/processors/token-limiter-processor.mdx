    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'number',
            description:
              "Caps each tool result produced in the current run to this many tokens before it reaches the model. The stored result is kept intact; the model receives a truncated copy ending in a `[truncated: showing N of M tokens]` marker. Results already mapped by a tool's `toModelOutput` are left alone. Unset by default.",
            isOptional: true,
          },
        ],
      },
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "In 'memory-only' trim mode, applies stored-history trimming at each step, preserving system messages and every message produced by the current run (tool calls, tool results, partial answers). In the 'best-fit' and 'contiguous' trim modes, it leaves trimming to processLLMRequest when the agent runs that method for this processor. Otherwise it trims stored history, for example in generateLegacy() and streamLegacy() or when the limiter is inside a processor workflow.",
    isOptional: true,
  },
  {
    ...
    description:
      "Enforces the input budget on the provider prompt in the 'best-fit' and 'contiguous' trim modes. Runs after earlier prompt processors have transformed the prompt, counts that exact prompt, and returns a trimmed copy for the model call only. System messages and every message produced by the current run (the triggering prompt, tool calls, tool results, partial answers) are always preserved, and tool call and tool result messages are grouped so they are kept or removed together. Set `maxToolResultTokens` to cap oversized current-run tool results instead.",
    isOptional: true,
  },
  {
...
Messages produced by the current run (the triggering prompt, tool calls, tool results, and partial answers) are never trimmed, whether trimming happens on the provider prompt in `processLLMRequest` (`best-fit`, `contiguous`) or on stored history in `processInputStep` (`memory-only`). Removing them mid-run would hide tool data from the next step, so the model would call the same tool again until `maxSteps` runs out. Only older history is trimmed to make room.

If a single tool result is large, set `maxToolResultTokens` so the model sees a truncated copy instead of the full result:
...
- **Current run exceeds limit**: If the current run's messages alone exceed the remaining budget, a TripWire is thrown instead of dropping tool data or the triggering prompt. Set `maxToolResultTokens` or raise `limit`.
