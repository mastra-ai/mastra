...
- **Current run exceeds limit**: Messages produced by the current run, such as tool calls and their results, are always kept so the agent loop can continue. If they alone exceed the remaining budget, a TripWire is thrown instead of removing them.
