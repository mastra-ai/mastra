...
- **No messages fit**: If no message fits within the remaining budget, a TripWire is thrown rather than sending a request with system messages only. This case cannot happen while the current run holds tool calls or results, because those are always kept (see below).

Tool calls and their results produced by the current run are never trimmed, so the agent loop keeps the tool traffic it needs for the next step and that traffic is still saved to memory. They are counted against the budget first, and older messages are trimmed to make room. If they exceed the budget on their own they are still kept, and every older message is trimmed.
