    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'number',
            description:
              "Caps every tool result to this many tokens via the `processToolResult` output-processor hook, before the engine commits the final tool result. The stored result is kept intact; the model receives a truncated copy ending in a `[truncated: showing N of M tokens]` marker, or an empty string if the cap is too small to fit even the bare marker. A tool's own `toModelOutput` mapping is computed later and always wins over this cap. Requires the processor to be registered in `outputProcessors`. Unset by default.",
            isOptional: true,
          },
        ],
      },
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "Only present on the processor instance when `maxToolResultTokens` is set (an agentic-loop optimization checks for the method's presence). When present, and the processor is registered in `outputProcessors`, it caps each tool result to that many tokens right after the tool runs, before the engine commits the final tool result. The full result stays in storage; the model-facing copy is capped via the tool-invocation part's `providerMetadata`, the same mechanism a tool's `toModelOutput` uses. A tool's own `toModelOutput` mapping is computed later and always wins over this cap. This cap bounds only the tool-result payload, not the whole prompt group it belongs to — set it below `processLLMRequest`'s token budget so a capped result plus the rest of its group still fits. This cap only applies to the run where the tool executes — memory persistence strips the truncated `providerMetadata` before saving, and `processToolResult` does not re-run against reloaded history, so later turns see the full, uncapped stored result again.",
    isOptional: true,
  },
  {
