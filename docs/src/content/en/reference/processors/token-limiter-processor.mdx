...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "In 'memory-only' trim mode, applies stored-history trimming at each step. In standard 'best-fit' and 'contiguous' modes, trims stored history when called from legacy or workflow-nested paths that never invoke processLLMRequest, and defers to processLLMRequest on the agentic-loop prompt-stage path where earlier prompt processors may have already transformed the prompt.",
    isOptional: true,
  },
  {
...
> **Note on processor workflow ordering:** If a `TokenLimiterProcessor` is nested inside a processor workflow (wrapped with `createStep`), it runs its trimming through `processInputStep`, which operates on stored messages before top-level prompt processors such as `ToolCallFilter` convert them. For post-filter counting, where earlier prompt processors like `ToolCallFilter` have already removed tool results from the prompt, register the limiter directly in `inputProcessors`, not wrapped in a `createStep`.
