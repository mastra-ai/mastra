...
Supports built-in presets for common patterns (PII, secrets, URLs, prompt injection) and custom regex rules. Can be applied to input, output, or both phases.
...
<PropertiesTable
  ...
  {
    name: 'presets',
    type: "('pii' | 'secrets' | 'urls' | 'injection')[]",
    description:
      "Built-in preset categories. 'pii' matches emails, phone numbers, SSNs, credit cards. 'secrets' matches API keys, bearer tokens, AWS keys. 'urls' matches HTTP/HTTPS URLs. 'injection' matches common prompt injection phrases.",
    isOptional: true,
  },
  {
    name: 'strategy',
...
- `matches`: Array of match objects with `rule`, `match`, and `index`
- `strategy`: `'block'`
...
| `injection` | Instruction override, system prompt override, role hijack | `[BLOCKED]` |
