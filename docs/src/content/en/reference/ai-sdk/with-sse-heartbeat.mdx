...
- A `RangeError` is thrown when heartbeats are enabled with a value that cannot be scheduled with a timer, meaning a non-finite number or a value greater than `2147483647`. Use `assertValidHeartbeatMs()` to apply the same check to user-supplied configuration before you start streaming.
