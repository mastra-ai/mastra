...
Agent networks are deprecated and will be removed in a future release. Use [supervisor agents](/docs/agents/supervisor-agents) with `agent.stream()` or `agent.generate()` instead. See the [migration guide](/docs/migrations/network-to-supervisor) to upgrade.
