...
If you want the server to continue generation and persist the final response after disconnect, build a [custom API route](/guides/server/custom-api-routes#continue-generation-after-client-disconnect) around `agent.stream()` and call `consumeStream()` on the returned `MastraModelOutput`.
