...
When converting resumed agent streams, `toAISdkStream()` continues from the resumed point in the run. This includes nested supervisor and router sub-agent streams that resume with tool result events instead of a fresh `start` chunk.
