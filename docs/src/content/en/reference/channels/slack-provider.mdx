...
`SlackProviderConfig` combines Slack-specific fields, Slack-adapter overrides (`toolDisplay`, `streaming`, `typingStatus`), and a curated subset of [`AgentChannels`](/docs/agents/channels) options forwarded to every connected agent. All fields are optional.
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Slack App Configuration access token for programmatic app creation. Optional — the provider rotates to a fresh token on startup using `refreshToken`.',
  },
  {
    name: 'baseUrl',
    type: 'string',
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      "Slash commands this agent supports. A string (for example, `'/ask'`) triggers the agent with the input text; an object form adds a custom prompt template with `{{text}}` as the input placeholder.",
  },
  {
    name: 'manifest',
...
Recreates a `SlackAdapter` for each active installation in storage and injects `AgentChannels` into the corresponding agent so it receives Slack events on startup. Does not auto-provision new apps — use `connect()` to create one. Mastra calls this automatically; you rarely call it directly.
...
- [Channels](/docs/agents/channels) — concepts, platform setup, and the `createSlackAdapter` path
