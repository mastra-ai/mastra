...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      "Whether the channel exposes channel tools (`add_reaction`, `remove_reaction`) via the deprecated `AgentChannels.getTools()`. These tools are no longer auto-injected into the agent's toolset — pass chat tools to your agent explicitly to use them.",
  },
  {
    name: 'state',
