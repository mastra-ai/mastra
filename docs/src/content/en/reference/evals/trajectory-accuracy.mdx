...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    description:
      'Overrides the scorer id. Results are keyed by id, so give each instance a unique id when using the scorer more than once in a single runEvals call.',
    isOptional: true,
    defaultValue: "'code-trajectory-accuracy-scorer'",
  },
  {
    name: 'name',
    type: 'string',
    description: 'Overrides the scorer display name.',
    isOptional: true,
    defaultValue: "'Trajectory Accuracy Scorer'",
  },
  {
    ...
    properties: [
      {
        ...
        description:
          "How step ordering is evaluated. 'strict' requires an exact match (same steps in the same order, no extra steps). 'relaxed' allows additional steps as long as expected steps appear in order. 'unordered' only checks that expected steps are present.",
        isOptional: true,
...
Only checks that each expected step is present. Order is ignored, and extra steps are still penalized as in relaxed mode.
...
### Multiple trajectory gates on one workflow

To assert several independent facts about a run, create one scorer per expectation and give each a unique `id`. Without an override every instance shares the default id, so `runEvals` merges them into a single averaged row and a failing gate can't be attributed to one expectation.

```typescript title="src/example-trajectory-gates.ts"
import { createTrajectoryAccuracyScorerCode } from '@mastra/evals/scorers/prebuilt'
import { runEvals } from '@mastra/core/evals'
...
const result = await runEvals({
  ...
  gates: [
    ...
    createTrajectoryAccuracyScorerCode({
      id: 'plan-activities-ran',
      expectedTrajectory: [
        { stepType: 'workflow_step', name: 'plan-activities', status: 'success' },
      ],
    }),
    createTrajectoryAccuracyScorerCode({
      id: 'plan-activities-called-weather-tool',
      expectedTrajectory: [
        {
...
console.log(result.gateResults) // [{ id: 'fetch-weather-ran', passed: true, score: 1 }, ...]
```
...
<PropertiesTable
  content={[
  {
    ...
    type: 'string',
    description:
      'Overrides the scorer id. Give each instance a unique id when using the scorer more than once in a single runEvals call.',
    isOptional: true,
    defaultValue: "'code-trajectory-scorer'",
  },
  {
    name: 'name',
    type: 'string',
    description: 'Overrides the scorer display name.',
    isOptional: true,
    defaultValue: "'Trajectory Scorer'",
  },
  {
    name: 'defaults',
    type: 'TrajectoryExpectation',
