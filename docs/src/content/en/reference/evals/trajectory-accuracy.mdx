...
To assert several independent facts about a run, create one scorer per expectation and give each a unique `id`. Without an override every instance shares the default id, so `runEvals` combines them into a single result and a failing gate can't be attributed to one expectation.
