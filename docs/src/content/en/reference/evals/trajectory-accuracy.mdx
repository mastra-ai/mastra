...
Full credit requires an exact match: the same steps in the same order with no extra or missing steps. Trajectories that don't match exactly earn partial credit based on how many expected steps matched in position, minus a penalty for extra steps. For example, the expected two steps followed by one extra step scores `0.75`.
