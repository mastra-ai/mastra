...
This scorer reads a single assistant message, so with [multi-turn `inputs`](/docs/evals/multi-turn) it grades only the last turn's response, and input-relative grading compares against the first turn's input. To grade a conversation, attach the scorer to a single turn with `turns[].scorers`, or write a [custom conversation scorer](/docs/evals/multi-turn#grading-a-whole-conversation).
