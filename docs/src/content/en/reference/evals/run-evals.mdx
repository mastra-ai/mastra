...
<PropertiesTable
  ...
  },
  {
    name: 'threshold',
    type: 'number',
    description: 'Score at or above this value passes. Must be between 0 and 1. If the average score across data items is below this threshold, the verdict includes it as a miss.',
  },
]}
/>
...
const result = await runEvals({
  ...
  scorers: [
    { scorer: faithfulnessScorer, threshold: 0.7 },
    { scorer: checks.similarity('Sunny, 72F'), threshold: 0.6 },
    checks.includes('Brooklyn'), // bare scorer, no threshold
  ],
})

if (result.verdict === 'failed') {
  console.log('Gate failures:', result.gateResults?.filter(g => !g.passed))
} else if (result.verdict === 'scored') {
  console.log('Threshold misses:', result.thresholdResults?.filter(t => !t.passed))
}
```
