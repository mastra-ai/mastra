...
<PropertiesTable
  content={[
    ...
    {
      name: "scorers",
      type: "(MastraScorer | MastraScorerEntry)[] | WorkflowScorerConfig",
      description:
        "Array of scorers (or scorer entries with config) for agents, or configuration object for workflows specifying scorers for the workflow and individual steps. Use MastraScorerEntry to specify modelSettings or run scorers multiple times with different temperatures.",
      isOptional: false,
    },
    {
      name: "concurrency",
...
## Scorer Configuration (MastraScorerEntry)

For LLM-based scorers, you can configure model settings and run scorers multiple times with different temperatures using `MastraScorerEntry`:
  ...
  content={[
    {
      name: "scorer",
      type: "MastraScorer",
      description: "The scorer instance to run.",
      isOptional: false,
    },
    {
      name: "modelSettings",
      type: "Omit<CallSettings, 'abortSignal'>",
      description: "Model settings applied to all LLM judge calls (e.g., temperature, maxTokens, topP). Note: If temperatures array is also provided, it will override modelSettings.temperature.",
      isOptional: true,
    },
    {
      name: "temperatures",
      type: "number[]",
      description: "Run the scorer multiple times with different temperature values. Each temperature creates a separate score entry keyed as 'scorerId@temperature'. Takes precedence over modelSettings.temperature when both are specified.",
      isOptional: true,
    },
  ]}
/>

### Temperature Array Example

Run an LLM-based scorer multiple times with different temperatures to get varied judge opinions:

```typescript
import { runEvals } from "@mastra/core/evals";
import { createAnswerRelevancyScorer } from "@mastra/evals/scorers/prebuilt";

const relevancyScorer = createAnswerRelevancyScorer({ model: "openai/gpt-4o" });
...
// Results are keyed with temperature suffix
console.log(result.scores["answer-relevancy@0.3"]); // Score at temp 0.3
console.log(result.scores["answer-relevancy@0.5"]); // Score at temp 0.5
console.log(result.scores["answer-relevancy@0.7"]); // Score at temp 0.7
```
