...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string[]',
    description:
      'Multi-turn inputs. Each entry is sent sequentially to the agent on the same thread. Scorers see the accumulated output from all turns. Only supported for Agent targets.',
    isOptional: true,
  },
  {
    name: 'groundTruth',
...
Each turn runs `agent.generate()` with the same `threadId`, so the agent sees the full conversation history. Mix single-turn (`input`) and multi-turn (`inputs`) items in the same `data` array.
