...
To grade a conversation semantically, attach the judge to a single turn with `turns[].scorers`, where each turn grades its own input and output. The alternative is a scorer that reads every assistant message: see [Grading a whole conversation](/docs/evals/multi-turn#grading-a-whole-conversation).
