...
Scoring uses the accumulated output from all turns as `run.output`, but only the first turn as `run.input`. Prefer output-based scorers (`checks.includes`, `checks.calledTool`, `checks.similarity`) for multi-turn. Input-relative scorers (e.g. faithfulness) only see the first turn's input. Trajectory scorers that read from the trace (`AgentScorerConfig.trajectory`) resolve against the last turn's span. Tool-call checks that read `run.output` (like `checks.calledTool`) still see every turn. See [Multi-turn scorer compatibility](#multi-turn-scorer-compatibility) for which scorers read the whole conversation.
...
## Multi-turn scorer compatibility

With `inputs`, `run.output` holds the accumulated messages from every turn, but scorers differ in how much of that they read.

| Scorer | What it sees with `inputs` |
| - | - |
| [Quick Checks](/reference/evals/checks) (`checks.calledTool`, `checks.includes`, `checks.similarity`, `checks.noToolErrors`, and the rest) | The whole accumulated conversation. Tool-call checks count calls across every turn |
...
| Prebuilt LLM-judge scorers (rubric, answer relevancy, answer similarity, faithfulness, hallucination, bias, toxicity, context precision, context recall, context relevance, noise sensitivity, prompt alignment, summarization) | Only the last assistant message that carries text, because they extract output through `getAssistantMessageFromRunOutput()`. Judges that compare input against output also see only the first turn's input |
| Trajectory scorers (`AgentScorerConfig.trajectory`) | The last turn's span when traces are available, otherwise the tool calls extracted from all accumulated messages |
| Custom scorers | Whatever they read from `run.output`. Use [`extractAgentResponseMessages()`](/reference/evals/scorer-utils#extractagentresponsemessages) to grade every assistant turn |

To grade a conversation semantically, use [`createMultiTurnJudgeScorer()`](/reference/evals/multi-turn-judge), which judges every assistant turn against one criterion. You can also attach a single-turn judge to one turn with `turns[].scorers`, where each turn grades its own input and output, or write your own scorer that reads every assistant message: see [Grading a whole conversation](/docs/evals/multi-turn#grading-a-whole-conversation).

## Score persistence

`runEvals` saves every scorer result to the scores store when the target's Mastra instance has `storage` configured. Stored results use `source: 'TEST'`, which keeps them separate from live scoring results.

Persistence also needs the scorer registered on the Mastra instance. Saving resolves scorer metadata (name, description, type) through [`Mastra.getScorerById()`](/reference/core/getScorerById), which matches on the scorer's own `id`, not on the key it's registered under.
...
export const mastra = new Mastra({
  ...
  scorers: {
    ...
    rubric: createRubricScorer({
      model: '__GATEWAY_ANTHROPIC_MODEL_HAIKU__',
      criteria: [{ description: 'placeholder' }],
    }),
  },
})
```

The registered instance only has to share the `id` of the scorer you pass to `runEvals`. Its arguments can differ, because the lookup ignores them. `checks.calledTool('get_weather')` and `checks.calledTool('search')` both have the id `check-called-tool`, so registering either one covers every use of that check, and the criteria on the registered `createRubricScorer` (id `rubric-scorer`) don't have to match the criteria you evaluate with.

Scorers attached to the agent or workflow you evaluate register themselves on the Mastra instance, so this applies to scorers passed directly to `runEvals`.

When a scorer isn't registered, `runEvals` still runs and returns correct scores, but each save fails with a `Failed to save score for scorer <id>: Scorer with id <id> not found` warning and nothing reaches the scores store.

## Related

- [Multi-turn evals](/docs/evals/multi-turn): Conceptual guide to multi-turn evaluation
- [Multi-turn Judge scorer](/reference/evals/multi-turn-judge): LLM judge that grades a whole conversation
- [`Mastra` class](/reference/core/mastra-class): Register scorers with the `scorers` option
