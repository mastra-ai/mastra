...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string[]',
    description:
      'Multi-turn inputs. Each entry is sent sequentially to the agent on the same thread. Scorers see the accumulated output from all turns. Only supported for Agent targets. When provided, `input` can be omitted.',
    isOptional: true,
  },
  {
    name: 'groundTruth',
...
Each turn runs `agent.generate()` with the same `threadId`, so the agent sees the full conversation history. Mix single-turn (`input`) and multi-turn (`inputs`) items in the same `data` array. When using `inputs`, `input` can be omitted.
