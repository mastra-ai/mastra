...
<PropertiesTable
  content={[
    ...
    {
      name: "scorers",
      type: "(MastraScorer | ScorerWithConfig)[] | WorkflowScorerConfig",
      description:
        "Array of scorers (or scorer configs) for agents, or configuration object for workflows specifying scorers for the workflow and individual steps. Use ScorerWithConfig to specify modelSettings or run scorers multiple times with different temperatures.",
      isOptional: false,
    },
    {
      name: "concurrency",
...
## Scorer Configuration (ScorerWithConfig)

For LLM-based scorers, you can configure model settings and run scorers multiple times with different temperatures using `ScorerWithConfig`:
