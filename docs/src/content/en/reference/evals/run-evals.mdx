...
<PropertiesTable
  ...
  },
  {
    ...
    type: '(string | string[] | CoreMessage[] | any)[]',
    description:
      'Multi-turn inputs. Each entry is one turn (same shape as `input`) sent sequentially to the agent on the same thread. Scorers see the accumulated output from all turns. Only supported for Agent targets. When provided, `input` can be omitted.',
    isOptional: true,
  },
  {
    name: 'groundTruth',
...
Each turn runs `agent.generate()` with the same `threadId`, so the agent sees the full conversation history. Cross-turn recall requires the agent to have a memory store configured; otherwise turns run in isolation. Mix single-turn (`input`) and multi-turn (`inputs`) items in the same `data` array. When using `inputs`, `input` can be omitted.
