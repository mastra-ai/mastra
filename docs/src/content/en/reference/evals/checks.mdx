---
title: "Reference: Quick Checks | Evals"
description: "Use Mastra Quick Checks as zero-LLM, composable micro-scorers for text matching, tool usage, tool ordering, latency, and other deterministic assertions."
packages:
  - "@mastra/evals"
---

# Quick Checks

Quick Checks are zero-LLM, composable micro-scorers for common assertions. They plug into the existing `scorers: [...]` array anywhere scorers are used: in `runEvals`, live scoring, experiments, and Studio.

Internally they're standard `createScorer()` instances, so they have the same observability, storage, and pipeline integration as any other scorer.

:::note[Register checks to persist their scores]

A check's score is only written to the scores store when the check is registered on the Mastra instance, alongside `storage`. Each check has a fixed id (`checks.includes()` is `check-includes`, `checks.calledTool()` is `check-called-tool`, and so on), so one registered instance covers every use of that check regardless of its arguments. See [Score persistence](/docs/evals/overview#score-persistence).

:::

## Usage example

```typescript title="src/evals/weather-checks.ts"
import { checks } from '@mastra/evals/checks'
import { runEvals } from '@mastra/core/evals'
import { myAgent } from '../agents'

const result = await runEvals({
  data: [{ input: 'What is the weather in Brooklyn?' }],
  target: myAgent,
  scorers: [
    checks.includes('sunny'),
    checks.calledTool('get_weather'),
    checks.toolOrder(['get_weather', 'summarize']),
    checks.noToolErrors(),
  ],
})

console.log(result.scores)
```

## Text checks

### `checks.includes(expected, options?)`

Scores 1 if the agent's output text contains the expected substring, 0 otherwise.

```typescript
checks.includes('sunny')
checks.includes('Sunny', { ignoreCase: false })
```

<PropertiesTable
  content={[
  {
    name: 'expected',
    type: 'string',
    description: 'Substring to search for in the output.',
  },
  {
    name: 'options.ignoreCase',
    type: 'boolean',
    isOptional: true,
    defaultValue: 'true',
    description: 'Case-insensitive matching.',
  },
]}
/>

Returns: `1` if found, `0` otherwise.

### `checks.excludes(unwanted, options?)`

Scores 1 if the agent's output text doesn't contain the substring, 0 otherwise.

```typescript
checks.excludes('error')
checks.excludes('Error', { ignoreCase: false })
```

<PropertiesTable
  content={[
  {
    name: 'unwanted',
    type: 'string',
    description: 'Substring that must not appear in the output.',
  },
  {
    name: 'options.ignoreCase',
    type: 'boolean',
    isOptional: true,
    defaultValue: 'true',
    description: 'Case-insensitive matching.',
  },
]}
/>

Returns: `1` if absent, `0` otherwise.

### `checks.equals(expected, options?)`

Scores 1 if the output text exactly equals the expected string after optional normalization.

```typescript
checks.equals('Hello, world!')
checks.equals('Hello', { ignoreCase: false })
```

<PropertiesTable
  content={[
  {
    name: 'expected',
    type: 'string',
    description: 'Exact string the output must match.',
  },
  {
    name: 'options.ignoreCase',
    type: 'boolean',
    isOptional: true,
    defaultValue: 'true',
    description: 'Case-insensitive matching.',
  },
]}
/>

Returns: `1` if equal, `0` otherwise.

### `checks.matches(pattern, options?)`

Scores 1 if the output matches the regular expression.

```typescript
checks.matches(/\d+°[FC]/)
checks.matches(/^hello$/, { exact: true })
```

<PropertiesTable
  content={[
  {
    name: 'pattern',
    type: 'RegExp',
    description: 'Regular expression to test against the output.',
  },
  {
    name: 'options.exact',
    type: 'boolean',
    isOptional: true,
    defaultValue: 'false',
    description: 'Anchor the pattern to match the entire output (adds ^ and $).',
  },
]}
/>

Returns: `1` if matched, `0` otherwise.

### `checks.similarity(expected, options?)`

Returns the string similarity score (0-1) between the output and an expected string using the Dice coefficient. When a `threshold` is set, returns binary 1/0 instead.

```typescript
checks.similarity('Sunny, 72°F')
checks.similarity('Sunny, 72°F', { threshold: 0.7 })
```

<PropertiesTable
  content={[
  {
    name: 'expected',
    type: 'string',
    description: 'Reference string to compare against.',
  },
  {
    name: 'options.threshold',
    type: 'number',
    isOptional: true,
    description: 'Minimum similarity score (0-1) to return 1. When omitted, returns the raw similarity score.',
  },
  {
    name: 'options.ignoreCase',
    type: 'boolean',
    isOptional: true,
    defaultValue: 'true',
    description: 'Case-insensitive comparison.',
  },
]}
/>

Returns: Raw similarity score (0-1), or binary `1`/`0` when `threshold` is set.

## Tool call checks

### `checks.calledTool(toolName, options?)`

Scores 1 if the agent called the specified tool at least the required number of times.

```typescript
checks.calledTool('get_weather')
checks.calledTool('search', { times: 2 })
```

<PropertiesTable
  content={[
  {
    name: 'toolName',
    type: 'string',
    description: 'Name of the tool to look for.',
  },
  {
    name: 'options.times',
    type: 'number',
    isOptional: true,
    defaultValue: '1',
    description: 'Minimum number of times the tool must be called.',
  },
]}
/>

Returns: `1` if called at least `times` times, `0` otherwise.

### `checks.didNotCall(toolName, options?)`

Scores 1 if the agent didn't call the specified tool.

```typescript
checks.didNotCall('delete_user')
```

<PropertiesTable
  content={[
  {
    name: 'toolName',
    type: 'string',
    description: 'Name of the tool that must not appear.',
  },
]}
/>

Returns: `1` if the tool wasn't called, `0` otherwise.

### `checks.toolOrder(expectedOrder, options?)`

Scores 1 if the tools were called in the specified order. Uses relaxed matching. Other tool calls between the expected tools are allowed.

```typescript
checks.toolOrder(['search', 'summarize', 'respond'])
```

<PropertiesTable
  content={[
  {
    name: 'expectedOrder',
    type: 'string[]',
    description: 'Tool names in the expected calling sequence. Must appear as a subsequence of the actual tool calls.',
  },
]}
/>

Returns: `1` if the expected order is satisfied, `0` otherwise.

### `checks.maxToolCalls(max, options?)`

Scores 1 if the agent used no more than `max` tool calls.

```typescript
checks.maxToolCalls(5)
```

<PropertiesTable
  content={[
  {
    name: 'max',
    type: 'number',
    description: 'Maximum number of tool calls allowed.',
  },
]}
/>

Returns: `1` if within the limit, `0` otherwise.

### `checks.usedNoTools(options?)`

Scores 1 if the agent made no tool calls at all.

```typescript
checks.usedNoTools()
```

Returns: `1` if no tools were called, `0` otherwise.

### `checks.noToolErrors(options?)`

Scores 1 if none of the tool invocations resulted in an error state. Detects both error results (`result.error` present) and incomplete tool calls (`state === 'call'`).

```typescript
checks.noToolErrors()
```

Returns: `1` if all tool calls succeeded, `0` otherwise.

## Combining checks with other scorers

Checks compose with LLM-based and code-based scorers in the same `scorers` array:

```typescript title="src/evals/combined-eval.ts"
import { checks } from '@mastra/evals/checks'
import { createAnswerRelevancyScorer } from '@mastra/evals/scorers/prebuilt'
import { runEvals } from '@mastra/core/evals'
import { myAgent } from '../agents'

const result = await runEvals({
  data: [{ input: 'What is the weather in Brooklyn?' }],
  target: myAgent,
  scorers: [
    // Zero-LLM checks
    checks.includes('Brooklyn'),
    checks.calledTool('get_weather'),
    checks.noToolErrors(),
    // LLM-based scorer
    createAnswerRelevancyScorer({ model: '__GATEWAY_OPENAI_MODEL_MINI__' }),
  ],
})
```

## Using the same check more than once

Every check accepts optional `id` and `name` overrides in its `options`. `runEvals` keys results by scorer id, so two checks with the same default id (for example two `checks.includes()` calls) are merged into a single averaged row and a failing gate can't be attributed to either one. Give each instance its own id when you use a check more than once:

```typescript title="src/evals/weather-checks.ts"
gates: [
  checks.includes('London', { id: 'mentions-city' }),
  checks.includes('°C', { id: 'mentions-temperature', name: 'Mentions temperature' }),
  checks.noToolErrors({ id: 'no-tool-errors' }),
]
```

`runEvals` logs a warning when two scorers or gates share an id.

## Multi-turn behavior

Checks read the accumulated `run.output`, so in a [multi-turn eval](/docs/evals/multi-turn) they see every turn. `checks.calledTool('get_weather', { times: 2 })` counts calls across the whole conversation, and `checks.includes()` searches all assistant text. Use per-turn `turns[].scorers` when a specific turn has to satisfy the check.

## Related

- [Quick Checks overview](/docs/evals/quick-checks)
- [Built-in Scorers](/docs/evals/built-in-scorers)
- [`createScorer()` reference](/reference/evals/create-scorer)
- [`runEvals()` reference](/reference/evals/run-evals)
- [Custom Scorers](/docs/evals/custom-scorers)
