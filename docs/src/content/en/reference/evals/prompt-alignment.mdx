...
By default the scorer only sees the current turn. In a conversation, a reply like `"A"` is meaningless on its own, so the judge cannot tell what the user asked for and scores the response as misaligned.
