...
To use a configured `Classifier` as the only model judge, use [`createClassifierScorer()`](/reference/evals/create-classifier-scorer). It projects one typed classifier answer to the required numeric score while retaining the answer and uncertainty evidence in `analyzeStepResult`.
