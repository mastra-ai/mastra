    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            isOptional: true,
            description:
              'Error processors for the internal V2+ judge agent. These implement `processAPIError` and can inspect LLM API rejections and signal a retry, e.g. `StreamErrorRetryProcessor`. V1 judges use `generateLegacy()` and do not run error processors.',
          },
          {
            ...
            isOptional: true,
            description:
              'Maximum number of times error processors can retry one V2+ judge generation. The runtime defaults to 10 when errorProcessors are configured without this value. Set it explicitly to bound the retry budget.',
          },
        ],
      },
    ],
...
## Retry V2+ judge requests

Use the existing judge `errorProcessors` configuration to retry transient failures inside the failed V2+ judge request. This doesn't retry the scorer workflow, trace target, batch item, score write, or a completed scorer step.
...
Coordinated processor retries require a V2+ judge model adapter. V1 judges call [`generateLegacy()`](/reference/agents/generateLegacy), bypass error processors, and use that API's separate AI SDK `maxRetries` default of `2`.
