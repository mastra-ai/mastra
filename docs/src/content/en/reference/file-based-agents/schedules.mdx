...
The handler's return value is merged over the schedule's stored fields. Returning `undefined` applies no overrides, so the fire falls back to those stored fields — and because a handler-mode schedule cannot declare a `prompt`, that fire fails for a missing prompt. Return a `prompt` to run, or `null` to skip.
