...
Two files that resolve to the same id, such as `heartbeat.ts` and `heartbeat.md`, are a build error.
...
The handler's return value is merged over the schedule's stored fields. Returning `undefined` fires with the declared defaults.
...
That in-process lookup means the process running the scheduler must have the owning agent registered. A normal deployment boots one entry, so this is automatic. If you run the scheduler as a standalone worker, ensure it boots the same entry as your server. A worker built from a trimmed entry has no handler to call, and those fires will fail rather than run.
...
Frontmatter accepts every option below except `handler`, which needs a function and so needs a `.ts` file. `prompt` isn't settable either, because the body is the prompt. Unknown frontmatter fields fail the build rather than being silently ignored, so a typo like `ifIdel` is caught at build time.
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Computes the fire at trigger time. Return overrides to apply, `null` to skip this fire, or nothing to use the declared defaults. Set this or `prompt`, not both.',
    isOptional: true,
  },
  {
    name: 'timezone',
...
This records a trigger with `triggerKind: "manual"` and does not advance `nextFireAt`, so the regular cadence is unaffected. Studio lists the same schedules and their trigger history.
...
Mastra syncs declared schedules into schedule storage when it starts, and again whenever an agent is registered afterwards. Declaring a schedule is enough to start the scheduler, with no `scheduler: { enabled: true }` needed.

The sync is a diff, not a rewrite:
