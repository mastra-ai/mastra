...
For usage patterns and deployment topologies, see [Workers](/ecosystem/deployment-patterns/workers).
...
A bearer token sent by the orchestration worker when calling the API's step execution endpoint. The API's configured auth provider must recognize this token. See [Worker authentication](/ecosystem/auth-providers/workers).
