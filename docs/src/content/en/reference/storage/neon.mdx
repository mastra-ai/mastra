...
The command signs you in, writes project context to `.neon`, and pulls the current branch's environment variables. Run `neon env pull` again whenever you need to refresh them:
...
`PgVector` installs the extension when it creates its first index. If schema changes run separately, create the extension and indexes over the direct connection, then use the pooled connection with `disableInit: true` at runtime.
