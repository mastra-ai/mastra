...
:::warning[Using `import { env } from 'cloudflare:workers'`]
If you use the `cloudflare:workers` env import, initialize `D1Store` inline inside `new Mastra({...})`. Extracting it to a variable outside runs at module load before `env` is populated.
