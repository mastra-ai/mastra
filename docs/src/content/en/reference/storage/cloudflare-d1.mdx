...
:::note
`D1Store` must be initialized inline inside `new Mastra({...})`. See [CloudflareDeployer reference](/reference/deployer/cloudflare#cloudflare-bindings) for details.
:::

### Using with Cloudflare Workers directly

If you're writing a raw Cloudflare Worker without `CloudflareDeployer`, access the D1 binding from the worker's `env` parameter inside the `fetch` handler.
