...
**Note:** The `observability` domain is not currently supported by Upstash storage. Use another storage adapter for observability features.
...
**Workflows Domain:**
- **Workflow Snapshots**: `mastra_workflow_snapshot:namespace:workflows:workflow_name:{workflowName}:run_id:{runId}[:resourceId:{resourceId}]`
