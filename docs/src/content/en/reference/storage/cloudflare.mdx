...
CloudflareStore implements the domain-based storage architecture. To access domain-specific operations (threads, messages, workflows, etc.), use `getStore()`:

```typescript copy
const storage = new CloudflareStore({
  bindings: {
    threads: THREADS_KV,
    messages: MESSAGES_KV,
  },
});
