...
`MastraStorageExporter` automatically selects the `insert-only` strategy when ClickHouse is the observability backend, which gives the highest write throughput. See [tracing strategies](/ecosystem/observability-integrations/exporters/mastra-storage#tracing-strategies) for details.
...
For the full strategy matrix and production guidance, see the [`MastraStorageExporter` reference](/ecosystem/observability-integrations/exporters/mastra-storage#storage-provider-support).

## Related

- [Storage overview](/reference/storage/overview)
- [Composite storage](/reference/storage/composite)
- [`MastraStorageExporter`](/ecosystem/observability-integrations/exporters/mastra-storage)
