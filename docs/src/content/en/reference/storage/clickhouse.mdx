...
If existing Mastra tables use local `MergeTree` or `ReplacingMergeTree` engines, initialization fails while `replication` is enabled. Migrate or recreate those tables manually before enabling replication.
