...
`MastraStorageExporter` automatically selects the `insert-only` strategy when ClickHouse is the observability backend, which gives the highest write throughput. See [tracing strategies](/guides/observability/integrations/exporters/mastra-storage#tracing-strategies) for details.
...
For the full strategy matrix and production guidance, see the [`MastraStorageExporter` reference](/guides/observability/integrations/exporters/mastra-storage#storage-provider-support).

## Related

- [Storage overview](/reference/storage/overview)
- [Composite storage](/reference/storage/composite)
- [`MastraStorageExporter`](/guides/observability/integrations/exporters/mastra-storage)
