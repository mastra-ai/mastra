...
PostgreSQL supports observability and can handle moderate trace volumes (tens of thousands of traces per day). For high-volume production environments, consider:
...
- Migrating observability to [ClickHouse via composite storage](/reference/storage/composite#specialized-storage-for-observability-recommended-for-production) if you need to scale further
