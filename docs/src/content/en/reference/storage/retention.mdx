...
Deletion requests are retained long enough to keep enforcing erasure after signal rows expire. When `tracing` retention is configured, the `mastra_deletion_requests` TTL is the longest configured signal retention period plus 30 days because trace deletion cascades to linked metrics, logs, scores, and feedback. Without `tracing` retention, the TTL is the longer configured `scores` or `feedback` period plus 30 days. For example, a 90-day score retention period produces a 120-day deletion-request TTL.
