...
| ClickHouse | Native TTL | Observability spans, metrics, logs, scores, and feedback. Deletion-request records expire after the longest configured deletion-capable signal retention plus 30 days |
...
Deletion requests are retained long enough to keep enforcing erasure after signal rows expire. When `tracing`, `scores`, or `feedback` retention is configured, the `mastra_deletion_requests` TTL is the longest of those configured periods plus 30 days. For example, a 90-day score retention period produces a 120-day deletion-request TTL.
