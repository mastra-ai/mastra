...
description: 'Configure age-based retention policies and call prune() to delete old rows in bounded, resumable batches.'
packages:
  - '@mastra/clickhouse'
  - '@mastra/core'
  - '@mastra/dsql'
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      "Replaces the store's configured retention policies for this call only, for example to skip a domain or prune more aggressively. The configured `retention` is unchanged.",
  },
]}
/>
...
Signals with no value, or a value of zero or less, get no TTL. When you remove a signal from `retention`, the next `init()` or `applyRetention()` removes that table's TTL.
...
- Your retention periods are fixed (for example, always 30 days)
