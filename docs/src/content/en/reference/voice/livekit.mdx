...
See [LiveKit](/ecosystem/voice-integrations/livekit) for setup and concepts.
...
Returns an [API route](/guides/server/custom-api-routes) that mints a LiveKit access token with the voice agent dispatched into the room. Frontends call it to join a session.
...
- [LiveKit](/ecosystem/voice-integrations/livekit)
