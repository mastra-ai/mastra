...
- `@mastra/livekit/plugin`: the LLM-component plugin — [`MastraLLM`](#mastrallm) and [`createRemoteAgentReplyGenerator()`](#createremoteagentreplygenerator). Import it in workers that build their own `voice.AgentSession`. Both symbols are also re-exported from `@mastra/livekit/worker`.
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Called as each tool call starts mid-reply, before its result is known. The building block for tool-driven side effects such as analytics or agent-initiated hang-up. Runs synchronously on the stream; keep it cheap. Applies to the agent reply path.',
    isOptional: true,
  },
  {
    name: 'onTurnComplete',
...
<PropertiesTable
  ...
  },
  {
    ...
    type: '(toolCall: VoiceToolCall) => void',
    description:
      'Called as each tool call starts, mid-stream. Pair with runEndCall() to rebuild agent-initiated hang-up.',
    isOptional: true,
  },
  {
    name: 'onTurnComplete',
...
<PropertiesTable
  ...
  {
    name: 'config',
    type: '{ message?: string; reason?: string; maxWaitMs?: number }',
    description:
      'Optional final message spoken before hang-up, the shutdown reason to record, and the safety cap on waiting for closing words.',
  },
  {
    name: 'logger',
