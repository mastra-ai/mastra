...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description:
        "Enable Mastra's built-in token auth. Disabled by default — most NestJS apps use their own auth guards. Query-string `apiKey` auth is opt-in for backward compatibility.",
      isOptional: true,
