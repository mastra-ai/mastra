...
<PropertiesTable
  ...
  },
  {
    ...
    description: 'Custom validation error handler for this route. Overrides the server-level `onValidationError` hook. Return `{ status, body }` to customize the response, or `undefined` to use the default.',
    isOptional: true,
  },
]}
/>
