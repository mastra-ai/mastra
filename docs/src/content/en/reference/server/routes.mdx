---
title: "Reference: Built-in Routes | Server"
description: "API reference for built-in HTTP routes registered by Mastra server adapters."
---

# Built-in Routes

Server adapters automatically register these routes when you call `server.init()`. All routes are prefixed with the `prefix` option if configured.
...
- [HTTPException](/reference/server/http-exception) - Error handling
