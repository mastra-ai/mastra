...
| `GET` | `/api/agents/:agentId/signals` | List signals queued for a thread that have not reached the model |
...
Use `GET /api/agents/:agentId/signals?threadId=…&resourceId=…` to list the selected agent's signals still queued for a thread, in delivery order. Use `DELETE /api/agents/:agentId/signals?threadId=…&resourceId=…` with a JSON body containing `signalIds` to remove one or more. The array must contain 1–1,000 non-empty strings. Neither route accesses signals owned by other agent IDs on that thread. Both routes only reflect the in-memory queue of the server process that handles the request.
