---
title: "Reference: Server routes | Server"
description: "API reference for HTTP routes registered by Mastra server adapters."
packages:
  - "@mastra/server"
---

# Server routes

Server adapters register these routes when you call `server.init()`. All routes are prefixed with the `prefix` option if configured.

## Agents

| Method | Path | Description |
| - | - | - |
| `GET` | `/api/agents` | List all agents |
| `GET` | `/api/agents/:agentId` | Get agent by ID (supports version query params) |
| `POST` | `/api/agents/:agentId/generate` | Generate agent response |
| `POST` | `/api/agents/:agentId/stream` | Stream agent response |
| `POST` | `/api/agents/:agentId/resume-stream` | Resume a suspended agent stream with custom data |
| `GET` | `/api/agents/:agentId/tools` | List agent tools |
| `POST` | `/api/agents/:agentId/tools/:toolId/execute` | Execute agent tool |

### Get agent query parameters

`GET /api/agents/:agentId` accepts optional query parameters to control which stored config version is applied as overrides to code-defined agents:

| Parameter | Type | Default | Description |
| - | - | - | - |
| `status` | `'draft' \| 'published'` | `'draft'` | Which stored version to resolve. `draft` returns the latest version, `published` returns the active published version. |
| `versionId` | `string` | — | A specific version ID to resolve. Takes precedence over `status`. |

```bash
# Get agent with latest draft overrides (default)
GET /api/agents/my-agent

# Get agent with published overrides
GET /api/agents/my-agent?status=published

# Get agent with a specific version's overrides
GET /api/agents/my-agent?versionId=abc123
```

### Generate request body

```typescript prettier:false
{
  messages: CoreMessage[] | string;       // Required
  instructions?: string;                   // System instructions
  system?: string;                         // System prompt
  context?: CoreMessage[];                 // Additional context
  memory?: { key: string } | boolean;      // Memory config
  resourceId?: string;                     // Resource identifier
  threadId?: string;                       // Thread identifier
  runId?: string;                          // Run identifier
  maxSteps?: number;                       // Max tool steps
  activeTools?: string[];                  // Tools to enable
  toolChoice?: ToolChoice;                 // Tool selection mode
  requestContext?: Record<string, unknown>; // Request context
  output?: ZodSchema;                      // Structured output schema
}
```

### Generate response

```typescript prettier:false
{
  text: string;
  toolCalls?: ToolCall[];
  finishReason: string;
  usage?: {
    promptTokens: number;
    completionTokens: number;
  };
}
```

## Agent rollouts

Rollouts split traffic between agent versions for canary releases and A/B experiments. Only one rollout can be active per agent. See [Rollouts](/docs/agents/rollouts) for the conceptual overview.

| Method | Path | Description |
| - | - | - |
| `GET` | `/api/agents/:agentId/rollout` | Get the active rollout, including live score summaries |
| `POST` | `/api/agents/:agentId/rollout` | Start a canary rollout or A/B experiment |
| `PATCH` | `/api/agents/:agentId/rollout` | Update the candidate weight (canary only) |
| `POST` | `/api/agents/:agentId/rollout/promote` | Promote the candidate or pick an A/B winner |
| `POST` | `/api/agents/:agentId/rollout/rollback` | Roll back to the stable version |
| `DELETE` | `/api/agents/:agentId/rollout` | Cancel an active rollout |
| `GET` | `/api/agents/:agentId/rollout/results` | Per-version score breakdown (avg, stddev, min, max) |
| `GET` | `/api/agents/:agentId/rollouts` | List rollout history (paginated) |

Allocation weights are fractional values in `[0, 1]` and must sum to `1`. The default `routingKey` is `resourceId`, which makes each user sticky to one version for the duration of the rollout.

### Start rollout request body

`POST /api/agents/:agentId/rollout` accepts a discriminated union on `type`.

Canary:

```typescript prettier:false
{
  type: 'canary';
  candidateVersionId: string;       // Version ID of the candidate
  candidateWeight: number;          // Fractional traffic for the candidate (0-1, exclusive)
  routingKey?: string;              // Request context field to hash for sticky routing (default: resourceId)
  rules?: Array<{
    scorerId: string;               // Scorer to monitor
    threshold: number;              // Minimum acceptable average score (0-1)
    windowSize: number;             // Number of recent scores to evaluate
    action: 'rollback';
  }>;
}
```

A/B test:

```typescript prettier:false
{
  type: 'ab_test';
  allocations: Array<{
    versionId: string;
    weight: number;                 // Fractional weight (0-1); weights must sum to 1
    label?: string;                 // Human-readable label (e.g. "control", "variant")
  }>;                               // Minimum 2 allocations
  routingKey?: string;
}
```

### Update rollout request body

`PATCH /api/agents/:agentId/rollout` updates the candidate weight on an active canary rollout. The stable allocation is recalculated as `1 - candidateWeight`.

```typescript prettier:false
{
  candidateWeight: number;          // New fractional traffic for the candidate (0-1, exclusive)
}
```

### Promote rollout request body

`POST /api/agents/:agentId/rollout/promote` promotes a version to become the new active version and completes the rollout. For canary rollouts, the body can be omitted — the candidate is promoted. For A/B tests, specify which version wins.

```typescript prettier:false
{
  versionId?: string;               // For A/B tests: which version to promote
}
```

### Rollout response

`GET`, `POST`, and `PATCH` on `/api/agents/:agentId/rollout` return the rollout record. `GET` enriches each allocation with live score summaries from the observability OLAP store.

```typescript prettier:false
{
  id: string;
  agentId: string;
  type: 'canary' | 'ab_test';
  status: 'active' | 'completed' | 'rolled_back' | 'cancelled';
  stableVersionId: string;
  allocations: Array<{
    versionId: string;
    weight: number;                 // Fractional weight (0-1)
    label?: string;
    scores?: Record<string, { avg: number; count: number }>;
  }>;
  routingKey?: string;
  rules?: Array<{
    scorerId: string;
    threshold: number;
    windowSize: number;
    action: 'rollback';
  }>;
  createdAt: Date;
  updatedAt: Date;
  completedAt?: Date | null;
}
```

`POST /promote`, `POST /rollback`, and `DELETE` return `{ success: true, rollout }` with the same rollout shape.

### Rollout results response

`GET /api/agents/:agentId/rollout/results` returns per-allocation score statistics computed from the scores store.

```typescript prettier:false
{
  rolloutId: string;
  type: 'canary' | 'ab_test';
  allocations: Array<{
    versionId: string;
    label?: string;
    requestCount: number;
    scores: Record<
      string,                       // scorerId
      {
        avg: number;
        stddev: number;
        count: number;
        min: number;
        max: number;
      }
    >;
  }>;
}
```

### List rollouts query parameters

`GET /api/agents/:agentId/rollouts` accepts standard pagination query parameters and returns a page of rollout records.

| Parameter | Type | Default | Description |
| - | - | - | - |
| `page` | `number` | `1` | Page number (1-indexed) |
| `perPage` | `number` | — | Results per page (max `100`) |

```typescript prettier:false
{
  rollouts: RolloutRecord[];
  total: number;
  page: number;
  perPage: number;
}
```

## Workflows

| Method | Path | Description |
| - | - | - |
| `GET` | `/api/workflows` | List all workflows |
| `GET` | `/api/workflows/:workflowId` | Get workflow by ID |
| `POST` | `/api/workflows/:workflowId/create-run` | Create a new workflow run |
| `POST` | `/api/workflows/:workflowId/start-async` | Start workflow and await result |
| `POST` | `/api/workflows/:workflowId/stream` | Stream workflow execution |
| `POST` | `/api/workflows/:workflowId/resume` | Resume suspended workflow |
| `POST` | `/api/workflows/:workflowId/resume-async` | Resume asynchronously |
| `GET` | `/api/workflows/:workflowId/runs` | List workflow runs |
| `GET` | `/api/workflows/:workflowId/runs/:runId` | Get specific run |

### Create run request body

```typescript prettier:false
{
  resourceId?: string;     // Associate run with a resource (e.g., user ID)
  disableScorers?: boolean; // Disable scorers for this run
}
```

### Request body for `/start-async`

```typescript prettier:false
{
  resourceId?: string;     // Associate run with a resource (e.g., user ID)
  inputData?: unknown;
  initialState?: unknown;
  requestContext?: Record<string, unknown>;
  tracingOptions?: {
    spanName?: string;
    attributes?: Record<string, unknown>;
  };
}
```

### Stream workflow request body

```typescript prettier:false
{
  resourceId?: string;     // Associate run with a resource (e.g., user ID)
  inputData?: unknown;
  initialState?: unknown;
  requestContext?: Record<string, unknown>;
  closeOnSuspend?: boolean;
}
```

### Resume request body

```typescript prettier:false
{
  step?: string | string[];
  resumeData?: unknown;
  requestContext?: Record<string, unknown>;
}
```

## Tools

| Method | Path | Description |
| - | - | - |
| `GET` | `/api/tools` | List all tools |
| `GET` | `/api/tools/:toolId` | Get tool by ID |
| `POST` | `/api/tools/:toolId/execute` | Execute tool |

### Execute tool request body

```typescript prettier:false
{
  data: unknown;  // Tool input data
  requestContext?: Record<string, unknown>;
}
```

## Memory

| Method | Path | Description |
| - | - | - |
| `GET` | `/api/memory/threads` | List threads |
| `GET` | `/api/memory/threads/:threadId` | Get thread |
| `POST` | `/api/memory/threads` | Create thread |
| `DELETE` | `/api/memory/threads/:threadId` | Delete thread |
| `POST` | `/api/memory/threads/:threadId/clone` | Clone thread |
| `GET` | `/api/memory/threads/:threadId/messages` | Get thread messages |
| `POST` | `/api/memory/threads/:threadId/messages` | Add message |

### Create thread request body

```typescript prettier:false
{
  resourceId: string;
  title?: string;
  metadata?: Record<string, unknown>;
}
```

### Clone thread request body

```typescript prettier:false
{
  newThreadId?: string;           // Custom ID for cloned thread
  resourceId?: string;            // Override resource ID
  title?: string;                 // Custom title for clone
  metadata?: Record<string, unknown>;  // Additional metadata
  options?: {
    messageLimit?: number;        // Max messages to clone
    messageFilter?: {
      startDate?: Date;           // Clone messages after this date
      endDate?: Date;             // Clone messages before this date
      messageIds?: string[];      // Clone specific messages
    };
  };
}
```

### Clone thread response

```typescript prettier:false
{
  thread: {
    id: string;
    resourceId: string;
    title: string;
    createdAt: Date;
    updatedAt: Date;
    metadata: {
      clone: {
        sourceThreadId: string;
        clonedAt: Date;
        lastMessageId?: string;
      };
      // ... other metadata
    };
  };
  clonedMessages: MastraDBMessage[];
}
```

## Vectors

| Method | Path | Description |
| - | - | - |
| `POST` | `/api/vectors/:vectorName/upsert` | Upsert vectors |
| `POST` | `/api/vectors/:vectorName/query` | Query vectors |
| `POST` | `/api/vectors/:vectorName/delete` | Delete vectors |

### Upsert request body

```typescript
{
  vectors: Array<{
    id: string
    values: number[]
    metadata?: Record<string, unknown>
  }>
}
```

### Query request body

```typescript prettier:false
{
  vector: number[];
  topK?: number;
  filter?: Record<string, unknown>;
  includeMetadata?: boolean;
}
```

## MCP

| Method | Path | Description |
| - | - | - |
| `GET` | `/api/mcp/servers` | List MCP servers |
| `GET` | `/api/mcp/servers/:serverId/tools` | List server tools |
| `POST` | `/api/mcp/:serverId` | MCP HTTP transport |
| `GET` | `/api/mcp/:serverId/sse` | MCP SSE transport |

## Responses API

| Method | Path | Description |
| - | - | - |
| `POST` | `/api/v1/responses` | Create a response through the OpenAI-compatible Responses API route |
| `GET` | `/api/v1/responses/:responseId` | Retrieve a stored response |
| `DELETE` | `/api/v1/responses/:responseId` | Delete a stored response |

For the full request and response contract, see the [Responses API reference](/reference/client-js/responses).

## Conversations API

| Method | Path | Description |
| - | - | - |
| `POST` | `/api/v1/conversations` | Create a conversation |
| `GET` | `/api/v1/conversations/:conversationId` | Retrieve a conversation |
| `DELETE` | `/api/v1/conversations/:conversationId` | Delete a conversation |
| `GET` | `/api/v1/conversations/:conversationId/items` | List stored items for a conversation |

For the full request and response contract, see the [Conversations API reference](/reference/client-js/conversations).

## Logs

| Method | Path | Description |
| - | - | - |
| `GET` | `/api/logs` | List logs |
| `GET` | `/api/logs/:runId` | Get logs by run ID |

### Query parameters

```typescript prettier:false
{
  page?: number;
  perPage?: number;
  transportId?: string;
}
```

## Telemetry

| Method | Path | Description |
| - | - | - |
| `GET` | `/api/telemetry/traces` | List traces |
| `GET` | `/api/telemetry/traces/:traceId` | Get trace |
| `GET` | `/api/telemetry/traces/:traceId/spans` | Get trace spans |

## Common query parameters

### Pagination

Most list endpoints support:

```typescript prettier:false
{
  page?: number;   // Page number (0-indexed)
  perPage?: number; // Items per page (default: 10)
}
```

### Filtering

Workflow runs support:

```typescript prettier:false
{
  fromDate?: string;  // ISO date string
  toDate?: string;    // ISO date string
  status?: string;    // Run status filter
  resourceId?: string; // Filter by resource
}
```

## Error responses

All routes return errors in this format:

```typescript prettier:false
{
  error: string;      // Error message
  details?: unknown;  // Additional details
}
```

Common status codes:

| Code | Meaning |
| - | - |
| 400 | Bad Request - Invalid parameters |
| 401 | Unauthorized - Missing/invalid auth |
| 403 | Forbidden - Insufficient permissions |
| 404 | Not Found - Resource doesn't exist |
| 500 | Internal Server Error |

## Related

- [createRoute()](/reference/server/create-route): Creating custom routes
- [Server Adapters](/docs/server/server-adapters): Using adapters
