...
```typescript prettier:false
{
  model: string;                    // Required. Mastra agent ID.
  input: string | Array<{
  ...
  instructions?: string;            // Override the agent instructions
  stream?: boolean;                 // Return SSE events instead of JSON
  store?: boolean;                  // Persist the response when the agent has memory configured
  previous_response_id?: string;    // Continue a stored conversation thread
}
```
