...
`GET /api/agents/:agentId` and `POST /api/agents/:agentId/generate` accept optional query parameters to control which stored config version is applied. Pass at most one selector:
...
| `versionId` | `string` | None | A specific immutable version ID to resolve. |
| `label` | `string` | None | A computed or custom version label to resolve. |
...
# Get the version currently targeted by a label
GET /api/agents/my-agent?label=candidate
```

Combining `status`, `versionId`, and `label` returns `INVALID_VERSION_SELECTOR`. Other JSON execution routes accept the root selector through `versions.self` in the request body instead of query parameters. See [Editor versioning](/reference/editor/versioning#select-an-agent-version) for selector behavior and the public error envelope.
...
```typescript prettier:false
{
  ...
  versions?: {
    self?: { versionId: string } | { label: string } | { status: 'draft' | 'published' };
    agents?: Record<string, { versionId: string } | { label: string } | { status: 'draft' | 'published' }>;
    defaultStatus?: 'draft' | 'published';
  };
}
```

`versions.self` selects the root agent for this execution only. `versions.agents` selects delegated sub-agents. If a query selector and `versions.self` are both present, they must identify the same target.

New executions resolve these selectors once and persist immutable root and explicit sub-agent version IDs with the run. Resume, approval, retry, and recovery routes consume those pins instead of looking up a label again. They reject `label`, `status`, or a different exact ID with `PINNED_VERSION_CONFLICT`. Repeating the pinned `versionId` is allowed.
...
```typescript prettier:false
{
  // Source run for a messages-based continuation. This lets another server
  // recover the immutable root and dependency version pins.
  runId?: string;
  resourceId: string;
  threadId: string;
  toolCallId: string;
...
Pass `runId` when returning client-tool result messages. The server uses it only as the source run identity. The acknowledgement contains the new continuation run ID. The JavaScript client's subscribed client-tool flow supplies this value automatically.
