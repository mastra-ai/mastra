...
Selective cancellation isn't forwarded across server instances. Clear-on-abort affects the receiving process and the active owner, not every process's queues. See the [client cancellation methods](/reference/client-js/agents#cancelqueuedmessages) for SDK usage.
