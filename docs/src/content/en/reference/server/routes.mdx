...
| `GET` | `/api/agents/:agentId/signals` | List signals queued for a thread that haven't reached the model |
| `DELETE` | `/api/agents/:agentId/signals` | Remove queued signals by ID before delivery |
...
### Pending signal routes

Use `GET /api/agents/:agentId/signals?threadId=…&resourceId=…` to list the selected agent's signals still queued for a thread, in delivery order. Use `DELETE /api/agents/:agentId/signals?threadId=…&resourceId=…` with a JSON body containing `signalIds` to remove one or more. The array must contain 1 to 1,000 non-empty strings. Neither route accesses signals owned by other agent IDs on that thread. Both routes only reflect the in-memory queue of the server process that handles the request.

```bash
curl 'http://localhost:4111/api/agents/supportAgent/signals?threadId=thread_456&resourceId=user_123'
```
...
```bash
curl -X DELETE 'http://localhost:4111/api/agents/supportAgent/signals?threadId=thread_456&resourceId=user_123' \
  -H 'Content-Type: application/json' \
  -d '{"signalIds":["msg_abc","msg_def"]}'
```

File parts in `contents` use string `data` (base64 or a URL). The delete route returns `{ removedSignalIds: string[] }` containing only IDs actually removed, in request order without duplicates. Missing, other-agent, and already-drained signals are skipped. If none match, the array is empty. Removal cancels queued delivery only, without aborting runs or reverting state updates, notification status, or acceptance results. See [cancellation semantics](/reference/agents/agent#removependingsignalsoptions).
