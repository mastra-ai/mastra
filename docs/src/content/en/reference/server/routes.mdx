...
Both routes return `{ "success": true }` after deletion. They return `501` when the installed `@mastra/core` predates experiment deletion support. If the configured storage doesn't provide observability storage or trace deletion, Mastra logs a warning, skips the trace cascade, and still deletes the experiment and its result records.
