...
| `POST` | `/api/agents/:agentId/threads/signals/cancel` | Cancel selected process-local pending signals |
...
Both routes require `agents:execute` permission and apply thread ownership checks using the authenticated request context. Cancellation covers Agents sharing the memory thread, not only the Agent named in the URL. It doesn't undo persisted messages, state updates, notification records, or acceptance acknowledgements, and it excludes continuations from `continueWithMessages()`.

Selective cancellation isn't forwarded across server instances. Clear-on-abort affects the receiving process and the active owner, not every process's queues. See the [client cancellation methods](/reference/client-js/agents#cancelpendingsignals) for SDK usage.
