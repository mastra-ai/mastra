...
Both delete routes remove the experiment, its result records, and the observability traces it produced. Trace deletion cascades to spans and trace-linked scores, feedback, metrics, and logs.
...
The route returns `404` if the experiment doesn't exist or doesn't belong to the dataset.
...
Both routes return `{ "success": true }` after deletion. They return `501` unless the installed `@mastra/core` advertises support through the `experiment-deletion` feature flag, including when an older version predates this support. Storage without observability or trace deletion support leaves the traces in place. Mastra logs a warning and still deletes the experiment with its result records.
