...
| `POST` | `/api/agents/:agentId/approve-tool-call-for-thread` | Approve a tool call and resume through a thread subscription |
| `POST` | `/api/agents/:agentId/decline-tool-call-for-thread` | Decline a tool call and resume through a thread subscription |
...
Use `POST /api/agents/:agentId/approve-tool-call-for-thread` or `POST /api/agents/:agentId/decline-tool-call-for-thread` when the client already has an active thread subscription. The route resumes the run and returns a JSON acknowledgement. Resumed stream chunks are delivered through `POST /api/agents/:agentId/threads/subscribe`.
