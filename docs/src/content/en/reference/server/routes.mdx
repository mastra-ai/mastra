...
| Method | Path | Description |
| - | - | - |
| `POST` | `/api/v1/responses` | Create a response through an OpenAI-compatible Responses API route |
...
```typescript prettier:false
{
  model: string;                    // Required. Underlying provider/model, such as "openai/gpt-5".
  agent_id: string;                 // Required on initial requests. Mastra agent ID.
  input: string | Array<{
  ...
  instructions?: string;            // Optional request-scoped instructions for this response
  text?: {
    format: {
      type: 'json_object';          // Optional. Request JSON object output
    } | {
      type: 'json_schema';          // Optional. Request schema-constrained structured output
      name: string;
  ...
  stream?: boolean;                 // Return SSE events instead of JSON
  store?: boolean;                  // Persist the response through the selected agent's memory
  previous_response_id?: string;    // Continue a previously stored response chain
}
```
...
```typescript prettier:false
{
  ...
  store?: boolean;                  // Reflects whether the response was actually persisted
}
```
