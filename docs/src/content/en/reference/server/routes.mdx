...
Locally cancelled IDs are published through the shared PubSub backend to other processes subscribed to the thread. Each subscriber removes matching pending copies. Propagation is best-effort and asynchronous, and the HTTP response doesn't confirm remote cancellation. IDs missing locally aren't published. Clear-on-abort affects the receiving process and the active owner, not every process's queues. See the [client cancellation methods](/reference/client-js/agents#cancelqueuedmessages) for SDK usage.
