...
Both routes return `{ "success": true }` after deletion, or `501` when the installed `@mastra/core` predates experiment deletion support. Storage without observability or trace deletion support leaves the traces in place. Mastra logs a warning and still deletes the experiment with its result records.
