...
Both delete routes remove the experiment and its result records when deletion succeeds. When storage supports observability and trace deletion, Mastra first removes the traces produced by the experiment, together with their spans and trace-linked signals, and then removes the experiment.
