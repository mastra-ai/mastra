...
For custom middleware ordering, call each method separately instead of `init()`. See [Server Adapters: Manual initialization](/docs/server/server-adapters#manual-initialization) for details.
...
- [Express Adapter](/reference/server/express-adapter) - Alternative adapter
