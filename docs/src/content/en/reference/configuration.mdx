...
**Type:** `boolean | string[] | { preset?: 'all' | 'none'; include?: string[]; exclude?: string[] }`\
**Default:** `true`
...
- `object`: Combines a preset with per-package overrides
...
#### Composing presets and overrides

The boolean and array forms are all-or-nothing. Use the object form when you need both behaviors at once, such as a project that must externalize everything because of a native `.node` addon but still has one package that needs to be bundled.
...
<PropertiesTable
  content={[
  {
    ...
    isOptional: true,
    description:
      "Which dependencies to externalize before overrides are applied. `'all'` externalizes every non-workspace dependency, `'none'` bundles everything. Defaults to `'none'`.",
  },
  {
    name: 'include',
    type: 'string[]',
    isOptional: true,
    description:
      'Package names to always mark as external, in addition to the preset. Use this for packages that static analysis cannot detect, such as dynamically imported drivers.',
  },
  {
    name: 'exclude',
    type: 'string[]',
    isOptional: true,
    description:
      "Package names to bundle even when the preset would mark them as external. Cannot override Mastra's built-in runtime externals or anything named in `include`.",
  },
]}
/>
...
export const mastra = new Mastra({
  bundler: {
    externals: {
      preset: 'all',
      // Ships a broken ESM entry, so bundle it instead of installing it.
      exclude: ['broken-package'],
      // Imported dynamically at runtime, so the analyzer cannot see it.
      include: ['pg-native'],
    },
  },
})
...
`exclude` only overrides what `preset` would externalize. It never removes Mastra's own runtime externals, and `include` takes precedence when a package appears in both lists.

:::note

`exclude` applies to `mastra build` only. `mastra dev` never bundles packages from `node_modules`, so an excluded package is still loaded from `node_modules` while developing.
