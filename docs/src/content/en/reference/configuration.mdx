...
Whether the server generated by `mastra dev` and `mastra build` installs its own `SIGINT`/`SIGTERM` handlers. By default, receiving a signal stops the server from accepting new connections, drains in-flight requests, runs `mastra.shutdown()`, and exits the process (see [`server.drainTimeout`](#serverdraintimeout)).
...
- Your handler has no access to the HTTP server handle in the generated entry file, so it cannot drain in-flight HTTP requests. If you only need a longer drain window, prefer [`server.drainTimeout`](#serverdraintimeout). For full control over the server lifecycle, use a [server adapter](/docs/server/server-adapters).
