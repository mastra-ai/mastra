...
Enables and configures the background task manager. When enabled, agents can dispatch long-running tool calls (including sub-agent invocations) to run asynchronously while the agentic loop continues. Tasks are persisted, so a configured `storage` backend is required.
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'How long the agentic loop waits for a background task to complete before moving on. If a task has not finished within this time, the loop proceeds without setting isContinued. Default: undefined (wait indefinitely). Can be overridden per-agent or per-tool.',
  },
  {
    name: 'onTaskComplete',
