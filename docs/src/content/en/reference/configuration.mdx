...
For deployer-built production servers, leaving `server.host` and `MASTRA_HOST` unset preserves the runtime's default bind address. Set either value explicitly when you need a specific bind host such as `0.0.0.0`.
