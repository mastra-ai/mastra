...
| `openAPIDocs` | `boolean` | `false` | Enable OpenAPI specification at `/openapi.json` (also available at `/api/openapi.json`). Built-in Mastra routes use `servers: [{url: "/api"}]` while custom routes get a per-path `servers: [{url: "/"}]` override. |
