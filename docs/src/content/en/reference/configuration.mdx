...
Visit the [PubSub documentation](/docs/server/pubsub) to learn more.
...
Use `false` to disable the all-dependencies preset. The built-in global externals still apply. When you pass a package-name array to `mastra build`, the CLI retains those packages as runtime dependencies and keeps the all-dependencies preset enabled. The source-backed `bundler.externals` entry above describes these settings.
