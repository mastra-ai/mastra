...
- `string[]`: An array of package names to mark as external, the rest will be bundled together. These packages are always included in the output `package.json`, even if they are not directly imported in your code. This is useful for packages that are dynamically loaded at runtime (for example, pino transports)
