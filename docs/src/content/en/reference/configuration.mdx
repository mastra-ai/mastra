...
Controls which packages are excluded from the bundle (inside `.mastra/output`) and installed separately through a package manager.

### bundler.sourcemap

Enables source map generation for debugging bundled code inside `.mastra/output`.
