...
Increase this when rolling deploys should let long-running agent turns or workflow steps finish. The HTTP drain and the workflow drain run one after the other, so the worst case is twice this value plus 5 seconds of cleanup. Keep that total below your platform's grace period, such as `terminationGracePeriodSeconds` on Kubernetes:
