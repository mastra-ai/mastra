...
- **No background processes**: The provider does not expose a `processes` manager.
- **No stdin**: Runtime command execution does not provide an interactive stdin stream through this provider.
- **No mounts**: Workspace filesystem mounting is not supported by this provider.
