...
description: 'Abstract base class for managing background processes in sandboxes. Provides methods to spawn processes, list them, get handles by PID, and kill them.'
packages:
  - '@mastra/core'
---
      ...
      properties: [
        {
          ...
          parameters: [
            ...
            },
            {
              ...
              type: 'string',
              description:
                "Working directory for the process. Defaults to the sandbox's configured `workingDirectory`, then the provider default.",
              isOptional: true,
            },
            {
              name: 'onStdout',
            ...
            },
            {
              ...
              type: "'pipe' | 'ignore'",
              description:
                "How stdin is wired. `'pipe'` (default) opens a writable stdin for `sendStdin()` and `writer`. `'ignore'` closes stdin so commands that read it see immediate EOF. Honored by the local, Docker, and E2B providers.",
              isOptional: true,
            },
          ],
        },
