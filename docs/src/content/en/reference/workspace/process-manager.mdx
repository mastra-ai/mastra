...
Close the process's stdin to signal end-of-file (EOF). Use this after sending all input to commands that read until stdin closes. Throws if the process has already exited, stdin isn't available, or the sandbox provider doesn't support closing stdin.
...
Calling `handle.writer.end()` also closes stdin.
