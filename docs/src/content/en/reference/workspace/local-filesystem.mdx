...
Update the allowed paths at runtime. Accepts a direct array or an updater callback receiving the current paths (React setState pattern).
...
filesystem.setAllowedPaths(prev => [...prev, '/home/user/.ssh']);
...
Check whether an absolute path is within `basePath` or any allowed path.
...
fs.isPathAllowed('/project/src/index.ts');         // true (within basePath)
fs.isPathAllowed('/home/user/.config/app.json');   // true (within allowedPaths)
fs.isPathAllowed('/etc/passwd');                    // false
```
...
Assert that an absolute path is within `basePath` or any allowed path. Throws `PermissionError` if not.

```typescript
filesystem.assertPathAllowed('/project/src/index.ts'); // OK
filesystem.assertPathAllowed('/etc/passwd');            // throws PermissionError
```
