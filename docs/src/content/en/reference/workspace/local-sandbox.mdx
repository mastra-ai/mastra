...
description: "Executes commands on the local system. For interface details, see WorkspaceSandbox interface."
packages:
  - "@mastra/core"
---
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string[]',
    description:
      'Paths that commands can read but never write, even when nested inside the workspace or a `readWritePaths` entry. Mounted filesystems with `readOnly: true` are added automatically. A deny always wins: if the same directory is mounted both read-only and read-write, commands cannot write to it through either mount.',
    isOptional: true,
  },
  {
