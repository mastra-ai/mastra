...
`start()` reports `{ created: true }` when the working directory didn't exist yet and `{ created: false }` when it reattaches to an existing directory. See [`start()`](/reference/workspace/sandbox#start) for the shared contract.
