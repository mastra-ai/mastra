...
<PropertiesTable
  ...
  {
    name: 'bootstrap',
    type: '{ command: string; env?: Record<string, string>; timeoutMs?: number }',
    description:
      'Command run once per sandbox lifetime after start(). For LocalSandbox the guard sentinel (.mastra-bootstrapped) lives inside the working directory. See the WorkspaceSandbox reference for full semantics.',
    isOptional: true,
  },
]}
/>

`start()` reports `{ created: true }` when the working directory did not exist yet and `{ created: false }` when it reattaches to an existing directory. See [`start()`](/reference/workspace/sandbox#start) for the shared contract.
