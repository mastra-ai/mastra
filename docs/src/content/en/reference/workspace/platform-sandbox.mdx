...
Each command (clone, fetch, checkout, and every setup command) becomes its own build step, so a failed build names the command that failed and the steps before it stay cached. `setupCommand` accepts an array to run entries as separate steps. `workingDirectory` sets an absolute directory as the cwd for the build steps and for sandboxes created from the template; the repository is cloned relative to it, at `<workingDirectory>/<repo>`. Omit it to use the base image's working directory. Pass build-only environment variables, such as remote cache credentials for a build step, through `buildEnv`; they are sent as transient build values and never enter the serialized definition:
...
const sandbox = new PlatformSandbox({
  ...
  template: createRepoTemplate({
    getRepositoryAccess: async () => ({ cloneUrl: 'https://github.com/mastra-ai/mastra.git' }),
    setupCommand: ['pnpm install --frozen-lockfile', 'pnpm build'],
    workingDirectory: '/workspace',
    buildEnv: { TURBO_TOKEN: process.env.TURBO_TOKEN },
  }),
})
```
