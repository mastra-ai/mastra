...
By default, every template operation is serialized and sent to Platform, which interprets it according to the selected provider's capabilities. Use `setEnvs(values, { ephemeral: true })` for short-lived build credentials: these values are sent separately, excluded from content identity and persistence, unavailable at runtime, and take precedence over serialized environment values with the same key. Supply them again on every build or fresh provision that may need to build. On Railway, the provider cache includes the transient build variables, so rotating a value may trigger another provider build even though the Platform template ID stays stable.
