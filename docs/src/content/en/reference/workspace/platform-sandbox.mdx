...
The sandbox exec container and [`PlatformFilesystem`](/reference/workspace/platform-filesystem) are separate storage planes. Files written through `PlatformFilesystem` (bucket-backed) aren't visible to `executeCommand`, and files written by `executeCommand` aren't visible through `PlatformFilesystem`.
