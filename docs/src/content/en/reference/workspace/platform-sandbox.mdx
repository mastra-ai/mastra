...
Pass a reusable template builder to `PlatformSandbox`. A fresh sandbox start builds the template asynchronously, waits until it's ready, and then creates the sandbox from it:
...
`Template()` supports `runCmd`, `setWorkdir`, `setEnvs`, `aptInstall`, `pipInstall`, and `npmInstall`. Platform serializes the builder internally. It replays operations for E2B and converts them to equivalent Railway template operations.
...
<PropertiesTable
  ...
  {
    name: 'template',
    type: 'SandboxTemplateBuilder | SerializedSandboxTemplate | (() => SandboxTemplateBuilder | SerializedSandboxTemplate | undefined | Promise<SandboxTemplateBuilder | SerializedSandboxTemplate | undefined>)',
    description:
      'Reusable template or lazy resolver built before a fresh sandbox provision. Resolution and build failures fall back to the selected provider default.',
    isOptional: true,
  },
  {
