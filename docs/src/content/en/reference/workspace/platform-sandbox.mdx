...
`createRepoTemplate()` also attaches a commit-independent `lineageId` (`repo:<repoFullName>:<workdir>`) to the definition. Platform uses `lineageId` to find a previous ready template for the same repository and boot the new commit on that warm filesystem while the exact commit template builds in the background. Callers using the raw `Template()` builder can attach their own lineage key with `.withLineageId(id)` (any non-empty string up to 200 characters); omit it to opt out of stale-lineage fallback.
...
When a sandbox boots on a fallback template (either same-lineage stale or the provider base), the platform response includes a `templatePending` field that `PlatformSandbox` surfaces for observability:
