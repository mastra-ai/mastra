...
`start()` reports `{ created: false }` on reattach and `{ created: true }` on a fresh provision — including a checkpoint-recovered boot, which is a new VM even when its filesystem was restored. See [`start()`](/reference/workspace/sandbox#start) for the shared contract.
