...
Client for provisioning ephemeral sandboxes in a Mastra Platform environment. Each `PlatformSandbox` instance owns one remote sandbox: `start()` provisions it, `executeCommand()` runs against it, and `destroy()` tears it down. Agents typically construct many `PlatformSandbox` instances per session, one per isolated task, tenant, or background job, and let them go when they're done.

Sandboxes boot from a pre-built recipe checkpoint with Python 3, Node 22, TypeScript, tsx, and common build tooling already installed. Use `clone()` to derive a fleet of sandboxes from one configured template (see [Cloning](#cloning-for-a-fleet-of-sandboxes)), and pass a stable `id` to opt into [checkpoint recovery](#checkpoint-recovery) so a new sandbox boots from the previous one's filesystem.
...
Every `PlatformSandbox` has an `id` — either the one you pass to the constructor or an auto-generated one — that is sent to the platform on `POST /sandbox` as an advisory recovery key. The platform uses it to opt into checkpoint-based sandbox recovery:
...
- If the `id` isn't recognised, the platform falls through to a fresh sandbox from the base recipe. This is the default behavior when you don't pass an `id`.

Pass a stable `id` when you want a sandbox's filesystem to survive across sessions or across a `destroy()`/`start()` cycle:

```typescript
const sandbox = new PlatformSandbox({
  id: `project-${projectId}`, // stable across sessions
})
...
Use one stable `id` for each independent filesystem. Don't share an `id` across unrelated sessions, tenants, or projects — the platform would boot them from each other's checkpoint.
...
Use `clone()` when one configured `PlatformSandbox` acts as the template for a fleet of independent sandboxes, for example one per project, tenant, or task. Combine it with a stable `id` per clone to opt each clone into [checkpoint recovery](#checkpoint-recovery) independently.
