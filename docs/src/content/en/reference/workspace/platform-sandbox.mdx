...
`Template()` supports `runCmd`, `setWorkdir`, `setEnvs`, `aptInstall`, `pipInstall`, and `npmInstall`. Platform serializes the builder internally, computes the content hash, replays operations for E2B, and converts them to equivalent Railway template operations.
