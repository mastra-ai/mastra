...
- **`_start()`**: Creates a new sandbox from the provided baseImage. On subsequent calls, attempts to reconnect to an existing running sandbox, or creates a sandbox using the latest snapshot.
...
`ModalSandbox` includes a built-in process manager for spawning and managing background processes. Each `spawn()` call creates a new `ContainerProcess` via the Modal SDK's `Process.spawn()` API.
...
`sendStdin()` is not supported — the Modal JS SDK does not expose stdin on `Process.spawn()`.
