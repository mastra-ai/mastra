...
Sandbox providers can implement sync operations to transfer files between the workspace filesystem and the sandbox. Each provider implements its preferred transfer mechanism (file copy for local, HTTP upload for cloud, etc.).

#### `syncFromFilesystem(filesystem, paths?)`

Sync files from a workspace filesystem into the sandbox.

```typescript
const result = await sandbox.syncFromFilesystem?.(workspace.filesystem, ['/src']);
  ...
  content={[
    {
      ...
      description: "The workspace filesystem to sync from",
    },
    {
      name: "paths",
      type: "string[]",
      description: "Specific paths to sync (default: all files)",
      isOptional: true,
    },
  ]}
/>

#### `syncToFilesystem(filesystem, paths?)`

Sync files from the sandbox back to a workspace filesystem.

```typescript
const result = await sandbox.syncToFilesystem?.(workspace.filesystem, ['/output']);
```
  ...
  content={[
    {
      name: "filesystem",
      type: "WorkspaceFilesystem",
      description: "The workspace filesystem to sync to",
    },
    {
      name: "paths",
      type: "string[]",
...
**Returns:** `Promise<SandboxSyncResult>`

```typescript
interface SandboxSyncResult {
  synced: string[];
