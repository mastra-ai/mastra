...
Update the sandbox's runtime environment overlay. Overlay values are merged into every command and process executed through the sandbox, so credentials installed or rotated after the sandbox is created reach every subsequent command. Optional, so check for support or use optional chaining.

```typescript
sandbox.setEnv?.(env => ({ ...env, GH_TOKEN: token }))
```

The updater receives a copy of the current overlay and returns the new one, so a single call can set, unset, or batch-update variables. Sandboxes that extend `MastraSandbox` can seed the overlay with the `env` constructor option.
