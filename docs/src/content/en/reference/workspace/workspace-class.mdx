...
description: "The Workspace class combines a filesystem and sandbox to provide agents with file storage and command execution capabilities."
packages:
  - "@mastra/core"
---
    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            description:
              'Custom tracker for read-before-write records. Defaults to an in-memory tracker keyed by memory thread. See Read-before-write tracking below.',
            isOptional: true,
          },
        ],
      },
...
- **Global settings**: `enabled` and `requireApproval` apply to all workspace tools unless overridden. `hooks`, `writeLockTimeoutMs`, and `readTracker` are global-only and can't be configured per tool.
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean | ((context: ToolConfigWithArgsContext) => boolean | Promise<boolean>)',
    description:
      'For write tools, require the agent to read an existing file before changing it. See Read-before-write tracking below.',
    isOptional: true,
    defaultValue: 'false',
  },
  {
...
When `requireReadBeforeWrite` is enabled, read records are stored in a `FileReadTracker`. By default, records are kept in memory per memory thread, so they survive suspend/resume cycles and multiple turns within the same conversation. A file read before a tool suspends (for example, awaiting approval) doesn't need to be re-read after the run resumes. If a file changes on disk after it was read, the write is still rejected until the agent re-reads it, and a successful write always clears the record. Runs without a memory thread track reads per run.

The default tracker is in-memory, so records don't survive process restarts. On serverless runtimes where the process may be torn down between suspend and resume, set `tools.readTracker` to an implementation backed by your own storage. All `FileReadTracker` methods may return promises:

```typescript {7}
import { Workspace } from '@mastra/core/workspace'
import { myStorageBackedTracker } from './read-tracker'
