...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Sandbox provider instance, or a resolver function that receives `requestContext` and returns a sandbox per request. See [dynamic sandbox](/docs/workspace/sandbox#dynamic-sandboxes).',
    isOptional: true,
  },
  {
...
See [Filesystem tools](/docs/workspace/filesystem#agent-tools), [Sandbox tools](/docs/workspace/sandbox#agent-tools), [LSP inspection](/docs/workspace/lsp#tool-name-remapping), and [Search tools](/docs/workspace/search#agent-tools) for capability-specific configuration.
...
With a static sandbox, capability checks (`executeCommand`, `processes`) decide which tool variants are exposed. With a [runtime-defined sandbox](/docs/workspace/sandbox#dynamic-sandboxes), all sandbox tools are registered and the runtime throws a clear error if the resolved sandbox doesn't implement a requested capability.
