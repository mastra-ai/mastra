...
Get human-readable instructions describing the workspace environment. When a sandbox with mounts exists, each mount path is classified as sandbox-accessible or workspace-only based on actual mount state. Otherwise falls back to provider-level instructions.

When provider `instructions` are set to a function, the `requestContext` is passed through so instructions can vary per-request (e.g. by tenant or locale).
...
These instructions are automatically injected into the agent system message via `WorkspaceInstructionsProcessor`, so agents understand which paths are accessible in shell commands vs. file tools.
  ...
  content={[
    {
      ...
      type: "RequestContext",
      description: "Optional request context, forwarded to provider getInstructions() calls",
      isOptional: true,
    },
  ]}
/>

#### `getPathContext()` (deprecated)

:::caution
`getPathContext()` is deprecated. Use `getInstructions()` instead, which is mount-state-aware and feeds into the system message via `WorkspaceInstructionsProcessor`.
:::
