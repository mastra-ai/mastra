...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Filesystem provider instance, or a resolver function that receives `requestContext` and returns a filesystem per request. See [multi-tenant filesystems](/docs/workspace/filesystem#multi-tenant-filesystems).',
    isOptional: true,
  },
  {
    name: 'sandbox',
    ...
    description:
      'Sandbox provider instance, or a resolver function that receives `requestContext` and returns a sandbox per request. See [multi-tenant sandboxes](/docs/workspace/sandbox#multi-tenant-sandboxes).',
    isOptional: true,
  },
  {
...
- **Global defaults** (`enabled`, `requireApproval`): Apply to all workspace tools unless overridden
...
const workspace = new Workspace({
  id: 'my-workspace',
  tools: {
    enabled: ({ requestContext }) => requestContext['workspace-tools'] === true,
    [WORKSPACE_TOOLS.FILESYSTEM.WRITE_FILE]: {
...
See [Filesystem tools](/docs/workspace/filesystem#using-mounts), [Sandbox tools](/docs/workspace/sandbox#using-the-sandbox), [LSP inspection](/docs/workspace/lsp#tool-name-remapping), and [Search tools](/docs/workspace/search#agent-tools) for capability-specific configuration.
...
With a static filesystem, write tools (`write_file`, `edit_file`, `delete`, `mkdir`) are excluded when the filesystem is in read-only mode. With a [runtime-defined filesystem](/docs/workspace/filesystem#multi-tenant-filesystems), write tools are always included and read-only is enforced at runtime.
...
With a static sandbox, capability checks (`executeCommand`, `processes`) decide which tool variants are exposed. With a [runtime-defined sandbox](/docs/workspace/sandbox#multi-tenant-sandboxes), all sandbox tools are registered and the runtime throws a clear error if the resolved sandbox doesn't implement a requested capability.
...
See [Background processes](/docs/workspace/sandbox#background-processes) for callback examples.
