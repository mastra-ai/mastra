...
For a web client that displays one exact thread, pass that thread's ID to `AgentControllerSession.subscribe({ threadId, onEvent, reconnect: true })` in `@mastra/client-js`. Use a separate session scope for each open thread. The client calls the native session creation route with the exact thread before each stream connection, including reconnects. A session scope alone doesn't restore a thread binding after a server restart; without an explicit binding the server can select the resource's newest thread. Continue refreshing the native display and stored messages in `onReconnect`, because the stream doesn't replay missed events.
