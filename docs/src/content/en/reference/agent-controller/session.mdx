...
Return the current `AgentControllerDisplayState` snapshot for UI rendering. `threadId` identifies the thread represented by the snapshot and is `undefined` while the session is unbound.

```typescript
const displayState = session.displayState.get()
```

The session restores `tasks` from native thread storage when it binds or reopens a thread, independently of message history. New and cloned threads start with empty tasks and zero token usage. Keep historical task receipts in the transcript without writing them back into the current snapshot.

Read cumulative token totals from `tokenUsage`. A `usage_update` event describes one step, not the cumulative total.
