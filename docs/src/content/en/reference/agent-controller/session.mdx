...
### `session.thread.create({ title?, id?, requestContext? })`

Create a thread, bind the session to it, and open its event stream. Pass `requestContext` when memory is a function that reads the signed-in user.
...
### `session.thread.clone({ sourceThreadId?, title?, resourceId?, requestContext? })`

Clone an owned thread and its messages, then bind the session to the clone. Pass `requestContext` when memory is a function that reads the signed-in user.
...
### `session.thread.switch({ threadId, emitEvent?, requestContext? })`

Switch to an owned stored thread and hydrate its mode, model, and observational memory settings. Pass `requestContext` when memory is a function that reads the signed-in user.
