...
A `Session` is a live runtime object. Its event bus, arbitrary `session.state`, permission grants, pending approvals, suspensions, follow-ups, run state, and stream state don't automatically survive controller or process recreation.
