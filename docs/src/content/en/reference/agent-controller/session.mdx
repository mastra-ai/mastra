...
### `session.om.observer.model()` / `session.om.reflector.model()`

Return the role's configured model.

```typescript
const observerModel = session.om.observer.model()
const reflectorModel = session.om.reflector.model()
```

Returns: `'auto' | string | undefined`

`'auto'` follows the active main model. Any other string is an explicit model ID.
...
### `session.om.observer.switchModel({ model })` / `session.om.reflector.switchModel({ model })`

Switch one role between automatic and explicit selection. Pass `'auto'` to follow the active main model, or pass a concrete model ID to pin the role. Automatic selection clears the role's stale concrete model ID. The other role is unchanged. The emitted `om_model_changed` event continues to contain the effective concrete `modelId`.
