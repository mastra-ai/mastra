...
Subscribe to this session's isolated event bus. The method returns an unsubscribe function. A listener attaching while a run is in flight first receives that run's messages so far, then the current display state as `display_state_changed`; the display state is replayed on every subscribe.
