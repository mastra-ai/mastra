...
Returns a live toolset resolver over the connections available to a Mastra platform project. Pass it to an agent's dynamic `tools` argument: Mastra calls it on every generate/stream, and it serves a cached snapshot of one toolset per connected provider, revalidating from the platform every `ttlMs`, so connections attached after startup are picked up without a restart.
...
Returns: `ConnectTools` — a resolver producing `Record<string, ToolsInput>` keyed by provider (`linear`, `notion`, `jira`, `gitlab`, `snowflake`, `neon`, `cloudflare`, `resend`, `anthropic`). See [The resolver](#the-resolver).
...
The returned `ConnectTools` resolver caches its toolset snapshot and revalidates stale-while-revalidate: within `ttlMs` of the last fetch, resolutions return the cached snapshot; after that, the cached snapshot is served immediately while a background refresh runs. Connections attached to the project after startup become available within the TTL, and detached connections are dropped — no restart or redeploy required.
...
A listed provider with no connection in the project yet is skipped with a one-time warning and appears automatically once a connection is attached. Without `integrations`, every connected provider is included.
