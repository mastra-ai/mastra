...
#### `createSession({ resourceId?, id?, ownerId? })`
...
You can override the resource, session ID, and owner:
...
`resourceId` defaults to `config.resourceId` then `config.id`. `id` defaults to the effective resource ID. `ownerId` defaults to `config.id`. All three are stable for the life of the session — switching the resource ID via [`harness.setResourceId()`](#setresourceid-resourceid-) changes only the resource ID, not `id` or `ownerId`. Read them through [`session.identity.getId()`](/reference/harness/session#sessionidentitygetid) and [`session.identity.getOwnerId()`](/reference/harness/session#sessionidentitygetownerid).
