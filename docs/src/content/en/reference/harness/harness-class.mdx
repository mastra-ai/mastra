...
- **Toolsets inherited, recursive forks blocked.** Forks inherit the parent's toolsets (`ask_user`, `submit_plan`, user-configured harness tools, …) but the `subagent` tool itself is stripped from the fork's toolset to prevent unbounded recursive forking.
