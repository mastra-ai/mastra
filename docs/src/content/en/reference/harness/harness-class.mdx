...
<PropertiesTable
  ...
  {
    name: 'channels',
    type: 'HarnessChannelConfig | HarnessChannels',
    description:
      'Chat channel configuration that connects this Harness to messaging platforms such as Slack. Accepts the channel config directly (the Harness injects itself) or a pre-built `HarnessChannels`. When the Harness is registered on a `Mastra` instance, its channel webhook routes are registered automatically. See [HarnessChannels](/reference/harness/harness-channels).',
    isOptional: true,
  },
  {
    name: 'subagents',
...
Return the [`HarnessChannels`](/reference/harness/harness-channels) instance that connects this Harness to messaging platforms, or `undefined` when no `channels` were configured.
...
Responding to a pending tool approval is owned by the session — see [`session.respondToToolApproval()`](/reference/harness/session#tool-approvals). The harness owns the permission _policy_ that decides when approval is required, documented under [Permissions](#permissions) below.
