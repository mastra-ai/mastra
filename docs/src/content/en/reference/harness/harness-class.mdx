...
| `tool_suspended` | A tool paused via the native tool-suspension primitive (for example `ask_user` or `request_access`). Includes `toolCallId`, `toolName`, and `suspendPayload`. Resume it with [`respondToToolSuspension`](#respondtotoolsuspension-resumedata-toolcallid-requestcontext-). |
