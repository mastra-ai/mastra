...
```typescript
await harness.setState({ currentModelId: '__ANTHROPIC_MODEL_SONNET__' })
```
...
await harness.switchModel({ modelId: '__ANTHROPIC_MODEL_SONNET__' })

// Persist to the current thread
await harness.switchModel({ modelId: '__ANTHROPIC_MODEL_SONNET__', scope: 'thread' })
```
...
```typescript
await harness.switchObserverModel({ modelId: '__ANTHROPIC_MODEL_HAIKU__' })
```
...
```typescript
await harness.switchReflectorModel({ modelId: '__ANTHROPIC_MODEL_HAIKU__' })
```
...
await harness.setSubagentModelId({ modelId: '__ANTHROPIC_MODEL_SONNET__' })

// Set per-type model
await harness.setSubagentModelId({ modelId: '__ANTHROPIC_MODEL_HAIKU__', agentType: 'explore' })
```
