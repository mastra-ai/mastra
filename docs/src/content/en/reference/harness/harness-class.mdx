...
When a granted tool pauses for approval, the harness waits for the suspended source stream to finish before resuming the run. This ensures the workflow snapshot is persisted before `approveToolCall()` is triggered.
