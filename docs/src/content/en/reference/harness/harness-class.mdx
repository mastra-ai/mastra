...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'Session',
    description:
      'The per-conversation session object. Owns runtime state such as session-scoped permission grants, token usage, the active mode/model, follow-up queue, tool-approval gate, and the live agent stream subscription. Access session-level APIs through `harness.session` — for example, `harness.session.respondToToolApproval()` and `harness.session.getGrants()`.',
  },
]}
/>
...
The `memory` property exposes thread management operations as convenience aliases. `memory.createThread`, `memory.switchThread`, `memory.listThreads`, and `memory.renameThread` delegate to the corresponding top-level Harness methods documented above.
...
#### `session.respondToToolApproval({ decision, requestContext? })`

Respond to a pending tool approval request. Called when a `tool_approval_required` event is received. Accepts `'approve'`, `'decline'`, or `'always_allow_category'`. When `always_allow_category` is used, the tool's category is granted for the rest of the session — future tools in the same category are auto-approved without prompting.

```typescript
harness.session.respondToToolApproval({ decision: 'approve' })
harness.session.respondToToolApproval({ decision: 'decline' })

// Grant the tool's entire category for this session
...
Permission grants live on the `session` object. They control which tools and categories are auto-approved for the current session.

#### `session.grantCategory(category)`
...
| `display_state_changed` | The canonical `HarnessDisplayState` snapshot changed. Use [`subscribeDisplayState`](#subscribedisplaystatelistener-options) to receive coalesced snapshots. |
