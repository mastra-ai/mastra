...
#### `send({ content, requestContext? })`

Send a message using the current message delivery mode. When the agent is idle, sends immediately. When the agent is running, either interrupts or queues based on `getMessageDeliveryMode()`.

```typescript
// Sends immediately (agent idle), interrupts (interrupt mode), or queues (queue mode)
await harness.send({ content: 'Do this next' })
```

#### `steer({ content, requestContext? })`

Steer the agent mid-stream: aborts the current generation and sends a new message. Clears any queued follow-ups.

```typescript
await harness.steer({ content: 'Focus on security implications' })
```
...
```typescript
await harness.followUp({ content: 'Now apply those changes' })
```
...
```typescript
const mode = harness.getMessageDeliveryMode()
// 'interrupt' | 'queue'
```

#### `setMessageDeliveryMode({ mode })`

Set how new messages are handled while the agent is running. The mode controls the behavior of `send()`:

- `'interrupt'` — abort the current generation and send immediately.
- `'queue'` — queue the message for processing after the current generation completes (default).

```typescript
// Switch to interrupt mode so new messages abort the current generation
harness.setMessageDeliveryMode({ mode: 'interrupt' })

// Switch back to queue mode (default)
...
| `message_delivery_mode_changed` | The message delivery mode changed (`'interrupt'` or `'queue'`). |
