...
By default, the command uses a default starter project. Use `--llm <provider>` to select OpenAI, Anthropic, Google Gemini, or xAI, `--template [template]` to use an arbitrary template, or `--empty` to create a minimal provider-free scaffold.
