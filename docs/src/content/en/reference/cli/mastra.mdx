...
mastra api tool execute weather '{"params":{"city":"San Francisco"}}'
```
...
Set the request timeout in milliseconds. Defaults to `30000`.
...
```bash
mastra api workflow run start data-pipeline '{"input":{"source":"s3://bucket/data.csv"}}'
```

Use positional arguments for stable IDs and JSON for filters or payloads. List commands accept `page` and `perPage` in the JSON input when the target route supports pagination:
...
```bash
mastra api memory search '{"query":"caching strategy"}'
```
...
- `mastra api thread delete <threadId>`
