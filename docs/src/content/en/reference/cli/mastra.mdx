...
mastra env db create production --kind postgres                      # managed Postgres in the "production" environment
```

#### `--kind`

Database provider (required). One of `turso`, `neon`, `redis`, or `postgres`.

`neon` and `postgres` are both PostgreSQL but differ in hosting and injected variables. `neon` provisions serverless Postgres and injects `DATABASE_URL`. `postgres` provisions a managed Postgres instance that runs alongside your environment on its private network and injects `POSTGRES_URL`. Both providers can coexist in the same environment, since they inject different variables.

#### `--name`

Database name. Defaults to a name derived from the project slug and provider (for example `my-app-turso` or `my-app-postgres`).

#### `--region`

Provider region ID for shared databases. Ignored for environment-scoped databases, which always use the environment's region. `redis` and `postgres` databases run in the environment itself, so they never take a region.

#### `--shared`

Attach as a project-scoped database that's shared by every environment. Can't be combined with an environment argument, and isn't supported for `postgres`: managed Postgres is always environment-scoped.
...
See [Import existing traces](/docs/mastra-platform/observability#import-existing-traces) for the complete workflow, data mapping, local-file lifecycle, and limits.
...
Pass a non-null `page.next` value back as `page.after` in the next query. See [Advanced trace queries](/reference/observability/tracing/trace-query) for supported fields and operators, recursive predicates, limits, pagination, and errors.
...
You can call the hosted observability API directly with your platform access token and project ID. The examples below use the United States host. Substitute `https://observability.eu.mastra.ai` for a European Union environment. The [Feedback API](/docs/mastra-platform/api#feedback-api) documents feedback endpoints that don't have CLI commands:
