...
mastra api trace query '{"timeRange":{"from":"2026-08-01T00:00:00.000Z","to":"2026-08-08T00:00:00.000Z"},"pagination":{"page":0,"perPage":25}}'
```
...
The response remains nested under `data`. Keyset mode returns cursor metadata:
...
Pass a non-null `data.page.next` value back as `page.after` in the next query. List-compatible page mode returns numeric pagination metadata instead:
...
`data.page.next` is only present in keyset mode. Page mode returns `data.pagination`. See [Advanced trace queries](/reference/observability/tracing/trace-query) for supported fields and operators, recursive predicates, limits, pagination modes, and errors.
