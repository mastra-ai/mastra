...
Restarts a paused server instance for the linked project.

### Flags

Same flags as [`mastra server pause`](#mastra-server-pause): **`--project`** and **`-c` / `--config`** only, with the same defaults and behavior.
