...
You can call the hosted observability API directly with your platform access token and project ID. The examples below use the United States host. Substitute `https://observability.eu.mastra.ai` for a European Union environment. The [HTTP Feedback API](/docs/mastra-platform/api#feedback-api) documents feedback endpoints that don't have CLI commands:
