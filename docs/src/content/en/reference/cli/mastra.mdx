...
A project provisioned by `create-factory` contains a non-secret `.mastra-project.json` file. From that project directory, the CLI uses the file to find the hosted Factory instance and applies your Mastra CLI credentials automatically. Pass `--url` to target a specific local, remote, or self-hosted Factory server instead:
