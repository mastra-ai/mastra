...
After you deploy the project with `mastra deploy`, the resulting non-secret `.mastra-project.json` lets the CLI find the hosted Factory instance, apply your Mastra CLI credentials, and select the deployed project's organization automatically. For an explicit hosted Factory `--url`, the CLI uses the organization selected by `mastra auth orgs switch`. An explicit `X-Mastra-Organization-Id` header takes precedence. Before deployment, or to target a specific local, remote, or self-hosted Factory server, pass `--url` instead:
