...
Database name. Defaults to a name derived from the project slug and provider (for example `my-app-turso` or `my-app-pg`).
...
Omitting `after` establishes a cursor and returns no historical traces. Changing the time range or predicate requires a fresh numbered request. See [Delta polling limitations](/reference/observability/tracing/trace-query#poll-after-loading-a-numbered-page) before using this workflow to maintain a local trace list.
