...
Starts a server which exposes [Studio](/docs/v0/getting-started/studio) and REST endpoints for your agents, tools, and workflows. You can visit [http://localhost:4111/swagger-ui](http://localhost:4111/swagger-ui) for an overview of all available endpoints once `mastra dev` is running.

You can also [configure the server](/docs/v0/getting-started/studio#configuration).
...
Enable local HTTPS support. [Learn more](/docs/v0/getting-started/studio#local-https).
...
If you're deploying to a [serverless platform](/docs/v0/deployment/cloud-providers) you need to install the correct deployer in order to receive the correct output in `.mastra`.
...
Starts a local server to serve your built Mastra application in production mode. By default, [OTEL Tracing](/docs/v0/observability/ai-tracing/overview) is enabled.
...
Disable the [OTEL Tracing](/docs/v0/observability/ai-tracing/overview).
...
Read the [Scorers overview](/docs/v0/scorers/overview) to learn more.
