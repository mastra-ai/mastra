...
Restarts a paused server instance for the linked project. After the platform accepts the restart, the CLI resolves the deploy id (from the API response or by polling project metadata until `latestDeployId` updates), then streams build and deploy logs the same way as [`mastra server deploy`](#mastra-server-deploy) until the deploy reaches a terminal state.
...
Fails if a deployment is still active (running, building, deploying, etc.).
