...
Builds a standalone companion worker that runs experiments without exposing an HTTP server. The worker loads your exported `Mastra` instance, accepts versioned newline-delimited JSON (NDJSON) protocol messages on standard input, and writes protocol events to standard output.
...
- The CLI version, creation time, and stable build ID embedded in the executable.
...
The content digest excludes `experiment-worker-manifest.json` to avoid a self-referential digest. Package the complete directory, including the manifest, and use an outer package digest when the manifest itself must be attested.
