...
After you deploy the project with `mastra deploy`, the resulting non-secret `.mastra-project.json` lets the CLI find the hosted Factory instance and apply your Mastra CLI credentials automatically. Before deployment, or to target a specific local, remote, or self-hosted Factory server, pass `--url` instead:
