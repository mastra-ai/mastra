...
`mastra studio deploy` continues to work but is superseded by [`mastra deploy`](#mastra-deploy), which supports named environments (`--env staging`, `--env production`) on a single project. New setups should use `mastra deploy`.
...
`mastra server deploy` continues to work but is superseded by [`mastra deploy`](#mastra-deploy), which deploys a single project to multiple named environments instead of separate Studio and Server commands. New setups should use `mastra deploy`.
...
```bash npm2yarn
npx mastra@latest create my-mastra-project --yes
```

By default, the command creates the managed agent harness. Use `--llm <provider>` to select OpenAI, Anthropic, Google Gemini, or xAI, `--template [template]` to use an arbitrary template, or `--empty` to create a minimal provider-free scaffold.
...
`mastra create` accepts the positional `[project-name]` argument and these options: `--yes`, `--empty`, `-l, --llm <provider>`, `-k, --llm-api-key <key>`, `--no-skills`, `--no-git`, `-t, --template [template]`, and `--timeout <milliseconds>`. See the [`create-mastra` reference](/reference/cli/create-mastra) for mode behavior, conflicts, validation, and complete flag descriptions.
