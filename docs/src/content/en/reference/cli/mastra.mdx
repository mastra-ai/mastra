...
Localhost requests do not send auth headers. Platform requests use the saved Mastra credentials or `MASTRA_API_TOKEN`. Explicit `--url` requests only send headers provided with `--header`.
...
```bash
mastra api agent run --schema --pretty
mastra api tool execute --schema --pretty
```
...
Run an agent with a text prompt:

```bash
mastra api agent run weather-agent '{"messages":"What is the weather in London?"}'
```

Run an agent with chat messages:

```bash
mastra api agent run weather-agent '{"messages":[{"role":"user","content":"What is the weather in London?"}]}'
```

Execute a tool with raw tool input. The CLI sends the input as the route `data` field. You can also pass an explicit `{"data":{...}}` wrapper.
...
Read and update current working memory. Updates require working memory to be enabled for the memory instance.

```bash
mastra api memory current get '{"threadId":"thread_abc123","agentId":"code-reviewer"}'
mastra api memory current update '{"threadId":"thread_abc123","agentId":"code-reviewer","resourceId":"user_123","workingMemory":"Remember the user prefers concise responses."}'
```

Create, update, and delete memory threads:

```bash
mastra api thread create '{"agentId":"weather-agent","resourceId":"user_123","threadId":"thread_abc123","title":"Support conversation"}'
mastra api thread update thread_abc123 '{"agentId":"weather-agent","resourceId":"user_123","title":"Updated title"}'
mastra api thread delete thread_abc123 '{"agentId":"weather-agent","resourceId":"user_123"}'
```

Inspect memory status and logs:
...
Create, list, and get observability scores:
...
mastra api score get score_123
```

Resume a workflow run only when the run is suspended:

```bash
mastra api workflow run resume weather-workflow run_123 '{"resumeData":{"approved":true}}'
```
