...
| Error processors | `errorProcessorDefaults: false` |
...
Specific network-reset and bad-request policies take precedence over the unknown-error policy. Passing `errorProcessors` sets the base list: any of the three shared defaults your list doesn't name by `id` is still added, at the position its `id` gives it. An empty list gets all three. Setting `errorProcessorDefaults: false` keeps your list exactly as given, adding none of the shared defaults. Without a list, the agent then runs no error processors. A `StreamErrorRetryProcessor` added this way uses the shared defaults, so it retries neither unknown errors nor bad-request (HTTP 400) responses. Supply your own instance in the list to keep either policy. `PrefillErrorHandler` and `ProviderHistoryCompat` are also included for provider compatibility: both repair errors caused by the shape of the message history, such as a provider rejecting a conversation that ends with an assistant message.
