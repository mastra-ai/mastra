...
Specific network-reset and bad-request policies take precedence over the unknown-error policy. Passing `errorProcessors` sets the base list: any of the three shared defaults your list doesn't name by `id` is still added, at the position its `id` gives it, and `errorProcessors: []` runs with none of them. A `StreamErrorRetryProcessor` added this way uses the shared defaults, so it does not retry unknown errors — supply your own instance in the list to keep the unknown-error policy. `PrefillErrorHandler` and `ProviderHistoryCompat` are also included for provider compatibility: both repair errors caused by the shape of the message history, such as a provider rejecting a conversation that ends with an assistant message.
