...
description: "API reference for createCodingAgent(), which builds a coding agent with portable defaults for workspace, task signal, error retries, and goal judge."
packages:
  - "@mastra/core"
---
...
Pass a model and instructions. `createCodingAgent()` fills in the workspace, task signal, error processors, and goal prompt:
...
`createCodingAgent()` accepts every field of [`AgentConfig`](/reference/agents/agent#constructor-parameters) plus the fields below. Fields you provide always take precedence over the defaults.
...
A default is only filled in when you don't provide the corresponding field. Caller-provided values always win.
