...
Code-defined workflows that use [`.agent()`](/reference/workflows/workflow-methods/agent), [`.classifier()`](/reference/workflows/workflow-methods/classifier), and [`.tool()`](/reference/workflows/workflow-methods/tool) produce the same declarative entries when serialized.
...
If `state` is omitted, the classifier evaluates the complete entry input. A state path can start with `inputData`, `initData`, `state`, or `stepResults`. Live selector functions are omitted from the serialized graph, so use a path mapping when the workflow will be stored and rehydrated.

Classifier output has three stable paths:

- `values.<question>` contains the routing-oriented choice literal, numeric score, or boolean `P(true)` probability.
- `answers.<question>` contains complete classifier evidence, including distributions when provided.
- `usage` contains normalized token usage.

Use an existing `conditional` entry to route on classifier output. For example, a following condition can compare `{ "path": "stepResults.classify-ticket.values.route" }` with `{ "literal": "billing" }`.
