...
If `state` is omitted, the classifier evaluates the complete entry input. A state path can start with `inputData`, `initData`, `state`, or `stepResults`. Live selector functions aren't JSON-serializable and are rejected when a code-defined workflow is stored.
