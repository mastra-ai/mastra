...
A dynamic workflow definition is a JSON-compatible `StoredWorkflowGraph` accepted by [`Mastra.addStoredWorkflow()`](/reference/core/addStoredWorkflow), the stored-workflow server routes, and the Client SDK workflows API.
