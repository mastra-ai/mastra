...
If you need to run multiple operations per item, use a nested workflow as the step. This keeps all operations for each item together and is cleaner than chaining multiple `.foreach()` calls. See [Nested workflows inside foreach](/guides/workflows/control-flow#nested-workflows-inside-foreach) for examples.
...
- [Looping with foreach](/guides/workflows/control-flow#looping-with-foreach)
