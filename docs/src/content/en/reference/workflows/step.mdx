...
- `values.route` is the selected choice literal.
- `values.urgent` is the probability that the boolean answer is `true`, not a thresholded boolean.
- `answers` contains the complete typed evidence, including distributions when provided.
...
A classifier without constructor-configured questions can't be used as a workflow step. A function-based `state` selector works in the live workflow but is omitted from the serialized graph. Use a path such as `{ path: 'inputData.message' }` when the workflow will be stored and rehydrated.
