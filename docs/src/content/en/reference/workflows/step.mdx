...
A classifier without constructor-configured questions can't be used as a workflow step. A function-based `state` selector works in a live workflow, but can't be serialized; use a path such as `{ path: 'inputData.message' }` for a storable workflow definition.
