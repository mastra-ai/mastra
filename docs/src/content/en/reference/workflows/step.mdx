...
const authenticatedStep = createStep({
  id: "authenticated-step",
  description: "A step that requires user authentication",
  inputSchema: z.object({
    query: z.string(),
  }),
  outputSchema: z.object({
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: "z.ZodType<any>",
      description:
        "Optional Zod schema defining the expected structure of request context values for this step. When provided, the framework validates that the required context is present before step execution. The validated requestContext is passed to the execute function and can be accessed via the `requestContext` parameter.",
      required: false,
    },
    {
      name: "execute",
