...
Returns the output for a step ID, including nested workflow dot paths such as `parent.child`.
...
Returns the payload that was passed to a step.
...
Returns the first suspended step from the workflow state.
...
<PropertiesTable
  ...
  {
    name: 'getStepOutput',
    type: '(stepId: string) => any | undefined',
