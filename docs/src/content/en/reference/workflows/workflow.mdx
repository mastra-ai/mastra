    ...
    properties: [
      {
        ...
        parameters: [
          ...
          {
            name: 'deleteSnapshotOnFinish',
            type: 'boolean',
            description:
              'Optional flag to delete the run row once the run reaches a terminal state that shouldPersistSnapshot opted out of persisting. Use it for runs that are never resumed or inspected after they finish, so frequently scheduled workflows do not accumulate rows.',
            isOptional: true,
            defaultValue: 'false',
          },
          {
