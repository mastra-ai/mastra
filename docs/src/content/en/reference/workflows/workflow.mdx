...
### With Request Context Validation

```typescript title="src/mastra/workflows/authenticated-workflow.ts"
  ...
  .then(
    createStep({
      ...
      execute: async ({ inputData, requestContext }) => {
        // Access validated request context in any step
        const userId = requestContext.get('userId');
        const orgId = requestContext.organizationId;
        console.log(`Processing for user ${userId} in org ${orgId}`);

        return {
          result: `Processed: ${inputData.text}`,
        };
      },
    })
  )
...
// When starting the workflow, the framework validates the requestContext
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: "z.ZodType<any>",
      description:
        "Optional Zod schema defining the expected structure of request context values. When provided, the framework validates that the required context is present before workflow execution. The validated requestContext is automatically passed to all steps in the workflow and can be accessed via the `requestContext` parameter in step execute functions.",
      isOptional: true,
    },
  ]}
/>
