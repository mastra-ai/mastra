  ...
  content={[
    {
      ...
      description:
        "A promise that resolves with { message: 'Workflow run cancelled' } when cancellation succeeds",
    },
  ]}
/>
...
**Note:** Steps must actively check the abort signal to be cancelled mid-execution. Steps that don't check the signal will run to completion, but subsequent steps won't execute.
