...
## Example with Inngest cron trigger

When using `startAsync()` with Inngest scheduled workflows, wrap the trigger in a memoized step to prevent duplicate executions:
...
inngest.createFunction(
  ...
  async ({ step }) => {
    // Step 1: Create run (memoized)
    const runId = await step.run('create-run', async () => {
      const run = await workflow.createRun();
      return run.runId;
    });

    // Step 2: Trigger workflow if not already running
    await step.run('trigger', async () => {
      const status = await workflow.getWorkflowRunExecutionResult(runId);
    ...
    // Step 3: Poll for completion (separate from trigger)
    const result = await step.run('wait', async () => {
      // Poll workflow.getWorkflowRunExecutionResult(runId)
      // until status is 'success', 'failed', or 'canceled'
    });
...
- [Inngest workflows](/docs/workflows/inngest-workflow)
