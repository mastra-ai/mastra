...
Sends Tracing data to Laminar via OTLP/HTTP (protobuf). Also supports attaching scorer results to traces/spans in Laminar via `addScoreToTrace()`.
...
Attaches a score to a Laminar trace/span.
