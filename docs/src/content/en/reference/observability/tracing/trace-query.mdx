...
PostgreSQL and ClickHouse stop an advanced trace query after 15 seconds by default. Set `traceQueryTimeoutMs` in the store's vNext observability configuration to an integer from 1 through 300,000 milliseconds to change the timeout.
...
| `504` | The query exceeded the configured database execution timeout. |
