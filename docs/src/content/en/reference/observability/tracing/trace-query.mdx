...
Retain `after` and call `poll()` again at your application's polling interval. Merge results by `traceId`: a trace can appear in the initial page and a later batch, or in multiple batches. Delta polling doesn't load historical traces omitted from the initial numbered page. Load additional numbered pages if you need that history.
...
Delta ordering follows the storage ingestion watermark, not `startedAt` or `endedAt`; matching timestamps don't make the cursor ambiguous. Don't pass `orderBy` in delta mode. Keyset and delta cursors aren't interchangeable, and delta cursors can't move between storage adapters.
...
ClickHouse's delta index retains two days of events and doesn't backfill historical rows. Reload numbered pages after a polling interruption longer than this retention window. Delta polling isn't a durable change feed.
