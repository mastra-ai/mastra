...
The `group` option remains supported until the next major release. Use `queryThreads()` to retrieve matching thread identities in new code.
...
| `group` | No | **Deprecated.** Set to `{ by: ['threadId'] }` to return distinct non-null thread IDs. Use `queryThreads()` for new code. |
...
A grouped query still returns distinct non-null thread IDs in ascending order. Grouping remains supported until the next major release, but use `queryThreads()` for new code:
...
// Replacement
const result = await mastraClient.queryThreads({
  traces: {
    timeRange,
  },
})
...
Related evidence isn't embedded in these responses. Use the trace-detail and branch APIs to load spans after selecting a result.
