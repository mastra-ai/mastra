...
| `orderBy` | No | One item ordering ungrouped results by `startedAt`, `endedAt`, or `modelCost`, in `asc` or `desc` order. Defaults to `startedAt desc`. Not accepted with `group` or in delta mode. |
...
| `include` | No | Optional projections. Set `{ tableSummary: true }` to return the bounded Traces-table fields on every trace row. Not accepted with `group`, and the page can't exceed 100 traces. |
...
| `page.limit` | 1,000 |
| `page.limit`, `pagination.perPage`, or delta `limit` with `include.tableSummary` | 100 |
| Feedback and score summaries per trace with `include.tableSummary` | 10 each |
...
| Trace | `startedAt`, `endedAt`, `durationMs`, `modelCost` | `eq`, `ne`, `in`, `notIn`, `lt`, `lte`, `gt`, `gte`, `exists`, `notExists` |
...
### Filter and order by model cost

At trace scope, `modelCost` is the complete cost, in USD, of every model call in the trace. The store calculates it from the trace's model-usage metric events before filtering, ordering, and pagination, so a page never contains a partial figure:

```typescript
const expensiveLowFactuality = {
  timeRange,
  where: {
    op: 'and',
    args: [
...
Each model call contributes the cost recorded on its total-token metrics, `mastra_model_total_input_tokens` and `mastra_model_total_output_tokens`. Detail metrics such as cache or reasoning tokens price the same tokens again, so they don't count. Retried exports of the same metric event count once. A gateway-reported cost counts once through its carrier metric.

The cost is unavailable when any model call in the trace is unpriced, records a cost error such as a missing model price or a partial cost, reports a negative or non-finite value, or is priced in a unit other than USD. A trace without model metrics also has no cost. An unavailable cost never becomes `0`: it satisfies `notExists`, `ne`, and `notIn`, fails every other operator, and sorts after every priced trace in both `asc` and `desc` order. Ties break on `traceId`.

Stores advertise support through the `trace-query-model-cost` feature. Requests that filter or order by `modelCost` against a store without it return `501`.
...
### Include Traces-table fields

Set `include: { tableSummary: true }` to add a `tableSummary` object to every trace row. The default response stays lightweight, and the projection never changes which traces match, their order, cursors, or totals. The store selects the page first and then enriches only those traces, so a trace always produces one row regardless of how many spans, feedback records, or scores it has.
...
{
  "traceId": "trace-123",
  "inputPreview": "Is this medication safe with…",
  "tableSummary": {
...
| - | - |
| `outputPreview` | Short text from the current root span's output, truncated like `inputPreview`. `null` when the root has no output. |
| `tags` | The current root span's tags. Empty when none are stored. |
| `model` | The `model` attribute of the trace's earliest current `model_generation` span. `null` when the trace made no model call or that span has no `model` attribute. |
| `timeToFirstTokenMs` | Milliseconds from that model span's start to its recorded first-token time. `null` when the span has no first-token time. |
| `errorCounts` | Current spans with an error: `total` counts every span including the root, `llm` counts model spans, and `tool` counts tool-call spans. |
| `promptCacheReadTokens`, `promptCacheCreationTokens` | Sums of the auto-extracted `mastra_model_input_cache_read_tokens` and `mastra_model_input_cache_write_tokens` metrics recorded on the trace's current spans. `null` when no metric row exists. `0` is a recorded value. |
| `modelCost` | The complete model cost in USD, calculated as described in [Filter and order by model cost](#filter-and-order-by-model-cost). `null` when the cost is unavailable. `0` is a recorded value. |
| `feedback`, `feedbackTruncated` | Up to 10 current feedback records, newest first with `feedbackId` as the tie-breaker. `feedbackTruncated` is `true` when the trace has more. |
| `scores`, `scoresTruncated` | Up to 10 current score records, newest first with `scoreId` as the tie-breaker. Each `scoreId` contributes its latest write only. `scoresTruncated` is `true` when the trace has more. |

A truncated list is a bounded view, not evidence of absence, so a scorer missing from a truncated `scores` list may still have scored the trace. The summaries also don't depend on the `where` predicate that selected the trace.

When the configured store doesn't advertise the `trace-query-table-summary` feature, requests that set `include.tableSummary` return `501`. When the enrichment exceeds the execution timeout, the whole request fails instead of returning partial rows.
