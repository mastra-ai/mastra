...
`queryTraces({ group: { by: ['threadId'] } })` remains available for compatibility, but it's deprecated and will be removed in the next major release. Use `queryTraceThreads()` for new thread-grouping queries. Legacy grouped trace queries return `{ groups: [{ threadId }], page }` and don't support thread-level predicates.
...
Cursors are bound to the operation, accepted normalized query, authorization state, and ordering. Reusing a cursor after changing the trace selection, predicates, grouping, or ordering returns `409`. Trace, legacy group, and thread cursors aren't interchangeable. A malformed cursor returns `400`.
...
Both operations consider completed traces only. They don't support running traces, custom projections, embedded evidence, summaries, aggregations, counts, measures, or grouping by fields other than `threadId`.
