...
A truncated list is a bounded view, not evidence of absence: a scorer missing from a truncated `scores` list may still have scored the trace. Score and feedback summaries are independent of the `where` predicate that selected the trace. Stores that don't advertise the `trace-query-table-summary` feature return `501` for requests that set `include.tableSummary`, and an enrichment that exceeds the execution timeout fails the whole request rather than returning partial rows.
