...
String comparisons are exact and case-sensitive. The planner rejects a string where a number is required, including for `durationMs`, and accepts only ISO timestamp strings for timestamp predicates. A missing field satisfies neither positive nor ordered predicates, although the negative operators `ne` and `notIn` do match it. Combine a negative predicate with `exists` when the field must also be present.
