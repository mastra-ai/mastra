...
ClickHouse discovery requests default to a 5-second timeout and a 256 MiB per-query memory limit. Configure these with `traceQueryDiscoveryTimeoutMs` and `traceQueryDiscoveryMemoryLimitBytes`. If only `traceQueryTimeoutMs` is configured, discovery uses that timeout instead of the 5-second default. These settings don't change ordinary `queryTraces()` execution limits.
...
| `501` | The configured observability store doesn't support the requested operation. Discovery returns `TRACE_QUERY_DISCOVERY_UNSUPPORTED` unless the store implements both field and value discovery. |
