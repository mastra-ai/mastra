...
`resourceId` is optional and narrows within the organization. Keyset cursors are bound to the scope, so a cursor minted under one scope and reused under another returns `409` before storage runs. Callers can't name `organizationId` or `projectId` in predicates. Without a scope the routes behave as before, so self-hosted installations keep their own tenant model.
