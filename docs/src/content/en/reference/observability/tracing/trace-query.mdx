...
Canonical fields come from the same registry used to validate trace queries. They don't consume `limit`, and their order follows that registry. `observedFields` contains executable scalar metadata paths found on qualifying current root spans. Discovery walks nested objects, but never arrays. It returns a dotted string such as `metadata.customer.id` when no segment contains a literal dot. When an exact segment contains a literal dot, it returns an array such as `['metadata', 'customer.id']` so the path can't be confused with nested `customer.id` metadata.

Each observed field reports the scalar kinds found at that path. `valueKind` is `string`, `number`, or `boolean` when every observed value has that type. It's `scalar` when the path contains multiple scalar types. Numeric-only fields include `lt`, `lte`, `gt`, and `gte`; all other metadata fields include equality, membership, and presence operators only. Fields are ordered by `occurrences` descending, then by their serialized path. Nulls, objects, arrays, empty keys, and oversized paths are omitted.
...
Metadata paths return typed string, number, and boolean suggestions without trimming or coercion. Empty and whitespace-only strings are preserved. Missing paths, nulls, objects, arrays, and oversized strings are omitted. Values are ordered by `count` descending, then deterministically by value. Suggestions are advisory: `queryTraces()` continues to accept valid manual literals that discovery doesn't return.
...
| Predicate path, after metadata segments are joined with `.` | 128 UTF-8 bytes |
| Metadata path segments, including `metadata` | 12 |
| Individual metadata path segment | 128 UTF-8 bytes |
| `page.limit` | 1,000 |
...
| Trace metadata strings and booleans | `metadata.customer.id` or `['metadata', 'customer.id']` | `eq`, `ne`, `in`, `notIn`, `exists`, `notExists` |
| Trace metadata numbers | `metadata.retry.count` or `['metadata', 'retry.count']` | `eq`, `ne`, `in`, `notIn`, `lt`, `lte`, `gt`, `gte`, `exists`, `notExists` |
...
String comparisons are case-sensitive, and literals are never coerced. Canonical string fields and metadata strings compare exact stored values. Metadata predicates preserve empty and whitespace-only strings. Numeric predicates, including metadata numbers, `score`, and `durationMs`, require finite JSON numbers. Timestamp predicates require ISO timestamp strings. A missing or unsupported value satisfies neither positive nor ordered predicates, although it does satisfy the negative operators `ne` and `notIn`. Combine a negative predicate with `exists` when the field must also be present.
...
Use dot notation for top-level and nested string, number, or boolean leaves in the current root span's metadata:

```typescript
const reviewedRetries = {
  op: 'and',
  args: [
    { op: 'eq', left: { path: 'metadata.customer.id' }, right: { literal: '123a' } },
...
Each dot after `metadata` separates an object segment. When a stored key contains a literal dot, use a segment array to preserve that exact key:

```typescript
const exactDottedKey = {
  op: 'eq',
  left: { path: ['metadata', 'customer.id'] },
  right: { literal: 'literal-key-value' },
}
```

`metadata.customer.id` addresses nested `{ customer: { id: ... } }` metadata. `['metadata', 'customer.id']` addresses `{ 'customer.id': ... }`. Arrays are only the escape hatch for a literal dotted segment. A redundant path such as `['metadata', 'customer', 'id']` is rejected; use `metadata.customer.id` instead.

Metadata paths must start with `metadata` and contain at least one metadata segment. They allow at most 12 total segments, including `metadata`. Each segment and the complete path joined with dots must fit within 128 UTF-8 bytes. Empty segments and NUL characters are rejected. Metadata paths are valid only in trace predicates, including predicates inside `traces.some` or `traces.none`, not in span, score, or feedback predicates.

Metadata comparisons preserve empty strings, whitespace-only strings, `0`, and `false`. They don't coerce types: numeric `3` doesn't equal string `'3'`, and boolean `false` doesn't equal string `'false'`. Ordered operators require finite numbers. Membership sets must be non-empty and contain only strings, only numbers, or only booleans.

Missing paths, JSON nulls, objects, and arrays count as absent scalar values. They satisfy `ne`, `notIn`, and `notExists`, but not `eq`, `in`, ordered comparisons, or `exists`. A scalar of the wrong type also satisfies `ne` and `notIn`, but it satisfies `exists` rather than `notExists`. Use `exists` alongside a negative predicate to require a present scalar leaf.

Metadata path segments address object properties only. Queries never descend into stored arrays, including through numeric-looking segments. Array traversal, object or array equality, wildcards, and text matching aren't supported.

#### Exact string migration

`metadata.<key>` previously trimmed stored string values and treated empty or whitespace-only strings as missing. Metadata strings now compare exactly for both top-level and nested paths:
...
// Now: include the whitespace to match that stored value.
...
// Empty strings are present and queryable.
...
Review queries that relied on trimming or empty-as-missing behavior. Use `notExists` only for missing, null, object, or array values. When metadata duplicates a canonical trace field such as `resourceId`, `threadId`, or `environment`, prefer the canonical field because it uses the dedicated storage column. Metadata and canonical values may differ.

Metadata fields aren't available for grouping or additional result projections. Trace-query discovery returns executable scalar metadata fields observed in the selected time range. Studio's trace field picker includes discovered metadata fields, type-specific operators, and typed value suggestions.
