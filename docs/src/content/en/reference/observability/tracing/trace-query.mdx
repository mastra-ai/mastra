...
description: 'Query completed traces and thread identities with recursive predicates, related records, ordering, and keyset or page pagination.'
packages:
  - '@mastra/client-js'
  - '@mastra/server'
---
...
## Migrate from `listTraces()`

Use `pagination` to keep the same zero-based page state and response metadata while moving a list view to recursive trace queries:

```typescript
const pagination = { page: 0, perPage: 25 }
...
if ('pagination' in result) {
  ...
  // { total, page, perPage, hasMore }
}
```

`pagination.page` defaults to `0`. `pagination.perPage` defaults to `10` and accepts values from `1` through `100`. An out-of-range page returns an empty `traces` array with the full `total` and the requested `page` and `perPage` values.

Use keyset pagination with `page: { limit, after }` when you want deterministic traversal without numeric page controls. Don't send `page` and `pagination` together. If both are omitted, the query uses keyset mode with a default limit of `100`.

Page mode applies only to trace results. `queryTraceThreads()` remains keyset-only. The deprecated direct-route `group` compatibility option also rejects `pagination`. Use `queryTraceThreads()` for thread identities.
...
| `page` | No | Keyset mode: `{ limit, after }`. `limit` defaults to 100 and accepts values from 1 through 1000. Pass the opaque `page.next` value as `after`. |
| `pagination` | No | List-compatible page mode: `{ page, perPage }`. `page` is zero-based and defaults to 0. `perPage` defaults to 10 and accepts values from 1 through 100. |

`page` and `pagination` are mutually exclusive. Omitting both selects keyset mode with its default limit.
...
Unknown fields are rejected. Requests can't select a projection, declare joins, request custom counts or measures, or control authorization. Page-mode responses include the total number of matching traces as pagination metadata. Request bodies are limited to 256 KiB.
...
A trace query with `pagination` returns list-compatible metadata instead of `page`:
...
A trace response contains exactly one pagination shape: `page` for keyset mode or `pagination` for page mode. An out-of-range page preserves `total` and the requested `page` and `perPage` while returning an empty `traces` array.
...
Ordering is deterministic. Trace ordering appends `traceId` ascending as a tie-breaker. Thread queries always use raw ordinal `threadId` ascending order. Callers can't override it. Page mode calculates `hasMore` as `(page + 1) * perPage < total`.
...
Each page-mode request reads its rows and count coherently. Neither pagination mode is a database snapshot across requests: traces or replacement signals written between requests can change later results and page totals. Keyset mode avoids offset shifts and is the better choice for deterministic traversal. Page mode retains numeric list controls.
...
Both operations consider completed traces only. They don't support running traces, custom projections, embedded evidence, summaries, custom aggregations, count-only queries, measures, or custom grouping. Page-mode responses still include the total matching trace count in their pagination metadata.
