...
Each trace includes the root span's `name`, `entityId`, `parentSpanId`, and `metadata`. `createdAt` is derived from the root span's `startedAt`. `inputPreview` contains a shortened text preview of the root span's input, not the full input. Missing metadata, entity IDs, and input previews are returned as `null`; `parentSpanId` is `null` for root spans.
