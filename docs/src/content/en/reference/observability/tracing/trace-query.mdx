...
Canonical fields come from the same registry used to validate trace queries. They don't consume `limit`. The canonical order follows that registry. `observedFields` contains executable scalar metadata paths found on qualifying current root spans. Top-level non-empty string fields retain their legacy `metadata.<key>` representation and `valueKind: 'string'`. Nested fields, dotted keys, numbers, booleans, and empty strings use segment-array paths with `valueKind: 'scalar'`. Scalar fields support equality, membership, presence, and numeric ordering operators. Fields are ordered by `occurrences` descending, then by their serialized path. Nulls, object and array leaves, empty keys, and oversized paths are omitted. Discovery doesn't traverse arrays.
...
Canonical and legacy metadata paths return string suggestions. Structured metadata paths return typed string, number, and boolean suggestions, preserving empty strings and whitespace. Missing paths, nulls, objects, arrays, and oversized strings are omitted. Values are ordered by `count` descending, then by value (serialized JSON for structured paths). Suggestions are advisory: `queryTraces()` continues to accept valid manual literals that discovery doesn't return.
...
| Raw predicate path, or structured segments joined with `.` | 128 UTF-8 bytes |
| Structured path segments, including `metadata` | 12 |
| Individual path segment | 128 UTF-8 bytes |
| `page.limit` | 1,000 |
...
| Trace metadata (legacy string paths) | `metadata.<key>` | `eq`, `ne`, `in`, `notIn`, `exists`, `notExists` |
| Trace metadata (structured paths) | `['metadata', 'customer', 'id']` | `eq`, `ne`, `in`, `notIn`, `exists`, `notExists`; `lt`, `lte`, `gt`, `gte` for numbers |
...
Use segment-array paths to compare string, number, or boolean leaves in the current root span's metadata:
...
Segments are exact object keys, not JSONPath expressions. `['metadata', 'a.b']` addresses a literal dotted key and is distinct from `['metadata', 'a', 'b']`. Numeric string keys address object properties, not array indexes. Empty segments and NUL characters are rejected. Structured metadata paths are only valid in trace predicates, including those inside `traces.some` or `traces.none`, not in span, score, or feedback predicates.

Structured paths preserve string whitespace, empty strings, `0`, and `false`. Comparisons don't coerce types: numeric `3` doesn't equal string `'3'`. Ordered operators require finite numbers. Membership sets must contain only strings, only numbers, or only booleans. Missing paths, JSON nulls, objects, and arrays count as absent scalar values. They satisfy `ne`, `notIn`, and `notExists`, but not `eq`, `in`, ordered comparisons, or `exists`. Use `exists` alongside a negative predicate to require a present scalar leaf.

Array traversal, object or array equality, wildcards, text matching, and structured input/output or span-attribute paths aren't supported.
...
In the legacy string form, the key must name one top-level property. Empty keys and keys containing dots are rejected. Use segment-array paths for nested properties or literal dotted keys. Metadata keys aren't trimmed, so leading and trailing whitespace remains part of the exact key identity. When metadata duplicates a canonical trace field, such as `resourceId`, `threadId`, or `environment`, prefer the canonical field because it uses the dedicated storage column. The `metadata.<key>` form remains available when you specifically need the value from the metadata object. Metadata and canonical values may differ.

Metadata fields aren't available for grouping or additional result projections. Trace-query discovery returns executable scalar metadata fields observed in the selected time range. Studio's trace field picker includes discovered metadata fields and typed equality-value suggestions.
