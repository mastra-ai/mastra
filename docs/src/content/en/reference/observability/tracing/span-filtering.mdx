...
description: "Span filtering controls which spans are exported from an observability config. Use it to reduce noise or lower per-span costs."
packages:
  - "@mastra/core"
...
The options above apply to every exporter on the observability instance. To drop span types from a single exporter, pass `excludeSpanTypes` to that exporter's config instead. Every exporter that extends `BaseExporter` supports it.
...
<PropertiesTable
  content={[
  {
    ...
    type: 'SpanType[]',
    description:
      "Span types this exporter drops before export. Other exporters on the same instance are not affected. Runs before the exporter's customSpanFormatter.",
    isOptional: true,
  },
]}
/>
...
1. Each exporter drops span types listed in its own `excludeSpanTypes`, then applies its `customSpanFormatter`.
