...
`describeSpanInput`, `describeSpanOutput` and `describeProcessorPipeline` return the
JSON fallback for a span with no recorded phase, or one written by a newer version
with a phase this release doesn't know. The stored payload and the JSON view remain
unchanged.
