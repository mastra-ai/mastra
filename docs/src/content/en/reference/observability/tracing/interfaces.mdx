...
JSON fallback for a span with no recorded phase, one written by a newer version
with a phase this release doesn't know, or an empty payload. The stored payload and the JSON view remain
unchanged.
