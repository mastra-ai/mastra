...
`SpanInputMap` and `SpanOutputMap` type `PROCESSOR_RUN` as the union of every phase's
payload, because the phase that picks one arm lives in `attributes`. Narrowing on
`phase` narrows `data` to that phase alone.
...
with a phase this release does not know. The stored payload and the JSON view remain
unchanged.
