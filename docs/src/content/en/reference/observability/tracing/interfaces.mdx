...
Mapping of span types to the `input` and `output` payload they record. Mastra fixes the shape only for the span types it produces itself; types that carry caller-defined data stay `unknown`, and `GENERIC` stays `any` so custom spans accept any value.
...
interface ModelGenerationMessage {
