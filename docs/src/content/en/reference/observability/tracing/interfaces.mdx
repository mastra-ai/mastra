...
Mapping of span types to the `input` and `output` payload they record. Mastra fixes the shape only for the span types it produces itself; types that carry caller-defined data stay `unknown`, as does `MODEL_CHUNK`, which carries several chunk shapes on one span type. `GENERIC` stays `any` so custom spans accept any value.
...
interface SpanOutputMap {
  ...
  MODEL_INFERENCE: ModelStepOutput
  GENERIC: any
  // every other span type: unknown
}
```
...
Output recorded on `MODEL_GENERATION` spans. Every field is optional: a durable run records only `text`.
...
Output recorded on `MODEL_STEP` and `MODEL_INFERENCE` spans when the step finishes: the step result without `usage`, which lives on the attributes.
...
Output recorded on `AGENT_RUN`, `MODEL_GENERATION` and `MODEL_STEP` spans when the run stops before the span's own result exists: a durable run suspended, or the caller aborted.
