...
```typescript
import { describeProcessorPipeline, describeSpanInput } from '@mastra/core/observability'

const input = describeSpanInput(span)

if (input?.type === 'processor') {
  input.value.phase // 'input' | 'inputStep' | 'llmRequest' | 'llmResponse'
...
`PROCESSOR_RUN` payloads stay untyped in `SpanInputMap` and `SpanOutputMap` because
existing executors can record different shapes. The description helpers narrow only
payloads with a recognized recorded phase and supported fields. Unknown fields stay
on the original payload.

`describeProcessorPipeline` returns `undefined` when the phase or known attributes
are unsupported. `describeSpanInput` and `describeSpanOutput` use their JSON fallback
for unsupported processor payloads, including older spans without a recorded phase.
The stored payload and the JSON view remain unchanged.
