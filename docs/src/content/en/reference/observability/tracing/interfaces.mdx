...
interface SpanInputMap {
  ...
  PROCESSOR_RUN: ProcessorRunInput
}
...
interface SpanOutputMap {
  ...
  PROCESSOR_RUN: ProcessorRunOutput
}
...
Both return `undefined` for a span recorded before the phase was tracked, so older
traces keep their previous untyped shape.
