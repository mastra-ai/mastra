...
Output recorded on `MODEL_STEP` spans: a `ModelStepResult` when the step finishes, or an `InterruptedSpanOutput` when the run stops first. `MODEL_INFERENCE` spans record a `ModelStepResult` only. The result is the step output without `usage`, which lives on the attributes.
