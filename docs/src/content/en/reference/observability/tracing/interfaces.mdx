...
For ClickHouse vNext, the method records the complete predicate before stamping matching rows with a deletion timestamp. It then waits for lightweight delete masks to be applied. Normal reads no longer return the rows matched by that operation when the call resolves. ClickHouse can physically remove masked rows during an earlier merge; otherwise, a conditional TTL makes stamped rows eligible for removal after 30 days. The deletion request remains available for 90 days for an external reapply and purge-verification scheduler.
