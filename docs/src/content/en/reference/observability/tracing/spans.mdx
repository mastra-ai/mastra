...
Creates an event span under this span. Event spans represent point-in-time occurrences with no duration: they are exported immediately on creation with `isEvent: true` and an `endTime` equal to their `startTime`, so exporters never treat them as still running.
