...
| `mastra_tool_duration_ms` | `TOOL_CALL`, `MCP_TOOL_CALL`, `SERVER_TOOL_CALL` | Execution time of a tool call, including MCP and provider-executed tool calls |
...
- **Span type maps to a metric**: Only `AGENT_RUN`, `TOOL_CALL`, `MCP_TOOL_CALL`, `SERVER_TOOL_CALL`, `WORKFLOW_RUN`, `MODEL_GENERATION`, and `PROCESSOR_RUN` spans produce duration metrics.
