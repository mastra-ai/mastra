...
When deploying to Mastra Cloud, logs are displayed on the [Logs](/docs/deployment/mastra-cloud/dashboard#logs) page. In self-hosted or custom environments, logs can be directed to files or external services depending on the configured transports.
