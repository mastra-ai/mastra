...
On a legacy-negotiated connection the shared verbs work (`tools/list`, `tools/call`, `resources/read`, `prompts/get`, list-changed notifications). `subscriptions.listen` and embedded input requests throw an error naming the negotiated revision.
...
Legacy resource subscriptions are removed from both sides. Use `subscriptions.listen` instead.
