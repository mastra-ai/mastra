...
const webhookProvider = new WebhookSignalProvider({
  ...
  extractDeliveryId: request => request.headers['x-github-delivery'],
...
await webhookProvider.subscribeThread(
...
<PropertiesTable
  ...
  {
    name: 'extractDeliveryId',
    type: '(request: SignalProviderWebhookRequest) => string | undefined',
    description: 'Extract a stable delivery ID used by the default notification deduplication key.',
    isOptional: true,
  },
  {
...
await webhookProvider.subscribeThread({ threadId: 'thread-1', resourceId: 'user-1' }, 'mastra-ai/mastra', {
  watchType: 'push',
});
```
...
```typescript prettier:false
const removed = await webhookProvider.unsubscribeThread(
  { threadId: 'thread-1', resourceId: 'user-1' },
  'mastra-ai/mastra',
);
```
