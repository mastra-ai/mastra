...
```typescript prettier:false
const sub = await this.subscribe({ threadId: 'thread-1', resourceId: 'user-1' }, 'github:mastra-ai/mastra#123', {
  pr: 123,
});
```

Returns: `Promise<SignalSubscription>`: the created subscription, or the existing one with merged metadata.
...
```typescript prettier:false
const removed = await this.unsubscribe({ threadId: 'thread-1', resourceId: 'user-1' }, 'github:mastra-ai/mastra#123');
```

Returns: `Promise<boolean>`: `true` if removed, `false` if no matching subscription existed.
...
Returns: `Promise<SignalSubscription[]>`
...
```typescript prettier:false
const subs = await this.getSubscriptionsForResource('github:mastra-ai/mastra#123');
...
```typescript prettier:false
const subs = await this.getSubscriptionsForThread({
  threadId: 'thread-1',
  resourceId: 'user-1',
});
```
...
```typescript prettier:false
if (await this.hasSubscription(target, 'github:mastra-ai/mastra#123')) {
  // already subscribed
}
```
...
```typescript prettier:false
const removed = await this.unsubscribeAll({
  threadId: 'thread-1',
  resourceId: 'user-1',
});
```

Returns: `Promise<number>`: count of removed subscriptions.
...
```typescript prettier:false
if ((await this.subscriptionCount) === 0) {
  // nothing to poll
}
```
