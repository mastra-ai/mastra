---
title: "Reference: createMastraCode() | Mastracode"
description: "API reference for createMastraCode(), the factory function that initializes the Mastracode coding agent with its harness, MCP manager, hooks, and auth."
packages:
  - "mastracode"
---
...
**Added in:** `mastracode@0.1.0`

`createMastraCode()` is the main factory function that bootstraps the Mastracode coding agent. It wires together project detection, storage, authentication, MCP servers, hooks, modes, subagents, and the Harness that drives the TUI or any other frontend.
  ...
  content={[
    {
      name: "cwd",
      type: "string",
      description: "Working directory for project detection. Mastracode uses this path to find the Git root, determine the project name, and scope threads.",
      isOptional: true,
    ...
    },
    {
      name: "modes",
      type: "HarnessMode[]",
      description: "Override the default agent modes. Each mode defines an agent, a default model, and a display color. When omitted, Mastracode ships with Build, Plan, and Fast modes.",
      isOptional: true,
    },
    {
      name: "subagents",
      type: "HarnessSubagent[]",
      description: "Override the default subagent definitions. When omitted, Mastracode includes Explore (read-only codebase exploration), Plan (read-only analysis and planning), and Execute (task execution with write access).",
      isOptional: true,
    },
    {
      name: "extraTools",
    ...
    },
    {
      ...
      description: "Custom LibSQL storage configuration. When omitted, Mastracode auto-detects storage from environment variables, project config, global config, or falls back to a local SQLite database.",
      isOptional: true,
    },
    {
      name: "initialState",
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description: "The MCP manager for connecting to external MCP servers. `undefined` when `disableMcp` is `true` or no servers are configured.",
    },
    {
      name: "hookManager",
...
When no `modes` are specified, Mastracode creates three modes:
...
When no `subagents` are specified, Mastracode creates three subagent types:
...
Mastracode automatically detects project identity from the working directory:
...
- [Mastracode configuration](/docs/mastracode/configuration)
