...
Each tool also carries `tool.title`, set from the server's tool `title` and falling back to `annotations.title`. This matches the MCP display-name precedence: `title`, then `annotations.title`, then the tool name.
