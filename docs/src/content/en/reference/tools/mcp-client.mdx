...
Use `requireToolApproval` on a server definition to require human approval before any tool from that server is executed. This works with the agent [human-in-the-loop](/docs/agents/human-in-the-loop) approval flow.
