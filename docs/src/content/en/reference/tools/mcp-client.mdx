...
<PropertiesTable
  ...
  },
  {
    ...
    defaultValue: "'throw'",
    description:
      "How a tool execution failure (a CallToolResult with `isError: true`) is surfaced. With 'throw' (the default), the tool call rejects with a MastraError so agents map it onto the failed-tool-call path and the model sees the error text. With 'return', the call resolves with the tool result so a non-LLM consumer can inspect it directly. See the Tool error handling section below.",
  },
]}
/>
...
Per the MCP specification, a tool that fails during execution reports the failure **in-band**: the server returns a normal `CallToolResult` with `isError: true`, the failure details in `content`, and (optionally) structured error data in `structuredContent`. The `onToolError` server option controls how `MCPClient` surfaces that result.

### `onToolError: 'throw'` (default)

The tool call rejects with a `MastraError` (`id: 'MCP_CLIENT_TOOL_EXECUTION_FAILED'`). This is the right default for agents: the error maps onto Mastra's failed-tool-call path so tool spans, stream chunks, scorers, and persisted message parts reflect the failure, and the model sees the error text so it can self-correct.

The server's `structuredContent`, when present, is preserved on the thrown error as a JSON string under `details.structuredContent`, so a caller that catches the error can recover the structured data:
...
try {
  ...
  if (error instanceof MastraError && error.id === 'MCP_CLIENT_TOOL_EXECUTION_FAILED') {
    const structured = error.details?.structuredContent
      ? JSON.parse(error.details.structuredContent as string)
      : undefined
    // structured?.code, structured?.validationResults, ...
  }
}
```
...
The tool call resolves instead of rejecting. Successful return shapes are unchanged: bare `structuredContent` when present, otherwise the full `CallToolResult`. Failed results return bare `structuredContent` only when it is an object or array. Scalar or `null` structured errors now return the full envelope, preserving the value under `result.structuredContent`. Content-only errors also return the full envelope, with `result.isError` and `result.content` available directly.

The client marks both bare structured errors and error envelopes. Use these helpers exported from `@mastra/mcp` to detect either form without confusing a successful user-data field named `isError` with MCP metadata:

<PropertiesTable
  content={[
    {
      name: 'getMcpCallToolIsError(result)',
      type: 'boolean | undefined',
      description:
        'Returns `true` for failed execute results, including scalar/null and content-only error envelopes. Returns `undefined` for successful results or values without the hidden flag. Does not inspect arbitrary objects for an `isError` field.',
    },
    {
      name: 'getMcpCallToolContent(result)',
      type: 'unknown',
      description:
        'Returns the MCP `content` blocks (the model-facing text) preserved alongside a structured result. Returns `undefined` when no content channel is present.',
    },
    {
      name: 'getMcpCallToolMeta(result)',
      type: 'Record<string, unknown> | undefined',
      description:
        'Returns the result-level `_meta` preserved on a structured result (for example `ui.resourceUri` for MCP App detection). Returns `undefined` when the result carried no `_meta`.',
    },
  ]}
/>
...
if (getMcpCallToolIsError(result)) {
  // Detects bare structured errors and error envelopes alike.
  const content = getMcpCallToolContent(result)
  // Object/array structured errors expose their fields directly.
  // Scalar/null errors preserve their value in result.structuredContent.
}
```

These helpers read non-enumerable symbol channels attached by the client. Those channels do not survive `JSON.stringify()` or object spreading. Error envelopes also retain their ordinary `isError`, `content`, and optional `structuredContent` fields. For raw or deserialized MCP envelopes, read those fields directly; the helpers require the original execute result.
