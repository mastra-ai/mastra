...
The client speaks the MCP `2026-07-28` revision only (exported as `MCP_CLIENT_PROTOCOL_VERSION`). Every request is self-contained and carries everything the server needs. The client never opens a session, sends `initialize` or falls back to the standalone HTTP+SSE transport. A server that doesn't offer `2026-07-28` fails with an explicit negotiation error. See the [v2 migration guide](/reference/migrations/mcp-v2) for what changed from 1.x.
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'For HTTP servers: The URL of the Streamable HTTP endpoint (for a Mastra-hosted server, `/api/mcp/:serverId/mcp`).',
  },
  {
    name: 'requestInit',
    type: 'RequestInit',
  ...
  },
  {
    ...
    type: 'number',
    isOptional: true,
    description: 'For HTTP servers: Timeout in milliseconds for the initial discovery request.',
  },
  {
    name: 'fetch',
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Answers embedded input requests (`input_required` results) from this server, one keyed request at a time. Configuring a handler advertises form elicitation support; widen it with `capabilities.elicitation` to accept url-mode requests. Without a handler an `input_required` result is surfaced as a typed error. See Input requests.',
  },
  {
    name: 'authProvider',
  ...
  },
  {
    ...
    defaultValue: 'true',
    description:
      'Whether to opt into per-request server logs. Attaches the `io.modelcontextprotocol/logLevel` metadata key to every request so the server delivers `notifications/message` for it, and forwards delivered messages to `logger`.',
  },
  {
    name: 'serverLogLevel',
    type: 'LoggingLevel',
  ...
  },
  {
    ...
    defaultValue: 'false',
    description:
      'Whether to attach a progress token to tool calls and forward `notifications/progress` to `progress.onUpdate` handlers.',
  },
  {
    name: 'traceContext',
    ...
    isOptional: true,
    description:
      'Returns the W3C `traceparent`, `tracestate` and `baggage` values to attach to each outgoing request’s `_meta`. Called when the request is sent. Explicit `_meta` keys supplied for a tool call take precedence.',
  },
  {
    name: 'onToolError',
    type: "'throw' | 'return'",
    ...
    defaultValue: "'throw'",
    description:
      "How to handle tool results with `isError: true`. `'throw'` surfaces the failure as a `MastraError` carrying the server's content text; `'return'` resolves with the raw result.",
  },
  {
...
If you trust the MCP server, you can use its [tool annotations](https://modelcontextprotocol.io/specification/2026-07-28/server/tools#tool-annotations) (`readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`, `title`) to drive approval decisions:
...
Returns a map of `MCPClientServerProxy` instances, one per configured server. Each proxy wraps the underlying client connection as an `MCPServerBaseV2` instance, allowing external (non-Mastra) MCP servers to be registered in `mcpServers` and appear in Studio.
...
const mcpClient = new MCPClient({
  servers: {
    myServer: {
      ...
      // Disabled by default; opt in per server
...
Progress notifications are delivered on the response stream of the tool call that carries the progress token, so they arrive only while that call is in flight.

## Input requests

A server tool, resource or prompt that needs information from the user answers with an `input_required` result instead of a final value. The result carries one or more keyed elicitation requests. The client collects an answer for each key and repeats the original call with the answers attached. Mastra drives that loop for you when the server definition has an `inputRequests` handler.

### How it works

1. **Server result**: `tools/call`, `resources/read` or `prompts/get` returns `input_required` with keyed `inputRequests` and opaque `requestState`
1. **Client handler**: your `inputRequests` handler is called once per key with the request `params` and a cancellation `signal`
1. **User interaction**: your handler collects the input (via UI, CLI, etc.) and returns accept, decline or cancel
1. **Retry**: Mastra repeats the call with the answers for this round and the echoed `requestState`
1. **Completion**: the server either completes or asks for another round; earlier rounds' answers are never re-sent
  ...
  servers: {
    interactiveServer: {
      ...
      inputRequests: async ({ key, params, signal }) => {
        if (params.mode === 'url') {
          // Open params.url for the user, or decline.
          return { action: 'decline' }
        }
        console.log(`Server needs "${key}": ${params.message}`)
        const content = await collectUserInput(params.requestedSchema, { signal })
...
Configuring a handler advertises the `elicitation.form` capability to the server. To accept url-mode requests as well, widen `capabilities.elicitation`. A server only embeds input requests for clients that advertise support; without a handler, an `input_required` result is surfaced as a typed error rather than answered on your behalf.

### Response types

The handler returns an `ElicitResult`:

- **Accept**: the user provided data matching `params.requestedSchema`
...
`params.requestedSchema` describes the data the server needs:
...
inputRequests: async ({ params }) => {
  ...
  for (const [fieldName, fieldSchema] of Object.entries(properties || {})) {
    ...
    if (value !== null) content[fieldName] = value
  }
...
- **Configure the handler up front**: it's part of the server definition, so tools that need input work from the first call
- **Validate input**: check that required fields are provided before accepting
- **Respect user choice**: handle decline and cancel responses gracefully; the server decides what they mean for the operation
- **Honor `signal`**: abort pending prompts when the originating call is cancelled or times out
- **Security**: never auto-accept requests for sensitive information, and treat `params.message` as untrusted server content
...
When `fetch` is provided, `requestInit` and `authProvider` become optional, as you can handle these concerns within your custom fetch function. Every request the client sends, including the `subscriptions/listen` stream it opens for notifications, goes through your `fetch` as a `POST`, so there is no background `GET` listener to short-circuit. If authentication is unavailable, return the request without a token and let the server answer `401`; the error surfaces to the caller of `listTools()`, `callTool` and friends.
...
- For more about the Model Context Protocol, see the [MCP 2026-07-28 specification](https://modelcontextprotocol.io/specification/2026-07-28).
