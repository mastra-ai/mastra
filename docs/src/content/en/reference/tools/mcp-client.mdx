...
<PropertiesTable
  ...
  },
  {
    ...
    type: "'auto' | '2025-11-25' | '2026-07-28'",
    isOptional: true,
    defaultValue: "'auto'",
    description:
      "MCP protocol version negotiation. Omitted or 'auto' probes the server at connect time and uses the stateless '2026-07-28' revision when available, with a fallback to the legacy handshake. '2025-11-25' selects the legacy connect sequence without probing. '2026-07-28' pins that revision exactly and fails with a typed error when the server does not offer it. Elicitation handlers work on both eras. On a '2026-07-28' connection, embedded elicitation requests from input_required results are dispatched through the same registered handler and the originating call retries automatically.",
  },
  {
...
Omitted `protocolVersion` uses automatic negotiation in `@mastra/mcp` v2. Set `protocolVersion: '2025-11-25'` for integrations that must keep the v1 handshake without a `server/discover` probe. See [Migrate to `@mastra/mcp` v2](/docs/connections/mcp-v2-migration) for session, notification, elicitation, OAuth, and schema changes.
...
For connecting to MCP servers that require OAuth authentication per the [MCP 2026-07-28 authorization specification](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization), use the `MCPOAuthClientProvider`. For clients without a pre-existing relationship with the authorization server, host a [Client ID Metadata Document](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization/client-registration#client-id-metadata-documents) at an HTTPS URL and provide that URL as `clientMetadataUrl`:
...
The document served at `clientMetadataUrl` must contain the same `client_id`, `client_name`, and `redirect_uris`. For loopback callbacks, include every fallback URL returned by `getCallbackUrlCandidates()` in the hosted document. When authorization-server metadata advertises `client_id_metadata_document_supported: true`, the provider uses this URL as the client ID and doesn't call the registration endpoint. Pre-registered `clientInformation` still takes priority. If the authorization server doesn't advertise metadata-document support, the SDK falls back to Dynamic Client Registration when available. DCR is deprecated in MCP 2026-07-28 and remains only for compatibility with older authorization servers.

Client information and tokens saved by `MCPOAuthClientProvider` are bound to the authorization server's validated `issuer`. The loopback callback also forwards the RFC 9207 `iss` parameter to the SDK and rejects a mismatch before exchanging the authorization code.
...
When a server rejects a connection because authorization is required, the client records a `'needs-auth'` state instead of failing outright. Calling `authenticate()` completes the flow. It starts a one-shot callback server on the provider's loopback redirect URL and falls back to the next sequential ports when it's in use. After discovery, the SDK first selects pre-registration. It prefers a Client ID Metadata Document next, with deprecated Dynamic Client Registration as the final fallback. `onRedirectToAuthorization` receives the authorization URL so your application can open it in the user's browser. The token exchange finishes after the browser returns the authorization code:
