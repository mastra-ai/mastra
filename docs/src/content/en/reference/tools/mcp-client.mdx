...
Tools without output schemas use `unknown`. Unsupported schema portions also widen to `unknown`, with a warning. External schema references aren't fetched. Malformed schemas and unresolved local references fail generation.
