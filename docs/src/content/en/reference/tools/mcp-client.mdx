...
The tool call resolves instead of rejecting, so a non-LLM consumer (for example, an MCP App UI host) can inspect the result directly. The resolved value keeps the same shape it has on success — the bare `structuredContent` when the server returns it, otherwise the full `CallToolResult` — so existing field access is unchanged.

Because a resolved value looks the same whether the tool succeeded or failed, read the failure signal and preserved metadata with these helpers exported from `@mastra/mcp`:
...
<PropertiesTable
  content={[
  {
    ...
    description:
      'Returns `true` when the result came from a tool call that reported `isError: true`. Returns `undefined` for successful results and for scalar results that cannot carry the flag.',
  },
  {
...
if (getMcpCallToolIsError(result)) {
  // The tool failed. `result` still exposes its structured fields directly,
  // and getMcpCallToolContent(result) returns the human-readable error text.
}
```

These helpers read non-enumerable symbol channels, so the failure signal and metadata do not appear in `Object.keys()`, `JSON.stringify()`, or a `console.log` of the result — always read them through the helpers.
