...
<PropertiesTable
  ...
  },
  {
    ...
    defaultValue: 'true',
    description:
      "Whether to append instructions advertised by this MCP server to an agent's system prompt when the agent uses this server's tools.",
  },
  {
...
When an MCP server advertises instructions during initialization, `MCPClient` stores them for that server. Agents that use tools from `listTools()` or `listToolsets()` automatically receive those instructions in their system prompt.
...
Set `forwardInstructions` to `false` for a server when you want to inspect or handle its instructions yourself.
