...
The tool pauses through the native [tool suspension](/guides/agents/agent-approval#runtime-suspension-with-suspend) primitive: it calls `suspend({ path })`, which makes the agent emit a `tool-call-suspended` event. The host reads the plan file and presents it to the user. It then resumes with an approval or rejection.
...
When used inside an [AgentController](/guides/agent-controller/overview), plan approval triggers an automatic mode switch from the planning mode to the default execution mode. See [Modes](/guides/agent-controller/modes) for details.
