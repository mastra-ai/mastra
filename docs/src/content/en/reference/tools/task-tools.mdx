...
The recommended setup is [`TaskSignalProvider`](/reference/signals/task-signal-provider), which bundles all four tools and the `TaskStateProcessor` in a single registration. See [Built-in tools](/guides/agents/using-tools#task-tracking) for the conceptual guide.
