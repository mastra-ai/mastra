...
Each vector store package exports a prompt constant that defines the supported filter operators and their usage. These prompts help agents construct valid filters for each specific vector store:

```typescript
import { PINECONE_PROMPT } from '@mastra/pinecone';

// Use these prompts to guide filter construction for each store
const queryTool = createVectorQueryTool({
  vectorStoreName: "pinecone",
  indexName: "docs",
  model: openai.embedding('text-embedding-3-small'),
  enableFilters: true,
  filterPrompt: PINECONE_PROMPT // Store-specific filter prompt
});
```
