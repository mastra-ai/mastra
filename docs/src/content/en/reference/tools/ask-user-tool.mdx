...
<PropertiesTable
        ...
        parameters: [
          {
            name: 'label',
            type: 'string',
            description: 'Short display text for this option (1-5 words). This value is returned to the model when selected.',
          },
          {
            name: 'description',
            type: 'string',
  ...
  },
  {
    ...
    description: "Controls how many options the user can select. Defaults to 'single_select' when options are provided. Requires options.",
    isOptional: true,
  },
]}
/>
...
```typescript
type AskUserAnswer = string | string[]
```

### `AskUserSuspendPayload`
