...
The tool pauses through the native [tool suspension](/guides/agents/agent-approval#runtime-suspension-with-suspend) primitive: it calls `suspend()` with the question payload, which makes the agent emit a `tool-call-suspended` event and persist run state. Resume the run with `agent.resumeStream(answer, { runId })`.
