...
It supports both [stdio (subprocess) and SSE (HTTP) MCP transports](https://modelcontextprotocol.io/guides/concepts/transports).
...
For more detailed information, refer to the [official MCP documentation on Resources](https://modelcontextprotocol.io/guides/concepts/resources).
...
Link a tool to its app resource by setting `_meta.ui.resourceUri` on the tool to the matching `ui://` URI. The server auto-normalizes this metadata when registering tools. Visit [MCP Apps](/guides/mcp/mcp-apps) for the full app bridge API and usage patterns.
