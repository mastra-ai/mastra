...
To protect your MCP server with OAuth authentication per the [MCP Authorization specification](https://modelcontextprotocol.io/specification/2025-06-18/basic/authorization), use the `createOAuthMiddleware` function:
...
| `_meta` | Request metadata sent by the client: trace headers, log-level opt-in, progress token |
