...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "MCP protocol revision. Omitted or '2026-07-28' serves the stateless modern revision with dual-era compatibility for legacy clients. Set to '2025-11-25' to retain the legacy sessionful server. See the Protocol versions section for details.",
  },
  {
    name: 'cacheHints',
...
By default, `MCPServer` uses the stateless MCP 2026-07-28 revision:

- HTTP requests go through a dual-era handler. Modern clients receive native stateless, per-request behavior. Legacy clients use the built-in stateless compatibility path on the same endpoint.
- `startStdio()` serves both eras. The opening exchange selects the era for the connection.
- Tool list, prompt list, resource list, and resource update notifications reach modern clients through `subscriptions/listen`.
...
- Tool elicitation (`options.mcp.elicitation.sendRequest()`) works on both eras. On modern requests, it uses the protocol's multi round-trip mechanism. The tool call first returns an `input_required` result. After the client answers, the tool runs again with the answer attached. Keep work before the final elicitation idempotent and keep `sendRequest()` calls in a deterministic order.
...
Set `protocolVersion: '2025-11-25'` to retain the v1 sessionful behavior. Legacy-only `startHTTP()` options, including `sessionIdGenerator` and serverless response settings, require this explicit pin. See [Migrate to `@mastra/mcp` v2](/docs/connections/mcp-v2-migration) for the full compatibility checklist.
...
The method returns the server's basic information. The `protocol_version` field contains the effective protocol version: `2025-11-25` or `2026-07-28`.
...
The method returns details about the server's information, including the effective `protocol_version`.

```typescript oxfmt:false
getServerDetail(): MCPServerDetailInfo
```
