...
|-------------|---------------|
| Direct tool call | `options?.mcp?.extra` |
| Agent tool call | `requestContext?.get("mcp.extra")` |

**Universal pattern** (works in both contexts):
```typescript
const authInfo = options?.mcp?.extra?.authInfo || requestContext?.get("mcp.extra")?.authInfo;
```
...
const fetchUserData = createTool({
  ...
  execute: async ({ context, requestContext }, options) => {
    // Universal access pattern
    const mcpExtra = options?.mcp?.extra || requestContext?.get("mcp.extra");
