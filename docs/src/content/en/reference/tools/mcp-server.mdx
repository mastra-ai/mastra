...
Use `MastraApiMCPServer` to give MCP clients the same operations as the `mastra api` CLI. It reads the target server's API schema when it starts and only registers operations supported by that server.
...
The server uses stateless MCP transport. Read operations, mutations, and destructive operations have separate MCP tool annotations. Each tool call sends one request to the target API and does not retry mutations. Routes outside the CLI catalog are not exposed.
