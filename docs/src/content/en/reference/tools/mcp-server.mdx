...
When the server is registered on a `Mastra` instance that has observability configured, every request it handles produces an `MCP_SERVER_REQUEST` root span: `tools/list`, `tools/call`, `resources/*`, `prompts/*`, and `logging/setLevel`. `executeTool()` produces the same span, so the Studio MCP server page is traced like an MCP client.
