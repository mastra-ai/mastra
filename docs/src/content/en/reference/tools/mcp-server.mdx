...
The `MCPServer` class automatically includes elicitation capabilities and provides an `elicitation.sendRequest()` method that tools can use to request user input.

### How Elicitation Works

A common use case is during tool execution. When a tool needs user input, it can call the server's elicitation functionality:

1. The tool calls `server.elicitation.sendRequest()` with a message and schema
    ...
    collectContactInfo: createTool({
      ...
      execute: async ({ context }) => {
        ...
        try {
          ...
          const result = await server.elicitation.sendRequest({
            message: reason 
...
The elicitation functionality is available on every `MCPServer` instance:
