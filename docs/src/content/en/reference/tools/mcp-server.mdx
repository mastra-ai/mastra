...
Link a tool to its app resource by setting `mcp._meta.ui.resourceUri` to the matching `ui://` URI. The server auto-normalizes this metadata when registering tools. Visit [MCP Apps](/docs/mcp/overview) for the full app bridge API and usage patterns.
