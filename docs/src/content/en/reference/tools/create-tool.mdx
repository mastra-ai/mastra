...
When exposing tools via MCP (Model Context Protocol), you can add annotations to describe tool behavior and customize how clients display the tool:
...
export const weatherTool = createTool({
  ...
  // MCP annotations for client behavior hints
  annotations: {
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description:
        "MCP tool annotations for describing tool behavior and UI presentation. These are exposed via MCP protocol and used by clients like OpenAI Apps SDK. Includes fields like `title`, `readOnlyHint`, `destructiveHint`, `idempotentHint`, and `openWorldHint`.",
      isOptional: true,
    },
    {
      name: "_meta",
      ...
      description:
        "Arbitrary metadata that will be passed through to MCP clients. This field allows custom metadata to be attached to tools for client-specific functionality.",
      isOptional: true,
    },
    {
      name: "execute",
