...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    description:
      'A human-readable display label for the tool. It is not sent to the model; it is snapshotted onto `tool-call` and `tool-result` stream chunks (`payload.title`) and onto persisted `tool-invocation` message parts, so UIs can show a friendly name instead of the tool `id`. Falls back to `mcp.annotations.title` when not set.',
    isOptional: true,
  },
  {
    name: 'inputSchema',
