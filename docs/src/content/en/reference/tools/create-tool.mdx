...
```typescript title="src/mastra/tools/authenticated-tool.ts"
import { createTool } from "@mastra/core/tools";
import { z } from "zod";

export const tool = createTool({
  id: "authenticated-tool",
  description: "A tool that requires user authentication",
  inputSchema: z.object({
    query: z.string(),
  }),
  ...
  execute: async (inputData, context) => {
    // Access validated request context
    const userId = context.requestContext.get("userId");
    const orgId = context.requestContext.get("organizationId");

    console.log(`Processing query for user ${userId} in org ${orgId}`);
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: "Zod schema",
      description:
        "A Zod schema defining the expected structure of request context values. When provided, the framework validates that the required context is present before tool execution. Access validated values via `context.requestContext` in the execute function.",
      isOptional: true,
    },
    {
