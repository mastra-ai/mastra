...
Deletes the observability traces produced by an experiment, then deletes the experiment and its result records. Trace deletion cascades to spans and trace-linked signals such as scores, feedback, metrics, and logs. Unsupported observability storage leaves the traces in place and logs a warning.
