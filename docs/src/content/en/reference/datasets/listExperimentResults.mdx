...
description: "Lists individual item results for a specific experiment with optional filters and pagination. Filters are applied at the storage layer."
packages:
  - "@mastra/core"
---
...
// Restrict to results tagged with both 'regression' and 'p0'
const { results: tagged } = await dataset.listExperimentResults({
  experimentId: 'exp-id',
  tags: ['regression', 'p0'],
})
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Restrict results to those that have every listed tag. Results with extra tags still match; an empty array applies no filter.',
  },
  {
    name: 'filters',
