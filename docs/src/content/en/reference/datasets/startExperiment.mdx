...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'number',
    isOptional: true,
    description: 'Fallback execution timeout per item in milliseconds. An item-level `timeout` overrides this value.',
  },
  {
    name: 'maxRetries',
    type: 'number',
...
For each item, `timeout` takes precedence over `itemTimeout`. An item without `timeout` inherits `itemTimeout`. If neither value is set, the item has no execution deadline.

The effective timeout is one budget for the initial attempt, retry backoff, and all retry attempts. The budget doesn't restart for each attempt. Cancelling the experiment through `signal` takes precedence over item deadlines and isn't retried.
