...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Global request context passed to `agent.generate()` or `workflow.start()`/`workflow.resume()` for all items. Per-item `requestContext` values take precedence. Use this for custom headers, tenant IDs, or authentication tokens.',
  },
]}
/>
