...
Runs an experiment on the dataset and waits for completion. Executes all items against a target (agent, workflow, or inline task function) with optional scoring.
...
```typescript
// Run with an inline task function
const summary = await dataset.startExperiment({
...
<PropertiesTable
  content={[
    ...
    {
      name: "task",
      type: "(args: { input: I; mastra: Mastra; groundTruth?: E; metadata?: Record<string, unknown>; signal?: AbortSignal }) => O | Promise<O>",
      isOptional: true,
      description: "Inline task function. Use instead of `targetType`/`targetId`.",
    },
    {
      name: "scorers",
