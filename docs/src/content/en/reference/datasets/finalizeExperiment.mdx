...
Finalizes an external experiment created with [`createExternalExperiment()`](/reference/datasets/createExternalExperiment). Mastra computes the final counts server-side from the persisted result rows and marks the experiment `'completed'` — the caller never tracks completion bookkeeping.
