...
description: 'Documentation for the `dataset.finalizeExperiment()` method, which computes final counts for an external experiment and marks it completed.'
packages:
  - '@mastra/core'
---
...
Finalizes an external experiment created with [`createExternalExperiment()`](/reference/datasets/createExternalExperiment). Mastra computes the final counts server-side from the persisted result rows and marks the experiment `'completed'`, so the caller never tracks completion bookkeeping.
