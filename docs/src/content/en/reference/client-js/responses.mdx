...
Gateway-backed agents follow the same stored-response contract. Use `store`, `conversation_id`, and `previous_response_id` the same way you would for local memory. If you need resource scoping, pass it through `requestContext` instead of sending `resource_id` in the request body.
