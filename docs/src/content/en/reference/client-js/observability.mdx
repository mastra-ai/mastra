...
Delete feedback records by id. Deletion is idempotent — missing ids are ignored — and can optionally be scoped to a tenant with `organizationId` and `resourceId`:
