...
Delete feedback or score records by id. Deletion is idempotent, so missing ids are ignored. Optional `organizationId` and `resourceId` fields restrict each deletion to records with matching scope fields:
