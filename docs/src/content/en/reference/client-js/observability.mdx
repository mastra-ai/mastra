...
`getTraceQueryFields()` returns canonical query fields and observed metadata fields for one predicate scope. In the `trace` scope, metadata discovery recursively walks objects and returns string, number, and boolean leaves. The response includes each field's value kind, supported operators, and whether value suggestions are available.
...
Both methods use case-insensitive literal substring search and accept an empty search. Metadata value suggestions preserve their string, number, or boolean type without trimming or coercion. Limits default to 25 and can't exceed 100. Results are ordered by occurrence count, then deterministically by path or value. `observedFieldsTruncated` and `valuesTruncated` indicate that the caller should refine `search`. Discovery has no cursor.
