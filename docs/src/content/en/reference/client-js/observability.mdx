...
Delete feedback records by id. Deletion is idempotent, so missing ids are ignored. You can scope the deletion to a tenant with `organizationId` and `resourceId`:
