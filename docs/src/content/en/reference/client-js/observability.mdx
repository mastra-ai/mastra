...
Use `pagination` when a list needs numeric page controls:
...
if ('pagination' in result) {
  const { total, page, perPage, hasMore } = result.pagination
  console.log({ total, page, perPage, hasMore })
}
```

Page mode uses the same zero-based `{ page, perPage }` request and `{ total, page, perPage, hasMore }` response metadata as `listTraces()`. Use `page` instead when traversing results with an opaque keyset cursor:
...
const firstPage = await mastraClient.queryTraces({
  timeRange,
  page: { limit: 100 },
})

if ('page' in firstPage && firstPage.page.next) {
  await mastraClient.queryTraces({
    timeRange,
    page: { limit: 100, after: firstPage.page.next },
  })
}
```

Don't send `page` and `pagination` together. Omitting both selects keyset mode. Use `queryTraceThreads()` for thread identities. It remains keyset-only.
...
The API limits predicate depth, nodes, related clauses, set members, literal bytes, and the total literal budget before storage execution. Each page-mode request returns coherent rows and count metadata, but neither pagination mode is a snapshot across requests. Signals written between requests can change later results and page totals. PostgreSQL and ClickHouse trace and thread queries have a configurable 15-second execution timeout.

See [Advanced trace queries](/reference/observability/tracing/trace-query) for the complete limits, request fields, predicates, thread qualification semantics, pagination modes, response shapes, and errors.
