...
Execute a workflow and return the final result in a single call. This method simplifies workflow execution by combining run creation and execution into a single API call, reducing network latency:
...
console.log(result.steps);  // All step execution details
```
...
This method combines workflow run creation and execution in a single call, returning the complete workflow result including status, output, and step details.
