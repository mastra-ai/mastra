...
The stream is a `ReadableStream<ChunkType>` that you can iterate over directly, and also provides:
- **`status`**: `Promise<string>` - Workflow execution status
- **`result`**: `Promise<any>` - Complete workflow result
