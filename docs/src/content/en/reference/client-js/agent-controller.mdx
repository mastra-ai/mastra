...
Each connection starts with a `session_snapshot` event containing `displayState`, the latest run's `messages`, and `streamingMessageId`. Merge these messages with thread history by ID and use `streamingMessageId` to identify the assistant message still being generated. It's `null` when no message is streaming, even if a run is active. The snapshot includes pending approvals and buffered tool output, so a client joining during a long tool execution can render the current activity immediately.

The snapshot retains the latest run until the next run starts or the session switches threads. It isn't the full conversation history and doesn't replay past lifecycle events. Load older messages with `listMessages()`.
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Called after reconnection, before the initial session snapshot is read. Refresh session.state() and session.listMessages() to recover persisted state and messages outside the latest run.',
  },
  {
    name: 'reconnect',
...
| Session | `session_snapshot`, `state_changed`, `display_state_changed`, `mode_changed`, `model_changed`, `thread_changed`, `thread_created`, `thread_deleted`, `thread_title_updated` |
...
`message_*` events carry a `MastraDBMessage` and `thread_created` carries a thread, with timestamps hydrated to `Date`. Messages in `session_snapshot` and `displayState.currentMessage` have the same timestamp hydration.
