...
| Messages | `message_start` (carries the full message), `message_update` (carries an ID-addressed delta), `message_end` (carries only the message ID) |
...
// Mirrors `applyUpdate` from `@mastra/core/agent-controller`, inlined so a
// browser bundle doesn't pull in that entry point's `node:crypto` import.
function applyUpdate(
  ...
  if (update.type === 'text-delta') {
    ...
    const reasoning = part.reasoning + update.delta
    parts[update.index] = { ...part, reasoning, details: [{ type: 'text', text: reasoning }] }
  } else {
    if (!Number.isInteger(update.index) || update.index < 0 || update.index > parts.length)
      return undefined
...
function handleEvent(event: AgentControllerEvent) {
  ...
  switch (event.type) {
    case 'message_start':
      ...
      break
    case 'message_update': {
      const updated = applyUpdate(activeMessages.get(event.id), event.event)
      if (!updated) break
...
`display_state_changed` carries the folded [`AgentControllerDisplayState`](/reference/agent-controller/session#displaystate), so a client doesn't fold message deltas to render the streaming message. Read `currentMessage` from it and pass the message to `agentControllerMessageText(message)` for its plain text:
...
function renderStreamingMessage(event: AgentControllerEvent) {
  if (!isKnownAgentControllerEvent(event)) return
  if (event.type !== 'display_state_changed') return
  const message = event.displayState.currentMessage
  if (message) render(agentControllerMessageText(message))
}
```

The state is coalesced for `message_update` and `tool_input_delta` over a short window, so it's the right input for rendering and not for per-delta accounting.

### Reuse the fold from the package

The fold above is inlined because `@mastra/core/agent-controller` imports `node:crypto` at its entry point, which a browser bundle shouldn't pull in. Server-side consumers can import that same `applyUpdate` instead of copying it; it has the contract shown here, so store the result only when it's defined. [Message lifecycle](/reference/agent-controller/message-lifecycle) documents it in full.
