...
Pass `onChunk` to receive raw agent chunks on the same connection. When a run is active in the server process, its buffered chunks replay before new chunks arrive. A client opening a thread during a long tool call can render the ongoing response immediately. Chunks go to `onChunk`, while controller events go to `onEvent`.

On reconnection, reset accumulated chunks in `onReconnect` before replay starts. Load persisted messages with `listMessages()`, including runs that finished while disconnected.
...
<PropertiesTable
  ...
  {
    name: 'onChunk',
    type: '(chunk: ChunkType) => void',
    isOptional: true,
    description:
      'Receives raw agent chunks, including buffered chunks from the active run. ChunkType is exported from @mastra/core/stream.',
  },
  {
    name: 'onError',
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Called after reconnection, before any events or chunks are read. Reset accumulated chunks here and refresh session.state() and session.listMessages() to recover persisted changes.',
  },
  {
    name: 'reconnect',
