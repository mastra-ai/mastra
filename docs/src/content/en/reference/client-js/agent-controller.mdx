...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Called each time the stream is re-established after a drop. Each connection starts with the current display snapshot, but missed message events are not replayed. Use `session.listMessages(threadId)` to recover the message gap and `session.state()` to refresh the mode and model.',
  },
  {
    name: 'reconnect',
...
Returns the active session's mode, model, thread, and runtime snapshot for initial UI hydration. The snapshot includes current tasks, cumulative token usage, queued follow-ups, memory progress, and buffering flags.

Pass `{ threadId }` to require that the snapshot belongs to that thread. This doesn't switch the session: a different owned thread returns HTTP `409`, and a missing thread or one owned by another resource returns HTTP `404`. Use [`switchThread(threadId)`](#switchthreadthreadid) to change the active thread before reading its snapshot.

When multiple clients share a session, don't automatically recover from `409` by switching back. Switching changes the shared session's active thread and aborts any current run. Let the user choose when to switch.
...
Every connection, including a reconnect, starts with `display_state_changed` containing the current runtime snapshot and its `threadId`, rather than a replay of historical events. Read cumulative totals from `displayState.tokenUsage`, not from the per-step `usage_update` event.
