...
Pass `scope` to address an independent session over the same `resourceId`. Two sessions with the same `resourceId` and different scopes have their own run loop, thread binding, mode, model, and state, for example one session per git worktree with the worktree path as the scope. The scope travels on every request as a `sessionScope` query parameter.
...
Sends a notification signal to the session. The agent's delivery policy determines whether the notification wakes an idle thread, is summarised, or is persisted for later.
...
A controller can also emit events the SDK doesn't type. `AgentControllerEvent` is the union of `KnownAgentControllerEvent` and `OtherAgentControllerEvent`, whose `type` is `string`, so comparing `event.type` to a literal doesn't narrow the union. Narrow with `isKnownAgentControllerEvent(event)` first:
