...
```typescript
import { agentControllerMessageText, isKnownAgentControllerEvent } from '@mastra/client-js'
...
The `applyUpdate` above is also exported from `@mastra/core/agent-controller`, so a server-side consumer can import it instead of inlining a copy:
...
function handleMessageEvent(event: AgentControllerEvent) {
  if (!isKnownAgentControllerEvent(event)) return
...
A delta whose part the consumer hasn't seen doesn't apply, and `applyUpdate` returns `undefined` for it, so store the result only when it does. Importing it into a browser bundle also pulls in the `node:crypto`-bearing `AgentController` module, which is why the folded `display_state_changed` state stays the preferred path in a browser app. [Message lifecycle](/reference/agent-controller/message-lifecycle) documents the full contract.
