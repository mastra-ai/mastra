...
Gateway-backed agents use the same conversation flow. Continue stored responses with `previous_response_id` or inspect the thread with `conversation_id`. If you need resource scoping, pass it through `requestContext` instead of sending `resource_id` in the request body.
