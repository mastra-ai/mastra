...
Stream a response and keep the stream open until every [background task](/docs/long-running-agents/background-tasks) dispatched during the run completes. The server re-enters the agentic loop on each task completion so the LLM can react to results in the same call. Requires background tasks to be [enabled on the Mastra instance](/reference/configuration#backgroundtasks) and a memory thread; otherwise the call falls through to a plain `stream()`.
...
Resume a suspended agent stream with custom data and keep the stream open until every [background task](/docs/long-running-agents/background-tasks) dispatched during the run completes. Use this to continue execution after a suspension point, such as a workflow suspend within an agent. Requires background tasks to be [enabled on the Mastra instance](/reference/configuration#backgroundtasks) and a memory thread; otherwise the call falls through to a plain `resumeStream()`:
...
Saves auto-publish by default. Pass `publishOnSave: false` to keep the initial version a draft until you call `activateVersion()`.
...
Pass `publishOnSave: false` to keep the version created by an update in draft until you call `activateVersion()`.
...
Clear the active published version. A code-defined agent falls back to its code configuration, and saved versions remain available for restore.

```typescript
await agent.unpublishVersion()
```
