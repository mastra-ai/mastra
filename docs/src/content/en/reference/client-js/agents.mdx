...
<PropertiesTable
  ...
  { name: 'threadId', type: 'string', description: 'Thread to abort.' },
  {
    name: 'clearPendingSignals',
    type: 'boolean',
    isOptional: true,
    defaultValue: 'false',
    description:
      'Clear pending signals across Agents sharing the thread before aborting. Continuations from continueWithMessages() are excluded.',
  },
]}
/>
...
Cancels selected pending signals without stopping the active run. The operation covers pending input across Agents sharing the same memory thread on the server process handling this request.

```typescript
const { cancelledSignalIds } = await agent.cancelPendingSignals({
  resourceId: 'user-123',
  threadId: 'thread-abc',
...
<PropertiesTable
  ...
  { name: 'threadId', type: 'string', description: 'Thread containing the pending signals.' },
  {
    name: 'signalIds',
    type: 'string[]',
    description: 'Between 1 and 1,000 nonempty signal IDs. Duplicate and unknown IDs are ignored.',
  },
]}
/>

Returns `{ cancelledSignalIds: string[] }` with each ID cancelled by this call once. Signals already handed to execution or another owner aren't included. Missing, empty, or oversized ID lists return HTTP 400. Thread ownership restrictions apply to both cancellation routes.

Selective cancellation isn't forwarded to another process. With multiple server instances, send the request to the instance holding the pending input. Cancellation doesn't undo saved messages, state updates, notification records, or acceptance acknowledgements. It doesn't cancel continuations from `continueWithMessages()`.
