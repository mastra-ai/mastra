...
List this agent's signals still queued for a thread, in delivery order. Other agent IDs' inputs are excluded, even on the same thread. Each entry reports its `scope` (`'pre-run'`, `'pending'`, or `'idle'`), the `runId` it is attached to, and the serialized `signal`. Only the in-memory queue of the server process that handles the request is reflected.
...
Remove this agent's queued signals by ID. Pass `signalIds: string[]` containing 1–1,000 non-empty IDs alongside `threadId` and optional `resourceId`. Use a single-element array to remove one signal.
