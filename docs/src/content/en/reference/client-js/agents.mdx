...
The server publishes locally cancelled IDs through its shared PubSub backend so other subscribed instances can remove matching pending copies. Propagation is best-effort and asynchronous. IDs missing on the receiving process aren't published, so send the request to an instance holding a pending copy. Cancellation doesn't undo saved messages, state updates, notification records, or acceptance acknowledgements. It doesn't cancel continuations from `continueWithMessages()`.
