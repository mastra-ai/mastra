...
List the agent's current non-terminal runs from storage. Base agents can return suspended runs, while durable and evented agents can also return running runs.
...
Accepts optional `status`, `threadId`, `resourceId`, `fromDate`, `toDate`, `perPage`, and `page` filters. Date filters are `Date` objects and returned date fields are ISO strings. Omitting `status` returns every discoverable running and suspended run. The `total` value is calculated before pagination. See [`Agent.listRuns()`](/reference/agents/listRuns) for response shapes, ordering, and agent support.
