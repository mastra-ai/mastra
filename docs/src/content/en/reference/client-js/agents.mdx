...
Control the behavior of the model for individual requests by passing model settings:

```typescript
const response = await agent.generate({
  messages: "Write a creative story about space",
  modelSettings: {
...
#### Model settings for streaming

Control model behavior while streaming responses:

```typescript
const response = await agent.stream({
  messages: "Tell me a long, creative story",
  modelSettings: {
    temperature: 0.8,
    topP: 0.9,
    maxTokens: 3000,
