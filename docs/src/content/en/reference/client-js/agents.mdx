...
### `approveToolCallSubscription()`

Approve a pending tool call for a subscribed thread. Use this with `subscribeToThread()` when the resumed chunks should arrive through the existing thread subscription instead of a separate continuation stream.
...
### `declineToolCallSubscription()`

Decline a pending tool call for a subscribed thread. The call returns a JSON acknowledgement. The resumed chunks arrive through the existing thread subscription.
