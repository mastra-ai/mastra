...
Use the client SDK schedule methods to manage persisted agent schedules over the `/api/schedules` routes. For concepts and server-side examples, see [Schedules](/docs/long-running-agents/schedules) and the [`mastra.schedules` reference](/reference/schedules/schedules).
