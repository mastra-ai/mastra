...
<PropertiesTable
  ...
  {
    name: 'signal',
    type: "{ type: 'user' | 'reactive' | 'state' | 'notification' | string; tagName?: string; contents: string | Array<TextPart | FilePart>; attributes?: Record<string, JSONValue>; metadata?: Record<string, unknown>; providerOptions?: ProviderMetadata }",
    description:
