...
Use the client SDK schedule methods to manage persisted agent schedules over the `/api/schedules` routes. For concepts and server-side examples, see [Schedules](/guides/long-running-agents/schedules) and the [`mastra.schedules` reference](/reference/schedules/overview).
...
Both `Agent` (code-defined) and `StoredAgent` instances have methods for managing configuration versions. See the [Editor overview](/guides/editor/overview#versioning) for an introduction to version management.
