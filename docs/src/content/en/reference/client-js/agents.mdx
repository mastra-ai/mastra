...
List this agent's signals still queued for a thread, in delivery order. Other agent IDs' inputs are excluded, even on the same thread. Each entry reports its `scope` (`'pre-run'`, `'pending'`, or `'idle'`), the `runId` it's attached to, and the serialized `signal`. Only the in-memory queue of the server process that handles the request is reflected.

```typescript
const { signals } = await agent.listPendingSignals({
  resourceId: 'user-123',
  threadId: 'thread-abc',
})
```
...
Remove this agent's queued signals by ID. Pass `signalIds: string[]` containing 1 to 1,000 non-empty IDs alongside `threadId` and optional `resourceId`. Use a single-element array to remove one signal.

Returns `{ removedSignalIds: string[] }` with only IDs actually removed, in request order without duplicates. Missing, other-agent, and already-drained signals are skipped. Removal doesn't abort a run or roll back state updates, notification status, or an earlier acceptance result. See [cancellation semantics](/reference/agents/agent#removependingsignalsoptions).

```typescript
const signalIds = signals.slice(0, 2).map(entry => entry.signal.id)
if (signalIds.length > 0) {
  const { removedSignalIds } = await agent.removePendingSignals({
    resourceId: 'user-123',
    threadId: 'thread-abc',
    signalIds,
  })
