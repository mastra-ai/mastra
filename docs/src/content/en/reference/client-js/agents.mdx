...
`subscribeToThread()` returns the underlying `Response` plus a `processDataStream()` helper. The helper reads the subscription stream until the connection closes or the request is aborted. Pass `reconnect: true` to resubscribe when the transport closes, such as after a proxy idle timeout.
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Reconnects the subscription stream after it closes. `true` retries indefinitely with a one second delay.',
  },
]}
/>
