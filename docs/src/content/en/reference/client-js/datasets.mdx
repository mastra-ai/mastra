...
Deletes an experiment through its dataset. The server also deletes the experiment's result records and observability traces, including their spans and trace-linked scores, feedback, metrics, and logs.
...
Returns `Promise<{ success: boolean }>`. A missing experiment or an experiment associated with another dataset returns a `404` response.

## deleteExperiment()

Deletes an experiment by ID without requiring a dataset reference. Use this method for experiments orphaned by dataset deletion. The server also deletes the experiment's result records and observability traces, including their spans and trace-linked scores, feedback, metrics, and logs.
