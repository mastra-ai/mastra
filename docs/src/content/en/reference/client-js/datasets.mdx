...
Permanently scrubs an item's content from every dataset version and linked experiment result, including result tags and comments, while preserving version history, experiment counters, and review status.
...
Returns `Promise<{ success: boolean }>`. The operation is idempotent and can't be undone. See [`dataset.purgeItem()`](/reference/datasets/purgeItem) for the complete erasure behavior.
