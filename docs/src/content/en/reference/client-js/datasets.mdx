...
Deletes an experiment through its dataset. The server deletes the experiment's result records and attempts to delete its observability traces, including their spans and trace-linked signals. If storage doesn't support observability or trace deletion, the server logs a warning and leaves the traces in place.
...
Deletes an experiment by ID without requiring a dataset reference. Use this method for experiments orphaned by dataset deletion. The server deletes the experiment's result records and attempts to delete its observability traces. If storage doesn't support observability or trace deletion, the server logs a warning and leaves the traces in place.
