...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description: "Metadata for each vector. Fields matching table column names are mapped to those columns.",
    },
    {
      name: "ids",
      type: "string[]",
    ...
    },
    {
      ...
      isOptional: true,
      description: "Optional mapping of metadata field names to table column names for custom schemas",
    },
  ]}
/>
...
The `upsert()` method detects table schemas and maps metadata fields to table columns.
...
// Name-based column mapping - fields matching column names are mapped
...
- Use `columnMapping` parameter for explicit field-to-column mapping
- Non-mapped fields are stored in the JSONB metadata column
- Schema detection occurs on first upsert and is cached per table
