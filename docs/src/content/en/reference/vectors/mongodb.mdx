...
Provisions an Atlas Search (BM25/full-text) index on the collection backing an index. This enables full-text search via `textQuery()` and hybrid search via `hybridQuery()`.
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    isOptional: true,
    defaultValue: '${collectionName}_search_index',
    description: 'Name for the Atlas Search index. Defaults to `${collectionName}_search_index`.',
  },
]}
/>
...
Runs a full-text (BM25) search against an Atlas Search index. The search index must be created with `createSearchIndex()` first.
