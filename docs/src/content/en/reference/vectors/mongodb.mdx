...
<PropertiesTable
  ...
  },
  {
    ...
    defaultValue: 'field',
    description:
      "`'field'` (default) projects the managed `metadata`/`document` fields. `'document'` returns the full source document as `metadata` — use for bring-your-own operational collections whose documents have their own shape. The embedding field is omitted from `metadata` by default (to avoid payload bloat); set `includeVector: true` to retain it in `metadata` and also expose it as a top-level `vector`.",
  },
]}
/>
...
- When `fields` is provided **without** an explicit `searchIndexName`, the field-mapped index is created under a **distinct** default name (`${collectionName}_search_fields_index`) so it does not collide with — and get silently ignored by — a managed collection's auto-created dynamic index. This distinct index is persisted as the text-search index, so `textQuery()`/`hybridQuery()` use the restricted mapping automatically.
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    isOptional: true,
    defaultValue: '${collectionName}_search_index (or ${collectionName}_search_fields_index when `fields` is given)',
    description:
      'Name for the Atlas Search index. When `fields` is provided and this is omitted, a distinct default name is used so the field mapping is not shadowed by the auto-created dynamic index.',
  },
]}
/>
...
Runs a hybrid search that fuses vector similarity and full-text results using MongoDB's server-side `$rankFusion` (requires MongoDB >= 8.1). A full-text search index must exist: it is auto-created for managed indexes, but for a bring-your-own collection you must call `createSearchIndex()` first (opt-in).
...
**Note:** `hybridQuery()` requires MongoDB >= 8.1 for the `$rankFusion` stage. If you're running an older version, use `query()` and `textQuery()` separately and merge the results client-side.
...
- Full-text and hybrid search on a BYO collection are **opt-in**: no full-text index is auto-created, so call `createSearchIndex()` before `textQuery()`/`hybridQuery()`
