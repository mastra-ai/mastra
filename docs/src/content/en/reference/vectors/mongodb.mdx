...
**Relationship with the auto-created dynamic index:** `createIndex()` already provisions a _dynamic_ full-text index named `${collectionName}_search_index` (covering all string fields). `createSearchIndex()` is therefore only needed when you want a **field-restricted** mapping or a **custom index name**:

- When `fields` is provided **without** an explicit `searchIndexName`, the field-mapped index is created under a **distinct** default name (`${collectionName}_search_fields_index`) so it does not collide with — and get silently ignored by — the auto-created dynamic index. This distinct index is persisted as the text-search index, so `textQuery()`/`hybridQuery()` use the restricted mapping automatically.
...
Runs a hybrid search that fuses vector similarity and full-text results using MongoDB's server-side `$rankFusion` (requires MongoDB >= 8.1). The search index must be created with `createSearchIndex()` first.
...
- **Bring-your-own index** (created with `collectionName`): drops only the Atlas vectorSearch index. The caller's operational collection and its documents are **preserved** — this store never drops a collection it did not create.
