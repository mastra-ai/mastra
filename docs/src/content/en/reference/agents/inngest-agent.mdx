...
<PropertiesTable
  content={[
  {
    ...
    type: 'Agent',
    isOptional: false,
    description: 'The Agent to wrap with Inngest durable execution. Agent methods (e.g., `generate()`, `listTools()`, `getMemory()`) delegate to this agent via a Proxy.',
  },
  {
    name: 'inngest',
    type: 'Inngest',
    isOptional: false,
    description: 'The Inngest client instance. Used to send workflow events and, in SDK v4, publish realtime stream events.',
  },
  {
    name: 'id',
    type: 'string',
  ...
  },
  {
    ...
    type: 'PubSub',
    isOptional: true,
    description: 'PubSub instance for streaming events. The default `InngestPubSub` uses Inngest Realtime, which works across processes.',
...
```typescript
const { runId, messageId, workflowInput, threadId, resourceId } =
  await durableAgent.prepare('Summarize the document', {
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      "Overrides the agent's default instructions for this run.",
  },
  {
    name: 'context',
    type: 'ModelMessage[]',
