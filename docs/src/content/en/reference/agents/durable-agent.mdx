...
#### `streamUntilIdle(messages, options?)`

Streams until all background tasks complete and the agent is idle. This is the durable variant of the regular [`Agent.streamUntilIdle()`](/reference/streaming/agents/streamUntilIdle). Accepts the same options as [`stream()`](#streamoptions) plus `maxIdleMs`.

```typescript
const { output, runId, cleanup } = await durableAgent.streamUntilIdle('Run the analysis', {
  maxIdleMs: 30000,
})
