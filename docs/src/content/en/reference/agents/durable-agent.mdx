...
Discovers all runs stuck in `running` status for this agent and re-drives them from the last persisted snapshot. Returns a summary of what was recovered.
...
```typescript
const { output, cleanup } = await durableAgent.recover(runId, {
  onChunk: chunk => console.log(chunk),
