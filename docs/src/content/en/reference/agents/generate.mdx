...
The supervisor pattern using `generate()` and `stream()` is the **recommended approach for coordinating multiple agents**, replacing the older `.network()` API. It provides better control, type safety, and monitoring through iteration hooks, delegation hooks, and completion scoring. See the [Supervisor Pattern example](#supervisor-pattern-with-delegation-and-completion) below and the [Agent Networks comparison](/docs/agents/networks#supervisor-pattern-comparison) for migration details.
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        "Completion scoring configuration that validates whether the task is complete. Uses Mastra's evaluation scorers to automatically check if the agent's response satisfies the completion criteria.",
      properties: [
        {
          parameters: [
            {
        ...
        },
        {
          parameters: [
            {
              name: "onComplete",
              type: "(result: CompletionRunResult) => void | Promise<void>",
              isOptional: true,
              description: "Callback called when completion check finishes. Receives the completion result with individual scorer scores.",
            },
          ],
        },
        {
    ...
    },
    {
      ...
      isOptional: true,
      description:
        "Configuration for sub-agent and workflow delegation. Use this to control and monitor when the agent delegates tasks to other agents or workflows, including the ability to modify, reject, or bail out of delegations.",
      properties: [
        {
          parameters: [
            {
              ...
              isOptional: true,
              description: "Called before delegating to a sub-agent or workflow. Use this to modify the delegation parameters or reject the delegation entirely.",
            },
          ],
        },
        {
          parameters: [
            {
              ...
              type: "(context: DelegationCompleteContext) => void | Promise<void>",
              isOptional: true,
              description: "Called after a sub-agent or workflow delegation completes. Use this to process the result or call context.bail() to stop further execution.",
            },
          ],
        },
        {
          parameters: [
            {
              ...
              isOptional: true,
              defaultValue: "'first'",
              description: "Strategy for handling bail() calls. 'first' stops on the first bail, 'all' requires all delegations to bail before stopping.",
            },
          ],
        },
        {
          parameters: [
            {
              name: "contextFilter",
              type: "ContextFilterConfig",
              isOptional: true,
              description: "Configuration for filtering messages before passing them to sub-agents. Use this to control what context information is shared with delegated tasks.",
            },
          ],
        },
      ],
...
### Supervisor Pattern with Delegation and Completion

This example demonstrates using the supervisor pattern features for coordinating sub-agents with intelligent monitoring and control.

```typescript
import { createScorer } from '@mastra/core/evals';

// Create completion scorers
const taskCompleteScorer = createScorer({
  id: 'task-complete',
  name: 'Task Completeness',
  description: 'Checks if the task has been fully completed',
  ...
  return hasAnalysis && hasRecommendations ? 1 : 0.5;
});

const result = await supervisorAgent.generate(
  'Research AI in education and provide recommendations',
  {
    maxSteps: 10,

    // Completion Scoring - Validate task completion
    completion: {
    ...
    // Iteration Hooks - Monitor progress after each step
    ...
    delegation: {
      // Modify or reject delegations before they happen
      onDelegationStart: async (context) => {
        console.log(`Delegating to: ${context.primitiveId}`);

        // Add extra instructions for specific sub-agents
        ...
        if (context.iteration > 8) {
          return {
            proceed: false,
            rejectionReason: 'Max iterations reached. Synthesize findings.',
          };
        }
      ...
      // Process results and bail out if needed
      ...
      contextFilter: {
        ...
        includeToolMessages: true,      // Include tool calls
        filter: (message) => {
          // Custom filter: remove sensitive data
