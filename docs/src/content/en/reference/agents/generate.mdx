    ...
    properties: [
      {
        ...
        parameters: [
          ...
          {
            name: 'approvalPersistence',
            type: "'full' | 'minimal'",
            isOptional: true,
            defaultValue: "'full'",
            description:
              "Controls persisted state for pre-execution tool approval. Use `'minimal'` to store an approval-only checkpoint that can continue through `approveToolCallGenerate()` or `declineToolCallGenerate()`. Runtime `suspend()` calls continue to use full persistence. See [Agent approval](/docs/agents/agent-approval#choose-approval-snapshot-persistence).",
          },
          {
