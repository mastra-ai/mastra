...
<PropertiesTable
  ...
  {
    name: 'usedFallbackValue',
    type: 'boolean',
    description:
      "True when `object` is the configured `fallbackValue`, substituted because the model output failed schema validation, or the separate structuring model failed, under `errorStrategy: 'fallback'`.",
  },
  {
    name: 'toolCalls',
    type: 'ToolCallChunk[]',
...
The `image` value, or `data` for a `type: 'file'` part, should be an absolute URL, a data URL, or base64 content. The model can't see an attachment given as a relative path such as `/api/images/foo.png`, so resolve it to an absolute URL before sending. The agent replaces it with a placeholder and logs a warning. It does the same for base64 content that can't be decoded or isn't the image or PDF its media type says, and for any attachment it can't download, instead of failing. See [attachment download failures](/docs/agents/processors#recover-from-attachment-download-failures).
