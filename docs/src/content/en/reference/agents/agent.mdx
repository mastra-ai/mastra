...
Use `cancelQueuedMessages({ resourceId, threadId, signalIds })` to remove specific queued messages before execution begins. It returns the signal IDs that were actually cancelled. It does not cancel an already-running message, messages owned by another process, or provide crash-durable delivery.
