...
Sends a data part to a thread. Data parts are streamed to any subscriber but never included in the LLM prompt and never wake the agent. They are always persisted to storage.
