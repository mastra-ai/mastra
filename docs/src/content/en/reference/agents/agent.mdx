...
`queueMessage()` accepts the same `message` and `options` shape as `sendMessage()` and returns `{ accepted: Promise<SendAgentSignalAccepted>, signal: CreatedAgentSignal, persisted?: Promise<void> }`, with the same `accepted` semantics as `sendMessage()`. Pass an optional `queueOwnerId` to group local queued messages for observation and cancellation. The owner ID is local metadata: it is neither serialized with the message nor an authorization mechanism.

Use `subscribeQueuedMessages({ resourceId, threadId }, listener)` to observe all locally pending messages on the shared thread, including messages submitted by other Sessions or Agents using the same runtime and PubSub instance. The listener receives a synchronous initial `{ count }` snapshot, then only receives later snapshots when its count changes. Its unsubscribe function is idempotent and doesn't cancel queued messages. Pass an optional `queueOwnerId` to restrict observation to that owner's messages submitted by the calling Agent.
...
const unsubscribe = agent.subscribeQueuedMessages(scope, ({ count }) => {
  console.log(`${count} messages are pending`)
})
