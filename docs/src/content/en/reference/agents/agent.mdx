...
SystemMessage types: string | string[] | CoreSystemMessage | CoreSystemMessage[] | SystemModelMessage | SystemModelMessage[]`,
  ...
  },
  {
    ...
    isOptional: true,
    description:
      "Default response cache configuration for `agent.stream()` and `agent.generate()`. Set to `true` to use the Mastra instance's server cache, or pass an options object to customize the TTL (seconds), scope, custom cache implementation, or bust behavior. Per-call options override these defaults; pass `responseCache: false` per call to opt out. See [Response caching](/docs/agents/response-caching).",
  },
]}
/>
