...
```typescript title="src/mastra/agents/authenticated-agent.ts"
import { Agent } from "@mastra/core/agent";
import { z } from "zod";
...
export const agent = new Agent({
  ...
  instructions: "You are a helpful assistant with access to user context.",
  model: "openai/gpt-5.1",
...
// When calling the agent, the framework validates the requestContext
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        "A Zod schema defining the expected structure of request context values. When provided, the framework validates that the required context is present before agent execution. The validated requestContext is passed to all execution methods (generate, stream, etc.) and can be accessed by tools, workflows, and dynamic configuration functions.",
    },
  ]}
/>
