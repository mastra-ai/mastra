...
Returns `{ accepted: true, runId: string, signal: CreatedAgentSignal, persisted?: Promise<void>, outcome: Promise<SendAgentSignalOutcome> }`. `persisted` is only present for `persist` behavior and resolves when Mastra finishes writing the message to memory. `outcome` resolves once Mastra decides what to do with the message: `{ action: 'wake', output }` when this process starts (or wins the lease to start) the agent run, or `{ action: 'deliver' | 'persist' | 'discard' }` otherwise. On the `wake` action, `output` is the agent stream, or `undefined` when another process owns the run.
...
`queueMessage()` accepts the same `message` and `options` shape as `sendMessage()` and returns `{ accepted: true, runId: string, signal: CreatedAgentSignal, persisted?: Promise<void>, outcome: Promise<SendAgentSignalOutcome> }`.
...
Returns `{ accepted: true, runId: string, signal: CreatedAgentSignal, persisted?: Promise<void>, outcome: Promise<SendAgentSignalOutcome> }`. `persisted` is only present for `persist` behavior and resolves when Mastra finishes writing the signal to memory. `outcome` resolves once Mastra decides what to do with the signal: `{ action: 'wake', output }` when this process starts (or wins the lease to start) the agent run, or `{ action: 'deliver' | 'persist' | 'discard' }` otherwise. `action` mirrors the winning `behavior` from `ifActive`/`ifIdle`. On the `wake` action, `output` is the agent stream, or `undefined` when another process owns the run.
...
Returns `{ accepted: true, runId: string, signal: CreatedAgentSignal, persisted?: Promise<void>, outcome: Promise<SendAgentSignalOutcome>, skipped?: false }` when Mastra accepts new state. Returns `{ accepted: true, skipped: true, reason: 'unchanged' }` when the same `cacheKey` and mode are already current for the state lane. `outcome` resolves once Mastra decides what to do with the signal: `{ action: 'wake', output }` when this process starts (or wins the lease to start) the agent run, or `{ action: 'deliver' | 'persist' | 'discard' }` otherwise. On the `wake` action, `output` is the agent stream, or `undefined` when another process owns the run.
