...
### `sendDataPart(dataPart, target)`

Sends a data part to a thread. Data parts are streamed to any subscriber but never included in the LLM prompt and never wake the agent. They are always persisted to storage as assistant `data-*` parts. During an active response, Mastra appends the part to the current assistant message. When the thread is idle or no current assistant message is available, Mastra writes a new assistant message that contains only the data part.
...
<PropertiesTable
  content={[
  {
    name: 'dataPart.type',
    type: '`data-${string}`',
    description: 'The data part type. Must start with `data-`.',
  },
  {
    name: 'dataPart.data',
    type: 'unknown',
    description: 'The payload. This is the exact value subscribers and reloaded UI messages receive.',
  },
  {
  ...
  },
  {
    ...
    type: 'string',
    description: 'Thread ID to target.',
  },
]}
/>

Returns `{ accepted: true, persisted: Promise<void> }`. Await `persisted` to confirm the data part has been written to storage.

### `subscribeToThread(options)`

Subscribes to raw stream chunks for a memory thread. Use this before calling `sendSignal()` or `sendDataPart()` when you need to render stream output, observe signal echoes, or abort the active run.
