...
Cancellation is local to this runtime and PubSub instance, with no requests sent to remote owners. It leaves continuations from `continueWithMessages()`, saved messages, state updates, and notification records unchanged. Accepted acknowledgements remain settled.
