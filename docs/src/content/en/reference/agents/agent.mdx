            ...
            properties: [
              {
                ...
                parameters: [
                  ...
                  },
                  {
                    ...
                    isOptional: true,
                    description:
                      'Options for a new stream, or request context for persistence. Pass an async factory to defer setup until Mastra wakes the thread or persists the message. Active delivery and discard do not evaluate the factory. The top-level `resourceId` and `threadId` still control memory identity.',
                  },
                  {
                    name: 'attributes',
                    type: 'Record<string, string | number | boolean>',
...
Set `ifIdle.behavior` to `wake` and pass an async `ifIdle.streamOptions` factory when stream setup requires deferred work:
...
agent.sendMessage('Continue with the next step.', {
  ...
  ifIdle: {
    ...
    streamOptions: async () => ({
      maxSteps: 3,
      requestContext: new RequestContext([['locale', 'en-US']]),
    }),
  },
})
```
            ...
            properties: [
              {
                ...
                parameters: [
                  ...
                  },
                  {
                    ...
                    isOptional: true,
                    description:
                      'Options for a new stream, or request context for persistence and state work. Pass an async factory to defer setup until wake, persistence, or state processing wins delivery policy. Active delivery and discard do not evaluate the factory. The top-level `resourceId` and `threadId` still control memory identity.',
                  },
                  {
                    name: 'attributes',
                    type: 'Record<string, string | number | boolean>',
                ...
                parameters: [
                  {
                    ...
                    isOptional: true,
                    description:
                      'Options for an immediate notification signal to wake an idle thread or persist without waking. Pass an async factory to defer setup until wake or persistence wins delivery policy. Active delivery and discard do not evaluate the factory. The top-level `resourceId` and `threadId` still control memory identity.',
                  },
                ],
              },
            ],
