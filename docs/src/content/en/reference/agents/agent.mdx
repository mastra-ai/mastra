...
SystemMessage types: string | string[] | CoreSystemMessage | CoreSystemMessage[] | SystemModelMessage | SystemModelMessage[]`,
  ...
  {
    name: 'toolPayloadProjection',
    type: 'ToolPayloadProjectionPolicy',
    isOptional: true,
    description:
      'Shared policy for projecting tool payloads before display streams or user-visible transcript messages receive them. Use per-tool `transform` on `createTool()` for tool-local rules.',
  },
  {
    name: 'workflows',
