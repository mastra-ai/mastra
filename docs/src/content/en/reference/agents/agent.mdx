...
The `generate()` and `stream()` methods support orchestration options that enable multi-step execution with routing and validation.

### routing

Enable routing agent orchestration. When enabled, a routing agent decides which primitive (agent/workflow/tool) handles each step.
...
Enable validation/completion checking. When configured, the agent loops until validation passes or `maxSteps` is reached.
...
### Combining routing and validation

For full orchestration, combine both options:

```typescript
await agent.stream('Build a complete feature', {
  routing: {
    verboseIntrospection: true,
  },
  validation: {
    scorers: [testsScorer, lintScorer],
    strategy: 'all',
  },
  maxSteps: 20,
  memory: {
...
:::note
Memory is required when using `routing` or `validation` options. Configure memory on the agent or provide it in options.
:::
