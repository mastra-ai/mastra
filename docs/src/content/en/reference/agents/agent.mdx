...
`subscribeThreadEvents()` is distinct from `subscribeToThread()`, which streams agent output. It does not currently emit composite thread state, individual message lifecycle events, run events, or approval events; those may be added as separate event types in the future.
