...
The `.network()` primitive has been deprecated and will be removed in a future major release. Use [supervisor agents](/guides/agents/supervisor-agents) with `agent.stream()` or `agent.generate()` instead. See the [migration guide](/guides/migrations/network-to-supervisor) to upgrade.
