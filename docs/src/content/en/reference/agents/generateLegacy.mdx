...
Scorer judges with V1 model adapters call `generateLegacy()`. They don't receive the coordinated `StreamErrorRetryProcessor` budget available to V2+ scorer judges. Use a V2+ judge model when you need error-processor retries. The legacy `maxRetries` option remains separate and defaults to `2`.
