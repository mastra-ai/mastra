...
Channels connect agents to messaging platforms. Configure them via the `channels` property on the `Agent` constructor. The object you pass is a `ChannelConfig`. See the [Channels guide](/docs/agents/channels) for concepts and platform setup instructions.
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "When the agent's text is posted to the channel. `'progressive'` (default) posts text as the run produces it — one message per text block, or a live-updating message when `streaming` is enabled. `'final'` accumulates all of the run's text and posts it once when the run ends. `'final'` renders text statically even under `streaming: true`, so streaming tool widgets are not combined with `'final'` text.",
  },
  {
    name: 'toolDisplay',
...
`textDisplay` controls _when_ the agent's text is posted, not what is posted —
both modes post all of the run's text. The default `'progressive'` posts text as
the run produces it: one message per text block when `streaming` is off, or a
single live-updating message when `streaming` is on.

`'final'` accumulates every text block across the run and posts them once, joined
with a blank line, when the run ends. Use it when you want a single clean answer
in the channel instead of a play-by-play. `'final'` is inherently non-streaming
for text, so it renders statically even under `streaming: true`.

Combine `'final'` with `toolDisplay: 'hidden'` to post only the final answer —
no per-step narration, no tool cards:

```typescript title="src/mastra/agents/final-answer.ts"
import { Agent } from '@mastra/core/agent'
import { createSlackAdapter } from '@chat-adapter/slack'
...
These render options are also overridable for a single run via the
`requestContext['channel.render']` key — see
[Per-run render overrides](/docs/agents/channels#per-run-render-overrides) in the
channels guide.
