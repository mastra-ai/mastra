...
Channel threads belong to the agent that created them. New threads get a `channel_agentId` metadata key set to the owning agent's id, and thread lookups are scoped by `(platform, external thread id, channel id, agent id)`. Agent controllers stamp the controller id instead. Agents that share one storage instance never resolve to each other's threads or subscriptions.

Threads created before `channel_agentId` existed match through a legacy fallback that only considers threads without the key. The first agent that touches such a thread claims it by writing its own id into the metadata. The claim keeps the thread id and stored `resourceId` and preserves all other metadata keys. A thread claimed by a different agent never matches, so the agent creates a fresh thread for that conversation instead.
