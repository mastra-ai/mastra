...
buttons. With a `toolDisplay` function, the `approval` event is passed to your
function in both streaming and static modes; return `{ kind: 'post', message }`
to replace the built-in card (for example, with a localized message). Returning
`undefined`, an empty message, or `{ kind: 'stream' }` falls back to the
built-in card so the approval stays actionable.
