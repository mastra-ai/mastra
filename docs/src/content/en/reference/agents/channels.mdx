...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Called when the bot receives an action (e.g. button click). Receives a `defaultHandler` that routes built-in `tool_approve:<toolCallId>` / `tool_deny:<toolCallId>` action IDs and returns `{ kind: "approved" | "denied" | "unknown" }` describing what it did.',
  },
  {
    name: 'onReaction',
