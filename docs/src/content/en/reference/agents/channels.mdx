...
The hook runs after `resolveResourceId`, so the resolved owner is available on the context. Like `resolveResourceId` it runs only when a new thread is created: reused threads keep their stored id and never call the hook. The returned id must be unique across the memory store — reusing an existing thread's id would write the conversation into that thread. Return `ctx.defaultThreadId` to keep the built-in behavior.
