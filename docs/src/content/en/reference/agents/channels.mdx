...
Returning `false` or `undefined` from `ActionHandler` / `ReactionHandler` falls through to the default handler. Any other return value (including the awaited result of `defaultHandler()`) is treated as "the override owned this event".
