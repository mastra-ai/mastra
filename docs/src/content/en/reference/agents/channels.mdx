...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    isOptional: true,
    defaultValue: 'true (false for Slack)',
    description:
      'Show the platform typing indicator while the agent is generating text or running tools. Set to `false` to suppress typing entirely — useful when a live streaming widget (e.g. `toolDisplay: "grouped"` in Slack) already conveys progress. Slack defaults to `false`; other adapters default to `true`.',
  },
]}
/>
