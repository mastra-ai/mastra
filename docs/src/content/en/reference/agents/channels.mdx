...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      '**Deprecated** — use `toolDisplay` instead. When `toolDisplay` is not set, `cards: true` maps to `toolDisplay: "cards"` and `cards: false` maps to `toolDisplay: "text"` (with a one-time deprecation warning).',
  },
  {
    name: 'cors',
    type: 'CorsOptions',
...
the active streaming widget, or `undefined` to fall back to the built-in
renderer for that mode.
