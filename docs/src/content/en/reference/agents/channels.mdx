...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Called when the bot receives an action (e.g. button click). Receives a `defaultHandler` that routes built-in `tool_approve:<toolCallId>` / `tool_deny:<toolCallId>` action IDs. Custom action IDs are returned to the override as `{ kind: "unknown", actionId }` so user code can branch on them.',
  },
  {
    name: 'onReaction',
    ...
    isOptional: true,
    description:
      'Called when the bot receives a reaction (emoji) event. Default is a no-op. Pair with `channels.approveTool` / `denyTool` to drive approvals via reactions on platforms without buttons.',
  },
]}
/>
