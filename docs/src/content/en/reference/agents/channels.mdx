...
<PropertiesTable
  ...
  },
  {
    ...
    defaultValue: 'true',
    description:
      'Controls whether the channel exposes channel-specific tools (`add_reaction`, `remove_reaction`) via `AgentChannels.getTools()`. Channel tools are never added to the agent automatically — pass them explicitly via `tools: { ...channels.getTools() }` if the agent should react to channel messages. Set to `false` to disable the channel tools entirely, e.g. for models that do not support function calling.',
  },
  {
    name: 'state',
    type: 'StateAdapter',
