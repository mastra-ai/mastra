...
Channels connect agents to messaging platforms. Configure them via the `channels` property on the `Agent` constructor. The object you pass is a `ChannelConfig`. See the [Channels overview](/guides/capabilities/channels/overview) for concepts and platform setup instructions.
...
`ctx.requestContext` is the [`RequestContext`](/guides/server/request-context) for the run this message is about to start, built fresh for every message. Write to it before calling `defaultHandler` and the value reaches the run, alongside the channel entries Mastra adds afterward:
...
- [Channels overview](/guides/capabilities/channels/overview): Concepts, quickstart, and platform setup
