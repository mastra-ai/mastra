...
`channel.status` is best-effort. Visible status requires the adapter to implement Mastra status methods such as `setStatus(threadId, text)` and `clearStatus(threadId)`. Adapters without these methods no-op. For Slack assistant status, use a Slack adapter version or wrapper that implements these methods. Mastra core does not call Slack-specific status methods directly. Mastra clears channel status when the stream finishes or errors.
...
<PropertiesTable
  content={[
  {
    ...
    description:
      'Set the active channel status through an adapter that implements `setStatus(threadId, text)` and return a handle that can clear that status if it is still active.',
  },
  {
    name: 'clear',
    type: '() => Promise<void>',
    description:
      'Clear the active channel status through `clearStatus(threadId)` when the adapter implements it. If only `setStatus` exists, Mastra clears by setting an empty status.',
  },
]}
/>
