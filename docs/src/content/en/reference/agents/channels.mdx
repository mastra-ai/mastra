...
description: "Channels connect agents to messaging platforms. Configure them via the channels property on the Agent constructor."
packages:
  - "@mastra/core"
---
...
`approved` / `denied`) and a `ToolDisplayContext` (`{ mode, platform }`); return
`{ kind: 'post', message }` for a discrete post/edit, `{ kind: 'stream', chunk }`
to push into the active streaming widget, or `undefined` to skip rendering that
event.
...
Approve/deny prompts (`requireApproval`) render as a separate built-in card in
every string mode, because inline task entries can't carry interactive buttons.
With a `toolDisplay` function, the `approval` event is passed to your function
in both streaming and static modes; return a non-blank `{ kind: 'post', message }`
to replace the built-in card with your own message (for example, a localized
one). Returning `undefined`, a blank message, or `{ kind: 'stream' }` falls back
to the built-in card so the approval stays actionable.

Once someone clicks a button, the same message is edited to its resolved state
and your function receives `approved` or `denied` for that `toolCallId`. This
gives your renderer ownership of the card's final text, so a localized approval
prompt resolves into a localized result instead of the built-in English one.
Both events carry `byUser`, the display name of whoever decided, except in a
direct message where it is `undefined`. The same fallback rule applies as for
`approval`: returning `undefined`, a blank message, or `{ kind: 'stream' }`
falls back to the built-in resolved card, because that edit is what removes the
...
toolDisplay: event => {
  ...
  if (event.kind === 'result') {
    return { kind: 'post', message: `✓ ${event.displayName} completado` }
  }

  // `approval` falls back to the built-in card; `running` and `error` are
  // suppressed, because `undefined` only falls back for approval-related events.
  return undefined
}
```

An approved tool then runs and emits a `result` event, which edits the same
message again, so handle `result` too or the resolved card is never updated. A
denied tool does not run, so the `denied` message is the card's resting state.
