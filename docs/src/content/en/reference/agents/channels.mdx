...
<PropertiesTable
  ...
  },
  {
    ...
    defaultValue: 'true',
    description:
      "Whether to automatically add the built-in `ChatChannelProcessor` input processor. Set to `false` to skip it entirely. For most customization, instead register your own input processor with `id: 'chat-channel-context'`, which shadows the built-in one.",
  },
  {
    name: 'chatOptions',
