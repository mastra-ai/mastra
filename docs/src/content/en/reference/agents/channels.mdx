...
<PropertiesTable
  ...
  },
  {
    ...
    defaultValue: 'true',
    description:
      "Controls whether the channel exposes channel-specific tools (`add_reaction`, `remove_reaction`) via the deprecated `AgentChannels.getTools()`. Channel tools are no longer auto-injected into the agent's resolved toolset — to let the agent send messages or add reactions on a channel run, pass the chat tools to your agent explicitly. Set to `false` to disable the channel tools entirely, e.g. for models that do not support function calling.",
  },
  {
    name: 'state',
    type: 'StateAdapter',
