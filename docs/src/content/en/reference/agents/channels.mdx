...
`ctx.requestContext` is the [`RequestContext`](/docs/server/request-context) for the run this message is about to start, built fresh for every message. Write to it before calling `defaultHandler` and the value reaches the run, alongside the channel entries Mastra adds afterward. Use it to stamp a tenant so per-tenant credentials resolve for channel runs:

```typescript
onDirectMessage: async (thread, message, defaultHandler, ctx) => {
  ctx.requestContext.set('user', { id: linkedUserId, organizationId: linkedOrgId })
  await defaultHandler(thread, message)
}
```

`ctx` is not optional. Mastra always passes it. Handlers that declare fewer parameters still satisfy the type, so a three-parameter handler needs no change, but code that calls a `ChannelHandler` value directly must pass all four arguments.
