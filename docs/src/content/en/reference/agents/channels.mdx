...
- **A function** `(thread, message, defaultHandler, ctx) => Promise<void>`: wraps or replaces the default
...
Handlers receive a 4th `ctx` argument that exposes the Mastra thread ids and lets you abort the currently active run on this thread. This is useful for "stop" / "cancel" commands:
...
`ctx.abortSignal` fires whenever the active run on this thread is aborted — from this handler, another handler on the same thread, `agent.abortRunStream()`, or `channels.close()` — so you can pass it to any async work the handler kicks off.
...
interface ChannelHandlerContext {
  /** The Mastra thread id resolved for this chat thread. */
  threadId: string
  /** The resource id used for this thread (typically `${platform}:${userId}`). */
  resourceId: string
  /** Fires when the active run on this thread is aborted, from anywhere. */
  abortSignal: AbortSignal
  /** Abort the currently active run on this thread. Returns true if a run was aborted. */
  abort: (reason?: string) => boolean
  /** The active runId on this thread, or `undefined` if idle. */
