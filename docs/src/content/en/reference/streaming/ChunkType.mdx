...
Contains a provider-specific content block that does not map to any other standardized chunk type. Emitted by providers on the AI SDK v7 specification.
...
Contains raw data from the provider. Content types Mastra does not recognize are also emitted as `raw` chunks rather than being discarded. Raw chunks only appear when `includeRawChunks` is enabled.
