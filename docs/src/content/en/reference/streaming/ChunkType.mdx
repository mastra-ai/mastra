...
Emitted when a tool call is dispatched as a [background task](/docs/agents/background-tasks). All payloads include `taskId`, `toolName`, and `toolCallId` so they can be correlated with the originating tool call.
