...
Emitted when a tool call is dispatched as a [background task](/guides/long-running-agents/background-tasks) and `streamUntilIdle()` is used.
...
Emitted on every evaluation of an agent [goal](/guides/long-running-agents/goals). Consumers use this to render judge progress and the result mid-run. A goal that has no judge model configured produces no `goal` chunk.
