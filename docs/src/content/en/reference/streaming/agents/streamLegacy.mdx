...
**Deprecated**: This method is deprecated and only works with V1 models. For V2 models, use the new [`.stream()`](./stream) method instead. See the [migration guide](/guides/v0/migrations/vnext-to-standard-apis) for details on upgrading.
...
The new `.stream()` method offers enhanced capabilities including AI SDK v5 compatibility, better structured output handling, and improved callback system. See the [migration guide](/guides/v0/migrations/vnext-to-standard-apis) for detailed migration instructions.
