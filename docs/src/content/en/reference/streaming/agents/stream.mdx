        ...
        parameters: [
          {
            ...
            defaultValue: '[]',
            description:
              "Signal types to omit from this caller's fullStream after experimental transforms. Does not filter model context, storage, aggregates, or other subscribers. See Signal visibility below.",
          },
          {
            name: 'maxSteps',
            type: 'number',
...
Signals, including reactive reminders, appear in `fullStream` by default. Set `excludeSignals` to omit selected signal chunks from your stream:
...
The same policy applies to `resumeStream()`, `untilIdle` continuations, and the deprecated `streamUntilIdle()` and `resumeStreamUntilIdle()` methods, including durable agents. This option is available on in-process core streaming and subscription APIs, not HTTP or client-js request options, `generate()`, or legacy `streamLegacy()` APIs.
