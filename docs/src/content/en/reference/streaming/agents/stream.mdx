    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            isOptional: true,
            description:
              "Defaults to `false`. When `true`, a tool call whose arguments have finished streaming starts executing immediately, instead of waiting for the model to finish the whole step. Tool results and message history keep their model-call order either way. Only server-side tools are started early: approval-gated, suspendable, provider-executed, client-side, missing and inactive tools keep the default behaviour, as does any run using `toolCallConcurrency.strategy: 'called'` (the full set of called tools is not known until the step finishes). Eager executions share the configured `toolCallConcurrency` limit, and aborting the run stops any further dispatch. One consequence of starting early: if the model stream fails after a tool call was already dispatched, that attempt is discarded and retried, but the tool has already begun — it is aborted rather than un-run, and its result is discarded with the attempt, so only enable this for tools where an aborted early start is acceptable. Not supported by durable agents, which reject the option.",
          },
          {
            name: 'providerOptions',
