...
Exceeding either budget fails with a `MastraTimeoutError`, which carries a `timeoutType` of `'total'` or `'step'`. The two budgets behave differently when the agent is configured with fallback [`models`](/reference/agents/agent):

- A `totalMs` timeout ends the run immediately and does not try the next model, because it is a hard deadline for the run as a whole.
- A `stepMs` timeout is not retried against the same model, but does advance to the next model, which makes it a way to fail over from a slow provider.
