    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            defaultValue: "'full'",
            description:
              "Controls persisted state for pre-execution tool approval. Use `'minimal'` to store an approval-only checkpoint that can continue through `approveToolCall()` or `declineToolCall()`. Runtime `suspend()` calls continue to use full persistence. See [Agent approval](/docs/agents/agent-approval#choose-approval-snapshot-persistence).",
          },
          {
