...
The supervisor pattern using `stream()` and `generate()` is the **recommended approach for coordinating multiple agents**, replacing the older `.network()` API. It provides better control, type safety, and monitoring through iteration hooks, delegation hooks, and completion scoring. See the [Supervisor Pattern example](#supervisor-pattern-with-delegation-and-completion) below and the [Agent Networks comparison](/docs/agents/networks#supervisor-pattern-comparison) for migration details.
