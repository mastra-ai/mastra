...
`streamUntilIdle()` requires both a [`BackgroundTaskManager`](/reference/configuration#backgroundtasks) and a [memory](/guides/memory/overview) backend. Without either, it uses a plain `agent.stream()` call.
...
- [Background tasks](/guides/long-running-agents/background-tasks)
