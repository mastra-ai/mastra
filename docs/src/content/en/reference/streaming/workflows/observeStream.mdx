...
description: "Use Run.observeStream() to reopen a ReadableStream for an active workflow run and continue observing events after the original stream is unavailable."
packages:
  - "@mastra/core"
---
...
- **`workflow-finish`**: Workflow execution completes with usage statistics. For successful runs, `payload.finalWorkflowResult` carries the workflow's final result. For failed runs, `payload.error` carries the run's error
