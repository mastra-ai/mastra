...
description: "The .stream() method enables real-time streaming of responses from a workflow. It returns a ReadableStream of events directly."
packages:
  - "@mastra/core"
---
...
- **`workflow-finish`**: Workflow execution completes with usage statistics. For successful runs, `payload.finalWorkflowResult` carries the workflow's final result, and for failed runs `payload.error` carries the run's error, so stream consumers don't need a follow-up fetch
