...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description: "Configuration for vector index optimization. Currently supported only by PostgreSQL with pgvector. Allows configuring index type (ivfflat, hnsw, flat), distance metric, and index-specific parameters.",
      isOptional: true,
    },
    {
      name: "options",
...
export const agent = new Agent({
  ...
  memory: new Memory({
    ...
    indexConfig: {
      ...
      distance: 'innerproduct',  // Optimal for OpenAI embeddings
