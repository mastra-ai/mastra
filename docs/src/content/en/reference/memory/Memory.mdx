...
const memory = new Memory({
  options: {
    threads: {
      generateTitle: {
        model: openai("gpt-3.5-turbo"), // Cheaper model for titles
      },
    },
  },
});
...
**Key Benefits:**
- **Cost Control**: Use cheaper models for title generation
- **Performance**: Title generation happens asynchronously without blocking responses
- **Flexibility**: Support for both static and dynamic model selection
- **Backward Compatibility**: Boolean configuration still works as before
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description:
        "Settings related to memory thread creation. `generateTitle` controls automatic thread title generation from the user's first message. Can be a boolean to enable/disable using the agent's model, or an object with a custom model for title generation (useful for cost optimization).",
      isOptional: true,
