...
<PropertiesTable
  content={[
    ...
    {
      name: "orderBy",
      type: "{ field: 'createdAt' | 'updatedAt' | 'lastMessageAt', direction: 'ASC' | 'DESC' }",
      description:
        "Sort configuration with field and direction (defaults to { field: 'createdAt', direction: 'DESC' }). Use 'lastMessageAt' to sort by when the last message was saved, ignoring title/metadata edits.",
      isOptional: true,
    },
  ]}
/>
