...
- `inputTokens > largest threshold` → throws an error
- OM automatically sets the input token count in the request context before resolving the model
...
<PropertiesTable
  content={[
  {
    ...
    description:
      'Returns the model for the given input token count. Throws if inputTokens exceeds the largest configured threshold.',
  },
  {
    name: 'getThresholds',
    type: '() => number[]',
