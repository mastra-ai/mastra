...
`compact()` observes pending messages even when Observational Memory's token estimate is below the observation threshold. Use it to recover when a provider rejects a request for exceeding its context window: provider-side token counts can be higher than Observational Memory's local estimate, and `observe()` does nothing below the threshold.

`compact()` activates any buffered observations, then observes the oldest pending messages in chunks until pending tokens drop to `targetTokens` (all pending messages by default), a pass makes no progress, or `maxIterations` is reached. The threshold bypass applies to this call only and isn't saved to the record's config. When you pass a `messageList`, compacted messages are removed from it, so a retried model call sends the smaller context. The request's in-flight messages, such as the user's current prompt, are saved to storage and stay in the list. With `scope: 'resource'` a pass also observes the resource's other threads, so a request that overflowed because of their injected context can be compacted even when the failed thread has nothing pending.

Call it from an [error processor](/docs/agents/processors#api-error-handling) and retry once:

```typescript title="src/mastra/processors/context-overflow-recovery.ts"
import type { Processor } from '@mastra/core/processors'
import type { ObservationalMemory } from '@mastra/memory/processors'

const CONTEXT_OVERFLOW = /context (window|length)|prompt is too long|maximum context/i

export function createContextOverflowRecovery(om: ObservationalMemory): Processor {
  return {
    id: 'context-overflow-recovery',
    async processAPIError({ error, retryCount, messageList, requestContext }) {
      if (retryCount > 0 || !(error instanceof Error) || !CONTEXT_OVERFLOW.test(error.message))
        return
...
Register it with `errorProcessors: [createContextOverflowRecovery(om)]` on the agent.
...
<PropertiesTable
  ...
  },
  {
    name: 'resourceId',
    type: 'string',
    description: 'Resource that owns the thread.',
    isOptional: true,
  },
  {
    name: 'messageList',
    type: 'MessageList',
    description:
      'Live message list of the failed request. Pending messages are read from it, and compacted messages are removed from it. Its input and response messages are saved and kept.',
    isOptional: true,
  },
  {
    name: 'messages',
    ...
    description:
      'Messages to compact when no `messageList` is given. When neither is given, unobserved messages are loaded from storage.',
    isOptional: true,
  },
  {
    name: 'targetTokens',
    type: 'number',
    description:
      'Stop once pending tokens are at or below this value. Pending tokens are the unobserved messages still to observe, counted once.',
    isOptional: true,
    defaultValue: '0',
  },
  {
    ...
    type: 'number',
    description:
      "Maximum tokens of messages sent to the Observer in one pass. A single message larger than this is still observed on its own. Not applied with `scope: 'resource'`, where each pass observes every pending message across the resource's threads.",
    isOptional: true,
    defaultValue: 'The observation threshold',
  },
  {
    name: 'maxIterations',
    type: 'number',
    description: 'Maximum number of observation passes.',
    isOptional: true,
    defaultValue: '10',
  },
  {
    name: 'hooks',
    type: 'ObserveLifecycleHooks',
    description:
      "Per-call lifecycle hooks, as in `observe()`. Config-level hooks receive `trigger: 'compact'` for these cycles.",
    isOptional: true,
  },
]}
/>

`compact()` returns `{ compacted, activated, iterations, tokensCompacted, pendingTokens, reachedTarget, record }`. `compacted` is `true` when buffered observations were activated or at least one pass observed messages. `pendingTokens` counts the unobserved messages still to observe once; unlike `getStatus().pendingTokens`, it doesn't add the formatted other-thread block on top of those same messages.
