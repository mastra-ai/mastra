...
export function createContextOverflowRecovery(om: ObservationalMemory): Processor {
  return {
    ...
    async processAPIError({ error, retryCount, messageList, requestContext }) {
      if (retryCount > 0 || !(error instanceof Error) || !CONTEXT_OVERFLOW.test(error.message)) return
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: 'MessageList',
      description:
        'Live message list of the failed request. Pending messages are read from it, and compacted messages are removed from it.',
      isOptional: true,
    },
    {
      name: 'messages',
    ...
    },
    {
      ...
      isOptional: true,
      defaultValue: 'Half the observation threshold',
    },
    {
      name: 'maxChunkTokens',
      type: 'number',
      description:
        'Maximum tokens of messages sent to the Observer in one pass. A single message larger than this is still observed on its own.',
      isOptional: true,
