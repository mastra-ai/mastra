    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'Extractor<any>[]',
            description:
              'Custom values to extract after observation. Inline extractors are requested in the Observer output. Structured extractors run as a follow-up structured output call and are stored in thread OM metadata.',
            isOptional: true,
          },
          {
    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'Extractor<any>[]',
            description:
              'Custom values to extract after reflection. Inline extractors are requested in the Reflector output. Structured extractors run as a follow-up structured output call and are stored in thread OM metadata.',
            isOptional: true,
          },
          {
...
<PropertiesTable
  ...
  },
  {
    name: 'instructions',
    type: 'string',
    description: 'Instructions for what to extract and when to update the value.',
  },
  {
    name: 'schema',
    type: 'ZodType<T>',
    description:
      'Optional Zod schema for validation. Structured extractors use the schema in a structured output call. Inline extractors parse the XML value as JSON when possible, then validate it against the schema.',
    isOptional: true,
  },
  {
    name: 'mode',
    type: "'inline' | 'structured'",
    description:
      '`inline` asks the Observer or Reflector to emit an XML tag in the main response. `structured` runs a follow-up structured output call after the main OM operation. Custom extractors default to `structured`.',
    isOptional: true,
  ...
  },
  {
    ...
    description:
      'Controls whether the previous value is shown to the extractor on future OM runs. Use `carry-forward` for values that should persist until updated.',
    isOptional: true,
...
- Structured extractors add a follow-up model call. The call uses temporary in-memory history with the Observer or Reflector output as the previous assistant message so providers that support prompt caching can reuse the shared prompt prefix.
