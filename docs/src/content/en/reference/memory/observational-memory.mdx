...
<PropertiesTable
  ...
  {
    name: 'model',
    type: "'auto' | string | LanguageModel | DynamicModel | ModelByInputTokens | ModelWithRetries[]",
    description:
        ...
        parameters: [
          {
            ...
            type: "'auto' | string | LanguageModel | DynamicModel | ModelByInputTokens | ModelWithRetries[]",
            description:
              "Model for the Observer agent. Use `'auto'` to select from the calling agent's effective model provider on each observation. Cannot be set if a top-level `model` is also provided. If neither this nor the top-level `model` is set, falls back to `reflection.model`.",
            isOptional: true,
          },
          {
            name: 'instruction',
        ...
        parameters: [
          {
            ...
            type: "'auto' | string | LanguageModel | DynamicModel | ModelByInputTokens | ModelWithRetries[]",
            description:
              "Model for the Reflector agent. Use `'auto'` to select from the calling agent's effective model provider on each reflection. Cannot be set if a top-level `model` is also provided. If neither this nor the top-level `model` is set, falls back to `observation.model`.",
            isOptional: true,
          },
          {
            name: 'instruction',
