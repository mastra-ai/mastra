...
- `recentMessages`: A bounded head-and-tail representation of the observed conversation window, capped at 11,264 characters and 3,072 estimated tokens across all messages, including formatting and omission markers. The token estimate is not a provider-tokenizer guarantee. Oversized text, reasoning, tool arguments, and tool results retain beginning and ending content; explicit markers identify omitted middle content. Tool results also retain the existing `DEFAULT_OBSERVER_TOOL_RESULT_MAX_TOKENS` safeguard of 10,000 tokens. The window limits do not bound the other hook fields or the entire reminder prompt.
