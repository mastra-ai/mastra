...
<PropertiesTable
  ...
  },
  {
    name: 'targetTokens',
    type: 'number',
    description: 'Stop once pending tokens are at or below this value.',
    isOptional: true,
    defaultValue: '0',
  },
  {
...
`compact()` returns `{ compacted, activated, iterations, tokensCompacted, pendingTokens, reachedTarget, record }`. `compacted` is `true` when buffered observations were activated or at least one pass observed messages.
