...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'ZodType<T>',
    description:
      'Optional Zod schema for structured extraction. When provided, OM runs a follow-up structured output call after the main OM operation. When omitted, the extractor is an inline string extractor emitted in the Observer or Reflector response.',
    isOptional: true,
  },
  {
...
- Schema-backed extractors add a follow-up model call. The call uses the same temporary in-memory Observer or Reflector thread so providers that support prompt caching can reuse the shared prompt prefix.
