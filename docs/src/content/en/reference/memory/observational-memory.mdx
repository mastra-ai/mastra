...
<PropertiesTable
  ...
  {
    name: 'temporalGapMarkers',
    type: 'boolean',
    description:
      'Insert temporal-gap reminder markers before new user messages when the previous message in the thread is at least 15 minutes older. The marker is persisted in memory and also emitted as an inline reminder event so clients can render it specially.',
    isOptional: true,
    defaultValue: 'false',
  },
  {
