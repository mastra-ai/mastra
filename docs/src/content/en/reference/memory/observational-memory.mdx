...
`skillResultFilter` builds a `beforeObservation` transform hook that drops the results of the built-in Agent Skills tools — `skill`, `skill_search`, and `skill_read` — before the Observer model sees them. Skill results contain a skill's full instructions or file contents, so filtering them keeps skill text out of observations while leaving the skill tool calls and all other messages intact.
...
<PropertiesTable
  content={[
  {
    ...
    description:
      'Tool names whose results are dropped. Defaults to the built-in skill tools: `skill`, `skill_search`, and `skill_read`. Use this to filter a different set of tools.',
    isOptional: true,
  },
]}
/>
...
The hook returns messages with matching tool results removed, or `undefined` when no message matched (which passes the payload through unchanged).
