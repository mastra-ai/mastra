...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "Memory scope for observations. `'thread'` (default, recommended) keeps observations per-thread. `'resource'` (deprecated) shares observations across all threads for a resource — this causes context bloat and task interference in practice. Use thread scope with `retrieval: true` for cross-session continuity instead.",
    isOptional: true,
    defaultValue: "'thread'",
  },
  {
...
### Resource scope with custom thresholds (deprecated)

:::danger[Deprecated]
Resource scope is deprecated due to context bloat and task interference. Use thread scope with `retrieval: true` instead. See [recommended alternatives](/docs/memory/observational-memory#recommended-alternatives-for-cross-session-continuity).
:::
...
// ⚠️ Deprecated — use thread scope with retrieval instead
export const agent = new Agent({
  name: 'my-agent',
  instructions: 'You are a helpful assistant.',
  model: '__GATEWAY_OPENAI_MODEL_MINI__',
...
Async buffering isn't supported with `scope: 'resource'` (deprecated) and is automatically disabled in resource scope.
...
// ⚠️ Resource scope is deprecated — use thread scope with retrieval instead
