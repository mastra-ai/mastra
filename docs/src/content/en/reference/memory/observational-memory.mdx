...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "**Experimental.** Enable retrieval-mode observation groups as durable pointers to raw message history. `true` enables current-thread browsing only. `{ vector: true }` also enables semantic search using Memory's vector store and embedder. `{ scope: 'resource' }` enables cross-thread browsing (list threads, browse other threads). Defaults to thread-scoped (current thread only). Only active when OM `scope` is `'thread'`.",
    isOptional: true,
    defaultValue: 'false',
  },
  {
...
When `retrieval` is set (any truthy value) with OM `scope: 'thread'`, a `recall` tool is registered so the agent can page through raw messages behind observation group ranges. With `retrieval: { scope: 'resource' }`, the tool also supports listing threads (`mode: "threads"`), browsing other threads (`threadId`), and cross-thread search. With `retrieval: { vector: true }`, semantic search is available (`mode: "search"`). The tool is automatically added to the agent's tool list — no manual registration is needed.
