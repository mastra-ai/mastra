...
type: "number",
description:
"Optional token budget for the observer's 'Previous Observations' context. When set, the observations passed to the Observer agent are tail-truncated to fit within this budget, keeping the most recent observations. Useful for controlling Observer input costs when observation history is large. Has no effect until observations exceed the budget.",
isOptional: true,
defaultValue: "undefined (disabled)",
},
{
name: "includeBufferedReflection",
type: "boolean",
description:
"When `true`, pending buffered reflection content is included in the observer's context. This gives the Observer richer context while a reflection is still buffered and waiting for activation. Only has an effect when async reflection buffering is enabled and a buffered reflection exists.",
isOptional: true,
defaultValue: "false",
},
{
name: "minContextTokenSavings",
type: "number",
description:
"Minimum token savings required before using the optimized observer context. Compares the baseline (unoptimized) token count against the optimized context. If savings are below this threshold, the full baseline context is used instead. Set to `0` to always use the optimized context when any optimization is applied. This is fully customizable — there is no enforced minimum.",
isOptional: true,
defaultValue: "undefined (disabled)",
},
]}
