    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'number | string',
            description:
              'Time before buffered observations are forced to activate after inactivity, even if the `messageTokens` threshold has not been reached. Accepts milliseconds or duration strings like `300_000`, `"5m"`, or `"1hr"`. When the gap between the current time and the last assistant message part timestamp exceeds this value, buffered observations activate regardless of pending token count. Useful for aligning with prompt cache TTLs.',
            isOptional: true,
          },
          {
    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'number | string',
            description:
              'Time before buffered reflections are forced to activate after inactivity. Accepts milliseconds or duration strings like `300_000`, `"5m"`, or `"1hr"`. Same semantics as `observation.activationTTL` but applies to the reflection step.',
            isOptional: true,
          },
        ],
      },
