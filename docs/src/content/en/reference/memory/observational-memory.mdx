...
- `WorkingMemoryExtractor` uses the normal extractor pipeline to update working memory through the active `Memory` instance. It returns `undefined` from `onExtracted`, so the working memory payload isn't duplicated under OM extracted metadata.
