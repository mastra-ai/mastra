...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      "**Experimental.** Enable graph-mode observation groups as durable pointers to raw message history. Graph mode is only active when `scope` is `'thread'`. If you set `graph: true` with `scope: 'resource'`, OM keeps resource-scoped memory behavior but skips graph-mode context and does not register the `recall` tool.",
    isOptional: true,
    defaultValue: 'false',
  },
  {
...
When `graph: true` is set with `scope: 'thread'`, OM registers a `recall` tool that the agent can call to page through the raw messages behind an observation group's `_range`. The tool is automatically added to the agent's tool list — no manual registration is needed.
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    isOptional: true,
    description: 'Present and `true` when the output was capped by the token budget. The agent can paginate or use `partIndex` to access remaining content.',
  },
  {
    name: 'tokenOffset',
    type: 'number',
