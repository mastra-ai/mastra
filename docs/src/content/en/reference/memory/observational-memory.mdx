...
When `retrieval: true` is set with `scope: 'thread'`, OM registers a `recall` tool that the agent can call to page through the raw messages behind an observation group's `_range`, list threads, or browse other threads. The tool is automatically added to the agent's tool list — no manual registration is needed.
