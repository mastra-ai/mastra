...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      "Enable graph-mode observation groups as durable pointers to raw message history. Graph mode is only active when `scope` is `'thread'`. If you set `graph: true` with `scope: 'resource'`, OM keeps resource-scoped memory behavior but skips graph-mode context and does not register the `recall` tool.",
    isOptional: true,
    defaultValue: 'false',
  },
  {
