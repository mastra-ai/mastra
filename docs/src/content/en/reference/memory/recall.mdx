...
description:
"Filter options for message retrieval. `dateRange` filters messages by creation date (use `startExclusive` or `endExclusive` to exclude boundary dates). `metadata` filters by key-value pairs stored in message content metadata (AND logic - all must match).",
isOptional: true,
},
{
name: "orderBy",
...
const filtered = await memory!.recall({
  threadId: "thread-123",
  filter: {
