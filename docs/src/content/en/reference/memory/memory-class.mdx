    ...
    properties: [
      {
        parameters: [
          {
            name: 'lastMessages',
            type: 'number | false | { maxMessages?: number | false; maxTokens?: number; atMaxRemoveTokens?: number }',
            description:
              'Controls how many messages to load into context. For count-based limiting, use a number (e.g. `10`) or `false` to disable history entirely. For token-based limiting, use object form with `maxTokens`. Set `maxMessages: Infinity` or `maxMessages: false` to load all messages with no count limit.',
            isOptional: true,
            defaultValue: '10',
          },
          {
            name: 'lastMessages.maxTokens',
            type: 'number',
            description:
              'Maximum token budget for conversation history. When the total token count of all messages (system messages + memory history + new input) exceeds this limit, the oldest memory history messages are automatically removed. Token counting uses tiktoken (o200k_base encoding) with caching on message metadata. Can be used alongside `maxMessages` for both count-based and token-based limits.',
            isOptional: true,
          },
          {
            name: 'lastMessages.atMaxRemoveTokens',
            type: 'number',
            description:
              'When the token budget is exceeded, drop by this many tokens instead of trimming to zero. Defaults to 25% of `maxTokens`. The resulting target window (`maxTokens - atMaxRemoveTokens`) is stored as a thread boundary so future turns start from that point instead of recounting all history.',
            isOptional: true,
          },
          {
            name: 'readOnly',
