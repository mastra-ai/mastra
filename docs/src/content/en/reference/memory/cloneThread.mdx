...
When Observational Memory is present, cloning copies only its current active record. It preserves the live observation tail and remaps available raw-message provenance to the cloned message IDs. Historical reflection and archive generations aren't copied, so the cloned thread starts with an empty observation archive catalog.
