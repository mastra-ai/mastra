...
`@mastra/mcp-docs-server` 2.x speaks the MCP `2026-07-28` revision. If your tool hasn't adopted that revision yet, it fails to connect with a version-negotiation error. Pin the previous major instead, which speaks the earlier `initialize` handshake:
...
To do the same for any tool below, use `@mastra/mcp-docs-server@^1` in place of `@mastra/mcp-docs-server@latest`, or append `@^1` where the package name has no version. The Cursor install button installs the latest version, so add the server through Cursor's MCP settings with the JSON above instead.
