---
title: "Reference: RedisStreamsPubSub | Events"
...
`RedisStreamsPubSub` is a [`PubSub`](./pubsub) implementation backed by [Redis Streams](https://redis.io/docs/latest/develop/data-types/streams/). It delivers events across processes and hosts, with persistence, consumer groups, and redelivery on failure.

Use it for distributed deployments where several services share an event stream. For single-process delivery, use [`EventEmitterPubSub`](./event-emitter). For Google Cloud, use [`GoogleCloudPubSub`](./google-cloud-pubsub).
...
`RedisStreamsPubSub` implements the [`PubSub`](./pubsub) contract. The methods below have behavior specific to this implementation.
