...
| Index Type | Best For | Build Time | Query Speed | Memory | Accuracy |
|------------|----------|------------|-------------|---------|----------|
| **Flat** | <1K vectors, 100% accuracy needed | None | Slow (linear) | Minimal | 100% |
| **IVFFlat** | 10K-1M vectors, balanced performance | Minutes | Fast | Moderate | 95-99% |
| **HNSW** | 100K+ vectors, fastest queries | Hours | Very Fast | High (2-3x) | ~99% |
...
- **cosine**: Best for text embeddings where direction matters more than magnitude
- **inner**: Optimal for normalized embeddings (OpenAI, Cohere)
- **euclidean**: For unnormalized embeddings where magnitude matters
...
- **Start with IVFFlat** for most use cases, it provides good balance
- **Use HNSW** when you need the fastest possible queries and can afford memory
- **Monitor index performance** using `describeIndex()` to track usage
- **Set appropriate lists/m values** based on your dataset size
- **Choose the right metric** for your embedding model (inner for OpenAI)
