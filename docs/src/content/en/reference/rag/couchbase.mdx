...
description: Documentation for the CouchbaseSearchStore class in Mastra, which provides vector search using Couchbase Vector Search.
---

# Couchbase Search Store

The `CouchbaseSearchStore` class provides vector search using [Couchbase Vector Search](https://docs.couchbase.com/server/current/vector-search/vector-search.html). It enables efficient similarity search and metadata filtering within your Couchbase collections.
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      defaultValue: "[]",
      description: "Metadata fields to index for filtering support",
    },
  ]}
/>
...
> **Note:** The `includeVector` parameter is not currently supported. To retrieve the vector embedding, fetch the full document by ID using the Couchbase SDK. Filtering is now supported for metadata fields that were indexed during index creation.
...
<PropertiesTable
  content={[
    ...
    {
      name: "filter",
      type: "CouchbaseVectorFilter",
      isOptional: true,
      description: "Metadata filters (only for indexed fields). All field names must be prefixed with 'metadata.'. See [Metadata Filters](./metadata-filters).",
    },
    {
      name: "includeVector",
      type: "boolean",
...
- **Filtering Support:** The `CouchbaseSearchStore` class now supports metadata filtering. Only fields specified in the `fields_to_index` parameter during index creation can be filtered.
...
- The `includeVector` parameter is not supported in query results.
- Filtering only works on metadata fields that were explicitly indexed during index creation.
