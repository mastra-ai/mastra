...
<OperatorsTable
  ...
  operators={[
    ...
    },
    {
      ...
      supportedBy: ["Qdrant", "Upstash", "PgVector", "LibSQL", "MongoDB", "Couchbase"],
    },
  ]}
/>
...
=======
- Supports metadata filtering for fields that were indexed during index creation using the `fields_to_index` parameter
- Metadata fields must be prefixed with `metadata.` in filter expressions (e.g., `'metadata.category': { $eq: 'tech' }`)
- Supports basic comparison operators: `$eq`, `$ne`, `$gt`, `$gte`, `$lt`, `$lte`
- Supports logical operators: `$and`, `$or`, `$not`, `$nor`
- Only indexed metadata fields can be filtered - fields not specified in `fields_to_index` during index creation cannot be used for filtering
