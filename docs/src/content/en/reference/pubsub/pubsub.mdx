...
Mastra uses pub/sub internally for workflow event processing, streaming, and cross-component communication. Most applications use the default [`EventEmitterPubSub`](./event-emitter) and never construct a `PubSub` directly. Implement this class only when you need a custom transport.

For built-in implementations, see [`EventEmitterPubSub`](./event-emitter), [`UnixSocketPubSub`](./unix-socket-pubsub), [`CachingPubSub`](./caching-pubsub), [`RedisStreamsPubSub`](./redis-streams), and [`GoogleCloudPubSub`](./google-cloud-pubsub).
...
Pass the instance to the [Mastra](../core/mastra-class) constructor:
...
These methods support resuming a stream after a disconnect. The default implementations fall back to a regular `subscribe`, so backends without history support behave as live-only. [`CachingPubSub`](./caching-pubsub) overrides them to replay cached events.
