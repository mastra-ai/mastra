...
#### `trimTopic(topic, entryIds)`

Deletes exactly the retained entries identified by `entryIds`, the IDs returned from earlier `publish` calls on the topic. Other entries are untouched. Mastra records the entry IDs of each thread run and calls this once a run completes successfully and its messages are saved to storage, so finished runs leave the topic while runs still in progress or waiting for approval stay. The default implementation is a no-op; [`RedisStreamsPubSub`](/reference/pubsub/redis-streams) overrides it. Same best-effort contract as `clearTopic`.

```typescript
const id = await pubsub.publish('agent.thread.thread-abc', event)
await pubsub.trimTopic('agent.thread.thread-abc', [id!])
```
