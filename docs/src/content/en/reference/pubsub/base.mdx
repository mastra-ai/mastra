...
#### `trimTopic(topic, { before })`

Removes retained entries published strictly before `before` (a `Date`), keeping the rest of the topic. Mastra calls this on a thread topic once a run completes and its messages are saved to storage, keeping everything from the oldest active or suspended run onward. The default implementation is a no-op; [`RedisStreamsPubSub`](/reference/pubsub/redis-streams) overrides it. Same best-effort contract as `clearTopic`.
