...
description: "Extend the PubSub abstract class to implement Mastra event publishing, subscriptions, message delivery, and provider lifecycle behavior."
packages:
  - '@mastra/core'
---
...
The default implementation is a no-op because transports that retain nothing per topic, such as `EventEmitterPubSub`, have nothing to clear. Backends that persist messages, like [`RedisStreamsPubSub`](/reference/pubsub/redis-streams), override it. Implementations follow a best-effort contract and log failures instead of throwing because callers invoke cleanup without awaiting it.

```typescript
await pubsub.clearTopic('workflow.events.v2.run-123')
```

#### `trimTopic(topic, options?)`

Removes retained entries published strictly before `options.before` (a `Date`), keeping the rest of the topic. Without `before`, it removes every entry retained at the time of the call. Mastra calls this on a thread topic once a run completes and its messages are saved to storage: it trims the whole topic when the thread is idle, otherwise it keeps everything from the oldest active or suspended run onward. The default implementation is a no-op; [`RedisStreamsPubSub`](/reference/pubsub/redis-streams) overrides it. Same best-effort contract as `clearTopic`.

```typescript
await pubsub.trimTopic('agent.thread.thread-abc', { before: new Date(startedAt) })
await pubsub.trimTopic('agent.thread.thread-abc')
```
