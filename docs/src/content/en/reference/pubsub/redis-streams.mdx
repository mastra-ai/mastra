...
description: "RedisStreamsPubSub is a PubSub implementation backed by Redis Streams. It delivers events across processes and hosts, with persistence, consumer groups, and redelivery on failure."
packages:
  - '@mastra/core'
...
Pass a `clientFactory` to back the pubsub with a Redis Cluster client. The factory is called once for the shared writer and once for each subscription's blocking reader, so every connection is a distinct client.
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Factory returning an unconnected `redis` client, called once for the writer and once per subscription reader. Use it to back the pubsub with a Redis Cluster client (`createCluster(...)`) or any pre-configured client. When set, `url`/`redisOptions` are not used to create clients.',
    isOptional: true,
  },
  {
