...
The prompt is shown only when three conditions hold: `REDIS_URL` is referenced by user code, the environment doesn't already have it set, and the terminal is interactive. The platform validates that your plan includes managed Redis during provisioning. Non-interactive runs (CI, `--yes`) fail preflight with a copy-pasteable `mastra env db create --kind redis` command instead of silently creating infrastructure.
