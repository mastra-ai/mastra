...
If you accept, `mastra deploy` calls the platform's project-databases API with `kind: 'redis'`. The CLI then waits for the Railway-backed service to reach the ready state before injecting `REDIS_URL` into the deploy so `new RedisStreamsPubSub({ url: process.env.REDIS_URL })` connects without extra setup.

The prompt is shown only when four conditions hold: `REDIS_URL` is referenced by user code, the environment doesn't already have it set, the terminal is interactive, and the org is on a plan that includes managed Redis. Non-interactive runs (CI, `--yes`) fail preflight with a copy-pasteable `mastra env db create --kind redis` command instead of silently creating infrastructure.
