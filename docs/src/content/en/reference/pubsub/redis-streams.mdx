...
When you deploy a project that reads `REDIS_URL` from the environment, the Mastra CLI's `deploy` preflight offers to attach a managed Redis on the Mastra platform in one step. If you accept, `mastra deploy` calls the platform's project-databases API with `kind: 'redis'`, waits for the Railway-backed service to become ready, and injects `REDIS_URL` into the deploy so `new RedisStreamsPubSub({ url: process.env.REDIS_URL })` connects without extra setup. The prompt only appears when `REDIS_URL` is referenced by user code, the environment doesn't already have it set, the terminal is interactive, and the org is on a plan that includes managed Redis. Non-interactive runs (CI, `--yes`) fail preflight with a copy-pasteable `mastra env db create --kind redis` command instead of silently creating infrastructure.
