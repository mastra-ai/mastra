...
Runs `XTRIM MINID` to drop stream entries older than `options.before`, or `XTRIM MAXLEN 0` to drop every current entry when `before` is omitted. Mastra calls it on thread topics after a run's messages are persisted, so an idle thread's stream empties instead of growing to `maxLen`. Best-effort; failures are logged at warn level.
