...
Runs `XDEL` for the given entry IDs. `publish` returns each entry's stream ID, and Mastra deletes a thread run's entries once its messages are persisted, so an idle thread's stream empties instead of growing to `maxLen`. Entries from other runs, including those from other processes, are never touched. Best-effort; failures are logged at warn level.
