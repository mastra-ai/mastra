...
Runs `XTRIM MINID` to drop stream entries older than `before`. Mastra calls it on thread topics after a run's messages are persisted, so an idle thread's stream shrinks to nearly nothing instead of growing to `maxLen`. Best-effort; failures are logged at warn level.

```typescript
await pubsub.trimTopic('agent.thread.thread-abc', { before: new Date() })
```
