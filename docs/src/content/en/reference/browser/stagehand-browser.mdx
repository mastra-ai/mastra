...
When `recording` is configured, `StagehandBrowser` also adds the alpha `browser_record` and `browser_record_caption` tools. See [Browser recording (alpha)](/ecosystem/browser/recording).
...
- [Stagehand guide](/ecosystem/browser/stagehand): Usage guide
