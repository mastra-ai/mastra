...
Capture a screenshot of the current page as PNG (viewport by default; set `fullPage: true` for full-page capture). The result includes multi-part content with the image and page metadata for vision-capable models. Use `stagehand_observe` or `stagehand_extract` when you only need text or structured data.
