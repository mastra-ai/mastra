...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "CDP WebSocket URL or async provider function. When provided, connects to an existing browser instead of launching a new one. Can't be used with scope: 'thread' (automatically uses shared scope). Useful for cloud providers (Browserbase, Browserless, etc.).",
    isOptional: true,
  },
  {
    name: 'scope',
