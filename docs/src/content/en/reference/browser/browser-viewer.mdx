...
Each CLI must be installed separately. Each also publishes a [skill](/guides/workspace/skills) that teaches the agent its commands and workflows. When a CLI command runs through `workspace_execute_command`, Mastra detects it and injects the CDP URL automatically using the correct flag.
...
- [BrowserViewer guide](/guides/browser/browser-viewer): Setup and usage walkthrough
- [MastraBrowser](/reference/browser/mastra-browser): Base class API reference
- [Workspace overview](/guides/workspace/overview): Workspace configuration
