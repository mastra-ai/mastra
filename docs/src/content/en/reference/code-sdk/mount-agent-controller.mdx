...
This function keeps Mastra Code's local and terminal defaults, including host configuration discovery. Use [`mountRestrictedAgentControllerOnMastra()`](/reference/code-sdk/mount-restricted-agent-controller) when a credentialed server needs an explicit, fail-closed capability boundary. Injecting a remote workspace into this function doesn't disable its other local discovery paths.
...
- [`mountRestrictedAgentControllerOnMastra()`](/reference/code-sdk/mount-restricted-agent-controller)
