...
The resolved, read-only `ModelPolicy` derived from this input (with `active` and `pickerVisible` flags) is documented on the [BuilderModelPolicy](/reference/editor/agent-builder/builder-model-policy) page.
...
Mastra validates the admin policy at server boot. Violations surface as warnings through [`IAgentBuilder.getModelPolicyWarnings()`](/reference/editor/agent-builder/iagent-builder).
...
- [BuilderModelPolicy](/reference/editor/agent-builder/builder-model-policy) — the read-only resolved policy served to the admin UI.
