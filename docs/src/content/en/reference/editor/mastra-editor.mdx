...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Browser providers for agent browser access (for example, Stagehand). No built-ins — required when `builder.features.agent.browser` is enabled. See [BrowserProvider](/reference/editor/browser-provider) for the provider interface.',
    isOptional: true,
    defaultValue: '{}',
  },
  {
    ...
    description:
      'Agent Builder configuration. See the [AgentBuilderOptions reference](/reference/editor/agent-builder/agent-builder-options). Omit or set `enabled: false` to disable the Builder.',
    isOptional: true,
  },
]}
/>
...
- [IAgentBuilder](/reference/editor/agent-builder/iagent-builder) — runtime interface returned by `resolveBuilder()`.

### Registering the Builder agent

The Builder UI runs against an agent named `builderAgent` exported from `@mastra/editor/ee`. Import it and register it under `agents` on your Mastra instance:
