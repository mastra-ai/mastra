...
<PropertiesTable
  ...
  },
  {
    ...
    type: "'code' | 'db'",
    description:
      "Where agent overrides are stored. With 'db', overrides live in the configured storage backend and Studio shows the save and publish flow. With 'code', overrides live as per-agent JSON files on disk (routed through a local FilesystemStore) and Studio shows filesystem actions. See Editor for the difference.",
    isOptional: true,
    defaultValue: "'db'",
  },
  {
...
await editor.agent.update({
  id: 'support-agent',
  instructions: "You are a friendly support agent for Acme. Respond in the user's language.",
})
```

`editor.agent.update()` creates a version and assigns it to `activeVersionId`. This differs from the stored-agent REST `PATCH` route, which creates an inactive draft unless `autoPublish` is enabled. See [Editor versioning](/reference/editor/versioning#database-lifecycle).
...
When `source` is omitted, Editor uses the configured database storage even though `getSource()` returns `undefined`.

With `source: 'code'`, active agent overrides are written as deterministic JSON under `<codePath>/agents/<encodedAgentId>.json`. The default `codePath` is `./mastra/editor`. The initial stored agent is published when it's created, while later server updates are written to the file when their version becomes active.

Studio reads recent Git revisions of each file as read-only versions. The default scan includes up to 50 commits and skips consecutive commits with the same parsed JSON snapshot. Git manages rollback and release for code-backed overrides.

See [Editor versioning](/reference/editor/versioning#code-source) for the complete code-source lifecycle.
