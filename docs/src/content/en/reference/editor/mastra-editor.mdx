...
`editor.agent.update()` creates a version and assigns it to `activeVersionId`. This differs from the stored-agent REST `PATCH` route, which creates an inactive draft unless `autoPublish` is enabled. See [Editor versioning](/docs/editor/overview#versioning).
...
See [Editor storage options](/docs/editor/overview#choose-where-changes-are-stored) for the code-source workflow.
