...
Both `editor.prompt.update()` and `editor.agent.update()` create an inactive draft when snapshot fields change. Neither method changes `activeVersionId` unless you pass it explicitly.

To publish an existing agent version through the SDK, pass its ID as `activeVersionId` with `status: 'published'`:
...
Here, `version` is the stored version you want to publish, not the runtime `Agent` returned by `editor.agent.update()`.

The stored-agent REST `PATCH` route also creates an inactive draft by default (`autoPublish: false`). Set `autoPublish: true` to publish the update, or publish an existing version with `POST /api/stored/agents/:agentId/versions/:versionId/activate` (using the default `/api` prefix).
