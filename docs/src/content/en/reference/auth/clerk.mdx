...
Set `organizationId` or `organizationSlug` to only allow members of a specific Clerk organization to sign in. Non-members are rejected during authorization. Configure only one of the two — setting both throws at startup so login cannot be granted to two different organizations.
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Custom authorization function to determine if a user should be granted access. Called after token verification. By default, allows all authenticated users when neither `organizationId` nor `organizationSlug` is configured; when either is set, users outside the required organization are denied.',
    isOptional: true,
  },
]}
/>
