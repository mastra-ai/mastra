...
For concepts, configuration, and the lifecycle points where Mastra enforces FGA, see [Fine-grained authorization](/ecosystem/auth-providers/fga).
...
Throws `FGADeniedError` when `user` lacks the permission. Mastra calls this at its [enforcement points](/ecosystem/auth-providers/fga#enforcement-points).
...
Treat `actor.permissions` as an untrusted claim. Resolve the agent's authoritative grants from a trusted source keyed by `actor.agentId`, rather than trusting the inline values. See [System actors](/ecosystem/auth-providers/fga#system-actors).
...
- [Fine-grained authorization](/ecosystem/auth-providers/fga)
- [System actors](/ecosystem/auth-providers/fga#system-actors)
