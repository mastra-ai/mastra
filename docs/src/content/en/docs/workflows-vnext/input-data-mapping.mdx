...
const step2 = createStep({
  ...
  execute: async ({ inputData }) => {
    console.log(`Step 2 received: ${inputData.outputValue} and original: ${inputData.initialValue}`);
...
const step2 = createStep({
  ...
  execute: async ({ inputData }) => {
    console.log(`Step 2 received: ${inputData.intermediateValue}`);
...
const step3 = createStep({
  ...
  execute: async ({ inputData }) => {
    console.log(`Step 3 combining all previous data`);
    return {
...
console.log("\nRunning Example 3: Using multiple outputs");
const run = mastra.vnext_getWorkflow("myWorkflow").createRun();
const res = await run.start({
