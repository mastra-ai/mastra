...
Configure a persistent storage adapter when state must survive restarts or be shared across processes, or alternatively be visible in Studio across sessions. The default in-memory store is useful for tests and short local experiments, but it loses data when the process exits.
