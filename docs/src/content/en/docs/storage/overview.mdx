...
- `scores`, `datasets`, and `experiments`: Store lower-frequency evaluation data that is often read later for analysis.
...
- Configure [retention](/reference/storage/retention) for growth-heavy tables, then call `storage.prune()` from a scheduler or maintenance job.
