...
Platform servers scale down to idle when they aren't receiving traffic, and that's fine for Slack. Slack retries webhook delivery, so the next event, such as a mention or direct message, wakes the server and it resumes handling requests. The first response after the server has been idle can take a little longer while it starts up. Subsequent messages respond at normal speed while the server stays warm.
