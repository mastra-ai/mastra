...
Each agent gets its own Mastra thread for every external conversation. Threads created by channels carry a `channel_ownerId` metadata key with the id of the agent that owns them, and thread lookups only match threads owned by the same agent. Agents that share one storage instance and talk in the same Slack channel or Telegram chat keep separate threads with separate history and subscription state.

Threads created before this key existed are adopted lazily. When an agent finds a matching thread without a `channel_ownerId` key, it claims the thread by stamping its own id onto it and keeps using it. Single-agent deployments keep their history and subscription state without any migration step. Threads claimed by another agent are never reused. The agent starts a fresh thread instead.
