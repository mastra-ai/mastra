...
Studio isn't limited to engineers. Deploy it and share it with your team, so collaborators can try an agent before it goes to production. With the [Editor](/docs/editor/overview) and [Agent Builder](https://agent-builder.mastra.ai/), non-technical teammates can create and iterate on agents themselves, with every change versioned in code.
