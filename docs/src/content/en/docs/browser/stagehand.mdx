...
When the agent uses the `stagehand_screenshot` tool, it captures a PNG image of the current page. The result includes the image and page metadata (title and URL) as multi-part content, which vision-capable models can interpret directly.
