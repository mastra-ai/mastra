...
When you assign a browser to an agent, Mastra includes the provider's tools in the agent's toolset. The agent uses these tools to control the browser: navigating to URLs, clicking elements, typing text, and reading page content.

Each provider offers a different set of tools optimized for its approach:

| Provider | Approach | Tools |
| - | - | - |
| agent-browser | Accessibility tree refs | `goto`, `click`, `type`, `snapshot`, `scroll`, `select`, `press`, `hover`, `back`, `wait`, `tabs`, `drag`, `dialog`, `evaluate`, `close` |
| Stagehand | AI-powered selectors | `navigate`, `act`, `extract`, `observe`, `tabs`, `close` |
...
This works with Browser-Use, Browserless, Steel.dev, or any CDP-compatible service.
...
| `cdpUrl` | `string \| () => Promise<string>` | — | CDP endpoint URL for remote browsers |
