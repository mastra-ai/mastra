...
For local launches (the default), AgentBrowser requires a Chromium binary installed via Playwright. This is normally downloaded automatically when you install `@mastra/agent-browser`. If launching the browser fails with `"browser executable is missing"`, run `npx playwright install chromium`. If you connect to a remote browser using the [`cdpUrl`](/reference/browser/agent-browser) option, no local Chromium is needed.

:::

## Screenshots

When the agent uses the `browser_screenshot` tool, it captures a PNG image of the current page. The result includes the image and page metadata (title and URL) as multi-part content, which vision-capable models can interpret directly.
