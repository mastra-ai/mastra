...
After messages are observed, the model receives the observation log, recent messages that haven't been observed, and a continuation reminder. Async buffered observations don't generate continuation hints (`currentTask` or `suggestedResponse`) because they may be stale by activation time. The raw messages remain stored but no longer occupy the active model context.
