...
After messages are observed, the model receives the observation log and recent messages that haven't been observed. Synchronous observations can also include a continuation reminder. Async buffered observations omit continuation hints because they may be stale by activation time. The raw messages remain stored but no longer occupy the active model context.
