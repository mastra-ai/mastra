...
{/* vale Mastra.Contractions = NO */}

## What is an eval?

{/* vale Mastra.Contractions = YES */}
...
The [companion repository](https://github.com/bookercodes/mastra-eval-loop-guide) contains the complete example, including environment configuration, database setup, and agent and scorer registration. This guide focuses on the eval loop and the code that explains it. Use the repository for setup instructions and runnable scripts.
...
![Aster refunds dataset items in Mastra Studio.](/img/guides/build-an-eval-loop/aster-refunds-dataset.png)
...
Choose a different model from the agent, ideally from another provider. This example uses OpenAI for the agent and Anthropic for the judge. Keep the judge fixed when comparing changes, and review its judgments because it can make mistakes too.
...
![Run Experiment dialog with Aster refunds, Aster Space Reservations, and a scorer selected.](/img/guides/build-an-eval-loop/run-experiment-dialog.png)
...
![Baseline experiment with all five cases scoring 1.000 for refund correctness.](/img/guides/build-an-eval-loop/experiment-results.png)
...
![A new experiment named 5.4-mini using the same dataset and scorer.](/img/guides/build-an-eval-loop/run-experiment-model-change.png)
...
![Baseline and 5.4-mini experiments completed with five items processed each.](/img/guides/build-an-eval-loop/model-experiments-list.png)

Select **Compare**, choose both experiments, then select **Compare Experiments**.

![Baseline and 5.4-mini selected for comparison.](/img/guides/build-an-eval-loop/select-experiments-to-compare.png)
...
![Answers, timings, and correctness scores compared side by side.](/img/guides/build-an-eval-loop/model-experiment-comparison.png)
...
![A trace where the agent asks for booking details, with Add full trace to dataset selected.](/img/guides/build-an-eval-loop/production-trace-add-to-dataset.png)
...
![Saving the trace to Aster refunds with must and mustNot criteria.](/img/guides/build-an-eval-loop/production-trace-ground-truth.png)
...
![An accepted-assignment case scoring zero, with the judge explaining the incorrect refund promise.](/img/guides/build-an-eval-loop/failed-case-judge-reason.png)
...
![The regression script reporting five passing cases and a passed verdict.](/img/guides/build-an-eval-loop/regression-check-passed.png)
...
![Failed cases, judge explanations, a scored verdict, and exit code 1.](/img/guides/build-an-eval-loop/regression-check-failed.png)
