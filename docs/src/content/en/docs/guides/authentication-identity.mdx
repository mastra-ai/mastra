...
By default, when an `MCPServer` runs behind `server.auth`, Mastra places the authenticated caller in the MCP `authInfo` object, which tools read from the `authInfo` key of their request context. Server tools can use this identity without parsing the request again:
