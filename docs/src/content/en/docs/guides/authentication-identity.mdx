...
Protect API endpoints, Studio, and agent calls with authentication, trusted identity, and authorization in runtime code. As system prompts only shape model behavior this guide explains how you can correctly enforce these security measures.
...
Authorization belongs in trusted code, as close as possible to the protected operation. Don't rely on an agent instruction as the security boundary. Trusted runtime code can derive ownership from the authenticated user in `RequestContext` rather than model-generated tool input:
...
This example uses the default bridge. If you configure `server.mcpOptions.setRequestAuth`, that hook defines the `authInfo` shape. Read the fields your hook provides instead.
