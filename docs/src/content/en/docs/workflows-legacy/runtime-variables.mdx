...
1. Pass runtime configuration variables to workflows through a type-safe requestContext
...
Steps can access requestContext variables and must conform to the workflow's requestContext type:
...
1. **Missing Variables**: Always check if required variables exist in the requestContext
...
```typescript
// Example of defensive programming with requestContext variables
const multiplier = requestContext.get("multiplier");
if (multiplier === undefined) {
  throw new Error("Multiplier not configured in requestContext");
}
...
1. **Type Safety**: Always define proper types for your requestContext and step inputs/outputs
2. **Validation**: Validate all inputs and requestContext variables before using them
3. **Error Handling**: Implement proper error handling in your steps
4. **Documentation**: Document the expected requestContext variables for each workflow
