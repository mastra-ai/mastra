...
:::info Smithery OAuth Authentication

Smithery has migrated to OAuth-only authentication. To use Smithery-hosted MCP servers, use the Smithery CLI which handles OAuth automatically:
...
The CLI will prompt for OAuth authentication on first use.
...
<Tabs>
  ...
  <TabItem value="smithery" label="Smithery.ai">
    ...
    :::warning OAuth Authentication Required
    
    Smithery has migrated to **OAuth-only authentication** and no longer supports API key–based access. The Smithery CLI handles OAuth authentication automatically.
    ...
    **How it works:**
    - On first use, the Smithery CLI will open a browser window for OAuth authentication
    - You'll sign in to Smithery and grant permissions
    - Credentials are securely stored for future use
    - Works the same on Unix/Mac and Windows
