...
<Tabs items={["Ampersand","mcp.run", "Composio.dev", "Smithery.ai"]}>
  <Tabs.Tab>

    [Ampersand](https://withampersand.com?utm_source=mastra-docs) is deep integration platform for AI developers to add enterprise-grade CRM & GTM integrations with your agents. 

    It offers a single multi-tenant [MCP Server](https://www.npmjs.com/package/@amp-labs/mcp-server) that allows you to connect your agent to 150+ B2B Integrations. 
    
    Sign up at [Ampersand](https://withampersand.com?utm_source=mastra-docs) to get your `AMPERSAND_PROJECT_ID`, `AMPERSAND_INTEGRATION_ID_OR_NAME` & set the `groupRef` to the unique customerId inside your application. 

    To check out how Ampersand works, check out their [documentation](https://docs.withampersand.com/)
    ...
    export const mcp = new MCPClient({
        ...
        "@amp-labs/mcp-server": {
          "url": `https://mcp.withampersand.com/sse?${new URLSearchParams({
            ...
            integrationId: process.env.AMPERSAND_INTEGRATION_ID_OR_NAME,
