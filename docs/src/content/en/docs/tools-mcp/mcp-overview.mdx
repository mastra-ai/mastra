...
<Tabs items={["Ampersand","mcp.run", "Composio.dev", "Smithery.ai"]}>
  <Tabs.Tab>
    ...
    Sign up at [Ampersand](https://withampersand.com?utm_source=mastra-docs) to get your `AMPERSAND_PROJECT_ID`, `AMPERSAND_INTEGRATION_ID` & set the `groupRef` to the unique customerId inside your application. 
