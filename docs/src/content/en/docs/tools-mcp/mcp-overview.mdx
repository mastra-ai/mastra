...
To use Mastra MCP, install the required dependency:
...
An MCP Client is the thing that allows Mastra primitives to connect to external MCP Servers, be it published npm packages, or remote MCP servers that operate over http(s)

To create a new MCP client....

```typescript filename="src/mastra/mcp-clients/test-mcp-client.ts" showLineNumbers copy
...
Provide access to the MCP Client from an agent, import the MCP Client, use the `tools` parameter with the `.getTools()` method:

```typescript {4,15} filename="src/mastra/agents/test-agent.ts" showLineNumbers copy
import { openai } from "@ai-sdk/openai";
import { Agent } from "@mastra/core/agent";

import { wikipediaMcp } from "../mcp-clients/wikipedia-mcp";

export const testAgent = new Agent({
  name: "Test Agent",
  description: "A test agent that uses the Wikipedia MCP Server",
  instructions: `
      You are a helpful assistant that has access to the Wikipedia MCP Server.
      - Wikipedia MCP Server

      Answer questions using the information you find using the Wikipedia Server.
`,
  model: openai("gpt-4o-mini"),
  tools: await wikipediaMcp.getTools()
});
```

This agent will now have access to the following Wikipedia tools:

- **wikipedia_search**: Returns a list of Wikipedia articles matching a query string.
- **wikipedia_readArticle**: Returns the full content of a specified Wikipedia article.

## Creating an MCP Server

An MCP Server is a thing that can be accessed by other stuff by being part of a deployed Mastra application that can be access over http(s).

```typescript filename="src/mastra/mcp-servers/test-mcp-server.ts" showLineNumbers copy
...
To blah an MCP Server register it in the main Mastra instance.

```typescript filename="src/mastra/index.ts" showLineNumbers copy
import { Mastra } from "@mastra/core/mastra";

import { testMcpServer } from "./mcp-servers/test-mcp-server";
