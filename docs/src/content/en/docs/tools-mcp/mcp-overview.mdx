...
1. **`MCPClient`**: Connects to external MCP servers and accesses their tools.
2. **`MCPServer`**: Exposes Mastra tools, agents, and workflows to MCP-compatible clients.
...
LLM providers require API keys, named using the provider’s identifier.
Providers like [Smithery](https://smithery.ai/) require an API key to access their hosted MCP servers.
...
An MCP server exposes agents, tools, and workflows from your Mastra application to external systems over HTTP(S). This makes them accessible to any system or agent that supports the protocol.
