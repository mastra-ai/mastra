...
When creating tools, focus on clear and descriptive definitions:

*   **Clear names and descriptions:** Keep tool descriptions simple and focused on **what** the tool does and **when** to use it, emphasizing its primary use case.
*   **Clear parameter schemas:** Technical details belong in the parameter schemas, guiding the agent on *how* to use the tool correctly with descriptive names, clear descriptions (using `.describe()`), and explanations of default values.
