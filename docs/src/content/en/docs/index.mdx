...
- [**Human-in-the-loop**](./workflows/suspend-and-resume) - Suspend an agent or workflow and await user input or approval before resuming. Mastra uses storage to remember execution state, so you can pause indefinitely and resume where you left off.

- **Context management** - Give your agents the right context at the right time. Provide [message history](./memory/message-history), [retrieve](./rag/overview) data from your sources (APIs, databases, files), and add human-like [working](./memory/working-memory) and [semantic](./memory/semantic-recall) memory so your agents behave coherently.
...
- **Production essentials** - Shipping reliable agents takes ongoing insight, evaluation, and iteration. With built-in [scorers](./evals/overview) and [observability](./observability/overview), Mastra gives you the tools to observe, measure, and refine continuously.
...
Choose a [getting started guide](./getting-started/start) to get started, or see the [manual installation guide](./getting-started/manual-install) if you need more control over your setup.
