...
Note that while each thread has one owner, messages within that thread can have different `resourceId` values, enabling multi-participant conversations (like group chats or multi-agent interactions).

To avoid accidentally reusing thread IDs across different owners, use UUIDs or include the resource ID in the thread ID (e.g., `user_alice_${randomId}`).
