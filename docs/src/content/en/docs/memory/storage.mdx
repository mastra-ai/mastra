...
Mastra enforces a strict one-to-one mapping between thread IDs and resource IDs. Once a thread is created with a specific resource, that pairing is permanent and cannot be changed. Attempting to use the same thread ID with a different resource ID will result in an error:
...
Thread IDs must be globally unique across your application to prevent this collision.
