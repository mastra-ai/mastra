...
Because both calls use `resource: 'project-42'`, the writer can access the researcher's working memory and semantic embeddings. Each agent still has its own thread, so message histories stay separate. Note: resource-scoped Observational Memory is deprecated — use thread scope with [retrieval mode](/docs/memory/observational-memory#retrieval-mode-experimental) for cross-thread recall.
