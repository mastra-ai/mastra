...
If your Observer model is text-only or its API rejects multimodal input, set `observation.observeAttachments` to `false` to drop attachment parts before they reach the Observer. The readable placeholders (`[Image #1: ...]`, `[File #1: ...]`) and tool-result attachments are still preserved in the transcript, so the Observer can reason about what was shared without receiving the binary payload:
