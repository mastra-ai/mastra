...
- **Reflection view**: Before reflection, grouped observations are rendered as Markdown sections such as `## Group \`a1b2\``with`_range: \`msg-1,msg-2\`_\` metadata.
...
When thread-scoped `graph: true` is enabled, OM keeps each group's `_range` metadata visible in the actor context and registers a `recall` tool. The actor passes a cursor message id plus pagination arguments when it needs exact wording, tool outputs, or chronology that the observation summary compressed away.
