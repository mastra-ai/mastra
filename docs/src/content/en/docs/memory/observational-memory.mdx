...
It uses background agents for managing memory. When no model is set, the default is `model: 'auto'`.
...
The Observer and Reflector run in the background. Any model that works with Mastra's [model routing](/models) (`provider/model`) can be used. When no model is set, the default is `model: 'auto'`.
...
For a plain router-backed model instance, automatic selection can reuse its `provider/model` ID and select the provider's low-cost model. It preserves the original instance when the router has custom routing configuration, such as a URL, API key, headers, transport API, or custom gateway, because a sibling model isn't known to share that route. It also preserves router instances from older compatible Core versions that don't expose routing provenance. Pass a concrete model ID to the role to override either behavior.
...
To pin a model instead, set a concrete model ID. We've also successfully tested `__GATEWAY_OPENAI_MODEL_MINI__`, `__GATEWAY_ANTHROPIC_MODEL_HAIKU__`, `deepseek/deepseek-reasoner`, `deepseek/deepseek-v4-pro`, `deepseek/deepseek-v4-flash`, `xai/grok-4-1-fast`, `qwen3`, and `glm-4.7`.
