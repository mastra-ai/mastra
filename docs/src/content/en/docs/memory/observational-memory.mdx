...
Use `observation` and `reflection` to select agents or override their instructions, model, and `maxSteps`. Set `tools: false` to remove the `knowledge_search`, `knowledge_read`, and `knowledge_browse` tools from the main agent.
...
Activity uses a bounded state signal with cache-key deduplication, so unchanged snapshots don't add another signal. Reminders use the reactive signal lane and remain quiet when no relevant knowledge is found.

Capture shares the Observer's structured call. Remind adds at most one bounded agent run per observation cycle. Curate and learn run sequentially after a committed reflection, each with a durable cursor. Every built-in enforces `maxSteps` and propagates cancellation. Subconscious memory can't be enabled recursively.
