...
OM uses fast local token estimation for this thresholding work by default. Text is estimated with `tokenx`, while image parts use provider-aware heuristics so multimodal conversations still trigger observation at the right time. The same applies to image-like `file` parts when a transport normalizes an uploaded image as a file instead of an image part. Exact grouped provider token counting is available through the low-level `TokenCounter` as an opt-in path, but OM threshold checks stay local unless that path is explicitly enabled. For example, OpenAI image detail settings can materially change when OM decides to observe.
