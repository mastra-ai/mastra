...
Graph mode is only active for thread-scoped OM. Setting `graph: true` with `scope: 'resource'` has no effect — OM keeps resource-scoped behavior but skips graph-mode context and does not register the `recall` tool.
