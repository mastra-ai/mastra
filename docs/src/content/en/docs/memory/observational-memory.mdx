...
If you're unsure which model to use, start with the default `google/gemini-2.5-flash`. We've also successfully tested `__OPENAI_MODEL_MINI__`, `__ANTHROPIC_MODEL_HAIKU__`, `deepseek/deepseek-reasoner`, `deepseek/deepseek-v4-pro`, `deepseek/deepseek-v4-flash`, `xai/grok-4-1-fast`, `qwen3`, and `glm-4.7`.
