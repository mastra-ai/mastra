...
Agent skills are injected into the agent as tools (`skill`, `skill_search`, `skill_read`). Their results contain the skill's full instructions or file contents, so without a filter the Observer re-observes that text every time a skill is used. `skillResultFilter()` is a ready-made `beforeObservation` hook that drops results from those tools and leaves everything else in place, including the skill tool calls themselves.
...
Pass `toolNames` to drop results from a different set of tools. Because a hook is a function over the messages, it composes with your own filtering by chaining the outputs:
