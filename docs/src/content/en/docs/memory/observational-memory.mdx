...
### Grouped reflection flow

OM now keeps durable observation-group boundaries in storage while giving the Reflector a cleaner editing surface.

- **Canonical storage**: Observations are persisted as XML with `<observation-group>` tags that include a stable `id` and raw message `range` provenance.
- **Reflection view**: Before reflection, grouped observations are rendered as Markdown sections such as `## Group \`a1b2\`` with `_range: \`startId:endId\`_` metadata. Ranges use a colon delimiter between start and end message IDs.
- **Ephemeral anchors**: The Reflector sees temporary ordinal anchors like `O1` and `O1-N1` to make edits and references easier. These anchors are not stored.
- **Derived groups**: If the Reflector merges content from multiple groups, OM reconciles the edited Markdown back into canonical XML and records the merged provenance with `source-group-ids` plus the combined message ranges.

This two-representation model keeps provenance durable for graph-mode memory while making consolidation easier for the Reflector.

### Graph mode (experimental)

<Note>
Graph mode is experimental. The API may change in future releases.
</Note>

Set `observationalMemory.graph: true` with `scope: 'thread'` to preserve observation groups as durable pointers to raw message history.
...
Graph mode is only active for thread-scoped observational memory. If you set `graph: true` with `scope: 'resource'`, OM keeps resource-scoped memory behavior but skips graph-mode context and does not register the `recall` tool.

Graph mode lets the actor inspect the raw messages behind a stored observation group.

When thread-scoped `graph: true` is enabled, OM keeps each group's `_range` metadata visible in the actor context and registers a `recall` tool. Ranges use the format `startId:endId` with a colon delimiter. The actor extracts either the start or end message ID from the range and passes it as the cursor, along with pagination arguments, when it needs exact wording, tool outputs, or chronology that the observation summary compressed away.

```typescript
const tools = memory.listTools()

// Low-detail paged recall (default)
...
// High-detail recall with full tool args/results
await tools.recall?.execute?.(
  {
    cursor: 'message-id-2',
    page: 1,
  ...
  { memory, agent: { threadId: 'thread-id' } },
)

// Fetch a single part by index
await tools.recall?.execute?.(
  {
    cursor: 'message-id-2',
    partIndex: 1,
...
Positive `page` values move forward from the cursor, negative values move backward, and `page: 0` is treated as `page: 1`. The result includes `hasNextPage` and `hasPrevPage` flags so the agent knows whether more pages exist in each direction.

#### Detail levels

The `detail` parameter controls how much content each part shows:

- **`"low"`** (default) — truncated text and tool names only. Each part is labeled with a positional index (`[p0]`, `[p1]`, etc.) for targeted follow-up.
- **`"high"`** — full message content including tool arguments and results, still subject to a token budget.

#### Part index

Use `partIndex` with a cursor to fetch a single part at full detail. This is useful when a low-detail recall shows a tool result at `[p1]` that you want to read in full — call `recall` again with `cursor: 'message-id', partIndex: 1` instead of loading every part.

#### Token limiting

Results are capped at a token budget. When the output exceeds it, the result includes `truncated: true` and a `tokenOffset` indicating how many tokens were cut. The agent can paginate or use `partIndex` to target specific content.

`recall` is only available when `observationalMemory.graph` is enabled in thread scope, and it only returns messages from the current thread around the requested cursor.
