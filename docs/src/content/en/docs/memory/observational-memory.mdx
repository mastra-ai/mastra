...
- Registers a `recall` tool the agent can call to:
  ...
  - List all threads for the current user (`mode: "threads"`), with optional `before`/`after` date filters
  - Browse messages in other threads (`threadId` parameter)
  - Search across all threads by semantic similarity (`mode: "search"` with a `query` string) — requires a vector store and embedder
