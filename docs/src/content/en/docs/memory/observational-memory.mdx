...
Custom extractors default to `mode: 'structured'`. This runs a follow-up structured output call after the Observer or Reflector finishes. Use `mode: 'inline'` for short scalar values that should be emitted directly in the Observer or Reflector response.
