...
When vector search is configured, new messages are automatically indexed at buffer time and during synchronous observation (fire-and-forget, non-blocking).
