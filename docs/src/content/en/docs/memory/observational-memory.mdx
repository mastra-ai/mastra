...
Attachments that the agent recorded as unavailable, because they could no longer be downloaded, are never sent to the Observer. The Observer still sees their `[Image #1: ...]` or `[File #1: ...]` line. See [attachment download failures](/docs/agents/processors#recover-from-attachment-download-failures) for how Mastra records them.
