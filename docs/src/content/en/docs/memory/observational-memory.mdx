...
Both settings are opt-in and disabled by default — existing behavior is preserved unless you explicitly enable them.

### Token budget truncation

Set `observer.previousObservationTokens` to limit how many tokens of previous observations are sent to the Observer. Observations are tail-truncated, keeping the most recent entries. You can also set `0` for full truncation (empty previous-observations context) or `false` to disable truncation explicitly:
...
const memory = new Memory({
  options: {
    observationalMemory: {
      ...
      observation: {
        observer: {
          previousObservationTokens: 10_000, // keep only ~10k tokens of recent observations
        },
      },
    },
  },
...
- `previousObservationTokens: 0` → omit previous observations completely.
- `previousObservationTokens: false` (or unset) → keep full previous observations.

### Buffered reflection in observer context

When async reflection is buffering in the background, the Observer normally doesn't see it. Enable `observer.useBufferedReflection` to give the Observer access to pending reflection content:
...
This is useful when combined with `observer.previousObservationTokens` — the truncated context can be supplemented with the richer reflection summary.
