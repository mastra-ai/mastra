...
When the active main model is a model **instance** rather than a `provider/model` string, auto uses that instance as-is instead of substituting a low-cost model. An agent configured with `openai('__AI_SDK_OPENAI_MODEL_BASE__')` observes and reflects with that same model, not `__GATEWAY_OPENAI_MODEL_MINI__`. Pass a concrete model ID to the role if you want the low-cost model in that case.
