...
## Experimental Subconscious knowledge

`experimental_subconscious: new Subconscious()` adds durable knowledge to Observational Memory. Its two built-in agents run from completed observations:

- `remind` retrieves relevant existing knowledge for the current turn.
- `curate` receives each newly committed observation, reads related knowledge, and creates or reconciles durable records through constrained knowledge tools.
...
The default built-in order is `remind`, then `curate`. Subconscious agents do not run from reflection hooks. Curation is best-effort: a failed curator run is reported, but the observation remains successfully committed and is not automatically replayed. Configure custom observation extractors through `Subconscious({ observation: [...] })`; the removed experimental `capture`, `learn`, cadence, and manual worklist-curation options are no longer supported.
