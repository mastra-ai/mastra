...
Degenerate model output (a repetition loop that persists after retries) is treated as a model failure. Synchronous Observer and Reflector cycles follow `failurePolicy` for it: `'abort'` fails the turn, `'continue'` skips the cycle and reports the error to `onObservationEnd` or `onReflectionEnd`. Buffered reflection runs in the background, so it never fails the turn under either policy: it commits nothing, records a failure marker when a stream writer is attached, and reports the error to `onReflectionEnd`.
