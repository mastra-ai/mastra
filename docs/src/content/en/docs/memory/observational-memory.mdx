...
`WorkingMemoryExtractor` uses the normal extractor pipeline. It reads the active working memory template or schema, writes updates through the memory instance, and doesn't duplicate the value under OM extracted metadata.
