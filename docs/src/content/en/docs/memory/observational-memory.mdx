...
Temporal gap markers are off by default. Enable them with `temporalGapMarkers: true` in the `observationalMemory` config:
...
Mastra inserts a temporal gap marker when the gap is at least 15 minutes. The marker is stored in memory and also emitted as a transient reminder event, so clients can render it as a lightweight timeline hint.
