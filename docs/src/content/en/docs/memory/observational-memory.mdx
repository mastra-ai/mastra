...
Degenerate model output (a repetition loop that persists after retries) is treated as a model failure. The Reflector only treats it as a failure when every retry was degenerate or empty; otherwise it commits the smallest usable candidate. Synchronous Observer and Reflector cycles follow `failurePolicy` for it: `'abort'` fails the turn. Under `'continue'` the Observer skips the cycle and leaves the messages unobserved, the Reflector keeps the existing observations unreflected, and the error is reported to `onObservationEnd` or `onReflectionEnd`. Buffered reflection runs in the background, so it never fails the turn under either policy: it commits nothing, records a failure marker when a stream writer is attached, and reports the error to `onReflectionEnd`.
