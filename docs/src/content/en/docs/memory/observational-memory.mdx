...
| `observation.activationTTL` | none | Forces buffered observations to activate after a period of inactivity, even if `messageTokens` has not been reached yet. Accepts milliseconds or duration strings like `300_000`, `"5m"`, or `"1hr"`. Set this to your prompt cache TTL if you want activation to happen before the next cold prompt. |
| `reflection.bufferActivation` | `0.5` | When to start background reflection. `0.5` means reflection begins when observations reach 50% of the `observationTokens` threshold. |
| `reflection.blockAfter` | `1.2` | Safety threshold for reflection, same logic as observation. |
| `reflection.activationTTL` | none | Forces buffered reflections to activate after a period of inactivity, even if `observationTokens` has not been reached yet. Accepts milliseconds or duration strings like `300_000`, `"5m"`, or `"1hr"`. |

If you're relying on prompt caching, set `activationTTL` to match your cache TTL. That way, once a thread has been idle long enough for the cache to expire, the next request can activate buffered observations or reflections first and send a smaller compressed context window.
