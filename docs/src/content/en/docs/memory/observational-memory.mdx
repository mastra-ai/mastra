...
For large-scale deployments, you can use `ModelByInputTokens` to automatically select cheaper models for small inputs and stronger models for large inputs. This is useful when the Observer or Reflector processes varying amounts of content — small message batches use a fast model while large batches automatically switch to a more capable one.
...
const memory = new Memory({
  options: {
    observationalMemory: {
      // Observer uses different models based on input token count
      ...
      // Reflector also uses token-tiered selection
      reflection: {
        model: new ModelByInputTokens({
          upTo: {
...
The `upTo` keys are inclusive upper bounds. OM automatically sets the input token count in the request context before resolving the model, so the selection happens transparently based on the actual content being processed by the Observer or Reflector.
