...
Subconscious memory extends OM with durable, scoped knowledge. Observation captures entities and facts. Reflection maintains curated pages and reusable skills, while reminders surface relevant knowledge during the conversation.

Subconscious memory requires a storage adapter with knowledge support, an embedder, and a vector store because semantic retrieval doesn't fall back to lexical-only search.
...
- `curate`: Merges duplicate entities and repairs facts, then writes pages after reflection.
...
The remind, curate, and learn agents first use their per-agent `model` override. Without an override, they use an explicitly configured observational memory model, either the observation model or the top-level OM model. The main agent's model is the final fallback. Hosts that run these agents without a main-agent reference only need an OM model configured.
...
By default the resource rung comes from the run's `resourceId`. Setting `knowledgeResourceId` on the request context overrides the resource rung across all Subconscious scope resolution, letting a host share one knowledge graph across many runs, for example scoping knowledge to a project instead of a session. When the key is absent, behavior is unchanged.

Entities provide identity. Facts contain scoped prose, provenance, capture time, an optional event time, and an optional metadata record. The capture agent can attach a short `reason` explaining why a fact was worth keeping. Mastra stores it at `metadata.reason` for auditing and display. Pages are curated documents with wikilinks, but they aren't graph entities. `knowledge_search` combines lexical and semantic ranking, while `knowledge_read` and `knowledge_browse` retrieve records, mentions, and backlinks within the caller's scope.
...
A pin is a knowledge fact on a reserved entity, so each pin has a stable id, its own scope, and provenance. The curator maintains the set with `knowledge_pin`, `knowledge_edit_pin`, and `knowledge_unpin`. Both write tools accept an optional `reason` (stored at `metadata.reason`) recording why the pin must stay in context. Edits carry existing metadata forward. Unpinning soft-deletes the fact. Editing replaces it under a new id because facts are append-only.
...
`maxPins` (default 20) and `maxCharacters` (default 2000, total across the set) are enforced in the pin tools, so the set can't grow past its budget. Pins are off by default because their cost is per turn and permanent.

Curator-maintained pins depend on reflection running. Hosts whose conversations rarely reach reflection can additionally set `pins.capturePinning` (off by default, even with `pins: true`) to let the capture agent mark a fact as pinned at observation time. Capture-time pins are intended for durable user preferences and hard constraints only. They write through the same budget-enforced path as curator pins, and an over-budget capture-time pin is dropped rather than failing the capture cycle. A custom capture schema is always used verbatim and never gains the pin field.

### Signals, costs, and failures

Activity uses a bounded state signal with cache-key deduplication, so unchanged snapshots don't add another signal. Set `activity: false` to suppress routine snapshots. Failure states still emit so clients can surface background errors. Reminders use the reactive signal lane and remain quiet when no relevant knowledge is found.

Capture shares the Observer's structured call and model. Remind adds at most one bounded agent run per observation cycle. Curate and learn run sequentially after a committed reflection, each with a durable cursor. The curator can also run without a reflection. Call `Memory.runCuration` to execute it directly over the pending fact worklist, such as when a work item changes phase. To run it after every N committed observation runs, set `curationCadence` on the Subconscious config. The agent-backed built-ins enforce `maxSteps` and propagate cancellation. Subconscious memory can't be enabled recursively.
