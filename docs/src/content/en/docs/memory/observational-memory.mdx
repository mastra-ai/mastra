...
Idle observation is deferred while any unobserved message contains an incomplete tool call, including a client-executed call. Raw messages are still saved. Once the tool results arrive, a later completed turn can buffer those messages.
