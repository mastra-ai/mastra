...
Set `observationalMemory.graph: true` to preserve observation groups as durable pointers to raw message history.
...
When `graph: true` is enabled, OM keeps each group's `_range` metadata visible in the actor context and registers a `recall` tool. The actor passes a cursor message id plus pagination arguments when it needs exact wording, tool outputs, or chronology that the observation summary compressed away.
...
`recall` is only available when `observationalMemory.graph` is enabled, and it only returns messages from the current thread around the requested cursor.
