...
Activity uses a bounded state signal with cache-key deduplication. Unchanged snapshots don't add another signal. Reminders use the reactive signal lane and remain quiet when no relevant knowledge is found.

Capture shares the Observer's structured call. Remind adds at most one bounded agent run per observation cycle. Curate and learn run sequentially after a committed reflection and use separate durable cursors. All built-ins enforce `maxSteps`, propagate cancellation, and can't recursively enable Subconscious memory.
