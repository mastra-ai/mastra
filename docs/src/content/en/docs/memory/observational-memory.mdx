...
Image parts now contribute to this threshold with model-aware estimates too, so multimodal conversations trigger observation more accurately than before. For example, OpenAI image detail settings can materially change when OM decides to observe.
