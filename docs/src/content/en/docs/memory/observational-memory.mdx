...
### Choose reflection or rolling archives

By default, OM uses reflection when observations exceed their threshold (default: 40,000 tokens). The Reflector combines related items, condenses older information, and rewrites the observation log. New observations append after the rewritten log, and later reflections process the complete log again.

Use rolling archives instead when you want the actor to keep a smaller live observation tail and retrieve older observations on demand. Archive mode retires complete old observation groups without calling the Reflector. The original raw messages remain in memory storage.

| Mode | Actor context | Older information | Model work |
| - | - | - | - |
| Reflection (default) | Rewritten observation log plus recent messages | Condensed into the active log | Reflector calls at the configured threshold |
| Rolling archives | Bounded archive catalog, live observation tail, and recent messages | Retired observation text remains directly recallable | No Reflector calls; catalog summaries come from the existing Observer call |

Configure archive mode under `observation.archive`:
...
When live observations reach `afterTokens`, OM retires complete oldest groups and keeps about `keepTokens` of the newest observations live. It never splits an observation group. The actor receives the live tail and an `<archived-observations>` catalog whose rendered size never exceeds `maxCatalogTokens`. Each catalog entry includes a stable archive ID and source metadata for later recall.

All three fields are optional inside the `archive` object and use the values shown above by default. The object itself is opt-in. Don't configure `reflection` on the same OM instance because archive mode and explicit reflection are mutually exclusive.

The result is a three-tier system:

1. **Recent messages**: Exact conversation history for the current task
1. **Live observations**: The newest information recorded by the Observer
1. **Reflections or archives**: Condensed observations in reflection mode, or retired observations available through recall in archive mode
...
Archive mode enables direct recall automatically, even when you omit `retrieval`. The agent first calls `recall` with `mode: "observations"` to read the retired observation text. Results include the archive ID, group ID, and raw-message source pointers. If exact wording or tool output is needed, the agent follows those pointers with a second `mode: "messages"` call.

Long retired groups are returned in bounded chunks. The result's `nextCharOffset` continues the same group when passed back with its `archiveId` and `groupId`. Archive browsing is newest-first and returns an opaque `nextCursor` for the next page. Resource and thread scope rules still apply, so projected results never include sibling-thread group text.
...
#### Transform recall results with Code Mode

Direct recall returns one bounded page at a time. Enable Memory-managed Code Mode when the agent needs to make several recall calls, filter or aggregate their results in TypeScript, and return only the transformed value. Mastra keeps the direct `recall` tool and also registers `execute_memory_recall`. The code tool exposes only `external_recall`, with the same modes, scope rules, authorization, request context, cancellation, and tracing as direct recall.

Pass an explicit sandbox when the Memory configuration owns the execution boundary:
...
const memory = new Memory({
  options: {
    observationalMemory: {
      ...
      retrieval: {
        vector: true,
        codeMode: { sandbox: new LocalSandbox() },
      },
    },
  },
})
...
`LocalSandbox` runs model-authored code as a host process. Use it only when host execution is intentional. A remote sandbox can be passed in the same `sandbox` field.

To use the agent's workspace sandbox, set `codeMode: true` and configure the workspace on the agent:
...
const agent = new Agent({
  ...
  instructions: 'Use recall when past context is needed.',
  model: '__GATEWAY_OPENAI_MODEL__',
  memory,
  workspace: new Workspace({ sandbox: new LocalSandbox() }),
})
```

A transport that owns its isolation boundary can run without a workspace sandbox:
...
const memory = new Memory({
  options: {
    observationalMemory: {
      ...
      retrieval: {
        codeMode: {
          transport: new IsolatedVmCodeModeTransport({ memoryLimitMb: 128 }),
        },
      },
    },
  },
...
If neither the Code Mode configuration nor the agent workspace provides a sandbox, invocation fails unless the transport declares `requiresSandbox: false`. Mastra doesn't fall back to host execution.

Memory-managed Code Mode requires `@mastra/core@1.68.0-alpha.0` or newer, where `createCodeMode` is exported. The Code Mode factory is loaded only on the first `execute_memory_recall` call, so this option doesn't narrow Memory's existing Core compatibility when Code Mode is disabled. If an installed Core version otherwise supports the configured Memory features but doesn't export `createCodeMode`, Memory construction, direct recall, and tool listing don't load that export. The first Code Mode call reports the installed Core version and asks you to upgrade and align the Core and Memory versions.
...
- Registers a `recall` tool the agent can call to:
  ...
  - Read retired observation groups (`mode: "observations"`) when archive mode is enabled
  - When `scope: 'thread'`: restrict browsing, search, and archived observations to the current thread only
