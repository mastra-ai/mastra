...
Switching from resource scope to thread scope doesn't migrate existing observations. Each thread starts a new, empty thread-scoped observation record, and messages already observed under resource scope aren't observed again. Keep facts that must carry over in resource-scoped working memory.
