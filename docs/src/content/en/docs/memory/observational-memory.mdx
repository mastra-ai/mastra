...
- The shape must be exactly `{ v: 0, source: 'client', key: 'client', tokens: N }`, where `tokens` is a finite non-negative number.
- Caller-supplied estimates survive cache rotations because they use the `'client'` source instead of the framework's content-hash key.
