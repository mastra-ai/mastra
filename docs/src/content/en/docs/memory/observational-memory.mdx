...
Set `retrieval: true` to enable the recall tool for browsing raw messages. No vector store needed.
...
#### With cross-thread browsing

Set `retrieval: { scope: 'resource' }` to enable cross-thread browsing — listing threads, browsing other threads, and navigating across conversations:
...
Set `retrieval: { vector: true }` to also enable semantic search. This reuses the vector store and embedder already configured on your Memory instance. Combine with `scope: 'resource'` for cross-thread search:
...
When vector search is configured, new messages are automatically indexed after each observation completes (fire-and-forget, non-blocking).

By default, `retrieval: true` and `retrieval: { vector: true }` scope the recall tool to the current thread only. Set `scope: 'resource'` to enable cross-thread access.
...
- Registers a `recall` tool the agent can call to:
  ...
  - With `scope: 'resource'`: list all threads (`mode: "threads"`), browse other threads (`threadId`), and search across all threads

Retrieval mode is only active for thread-scoped OM (`observationalMemory.scope: 'thread'`). Setting `retrieval` with OM `scope: 'resource'` has no effect.
