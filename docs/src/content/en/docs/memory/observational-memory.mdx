...
Set `observer.previousObservationTokens` to limit how many tokens of previous observations are sent to the Observer. Observations are tail-truncated, keeping the most recent entries. When a buffered reflection is pending, the already-reflected lines are automatically replaced with the reflection summary before truncation is applied.
...
- `previousObservationTokens: 2000` → default; keeps \~2k tokens of recent observations.
- `previousObservationTokens: 0` → omit previous observations completely.
- `previousObservationTokens: false` → disable truncation and keep full previous observations.
