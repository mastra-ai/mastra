...
- **The `update-working-memory` tool is unchanged.** Writes still flow through the existing tool; the next model step picks the new value up automatically.
...
The default (`useStateSignals: false`) keeps the existing system-message behavior unchanged.
