...
Trimmed messages stay in storage, but a per-thread boundary is persisted so they are excluded from later turns. Setting `messageHistory` without `lastMessages` disables the default 10-message cap; set both to combine a count cap with a token budget.
