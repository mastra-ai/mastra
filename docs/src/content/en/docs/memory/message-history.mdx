...
Boundary persistence uses an optional storage operation. Official storage adapters with compare-and-set or transaction support implement it, so a persisted boundary never moves backwards even when several app instances share one database. ClickHouse and Cloudflare Workers KV cannot return an authoritative boundary across replicas, so they never persist one; the token budget is still applied per request and prompts stay within budget.

Older or custom storage adapters don't implement the operation. Each request still applies the token budget, but the trim boundary isn't persisted across independent storage instances, so trimmed messages can reappear in a later turn. Upgrade to the latest official storage package to enable distributed boundary persistence — no other package needs to upgrade at the same time.
