...
Boundary persistence uses an optional storage operation. Official storage adapters with compare-and-set or transaction support implement it, so a persisted boundary never moves backwards even when several app instances share one database. ClickHouse and Cloudflare Workers KV can't return an authoritative boundary across replicas, so they never persist one. The token budget is still applied per request, so prompts stay within budget.

Older or custom storage adapters don't implement the operation. Each request still applies the token budget, but the trim boundary isn't persisted across independent storage instances, so trimmed messages can reappear in a later turn. Upgrade the official storage package to enable distributed boundary persistence. No other package needs to upgrade.
