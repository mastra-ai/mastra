...
For models with large context windows, count-based limiting (`lastMessages`) may not be sufficient. The `maxTokens` option lets you limit memory history by estimated token count instead:
...
export const agent = new Agent({
  ...
  memory: new Memory({
    options: {
      lastMessages: Number.MAX_SAFE_INTEGER, // Retrieve all messages
...
1. Estimates the total token count of all messages (memory history + new input) using a character count / 4 approximation
1. If the total exceeds `maxTokens`, removes the oldest memory history messages first
1. Never removes input messages from the current turn

Token estimation uses a character count / 4 approximation, which works well for English text. For CJK languages (Japanese, Chinese, Korean), actual token counts may differ. Consider setting `maxTokens` conservatively (e.g., 80% of your model's context window) to account for estimation variance.

You can use `maxTokens` alongside `lastMessages` to apply both count-based and token-based limits. This is useful for preventing context window overflow while keeping a reasonable number of recent messages.
