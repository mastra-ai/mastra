...
`lastMessages` counts conversation messages, not the rows Mastra stores. Features such as [state signals](/docs/harness/signals) add their own rows alongside a turn; those rows stay in the window when they fall inside it, but they never reduce the number of messages you asked for. When a state snapshot falls outside the window, the processor that owns it sends a fresh snapshot.
