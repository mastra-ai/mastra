...
Install the Mastra memory module along with a [storage adapter](/docs/memory/storage#supported-providers) for your database. For the purpose of these examples you'll use the local `@mastra/libsql` provider that stores the database inside a `mastra.db` file.
...
Threads and messages are created automatically. To create them programmatically, see [`createThread()`](/reference/memory/createThread) and [`saveMessages()`](/reference/memory/memory-class#savemessages).
...
To query threads and messages, call `getMemory()` on an agent to get a `Memory` instance:
...
The `Memory` instance gives you access to functions for listing threads, recalling messages within them, and more.
