...
If you don't need the copied messages returned, for example when forking a long thread, use `copyThread()`. It copies the thread inside the storage backend without loading message content into your process.
