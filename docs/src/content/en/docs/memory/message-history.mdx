...
The tool name, call ID, and args always come from the stored record. Echo-only metadata and other client fields are dropped. An edited resend can replace an unsealed user message, but edits to a sealed user message are ignored because observational memory already depends on its canonical content.

Stored-record lookup fails closed. If lookup is unavailable or fails, Mastra aborts persistence rather than risk replacing a canonical record with a lossy echo. Sending only the new message remains the recommended client behavior.
:::
