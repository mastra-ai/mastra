...
- **Input**: Estimates the total token count of all messages (memory history + new input). If the total exceeds `maxTokens`, removes the oldest memory history messages until the total is within budget. Input messages are never removed.
