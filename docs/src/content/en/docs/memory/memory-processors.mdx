...
1. Adds it to the agent's input processors (runs after `MessageHistory`)

**What it does:**

- **Input**: Counts the total tokens of all messages (system messages + memory history + new input) using tiktoken (o200k\_base encoding). If the total exceeds `maxTokens`, removes the oldest memory history messages until the total is within budget. Input and system messages are never removed.
...
const agent = new Agent({
  ...
  memory: new Memory({
    ...
    options: {
      lastMessages: Number.MAX_SAFE_INTEGER, // Load all messages
