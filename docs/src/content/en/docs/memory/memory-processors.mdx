...
- **Input**: Counts the total tokens of all messages (memory history + new input) using tiktoken (o200k\_base encoding). If the total exceeds `maxTokens`, removes the oldest memory history messages until the total is within budget. Input messages are never removed.
