...
FastEmbed also exposes the multilingual E5 model for non-English content. E5 is asymmetric, so it ships as two models: `multilingualE5LargePassage` for text you index and `multilingualE5LargeQuery` for search text.
...
Multilingual E5 produces 1024-dimensional vectors. Your vector index must be created with matching dimensions, and E5 vectors cannot be mixed with vectors from another embedder in the same index.
