...
<Steps>
  ...
  <StepItem>
    ...
    <Inject>
      You can also use OpenAI (`OPENAI_API_KEY`) or Anthropic (`ANTHROPIC_API_KEY`) instead of Google Gemini. Go to [https://mastra.ai/models/env-list](https://mastra.ai/models/env-list) for a full list of supported providers and their environment variable names.
    </Inject>
  </StepItem>
  ...
  <StepItem>
    ...
    <Inject>
      Use `import { Agent } from "@mastra/core/agent"`. Constructor takes `{ id, name, instructions, model: '__GATEWAY_OPENAI_MODEL__' }`. The model string is Mastra's model router format ('provider/model'; use / and not : to separate provider and model). Do not install any ai-sdk package unless Mastra's documentation says otherwise.
    </Inject>
