...
:::info
Use this guide to manually build a standalone Mastra server step by step. In most cases, it's quicker to follow a [getting-started guide](start), which achieves the same result using the [`mastra create`](../../../reference/v1/cli/create-mastra) command. For existing projects, you can also use [`mastra init`](../../../reference/v1/cli/mastra#mastra-init).
:::
...
We've shortened and simplified the `weatherTool` example here. You can see the complete weather tool under [Giving an Agent a Tool](../agents/using-tools).
...
You can now launch [Studio](studio) and test your agent.
