...
- You'll need an API key from a large language model provider to complete setup. We recommend starting with [Gemini](https://aistudio.google.com/app/api-keys), as you likely already have a Google account and they don't require a card.
- Node.js 20 or later.
...
You can use flags with `create mastra` like `--no-example` to skip the example weather agent or `--template` to start from a specific [template](/templates). Read  the [CLI reference](/references/cli/create-mastra) for all options.
</Callout>
...
- Learn more about [Mastra's features](/docs#why-mastra)
