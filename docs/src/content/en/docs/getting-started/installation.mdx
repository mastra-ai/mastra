...
Update the `planActivities` step in `src/mastra/workflows/weather-workflow.ts` from:
```ts filename="src/mastra/workflows/weather-workflow.ts" showLineNumbers{150} copy /stream/
...
To:
```ts filename="src/mastra/workflows/weather-workflow.ts" showLineNumbers{150} copy /streamVNext/
