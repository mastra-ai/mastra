...
<Tabs items={["Automatic Installation", "Manual Installation"]}>
<Tab>
...
3. Create an example agent, tool` and workflow
4. Configure agents to use `openai` as the LLM provider from `@ai-sdk`
5. Create a `.env` file with an `OPENAI_API_KEY` variable
6. Configure the MCP server in your IDE (if selected) for instant access to docs, examples, and help while you code
