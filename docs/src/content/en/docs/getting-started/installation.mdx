...
- Node.js `v20.0` or higher (or Deno `v1.40` or higher)  
...
Each tab displays the equivalent command for that specific package manager (npx, npm, yarn, pnpm, bun, deno).
...
<Tabs items={["npx", "npm", "yarn", "pnpm", "bun", "deno"]}>
  <Tab>
    ```bash copy
  ...
  <Tab>
    ```bash copy
    deno run -A npm:create-mastra@latest
    ```
  </Tab>
</Tabs>
...
<Tabs items={["npm", "pnpm", "yarn", "bun", "deno"]}>
  ...
  <Tab>
    ...
    deno add npm:typescript npm:tsx npm:@types/node npm:mastra@latest

    deno add npm:@mastra/core@latest npm:zod@^3 npm:@ai-sdk/openai
    ```
...
> **Note for Deno users**: You can also run Mastra commands directly with `deno run -A npm:mastra dev` and `deno run -A npm:mastra build`.
