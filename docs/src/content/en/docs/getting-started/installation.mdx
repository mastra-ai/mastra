...
- **Node.js** `v20.0` or higher (required for server-side execution).
- **Bun**: compatible via Node.js API support.
- **Deno**: compatible when using `--compat` and `--unstable` flags.
- **API key** from a supported [Model Provider](/docs/getting-started/model-providers).
