...
Each agent also has a session page at `/agents/<agentId>/session` that strips away the sidebar and information pane, showing only the chat interface. This is useful for quick internal testing or for sharing with less technical team members who don't need the full Studio chrome.

If you have [request context presets](/docs/server/request-context) configured via the `MASTRA_REQUEST_CONTEXT_PRESETS` environment variable, a preset dropdown appears in the header next to the Mastra logo, letting you switch between presets without navigating to the full request context page.
