...
Configure Studio's appearance and connection options. Choose a theme mode (Light, Dark, or System), change the Mastra instance URL to point Studio at a different running server, and set a custom API base path if your server uses a non-default route prefix. You can also add custom headers that Studio includes with every API request, which is useful for passing authentication tokens or other required headers to your server.
