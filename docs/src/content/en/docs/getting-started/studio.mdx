...
You can host the Mastra Studio on a sub-path of your existing application using the `path` configuration:
...
export const mastra = new Mastra({
  server: {
    port: 3000,
    path: "my-mastra-studio", // Studio will be available at /my-mastra-studio
  },
});
```
...
**Example URLs:**
- Default: `http://localhost:3000/` (studio at root)
- With sub-path: `http://localhost:3000/my-mastra-studio/` (studio at sub-path)
