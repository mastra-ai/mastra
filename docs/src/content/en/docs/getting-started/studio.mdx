...
The `mastra studio` command supports connecting to remote Mastra servers with custom API routes and authentication:

```bash
# Connect to a server with a custom API prefix
mastra studio --server-host myapp.vercel.app --server-api-prefix /api/v1

# Connect with authentication
mastra studio --server-host production.example.com --auth-header "Authorization: Bearer your-token"
...
**Available options:**
- `--server-host <host>` - Host of the Mastra API server (default: localhost)
- `--server-port <port>` - Port of the Mastra API server (default: 4111)
- `--server-protocol <protocol>` - Protocol of the Mastra API server (default: http)
- `--server-api-prefix <prefix>` - API route prefix of the Mastra server (default: /api)
- `--auth-header <header>` - Auth header to include with requests (format: "name: value")
