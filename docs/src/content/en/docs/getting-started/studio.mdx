...
Playground helps you build agents quickly. It provides an interactive UI for testing your agents, workflows, and tools, along with a REST API that exposes your Mastra application as a local service. This lets you start building without needing to integrate Mastra into your project right away.
...
The local development server exposes a complete set of REST API routes, allowing you to programmatically interact with your agents, workflows, and tools during development. This is particularly helpful if you plan to deploy the Mastra server because the local development server uses the exact same REST API routes as the [production environment](/docs/server-db/production-server).
...
By default, the development server runs at http://localhost:4111, but you can change the `host` and `port` in the Mastra server configuration:
