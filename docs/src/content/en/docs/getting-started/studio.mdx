...
In Agent Chat, runtime controls are grouped in **Chat Settings** (next to the dictation button), thread actions (clone/delete) live in the left conversation sidebar, and **Memory** is available from that same left sidebar. The right panel is now a compact **Agent Overview** panel that starts collapsed by default.
