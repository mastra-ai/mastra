...
Mastra is built on top of the AI SDK to help teams build their proof-of-concepts into production-ready apps.
When creating agents in Mastra, you plug in any AI SDK-compatible model provider.
...
<Tabs items={["Official providers", "OpenAI compatible providers", "Community providers"]}>
  ...
  <Tabs.Tab>
    ...
    We will look at one such provider, the Ollama provider available in the `ollama-ai-provider` package.
