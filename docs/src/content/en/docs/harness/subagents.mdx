...
- `id`: Unique identifier for this subagent type
- `name`: Human-readable name shown in tool output
- `description`: Used in the auto-generated tool description
- `instructions`: System prompt for the subagent
- `tools`: Direct tool access for the subagent
- `allowedHarnessTools`: Subset of harness-level shared tools
- `allowedWorkspaceTools`: Subset of workspace tools (by exposed name)
- `defaultModelId`: Model for this subagent type
- `maxSteps`: Maximum loop iterations (defaults to 50)
