...
When a tool's policy is `ask`, the harness emits a `tool_approval_required` event. Your UI should display a prompt and call `respondToToolApproval`:
...
Grant a category or tool for the rest of the session so it runs without further prompting:
