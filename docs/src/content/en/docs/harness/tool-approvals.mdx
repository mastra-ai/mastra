...
The Harness owns permission *policy* (which categories require approval); the [`Session`](/docs/harness/session) owns the *grants* a user makes during a conversation. Grant a category or tool for the rest of the session so it runs without further prompting:
