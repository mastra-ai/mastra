...
If you pass `idleTimeoutMs`, a stream that goes quiet for that long ends with an error chunk and detaches the observer. The run itself is only cleaned up when you also pass `isAlive` and it reports the run is no longer running.
...
Every `stream()` and `observe()` call returns a `cleanup` function. Calling it unsubscribes from PubSub, removes the run from the internal registry, and clears the run's cached events, so no one can replay the run afterward. When you don't call it, an automatic timer (`cleanupTimeoutMs`) runs cleanup once the run finishes, errors, or is aborted, but detaching an observer or hitting a plain `idleTimeoutMs` doesn't start this timer. Set `cleanupTimeoutMs: 0` to turn the timer off.
