...
All three return an object you register with `Mastra` the same way as a regular agent. `createDurableAgent()` and `createEventedAgent()` return class instances that extend `Agent`. `createInngestAgent()` returns a Proxy-backed object that forwards `Agent` methods to the underlying agent. With all three, a signal that wakes an idle thread (for example from `sendSignal()` or `sendNotificationSignal()`) runs through the durable `stream()`.
