...
If you pass `idleTimeoutMs`, a stream that receives no chunks for that long ends with an error chunk and detaches the observer. The run itself is only cleaned up when you also pass `isAlive` and it reports the run is no longer running.
