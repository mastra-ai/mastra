...
These `running` checkpoints are only written when crash recovery is enabled. With the default `recovery.durableAgents: 'off'`, durable agents persist snapshots only for `pending`, `paused`, and `suspended` runs (the artifacts human-in-the-loop resume depends on) and skip the per-step `running` writes entirely. See [snapshot persistence](#snapshot-persistence) for the full policy and how to override it.
