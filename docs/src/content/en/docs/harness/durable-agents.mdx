...
Every published event is recorded in the cache before it's delivered live, so publish latency is bounded by the round-trip to the cache. `@mastra/redis` and `@mastra/valkey` record each event in a single round-trip. If the cache is remote (for example, in another region) and you don't need to resume a particular topic, use `shouldCache` to publish that topic straight through:
