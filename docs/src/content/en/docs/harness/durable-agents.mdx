...
Every `stream()` and `observe()` call returns a `cleanup` function. Calling it unsubscribes from PubSub, removes the run from the internal registry, and clears the run's cached events, so no one can replay the run afterwards. If you forget to call it, an automatic timer fires after the stream ends, but calling `cleanup()` yourself frees resources immediately.
