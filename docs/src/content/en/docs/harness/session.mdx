...
The resource ID is set when creating a session via [`harness.createSession()`](/reference/harness/harness-class#createsession-resourceid-id-ownerid) and defaults to the harness `id`. See [Resource IDs](/docs/harness/threads-and-state#resource-ids) for how it scopes threads.
