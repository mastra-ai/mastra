...
Use [`cancelPendingSignals()`](/reference/agents/agent#cancelpendingsignalsoptions) to remove selected pending input without stopping the active run. Cancellation is scoped to the memory thread, including input submitted by other Agents sharing the same runtime and PubSub instance.
...
const { cancelledSignalIds } = agent.cancelPendingSignals({
  ...thread,
...
Only IDs removed by this call appear in `cancelledSignalIds`. Input already handed to execution or another owner can't be cancelled this way. Cancellation doesn't delete saved messages, notification records, or state updates. It doesn't revoke an earlier acceptance acknowledgement.
...
You can also call `agent.abortThreadStream({ ...thread, clearPendingSignals: true })` without a subscription. These controls don't unsubscribe listeners. Later messages can still start a new run.

Selected-ID cancellation only affects the current process. Clear-on-abort also sends the clear flag to a remote active owner through PubSub, but it doesn't clear every process's local queues. Neither operation cancels continuations created by `continueWithMessages()`.
