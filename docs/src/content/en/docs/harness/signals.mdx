...
Only IDs removed locally by this call appear in `cancelledSignalIds`. Mastra publishes those IDs through PubSub so other processes subscribed to the thread can cancel matching pending copies, but this asynchronous propagation doesn't confirm remote cancellation. IDs missing locally aren't published, and input already handed to execution can't be cancelled this way. Cancellation doesn't delete saved messages, notification records, or state updates. It doesn't revoke an earlier acceptance acknowledgement.
...
Cross-process cancellation requires a shared PubSub backend. Clear-on-abort sends the clear flag to a remote active owner through PubSub, but it doesn't clear every process's local queues. Neither operation cancels continuations created by `continueWithMessages()`.
