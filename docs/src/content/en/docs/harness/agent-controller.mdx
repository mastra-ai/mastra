...
### Select Observational Memory models

Observer and reflector model choices are independent. Each role can use an explicit model or an automatic selection that the host resolves from the active main model and session state.

Configure automatic selection with a synchronous resolver that returns a concrete model ID:
...
const controller = new AgentController({
  ...
  omConfig: {
    ...
    defaultObserverModelId: '__GATEWAY_OPENAI_MODEL_MINI__',
    defaultReflectorModelId: '__GATEWAY_OPENAI_MODEL_MINI__',
    resolveAutoModelId: ({ currentModelId }) => currentModelId,
  },
})
```

`session.om.observer.selection()` returns the persisted intent. `session.om.observer.modelId()` returns the effective concrete model. A host can therefore change the main model without rewriting automatic role settings.

Reset one role to automatic selection without changing the other:
...
If the resolver returns `undefined` or throws, the role uses its concrete `defaultObserverModelId` or `defaultReflectorModelId` fallback. See the [`Session` Observational Memory API](/reference/agent-controller/session#observational-memory) for explicit selection and compatibility methods.
