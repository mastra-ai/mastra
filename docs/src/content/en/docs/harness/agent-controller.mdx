...
### Subscribe from a browser or React app
...
const subscription = await session.subscribe({
  ...
  onReconnect: async () => hydrate(await session.state()),
  reconnect: true,
})
...
The server doesn't replay events missed while the stream was down, so `onReconnect` has to re-sync. Call `session.state()` for the current mode, model, and thread, and `session.listMessages(threadId)` when the UI also needs the messages it missed. `session.displayState.get()` isn't part of the client surface.
