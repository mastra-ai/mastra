...
Pass `modeId` and `modelId` to seed a newly created Session's initial mode and model instead of creating it on the controller's defaults and switching afterwards. The seeds persist, so a restart restores them. They are creation-time inputs, not overrides: a resumed thread's persisted selection wins, and a cached Session is returned unchanged.
