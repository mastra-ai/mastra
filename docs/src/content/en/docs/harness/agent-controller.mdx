...
For a coding agent, swap `new Agent()` below for [`createCodingAgent()`](/reference/coding-agent/create-coding-agent) and pass `workspace: undefined` so the controller's workspace is the only one in play.
