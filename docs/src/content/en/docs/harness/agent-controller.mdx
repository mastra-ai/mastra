...
How you connect depends on where the UI runs. A terminal UI or a server that owns the controller holds the `Session` object and subscribes to it directly. A browser or React UI runs in a different process, so it reaches the same session over the controller's HTTP routes with [`@mastra/client-js`](/reference/client-js/mastra-client).

### Server-side sessions (TUI or Node)
...
### Client-side sessions (browser or React)
...
async function hydrate() {
  const [state, messages] = await Promise.all([session.state(), session.listMessages(threadId)])
...
`create()` returns the bound `threadId`, and a resumed session already has messages the subscription won't replay, so hydrate from `session.state()` and `session.listMessages(threadId)` before subscribing. The server doesn't replay events missed while the stream was down either, so run the same hydration from `onReconnect` before applying further events. `session.displayState.get()` isn't part of the client surface.
