...
Every stream connection, including a reconnect, starts with a `display_state_changed` event carrying the current thread's tasks, cumulative token usage, queued follow-ups, and memory progress.

Message history isn't part of that snapshot. Recover messages missed during a disconnect with `session.listMessages(threadId)`. For the current mode, model, and runtime state, call `session.state()` instead of the server-side `session.displayState.get()`.
