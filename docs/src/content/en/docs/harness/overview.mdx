...
Each Harness manages a `Session` — the per-conversation runtime state that tracks the active mode, model, thread binding, permission grants, follow-up queue, and token usage. In a multi-user host one Harness can serve many sessions.
