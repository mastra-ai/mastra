...
Thread *lifecycle* transitions (create, switch, clone, delete) live on the Harness because they coordinate the shared thread lock and emit events. The active thread binding and thread/message *reads* live on the [`Session`](/docs/harness/session) as `harness.session.thread`.
