...
You can manage all three sources through the Studio UI or programmatically through the server API. You can mix tool sources freely, override descriptions per-agent, and version changes alongside other agent configuration.

## When to use editor tools

Use the editor to manage tools when you want to:

- Add or remove tools from an agent without changing code.
- Browse and pick tools from a catalog.
...
- Let non-developer team members manage tool access.
