...
The typical flow is: Edit the draft, test it, then activate it to make it the published version. The previously published version becomes archived so you can restore it if needed. You can also unpublish to clear the active version. A code-defined agent then falls back to its code configuration, and saved versions stay available.

Saves through the API auto-publish by default. Pass `publishOnSave: false` on create or update to keep drafts unpublished until you activate them. You can do this through Studio or programmatically through the API. See [version management](/reference/client-js/agents#version-management) for the Client SDK methods and code examples.
