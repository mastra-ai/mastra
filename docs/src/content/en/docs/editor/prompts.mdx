...
Non-technical team members can edit prompt content and test different phrasings, plus publish changes. Every edit is versioned, so you can compare prompt variations and roll back, plus track what changed over time.
...
Prompt blocks follow the same [versioning lifecycle](/docs/editor/overview#versioning) as agents. Each prompt block has a draft that you can edit and publish as a versioned snapshot. As a result, prompt content can be versioned and rolled back independently from the agent that uses it.
