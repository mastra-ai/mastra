...
Mastra requires `module` and `moduleResolution` values that support **ES modules**. Older settings like `CommonJS` or `node` are incompatible with Mastra’s ESM-only packages and will cause resolution errors.
...
> Set `module` to `ES2022` and `moduleResolution` to `bundler` to ensure compatibility with Mastra’s ESM-only packages. Alternatives like `Node16` or `NodeNext` are also supported.
