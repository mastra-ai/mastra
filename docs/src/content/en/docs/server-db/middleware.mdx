...
### Setting user or request context via `runtimeContext`

You can populate `runtimeContext` dynamically in server middleware by extracting information from the request. Use `runtimeContext` when you need to set user or request context that you need to pass to agents, tools, or workflows.

In this example, the `temperature-unit` is set based on the Cloudflare `CF-IPCountry` header to ensure responses match the user's locale.
