...
The `base` option allows you to host the Mastra Studio/Playground on a sub-path of your existing application. When configured, the studio will be available at `http://your-host:port/your-base` instead of the root. This is useful when integrating with existing applications or when using tools like Cloudflare Zero Trust where having all applications on the same domain simplifies authentication.

**Example with sub-path:**
- Without base: `http://localhost:4111/` (studio at root)
