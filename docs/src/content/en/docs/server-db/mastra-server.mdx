...
You can configure server `port`, `host`, `path`, and `timeout` in the Mastra instance.
...
export const mastra = new Mastra({
  ...
  server: {
    ...
    path: "my-mastra-studio", // Sub-path for the studio (optional)
    timeout: 10000, // Defaults to 30000 (30s)
  },
});
```

The `path` option allows you to host the Mastra Studio/Playground on a sub-path of your existing application. When configured, the studio will be available at `http://your-host:port/your-path` instead of the root. This is useful when integrating with existing applications or when using tools like Cloudflare Zero Trust where having all applications on the same domain simplifies authentication.

**Example with sub-path:**
- Without path: `http://localhost:4111/` (studio at root)
- With path: `http://localhost:4111/my-mastra-studio/` (studio at sub-path)
