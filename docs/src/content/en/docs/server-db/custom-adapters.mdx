...
Create a custom adapter when you need to run Mastra with a framework other than Hono or Express. This might be necessary if you have specific request/response handling requirements that the built-in adapters don't support.
...
Here's a complete skeleton showing all the required methods. This uses pseudocode for framework-specific parts—replace with your framework's actual APIs:
...
Once your adapter is implemented, use it the same way as the built-in adapters:
