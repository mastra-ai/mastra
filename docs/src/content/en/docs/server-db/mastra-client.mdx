...
Use `.stream()` for real-time responses with an array of message objects that include `role` and `content`. The `stream()` method returns a `MastraClientModelOutput` object:
...
export async function action() {
  ...
  return new Response(stream.fullStream, {
