...
Shared pub/sub coordinates active runs. Shared storage coordinates deduplication and interactive state, on a backend that supports shared channel state (see [Error handling and delivery](#error-handling-and-delivery)). Multi-instance deployments need both.
