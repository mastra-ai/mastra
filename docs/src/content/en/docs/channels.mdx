...
Once the adapter returns `200`, the platform considers the event delivered and never retries it, no matter what happens to the agent run. Errors at this stage are handled in Mastra:
...
Retries mean the same event can arrive more than once. This happens when the acknowledgment is slow or lost, for example a Slack retry after the three-second deadline, or a Telegram redelivery after a network failure.
