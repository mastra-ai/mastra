...
Use `handlers.onAction` to handle button clicks and select changes from cards you post yourself. The handler receives the Chat SDK action event, the default handler, and the same context as the other channel handlers. The default handler processes the built-in Approve and Deny buttons and ignores every other action ID, so call it for events you don't handle to keep tool approval working:
