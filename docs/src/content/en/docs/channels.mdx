...
Channels require [storage](/docs/storage) unless you provide your own `state` adapter in the channel config. Starting an agent with channels and neither configured throws. Storage lets Mastra persist channel state, thread subscriptions, tool approvals, and memory across restarts:
...
Storage shares that channel state between instances when the backend supports it. The Convex, Google Cloud Spanner, libSQL, MySQL, and PostgreSQL stores support shared channel state, and every instance needs to reach the same store. Other backends, and store packages older than your `@mastra/core` version, keep channel state per-process: each instance keeps its own deduplication keys and they all reply to the same message. Run those with a single instance, or provide a shared `state` adapter in the channel config. Interactive elements have the same constraint: a modal opened by one instance can't be submitted through another.
...
Shared pub/sub coordinates active runs. Deduplication and interactive state need shared channel state, through storage on a backend that supports it (see [Error handling and delivery](#error-handling-and-delivery)) or a shared `state` adapter in the channel config. Multi-instance deployments need both.
