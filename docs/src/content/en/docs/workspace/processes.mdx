...
Create a workspace with a sandbox that supports processes. `LocalSandbox`, `E2BSandbox`, and `DaytonaSandbox` all include built-in process managers:
...
When a sandbox has a process manager, Mastra automatically gives agents three tools for background process management:

| Tool | Description |
| - | - |
| `execute_command` with `background: true` | Start a command in the background. Returns a PID immediately. |
| `get_process_output` | Read stdout, stderr, and exit status of a background process by PID. Supports `tail` to limit lines and `wait: true` to block until exit. |
| `kill_process` | Kill a background process by PID. Returns the last 50 lines of output. |

Output from all tools is automatically truncated to the last 200 lines (configurable via `tail`) with a 2,000-token safety limit.

You can configure these tools through the workspace's `tools` option:
...
When agents start background processes, you can receive lifecycle callbacks for stdout, stderr, and process exit. Configure these through the `backgroundProcesses` option on the `execute_command` tool:
...
Use `null` or `false` for cloud sandboxes (for example, E2B) where processes should outlive the agent.
