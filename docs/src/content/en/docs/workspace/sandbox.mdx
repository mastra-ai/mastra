...
All three providers include built-in process managers for background process support.

## Basic usage

Create a workspace with a sandbox and assign it to an agent. The agent receives the `execute_command` tool automatically:
...
When you configure a sandbox on a workspace, agents receive the `execute_command` tool for running shell commands. When the sandbox includes a process manager (all built-in providers do), agents also receive tools for background process management:

| Tool | Description |
| - | - |
| `execute_command` | Run a shell command. Returns stdout, stderr, and exit code. Accepts `background: true` to start a long-running process and return a PID. |
| `get_process_output` | Check stdout, stderr, and status of a background process by PID. Supports `tail` to limit output lines and `wait: true` to block until exit. |
| `kill_process` | Stop a background process by PID. Returns the last 50 lines of output. |
