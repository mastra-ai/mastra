...
The resolver must return a sandbox that is ready to use (already started, or able to handle calls without explicit startup), and must clean it up when the request finishes.
