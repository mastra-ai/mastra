...
When a background process exits, the invoking agent is woken with a `system-reminder` signal describing the exit. This means an agent that stops talking while a long-running task is still running resumes the moment the task finishes. The signal only fires when `execute_command` is invoked from an agent run.

Control this with the `signalOnExit` option:

- **`true`** (default): Fires a built-in `system-reminder` signal with the pid, exit code, and (truncated) stdout/stderr.
- **`false`**: Skip the signal — useful if you handle exit notification yourself in `onExit`.
- **function**: Build a custom signal payload. Return `undefined` to skip firing for a given exit (for example, only signal on non-zero exit codes).
