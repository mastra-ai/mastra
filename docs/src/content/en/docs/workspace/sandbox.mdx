...
After applying the line limit, Mastra limits the output to 2,000 tokens by default. Set `maxOutputTokens` on `execute_command`, `get_process_output`, or `kill_process` to change the token limit for that tool.
