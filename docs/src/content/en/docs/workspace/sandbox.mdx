...
If your sandbox provider includes a process manager, the `execute_command` tool also supports `background: true` for starting long-running processes, and two additional tools are registered:
...
All built-in providers (`LocalSandbox`, `DaytonaSandbox`, `E2BSandbox`) include a process manager. These tools are registered automatically — see [Workspace class reference](/reference/workspace/workspace-class#sandbox-tools) for the full tool name list.
