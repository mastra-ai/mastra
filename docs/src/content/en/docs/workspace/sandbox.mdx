...
Sandboxes give Mastra agents an environment for running commands and working with files. Agents can also start processes there, isolating their activity from your application and sensitive host resources.
...
| - | - |
| Mastra-level workspace | Shared by agents that inherit the workspace from the Mastra instance. |
| Agent-level workspace | Shared by all requests handled by that agent instance. |
| Resource-scoped resolver | One sandbox per resource, shared across requests with the same resource ID. |
| Thread-scoped resolver | One sandbox per memory thread, shared across requests in that thread. |
...
Environment variables passed to a sandbox or command are available to code running there. Mastra doesn't provide a backend-neutral credential broker that keeps secrets outside the sandbox while authorizing its requests. Pass only the credentials the command needs, scope them to the task, and follow the selected backend's secret-management guidance.
