...
You can also attach skills directly to an agent without a workspace using `defineSkill()` and the agent's `skills` config. When both agent-level and workspace-level skills exist, they merge — agent-level skills take precedence on name conflicts.
