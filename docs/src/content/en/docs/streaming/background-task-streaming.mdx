...
Background tasks must be [enabled on the Mastra instance](/reference/configuration#backgroundtasks) before `backgroundTaskManager` is available. When disabled, `mastra.backgroundTaskManager` is `undefined`.
