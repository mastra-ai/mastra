...
### AI SDK v4 vs v5 Compatibility

The `.streamVNext()` method requires AI SDK v5 models (`LanguageModelV2`). If you're using AI SDK v4 models (`LanguageModelV1`), use the `.stream()` method instead.

#### How to identify your SDK version

Your model's SDK version depends on the import path:

**AI SDK v5 (Compatible with `streamVNext`):**
```typescript
...
**AI SDK v4 (Use `stream` instead):**
```typescript
import { openai } from 'ai/openai';      // v4
import { anthropic } from 'ai/anthropic'; // v4
import { google } from 'ai/google';      // v4
```

#### Popular provider examples

Here's how to configure popular AI providers for use with `streamVNext`:
