...
Mastra currently supports two streaming methods, the page explains how to use `streamVNext()`.
...
This is the experimental API. Iterate over the `textStream` using a `for await` loop to inspect each stream chunk.
