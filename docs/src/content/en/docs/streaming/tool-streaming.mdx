...
Some model providers can execute built-in tools on the provider side instead of calling your local `execute()` function. In those cases, Mastra marks the tool call as `providerExecuted`, emits the matching tool result in the stream, and persists that result so memory replay and resumed streams do not try to execute the tool again.
