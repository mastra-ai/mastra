...
When a model generates a tool call, the arguments arrive incrementally as `tool-call-delta` stream chunks before the final `tool-call` chunk. The [Harness](/reference/core/harness) emits `tool_input_start`, `tool_input_delta`, and `tool_input_end` events that correspond to these chunks. UIs can listen for these events to render tool arguments as they stream in — for example, showing a file path or command immediately rather than waiting for the complete tool call.
