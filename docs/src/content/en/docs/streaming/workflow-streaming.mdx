...
If a workflow stream is closed or interrupted for any reason, you can resume it with the `observeStreamVNext` method. This will return a new `ReadableStream` that you can use to observe the workflow events.

```typescript showLineNumbers copy
const newStream = await run.observeStreamVNext();
