...
Add the MCP server to an agent's `mcpServers` config. The agent automatically gets all tools from its declared MCP servers, and Studio scopes app resource lookups to those servers.
...
You do not need to extract tools from the MCP server and add them to the agent's `tools` config. The `mcpServers` config handles tool resolution automatically. The MCP server is also auto-registered at the Mastra instance level so Studio can access its resources.
...
export const mastra = new Mastra({
  agents: { myAgent },
  // myAppServer is auto-registered through the agent — no need to list it here
})
```
