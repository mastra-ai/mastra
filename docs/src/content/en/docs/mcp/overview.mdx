...
You can require human approval before MCP tools are executed by setting `requireToolApproval` on a server definition. This integrates with the agent [human-in-the-loop](/docs/agents/human-in-the-loop) approval flow.
