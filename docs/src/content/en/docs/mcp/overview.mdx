...
<Tabs>
  ...
  <TabItem value="apify" label="Apify">
    [Apify](https://apify.com?utm_source=mastra-docs) is the largest marketplace of web scraping and automation tools, called [Actors](https://apify.com/store?utm_source=mastra-docs). The [Apify MCP Server](https://mcp.apify.com) gives agents access to 5,000+ ready-made Actors for scraping social media, search engines, e-commerce sites, maps, and more.
    ...
    To restrict the agent to specific Actors, append the `tools` query parameter:

    ```typescript
    url: new URL('https://mcp.apify.com?tools=apify/rag-web-browser,apify/google-search-scraper'),
    ```

    For more details, see the [Apify MCP documentation](https://docs.apify.com/platform/integrations/mcp).
  </TabItem>
