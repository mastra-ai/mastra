...
When `experimental_auth` is enabled, all requests made with the [Mastra Client SDK](/docs/client-js/overview) must include a valid JWT in the `Authorization` header:
