...
```bash copy
npm install @mastra/auth-clerk@^0
```
...
> See the [MastraAuthClerk](/reference/v0/auth/clerk) API reference for all available configuration options.
...
> See [Mastra Client SDK](/docs/v0/server-db/mastra-client) for more configuration options.
