...
For delegations run as [background tasks](/docs/harness/background-tasks), configured retries can invoke `onDelegationComplete` once per attempt. Account for repeated calls if your hook has side effects.
...
const result = await parentAgent.generate('Research and write about AI safety', {
  versions: {
    agents: {
      ...
      'writing-agent': { label: 'candidate' },
    },
  },
})
```

Entries under `versions.agents` propagate through delegation. A `versions.self` selector applies only to the root agent for the current execution and isn't inherited by subagents. See [Subagent versioning](/reference/editor/versioning#sub-agent-versioning) for selectors, resolution order, and server API usage.

Mastra resolves explicit subagent labels to immutable version IDs before the run begins, then persists those pins through suspension and recovery. Moving a label affects a later run, but it can't change which subagent version an in-flight run invokes.
