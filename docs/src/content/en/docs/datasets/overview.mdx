...
Call `mastra.datasets.create()` with a name and optional description:
...
You can enforce the shape of `input` and `groundTruth` by passing Zod schemas. Mastra converts them to JSON Schema automatically:
...
// Update an item — this creates a new dataset version
...
See how a specific item changed across versions:
...
You can also pin experiments to a version — see [Running experiments](/docs/datasets/running-experiments).
