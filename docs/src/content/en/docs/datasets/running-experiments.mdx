...
If you run evaluations on your own infrastructure (for example a Temporal, Airflow, or custom worker fleet), Mastra can act as the system of record without owning execution. An external experiment is created with `targetType: 'external'`, your workers submit per-item results as they finish, and a final call computes counts and marks the experiment complete. Results are written to the same tables as native runs, so Studio views, comparisons, and review summaries work unchanged.
...
- **The dataset version is pinned at creation.** Submissions are validated against the items visible at that version, so editing or deleting items afterwards doesn't affect an in-flight experiment. Submitting an `itemId` that isn't visible at the pinned version fails with `404`.
