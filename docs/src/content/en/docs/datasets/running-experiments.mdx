...
If you run evaluations on your own infrastructure (for example a Temporal, Airflow, or custom worker fleet), Mastra can act as the system of record without owning execution. An external experiment is created with `targetType: 'external'`, your workers submit per-item results as they finish, and a final call computes counts and marks the experiment complete. Results land in the same tables as native runs, so Studio views, comparisons, and review summaries work unchanged.

Create the experiment with [`createExternalExperiment()`](/reference/datasets/createExternalExperiment). Pass your own `id` (for example a workflow run id) to make creation idempotent — retrying the call with the same id returns the existing experiment instead of failing:

```typescript title="src/mastra/experiments/external.ts"
const dataset = await mastra.datasets.get({ id: 'translation-dataset-id' })
...
`input` and `groundTruth` default to the dataset item's values at the experiment's pinned dataset version. Inline `scores` are persisted to the scores store under the experiment, so they show up in comparisons alongside native scorer runs.

When all items are done, call [`finalizeExperiment()`](/reference/datasets/finalizeExperiment). Mastra computes `succeededCount`, `failedCount`, and `skippedCount` from the persisted rows — your workers never track completion bookkeeping. Finalization is idempotent; calling it again returns the stored record:
...
The same lifecycle is available over HTTP (`POST /api/datasets/:datasetId/experiments/external`, `POST .../experiments/:experimentId/results`, `POST .../experiments/:experimentId/finalize` — see [Server routes](/reference/server/routes#datasets-and-experiments)) and through `@mastra/client-js` (see the [Datasets API](/reference/client-js/datasets)).

### Retry and idempotency contract

External submissions are designed to be retried aggressively (for example by a Temporal retry policy). The exact guarantees:

- **Create is idempotent on `id`.** Calling `createExternalExperiment` again with the same caller-supplied `id` returns the existing experiment. Reusing an `id` that belongs to another dataset or a non-external experiment fails with `EXPERIMENT_ID_CONFLICT` (HTTP `409`).
- **Results upsert on `(experimentId, itemId, attempt)`.** Re-submitting the same key updates the existing row — last write wins for all mutable fields. A retried worker never creates duplicates.
...
- **The dataset version is pinned at creation.** Submissions are validated against the items visible at that version; editing or deleting items afterwards doesn't affect an in-flight experiment. Submitting an `itemId` that isn't visible at the pinned version fails with `404`.
