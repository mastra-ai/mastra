...
In the **Experiments** tab, select **Compare** and choose two experiments to compare their scores and results side by side. The comparison view shows average scores per scorer, then a three-column layout: the list of dataset items, the baseline result, and the contender result. Selecting an item shows both runs next to each other, with their output, scores and scorer reasons, comment, metadata, and any run error, so you can read what changed between the two experiments.
