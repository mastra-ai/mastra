...
The Platform commands use the authenticated, project-scoped control plane. They submit authoritative Platform runs rather than downloading or executing hosted assets locally. All success responses are written as JSON to standard output. Errors are written as JSON to standard error and set a nonzero exit code. Pass `--pretty` to format the JSON for reading.
...
Before submitting a run, the selected Platform environment must have a runnable deployment and experiment worker build. The target agent or workflow and each registry scorer in the request must be registered in that deployed worker. The CLI doesn't build or run these components locally.
