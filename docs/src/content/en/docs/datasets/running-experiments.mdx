...
An experiment runs every item in a dataset through a target — an agent, a workflow, or a scorer — then optionally scores the outputs. Use a scorer as the target when you want to evaluate an LLM judge itself. Results are persisted to storage so you can compare runs across different prompts, models, or code changes.

## Basic experiment

Call `dataset.startExperiment()` with a target and scorers:
...
Scorers run automatically after each item's target execution. Pass scorer instances or registered scorer IDs:
...
Visit the [Scorers overview](/docs/evals/overview) for details on built-in and custom scorers.
