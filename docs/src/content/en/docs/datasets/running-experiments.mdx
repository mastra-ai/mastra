...
Use `mastra experiments` to run an experiment against an immutable dataset version hosted by Mastra Platform. These commands use the authenticated, project-scoped Platform control plane. They submit authoritative Platform runs rather than executing assets locally.
