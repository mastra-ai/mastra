...
Set timeout overrides on individual items and use `itemTimeout` as the fallback for other items:

```typescript title="src/mastra/experiments/timeouts.ts"
await dataset.addItem({
  input: 'Summarize this short article',
  timeout: 5_000,
})

await dataset.addItem({
  input: 'Analyze this long report',
  timeout: 60_000,
})
...
The first two items use their `timeout` values. The third item inherits the 30-second `itemTimeout`. When neither value is set, an item has no execution deadline. Both values must be positive integers no greater than 1,800,000 milliseconds (30 minutes).

Each timeout is one budget for the initial attempt, exponential retry backoff, and all retry attempts. It doesn't restart for each attempt. A timeout stops further retries. Cancelling the experiment through `signal` takes precedence and isn't retried.
