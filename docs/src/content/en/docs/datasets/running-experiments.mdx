...
Use `mastra api experiment platform` to run an experiment against an immutable dataset version hosted by Mastra Platform. These commands use the authenticated, project-scoped Platform control plane. They don't connect to a local or deployed Mastra server, so `--url` and `--server-api-prefix` aren't supported.
...
```bash npm2yarn
npx mastra api experiment platform dataset list
npx mastra api experiment platform dataset versions dataset_123
```

Submit the version with a scorer registered in the experiment worker:

```bash npm2yarn
npx mastra api experiment platform run '{
  "experimentId": "translation-baseline-2026-08-25",
  ...
  "idempotencyKey": "translation-baseline-2026-08-25"
}'
```
...
```bash npm2yarn
npx mastra api experiment platform get translation-baseline-2026-08-25
npx mastra api experiment platform poll translation-baseline-2026-08-25
npx mastra api experiment platform results translation-baseline-2026-08-25
```

Platform-hosted scorer versions can be discovered with `mastra api experiment platform scorer list` and `scorer versions`. Hosted scorer execution isn't available yet. A run that supplies `hostedScorers` returns the typed `HOSTED_SCORER_EXECUTION_UNAVAILABLE` error without creating a job.
