...
description: "Use Claude Agent SDK, Cursor Agent SDK, OpenAI Agents SDK, and OpenCode SDK agents from Mastra."
packages:
  - "@mastra/claude"
  - "@mastra/cursor"
  - "@mastra/openai"
...
- [OpenCode SDK](#opencode-sdk): Use `@mastra/opencode-sdk` to register an OpenCode SDK agent and call it with Mastra `generate()` and `stream()`.

Coding agents without a dedicated Mastra package, such as Cline, run through the [Agent Client Protocol](/docs/connections/acp). You can also use OpenCode through ACP when your app needs an ACP coding-agent process rather than the OpenCode SDK runtime.
...
Use `@mastra/opencode-sdk` to run an OpenCode SDK agent through Mastra. The wrapper can start a local OpenCode server, connect to an existing server, or use an existing OpenCode SDK client.

### Install OpenCode packages

Install the Mastra package and the OpenCode SDK peer dependency. Install the [OpenCode CLI](https://opencode.ai/docs) when the wrapper starts a local server.

```bash npm2yarn
npm install @mastra/opencode-sdk @opencode-ai/sdk
```

### Create an OpenCode SDK agent

Pass `serverOptions` to start a local OpenCode server when the agent first runs. The wrapper reuses the client for later runs. Call `close()` during application shutdown to stop the server.

```typescript title="src/mastra/agents/opencode-sdk-agent.ts"
import { OpenCodeSDKAgent } from '@mastra/opencode-sdk'

export const openCodeSDKAgent = new OpenCodeSDKAgent({
  id: 'opencode-sdk-agent',
  name: 'OpenCode SDK Agent',
  description: 'Use OpenCode SDK through Mastra.',
  serverOptions: {},
})
```

To connect to an OpenCode server that is already running, pass its URL with `config`. You can also pass a preconfigured OpenCode SDK `client`.
...
import { openCodeSDKAgent } from './agents/opencode-sdk-agent'
...
OpenCode SDK agents can resume a known session with `sessionId`:

```typescript title="src/mastra/resume-opencode.ts"
const result = await openCodeSDKAgent.resumeGenerate({
  message: 'Continue the previous task.',
...
Claude, OpenAI, and OpenCode SDK agents support Mastra `structuredOutput` through their provider-native structured output APIs. OpenCode passes the JSON Schema to its native schema-constrained output format, then Mastra validates the returned value. The validated value is available on `result.object`.

```typescript title="src/mastra/run-opencode-structured-output.ts"
import { z } from 'zod'

const result = await openCodeSDKAgent.generate<{ summary: string }>('Summarize this project.', {
  structuredOutput: {
    schema: z.object({
      summary: z.string(),
    }),
...
Claude SDK runs can include SDK-estimated cost from the Claude result message. Cursor SDK runs include token usage from Cursor interaction updates. OpenAI SDK runs include token usage from OpenAI run state. OpenCode SDK runs include token usage, estimated cost, tool activity, and provider metadata from OpenCode session events.
