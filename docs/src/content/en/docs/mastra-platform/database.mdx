...
Supported kinds are `turso`, `neon` (Postgres), `redis`, and `postgres` (managed Postgres on Railway). Useful flags:

- `--shared`: Attach a project-scoped database shared by every environment. Can't be combined with an environment argument. Not supported for `redis` or `postgres`, which are always environment-scoped.
- `--name <name>`: Database name. Defaults to a name derived from the project slug.
- `--region <region>`: Provider region ID for project-scoped databases (for example `fra`). Environment-scoped databases are placed near the environment's region automatically, and an explicit `--region` is ignored. `redis` and `postgres` run inside the environment itself and never accept a region.
...
Neon-hosted serverless Postgres exposes a single `DATABASE_URL` connection string. The following example connects a [PostgresStore](/integrations/databases/postgresql) using that variable.

```ts title="src/mastra/storage.ts" caption="Connect a Neon Postgres database"
...
`--kind postgres` provisions a managed Postgres instance on Railway that runs alongside your environment on its private network. It's always environment-scoped and injects a single `POSTGRES_URL` connection string. Connect it with the same `@mastra/pg` adapter:

```ts title="src/mastra/storage.ts" caption="Connect a managed Postgres (Railway) database"
import { PostgresStore } from '@mastra/pg'
...
Managed Postgres uses a distinct variable from Neon (`POSTGRES_URL` versus `DATABASE_URL`), so both can coexist in the same environment when you need a serverless Neon database and an in-environment Postgres instance for different workloads.
...
| Postgres (Railway) | `POSTGRES_URL` |
| Redis | `REDIS_URL` |

:::warning
Treat connection credentials as secrets. The auth token (`TURSO_AUTH_TOKEN`) and the Postgres connection strings (`DATABASE_URL`, `POSTGRES_URL`) grant full access to your data. The platform masks them by default and only reveals them on request.
:::
