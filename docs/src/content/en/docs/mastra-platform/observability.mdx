---
title: "Observability | Mastra platform"
description: Collect searchable traces, logs, and metrics across Mastra projects and deploys with Mastra Platform Observability.
---

# Observability on Mastra platform

Observability on Mastra platform is a standalone hosted product for searchable traces, logs, and metrics across Mastra projects and deploys. Use it when you want observability without deploying Studio first or setting up local storage.

Use `mastra init` to configure platform observability while adding Mastra to an existing application, or configure it manually in a newly created Mastra project.

## Quickstart

Choose the setup path that matches your project.

### New project

Create a Mastra project, then follow the [manual setup](#manual-setup) steps to connect it to Mastra Platform Observability:

```bash npm2yarn
npm create mastra@latest my-mastra-app -- --yes
```

The managed agent harness includes observability support, but the create command doesn't authenticate with Mastra platform or provision a platform project.

### Existing non-Mastra projects

If you already have an application, such as a Next.js app, but haven't added Mastra yet, run `mastra init` and enable Mastra Observability when prompted:

```bash npm2yarn
npx mastra init
```

The CLI can select an existing platform project or create a new one.

### Manual setup

Create a project in [Mastra Platform](https://projects.mastra.ai), add `MASTRA_PLATFORM_ACCESS_TOKEN` and `MASTRA_PROJECT_ID` to `.env`, then register `MastraPlatformExporter` in your Mastra config. Add `MastraStorageExporter` if you also want to persist observability events to your configured Mastra Storage:

```ts title="src/mastra/index.ts"
import { Mastra } from '@mastra/core/mastra'
import { Observability, MastraPlatformExporter } from '@mastra/observability'

export const mastra = new Mastra({
  observability: new Observability({
    configs: {
      default: {
        serviceName: 'mastra',
        exporters: [new MastraPlatformExporter()],
      },
    },
  }),
})
```

See [Mastra platform configuration](/docs/mastra-platform/configuration#observability) for the environment variables used by platform observability.

## Query observability data

After your project is publishing traces, logs, or scores to Mastra Platform Observability, you can query that data from your terminal with the Mastra CLI or with `curl`.

```bash npm2yarn
npx mastra api trace list
```

For observability commands, the CLI targets the hosted observability API and can infer credentials from your project environment. See the [`mastra api` CLI reference](/reference/cli/mastra#mastra-api) for available observability commands, filtering, pagination, credential resolution, and direct `curl` examples.

## Related

- 📹 [Mastra observability and Studio workshop](https://www.youtube.com/watch?v=dKO_a3RPra0)
