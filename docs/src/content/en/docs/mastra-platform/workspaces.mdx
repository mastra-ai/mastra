...
The static `PlatformSandbox` passed to `Workspace` is shared by every request and agent that uses that workspace. Requests and memory threads don't receive separate sandboxes automatically. `PlatformFilesystem` is also a separate provider: configuring both providers doesn't mount the environment bucket inside the sandbox.

When your agent needs another isolated environment, for example a per-task workspace, a per-user tenant, or a background job that shouldn't touch shared shell state, construct another `PlatformSandbox`:
