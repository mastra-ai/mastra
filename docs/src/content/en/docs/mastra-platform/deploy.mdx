...
`NPM_TOKEN` is scoped to the dependency install, and Mastra redacts its value from the build logs it streams. The generated Dockerfile receives it as a build argument, so the container image itself stays free of the token. Like your other stored variables, `NPM_TOKEN` is injected into the running service, so treat it as a secret your application can read.

Projects that depend only on public packages need no changes.
