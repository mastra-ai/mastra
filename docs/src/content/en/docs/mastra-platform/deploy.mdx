...
`NPM_TOKEN` reaches the dependency install and stops there. The build step, the container image, and your running application all exclude it, and its value is redacted from build logs. Store a separate variable when your code needs registry access at runtime.
