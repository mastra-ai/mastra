...
`mastra server deploy` and `mastra studio deploy` are the earlier split deploy paths. They're being **removed on October 30, 2026**. After that date both commands will fail and legacy-pipeline projects can't deploy a new build until they migrate.
...
**My organization isn't opted in yet.** Contact support. Opt-in will be automatic before the October 30, 2026 cutoff.
