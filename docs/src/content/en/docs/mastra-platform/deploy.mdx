...
`mastra server deploy` and `mastra studio deploy` are the earlier split deploy paths. They are being **removed on October 30, 2026**. After that date both commands will fail and legacy-pipeline projects cannot ship a new deploy until they migrate.

Existing deployed services keep running — this only affects your ability to ship new deploys.
...
- Your project's most recent successful deploy shipped `@mastra/core` older than `1.44`.
- Your organization has not been opted in to environment deploys.
...
<Steps>
  <StepItem>
    ...
    Do not change my deploy scripts, my infra, or my environment variables —
    only the Mastra usage inside my code.
    ```
  </StepItem>
  ...
  <StepItem>
    ...
    `mastra deploy` builds once and deploys both the server and the Studio UI shell for the target environment. Any recent `mastra` CLI supports it — if yours does not, upgrade with `npm install -g mastra@latest`.
  </StepItem>
...
**My organization is not opted in yet.** Contact support. Opt-in will be automatic before the October 30, 2026 cutoff.
