...
One command builds your project and validates it before anything ships, plus creates the platform project and environment on your first run, deploys, streams build logs, and prints your public URL once the deploy is serving traffic.
...
<Steps>
  ...
  <StepItem>
    ...
    If the build statically enables `backgroundTasks`, preflight also checks whether the target environment has dedicated background workers enabled. The CLI offers to enable them before deployment:

    ```text
    Background tasks are enabled in your Mastra config. Enable dedicated background workers for the production environment? (Y/n)
    ```

    Accept the prompt to enable **Background Workers**. If the environment doesn't have a managed or platform-stored `REDIS_URL`, the CLI creates and attaches a managed Redis database first. Starter and Team projects require the **Scalable Server** add-on. Background workers are included with Enterprise projects.

    Declining the prompt doesn't block the deploy — background tasks run in-process on the API server until dedicated workers are enabled. Non-interactive deploys and deploys that use `--yes` never create this infrastructure; they warn and continue. Enable **Background Workers** in the environment settings to move tasks onto dedicated workers.
  </StepItem>
...
- **Background workers disabled**: A warning when the build statically enables `backgroundTasks` but the target environment doesn't have dedicated workers enabled. Interactive deploys offer to enable workers and provision managed Redis when needed; otherwise tasks run in-process on the API server.
