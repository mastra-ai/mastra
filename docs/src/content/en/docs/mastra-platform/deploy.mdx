...
`NPM_TOKEN` is scoped to the dependency install. It isn't passed to your build or lint steps, isn't written into the container image, and isn't available to your application at runtime. Its value is redacted from build logs. If your code needs registry access after deploy, read it from a separate variable.

Deploys behave exactly as before when `NPM_TOKEN` isn't set, so public-only projects need no changes.
