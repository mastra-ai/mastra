...
<Steps>
  ...
  <StepItem>
    ...
    Accept the prompt to enable **Background Workers**. If the environment doesn't have a managed or user-provided `REDIS_URL`, the CLI creates and attaches a managed Redis database first. Starter and Team projects require the **Scalable Server** add-on. Background workers are included with Enterprise projects.

    Non-interactive deploys and deploys that use `--yes` don't create this infrastructure. Enable **Background Workers** in the environment settings before rerunning the deploy.
  </StepItem>
...
- **Background workers disabled**: A hard block when the build statically enables `backgroundTasks` but the target environment doesn't have dedicated workers enabled. Interactive deploys offer to enable workers and provision managed Redis when needed.
