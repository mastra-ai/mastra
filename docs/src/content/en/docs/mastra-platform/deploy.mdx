...
`mastra server deploy` and `mastra studio deploy` are the earlier split deploy paths. They'll be **removed in the Mastra 2.0 major release**. Once removed, both commands will fail and legacy-pipeline projects can't deploy a new build until they migrate.
...
**My organization isn't opted in yet.** Contact support. Opt-in will be automatic before Mastra 2.0 is released.
