...
Managed Redis (`--kind redis`) and managed Postgres (`--kind postgres`) run inside the environment itself on Railway's private network, so they inherit the environment's Railway region and never accept `--region`.
