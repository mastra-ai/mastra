...
1. Select a theme or **Noise** node in the flow to filter the chart to traces assigned to it. The selected trace signal column moves from the chart to a filter above it because every matching trace now has the same assignment in that column.
1. Select another theme or **Noise** node in a remaining column to narrow the results further. Active filters combine with AND logic, so a trace must match every filter.
1. Select **Details →** next to a filter to inspect its filtered summary and generated examples. The summary reports trace count and stage share. Select **Next examples** to continue through examples from the filtered trace set. Theme history remains unfiltered because it spans snapshots.
1. Remove one filter to recompute the flow, or select **Clear all** to restore the complete flow.

You can also:

- Select a theme in a distribution card to open its details and generated example summaries for the current flow.
- Select **Noise** in a distribution card to inspect its distribution and generated example summaries for the current flow.
...
Drilling into the flow is unavailable for snapshots with more than 2,000 traces. Choose another snapshot to use flow filters. Theme and Noise details remain available from the distribution cards.
