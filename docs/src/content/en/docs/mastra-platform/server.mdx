...
:::warning[Deprecated, removed October 30, 2026]
`mastra server deploy` is the earlier split deploy path and is being **removed on October 30, 2026**. After that date the command will fail, and projects still on the legacy pipeline can't deploy a new build until they migrate. Existing deployed services keep running.
