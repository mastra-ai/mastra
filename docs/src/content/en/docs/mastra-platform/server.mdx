...
:::warning[Deprecated, removed in Mastra 2.0]
`mastra server deploy` is the earlier split deploy path and will be **removed in the Mastra 2.0 major release**. Once removed, the command will fail and projects still on the legacy pipeline can't deploy a new build until they migrate. Existing deployed services keep running.

Migrate to the unified [`mastra deploy`](/docs/mastra-platform/deploy) command. See [Migrating from server and studio deploys](/docs/mastra-platform/deploy#migrating-from-server-and-studio-deploys).
:::
