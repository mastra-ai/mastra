...
`mastra server deploy` is the earlier split deploy path and is being **removed on October 30, 2026**. After that date the command will fail, and projects still on the legacy pipeline cannot ship a new deploy until they migrate. Existing deployed services keep running.
