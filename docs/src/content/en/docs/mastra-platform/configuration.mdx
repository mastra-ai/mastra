...
| `MASTRA_PROJECT_ID` | UUID of the platform project. `MastraPlatformExporter`, `mastra studio deploy`, and `mastra server deploy` use it to link observability data and deploys to an existing platform project. |
| `MASTRA_PLATFORM_TRACES_ENDPOINT` | Optional traces endpoint override. This is only set automatically for local platform development. Defaults to `https://observability.mastra.ai`. |
...
`MastraPlatformExporter` currently reads `MASTRA_CLOUD_ACCESS_TOKEN` to activate platform export. The CLI provisioning flow writes this alongside `MASTRA_PLATFORM_ACCESS_TOKEN`.

## Multiple environments

A platform project maps to a single deployed instance with one set of env vars. Platform projects don't have built-in `staging` vs `production` slots. To run the same codebase across multiple environments, create one project per environment and override `.mastra-project.json` per deploy.
...
Each project has its own Studio URL and can send observability data to the Mastra platform. When using [`MastraPlatformExporter`](/docs/observability/tracing/exporters/mastra-platform), set `MASTRA_PROJECT_ID` and `MASTRA_CLOUD_ACCESS_TOKEN` per environment so traces route to the matching platform project.
