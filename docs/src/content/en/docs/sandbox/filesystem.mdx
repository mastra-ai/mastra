...
A filesystem gives an agent tools for reading, writing, listing, and searching files. Use it as a [knowledge base](/docs/sandbox/search) or to keep a sandbox's inputs and outputs after the sandbox stops.
...
- `mounts` creates a `CompositeFilesystem` from path-prefixed providers. When you also configure a static sandbox, Mastra attempts to expose those paths to sandbox commands.
