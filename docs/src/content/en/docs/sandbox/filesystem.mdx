...
A filesystem gives an agent tools for reading, writing, listing, and searching files. Use [search](/docs/sandbox/search) to index its contents for retrieval. It can also keep a sandbox's inputs and outputs after the sandbox stops.

Configure files in two ways:

- `filesystem` accepts one filesystem provider, a per-request resolver, or a `CompositeFilesystem` that you create yourself.
- `mounts` creates a `CompositeFilesystem` from path-prefixed providers. When the workspace also has a static sandbox, Mastra attempts to mount each provider at its configured path inside the sandbox so commands can access the same files. Remote sandbox mounts typically use Filesystem in Userspace (FUSE).

Both configurations give the agent file tools. Sandbox commands can only see a provider when the sandbox can mount it successfully. Configure either `filesystem` or `mounts`, not both. Configuring both throws a `WorkspaceError` with the code `INVALID_CONFIG`.
...
Mount an S3 bucket at `/workspace` inside a Daytona sandbox:
...
Objects already in the bucket become available under `/workspace`. The agent can call `read_file('/workspace/report.md')` or run `cat /workspace/report.md`. Files written through either interface are saved to S3 and remain available after the sandbox stops.
...
When command access is required, initialize the workspace and inspect each mount:
...
Also test the path from inside the sandbox. For a remote `/workspace` mount, run `ls /workspace`. With `LocalSandbox`, run `ls workspace` from its `workingDirectory`.
...
Filesystem providers initialize on first use. Most applications can rely on this lazy behavior.

Call `workspace.init()` during startup when provider credentials or connectivity failures need to surface before requests are served. Explicit initialization also prepares static sandbox mounts.

`autoIndexPaths` only runs during explicit initialization. When it's configured, call `workspace.init()` once during application startup. See [auto-indexing](/docs/sandbox/search#auto-indexing).

Mount and per-file indexing failures may be logged without causing initialization to fail. If every mount or indexed file is required, verify availability before accepting requests.
