...
Set `readOnly: true` when the agent should use files without changing them. For a static provider, Mastra removes write-related file tools. When a read-only `LocalFilesystem` is mounted into a `LocalSandbox` with `seatbelt` or `bwrap` isolation, commands can read the mount but can't write to it. Without isolation, `readOnly` applies to file tools only.
