...
Omitting a tool entry keeps its capability-driven default. Set `{ enabled: false }` on one tool to remove it, or set the top-level `enabled: false` to disable workspace tools by default. A per-tool `{ enabled: true }` overrides that global setting. Tool entries don't accept a bare `false` shorthand. The top-level `requireApproval` policy applies to every workspace tool unless a per-tool entry overrides it.

Authored application code can use the resolved provider API directly. Inside a tool or workflow step, pass its `requestContext` to `workspace.resolveSandbox({ requestContext })` so the code reaches the same cached sandbox as the workspace tools. Background processes start through `sandbox.processes.spawn()` when the resolved provider supports them.

The application code itself still runs in the Mastra process. Only operations called through the sandbox provider run in the sandbox. See the [workspace tool configuration reference](/reference/workspace/workspace-class#tool-configuration) for per-tool approvals, read-before-write policies, output limits, and hooks.
...
The `Workspace` constructor validates structural configuration synchronously. It doesn't contact providers or provision an execution environment. `workspace.init()` first initializes a static filesystem. It then starts a static sandbox and processes its mounts before processing `autoIndexPaths`. Credentials, network access, and provider setup can therefore fail during `init()`. If you skip `init()`, providers can instead fail when their first lazy operation runs.

Auto-indexing only runs during `workspace.init()`. A command can start a sandbox lazily without building the configured search index. Call `init()` during application startup when the workspace uses `autoIndexPaths` or when provider startup failures need to surface before requests are served. Mount and per-file indexing failures can be recorded or logged without rejecting `init()`, so applications that depend on complete mounts or indexes must verify them separately.
