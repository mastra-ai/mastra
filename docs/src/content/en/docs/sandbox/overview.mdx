...
A sandbox gives an agent an execution environment where it can run commands, execute code, install dependencies, and manage processes. Isolating that environment limits what agent-run code can access on your host, including files, credentials, and the network.
...
Files stored only inside a sandbox follow that backend's lifecycle. Configure a filesystem when inputs or outputs must remain available outside the sandbox environment.

Use [`mounts`](/docs/sandbox/filesystem#mounts) when file tools and sandbox commands should access the same persistent files. File tools access each provider directly. Commands can use a mount path only when the sandbox backend mounts that provider successfully. See [Mount availability](/docs/sandbox/filesystem#mount-availability) for supported combinations and failure behavior.
