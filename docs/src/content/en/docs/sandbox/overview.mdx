---
title: "Sandboxes and Filesystems"
description: Choose how agents access files, run commands, and isolate state between requests.
packages:
  - "@mastra/core"
---
...
Sandboxes run commands and processes. Filesystems give agents tools for reading and changing files. You can use either capability independently or arrange them so file tools and commands access the same data.

The arrangement determines where files live, whether commands can access them, and who shares the environment.

Supported remote backends can mount cloud filesystems into the sandbox, letting you seed ephemeral environments with files and persist their output between runs.

## Choose an arrangement

| Arrangement | Use it for | File and command access |
| - | - | - |
| Standalone filesystem | Knowledge bases, uploads, review, and retrieval without command execution | Agents receive file tools. No sandbox is required. |
| Separate filesystem and sandbox | Storage that the sandbox backend can't mount, or controlled transfer between storage and commands | File tools access the provider. The agent or application passes selected content to commands. |
| Shared local directory | Local coding and development | `LocalFilesystem` and `LocalSandbox` point to the same host directory. |
| Mounted filesystem | Persistent cloud files that remote commands need to use | File tools access the provider, and the sandbox materializes it at a configured mount path. |
| Shared static sandbox | One project environment shared across requests | Files, processes, and lifecycle state remain shared. |
| Per-user or per-thread sandbox | Tenant, session, or conversation isolation | A resolver selects an environment, and a stable cache key preserves it across related requests. |
...
Give an agent a local command environment:
...
Omitting a tool entry keeps its capability-driven default. Set `{ enabled: false }` on one tool to remove it, or set the top-level `enabled: false` to disable generated tools by default. A per-tool `{ enabled: true }` overrides that global setting. Tool entries don't accept a bare `false` shorthand. The top-level `requireApproval` policy applies to every generated tool unless a per-tool entry overrides it.

Authored application code can use the resolved provider API directly. Inside a tool or workflow step, call `workspace.resolveSandbox({ requestContext })` so the code reaches the same cached sandbox as the generated tools. Background processes start through `sandbox.processes.spawn()` when the resolved provider supports them.

The application code itself still runs in the Mastra process. Only operations called through the sandbox provider run in the sandbox. See the [tool configuration reference](/reference/workspace/workspace-class#tool-configuration) for per-tool approvals, read-before-write policies, output limits, and hooks.
...
Use `LocalSandbox.detectIsolation()` to check whether Seatbelt or Bubblewrap is available on the current operating system. The `nativeSandbox` options control network access, read-only or writable paths, write access to the working directory, and system binaries. You can also provide a custom Seatbelt profile or Bubblewrap arguments. See [`LocalSandbox`](/reference/workspace/local-sandbox) for the full configuration.
...
Configure mounts alongside the sandbox:
...
The agent receives file tools for the mounted storage, and commands in the sandbox can access the same files under `/data`. This lets you seed an ephemeral sandbox with existing files and persist its output after the sandbox stops.
...
Static `mounts` configuration can't be combined with a sandbox resolver. This is a known limitation, so the example mounts the filesystem inside the resolver instead. Generated tools resolve the filesystem and sandbox from the request context automatically.

### Resolver ownership

The application owns sandboxes returned by a resolver. Return a sandbox that's ready to use and destroy it through application lifecycle code when it's no longer needed, because `workspace.destroy()` doesn't destroy resolver-returned sandboxes.
...
Sandbox scope depends on where you assign the configuration and whether you use a resolver:

| Configuration | Sandbox scope |
| - | - |
| Mastra-level configuration | Agents that inherit the configuration from the Mastra instance use the same sandbox. |
...
Static sandbox backends are instantiated with your application, but their execution environment usually starts lazily on the first command. Initialize explicitly when you want to provision the environment during application startup instead.

Explicit initialization doesn't start resolver-backed sandboxes because no backend is selected until the resolver runs. Return a sandbox that's already started or can start itself on first use.

Configuration is validated synchronously before Mastra contacts providers or provisions an execution environment. Initialization first prepares a static filesystem. It then starts a static sandbox and processes its mounts before processing `autoIndexPaths`. Credentials, network access, and provider setup can therefore fail during initialization. If you skip explicit initialization, providers can instead fail when their first lazy operation runs.

Auto-indexing only runs during explicit initialization. A command can start a sandbox lazily without building the configured search index. Initialize during application startup when `autoIndexPaths` is configured or when provider startup failures need to surface before requests are served. Mount and per-file indexing failures can be recorded or logged without rejecting initialization, so applications that depend on complete mounts or indexes must verify them separately.
...
Static configuration owns its sandbox. `mastra dev` and the generated Mastra server handle shutdown signals and destroy registered resources automatically. If you embed Mastra in a custom server or process, call `mastra.shutdown()` from its shutdown hook. For standalone use, call `workspace.destroy()` directly. The effect of stopping or destroying the underlying environment depends on the backend.
