...
LSP starts a long-running language-server process and exchanges JSON-RPC messages with it. Before enabling LSP, ensure your workspace has:
...
This local TypeScript setup satisfies those requirements. Install the protocol packages, TypeScript, and its language server:
...
`maxOpenClients` limits the language-server clients retained by one workspace. It must be a positive integer. When the limit is reached, Mastra closes the least recently used client that has no active query lease.
...
Use `WORKSPACE_TOOLS.LSP.LSP_INSPECT` to configure the agent inspection tool. For example, add this entry to the workspace configuration to expose a shorter name:
...
Pass `tools` to the `Workspace` constructor. The `name` changes the exposed tool name, but the configuration key remains `WORKSPACE_TOOLS.LSP.LSP_INSPECT`. Set `enabled: false` on the same entry to remove the tool.
...
Use `workspace.lsp` when application code needs LSP results without the agent tool. `getDiagnostics()` and `getDiagnosticsMulti()` manage their client leases and document open and close notifications internally.
...
Direct client positions are 0-indexed. A leaked lease can prevent an idle client from being evicted when `maxOpenClients` is set. During workspace teardown, Mastra waits up to five seconds for active leases before forcing the language servers to shut down. `workspace.destroy()` closes all managed LSP clients before it destroys the sandbox.
