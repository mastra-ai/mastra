...
By default, the vector index name is derived from the workspace ID. To set a custom name, use `searchIndexName`:
...
After an edit, index the complete updated content again because editing or deleting the file leaves its previous search document in place. Workspace doesn't currently expose a public method for removing one indexed document. Use the `grep` file tool when the agent needs to search the current filesystem state without maintaining a separate index.
...
Auto-indexing performs one scan during initialization and doesn't watch for later filesystem changes. It recursively discovers matching files and reads their content. Large files are split into chunks before Mastra writes the search records. Vector mode also generates embeddings. `workspace.init()` waits for this work to finish, though individual unreadable, missing, or unsupported files can be skipped or logged as errors.

Keep `autoIndexPaths` narrow. Exclude dependencies, generated output, and binary trees, use a batch-capable embedder for vector search, and reuse a stable vector index where possible. Set a stable workspace `id` or `searchIndexName` when the vector index must survive process restarts. Repeated calls to `workspace.init()` process the configured paths again, so avoid large auto-index sets on short-lived or per-thread workspaces. Initialize during application startup when indexing cost must be paid before serving requests.
...
| Option | Description |
| - | - |
| `topK` | Maximum number of results to return. `workspace.search()` defaults to 10. The agent search tool defaults to 5. |
...
The index tool accepts the same supplied `path`, `content`, and optional metadata as `workspace.index()`. It can trigger embedding calls and vector-store writes. Require approval when agents shouldn't mutate the index without review:
...
See [workspace class reference](/reference/workspace/workspace-class#search-tools) for the complete tool list and [`WorkspaceToolsConfig`](/reference/workspace/workspace-class#tool-configuration) for shared settings.
