...
| `minScore` | Filter out results below this score threshold. Its scale depends on the search mode and vector provider. |
...
- Choose `minScore` for the configured mode and provider instead of assuming one threshold works across modes.
