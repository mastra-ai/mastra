...
Set your LiveKit credentials. Create a free project on [LiveKit Cloud](https://cloud.livekit.io), or run a local server with [`livekit-server --dev`](https://docs.livekit.io/home/self-hosting/local/):
...
## Create a voice worker

The worker answers LiveKit sessions with your Mastra agents. The following example uses LiveKit Inference model strings for speech-to-text and text-to-speech, so no provider plugins are required:
...
The `agent` option selects which Mastra agent answers each session. Pass a fixed key as shown, or omit it to use the `agentId` from the dispatch metadata, which lets one worker serve every agent on your Mastra instance.

Download the turn detection and voice activity detection models once, then run the worker in development mode:
...
The worker registers with your LiveKit server and waits for sessions. Run it alongside `mastra dev`.

## Add a connection endpoint

Frontends need a token to join a room. The `liveKitConnectionRoute()` helper returns an API route that mints the token and dispatches your voice agent into the room:
...
`POST /voice/livekit/connection-details` accepts optional `agentId`, `threadId`, and `resourceId` fields in the request body and returns:
...
This response matches the contract used by LiveKit's frontend starters, so apps built from [agent-starter-react](https://github.com/livekit-examples/agent-starter-react) or the [LiveKit React components](https://docs.livekit.io/reference/components/react/) work without changes. You can also test the worker without a frontend using the hosted [LiveKit Agents Playground](https://agents-playground.livekit.io).
...
- `preemptiveGeneration`: Starts the Mastra agent's reply while the user is still finishing, hiding time-to-first-token. The worker disables it by default: each preemptive attempt runs the Mastra agent on an interim transcript, and every run persists the user message — duplicating messages in the thread. Re-enable it with `preemptiveGeneration: { enabled: true }` if latency matters more than exact thread history.
...
Each turn sends only the new user input; Mastra Memory supplies history, semantic recall, and working memory. Pin a session to an existing thread by passing `threadId` in the connection request body — useful for continuing a text conversation by voice. In Studio, starting a call from an open chat binds the call to that thread, and the transcript fills into the chat after each exchange.
...
- `generate`: For full control, pass a `generate` function instead — any reply generator that turns a turn into a text stream.
