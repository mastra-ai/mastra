...
<Tabs>
  ...
  <TabItem value="smallest" label="Smallest AI">
    ```typescript
    import { Agent } from '@mastra/core/agent'
    ...
    // Convert text to speech to an Audio Stream
    const audioStream = await voiceAgent.voice.speak(text, {
      speaker: 'meher', // Optional: specify a voice from getSpeakers()
    })

    playAudio(audioStream)
    ```

    Visit the [Smallest AI Voice Reference](/reference/voice/smallest) for more information on the Smallest AI voice provider.
  </TabItem>
...
<Tabs>
  ...
  <TabItem value="smallest" label="Smallest AI">
    ...
    const audioStream = await createReadStream('./how_can_i_help_you.wav')
...
<Tabs>
  ...
  <TabItem value="smallest" label="Smallest AI">
    ```typescript
    // Smallest AI Voice Configuration
    const voice = new SmallestVoice({
      speechModel: {
        model: 'lightning_v3.1_pro', // TTS pool (lightning_v3.1 or lightning_v3.1_pro)
        apiKey: process.env.SMALLEST_API_KEY,
        language: 'auto', // ISO 639-1 code, or 'auto' to route on the input text
        sampleRate: 24000,
      },
      listeningModel: {
        model: 'pulse', // STT model (pulse or pulse-pro)
        apiKey: process.env.SMALLEST_API_KEY,
      },
      speaker: 'meher', // Default voice id
    })
    ```
