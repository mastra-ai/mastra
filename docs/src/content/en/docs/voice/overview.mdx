...
<Tabs>
  ...
  <TabItem value="hume" label="Hume">
    ...
    const voiceAgent = new Agent({
      ...
      instructions: 'You are a voice assistant that can help users with their tasks.',
      model: 'openai/gpt-5.1',
      voice: new HumeVoice({ speaker: 'your-voice-name' }),
    })
    ...
    // Convert text to speech to an Audio Stream
    const audioStream = await voiceAgent.voice.speak(text, {
      speaker: 'your-voice-name', // Optional: specify a speaker
    })

    playAudio(audioStream)
    ```

    Visit the [Hume Voice Reference](/reference/voice/hume) for more information on the Hume voice provider.
  </TabItem>
