...
const agent = new Agent({
  ...
  voice: new InworldRealtimeVoice({
    ...
    speaker: 'Dennis',
    // Inworld-specific options (MCP tool routing, playback speed, etc.)
    // providerData: { audio: { output: { speed: 1.1 } } },
  }),
})
...
- Inworld-specific session knobs (MCP tool routing, semantic VAD eagerness, playback speed) are surfaced through `providerData`, which is shallow-merged into every `session.update`.
