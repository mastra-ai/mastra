---
title: "Worker authentication | Deployment"
...
When workers run in separate processes from the API, they communicate over HTTP. The orchestration worker calls the API's step execution endpoint, and push-mode brokers (like Google Cloud Pub/Sub) deliver events to the API's event endpoint. Both endpoints require authentication when an auth provider is configured.

## How it works

Worker authentication uses the same auth pipeline as the rest of your Mastra server. There is no separate "worker auth" system. The orchestration worker sends credentials with each HTTP request, and the server's configured `authenticateToken` provider validates them.

Two internal endpoints are involved:

| Endpoint | Used by | Purpose |
| - | - | - |
| `POST /api/workflows/:id/runs/:runId/steps/execute` | Orchestration worker via `HttpRemoteStrategy` | Execute a workflow step on the API |
| `POST /api/workflows/events` | Push-mode brokers (GCP Pub/Sub, SNS) | Deliver workflow events to the API |

Both routes have `requiresAuth: true`. When no auth provider is configured, they are publicly accessible.
...
### 1. Configure an auth provider on the server
...
- **Rotate tokens regularly.** Update the `WORKER_TOKEN` environment variable and restart the affected containers.
...
- [Simple Auth](/docs/server/auth/simple-auth): Token-to-user mapping authentication
- [Worker deployment](/docs/deployment/worker-deployment): Set up split worker processes
- [Worker configuration reference](/docs/deployment/worker-reference): Environment variables and CLI commands
