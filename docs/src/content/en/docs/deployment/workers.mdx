...
:::tip Helm
On Kubernetes, the [`mastra-projects` Helm chart](/integrations/deploy/kubernetes-helm#background-workers) renders these Deployments for you, keeps the workers on the same image and environment as the Server, and refuses to install combinations that would run every worker twice. The manifests below are for writing them by hand.
:::
