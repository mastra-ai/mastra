...
Studio also supports optional runtime feature flags through `MASTRA_*` environment variables. For example, set `MASTRA_THEME_TOGGLE=true` to enable the Settings page theme switch and allow users to persist a light/dark preference in local storage. If the flag is unset (or set to any value other than `true`), Studio keeps the default dark-only behavior and hides the theme switch.
