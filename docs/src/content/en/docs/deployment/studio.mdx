...
Studio also supports optional runtime feature flags through `MASTRA_*` environment variables. Set `MASTRA_THEME_TOGGLE=true` to enable the Settings page theme switch and let users persist a theme preference of `dark | light | system` (with `system` following the OS) in local storage. If the flag is unset (or set to any value other than `true`), Studio remains dark-only and hides the Settings page theme switch.
