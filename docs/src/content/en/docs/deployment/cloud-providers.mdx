...
<Tabs items={["Digital Ocean"]}>
   ...
   <Tabs.Tab>
      Deploy your Mastra application to Digital Ocean's Droplets. This guide will cover setting up a droplet, a reverse proxy using Nginx, and running your Mastra application.
      ...
      <Steps>
         ...
         ```bash copy
         sudo rm -rf /etc/nginx/sites-available/$SITE_NAME
         sudo rm -rf /etc/nginx/sites-enabled/$SITE_NAME
         ```
