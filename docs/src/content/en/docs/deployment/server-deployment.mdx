...
The `mastra build` commands starts the build process:
...
3. **Copies static assets**: Moves the `public/` folder to the output directory for serving static files.
...
<FileTree>
  <FileTree.Folder name=".mastra" defaultOpen>
    ...
    <FileTree.Folder name="output" defaultOpen>
      ...
      <FileTree.File name="instrumentation.mjs" />
      <FileTree.Folder name="public" />
    </FileTree.Folder>
  </FileTree.Folder>
</FileTree>
