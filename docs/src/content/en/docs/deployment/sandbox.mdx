...
Open the Studio URL, the sandbox root, in a browser. `SandboxDeployer()` includes Studio by default, so the root serves the Studio interface. With `studio: false`, the root serves the Mastra welcome page instead.
...
`GET /api/agents` lists the deployed agents and works in a browser, which makes it a quick smoke test.
...
{
  "scripts": {
    "build": "dotenv -e .env -- mastra build"
  }
}
```

Then run `npm run build` instead of `mastra build`. In continuous integration (CI), export the credentials as secrets instead. Whatever mechanism you use, the variables must exist in the shell environment, not only in the `.env` file.
