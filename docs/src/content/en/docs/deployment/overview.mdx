...
For production workloads, you can move background processing into separate processes or containers. Workers handle workflow orchestration, cron schedules, and background tool execution. Each worker type scales independently from the API.

See the [Workers overview](/docs/deployment/workers) for concepts and the [worker deployment guide](/guides/deployment/mastra-workers) for Docker Compose and Kubernetes setup.
