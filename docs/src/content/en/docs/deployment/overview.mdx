...
For larger deployments, run the orchestration, scheduler, and background-task workers as separate processes that share storage and a `PubSub` backend with the server.

Read about [standalone workers](/docs/deployment/standalone-workers).
