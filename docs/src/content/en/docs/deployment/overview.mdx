...
- **Node.js**: Fully compatible; v20.0+ recommended. All features are tested and officially supported.
- **Bun**: Compatible via its Node.js API layer and npm support. No modifications required in most cases.
- **Deno**: Compatible using its Node.js compatibility mode. Requires `node:` specifiers and may need minor adjustments for full interop.
