...
- **API Token**: TODO
- **Build variables**: Your LLM API key, e.g `OPENAI_API_KEY`
- **Non-production branch deploy command**: TODO `npx wrangler versions upload`
...
To add secrets to your Cloudflare Workers application for manual deployment, use the Wrangler CLI. Secrets are environment variables that are encrypted and stored securely.

Run the following to add an API key to your local environment:
