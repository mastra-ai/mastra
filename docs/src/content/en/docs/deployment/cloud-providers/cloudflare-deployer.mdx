...
The `CloudflareDeployer` class handles deployment of standalone Mastra applications to Cloudflare Workers. It manages configuration, deployment, and extends the base [Deployer](/reference/v0/deployer/) class with Cloudflare specific functionality.

## Installation

```bash copy
npm install @mastra/deployer-cloudflare@^0
```
...
> See the [CloudflareDeployer](/reference/v0/deployer/cloudflare) API reference for all available configuration options.
...
- [Mastra Client SDK](/reference/v0/client-js/mastra-client)
