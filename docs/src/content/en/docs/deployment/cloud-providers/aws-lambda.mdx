...
Lambda functions have limitations with file system storage. Configure your Mastra application to use either ephemeral memory or external storage providers:

#### Option 1: Ephemeral Memory (Simplest)

```typescript filename="src/mastra/index.ts" copy showLineNumbers
import { Mastra } from "@mastra/core";
import { EphemeralMemory } from "@mastra/core/memory";
...
For persistent memory across Lambda invocations, use external storage providers like LibSQL with Turso or PostgreSQL:
...
# Set the port for Lambda Web Adapter
ENV PORT=8080
...
2. Visit the URL in your browser to see the Mastra home screen
