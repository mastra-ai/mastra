...
### How custom routes appear in the OpenAPI spec

Custom routes are served at the root path (e.g. `http://localhost:4111/health`), while built-in Mastra routes are served under the `/api` prefix (e.g. `http://localhost:4111/api/agents`). To reflect this, the generated OpenAPI spec uses a per-path [`servers` override](https://spec.openapis.org/oas/v3.1.0#path-item-object) on custom route paths:
...
OpenAPI-compliant clients resolve built-in routes against `/api` (the top-level `servers`) and custom routes against `/` (the path-level override).
