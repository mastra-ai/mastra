...
By default Mastra exposes routes under the `/api` prefix, so custom routes cannot use that prefix.
If you need to use `/api` for your custom routes, you can change the Mastra API prefix to something else (e.g. `server: { apiPrefix: '/internal-api' }`) to free up `/api` for your routes. Just be sure to choose a prefix that doesn't conflict with your custom routes.
...
Built-in streaming helpers such as [`chatRoute()`](/reference/ai-sdk/chat-route) forward the incoming request's `AbortSignal` to `agent.stream()`. That is the right default when a browser disconnect should cancel the model call.
