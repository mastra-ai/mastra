...
## Use `@mastra/react` in browser apps

If you're building a React UI, install `@mastra/react` and import its hooks from the package root. The root entry is safe to use in browser bundlers such as Vite, so you do not need custom aliases or Node.js polyfills.

```bash npm2yarn
npm install @mastra/react@latest
```

The following example demonstrates the supported browser import path:
...
Mastra's client-side streaming APIs expose standard Web Streams types such as `ReadableStream`, so they work in browser environments without relying on Node.js stream types.
