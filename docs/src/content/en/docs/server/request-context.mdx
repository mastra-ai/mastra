...
### Memory requires a resource scope

`MASTRA_RESOURCE_ID_KEY` is the authorization boundary for memory. When a request is authenticated but no resource ID can be derived from the request context, memory routes fail closed with a 403 instead of returning every resource's threads. Client-provided resource IDs are ignored in this case, because a caller could otherwise pass another user's resource ID.

Configure `mapUserToResourceId` (or set `MASTRA_RESOURCE_ID_KEY` in middleware) so authenticated users are scoped to their own memory.

Deployments that intentionally expose all memory to every authenticated caller can opt out:
...
Unauthenticated local development, Studio auth, and deployments using an FGA provider are unaffected.
