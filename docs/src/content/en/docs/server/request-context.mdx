...
`MASTRA_RESOURCE_ID_KEY` is the authorization boundary for memory. Memory routes return a 403 when a request is authenticated but no resource ID can be derived from the request context. Client-provided resource IDs are ignored in that case, because a caller could otherwise pass another user's resource ID.
