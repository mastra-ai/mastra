...
These paths use the default [`server.apiPrefix`](/reference/configuration#serverapiprefix) of `/api`. If you configure a different prefix, update the middleware paths to match it. Custom API routes must live outside the configured API prefix and aren't blocked by these patterns.
