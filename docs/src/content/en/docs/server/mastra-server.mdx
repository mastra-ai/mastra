...
Mastra also exposes an OpenAI-compatible Responses API route. By default, the generated server serves it at `/api/v1/responses`, along with `GET /api/v1/responses/:responseId` and `DELETE /api/v1/responses/:responseId`.

Use the agent ID as the `model` value and point your OpenAI Responses API client at your Mastra server base URL:
...
Set `stream: true` to receive SSE events compatible with the Responses API event model. When the target agent has memory configured, `store: true` persists the response ID on the underlying memory thread so you can continue the conversation later with `previous_response_id`. If the agent is stateless, the request still succeeds and the returned `store` flag remains `false`.
