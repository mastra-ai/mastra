...
Mastra also exposes a Responses API-compatible route. By default, the generated server serves it at `/api/v1/responses`, along with `GET /api/v1/responses/:responseId` and `DELETE /api/v1/responses/:responseId`.
