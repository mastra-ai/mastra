...
Use a Mastra model string such as `openai/gpt-5` for `model`. If you want agent-backed execution, pass a separate `agent_id`. Without `agent_id`, the route runs as a stateless model-only request:
