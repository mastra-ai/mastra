...
If you're using server adapters directly, you can configure redaction behavior. Visit the [Stream data redaction](/docs/v1/server/server-adapters#stream-data-redaction) docs to learn more.

## TypeScript configuration

Mastra requires `module` and `moduleResolution` values that support modern Node.js versions. Older settings like `CommonJS` or `node` are incompatible with Mastra's packages.
