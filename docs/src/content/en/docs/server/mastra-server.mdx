...
This page covers the `server` configuration options passed to the `Mastra` constructor. For running Mastra with your own HTTP server (Hono, Express, etc.), see [Server Adapters](./server-adapters).
...
See the [Middleware](./middleware) and
[Custom API Routes](./custom-api-routes) pages for details on
...
If you're using server adapters directly, you can configure redaction behavior. See [Stream data redaction](./server-adapters#stream-data-redaction) in the Server Adapters docs.
