...
- An API key from a supported [Model Provider](/models/v0)

## Adding Mastra

First, install the necessary Mastra dependencies to run an Agent. This guide uses OpenAI as its model but you can use any supported [model provider](/models/v0).

```bash copy
npm install mastra@^0 @mastra/core@^0 @mastra/libsql@^0 zod@^3.0.0 @ai-sdk/openai@^1.0.0
```
...
By importing the `src/mastra/index.ts` file you can use methods like [`.getAgent()`](/reference/v0/core/getAgent) to get programmatic access. With [`.generate()`](/reference/v0/agents/generate) you then can interact with the respective agent.

:::note

Read the [Agent reference docs](/reference/v0/agents/agent) to learn more.
...
In production it's not necessary to run Mastra alongside your Express server. But for development Mastra offers a [Local Development Environment](/docs/v0/getting-started/studio) which you can use to improve and debug your agent.
