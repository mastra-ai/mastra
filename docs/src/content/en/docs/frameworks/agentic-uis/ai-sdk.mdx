...
Mastra integrates nicely with [Vercel's AI SDK](https://sdk.vercel.ai) for model routing support, UI Hooks, and data streaming.
...
> See [Model Providers](/docs/getting-started/model-provider#ai-sdk-compatibility) for more information.
...
To manaully transform Mastra streams to AI SDK compatible format, leverage the `toAISdkFormat` util
...
This is a helpful util from the `ai` package for creating a readable stream and can be used alongside the `mastra/ai-sdk` package
`toAISdkFormat` util for stream transformations.
...
createUIMessageStreamResponse creates a Response object that streams messages to the client
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <input value={input} onChange={e=>setInputValue(e.target.value)} placeholder="Name of city" />
      </form>
    </div>
  );
}
...
### Passing additional useChat data

The useChat `sendMessage` allows you to pass additional data from the frontend to Mastra. This data can then be used on the server as `RuntimeContext`.
...
This example shows how to extract the custom data and populate a `RuntimeContext` instance.
...
## AI SDK V4 - V5 Official migration guide
