...
<Callout type="info">
  **Note**: The `stream` method with format support is experimental and may change as we refine the feature based on feedback. See the [Agent Streaming documentation](/docs/agents/streaming) for more details about stream.
</Callout>
