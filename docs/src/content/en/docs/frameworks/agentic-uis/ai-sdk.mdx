...
### Node.js Streaming (Express/NestJS)

For Node.js environments like Express or NestJS, use `pipeUIMessageStreamToResponse()` to stream UI messages to a `ServerResponse` object via Server-Sent Events.
...
The method accepts the same `uiOptions` as `toUIMessageStreamResponse()`, along with Node.js-specific parameters like `status`, `statusText`, and `headers`.
