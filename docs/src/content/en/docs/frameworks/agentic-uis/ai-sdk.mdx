...
> See [Using AI SDK with Mastra](/models/v0#use-ai-sdk-with-mastra) for more information.
...
When setting up a [custom API route](/docs/v0/server-db/custom-api-routes), use the `chatRoute()` utility to create a route handler that automatically formats the agent stream into an AI SDK-compatible format.
...
To  enable agent text chunks and tool calls streaming when a workflow step pipes an agent's stream to the workflow writer (see [Workflow Streaming](/docs/v0/streaming/workflow-streaming#workflow-using-an-agent)).
...
Here's an example: For a [nested agent stream within a tool](/docs/v0/streaming/tool-streaming#tool-using-an-agent), `data-tool-agent` UI message parts will be emitted and can be leveraged on the client as documented below:
...
For more information about tool streaming see [Tool streaming documentation](/docs/v0/streaming/tool-streaming)
...
> When using `useChat()` with agent memory, refer to the [Agent Memory section](/docs/v0/agents/agent-memory) for key implementation details.
...
> You can then access this data in your tools via the `runtimeContext` parameter. See the [Runtime Context documentation](/docs/v0/server-db/runtime-context) for more details.
