...
export async function POST(req: Request) {
  ...
  const stream = await myAgent.stream(messages, { requestContext, format: 'aisdk'  });
  return stream.toUIMessageStreamResponse();
}
```
...
> You can then access this data in your tools via the `requestContext` parameter. See the [Request Context documentation](/docs/server-db/runtime-context) for more details.
