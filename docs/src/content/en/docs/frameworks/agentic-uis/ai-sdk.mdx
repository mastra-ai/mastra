...
### Passing metadata with messages

Mastra supports attaching metadata to individual messages using the `UIMessageWithMetadata` type. This is useful when migrating from AI SDK or when you need to store additional context on specific messages that should persist with the conversation history.

```typescript {2,8-10} filename="app/test/chat-metadata.tsx" showLineNumbers copy
"use client";

import { UIMessageWithMetadata } from "@mastra/core/agent";
import { useChat } from "@ai-sdk/react";

export function ChatWithMetadata() {
  const { messages, input, handleInputChange, handleSubmit } = useChat({
    api: "/api/chat-metadata",
    // Include metadata when constructing messages
    transformMessages: (messages) => messages.map(msg => ({
      ...msg,
      metadata: {
        source: "web-ui",
        timestamp: Date.now(),
    ...
    <div>
      {messages.map((message) => (
        <div key={message.id}>
          ...
          {message.metadata && (
            <small> (metadata: {JSON.stringify(message.metadata)})</small>
          )}
        </div>
      ))}
      <form onSubmit={handleSubmit}>
...
On the server side, the metadata is preserved throughout the agent's processing:

```typescript {1,7,11} filename="app/api/chat-metadata/route.ts" showLineNumbers copy
import { UIMessageWithMetadata } from "@mastra/core/agent";
import { mastra } from "../../../mastra";
...
export async function POST(req: Request) {
  ...
  const messagesWithMetadata: UIMessageWithMetadata[] = messages;
  
  const myAgent = mastra.getAgent("weatherAgent");
  const stream = await myAgent.stream(messagesWithMetadata);

  return stream.toDataStreamResponse();
}
```

The metadata persists when using agent memory:

```typescript filename="app/api/chat-memory-metadata/route.ts" showLineNumbers copy
import { UIMessageWithMetadata } from "@mastra/core/agent";
import { mastra } from "../../../mastra";
...
export async function POST(req: Request) {
  ...
  const messagesWithMetadata: UIMessageWithMetadata[] = messages.map(msg => ({
    ...msg,
    metadata: {
      ...msg.metadata,
  ...
  // Metadata is saved with messages in memory
  const stream = await myAgent.stream(messagesWithMetadata, {
    memory: {
...
<Callout type="info">
  When migrating from AI SDK, wrap any custom message properties in a `metadata` object. For example, if you had `{ role: "user", content: "Hello", customField: "value" }`, update it to `{ role: "user", content: "Hello", metadata: { customField: "value" } }`.
</Callout>
