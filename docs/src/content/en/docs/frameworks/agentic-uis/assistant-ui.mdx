...
This middleware ensures that when Assistant UI sends a request with `state: null` in the request body, we remove that property to make the request work properly with Mastra.
