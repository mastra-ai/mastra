...
SvelteKit uses the `$env` alias to access environment variables.  As a result, the model constructor must explicitly receive the `apiKey` from the SvelteKit environment like this:
