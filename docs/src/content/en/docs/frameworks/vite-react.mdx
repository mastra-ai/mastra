...
<Callout type="warning">
  This guide assumes you're using the Vite/React with React Router v7 at the
  root of your project, e.g., `app`.
</Callout>
...
<Tabs items={["npm", "CLI"]}>
  <Tabs.Tab>```bash copy npm run dev:mastra ```</Tabs.Tab>
  <Tabs.Tab>```bash copy mastra dev:mastra ```</Tabs.Tab>
</Tabs>
