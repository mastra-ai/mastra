...
await ciSignals.watch({ resourceId: 'user_123', threadId: 'thread_456' }, 'acme-app-main')
```
...
export class CiSignals extends SignalProvider<'ci-signals'> {
  ...
  async handleWebhook(request: SignalProviderWebhookRequest) {
    ...
    const subscriptions = await this.getSubscriptionsForResource(payload.pipeline)
...
For generic webhook sources, use [`WebhookSignalProvider`](/reference/signals/webhook-signal-provider) instead of writing a subclass. Configure it with a function that extracts a resource ID from the payload. You can also extract a stable webhook delivery ID for cross-replica deduplication or provide a custom notification builder.
...
await webhooks.subscribeThread({ resourceId: 'user_123', threadId: 'thread_456' }, 'acme/app')
```
