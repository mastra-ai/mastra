...
The thread must exist before the first fire. `agent.sendSignal()` throws if the target thread is missing, and every fire will fail with `thread "<id>" not found`. Create the thread with `memory.createThread()` before calling `mastra.schedules.create()`.
...
// Register `pinger` on your Mastra instance, then:

const memory = await mastra.getAgent('pinger').getMemory()
...
#### Wake behavior on idle threads

When the target thread has no stream running, Mastra needs an explicit instruction to start one. Without `ifIdle: { behavior: 'wake' }` (or `'queue'` to hold the signal for the next stream), the signal is delivered to the thread but the agent doesn't run, and no response is written to the thread. For scheduled fires that should always produce an agent response, set `ifIdle: { behavior: 'wake' }` on every threaded schedule.

#### Signal type and thread visibility

`signalType` controls how the fire lands in the thread. The default is `'notification'`, which wakes the agent but doesn't render as a user-authored message in the thread's message history. To make each fire visible set `signalType: 'user'`.
...
#### Fixed fields on threaded schedules

`threadId` and `resourceId` are set at create time and cannot be patched via `update()`. To change the thread a schedule delivers into, delete the schedule and create a new one.
