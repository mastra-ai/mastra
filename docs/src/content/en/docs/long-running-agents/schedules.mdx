...
These fields require a `threadId`. For the full threaded input shape, see the [agent schedule input reference](/reference/schedules/schedules#createinput).
...
`pause` and `resume` are durable. `run` fires the schedule once immediately without affecting its cadence. For the complete method list, filters, and patch fields, see the [`mastra.schedules` reference](/reference/schedules/schedules).
...
See the [`create(input)` reference](/reference/schedules/schedules#createinput) for ID normalization rules and duplicate ID behavior.
...
- [`mastra.schedules`](/reference/schedules/schedules): API reference for creating and managing schedules.
