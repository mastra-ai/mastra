...
The thread must exist before the first fire. `agent.sendSignal()` throws if the target thread is missing, and every fire will fail with `thread "<id>" not found`. Create the thread with `memory.createThread()` before the first cron fire.
:::
