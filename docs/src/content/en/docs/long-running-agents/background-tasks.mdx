...
Once a tool has opted in, the LLM can optionally include a `_background` field in the tool arguments to select `foreground`, `deferred`, or `awaited` execution for a specific call and override its timeout or retries.
...
1. LLM `_background.disposition` or `_background.enabled` override (only applied when the tool was opted in at one of the layers above).
...
Use this pattern when you want a subagent to behave consistently regardless of which supervisor invokes it. Use the supervisor-side opt-in above when you want to tune background behavior centrally per supervisor. Arbitrary-depth delegated background work, workflow integration, and Alexandria integration aren't supported by this boundary.
