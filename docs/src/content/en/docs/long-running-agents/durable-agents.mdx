...
## Run until completion or abort

Durable agents use a finite step ceiling by default. When the task itself must decide how long it runs, set `maxSteps: false` on the durable wrapper:
...
With no step ceiling, the loop continues until the model completes normally, a semantic stop condition completes the task, the run suspends, or the run is aborted. An unbounded run can consume resources indefinitely if none of those conditions occurs, so expose an abort path and monitor run cost and duration.
