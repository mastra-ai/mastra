...
A subagent's `description` is routing text for the parent model. Write it like a tool description: explain what the subagent does and when to use it.
...
```typescript
agentConfig({
  description: 'Use to turn research notes into a structured draft with headings and citations.',
})
```
