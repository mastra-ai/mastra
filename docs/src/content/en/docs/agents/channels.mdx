...
### Disabling the default channel processor

On every step, Mastra adds a short system message telling the agent which channel and platform a request came from (DM vs public channel, platform name, bot display name) via the built-in `ChatChannelProcessor`. Set `addProcessor: false` to skip it entirely:
...
For most customization, register your own input processor with `id: 'chat-channel-context'` instead — it shadows the built-in one, so you don't need to disable it separately.
