  ...
  channels: {
    adapters: {
      slack: {
        ...
        onToolStart: async ({ displayName, channel }) => {
          await channel.status.set(`Running ${displayName}…`)
        },
        onToolEnd: async ({ channel }) => {
          await channel.status.clear()
        },
      },
    },
  },
...
`channel.status` is best-effort. Platforms with status support show the update. Other platforms no-op. Mastra also clears status when the stream finishes or errors. Use `isError` in `onToolEnd` to handle failed tool results.
