...
On every step, Mastra adds a short system message telling the agent which channel and platform a request came from (DM vs public channel, platform name, bot display name). Customize or disable this via `threadContext.systemMessage`:
...
- `false` skips the channel system message entirely.
- A `string` is used verbatim.
- A function `(ctx) => string | undefined` builds it dynamically. Return `undefined` or `''` to skip for that request.

Branch only on thread-stable inputs (`platform`, `isDM`) so the resolved content stays the same across turns and prompt caching can hit. For mid-conversation state, send a signal into the thread instead. `defaultChannelSystemMessage` is exported from `@mastra/core/channels` if you want to compose with the built-in template.
