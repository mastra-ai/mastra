...
The `toolDisplay` fn posts a Block Kit card with an additional "Explain" button. `onAction` routes the custom `explain:*` action ID and falls through to the default handler for the built-in `tool_approve:*` / `tool_deny:*` IDs:
