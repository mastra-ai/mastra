...
### Disabling the channel system message

Mastra adds a short system message telling the agent which channel and platform a request came from (DM vs public channel, platform name, bot display name). Set `threadContext.addSystemMessage: false` to skip it entirely:
