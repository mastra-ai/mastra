...
Use per-adapter tool lifecycle hooks to run side effects when tools start and finish. For example, set a channel status while a tool runs:
  ...
  channels: {
    adapters: {
      slack: {
        ...
        // Optional: bridge channel.status to this adapter's platform-specific status API.
        // If omitted and the adapter does not implement setStatus/clearStatus, status no-ops.
        status: {
          set: async ({ threadId, text }) => {
            // Map threadId/text to your adapter's status API.
          },
          clear: async ({ threadId }) => {
            // Clear the platform status for threadId.
          },
        },
...
`channel.status` is best-effort. Visible status requires either per-adapter `status` handlers or adapter methods such as `setStatus(threadId, text)` and `clearStatus(threadId)`. Other adapters no-op. For Slack assistant status, bridge to the Slack adapter's status API in `status.set` and `status.clear`. Mastra core does not call Slack-specific status methods directly. Mastra also clears status when the stream finishes or errors. Use `isError` in `onToolEnd` to handle failed tool results.
