...
Mastra adds a short system message telling the agent which channel and platform a request came from (DM vs public channel, platform name, bot display name) via the built-in `ChatChannelProcessor`. Set `addProcessor: false` to skip it entirely:
