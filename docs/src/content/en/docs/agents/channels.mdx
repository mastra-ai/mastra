...
export const supportAgent = new Agent({
  ...
  channels: {
    adapters: {
      slack: {
        // Status updates no-op unless the adapter implements Mastra status methods.
...
`channel.status` is best-effort. Visible status requires the adapter to implement Mastra status methods such as `setStatus(threadId, text)` and `clearStatus(threadId)`. Other adapters no-op. For Slack assistant status, use a Slack adapter version or wrapper that implements these methods. Mastra core does not call Slack-specific status methods directly. Mastra also clears status when the stream finishes or errors. Use `isError` in `onToolEnd` to handle failed tool results.
