...
:::note
When streaming responses over HTTP, Mastra also automatically redacts sensitive request data (system prompts, tool definitions, API keys) from stream chunks at the server level. See [Stream data redaction](/docs/v1/server-db/mastra-server#stream-data-redaction) for details.
:::
