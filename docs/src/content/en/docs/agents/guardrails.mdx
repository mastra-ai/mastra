...
The `ClassifierProcessor()` runs a typed [`Classifier`](/reference/classifier/classifier) over messages and applies a policy you define. The classifier returns evidence such as probabilities, choices, and scores; your `decide` function turns that evidence into `pass`, `block`, or `filter`. Built-in `decisions` helpers cover common thresholds.

```typescript {8-14} title="src/mastra/agents/classified-agent.ts"
import { Classifier } from '@mastra/core/classifier'
import { ClassifierProcessor, decisions } from '@mastra/core/processors'

export const classifiedAgent = new Agent({
  id: 'classified-agent',
...
Visit [`ClassifierProcessor()`](/reference/processors/classifier-processor) reference for decision semantics and the `decisions` helpers.
