...
If a user message or agent response fails the checks defined by a processor, the request is immediately stopped and no further processors run. The agent will still respond with a message but will also include details explaining what was detected and how it was handled.
