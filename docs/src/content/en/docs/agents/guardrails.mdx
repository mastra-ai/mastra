...
The `ClassifierProcessor()` runs a typed [`Classifier`](/reference/classifier/classifier) over messages. The classifier returns evidence such as probabilities, choices, and scores; your `onResult` callback turns that evidence into a decision by calling `abort(reason)` to stop the request or `filter()` to drop the content.

```typescript {8-17} title="src/mastra/agents/classified-agent.ts"
...
Visit [`ClassifierProcessor()`](/reference/processors/classifier-processor) reference for behavior by phase and the `filter()` option.
