...
```typescript {10} filename="src/mastra/agents/private-agent.ts" showLineNumbers copy
import { PIIDetector } from "@mastra/core/processors";
...
When a processor blocks a request, the agent will still return successfully without throwing an error. To handle blocked requests, you can check for a `tripwire` or `tripwireReason` in the response.

For example, if an agent uses the `PIIDetector` with `strategy: "block"` and a `.generate()` or `.stream()` call includes a credit card number, the request would be blocked, and the response will include a `tripwireReason`:
...
The response will include a `tripwireReason` indicating the type of PII that was detected.
