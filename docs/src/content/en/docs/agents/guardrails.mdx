...
Agents use processors to implement guardrails by intercepting, modifying, validating, or blocking messages at different stages of the AI interaction. Processors can transform, filter, or abort requests based on defined logic.
...
<Tabs items={["Input", "Output","Hybrid",]}>
  <Tabs.Tab>
...
Processors behave differently depending on their purpose:

- **Transformers**: Modify messages (e.g. normalize input, translate language). These allow processing to continue.
- **Validators**: Check for violations (e.g. prompt injection, PII, harmful content) and may stop further processing if certain conditions are not met.

If a validator stops processing:

- No subsequent processors in the same `inputProcessors` or `outputProcessors` array are run.
- The agent still returns a 200 response.

In all cases:

- The response includes details about what was detected and how it was handled.
...
```typescript
const result = await agent.generate("Is this credit card number correct? 4543 1374 5089 4332");

console.log(result.text);

const stream = await agent.stream("Is this credit card number correct? 4543 1374 5089 4332");
...
The response would include a message from the agent along with details explaining what was detected and how it was handled.

```text
[PIIDetector] Redacted PII: PII detected. Types: credit-card. Detections: 1 items

I'm sorry, but I can't assist with entering or processing sensitive information like credit card numbers.
```
