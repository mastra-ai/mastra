...
With code mode, your tools keep running on the host with full validation, authorization, request context, cancellation, and tracing. Only the model's orchestration code runs in the sandbox. Each `external_*` call is bridged back to the real tool on the host and authorized as that tool, so permission to invoke the Code Mode tool doesn't grant permission to invoke every tool in its allow-list. The function can reduce or aggregate results before returning one response to the agent.
...
The generated code for `sales_code` can't call an inventory tool, and the reverse holds too. Code Mode also checks the caller's permission for each allowed tool when `external_*` invokes it. Use both controls for least-privilege scoping and to keep each tool's prompt surface small.
