...
You can attach skills directly to an agent through its `skills` config, or manage them centrally on a [workspace](/docs/workspace/skills) that agents share. This page covers the agent-level approach — defining skills in code, loading them from files, and resolving them per request.
