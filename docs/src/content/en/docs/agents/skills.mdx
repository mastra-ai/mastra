...
Attaching skills directly to an agent is the simplest way to get started. Your capabilities are defined in code and travel with the agent definition, so there's nothing extra to configure. As your project grows and you want to share skills across many agents or load them from files on disk, you can also organize them in a [workspace](/docs/workspace/skills).
