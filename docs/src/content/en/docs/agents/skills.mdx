...
Use `defineSkill()` to create skills entirely in code:

```typescript title="src/mastra/skills.ts"
import { defineSkill } from '@mastra/core/skills'

export const releaseChecklist = defineSkill({
...
:::note
Visit [`defineSkill()` reference](/reference/agents/defineSkill) for the full API.
:::
...
const customReview = defineSkill({
...
Use `agent.skill()` and `agent.listSkills()` to access skills from application code (e.g., in workflows or API routes):

```typescript title="src/routes/skills.ts"
import { reviewer } from '../mastra/agents'

// Get a specific skill by name
const skill = await reviewer.skill('code-review')
if (skill) {
...
:::note
Visit [`.skill()` reference](/reference/agents/skill) and [`.listSkills()` reference](/reference/agents/listSkills) for the full API.
:::
