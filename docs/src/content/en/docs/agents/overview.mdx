...
Call `.generate()` with a single string:
...
Call `.generate()` with an array of strings:

```typescript showLineNumbers copy
const response = await testAgent.generate([
  "Help me organize my day",
  "My day starts at 9am and finishes at 5.30pm",
  "I take lunch between 12:30 and 13:30",
  "I have meetings Monday to Friday between 10:30 and 11:30"
]);
...
Call `.stream()` with a single string:
...
Call `.stream()` with an array of strings:
...
```typescript showLineNumbers copy
const stream = await testAgent.stream([
  { role: "assistant", content: "Help me organize my day" },
  { role: "user", content: "My day starts at 9am and finishes at 5.30pm" },
  { role: "user", content: "I take lunch between 12:30 and 13:30" },
  { role: "user", content: "I have meetings Monday to Friday between 10:30 and 11:30" }
]);
...
Agents can return structured, type-safe data by defining the expected output with either a [JSON Schema](https://json-schema.org/) or a [Zod](https://zod.dev/). In both cases, the parsed result is available on `response.object`, allowing you to work directly with validated and typed data.

### Using JSON Schema

Define the `output` shape using [JSON Schema](https://json-schema.org/):
...
Define the `output` shape using a [Zod](https://zod.dev/):
...
If you need to generate structured output with an agent that uses a tool, use `experimental_output` instead of `output`:
...
```typescript showLineNumbers copy
const response = await testAgent.generate([{ role: "user", content: "If a taxi driver earns $41 per hour and works 12 hours a day, how much do they earn in one day?" }], {
  maxSteps: 5
});
...
```typescript showLineNumbers copy
const response = await testAgent.generate([{ role: "user", content: "Calculate the taxi driver's daily earnings." }], {
  maxSteps: 5,
