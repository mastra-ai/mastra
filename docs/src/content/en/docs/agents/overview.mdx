...
Some model providers have specific schema requirements. For example, OpenAI's strict mode requires all properties to be in the `required` array. Use `processSchema` to transform your Zod schema before passing it to the agent, ensuring the same schema works for both backend generation and frontend validation.
...
// Use in agent
const response = await testAgent.generate("Extract user information", {
  structuredOutput: {
    schema: openaiSchema,
  },
});
...
const validatedData = openaiSchema.parse(response.object);
```

The `processSchema` utility converts `.optional()` fields to `.nullable()` for OpenAI models, ensuring all fields are in the required array but can accept null values. Other providers have their own transformations: `processSchema.anthropic()`, `processSchema.google()`, `processSchema.deepseek()`, `processSchema.meta()`, and `processSchema.openaiReasoning()` for reasoning models.
