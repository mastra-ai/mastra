...
Some model providers have specific schema requirements. For example, OpenAI's strict mode requires all properties to be in the `required` array. Use `applySchemaCompatTransformation` to automatically transform your Zod schema to meet these requirements.
...
const response = await testAgent.generate(
  "Extract user information",
  {
    structuredOutput: {
      ...
      applySchemaCompatTransformation: true, // Automatically fixes compatibility issues
    },
  },
);
...
This is particularly useful when working with optional fields (`.optional()`), which get transformed to just `.nullable()` for OpenAI models, ensuring the field is in the required array but can accept null values.
