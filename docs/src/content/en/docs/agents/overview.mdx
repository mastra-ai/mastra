...
Agents can return structured, type-safe data by defining the expected output with either a [JSON Schema](https://json-schema.org/) or a [Zod](https://zod.dev/) schema. In both cases, the parsed result is available on `response.object`, allowing you to work directly with validated and typed data.
