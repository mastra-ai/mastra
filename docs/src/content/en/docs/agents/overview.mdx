...
If you need to generate structured output with agents that use tools, you'll need to use the `experimental_output` or `structuredOutput` property instead of `output`.

#### `experimental_output`

Define the `experimental_output` shape using [Zod](https://zod.dev/):
...
Define the `structuredOutput` shape using [Zod](https://zod.dev/):
