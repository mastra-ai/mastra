...
| `frequencyPenalty` | `number` | Penalizes tokens based on their frequency in the text so far (−2 to 2). Reduces repetition. |
| `presencePenalty` | `number` | Penalizes tokens based on whether they appear in the text so far (−2 to 2). Encourages new topics. |
