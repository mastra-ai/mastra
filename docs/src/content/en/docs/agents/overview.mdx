...
You can also return a full fallback array from `model` when using `RequestContext`.
...
export const testAgent = new Agent({
  ...
  model: ({ requestContext }) => {
    ...
    // Models without explicit maxRetries inherit agent-level maxRetries (2).
    return [
      { model: "openai/gpt-4.1-nano" },
      { model: "openai/gpt-4.1-mini", maxRetries: 1 },
    ];
  },
});
```
