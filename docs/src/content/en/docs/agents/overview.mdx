...
When referencing an agent, use `mastra.getAgentById()` instead of a direct import. `mastra.getAgentById()` wires up shared services (storage, logging, other agents, etc.). A directly imported agent skips this setup, so features like memory, tool approval, and subagent delegation won't work.
