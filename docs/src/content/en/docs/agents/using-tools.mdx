...
Use `hooks` to run custom logic before and after every tool call an agent makes. Hooks apply to all tool sources: assigned tools, memory tools, toolsets, client tools, agent and workflow tools, and [workspace tools](/ecosystem/workspace/overview#tool-hooks). Common uses include logging, auditing, input validation, and blocking specific calls.
