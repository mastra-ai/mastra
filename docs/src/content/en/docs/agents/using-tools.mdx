...
See the [`createTool()` reference](/reference/tools/create-tool#example-with-transform) for a `transform` example. For shared rules across several tools, configure the agent-level `toolPayloadProjection` policy in the [`Agent` constructor](/reference/agents/agent#constructor-parameters).
