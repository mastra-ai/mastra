...
This example uses three different names:

- `weather`: The Mastra registration key used by [`mastra.getTool('weather')`](/reference/core/getTool).
- `weather-tool`: The intrinsic `id` defined by `weatherTool` in the quickstart. [`mastra.getToolById('weather-tool')`](/reference/core/getToolById) searches intrinsic IDs first, then falls back to registration keys.
- `weatherTool`: The key returned by the Agent's `tools` callback and exposed to the model.

Registering `sharedWeatherAgent` in `new Mastra({ agents: { sharedWeatherAgent } })` associates the Agent with that Mastra instance before its function-based tools resolve. For a standalone Agent, pass the `mastra` constructor option explicitly and keep the guard because the callback type is optional. See the [`Agent` reference](/reference/agents/agent) for the constructor type.

[`mastra.listTools()`](/reference/core/listTools) returns the Mastra-level registry. It doesn't include tools that exist only in a function-based Agent `tools` callback.
