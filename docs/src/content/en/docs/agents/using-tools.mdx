...
| `webSearchTool` | Provider-defined | Run provider-native web search with the active model |
| `webFetchTool` | `web_fetch` | Fetch a web page by URL and return its text content |

`webSearchTool` has no Mastra tool ID. It resolves to the active provider's own web search tool, so the ID depends on the provider: `openai.web_search`, `anthropic.web_search_20250305`, `google.google_search`, or `xai.web_search`.
