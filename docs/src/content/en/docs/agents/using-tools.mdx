...
When the agent calls a tool, the `toolName` in the stream response is determined by the **object key** you use when passing tools to the agent, not the tool's `id` property.
...
This gives you full control over how tools are identified in the stream. If you want the `toolName` to match the tool's `id`, use the tool's `id` as the object key.

### Sub-agents as tools

The same pattern applies to sub-agents. When you pass agents via the `agents` property, they are converted to tools with a hardcoded `agent-` prefix followed by your object key:
...
const orchestrator = new Agent({
  // ...
  agents: {
    weather: weatherAgent,  // object key is "weather"
  },
});
// Tool name becomes: "agent-" + "weather" = "agent-weather"
```

Note that you'll see two different identifiers in stream responses:
...
To have these align more closely, use the agent's id as the object key:
