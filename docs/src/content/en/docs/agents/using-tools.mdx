...
Your application code still receives `apiKey: 'secret-value'` from the tool. Browser-facing streams get the `display` output, and user-visible transcript messages get the `transcript` output.
