...
You can learn more about tools and MCP on the following links:

- [Tools overview](../tools-mcp/overview.mdx)
- [MCP Overview](../tools-mcp/mcp-overview.mdx)
...
```typescript {14} filename="src/mastra/tools/weather-tool.ts" showLineNumbers copy
...
> This examples uses the [wttr.in](https://github.com/chubin/wttr.in) API.
...
When the agent receives a message that includes a location, in this example London, it makes that data available to the tool's context.
...
An agent can use multiple tools to accomplish more complex tasks by delegating specific parts to individual tools. Tools are called in parallel, and the agent combines their responses into a single reply.
