...
Every agent adds whichever of these three your `errorProcessors` list does not already contain:
...
A default is only added when nothing in your list carries its `id`, so your own instance always wins and is never duplicated. An added default is placed at the position its `id` gives it among the defaults, so naming only a later one does not invert the pair — supplying just your own `StreamErrorRetryProcessor` still resolves with `ProviderHistoryCompat` and `PrefillErrorHandler` ahead of it.

Error processors stop at the first one that returns `{ retry: true }`, so that relative order decides which repair runs. The two repairing processors come first and the retrying one comes last, because `StreamErrorRetryProcessor`'s bad-request matcher matches any `400`: placed earlier it would claim a request the repairs could have fixed and resend it unchanged.
