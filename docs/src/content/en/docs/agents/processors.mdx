...
Mastra includes a built-in [`PrefillErrorHandler`](/reference/processors/prefill-error-handler) that automatically handles the Anthropic "assistant message prefill" error. This processor is included by default for all agents and requires no configuration — you do not need to add it to `errorProcessors` manually.
