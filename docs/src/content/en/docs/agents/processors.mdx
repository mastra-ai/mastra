...
Use `defaultModel` or `fallbackModel` when unmatched requests should still use a known model. Omit both to leave the agent's configured model unchanged when no rule matches.

See the [`ModelSwapperProcessor` reference](/reference/processors/model-swapper) for routing rules, thresholds, and behavior notes.
