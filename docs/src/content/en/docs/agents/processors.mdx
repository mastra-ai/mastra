...
### processOutputStream behavior

The return value of `processOutputStream` controls how chunks are handled:

- `return part` - emits the chunk
- `return null` - suppresses the chunk (nothing is emitted)
- `return undefined` - skips processing and passes the original chunk through unchanged

#### Using writer with suppression

You can inject custom output while suppressing the original chunk:

```ts
export async function processOutputStream({ part, writer }) {
...
Returning `null` after `writer.custom(...)` does not stop the stream. Processing continues normally.
