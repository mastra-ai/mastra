...
Error processors stop at the first one that returns `{ retry: true }`, so that relative order decides which repair runs. The processors that repair a request come first and the retrying one comes last, because `StreamErrorRetryProcessor`'s bad-request matcher matches any `400`: placed earlier it would claim a request the repairs could have fixed and resend it unchanged.
...
`processAPIError()` has a separate retry budget. When `errorProcessors` resolve to a non-empty list and `maxProcessorRetries` is omitted, the runtime allows up to `10` error-processor retries. Error processors are enabled by default on every agent (see [Error processors are enabled by default](#error-processors-are-enabled-by-default)), but `maxProcessorRetries` itself stays unset, so `processInputStep()` and `processOutputStep()` retries still need an explicit limit.
