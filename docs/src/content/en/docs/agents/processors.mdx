...
Every agent starts from your `errorProcessors` list and adds whichever of these three it does not already contain:
...
A default is only added when nothing in your list carries its `id`, so your own instance always wins and is never duplicated. Your processors keep their positions at the front of the list, ahead of the added defaults — error processors stop at the first one that returns `{ retry: true }`, so a processor you supply is always reached before the framework's retry.
...
The `StreamErrorRetryProcessor` above is the only stream-retry processor on the agent; the default `ProviderHistoryCompat` and `PrefillErrorHandler` are still added because your list does not name them. Pass all three ids to run a completely custom set.
