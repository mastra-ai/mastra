...
- `{ messages, systemMessages }` — Both messages and modified system messages. Use this to trim verbose system prompts, filter semantic-recall content, or adjust instructions per conversation.
...
For simpler per-step logic, use the `prepareStep` callback on `generate()` or `stream()` instead of creating a full processor:
...
The `result` object contains the resolved generation data — token usage, text, finish reason, and steps. Access it to track usage or inspect tool calls:
