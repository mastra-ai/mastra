...
Memory processors (when memory is enabled) are automatically added to the agent's processor pipeline. See [Memory Processors](/docs/v1/memory/memory-processors) for details on how memory processors integrate with manually configured processors.
