...
The built-in Anthropic tool-call ID rule rewrites IDs that fall outside Anthropic's `^[a-zA-Z0-9_-]+$` pattern in the outbound request only, on both the call and its paired result, so pairing survives. Persisted history keeps the original IDs, and the reactive recovery that used to repair them after a failed call is deprecated.
