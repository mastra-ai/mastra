...
For safety, privacy, moderation, and cost policies, start with [Guardrails](/docs/agents/guardrails). Guardrails compile policy groups into processors with sensible defaults. Use processors when you need advanced runtime customization or custom lifecycle hooks.
...
- Add custom lifecycle behavior before or after model calls.
- Transform messages, such as translating language or filtering tool calls.
- Limit token usage or message history length in a custom way.
- Add reliability behavior such as retries, provider compatibility, or caching.
- Build custom safety checks that aren't covered by guardrail policy groups.
- Apply custom business logic to messages.

Mastra includes several processors for common use cases. For built-in safety and compliance checks, use [Guardrails](/docs/agents/guardrails) first and drop down to processors when you need more control.
