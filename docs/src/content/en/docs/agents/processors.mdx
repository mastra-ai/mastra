...
This ordering ensures that if your output guardrail calls `abort()`, memory processors are skipped and no messages are saved. See [Memory Processors](../memory/memory-processors#processor-execution-order) for details.
...
**For security and validation processors**, see the [Guardrails](./guardrails) page for input/output guardrails and moderation processors.
**For memory-specific processors**, see the [Memory Processors](../memory/memory-processors) page for processors that handle message history, semantic recall, and working memory.
...
- [Guardrails](./guardrails) - Security and validation processors
- [Memory Processors](../memory/memory-processors) - Memory-specific processors and automatic integration
