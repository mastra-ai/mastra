...
To run with no error processors at all, pass an explicitly empty array:

```typescript
new Agent({
  id: 'no-error-recovery-agent',
  name: 'no-error-recovery-agent',
  model: '__GATEWAY_OPENAI_MODEL_BASE__',
  instructions: '...',
  errorProcessors: [],
})
...
To run only the processors you configure, with none of the defaults merged in, set `errorProcessorDefaults: false`:
