...
A transient signal is still included in the prompt for the current call, so the model sees it near the latest turn. It's not retained, so re-sending it each turn keeps a single fresh copy in context instead of an accumulating history, and it never shows up in stored thread history. Because nothing is written, it also keeps a stable prompt cache prefix across turns.
