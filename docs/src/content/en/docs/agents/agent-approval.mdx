...
```typescript
// In the request handler that receives the user's approval decision
...
if (runs[0]) {
  console.log('Pending tool call:', runs[0].toolCalls[0]?.toolName)
...
Each returned run includes the suspended tool calls (`toolCallId`, `toolName`, `args`, and whether it's waiting on an approval or on `resumeStream()` data), so you can rebuild an approval UI without keeping any state in memory.
...
Suspended runs only survive restarts when your Mastra instance is configured with a persistent [storage provider](/docs/memory/storage). The default in-memory store loses snapshots when the process exits.
