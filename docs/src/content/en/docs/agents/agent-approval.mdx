...
There are two types of tool call approval. The first uses `requireToolApproval` and prevents an agent from calling a tool until `resumeStream` is invoked. The second uses `suspend` and lets the agent provide context or confirmation prompts so the user can decide whether the tool call should continue.

### Tool approval using `requireToolApproval`

In this approach `requireApproval` is moved from the agent definition to the tool.
...
With this approach neither the agent or tool use `requireApproval`. Instead, the tool implementation calls `suspend` to pause execution and return context or confirmation prompts to the user.
