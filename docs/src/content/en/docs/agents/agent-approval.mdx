...
When a tool call requires approval, execution pauses _before_ the tool runs. The agent still decides to call the tool and provides arguments but the actual `execute` function doesn't run until the call is explicitly approved.

There are three ways to trigger this pause:

| Level | Mechanism | Scope |
| - | - | - |
| Agent-level | `requireToolApproval: true` on `stream()` / `generate()` | Pauses **every** tool call |
| Tool-level | `requireApproval: true` on `createTool()` | Pauses calls to **that specific tool** |
| Manual | `suspend()` inside the tool's `execute` function | Pauses at a specific point **during** execution |

Agent-level and tool-level checks are combined with OR logic. If _either_ flag is `true`, the tool call pauses.
...
When `requireApproval` is `true`, the stream includes chunks of type `tool-call-approval` to indicate that the call is paused. Call `resumeStream` with the `resumeSchema` data and `runId` to continue:
