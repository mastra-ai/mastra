...
When using the supervisor pattern (multi-agent coordination via `.stream()` or `.generate()`), tool approvals properly propagate through the delegation chain. This means when a supervisor agent delegates to a sub-agent that uses a tool requiring approval, the approval request surfaces to the supervisor level where it can be handled.

### How it works

1. Supervisor delegates to sub-agent
2. Sub-agent calls a tool with `requireApproval: true` or `suspend()`
3. Approval request bubbles up through delegation chain
4. You handle the approval at the supervisor level
5. Approval/decline propagates back down to the sub-agent
6. Sub-agent continues or terminates based on approval decision
...
You can also decline tool calls at the supervisor level:
...
The `suspend()` approach also works through the supervisor delegation chain:
...
// When using this tool through a sub-agent in supervisor pattern
for await (const chunk of stream.fullStream) {
  if (chunk.type === 'tool-call-suspended') {
...
Tool approvals propagate through multiple levels of delegation:

```
User Request
  → Supervisor Agent
    → Sub-Agent A
      → Sub-Agent B (has tool with requireApproval: true)
        → Tool requires approval
      ← Approval bubbles up to supervisor level
    ← You handle approval/decline at supervisor
  ← Result flows back through delegation chain
```

The approval request will surface at the top-level supervisor, regardless of how deep the delegation chain goes.
