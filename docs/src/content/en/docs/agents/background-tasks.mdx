...
export const reviewTool = createTool({
  ...
  execute: async ({ draft }, { suspend, resumeData }) => {
    if (!resumeData) {
...
:::note
Suspending requires `engine: 'workflow'` on the manager (the default). Calling `resume()` with `engine: 'legacy'` throws.
:::

### What happens to the agent loop

When a task suspends mid-`streamUntilIdle()`, the loop treats it as terminal for the current iteration and the response ends. The eventual completion (post-resume) writes its tool result into the message list, so the next user turn picks up where it left off.
