...
When a tool call dispatches as a background task. The agent stream emits `background-task-started` and `background-task-progress`, other `background-task-` chunks are emitted when [`backgroundTaskManager.stream()`](/reference/streaming/background-task-streaming) is used.
