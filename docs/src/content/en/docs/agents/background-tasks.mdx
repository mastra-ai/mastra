...
The override is stripped from the arguments before the tool runs. All fields in `_background` are optional, the model only includes what it wants to override.
...
| `background-task-progress` | Shows number of running background tasks. |
...
Sub-agent invocations are dispatched as tool calls under the hood, so the same background configuration applies. The recommended pattern is to opt each sub-agent in on the supervisor, it's clearer and lets you tune `timeoutMs` per sub-agent in one place:
...
### Inheriting from the sub-agent

If a sub-agent isn't listed under the supervisor's `backgroundTasks.tools` but has background-eligible tools of its own (either via tool-level `backgroundTasks.enabled: true` or its own `backgroundTasks.tools` entry) the framework still dispatches the entire sub-agent invocation as a background task. The supervisor inherits the sub-agent's intent: the sub-agent itself becomes the background task, and its inner tools run in the foreground inside the sub-agent's loop.

The background config used for the inherited dispatch (for example `waitTimeoutMs`) is derived from the sub-agent's own `backgroundTasks` config.
...
When this `researchAgent` is delegated to from a supervisor that has no `backgroundTasks.tools` entry for it, the supervisor still dispatches the whole `researchAgent` invocation as a background task, and `deepResearchTool` runs in the foreground inside that invocation, instead of dispatching its own nested background task.

Use this pattern when you want a sub-agent to behave consistently in the background regardless of which supervisor invokes it. Use the supervisor-side opt-in (above) when you want to tune background behavior centrally per supervisor.
