...
## What appears in background task stream

When a tool call dispatches as a background task, the background task stream emits a sequence of chunks:
...
The full payload shapes are documented in the [stream chunk reference](/reference/streaming/ChunkType).

When `agent.streamUntilIdle()` is used, these background task chunks are also included in the stream response.
