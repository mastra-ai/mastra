...
### Controlling channel output

When a heartbeat runs on a [channel-backed thread](/docs/agents/channels), its output posts back to the channel. Use `ifIdle.streamOptions.requestContext` with the `channel.render` key to suppress that post or change how it renders for the heartbeat's runs only. Pair it with `ifActive: 'discard'` so the override applies deterministically on the wake path.
...
await mastra.heartbeats.create({
  ...
  ifIdle: {
    ...
    streamOptions: {
      ...
      // Or `{ 'channel.render': false }` to stay silent
    },
  },
})
```

See [Per-run render overrides](/docs/agents/channels#per-run-render-overrides) for the full list of options.
