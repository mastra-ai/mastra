...
Threaded heartbeats accept extra fields that control how the signal behaves:

- `signalType`: the [signal type](/docs/agents/signals) to send, for example `notification` or `system-reminder`.
- `ifActive`: action when the thread is already streaming. One of `deliver`, `persist`, or `discard`.
- `ifIdle`: action when the thread is idle. One of `wake`, `persist`, or `discard`.
- `idleThresholdMs`: treat the thread as idle only after this many milliseconds of inactivity. Below the threshold, the run is skipped.

These fields require a `threadId`. Passing them on a threadless heartbeat throws.

### Controlling the run's stream

The `broadcast` option controls what happens to the stream chunks produced by a heartbeat run. It applies to both threaded and threadless heartbeats and defaults to `live`.

- `live` (default): pass every chunk through as the run produces it.
- `on-complete`: drop intermediate chunks and replay the full text once the run finishes.
- `never`: drop every chunk. The run still happens server-side; nothing is streamed.

```typescript
await mastra.heartbeats.create({
  agentId: 'pinger',
  cron: '0 * * * *',
  prompt: 'Quiet background check.',
  broadcast: 'never',
})
```
