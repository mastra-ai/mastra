...
Agent networks coordinate multiple agents, tools, and workflows to handle complex tasks that aren't clearly defined upfront. They allow decisions to be made at runtime about which primitives to call, in what order, and with what data, based on the user's message or context.
