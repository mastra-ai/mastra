...
:::warning[Deprecated — Use the supervisor pattern]

Agent networks are deprecated and will be removed in a future major release. The [supervisor pattern](./supervisor-agents) using `agent.stream()` or `agent.generate()` is now the recommended approach. It provides the same multi-agent coordination with better control, a simpler API, and easier debugging.
