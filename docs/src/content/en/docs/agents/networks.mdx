...
Agent networks coordinate multiple agents, tools, and workflows to handle complex tasks that aren't clearly defined upfront. They allow decisions to be made at runtime about which primitives to call, in what order, and with what data, based on the user message or context.
...
- Memory is required when using `.network()`. It stores task history and is used to decide which primitives to call.
...
```typescript {22-23,26,29} filename="src/mastra/agents/network-agent.ts" showLineNumbers copy
