...
Networks coordinate multiple agents, workflows, and tools to handle tasks that aren't clearly defined upfront but can be inferred from the user's message or context. A top-level **routing agent** uses an LLM to interpret the request and decide which primitives to call, in what order, and with what data.
...
An agent network is built around a top-level routing agent that delegates tasks to agents, workflows, and tools defined in its configuration. Memory is set using the `memory` option, and `instructions` define its role and behavior.
...
The routing agent uses each primitive’s description and schema to decide what it does and how to use it. Clear descriptions and well-defined input and output schemas improve routing and primitive selection.

#### Agents

Each agent should include a clear `description` that explains what the agent does.
...
Workflows should include a `description` to explain their purpose, along with `inputSchema` and `outputSchema` to describe the expected data.
...
Tools should include a `description` to explain their purpose, along with `inputSchema` and `outputSchema` to describe the expected data.
...
Call an agent network using `.network()` and pass a user message as input to stream the result.
...
```typescript showLineNumbers copy
const result = await agent.network("What's the weather in London?");
