...
Mastra Server exposes A2A routes for registered agents automatically. You don't need to create a separate remote-agent wrapper. Use the agent's registered ID in the agent card URL and execution URL.
...
1. Mastra exposes each registered agent through an agent card at `/.well-known/:agentId/agent-card.json` and an execution endpoint at `/a2a/:agentId`.
...
Use `MastraClient.getA2A()` when an application needs direct programmatic access to a remote Mastra agent over A2A:
...
Use verification to enforce trusted keys, expected providers, expected endpoints, certificate-bound identities, signed cards, or other trust requirements before a client calls the remote agent.
