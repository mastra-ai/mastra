...
Visit [.generate()](/reference/agents/generate#structuredoutput) for a full list of configuration options.
...
Structured output support varies across LLM's due to the model itself. There are various workarounds provided to be able to use the full range of features supported for structured output, mainly returning structured output, tool calling, and the combination of the two.
