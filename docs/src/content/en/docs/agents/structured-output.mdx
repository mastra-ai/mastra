...
By default the injected instruction embeds the full JSON schema. For large schemas, pass `instructions` to inject your own description of the expected output instead; the response is still validated against `schema`.
...
const response = await testAgent.generate('Tell me about TypeScript.', {
  structuredOutput: {
    ...
    instructions:
      'Reply with a JSON object: "summary" (one paragraph) and "keyFeatures" (3-5 short strings). No markdown.',
  },
})
```
