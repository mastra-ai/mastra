...
### Retry the structuring step

When a separate structuring `model` is configured and its output fails validation under the `strict` strategy, the structuring processor asks the agent loop to retry the step. Set `maxProcessorRetries` on the agent or the call to allow the retry; the model is told why the previous response was not accepted, and the structuring pass runs again on the new response. When the budget is exhausted, or when `maxProcessorRetries` is not set, the run ends with a tripwire as before.
