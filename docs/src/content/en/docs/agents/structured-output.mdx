...
When schema validation fails, you can control how errors are handled using `errorStrategy`. The default `strict` strategy throws an error, while `warn` logs a warning and continues. The `fallback` strategy returns the values provided using `fallbackValue`; the result then reports `usedFallbackValue: true` so you can tell a substituted object from a real answer.
