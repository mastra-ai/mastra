...
Add a `ResponseCache` to the agent's `inputProcessors`. You bring the cache backend (any `MastraCache` implementation) — for development, the helper below adapts the `MastraServerCache` registered on a `Mastra` instance:
...
`ResponseCache` requires a `MastraCache`. Implement the interface for any backend (Redis, DynamoDB, etc.):
...
To wire an existing `MastraServerCache` (e.g. the `RedisCache` from `@mastra/redis`), use `createMastraCacheFromServerCache`:

```typescript
import { createMastraCacheFromServerCache } from '@mastra/core/cache'
import { RedisCache } from '@mastra/redis'

const cache = createMastraCacheFromServerCache(new RedisCache({ url: process.env.REDIS_URL }))
```
