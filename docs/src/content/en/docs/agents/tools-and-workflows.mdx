---
title: "Tools and Workflows | Agents | Mastra Docs"
description: Learn how to extend agents with tools and workflows to perform specialized tasks.
---

# Tools and Workflows

Agents use tools and workflows to extend their capabilities beyond text generation. Tools let agents call APIs, query databases, or run custom logic with structured inputs and outputs. Workflows allow agents to follow defined sequences of steps or decision trees. Together, these features enable agents to perform complex tasks, interact with systems, and produce reliable, goal-directed outcomes.

## OLD ---

# Capabilities (OLD)

Agents can do more than generate text. You can extend them with tools, workflows, memory, guardrails, and context-aware configuration to handle more complex tasks. These capabilities let agents interact with APIs and databases, follow multi-step procedures, store conversation state, enforce custom policies, and adapt their behavior based on request-specific values.
...
export const weatherTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    return { temperature: `${temp_C}℃`, conditions: weatherDesc[0].value };
  }
});
```
...
```typescript filename="src/mastra/agents/weather-agent.ts" showLineNumbers copy
...
export const weatherAgent = new Agent({
  ...
  instructions: `
      ...
      Use the weatherTool to fetch weather for a location data.
`,
