...
Agents use tools to call APIs, query databases, run custom functions from your codebase, or load additional tools from remote MCP servers. Tools give agents capabilities beyond LLM text generation and offer a structured, deterministic way to access and work with data.
...
To make a tool available to an agent, add it to the `tools` parameter and reference it by name in the agent’s instructions.
...
This example shows how to create a tool that fetches weather data from an API. Because the tool is defined in the agent’s `tools` configuration, the agent can provide the required `city` input via the tool’s `context` argument. The tool then returns a structured response with `temperature` and `conditions`.
