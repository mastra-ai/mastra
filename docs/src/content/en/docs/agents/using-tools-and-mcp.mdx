...
Similarly, Mastra `Workflow` instances can also be exposed in the same way.
...
- It is automatically converted into a callable tool.
- The tool is named `run_<workflowKey>`.
- The workflow's `description` is used for the tool's description.
- The `workflow.inputSchema` is used as the tool's input schema.
- Executing the tool calls `workflow.createRun().start({ inputData: <tool_input> })`.

**Example `MCPServer` Configuration with an Agent and Workflow:**
...
import { dataPipeline } from "../workflows/dataPipeline"; // Assuming you have a workflow defined

const server = new MCPServer({
  name: "My Custom Server with Agent and Workflow Tools",
  version: "1.0.0",
  tools: {
    weatherInfo,
  },
  agents: { generalHelper }, // Exposes 'ask_generalHelper' tool
  workflows: { dataPipeline }, // Exposes 'run_dataPipeline' tool
});
```

For an agent to be successfully converted into a tool by `MCPServer`, its `description` property must be set to a non-empty string in its constructor configuration. The same requirement applies to `Workflow` instances; their `description` must also be a non-empty string. If the description is missing or empty for either an agent or a workflow, `MCPServer` will throw an error during initialization. Workflows use their `inputSchema` for the tool's input details.
