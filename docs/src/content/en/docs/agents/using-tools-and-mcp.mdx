...
Mastra supports three patterns for providing tools to agents:
- **Direct assignment**: Static tools available at initialization
- **Function-based**: Dynamic tools resolved based on runtime context  
- **Promise-based (lazy loading)**: Asynchronous tool resolution to prevent circular dependencies and race conditions to enable MCP server integration
...
When creating agents that will consume an MCP server in the same repo they need to connect to, always use promise-based (lazy) tools to prevent race conditions.
...
const myAgent = new Agent({
  ...
  model: openai("gpt-4o-mini"),
  tools: async () => {
    // Tools resolve lazily when needed, not during initialization
