...
To use the new protocol, you can use the `streamVNext` method on an agent. This method will return a custom MatraAgentStream. This stream extends a ReadableStream, so all basic stream methods are available.
