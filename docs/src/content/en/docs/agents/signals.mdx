...
When Mastra accepts a state signal, it stores compact tracking metadata on the thread. The metadata records the lane's current `cacheKey`, version, last signal id, and last snapshot signal id. If a producer sends the same `cacheKey` again while that state is still current, Mastra skips the duplicate.
