...
- `cacheKey`: A producer-owned key for the current state version
...
Mastra skips a state signal when the same state lane already has an active copy with the same `cacheKey`. When the `cacheKey` changes, Mastra increments that lane's version and stores compact tracking metadata on the thread.
...
Processors can also send state from `processInputStep()` with the same state shape.

```typescript title="src/mastra/processors/browser-state.ts"
const browserStateProcessor = {
  id: 'browser-state',
  async processInputStep({ messageList, sendStateSignal }) {
    const browser = readCurrentBrowserState()

    await sendStateSignal?.({
      id: 'browser',
      mode: 'delta',
      cacheKey: `browser:${browser.activeUrl}:${browser.tabCount}`,
      contents: `Browser changed to ${browser.activeUrl}.`,
      delta: {
...
Processors that implement `computeStateSignal()` use the same tracking behavior. If a processor omits `id`, Mastra uses the processor id as the state lane id. Mastra passes `lastSnapshot` and `deltasSinceSnapshot` into `computeStateSignal()`, resolving them from message history when the current message list does not contain the latest snapshot. The processor still owns any merge or diff logic.
...
If you call Mastra over HTTP directly, use `POST /api/agents/:agentId/send-message` for immediate messages and `POST /api/agents/:agentId/queue-message` for next-turn messages. See [Server routes reference](/reference/server/routes#agent-message-routes) for request and response schemas.
