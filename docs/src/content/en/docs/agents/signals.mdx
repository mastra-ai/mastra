...
Use `reconnect: true` for long-lived subscriptions. The client resubscribes when the stream closes, such as after a proxy idle timeout or a dropped network connection.
