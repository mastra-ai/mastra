...
Use `deliveryAttributes` to tag a signal with context that depends on whether the agent is active or idle at delivery time. Mastra resolves the correct branch when the signal is accepted.
...
```xml
<user-message delivery="while-active">Also cover the edge cases.</user-message>
```

When the agent is idle:

```xml
<user-message delivery="message">Also cover the edge cases.</user-message>
```

The `ifActive` branch merges into `attributes` when the signal is delivered to a running agent loop. The `ifIdle` branch merges when the signal wakes an idle thread. If the selected branch is `undefined`, no extra attributes are added.

Delivery attributes work with any signal type and can carry any key-value pairs — the `delivery` attribute shown above is just a convention used by mastracode.

:::note
Visit [Agent.sendSignal() reference](/reference/agents/agent#sendsignalsignal-options) for the full `AgentSignalInput` type.
:::
