...
Use `/queue-message` when the active run should finish before the new message starts a follow-up run:
...
The lower-level `/agents/:agentId/signals` route remains available for system-generated signals and older clients.
