...
Memory requires a storage provider to persist message history, including user messages and agent responses. For more details on available providers and how storage works in Mastra, see the [Storage](../memory/storage) documentation.
...
Use [RequestContext](../server/request-context) to access request-specific values. This lets you conditionally select different memory or storage configurations based on the context of the request.
...
See [Request Context](../server/request-context) docs for more information.
