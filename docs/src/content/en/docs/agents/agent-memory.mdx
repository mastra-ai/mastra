...
Mastra agents can be configured to store conversation history, with optional [working memory](./working-memory) to maintain recent context or [semantic recall](./semantic-recall) to retrieve past messages based on meaning.
