...
:::warning
Thread IDs must be globally unique. Once a thread is created with a specific resource, that pairing is permanent. Reusing a thread ID with a different resource ID will cause an error.
:::
