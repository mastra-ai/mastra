...
While input processors handle user messages before they reach the language model, **output processors** handle the AI's responses after generation but before they're returned to the user. This is useful for response validation, content filtering, and safety controls on AI-generated content.

See the [Output Processors documentation](/docs/agents/output-processors) for details on processing AI responses. 