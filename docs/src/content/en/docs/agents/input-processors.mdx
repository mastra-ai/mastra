...
Use a loop with `Array.prototype.splice()` to remove messages from the array.
...
const messageFilter: InputProcessor = {
  ...
  process: async ({ messages }) => {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      for (const part of messages.content.parts) {
...
1.  **Sequential Execution**: Processors run in the order they're defined in the `inputProcessors` array.
2.  **Input/Output Chain**: The `messages` array returned by one processor becomes the input for the next.
3.  **Async Support**: Each processor can perform async operations before returning the messages.
4.  **Early Termination**: Any processor can call `abort()` to stop the entire pipeline.
