...
To scaffold a new project with the Agent Builder pre-configured, use [`create-agentbuilder`](/reference/cli/create-agentbuilder):

```bash npm2yarn
npm create agentbuilder@latest
```

To add the Agent Builder to an existing project, install `@mastra/editor` alongside a storage adapter:
