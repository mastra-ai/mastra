...
- **Model credentials** (`OPENAI_API_KEY`): OpenAI powers both the builder agent and observational memory by default, so a single OpenAI key is enough to get started. `createBuilderAgent()` uses an OpenAI model — pass a `model` override to `createBuilderAgent({ model })` to use a different provider (see the [`createBuilderAgent()` reference](/reference/editor/agent-builder/create-builder-agent)). Builder-created agents also enable observational memory by default, using `__GATEWAY_OPENAI_MODEL_MINI__`; to change it, see [Observational memory model](/docs/agent-builder/memory#observational-memory-model).
