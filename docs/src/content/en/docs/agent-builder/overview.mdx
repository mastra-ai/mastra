...
- **Model provider credentials**: The Builder agent defaults to `openai/gpt-5.5` when no `model` is passed to `createBuilderAgent()`. Pass a different model string or config object to use another provider. See [Configuring the Builder Agent model](#configuring-the-builder-agent-model) for examples.

## Configuring the Builder Agent model

The `createBuilderAgent()` factory accepts a partial agent config. Pass `model` to use a provider other than the default.

Use a model string for any provider in the Mastra model router:
...
Use a config object to point at a custom endpoint or gateway:
...
Custom gateways registered on the `Mastra` instance also work. The model router resolves the gateway automatically:
...
export const mastra = new Mastra({
  gateways: {
    myGateway: new MyGateway(),
  },
  agents: {
    builderAgent: createBuilderAgent({
      model: 'my-gateway/my-provider/my-model',
    }),
  },
})
```

See the [custom gateways](/models/gateways/custom-gateways) page for how to implement `MastraModelGateway`.
