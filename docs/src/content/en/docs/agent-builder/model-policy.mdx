...
Pin a non-OpenAI model as the default for every new Builder-created agent:
...
new MastraEditor({
  builder: {
    ...
    configuration: {
      agent: {
        models: {
          default: { provider: 'anthropic', modelId: 'claude-sonnet-4-6' },
          allowed: [
...
End users see only the two Anthropic models in the picker, with `claude-sonnet-4-6` pre-selected.

## Custom gateways

Use `kind: 'custom'` entries to allow models that are not in the built-in provider registry. This applies to self-hosted deployments, private API gateways, and any provider the registry does not recognize.
...
Register the matching custom gateway on the `Mastra` instance so the model router can resolve these IDs at runtime:
