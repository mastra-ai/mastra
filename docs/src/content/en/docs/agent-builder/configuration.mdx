...
This hides the skills section in the Builder UI and pins observational memory as the default for every Builder-created agent.
...
The Builder is enabled by default once you add a `builder` config. Set `enabled: false` to keep the config in place but turn the surface off:
