...
The `createBuilderAgent()` factory creates the chat agent that powers the Builder's create-via-conversation flow. It defaults to `openai/gpt-5.5`, but accepts any model supported by the Mastra model router.
...
Pass a config object for custom endpoints:
...
This model controls the Builder Agent itself. To control which models end users can select for the agents they create, use the [model policy](/docs/agent-builder/model-policy).

## Related

- [Model policy](/docs/agent-builder/model-policy) — pin the allowed models and default model.
- [Custom gateways](/models/gateways/custom-gateways) — implement a `MastraModelGateway` for private providers.
