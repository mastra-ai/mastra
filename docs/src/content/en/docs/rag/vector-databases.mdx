...
### Complete Example: Document Management Workflow

Here's a complete example showing how to handle document deletion in a RAG app:

```ts title="document-management.ts" showLineNumbers copy
import { PgVector } from "@mastra/pg";
...
// Index a document with docId in metadata
async function indexDocument(docId: string, content: string) {
  ...
  await store.upsert({
    ...
    metadata: chunks.map((chunk) => ({
      text: chunk.text,
      docId: docId, // Store docId for easy cleanup
    })),
  });
}
...
// Usage
await indexDocument("doc-123", "Your document content here...");
// Later, when the document is deleted:
await deleteDocument("doc-123");
```

> **Note**: The `deleteVectors` method with filters is supported by all major vector stores (MongoDB, PgVector, Pinecone, Qdrant, Chroma, etc.). The filter syntax matches the same syntax used for querying vectors, so you can use operators like `$in`, `$and`, `$or`, and comparison operators.
