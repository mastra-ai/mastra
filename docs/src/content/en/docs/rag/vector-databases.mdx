...
<Tabs items={['Pg Vector', 'Milvus', 'Pinecone', 'Qdrant', 'Chroma', 'Astra', 'LibSQL', 'Upstash', 'Cloudflare', 'MongoDB', 'OpenSearch']}>
  <Tabs.Tab>
...
const store = new MilvusStore({
  address: 'localhost:19530',  // Default
  username: process.env.MILVUS_USERNAME, // Optional
  password: process.env.MILVUS_PASSWORD  // Optional
})
await store.createIndex({
  indexName: "myCollection",
  dimension: 1536,
...
### Using Milvus for Vector Search

Milvus is a high-performance, open-source vector database designed for AI applications and similarity search. It offers excellent scalability, high throughput, and support for advanced filtering.

For detailed deployment options and best practices, see the [official Milvus documentation](https://milvus.io/docs).
</Tabs.Tab>
<Tabs.Tab>
```ts filename="vector-store.ts" showLineNumbers copy
