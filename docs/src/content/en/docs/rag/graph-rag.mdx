...
Graph RAG is particularly effective when:
...
Graph RAG combines vector similarity with knowledge graph traversal:
...
import { createGraphQueryTool } from "@mastra/rag";
...
const strictGraphTool = createGraphQueryTool({
  vectorStoreName: "pgVector",
  indexName: "embeddings",
  model: new ModelRouterEmbeddingModel("openai/text-embedding-3-small"),
  graphOptions: {
...
const broadGraphTool = createGraphQueryTool({
  vectorStoreName: "pgVector",
  indexName: "embeddings",
  model: new ModelRouterEmbeddingModel("openai/text-embedding-3-small"),
  graphOptions: {
...
Graph RAG can be used alongside other retrieval approaches:
...
- [createGraphQueryTool()](/reference/v1/tools/graph-rag-tool)
