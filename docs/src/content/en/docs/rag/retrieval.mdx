...
import { COUCHBASE_SEARCH_STORE_PROMPT, COUCHBASE_QUERY_STORE_PROMPT } from "@mastra/couchbase";
...
export const ragAgent = new Agent({
  ...
  instructions: `
  Process queries using the provided context. Structure responses to be concise and relevant.
  {/* Choose the prompt that matches your vector store */}
  {/* Note: CouchbaseQueryStore requires at least one vector to be upserted before index creation. */}
  ${COUCHBASE_SEARCH_STORE_PROMPT}
  ${COUCHBASE_QUERY_STORE_PROMPT}
  `,
  tools: { vectorQueryTool },
});
```
