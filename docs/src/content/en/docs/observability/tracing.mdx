...
2. Create an instrumentation file in your project (e.g., `instrumentation.ts`) and set the global variable:
...
```bash
bun run --hot --require ./instrumentation.ts src/index.ts
```
