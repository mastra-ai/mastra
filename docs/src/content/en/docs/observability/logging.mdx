...
When a log call happens inside a traced operation (an agent run, workflow step, or tool call), Mastra injects `trace_id` and `span_id` into the logger's native output. The fields use W3C trace context format and match the trace shown in Studio, so you can correlate a stdout log line with its trace directly. With `PinoLogger`, the record looks like this:
