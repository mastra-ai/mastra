...
Serverless environments have ephemeral filesystems. Use external storage like PostgreSQL instead of local file storage (`file:./mastra.db`). See the [Vercel deployment guide](/guides/deployment/vercel-deployer#observability) for a complete setup example with PostgreSQL and observability.
:::
