...
Span filtering lets you reduce noise and per-span costs before data reaches your observability platform. Configure it per observability instance, or per exporter when only one destination should skip certain span types.
...
Every exporter also accepts `excludeSpanTypes`. Spans of those types are dropped by that exporter only, so other exporters on the same instance still receive them. This is useful when one platform bills per span and another (for example Mastra Studio) needs the full detail:
  ...
  observability: new Observability({
    configs: {
      default: {
        serviceName: 'my-app',
        exporters: [
          // Studio keeps every span, including streamed chunks
          new MastraStorageExporter(),
          // Langfuse skips per-chunk spans
          new LangfuseExporter({ excludeSpanTypes: [SpanType.MODEL_CHUNK] }),
        ],
      },
    },
  }),
...
Exporter-level exclusion runs after the instance-level filtering above and before the exporter's `customSpanFormatter`.
