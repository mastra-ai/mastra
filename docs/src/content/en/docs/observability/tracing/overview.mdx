...
Processors mutate the span they receive and return the same instance, or return `undefined` to drop the span. Returning a copy such as `{ ...span, input: redacted }` does not work: only the live span can be exported, so the copy is dropped and a processor error is logged.
