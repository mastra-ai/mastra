...
Set the top-level `environment` field on Mastra to automatically attach the deployment environment to every trace, log, and metric without passing `tracingOptions.metadata.environment` on each call.
