...
For more fine-grained control over sensitive data, consider using the [Sensitive Data Filter](/ecosystem/observability-integrations/processors/sensitive-data-filter) processor, which can redact specific fields (like passwords, tokens, and keys) while preserving the rest of the input/output.
...
- [Sensitive Data Filter](/ecosystem/observability-integrations/processors/sensitive-data-filter) redacts sensitive information. It's enabled in the default observability config.
