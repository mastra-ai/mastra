...
For a simpler setup with optimized defaults, use `createArizeConfig`:
...
- **Built-in workflow filter** - Automatically filters out workflow loop/parallel/conditional spans
...
For a simpler setup with optimized defaults:
...
The `createArizeConfig` helper works with both Phoenix and Arize AX. You can customize serialization options and span processors regardless of which platform you're using.

### Customizing Serialization Options

By default, `createArizeConfig` uses optimized serialization options that prevent truncation of outputs and large payloads. You can customize these options by providing `serializationOptions`:
