...
Different storage providers support different tracing strategies. Not all storage providers support observability—only those designed to handle high-volume telemetry data include the observability domain.
...
| ----------------------------------------------- | ------------------ | ----------------------------------------- | ------------------------------------- |
| **ClickHouse** (`@mastra/clickhouse`)           | insert-only        | batch-with-updates, insert-only           | Production (high-volume)              |
| **[PostgreSQL](/reference/storage/postgresql)** | batch-with-updates | batch-with-updates, insert-only           | Production (moderate volume)          |
| **[MSSQL](/reference/storage/mssql)**           | batch-with-updates | batch-with-updates, insert-only           | Production (moderate volume)          |
| **[MongoDB](/reference/storage/mongodb)**       | batch-with-updates | batch-with-updates, insert-only           | Production (moderate volume)          |
...
Observability data grows quickly in production environments. A single agent interaction can generate dozens of spans, and high-traffic applications can produce millions of traces per day. Most general-purpose databases aren't optimized for this write-heavy, append-only workload.

### Recommended: ClickHouse for High-Volume Production

[ClickHouse](/reference/storage/composite#specialized-storage-for-observability-recommended-for-production) is a columnar database designed for high-volume analytics workloads. It's the recommended choice for production observability because:
...
If you're using a provider without observability support (like Convex or DynamoDB) or want to optimize performance, use [composite storage](/reference/storage/composite#specialized-storage-for-observability-recommended-for-production) to route observability data to ClickHouse while keeping other data in your primary database.
