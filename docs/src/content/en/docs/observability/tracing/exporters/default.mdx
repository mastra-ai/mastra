...
`DefaultExporter` emits structured drop telemetry when it cannot persist observability data. Register an exporter or bridge with `onDroppedEvent` to forward these drops to alerting or metrics.

Drop events use two reasons:

- `unsupported-storage`: The storage provider does not implement the signal type.
- `retry-exhausted`: The exporter retried a batch until `maxRetries` and then dropped it.

The following example demonstrates forwarding drop counts to a metrics counter:
