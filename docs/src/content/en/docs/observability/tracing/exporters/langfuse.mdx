...
Add a `langfuse.prompt` object to your trace metadata with `name` and `version`:
...
For dynamic prompt management, fetch prompts directly from Langfuse:

```typescript title="src/agents/dynamic-agent.ts"
import { Agent } from "@mastra/core/agent";
import { openai } from "@ai-sdk/openai";
      ...
      metadata: {
        langfuse: {
          prompt: {
            ...
            id: prompt.id, // Optional: include prompt ID for direct linking
          },
        },
      },
    },
...
The `langfuse.prompt` object supports these fields:
