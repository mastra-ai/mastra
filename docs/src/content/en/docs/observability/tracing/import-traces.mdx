---
title: "Import traces | Observability"
description: "Import historical traces from supported observability providers into Mastra Platform with dry runs, resume, and read-back verification."
packages:
  - "mastra"
---

# Import traces into Mastra Platform
...
For a headless environment, set the API token and organization ID. Set the project ID or pass `--project`:
...
Langfuse Cloud defaults to `https://cloud.langfuse.com`. Set `LANGFUSE_BASE_URL` when the project uses another Langfuse Cloud region or a self-hosted instance:

```bash title=".env"
LANGFUSE_BASE_URL=https://your-langfuse-host.example.com
```
