...
- **Normal operation**: ~2-10MB (1000 spans)
- **Buffer full**: ~20-100MB (10000 spans)
- **Very reasonable for production systems**

### Throughput Benefits

- **Batch strategies**: 10-100x improvement vs realtime
- **Insert-only**: Additional 70% reduction in DB operations
- **Network efficiency**: Fewer round trips, amortized costs

### Storage Requirements

- **Disk Space**: Plan for ~1-2KB per span on average
- **Write throughput**: Batch strategies significantly reduce write operations
- **Query performance**: Insert-only strategy optimizes for analytical queries
