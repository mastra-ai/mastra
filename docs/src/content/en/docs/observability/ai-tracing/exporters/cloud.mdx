...
- Sensitive data filtering is applied before transmission

### Data Retention

- Traces are retained for 30 days (free tier)
- Extended retention available with paid plans
...
<Callout type="warning">
  **Common Issues**:
  - No traces appearing? Check your access token is valid
  - Rate limiting? CloudExporter handles this automatically with retries
  - Network issues? Traces are buffered locally and retried
</Callout>
...
```typescript
console.log('Token set:', !!process.env.MASTRA_CLOUD_ACCESS_TOKEN);
console.log('Token prefix:', process.env.MASTRA_CLOUD_ACCESS_TOKEN?.substring(0, 4));
```
