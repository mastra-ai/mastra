...
Mastra exposes four metric analytics queries (`getMetricAggregate`, `getMetricBreakdown`, `getMetricTimeSeries`, and `getMetricPercentiles`) and metric discovery helpers through three surfaces: an in-process store accessor and the runtime HTTP API, plus the `mastra api metric` CLI. All three accept the same Zod-validated input shapes, so you can move from a one-off CLI investigation to a programmatic dashboard tool without re-learning the API.

<Inject>
  **For AI agents:** Run `npx mastra api metric aggregate '{"name":"mastra_agent_duration_ms","aggregation":"avg"}'` to query average agent latency directly instead of writing a temporary script. Querying local metrics requires a running Mastra server whose observability store supports metric queries; start the local server with `npx mastra dev`, or pass the reachable server's base URL with `--url`. Run `npx mastra api metric aggregate --schema` before constructing a different query. Install Mastra's skill with `npx skills add mastra-ai/skills --skill mastra` for complete API CLI discovery, targeting, schema, authentication, and error-handling guidance.
</Inject>
...
:::note
Metric queries are served by the observability domain. DuckDB, ClickHouse, and Postgres v-next support these queries. Spanner also supports them when its `disableMetrics` option is set to `false`; metrics are disabled by default because Spanner is intended only for lower-volume metrics workloads. See [Metrics overview](/docs/observability/metrics/overview) for setup. If the observability domain isn't configured, `getStore('observability')` returns `undefined`.
:::
...
export const agentLatencyTool = createTool({
  ...
  execute: async (_input, context) => {
    ...
    if (!observability) {
      throw new Error('Observability domain is not configured')
    }
...
`getStore('observability')` returns `undefined` when the observability domain isn't configured. If the domain exists but its backend doesn't implement metric queries, the query method throws a not-implemented error.
...
- `distinctColumn`: Required when `aggregation` is `count_distinct`. Must be an allowlisted categorical column.
...
`filters.timestamp` is optional, but you should treat it as required for any query that runs against a production store. Observability tables are typically partitioned (or chunked, for TimescaleDB) by event time. When you supply `timestamp.start` and, ideally, `timestamp.end`, the backend can limit the scan to data that overlaps the requested range. Without a time range, the query may scan the store's full retention history, which can make analytics queries substantially slower.

A safe default for ad-hoc queries is the last 24 hours; alerts and dashboards should match their actual evaluation window:

```typescript
await observability!.getMetricPercentiles({
  name: 'mastra_agent_duration_ms',
  percentiles: [0.95],
  interval: '1h',
  filters: {
...
This guidance applies to every supported metrics backend, but matters particularly for Postgres v-next because time bounds allow Postgres or TimescaleDB to exclude partitions or chunks outside the requested range.
