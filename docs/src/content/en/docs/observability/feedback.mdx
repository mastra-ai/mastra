...
Feedback created through the `POST /api/observability/feedback` route takes the same path: the server calls `addFeedback()`, so every configured exporter receives it.
