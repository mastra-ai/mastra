...
Use `deleteFeedback()` to hide feedback records from reads by id. This is useful when a comment contains sensitive data. Deletion is idempotent. The optional `organizationId` and `resourceId` fields restrict the delete to a single tenant's records. In Studio, each comment on a trace or span Feedback tab has a delete action.
