...
Use `deleteFeedback()` to remove feedback records by id, for example when a comment contains sensitive data that must be purged. Deletion is idempotent, and the optional `organizationId` and `resourceId` fields restrict the delete to a single tenant's records. In Studio, each comment on a trace or span Feedback tab has a delete action.
...
Deleted records also disappear from feedback analytics. On ClickHouse, rows in the short-lived delta table used for incremental polling aren't deleted; they expire via TTL within two days.
