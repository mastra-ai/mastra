---
title: "Configuration | Observability"
description: "Configure observability for traces, logs, and metrics in Mastra."
packages:
  - "@mastra/observability"
---

# Configuration

Observability is configured once on your Mastra instance and applies across traces, logs, and metrics. Use observability configs to choose exporters, logging behavior, processors, and runtime routing.

## When to use configuration

- Set up a default observability pipeline for local development.
- Route different environments or request types to different observability configs.
- Keep Mastra Studio and Mastra platform access enabled while also sending data to external providers.

## Quickstart

The following example demonstrates a single observability config with local storage, Mastra platform export, and sensitive-data redaction:

```ts title="src/mastra/index.ts"
import { Mastra } from '@mastra/core'
import { LibSQLStore } from '@mastra/libsql'
import {
  Observability,
  MastraStorageExporter,
  MastraPlatformExporter,
  SensitiveDataFilter,
} from '@mastra/observability'

export const mastra = new Mastra({
  storage: new LibSQLStore({
    id: 'mastra-storage',
    url: 'file:./mastra.db',
  }),
  observability: new Observability({
    configs: {
      default: {
        serviceName: 'mastra',
        exporters: [new MastraStorageExporter(), new MastraPlatformExporter()],
        spanOutputProcessors: [new SensitiveDataFilter()],
        logging: {
          enabled: true,
          level: 'info',
        },
      },
    },
  }),
})
```

## Basic config

An observability config usually contains:

- `serviceName`: The service identifier attached to exported observability data.
- `exporters`: One or more destinations for traces, logs, and derived metrics.
- `spanOutputProcessors`: Transformations that run before spans are exported.
- `logging`: Log forwarding settings for observability storage.

For destinations and processors, see [Integrations overview](/docs/observability/integrations/overview).

## Multi-config setup

Use multiple configs when different environments or request types need different exporters or sampling behavior. Select the active config at runtime with `configSelector`.

```ts title="src/mastra/index.ts"
import { Mastra } from '@mastra/core'
import { Observability, MastraStorageExporter } from '@mastra/observability'
import { LangfuseExporter } from '@mastra/langfuse'

const localExporter = new MastraStorageExporter()
const langfuseExporter = new LangfuseExporter()

export const mastra = new Mastra({
  observability: new Observability({
    configs: {
      development: {
        serviceName: 'my-service-dev',
        exporters: [localExporter],
      },
      production: {
        serviceName: 'my-service-prod',
        exporters: [localExporter, langfuseExporter],
      },
    },
    configSelector: () => process.env.NODE_ENV || 'development',
  }),
})
```

Trace sampling is still documented in [Tracing](/docs/observability/tracing/overview), since it controls trace collection rather than shared observability setup.

## Maintaining Studio access

When you add external exporters, keep `MastraStorageExporter` or `MastraPlatformExporter` in the same config if you still want local Studio access or hosted Mastra platform observability.

```ts title="src/mastra/index.ts"
import { Mastra } from '@mastra/core'
import { Observability, MastraStorageExporter, MastraPlatformExporter } from '@mastra/observability'
import { ArizeExporter } from '@mastra/arize'

export const mastra = new Mastra({
  observability: new Observability({
    configs: {
      production: {
        serviceName: 'my-service',
        exporters: [
          new ArizeExporter({
            endpoint: process.env.PHOENIX_COLLECTOR_ENDPOINT,
            apiKey: process.env.PHOENIX_API_KEY,
          }),
          new MastraStorageExporter(),
          new MastraPlatformExporter(),
        ],
      },
    },
  }),
})
```

## Flushing traces in serverless environments

In serverless environments, flush observability exporters before the runtime pauses or exits:

```ts
const observability = mastra.getObservability()
await observability.flush()
```

Use external storage in serverless environments instead of local file storage. For storage selection and routing, see [Storage](/docs/observability/storage).

## Related

- [Observability overview](/docs/observability/overview)
- [Storage](/docs/observability/storage)
- [Tracing](/docs/observability/tracing/overview)
- [Integrations overview](/docs/observability/integrations/overview)
