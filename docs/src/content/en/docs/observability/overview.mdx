...
- [**Configuration**](/docs/observability/config): Configure observability once for traces, logs, metrics, exporters, and processors.
...
- [**Integrations**](/docs/observability/integrations/overview): Choose exporters, bridges, and processors for local, hosted, or external observability workflows.
...
For setup details, see [Configuration](/docs/observability/config), [Storage](/docs/observability/storage), and [Integrations overview](/docs/observability/integrations/overview).
...
Not all storage backends support every signal. Traces and logs work with observability-capable backends, but metrics require an OLAP-capable store like DuckDB (development) or ClickHouse (production). For setup guidance, see [Storage](/docs/observability/storage).
