...
Studio includes three observability views:
...
Metrics require DuckDB or in-memory observability storage. PostgreSQL, LibSQL, and other relational databases don't support metrics collection. If you use in-memory storage, metrics reset on every server restart.
...
For detailed instructions, follow the [observability instructions](/docs/observability/overview). To get up and running quickly, add the `@mastra/observability` package to your project and configure it with LibSQL and DuckDB for a local development setup that supports both traces and metrics.
