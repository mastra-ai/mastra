...
description: "Monitor Mastra agents, workflows, tools, and model calls in Studio with metrics dashboards, hierarchical traces, and searchable correlated logs."
packages:
  - "@mastra/core"
...
Above the trace list, open **Columns** to show or hide type, input, status, duration, environment, end time, input token, output token, total token, estimated cost, and custom metadata columns. To display custom metadata, pick a top-level key from `tracingOptions.metadata`. The picker suggests keys observed on traces in the selected time range, and you can type a key that has not been seen yet. You can also add a custom column for any trace property, such as thread ID or resource ID. Each custom column header opens a menu that lists the values currently loaded, and choosing one adds an **is** filter for that property. Studio saves the selection in browser storage for the current Mastra server and API prefix.

Filter operators use plain language, such as **is any of**, **greater than**, and **at most**. For properties the API can enumerate, such as environment or entity name, the **is** operator offers a dropdown of known values; ID fields accept free text.
