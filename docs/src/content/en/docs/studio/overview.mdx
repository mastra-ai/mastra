...
A dashboard summarizing agent, workflow, and tool performance over a configurable time range. KPI cards show total agent runs, model cost, token consumption, and average scorer performance. Detailed cards break down model usage, token usage per agent, trace volume, latency percentiles, and scorer trends.

### Traces

Inspect individual requests as hierarchical timelines. Search by trace ID, name, or entity, filter by status, entity type, or tags, and drill into any span to see inputs, outputs, token usage, and timing.

### Logs

Browse internal logs forwarded to your observability storage. Search across message content, entity names, and trace IDs, filter by level, entity type, or entity name, and navigate from a log entry directly to its correlated trace and span.

Visit the [Studio observability](/docs/studio/observability) docs for setup and configuration details.
