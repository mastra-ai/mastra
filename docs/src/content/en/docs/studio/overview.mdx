...
Queued messages wait in server memory, so a server restart loses messages that haven't started. Queue labels are local to the current chat session; other tabs can observe active responses, but don't show waiting messages. Stopping the current run doesn't cancel queued messages.
