...
When an agent emits a `submit_plan` suspension, Studio renders it inline in the agent chat. If the suspension includes a plan body, Studio renders the Markdown. If it includes only a path, Studio shows the path. Both views let you approve, reject, or request changes.
