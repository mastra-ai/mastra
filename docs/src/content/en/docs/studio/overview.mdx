...
Set runtime variables that flow into your agent's instructions and tools through dependency injection. Open the **Run options** popover next to the run action (agents, workflows, and tools) to edit request context as JSON or use a schema-driven form when your agent defines a `requestContextSchema`. Values are saved in your browser per agent, workflow, and tool, so you can trigger conditional flows without restarting.
