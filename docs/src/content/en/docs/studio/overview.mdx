...
When the agent is idle, select **Send** to start a response. During an active response, chats using the thread signals transport offer two actions in the send button's dropdown:

- **Queue** (default): Wait for the current response to finish, then process the message as a separate turn. Each message shows **Queued** until its run starts.
- **Steer**: Send the message into the current run to change its direction. The message shows **Sent to current run**. Steering can redirect the agent before it answers the earlier request.

Choosing an action doesn't submit the message. Press **Enter** or select the button to use the selected action. When the agent becomes idle, the button returns to **Send** without clearing your draft.

Queued messages wait in server memory, so a server restart loses messages that haven't started. Queue labels are local to the current chat session; other tabs can observe active responses, but don't show waiting messages. Queued messages still run after you stop the current response.
