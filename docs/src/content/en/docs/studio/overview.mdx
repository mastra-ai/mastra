...
When an agent emits a `submit_plan` suspension, Studio renders the submitted plan inline in the agent chat. If the host provides the plan body, Studio renders it as Markdown; otherwise, Studio shows the submitted plan path and lets you approve, reject, or request changes.
