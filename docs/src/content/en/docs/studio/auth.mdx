...
- You deploy Studio so multiple team members can interact with agents, workflows, and tools.
- You need to restrict who can execute agents, edit workflows, or delete datasets.
- You want a login screen (SSO, email/password, or both) in front of your Studio deployment.
...
RBAC lets you control what each user can see and do inside Studio. It is separate from authentication: `server.auth` handles who the user is, while `server.rbac` handles what they can do.
...
When RBAC is active, Studio hides actions the user does not have permission for. A viewer does not see delete buttons; a member cannot modify agent configurations.
...
Studio Auth features (SSO login, RBAC, permission-based UI) are part of the Mastra Enterprise Edition. They work without a license during local development and with Simple Auth. For production deployments with third-party providers, a valid EE license is required.
