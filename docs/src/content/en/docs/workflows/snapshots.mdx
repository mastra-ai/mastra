...
Snapshots are saved to the configured storage system. By default, they use libSQL, but you can configure Upstash, PostgreSQL, OracleDB, or MongoDB instead. Each snapshot is identified by the workflow's `runId`.
