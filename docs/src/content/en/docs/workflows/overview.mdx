...
> See [Run.start()](../../reference/workflows/run-methods/start.mdx) for a full list of configuration options.
  ...
  <Tabs.Tab>
Create a workflow run with `createRunAsync()`, then call `.stream()` with `inputData` matching the workflow's `inputSchema`. The workflow emits events as steps run, allowing you to monitor progress in real time.
