...
Workflows let you define complex sequences of tasks using clear, structured steps rather than relying on the reasoning of a single agent. They give you full control over how tasks are broken down and how data moves between them, plus what gets executed when. Workflows run using the built-in execution engine by default, or can be deployed to [workflow runners](/ecosystem/deployment-patterns/workflow-runners) like Inngest for managed infrastructure.
...
- [Workers](/ecosystem/deployment-patterns/workers): Offload workflow orchestration to dedicated processes in production
