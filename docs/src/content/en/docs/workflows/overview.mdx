...
Workflows let you define and orchestrate complex sequences of tasks as **typed steps** connected by data flows. While a single agent may be suitable for simple tasks, real-world applications often involve multiple steps with different logic, tools, external services, or context requirements.
...
- Handle a user question like “Can I return my last order?”
- Fetch user-specific data from a database using their `user_id`.
- Check return eligibility via an external API or business logic.
- Make a conditional decision (e.g. approve, deny, escalate) based on the data.
- Generate a tailored response based on the outcome.
