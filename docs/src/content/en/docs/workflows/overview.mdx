...
:::note
Workflow state is currently supported only when using [Run.start()](/reference/workflows/run-methods/start). Support for `Run.stream()` will be added soon.
:::
