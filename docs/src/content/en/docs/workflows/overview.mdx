...
A workflow can be watched to observe each event as it's emitted in real time. Events are emitted when a step starts or completes, the workflow status changes, or the workflow is paused awaiting input.

```typescript {6-7} filename="src/test-workflow.ts" showLineNumbers copy
import { mastra } from "./mastra";
...
run.watch((event) => {
  ...
  console.log('Workflow status:', event?.payload?.workflowState?.status);
});
