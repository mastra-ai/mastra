...
> **NOTE:** The step created from agent (`createStep(agent)`) takes `prompt` as an input and returns `text` as an output. You will need to map the output of the previous step to the input of the agent step.
