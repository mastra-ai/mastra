...
Note that using `.map()` is optional because tools define their own input and output schemas, but can be helpful when the workflow `inputSchema` doesn't match the tool's `inputSchema`.

In this example, the workflow accepts a `city`, which is mapped to the tool's input. The tool returns an output string, which is passed directly to the workflow's output without any extra transformation.
...
Once a workflow has been registered it can be referenced using `getWorkflow` from within a tool.
