...
Alternatively, when exactly one step is suspended, you can omit the `step` parameter and the workflow will automatically resume that step:
...
if (result.status === "suspended") {
  const resumedResult = await run.resume({
    ...
    // step parameter omitted - automatically resumes when exactly one step is suspended
  });
}
```
