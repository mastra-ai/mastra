...
```mermaid
flowchart LR
  accTitle: Pausing a workflow for human input
  accDescr: step1 either completes and moves to end, or suspends and waits for human input.
  paused@{ shape: manual-input, label: "awaiting<br/>human input" }
  start(( start )) -- in --> step1([step1])
  step1 accent1@-- out --> stop(( end ))
  step1 pending1@-. suspend .-> paused
...
```mermaid
flowchart LR
  accTitle: Resuming a workflow after human input
  accDescr: The paused step receives resume data, step1 continues, and the workflow completes at end.
  paused@{ shape: manual-input, label: "awaiting<br/>human input" }
  paused accent1@-. resume .-> step1([step1])
  step1 accent2@-- out --> stop(( end ))
...
```mermaid
flowchart LR
  accTitle: Stopping a workflow with bail
  accDescr: After review step1 either completes at end, or stops at bail when the request is rejected.
  start(( start )) -- in --> step1([step1])
