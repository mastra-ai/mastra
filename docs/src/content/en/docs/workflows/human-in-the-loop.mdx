...
Use `bail()` to stop workflow execution at a step without triggering an error. This can be useful when a human explicitly rejects an action. The workflow completes with a `bailed` status, and any logic after the call to `bail()` is skipped.

**Important:** `bail()` must return a value that matches the step's `outputSchema`. You can use discriminated unions to handle different outcomes.

```typescript {7-11,13-19,24-28}
const step1 = createStep({
  id: "step-1",
  inputSchema: z.object({
    userEmail: z.string()
