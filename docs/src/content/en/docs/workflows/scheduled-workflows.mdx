...
- To unpause permanently, either call `resumeSchedule` or change the `cron` or `timezone` in code, which forces a recompute on the next deploy.
...
The underlying HTTP routes are `POST /schedules/:id/pause` and `POST /schedules/:id/resume`. Both require the `schedules:write` permission.
