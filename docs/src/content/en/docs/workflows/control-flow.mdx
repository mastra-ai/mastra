...
description: "Learn how to compose workflow steps using sequential, parallel, conditional, and looping patterns, and how to transform data between steps with input mapping."
---
...
```typescript {30-31} filename="src/mastra/workflows/test-workflow.ts" showLineNumbers copy
...
```typescript {42} filename="src/mastra/workflows/test-workflow.ts" showLineNumbers copy
const step1 = createStep({
  id: "step-1",
  // ...
});
...
```typescript {33-36} filename="src/mastra/workflows/test-workflow.ts" showLineNumbers copy
const step1 = createStep({...})
...
- [`getStepResult()`](../../reference/workflows/workflow-methods/map.mdx#using-getstepresult): Access a specific step's full output
- [`getInitData()`](../../reference/workflows/workflow-methods/map.mdx#using-getinitdata): Access the workflow's initial input data
- [`mapVariable()`](../../reference/workflows/workflow-methods/map.mdx#using-mapvariable): Use declarative object syntax to extract and rename fields
...
Use `.foreach()` to run the same step for each item in an array. The input must be of type `array` so the loop can iterate over its values.
