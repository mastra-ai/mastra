...
A configured classifier can produce typed routing values before `.branch()`. The complete classifier evidence remains available under `answers`.
...
Choice values retain the configured criteria-key union, so `inputData.values.route` is typed as `'billing' | 'support' | 'other'`. Score values are numbers. Boolean values are `P(true)` probabilities and aren't automatically thresholded.
