...
Alternatively, you can pass `{ allowFailure: true }` to `.parallel()`. When enabled, individual step failures no longer cause the entire parallel block to fail. Failed steps produce `null` in the downstream step's input, while error details remain available in the workflow result's `steps` object:
...
const workflow = createWorkflow({
  ...
  .parallel([researcherA, researcherB], { allowFailure: true })
  .then(writerStep)
  .commit()
```

The downstream step receives `null` for any step that failed:

```typescript title="src/mastra/workflows/test-workflow.ts"
const writerStep = createStep({
  id: 'writer',
  inputSchema: z.object({
    'researcher-a': z.object({ brief: z.string() }).nullable(),
    'researcher-b': z.object({ brief: z.string() }).nullable(),
  }),
  outputSchema: z.object({ synthesis: z.string() }),
  execute: async ({ inputData }) => {
    const briefs = Object.values(inputData)
      .filter((v): v is { brief: string } => v !== null)
      .map(v => v.brief)
    return { synthesis: briefs.join('; ') }
  },
})
...
:::note
Tripwire failures (processor hard limits like token budgets) always cause the parallel block to fail, even with `allowFailure: true`.
:::
