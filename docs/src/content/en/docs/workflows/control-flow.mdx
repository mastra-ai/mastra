...
Branch conditions can also be written as a structural JSON expression instead of a JS closure by passing `{ predicate }` in place of the function. Declarative predicates are the only form that can be persisted through the workflow storage APIs (`Mastra.addStoredWorkflow`, `POST /stored/workflows`), so use them when the workflow is authored as data — for example by the `mastracode` workflow builder or by a Studio-driven CRUD UI.
...
export const testWorkflow = createWorkflow({
  ...
  .then(step1)
  .branch([
    [{ predicate: { op: 'gt', left: { path: 'inputData.value' }, right: 10 } }, highValueStep],
    [{ predicate: { op: 'lte', left: { path: 'inputData.value' }, right: 10 } }, lowValueStep],
  ])
  .commit()
```

Predicates support the comparison operators `eq`, `ne`, `lt`, `lte`, `gt`, `gte`, `in`, `notIn`, `exists`, `notExists`, `truthy`, `falsy`, and the logical combinators `and`, `or`, `not`. Values are either literals or `{ path: '<scope>.<field>...' }` references to workflow data:

- `inputData.<field>` — the previous step's output.
- `initData.<field>` — the workflow's initial input, valid from any step.
- `stepResults.<id>[.<path>]` — a named earlier step's output; scalar results resolve with no subpath.
- `state.<field>` — the workflow state slot.

Missing paths resolve to `undefined` rather than throwing, so `exists` / `notExists` behave the way you'd expect. Closure-based `.branch()` conditions continue to work unchanged; the two forms can be mixed inside the same `.branch([...])` call.
...
#### Declarative predicates for loops

`.dowhile()` and `.dountil()` accept the same `{ predicate }` object form as `.branch()`. Use it when the loop needs to survive workflow storage (`Mastra.addStoredWorkflow`, `POST /stored/workflows`) — closure-based loop conditions remain live-only and are rejected at persistence time.

```typescript title="src/mastra/workflows/test-workflow.ts"
export const testWorkflow = createWorkflow({})
  .then(step1)
  .dountil(step2, {
    predicate: { op: 'gte', left: { path: 'inputData.number' }, right: 10 },
  })
  .commit()
```

The predicate shape and available path scopes match [Declarative predicates](#declarative-predicates) above.
...
### Nested workflows in stored graphs

Nested workflows are a first-class step type in the serialized graph — they can be referenced by id from JSON `WorkflowDefinition`s persisted through `Mastra.addStoredWorkflow` or `POST /stored/workflows`. Instead of inlining the sub-workflow's steps, the stored graph carries a compact reference:
...
The `workflow` entry can appear anywhere a step is expected — at the top level of the graph, or inside `parallel` / `conditional` / `foreach` / `dowhile` / `dountil`. At rehydration time, `workflowId` is resolved against the registered workflows on the `Mastra` instance; missing references are rejected with an actionable error.

Cross-workflow references between stored workflows are supported. `Mastra.addStoredWorkflow` runs a pre-flight `collectRefs` check that also considers workflows currently being seeded, and boot-time loading uses a two-pass topological sort so stored workflows can depend on other stored workflows regardless of storage order. Cycles (including self-reference) are detected and rejected.

Code-defined nested workflows (`.then(subWorkflow)`, `.parallel([subA, subB])`, `.foreach(subWorkflow)`, etc.) serialize to the same `type: 'workflow'` variant, so Studio and consumers of `serializedStepGraph` see one consistent shape whether the parent workflow was authored in code or seeded from JSON. The live `stepFlow` is unchanged; only the serialized/stored representation carries the new variant.
