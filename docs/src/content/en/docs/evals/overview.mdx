...
Eligibility filters decide _whether a scorer runs_; to filter _which messages a scorer sees_ once it runs, use [`filterRun()`](/reference/evals/filter-run). Filters cannot see the run's input or output. When eligibility depends on what happened in the run, such as whether a specific tool was called, return [`notScorable()`](/reference/evals/not-scorable) from a scorer step instead: the remaining steps are skipped and no score is stored.
