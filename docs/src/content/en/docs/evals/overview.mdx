...
**Durable runs**: When storage is configured, each live evaluation executes inside an internal workflow run. A `pending` run record is written before the scorer executes, and the run ends with a queryable `success` or `failed` status (including the error), so scoring failures leave a record instead of a log line. Transient scorer failures, such as a rate-limited judge model, are retried automatically. If the internal workflow isn't registered (for example in a custom host that doesn't register internal workflows), the scorer falls back to direct execution with the previous fire-and-forget behavior: failures are logged but no durable run record is written.

**Sampling decision records**: On supported storage adapters, sampling outcomes are recorded in the `mastra_scoring_decisions` table, including declines. Comparing decision rows against saved scores shows how much eligible traffic was actually scored.
