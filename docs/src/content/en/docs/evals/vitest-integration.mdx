...
description: "Run runEvals evaluations as Vitest tests with expectItems, expectItem, custom matchers, and a score reporter using @mastra/evals/vitest."
packages:
  - "@mastra/evals"
---
...
The `@mastra/evals/vitest` module integrates [`runEvals`](/reference/evals/run-evals) with [Vitest](https://vitest.dev/) so evaluations behave like regular tests: fluent assertions fail the test when the eval does not pass, custom matchers assert on scores, and a reporter prints a score table in the runner output.
...
## Asserting on a dataset with `expectItems`

`expectItems` runs a `runEvals` evaluation inside a regular `test()` and asserts a minimum pass rate. It accepts the same configuration as `runEvals`: a `target` agent or workflow, `data` items, and `scorers`, `gates`, or thresholds. Gates score each item pass/fail, so `toPass(0.8)` requires at least 80% of items to pass every gate. Scorer thresholds still compare the average score across items and must pass regardless of the rate:
...
LLM-backed evals are far slower than Vitest's default 5 second timeout, so pass a per-test `timeout` as shown above (or set `testTimeout` in the Vitest config).

## Matrix testing with `expectItem`

`expectItem` is the single-item variant: `data` is one item instead of an array. Combine it with `test.for` (or `test.each`) to get one test — and one reporter entry — per data item instead of one aggregated result per dataset:

```typescript title="src/mastra/agents/capitals.matrix.eval.test.ts"
import { test } from 'vitest'
import { expectItem } from '@mastra/evals/vitest'
import { capitalsAgent } from './capitals-agent'
import { containsGroundTruth } from '../scorers'
...
Each entry shows the run's verdict, gates, thresholds, and the average score per scorer. The reporter reads the metadata that `expectItem`/`expectItems` attach to `task.meta.mastraEval`, so it works with parallel test files and any test that populates that field.
