...
- Get a single pass/fail signal from an eval run without writing custom assertion logic
...
Wrap a scorer in `{ scorer, threshold }` to set a minimum acceptable score. The threshold is compared against the scorer's average score across all data items.
...
const result = await runEvals({
  ...
  scorers: [
    { scorer: faithfulnessScorer, threshold: 0.7 },
    { scorer: completenessScorer, threshold: 0.5 },
...
//   { id: 'completeness', passed: false, averageScore: 0.4, threshold: 0.5 },
// ]
```
...
if (result.verdict === 'scored') {
  console.warn('Threshold misses:', result.thresholdResults?.filter(t => !t.passed))
}
```
