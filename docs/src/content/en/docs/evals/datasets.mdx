...
The digest is a lowercase hexadecimal SHA-256 hash of the RFC 8785 canonical JSON representation of the envelope without `digest`. Items are sorted by portable identity for hashing; authored array order is preserved. Provenance is covered, so a new capture time changes the digest. This detects modification, not authorship or approval. The utilities don't redact sensitive data.
