...
const listed = await copy.listItems({ page: 0 })
...
Import repeats the reference checks before creating a dataset, though recovering an already completed request doesn't require those references to remain registered. Use `destination: { name, organizationId, projectId }` to override the destination name or tenancy. Source ownership and candidate bindings aren't copied. A `null` dataset setting or item override, such as `scorerIds: null`, imports as unset, matching how ordinary writes store a cleared value. Authored `input`, `groundTruth`, and `expectedTrajectory` values keep JSON `null`. In server code, derive tenancy from the authenticated caller and apply authorization before calling these APIs.
