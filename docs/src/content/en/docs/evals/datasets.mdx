...
Preflight compiles every `pattern` and `patternProperties` key in the captured `inputSchema`, `groundTruthSchema`, and `requestContextSchema` on the same linear-time engine that dataset validation uses, so an artifact's regular expressions can't stall the importer. A pattern that uses syntax the engine doesn't support, such as a backreference or lookaround, is rejected with `DATASET_SCHEMA_PATTERN_UNSUPPORTED` before any write.
