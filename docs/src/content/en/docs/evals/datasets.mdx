...
In-memory, LibSQL, and PostgreSQL dataset storage preserve JSON `null` in `input`, `groundTruth`, and `expectedTrajectory`, along with empty strings, arrays, and objects. Omitted optional JSON fields remain absent when serialized. Dataset schemas still determine which values an item accepts.

For configuration updates, `null` clears schemas, tags, target type, target IDs, or scorer IDs. All three adapters return cleared settings as `undefined`, while preserving empty arrays and objects. An item update with `scorerIds: null` removes its scorer override, whereas `scorerIds: []` disables scoring for that item. Omitted update fields leave existing values unchanged.
...
Older LibSQL and PostgreSQL writes stored optional JSON `null` values as SQL `NULL`, so those values remain indistinguishable from omitted fields. The storage fix preserves new writes but doesn't reconstruct previously lost distinctions.
...
Each item requires `createdAt` and `updatedAt` as UTC ISO 8601 strings with millisecond precision, matching `Date.toISOString()` for four-digit years. Both timestamps are included in the integrity digest. They represent the item's actual dates, not transfer provenance: an importer must preserve them as the destination item's `createdAt` and `updatedAt`, rather than replacing them with import time. Import time belongs in a separate import receipt. Later destination edits update `updatedAt` normally. These format helpers don't implement storage import.
