...
Some scorers only apply to a subset of runs. A scorer that judges how well a refund was handled has nothing to say about a run where no refund was requested. Judging it anyway costs a model call and stores a score that doesn't measure what the scorer is for, which skews averages and pass rates.
