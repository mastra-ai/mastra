...
Deleting an experiment permanently removes its result records and the observability traces it produced. Trace deletion also removes spans and trace-linked scores, feedback, metrics, and logs. If your storage adapter doesn't support trace deletion, Mastra logs a warning and still deletes the experiment and its result records.

You can also delete experiments with the [Core API](/reference/datasets/deleteExperiment), [Client SDK](/reference/client-js/datasets#deleteexperiment), or [server routes](/reference/server/routes#delete-an-experiment).
