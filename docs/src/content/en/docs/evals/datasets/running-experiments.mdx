...
- `TOOL_MOCK_NOT_DECLARED` — the tool has no mock and `unmockedToolPolicy` is `'deny'`.

The agent run is aborted immediately, so the model can't go on to call any further tools. These failures are deterministic, so they're not retried. Mocks that are declared but never used don't fail the item. They're reported as unconsumed.
