...
Each item resolves its source trace in this order:

1. `replayTraceId` on the item (inline data) or `metadata.replayTraceId` (storage-backed items)
1. The `traceId` recorded for the same item ID in `fromExperimentId`'s results
