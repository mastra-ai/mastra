...
The first two items use their `timeout` values. The third item inherits the 30-second `itemTimeout`. When neither value is set, an item has no execution deadline.
