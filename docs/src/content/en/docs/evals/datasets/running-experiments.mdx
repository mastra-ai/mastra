...
Mastra resolves all item-level IDs before running the target. It checks registered scorers first, then asks the Editor to hydrate a stored scorer. A stale item-level ID produces the `EXPERIMENT_ITEM_SCORER_NOT_FOUND` code for that item. Mastra skips its target execution and retries, while the rest of the experiment continues. Missing run-level and dataset-level IDs keep their previous skip-and-warn behavior.
