...
For implementation details and technical specifications, see [Creating Custom Scorers](/docs/scorers/custom-scorers).

### Running scorers

Scorers evaluate AI outputs by analyzing input/output pairs and returning numerical scores (typically 0-1) along with optional explanations. You can run scorers manually for testing or integrate them into your agents and workflows for continuous evaluation.

For technical details on the `.run()` method and input/output structures, see the [Custom Scorers documentation](/docs/scorers/custom-scorers).
...
You can use custom scorers in both your agents and workflow steps, as shown above. [Learn how to create your own custom scorers.](/docs/scorers/custom-scorers)
