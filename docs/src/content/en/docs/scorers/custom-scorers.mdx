...
- **Receives:** `{ input, output, extractStepResult? }`
...
- **Receives:** `{ input, output, score, analyzeStepResult, extractStepResult? }`
...
#### Example: Multi-step scorer with extract, analyze, and reason

Here's a complete example showing all three steps of the evaluation pipeline:
