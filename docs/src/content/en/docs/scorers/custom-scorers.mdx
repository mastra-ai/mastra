...
- **Use when:** You need explanations for scores, actionable feedback, or compliance documentation
...
All LLM scorer steps share a judge configuration that defines the model and system instructions.
...
The extract step preprocesses input/output data using an LLM.
...
The analyze step performs the core evaluation using the LLM.
...
The calculateScore function converts LLM analysis into a numerical score.
...
The reason step generates human-readable explanations for the score.
