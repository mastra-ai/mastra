---
title: "Modes | Mastracode"
description: "Learn how Mastracode's three operating modes — Build, Plan, and Fast — shape the agent's behavior, tool access, and response style."
packages:
  - "mastracode"
---
...
Mastracode operates in one of three modes, each tailored to a different kind of task. Modes control which tools the agent has access to, how it approaches problems, and which default model it uses.

Switch modes with the `/mode` slash command or by pressing the mode indicator in the status bar.
...
When a plan is approved in Plan mode, Mastracode automatically switches to Build mode and injects the plan into the agent's instructions. The agent then implements the plan step by step.
...
- **Approve** — Mastracode switches to Build mode and begins implementation.
...
Each mode has a default model that's automatically selected when you switch to it:
...
You can override the model for any mode using the `/models` command. Per-mode model selections are persisted to the thread, so switching back to a mode restores the last model you used in that mode.
...
YOLO mode auto-approves all tool calls without prompting. It's enabled by default but can be toggled through `/settings`. When disabled, Mastracode prompts for approval before executing write operations or shell commands, based on the permission system.
