...
description: "Learn how to configure Mastracode with authentication, MCP servers, hooks, custom commands, skills, database settings, and environment variables."
packages:
  - "mastracode"
---
...
Mastracode is configured through project-level files, global settings, and environment variables. Configuration files live in `.mastracode/` directories at the project and user levels.
...
For providers not covered by OAuth, Mastracode uses Mastra's [model router](/models), which auto-detects API keys from environment variables:
