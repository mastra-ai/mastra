...
- **[Filesystem](/ecosystem/workspace/filesystem)**: File storage (read, write, list, delete, copy, move, grep)
- **[Sandbox](/ecosystem/workspace/sandbox)**: Command execution (shell commands) and background processes
- **[LSP inspection](/ecosystem/workspace/lsp)**: Hover, definition, and implementation queries through language servers
- **[Search](/ecosystem/workspace/search)**: BM25, vector, or hybrid search over indexed content
- **[Skills](/ecosystem/workspace/skills)**: Reusable instructions for agents
...
The `skills` array specifies paths to directories containing skill definitions, see [Skills](/ecosystem/workspace/skills).
...
Static providers are owned by the workspace. Resolver-backed providers are owned by your application because the workspace creates them at request time. See [runtime sandbox lifecycle ownership](/ecosystem/workspace/sandbox#lifecycle-ownership) for the resolver cleanup model.
...
Under the hood, `mounts` creates a [CompositeFilesystem](/ecosystem/workspace/filesystem#mounts-and-compositefilesystem) that routes file tool operations to the correct provider based on path prefix. Commands in the sandbox access the mounted paths directly (e.g., `ls /data`).
...
One workspace instance serves all requests. The resolver runs at tool execution time, so each request gets its own filesystem. See [dynamic filesystem](/ecosystem/workspace/filesystem#dynamic-filesystem) for details.
...
The resolver is incompatible with `mounts` and `lsp: true`, since both require a concrete sandbox instance at construction time. See [dynamic sandbox](/ecosystem/workspace/sandbox#dynamic-sandbox) for details.
...
See [LSP inspection](/ecosystem/workspace/lsp) for configuration, examples, and tool name remapping.
...
- `Search` (if configured): Indexes files from `autoIndexPaths`, see [Search and Indexing](/ecosystem/workspace/search)
...
- [LSP inspection](/ecosystem/workspace/lsp)
- [Skills](/ecosystem/workspace/skills)
