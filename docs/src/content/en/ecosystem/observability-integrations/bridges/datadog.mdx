...
If you only need to send LLM Observability data and don't use `dd-trace` APM auto-instrumentation, the [Datadog Exporter](/ecosystem/observability-integrations/exporters/datadog) is simpler, it supports agentless mode and sends spans directly to Datadog without a local agent.
...
- Reuses the same data shape and span-kind mapping as the [Datadog Exporter](/ecosystem/observability-integrations/exporters/datadog#span-type-mapping)
...
The bridge requires `dd-trace` to be installed and a local Datadog Agent (or compatible OTLP receiver) to receive APM data. See the [APM prerequisites](/ecosystem/observability-integrations/exporters/datadog#prerequisites) on the exporter page for agent setup details.
...
The bridge uses the same span-kind mapping as the Datadog Exporter for LLM Observability. See [span type mapping](/ecosystem/observability-integrations/exporters/datadog#span-type-mapping) on the exporter page.
...
For native-module compatibility issues with `dd-trace` and bundler externals, see the [Datadog exporter troubleshooting](/ecosystem/observability-integrations/exporters/datadog#troubleshooting) section.

## Related

- [Tracing Overview](/guides/observability/tracing/overview)
- [Datadog Exporter](/ecosystem/observability-integrations/exporters/datadog) - LLM Observability only, no `dd-trace` APM
