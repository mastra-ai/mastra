...
You can deploy [Studio](/guides/studio/overview) alongside your API by enabling the `studio` option. Studio is deployed as static assets served from Vercel's Edge CDN, so it doesn't consume function invocations.
...
:::warning
The Vercel deployer doesn't include `flush` calls. If you need this, you'll need to wrap the handler yourself to add the logic before returning the response. Alternatively, deploy to a long-running server like a [virtual machine](/ecosystem/deployment-targets/digital-ocean) where this isn't an issue.
:::
...
- [Server adapters](/guides/server/server-adapters)
- [Web framework integration](/guides/deployment/web-framework)
