...
:::warning
Multi-pod support relies on [durable agents](/guides/long-running-agents/durable-agents), which are currently in **beta**. APIs may change in minor versions. Read [Known limitations](#known-limitations) before you rely on this in production.
:::
...
- **Pub/sub** carries events between pods. When an event is published on one pod, the others receive it. Mastra uses [`RedisStreamsPubSub`](/reference/pubsub/redis-streams), which also provides the per-thread leasing that keeps a single pod as the owner of a conversation at a time. See [PubSub](/guides/server/pubsub).
- **Storage** persists run state. [Durable agents](/guides/long-running-agents/durable-agents) save each run as a workflow snapshot, so any pod can resume a run from the database after a restart or when a request is routed elsewhere.
...
<Steps>
  <StepItem>
    Build and containerize the Mastra server, then push the image to your registry. Follow the [Mastra server](/guides/server/mastra-server) guide for the build, and ensure the server reads `process.env.PORT` and listens on `0.0.0.0`.
  </StepItem>
...
Because the run state is in Postgres and the events are in Redis, the reconnecting request can be served by any pod, not only the one that started the run. See [Resumable streams](/guides/long-running-agents/durable-agents#resumable-streams).
...
The pod that handles the approval loads the suspended run from Postgres. It then runs the approved tool and publishes the result over the shared pub/sub, so a client observing the run receives the continuation. See [Tool approval](/guides/long-running-agents/durable-agents#tool-approval).

## Known limitations

- The default in-process setup keeps run state in one pod's memory and doesn't share it across pods. Use [durable agents](/guides/long-running-agents/durable-agents) with shared Redis and Postgres so streaming, approvals, and reconnection work across pods.
...
- [Durable agents](/guides/long-running-agents/durable-agents)
