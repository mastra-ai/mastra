...
Run [Mastra workers](/ecosystem/deployment-patterns/workers) as separate processes so you can scale orchestration, scheduling, and background tasks independently from the API. This guide walks through a fully split deployment using Docker Compose.

:::info
This guide covers splitting workers into their own containers. If you only need workers to run in-process alongside the API, see [Workers](/ecosystem/deployment-patterns/workers). No extra setup is required.
:::
...
- [Workers](/ecosystem/deployment-patterns/workers): What workers are and when to use them
