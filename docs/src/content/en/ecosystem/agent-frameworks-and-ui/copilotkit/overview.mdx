...
Beyond rendering agent output as UI (see [generative UI](/ecosystem/agent-frameworks-and-ui/copilotkit/generative-ui)), CopilotKit lets the agent act on your application and pause for the user. Both patterns run against the same Mastra setup.
...
By default, the endpoint exposes every agent registered on the Mastra instance, and the frontend chooses one with the `agent` prop. Other CopilotKit runtime options are forwarded to the underlying runtime. For example, see [Open-ended generative UI](/ecosystem/agent-frameworks-and-ui/copilotkit/generative-ui#open-ended) for `mcpApps`.
