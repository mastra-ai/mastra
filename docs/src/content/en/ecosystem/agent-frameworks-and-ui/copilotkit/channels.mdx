...
The full setup lives in the [CopilotKit bot documentation](https://docs.copilotkit.ai/slack). This page shows how it fits together with a Mastra agent.

:::

## How it fits together

Your Mastra agent stays where it's, exposed through `registerCopilotKit()` as described in [CopilotKit overview](/ecosystem/agent-frameworks-and-ui/copilotkit/overview). The bot is a separate process built with `@copilotkit/bot`: you attach one or more platform adapters and point the bot at your agent. `createBot` takes an array of adapters, so a single bot can serve several platforms at once.
