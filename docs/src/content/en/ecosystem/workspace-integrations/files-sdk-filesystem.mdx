...
- **`deleteFile`** on a directory delegates to `rmdir({ recursive: true })`, matching the [`S3Filesystem`](/ecosystem/workspace-integrations/s3-filesystem) and [`GCSFilesystem`](/ecosystem/workspace-integrations/gcs-filesystem) behavior.
