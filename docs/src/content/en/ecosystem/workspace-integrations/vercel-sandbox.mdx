...
This is distinct from [`VercelServerlessSandbox`](/ecosystem/workspace-integrations/vercel-serverless), which runs commands as stateless Vercel serverless **Functions**. `VercelSandbox` runs a full Linux MicroVM with a persistent filesystem and long-running processes.
