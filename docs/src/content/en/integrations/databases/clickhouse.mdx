...
Trace deletion cascades to spans, trace roots and branches, metrics, logs, scores, and feedback linked by trace ID. Signals without a trace ID are preserved. Before applying ClickHouse lightweight delete masks, Mastra stamps matching rows with a deletion timestamp to establish a 30-day TTL purge path. A successful call returns after the timestamp mutations and delete masks complete, so normal reads no longer return the rows matched by that operation. ClickHouse can remove masked data during an earlier merge; otherwise, the conditional TTL makes stamped rows eligible for removal after 30 days.

Mastra also retains each deletion predicate for 90 days so an external scheduler can reapply requests and verify physical purging. Mastra OSS provides the storage primitives but doesn't run this scheduler. Adding the lifecycle TTL doesn't automatically materialize it against existing parts; schedule `MATERIALIZE TTL` separately if you need retroactive processing because it can create a large mutation.
