...
<PropertiesTable
  ...
  {
    name: 'replication',
    type: '{ cluster?: string; zookeeperPath?: string; replicaName?: string; allowMixedEngines?: boolean }',
    description:
...
If existing Mastra tables use local `MergeTree` or `ReplacingMergeTree` engines, Mastra emits a warning during initialization when `replication` is configured (since `CREATE TABLE IF NOT EXISTS` will leave existing tables untouched). To suppress this warning when deliberately keeping existing local tables, set `allowMixedEngines: true` in `replication`. To migrate local tables to replicated engines, recreate the affected tables as `Replicated*` before enabling replication. To migrate safely, rename the local table, run `CREATE TABLE ... ENGINE = ReplicatedMergeTree(...) ON CLUSTER ...`, run `INSERT INTO ... SELECT * FROM <renamed_local>`, then drop `<renamed_local>`.
