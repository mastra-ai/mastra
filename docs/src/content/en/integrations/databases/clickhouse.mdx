...
:::note
Nested trace metadata field discovery requires ClickHouse 24.8 or later with the analyzer enabled (`enable_analyzer=1`). Field enumeration uses recursive common table expressions (CTEs). Filtering and value discovery for a known metadata path use targeted JSON extraction instead. See [trace queries](/reference/observability/tracing/trace-query) for the supported paths and operators.
:::
