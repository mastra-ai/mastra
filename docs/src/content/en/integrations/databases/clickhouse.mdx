...
Trace deletion uses ClickHouse lightweight deletes and cascades to spans, trace roots and branches, metrics, logs, scores, and feedback linked by trace ID. Signals without a trace ID are preserved. Deletion is eventually consistent: rows may remain visible briefly while ClickHouse applies the delete masks, and the underlying data is removed asynchronously.
