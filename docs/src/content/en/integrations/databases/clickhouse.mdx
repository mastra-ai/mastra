...
Feedback review-status updates require `ALTER UPDATE` permission on `mastra_feedback_events` and, when delta polling is enabled, `INSERT` permission on `mastra_feedback_events_delta`. These updates wait for a ClickHouse mutation to finish, including on all replicas when replication is configured, so latency depends on the mutation queue. They modify the existing feedback row and preserve deletion masks: a concurrent review update cannot recreate deleted feedback. Successful updates remain available through delta polling. The status mutation and delta insert are separate operations. If the delta insert fails, the API returns an error even though the status may have changed. Retry the review update after resolving the error to publish the delta notification. If newer feedback content is ingested while a review update is in flight, the update is re-applied to the newest version; after repeated conflicts it fails with a conflict error and the caller retries.
