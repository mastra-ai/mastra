...
When retention is configured, Mastra also applies a TTL to deletion requests so they outlive the signal rows they protect. See [storage retention](/reference/storage/retention#clickhouse-native-ttl) for how the deletion-request TTL is calculated.
