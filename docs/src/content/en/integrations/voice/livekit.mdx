...
Pass `recording` to `dispatchVoiceSession()` or `liveKitConnectionRoute()` to create a room with [LiveKit auto egress](https://docs.livekit.io/transport/media/ingress-egress/egress/autoegress/). Mastra creates the room with recording configured, then dispatches the voice agent. Omitting `recording` preserves the existing behavior and return values.

Recording requires LiveKit Cloud or a self-hosted [egress service](https://docs.livekit.io/transport/self-hosting/egress/), plus an output destination. The example below records the caller and agent into a single mixed OGG audio file in an S3 bucket. Set `RECORDINGS_S3_BUCKET`, `RECORDINGS_S3_REGION`, `RECORDINGS_S3_ACCESS_KEY`, and `RECORDINGS_S3_SECRET` on your server, alongside the LiveKit credentials.
...
```typescript title="src/mastra/recording.ts"
...
Room creation and dispatch errors reject the operation; the route doesn't issue a token on failure. If dispatch fails after room creation, the room remains in LiveKit. Successful setup doesn't guarantee that the recording finishes or uploads successfully. Use LiveKit's [egress events](https://docs.livekit.io/intro/basics/rooms-participants-tracks/webhooks-events/#webhook-events) to check completion and retrieve file details before making a recording available to users. To verify the setup, make a test call, end the room, and listen to the completed file under `calls/` in your bucket for audio from both participants.

#### Recording lifecycle and consent
...
```typescript title="src/mastra/recording-playback.ts"
import {
  GetObjectCommand,
  HeadObjectCommand,
  S3Client,
...
The review route requires authentication by default. Configure your server's authentication before exposing it. For application or tenant rules, supply an `authorize({ traceId, context })` callback that returns a boolean; it runs before the trace is read or a playback URL is generated. The resolver also receives the stored `span` and request `context` for application-specific lookup. Routes with `requiresAuth: false` are appropriate only for a local demo.

Recording upload permissions alone don't grant playback access. The signing credentials need `s3:GetObject` for the recording objects, including the `HEAD` check. S3 also requires `s3:ListBucket` to return a missing-object `404` instead of `403`. Your application can use separate read credentials for playback. The browser receives a temporary URL that authorizes access to that file, so treat it as sensitive. See [S3 object permissions](https://docs.aws.amazon.com/AmazonS3/latest/API/API_HeadObject.html) and [presigned URLs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/using-presigned-url.html).
...
The package includes an adapter-based benchmark runner, fixed scenario definitions, result aggregation, and an experiment-result converter. Import these server-safe utilities from `@mastra/livekit`. They don't provision rooms, dial phone numbers, or load browser automation libraries.

```typescript title="src/voice-benchmark.ts"
import {
  runVoiceBenchmarks,
  summarizeVoiceBenchmarks,
  voiceBenchmarkScenarios,
} from '@mastra/livekit'
import type { RunVoiceBenchmarksOptions } from '@mastra/livekit'

export async function benchmark(run: RunVoiceBenchmarksOptions['run']) {
  const results = await runVoiceBenchmarks({
    scenarios: voiceBenchmarkScenarios.map(scenario => ({ ...scenario })),
    iterations: 5,
    timeoutMs: 30_000,
    mode: 'ci',
    ...
    versions: {
      livekitAgents: '1.9.0',
      fixture: '1',
      // Also record the tested @mastra/livekit version and adapter revision.
    },
    run,
  })
  return { results, summary: summarizeVoiceBenchmarks(results) }
}
```

Your `run(scenario, { signal, attempt })` adapter owns each trial's input and output. It returns `outcome`, `measurement`, the played transcript, tool names, and available latency measurements. It can also return `generatedText`, `traceId`, and a cost with its currency and pricing-version identifier. Honor the abort signal and release test rooms, streams, and provider resources in `finally`. The runner's timeout bounds its wait; it cannot forcibly terminate a callback or remote tool that ignores cancellation.

The supplied scenarios cover a simple answer, a lookup, a slow lookup, interruption, an injected failure, and a grounded follow-up. The lookup fixture's answer is `42`. For controlled pipeline tests, `createVoiceBenchmarkReplyGenerator(scenario)` from `@mastra/livekit/worker` implements fake replies, tool delay, cancellation, and failure injection. Pass it as `generate` to `MastraVoiceAgent`, with a fake TTS node and audio output. This fixture tests transport behavior, not model quality or actual business-tool execution.

For opt-in real-audio trials, select `mode: 'audio'` and either `measurement: 'server-playout'` or `'client-playout'`. Supply an adapter that sends synthetic caller audio through your application and its actual providers. The adapter must return the same measurement source as the run configuration; simulated results cannot pass as real audio. Run grounded follow-ups against the application agent, not the controlled reply fixture. Record the package, model, speech-provider, adapter, region, and configuration versions, and run cold and warm trials separately. The runner records these labels; it doesn't restart workers or warm models for you.

Use three separate latency values, each anchored to caller speech ending:

- `firstAudioMs`: the first audio, including an acknowledgment or filler.
- `firstUsefulAnswerMs`: the first spoken content satisfying the scenario's expected answer. The adapter needs aligned transcript or audio evidence for this; leave it unset when unavailable.
- `completionMs`: the end of completed answer playback.

The default success check requires the expected outcome, expected tool names, and expected text in `playedText`. Text generated but never played can't satisfy that check. The expected-text check is a case-insensitive substring assertion, not a semantic grounding scorer. Use a Mastra scorer for richer quality criteria.

Summaries group by scenario, measurement source, startup mode, and versions. They include every trial in the success rate and report failures and timeouts. Latency distributions use completed trials only, report p50/p95, and include a separate sample count for each measurement. Retain the raw results when interpreting interrupted trials or a small sample.

To store trials in a caller-driven dataset experiment, use the converter with Mastra core 1.61 or newer:

```typescript
import { voiceBenchmarkToExperimentResult } from '@mastra/livekit'

await dataset.submitExperimentResult(
  voiceBenchmarkToExperimentResult(result, { experimentId, itemId }),
)
```

Create the experiment without an execution target, map each scenario to an item in its pinned dataset version, and finalize it after submitting the trials. The converter includes the trace ID, timestamps, attempt number, raw measurements, and a success score. Use a separate experiment for each benchmark run so repeated attempt numbers don't overwrite earlier results. The converter doesn't load dataset APIs, so older Mastra core versions can still use the runner and JSON reports. See [caller-driven experiments](/docs/evals/experiments#caller-driven-experiments).
...
The 1.0 prerelease targets LiveKit Agents 1.9. Test the supplied prerelease version or package archive before deploying it. Updating a dependency range doesn't update an existing lockfile: check the installed versions after installation.
...
| `@livekit/agents-plugin-livekit` | `1.9.0` |
...
```bash npm2yarn
npm install ./mastra-livekit-1.0.0-next.0.tgz @livekit/agents@1.9.0 @livekit/agents-plugin-livekit@1.9.0 @livekit/agents-plugin-silero@1.9.0 @livekit/rtc-node@0.13.35
```

Replace the archive path with the supplied prerelease. Install any other `@livekit/agents-plugin-*` packages at the matching release. Agents 1.9 and Silero 1.9 require RTC `^0.13.34`; RTC 1.x doesn't satisfy that requirement. Don't override peer dependency errors to install it.

Keep Node.js 22.13 or newer and use a Zod version accepted by Agents: `^3.25.76 || ^4.1.8`. Verify the installed dependency tree and initialize the worker's models:
...
### Review custom LiveKit tools and speech providers
...
See the [LiveKit tool API](https://github.com/livekit/agents-js/blob/main/agents/src/llm/tool_context.ts) for custom plugin migration details.

ElevenLabs models no longer route through LiveKit Inference. Replace affected inference model strings with a supported provider or pass an instance from the standalone ElevenLabs plugin, configured with that provider's credentials. The standalone plugin remains available. See [LiveKit's provider change](https://github.com/livekit/agents-js/pull/2415).

### Review tokens, telemetry, and shutdown

Server SDK 2.19.1 rejects signed tokens without an `exp` claim. Tokens from `liveKitConnectionRoute()` already include an expiry, with a default lifetime of 15 minutes. Update custom token issuers and webhook fixtures that omit expiry. Cloud server API calls also support automatic region failover, which can change request latency and the region handling a retry.

Agents 1.8 added OpenTelemetry GenAI attributes for conversations and tool activity. Check exporter settings and any dashboards that read LiveKit attributes. To withhold conversational content from third-party exporters, set `LIVEKIT_TELEMETRY_ALLOW_PII=0`, or configure LiveKit's tracer provider with `allowPii: false`. Project redaction settings still apply. These controls don't configure Mastra's own exporters or room recordings. See [LiveKit's telemetry change](https://github.com/livekit/agents-js/pull/2411).

The `onCallEnd` hook still uses LiveKit's job shutdown callbacks. Keep cleanup within the configured shutdown window, and verify that it completes when a caller disconnects or a worker receives a shutdown signal. LiveKit's newer `onSessionEnd` callback is a separate lifecycle hook; the integration doesn't switch to it automatically.
...
<PropertiesTable
  ...
  {
    name: 'recording',
    type: 'LiveKitRecordingOptions | (args) => LiveKitRecordingOptions | undefined | Promise<LiveKitRecordingOptions | undefined>',
    description:
      'Creates a fresh room with automatic recording and dispatches the agent before returning connection details. The server-side callback receives body, context, and the resolved roomName. Return undefined to skip recording. Existing rooms and setup failures reject the request.',
    isOptional: true,
  },
]}
/>
