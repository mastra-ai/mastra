...
<Steps>
  ...
  <StepItem>
    ...
    This is only the minimal configuration — see the [Helm values reference](/integrations/deploy/kubernetes-helm-values) for every available value. You can also inspect the chart's annotated defaults directly (after `helm registry login`):
...
- Reference credentials with `existingSecret` per component instead of plaintext values.
...
- Enable autoscaling with `mastra-server.autoscaling` once resource requests are set.
...
- [Helm values reference](/integrations/deploy/kubernetes-helm-values): All configurable chart values
