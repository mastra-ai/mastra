...
The endpoint returns a streamed JSON response from the agent. Keep the AI SDK UI route for the streaming chat interface, and use the catch-all route to expose the full Mastra API. The adapter is documented in full on the [Next.js adapter](/reference/server/next-adapter) reference page.
