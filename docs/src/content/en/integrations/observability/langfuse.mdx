...
description: "Export Mastra traces to Langfuse for open-source LLM observability, configure credentials and endpoints, and inspect prompts, tools, and generations."
packages:
  - "@mastra/langfuse"
...
To suppress high-volume span types entirely (for example `MODEL_CHUNK` spans from streamed responses), pass `excludeSpanTypes` to the exporter. Langfuse bills per observation, and a streamed response produces one `MODEL_CHUNK` span per chunk, so this can noticeably reduce cost. Only the Langfuse exporter is affected; other exporters on the same observability instance still receive those spans:
...
To drop those span types from every exporter at once, use the observability-level [`excludeSpanTypes` option](/reference/observability/tracing/span-filtering) instead.
...
The Mastra CLI can import historical trace observations from Langfuse Cloud or a self-hosted Langfuse v4 or later instance. This is separate from configuring the exporter on this page. See [Import existing traces](/docs/mastra-platform/observability#import-existing-traces) for credentials, dry runs, resume, verification, and current limits.
