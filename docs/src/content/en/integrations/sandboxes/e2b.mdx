...
name is a deterministic hash of those inputs, and the commit sha rides as a
tag on that name (`mastra-repo-<hash>:sha-<sha>`). Without an explicit
`sha`, the spec pins itself to the repository's current default-branch head
at resolution time: right before the template lookup it runs `git ls-remote`
(no clone, \~100ms) and keys the tag on that sha. When the default branch
moves, the next new sandbox rebuilds the same template in place under a new
tag — old sha tags remain as prunable build history instead of piling up as
stale templates. If the head can't be resolved, the ref degrades to the
untagged name and the build clones whatever the default branch is at build
time. Pass `sha` to pin a specific commit instead, or `resolveHead` to
supply the head sha yourself.

Resolution is lazy: the first sandbox to reference a tagged ref checks
whether it exists on E2B and builds it once when it doesn't; later sandboxes
reuse it without rebuilding. A changed setup command hashes to a new
template name.

The template layers on top of the default mountable base (E2B's `base` image,
so git and common tooling are present, plus `s3fs`/FUSE for mount support).
Because build steps run as the sandbox's non-root `user`, the spec preps the
workspace root as root first (`mkdir` + `chown` to `user`) and then clones and
runs setup as `user`, keeping runtime file ownership correct.

Failure handling is designed so a broken template never wedges a session:

- A failed build falls back to a named workspace-base template (the default
  mountable base with a user-writable `/workspace`), so a runtime cold clone
  still works.
- E2B keeps a failed build's alias visible to `Template.exists`, so a broken
  alias could otherwise be reused forever. When creating a sandbox from a
  named alias fails, the cached resolution is dropped and creation retries
  once on the fallback.

Private repositories build warm templates through `getAuthToken`: a callback
that mints a fresh, short-lived credential (such as a GitHub App
installation token) per template resolution. The token authenticates the
head lookup and the build's clone through an in-shell `http.extraheader` —
it enters the template definition's environment (visible to build steps but
not persisted into runtime sandbox environments) and never touches the image
filesystem, so no captured layer can contain it. Only pass short-lived
credentials here — the value stays in the template definition until the next
rebuild, so its expiry is what bounds the exposure. Note that the cloned
repository contents become part of a team-visible template image. Without
`getAuthToken` the clone is tokenless: public repositories build fine, and a
private repository's build degrades to the fallback, where the session's own
runtime setup (with runtime-injected auth) does the clone.
...
E2B's idle timeout and resumes by id), the repo template comes from
`createRepoTemplate`, and the host's `onStart` setup hook runs inside the
start lifecycle.
      ...
      ...(ctx.repoFullName
        ? {
            template: createRepoTemplate({
              ...
              // Short-lived installation token so private repos build warm
              // templates too.
              getAuthToken: ctx.getGithubToken,
            }),
          }
        : {}),
      onStart: ctx.onStart,
