...
(`mastra-repo-<owner>-<repo>-<hash>:sha-<sha>`). Without an explicit
...
plus a user-writable `/workspace` — build steps run as the sandbox's
non-root `user`, so the base preps the workspace root as root first). The
clone and setup then run as `user`, keeping runtime file ownership correct.

Failure handling is designed so a broken template never wedges a session:

- A failed build falls back to the default mountable template, whose
  writable `/workspace` keeps a runtime cold clone working.
