...
const sandbox = new E2BSandbox({
  ...
  template: createRepoTemplate({
    ...
    sha: latestMainSha, // optional: omit for a sha-less alias
    setupCommand: 'pnpm install',
  }),
})
```

The spec carries a deterministic alias derived from the repository, sha, and
setup command, so the same inputs resolve to the same template. Resolution is
...
rebuilding. A changed sha or setup command hashes to a new alias.
...
### Session sandbox preset

For hosts that construct one sandbox per session, such as Mastra Factory's
`sandbox` config, `e2bSessionSandbox` packages the pattern above into a
ready-made callback: sandbox identity is the session id, the repo template is
derived from the session's repo intent, the sandbox pauses on idle (default
30 minutes) and resumes by id, and the host's `onStart` setup hook runs
inside the start lifecycle.
...
new MastraFactory({
  sandbox: e2bSessionSandbox(),
  // or with overrides:
  // sandbox: e2bSessionSandbox({ idleTimeoutMinutes: 60, template: myPrebuiltTemplateId }),
})
```

Writing the callback by hand gives the same behavior with full control:
