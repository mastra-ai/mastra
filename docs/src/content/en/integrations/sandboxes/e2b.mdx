...
support). The clone lands in the build user's home directory
(`$HOME/<repo>` by default) — build steps and runtime commands both run as
the sandbox's non-root `user` in its home, so no directory prep is needed
and runtime file ownership stays correct. Pass `workdir` to clone somewhere
else.
