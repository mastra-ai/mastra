...
Without an explicit `sha`, the spec pins itself to the repository's current
default-branch head at resolution time: right before the template lookup it
runs a tokenless `git ls-remote` (no clone, \~100ms) and keys the template
alias on that sha. There is exactly one template per repository and setup
command at any given head; when the default branch moves, the next new
sandbox builds a fresh template. If the head can't be resolved (private
repository, offline), the alias degrades to a sha-less form and the build
clones whatever the default branch is at build time. Pass `sha` to pin a
template to a specific commit instead, or `resolveHead` to supply the head
sha from an authenticated source.

The alias is deterministic — derived from the repository, sha, and setup
command — so the same inputs resolve to the same template. Resolution is
lazy: the first sandbox to reference an alias checks whether it exists on E2B
and builds it once when it doesn't; later sandboxes reuse it without
rebuilding. A changed head or setup command hashes to a new alias.
...
Template builds receive no credentials: the clone is tokenless, so only
public repositories can be baked into templates today. A private repository's
build fails and degrades to the fallback, where the session's own runtime
setup (with runtime-injected auth) does the clone. Private-repo template
builds are a follow-up pending E2B build secrets.

`repoTemplateAlias` computes the alias for a spec without building anything —
useful for pruning old templates with `e2b template delete`.
...
new MastraFactory({
  sandbox: ctx =>
    new E2BSandbox({
      id: ctx.sessionId,
      ...(ctx.repoFullName
        ? { template: createRepoTemplate({ repoFullName: ctx.repoFullName }) }
        : {}),
      onStart: ctx.onStart,
    }),
})
