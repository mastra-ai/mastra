...
Each command (clone, fetch, checkout, and every setup command) is its own
build step, so a failed build names the command that failed and the steps
before it stay cached across attempts. `setupCommand` accepts an array to run
entries as separate steps. `workingDirectory` sets an absolute directory as
the cwd for the build steps and for sandboxes created from the template; the
repository is cloned relative to it, at `<workingDirectory>/<repo>`. Omit it
to use the base image's working directory. Templates build at E2B's default
machine size (2 vCPU, 1024 MB) unless `cpuCount` and `memoryMB` ask for more:
...
Setup commands, working directory, and resources are all part of the
template's identity, so changing any of them builds a new template instead of
silently reusing one built with the old inputs. When a repo template's build
fails and the sandbox falls back to the default mountable template, the
fallback builds at the requested size too, so setup never lands in a smaller
machine than it asked for. Account tier limits apply to the values E2B
accepts.
