...
Mount an S3-compatible bucket such as Cloudflare R2 through the Workspace `mounts` option. The sandbox mounts it on `start()`. The Cloudflare Sandbox SDK forgets its mounts when the container stops, so before each command or file operation the sandbox checks that every mounted path is still a mountpoint and re-mounts any that a fresh container lost. This costs one short probe command per call while mounts are configured.
