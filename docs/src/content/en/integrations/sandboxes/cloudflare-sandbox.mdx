...
Mount an S3-compatible bucket such as Cloudflare R2 through the Workspace `mounts` option. The sandbox mounts it on `start()`, and the Cloudflare Sandbox re-establishes the mount when a slept container wakes.
