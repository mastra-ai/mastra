...
The stored tool name, call ID, and args remain unchanged, and other client-supplied fields are dropped. An edited resend can replace an unsealed user message, but edits to a sealed user message are ignored because observational memory already depends on its canonical content. If the stored-record lookup is unavailable or fails, Mastra aborts persistence rather than risk replacing a canonical record with a lossy echo. Sending only the new message remains recommended.
