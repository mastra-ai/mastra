...
description: Example of creating a custom LLM-based scorer using createScorer with prompt objects.
---
...
This example shows how to create a custom LLM-based scorer using `createScorer` with prompt objects. We'll build a "Gluten Checker" that evaluates whether a recipe contains gluten, using a language model as the judge.
...
A custom LLM scorer in Mastra uses `createScorer` with four core components:
