...
You can customize the Prompt Alignment Scorer by adjusting the scale parameter to fit your scoring needs.

```typescript showLineNumbers copy
const scorer = createPromptAlignmentScorerLLM({ 
  model: openai("gpt-4o-mini"), 
  options: { scale: 10 } // Score from 0-10 instead of 0-1
});
```
...
The scorer evaluates four weighted dimensions:

- **Intent Alignment (40%)**: Whether the response addresses the core purpose of the prompt
- **Requirements Fulfillment (30%)**: If all explicit and implicit requirements are met  
- **Completeness (20%)**: Whether the response is comprehensive and thorough
- **Response Appropriateness (10%)**: If the format and tone match expectations
