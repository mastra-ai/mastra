...
Use `PromptAlignment` to evaluate how well a response follows a given set of instructions. The metric accepts a `query` and a `response`, and returns a score and reason based on how closely the response aligns with the provided instructions.
