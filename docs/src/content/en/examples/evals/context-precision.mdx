...
Use `ContextPrecisionMetric` to evaluate how precisely a response uses the provided context. The metric accepts a `query` and a `response`, and returns a score and reason based on the inclusion of relevant versus irrelevant context information.
