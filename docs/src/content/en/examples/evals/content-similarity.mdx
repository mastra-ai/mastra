...
Use `ContentSimilarityMetric` to evaluate how similar a response is to a given query. The metric accepts a `query` and a `response`, and returns a score and reason based on the degree of textual similarity.
...
`ContentSimilarityMetric` returns an object with the following structure:

- `score`: A number between 0 and 1 indicating how similar the response is to the query.
- `info`: Additional similarity metadata, including:

  - `similarity`: The calculated similarity value between the query and response.
