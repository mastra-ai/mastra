...
Use `FaithfulnessMetric` to evaluate how accurately a response reflects the provided context. The metric accepts a `query` and a `response`, and returns a score and reason based on how well the output aligns with the source material.
