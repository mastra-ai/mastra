...
Use `BiasMetric` to evaluate responses for potential bias. The metric accepts a `query` and a `response`, and returns a score and reason based on the presence, type, and severity of biased language or assumptions.
...
You can adjust how the metric scores responses by configuring optional parameters. For example, `scale` sets the maximum possible score returned by the metric.
...
`BiasMetric` returns `score` and `info`:
