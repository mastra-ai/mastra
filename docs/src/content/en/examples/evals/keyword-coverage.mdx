...
Use `KeywordCoverage` to evaluate how accurately a response includes required keywords or phrases from the context. The metric accepts a `query` and a `response`, and returns a score and reason based on how well key terms are preserved in the answer.
