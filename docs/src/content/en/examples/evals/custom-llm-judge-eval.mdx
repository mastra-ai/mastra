...
This example shows how to create a custom LLM-base evaluation metric to determine real countries of the world. The metric accepts a `query` and a `response`, and returns a score and reason based on how accurately the response matches the query.
...
A custom eval in Mastra uses an LLM to judge the quality of a response based on structured prompts and evaluation criteria. It consists of four core components:
...
```typescript filename="src/example-real-world-countries.ts" showLineNumbers copy
...
```typescript filename="src/example-high-custom.ts" showLineNumbers copy
import { openai } from "@ai-sdk/openai";
import { WorldCountryMetric } from "./example-real-world-countries";
...
```typescript filename="src/example-partial-custom.ts" showLineNumbers copy
import { openai } from "@ai-sdk/openai";
import { WorldCountryMetric } from "./example-real-world-countries";
...
```typescript filename="src/example-low-custom.ts" showLineNumbers copy
import { openai } from "@ai-sdk/openai";
import { WorldCountryMetric } from "./example-real-world-countries";
...
A score between 0 and 1 reflects how well the response meets the evaluation criteria:
...
Additional metadata explaining the score:
