...
Use `CompletenessMetric` to evaluate how well a response captures all meaningful elements from a given `query`. The metric accepts a `query` and a `response`, and returns a score and a reason based on the degree of content overlap.
...
`CompletenessMetric` returns an object with the following structure:

- `score`: A number between 0 and 1 indicating how completely the response covers the input.
- `info`: A breakdown of the comparison between input and response, including:

  - `inputElements`: A list of key elements extracted from the input.
  - `outputElements`: A list of key elements found in the response.
  - `missingElements`: Input elements that were not found in the response.
  - `elementCounts`: A count of total elements in both input and output.
