...
Use `CompletenessMetric` to evaluate how well a response captures all meaningful elements from a given query. The metric accepts a `query` and a `response`, and returns a score and reason based on the degree of content overlap.
