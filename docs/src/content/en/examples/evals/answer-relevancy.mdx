...
Use `AnswerRelevancyMetric` to evaluate how well a response answers a given query. The metric accepts a `query` and a `response`, and returns a score and reason based on how accurately the response answers the question.
...
```typescript filename="src/example-high-relevancy.ts" showLineNumbers copy
...
```typescript filename="src/example-partial-relevancy.ts" showLineNumbers copy
import { openai } from "@ai-sdk/openai";
import { AnswerRelevancyMetric } from "@mastra/evals/llm";
...
```typescript filename="src/example-low-relevancy.ts" showLineNumbers copy
import { openai } from "@ai-sdk/openai";
import { AnswerRelevancyMetric } from "@mastra/evals/llm";
...
You can customize how the metric calculates scores by adjusting optional parameters. For example, `uncertaintyWeight` controls how much weight to give to uncertain responses, and `scale` sets the maximum possible score.
...
`AnswerRelevancyMetric` returns `score` and `info`:
