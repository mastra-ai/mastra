...
Use `ContextualRecallMetric` to evaluate how well a response incorporates information from the provided context. The metric accepts a `query` and a `response`, and returns a score and reason based on the inclusion of context elements in the output.
