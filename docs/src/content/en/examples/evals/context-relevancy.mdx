...
Use `ContextRelevancyMetric` to evaluate how relevant the provided context is to a given query. The metric accepts a `context` array and a `query`, and returns a score and explanation based on how well the context aligns with the question.
