...
This example uses the `openai` model. Add `OPENAI_API_KEY` to `.env`.
...
```typescript filename="src/mastra/functions/save-audio-to-file.ts" showLineNumbers copy
...
```typescript filename="src/mastra/functions/convert-to-text.ts" showLineNumbers copy
...
import { saveAudioToFile } from "./mastra/functions/save-audio-to-file";
import { convertToText } from "./mastra/functions/convert-to-text";
