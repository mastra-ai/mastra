...
Create an API route that streams responses from your Mastra agent using the experimental `stream` with AI SDK v5 format:
...
<Callout type="info">
  **Note**: The `stream` method with format support is experimental and may change as we refine the feature based on feedback. See the [Agent Streaming documentation](/docs/agents/streaming) for more details.
</Callout>
