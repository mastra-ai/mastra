...
When building AI agents, you need to define their role, personality, and capabilities. A system prompt (provided via the `instructions` field) acts as the agent's foundational identity - it tells the agent "who" it is and how it should behave. Unlike user messages that ask "what" to do, the system prompt defines the agent's expertise, tone, and approach to all interactions.
...
Use `getAgent()` to retrieve a reference to your agent, then call `generate()` with a prompt.
...
```text
Beneath the streets of London lies the world's first underground railway, the Metropolitan Railway, which opened in 1863.
```
