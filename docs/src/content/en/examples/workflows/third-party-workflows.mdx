...
description: Example of integrating Mastra workflows with third-party engines like Inngest
---

# Third-party Workflow Engines

This example demonstrates how to integrate Mastra workflows with third-party engines like Inngest.
...
Update your weather workflow to use Inngest. Start with the CLI-generated `src/mastra/workflows/weather-workflow.ts`:

```ts {1-5,11-15} filename="src/mastra/workflows/weather-workflow.ts" showLineNumbers copy
...
The existing `fetchWeather` step works the same - only the imports change:
...
The existing `planActivities` step also works the same:
...
You can add additional steps like indoor activities:
...
### Configure Mastra with Inngest

Add the Inngest integration to your existing Mastra configuration:

```ts {6-7,20-26} filename="src/mastra/index.ts" showLineNumbers copy
