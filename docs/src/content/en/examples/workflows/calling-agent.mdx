...
Workflows can call agents to generate dynamic responses from within a step. This example shows how to define an agent, register it with the Mastra instance, and invoke it using `.generate()` inside a workflow step. The workflow takes a city name as input and returns a fact about the corresponding country.
