...
const customMemory = new Memory({
  ...
  options: {
    ...
    threads: {
      generateTitle: {
        model: openai("gpt-3.5-turbo"), // Use cheaper model for titles
      },
    },
  },
});
...
3. Enabling automatic thread title generation with a cost-optimized model
4. Creating an agent with memory integration
5. Using semantic search to find relevant messages in conversation history
6. Including context around matched messages using `messageRange`
