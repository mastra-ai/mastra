---
title: "Example: Working memory templates | Memory | Mastra Docs"
description: Example showing how to use working memory templates to structure a todo list with UI feedback.
---

# Working memory templates

Use templates to define the structure of information stored in working memory. Templates help agents extract and persist consistent, structured data across conversations.
...
export const workingMemoryTemplateAgent = new Agent({
  ...
  instructions: `
You are a todo list AI agent. Always show the current list when starting a conversation. For each task, include: title with index number, due date, description, status, and estimated time. Use emojis for each field. Support subtasks with bullet points. Ask for time estimates to help with timeboxing.
  `,
  model: openai("gpt-4o"),
  memory: new Memory({
