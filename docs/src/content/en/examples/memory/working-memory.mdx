...
Sends two messages to the agent within the same thread. The second response is streamed and includes information remembered from the first.
...
Uses `.generate()` to send and receive full messages in a single call. The agent recalls details from the first interaction when responding to the second.
