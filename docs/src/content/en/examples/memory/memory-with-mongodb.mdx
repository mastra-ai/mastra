...
MONGODB_VECTOR_DB_NAME=<your-vector-db-name>
MONGODB_STORAGE_DB_NAME=<your-storage-db-name>
```
...
export const mongodbAgent = new Agent({
  ...
  memory: new Memory({
    storage: new MongoDBStore({
      ...
      dbName: process.env.MONGODB_STORAGE_DB_NAME!
    }),
    options: {
      threads: {
        generateTitle: true
...
Embeddings are numeric vectors used by memory's `semanticRecall` to retrieve related messages by meaning (not keywords). This setup uses FastEmbed, a local embedding model, to generate vector embeddings. 
...
export const mongodbAgent = new Agent({
  ...
  memory: new Memory({
    ...
    vector: new MongoDBVector({
      ...
      dbName: process.env.MONGODB_VECTOR_DB_NAME!
    }),
    embedder: fastembed,
    options: {
      lastMessages: 10,
