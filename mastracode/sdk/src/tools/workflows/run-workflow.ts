...
export function createRunWorkflowTool(accessPolicy?: DynamicWorkflowAccessPolicy) {
  ...
  return createTool({
    ...
    execute: async ({ workflowId, inputData }, { mastra, requestContext }) => {
      ...
      return withEphemeralMemory(requestContext, async ephemeralRequestContext => {
        const result = await service.runWorkflow(mastra as Mastra, workflowId, inputData, ephemeralRequestContext);
...
export const runWorkflowTool = createRunWorkflowTool();
