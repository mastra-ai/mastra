...
export async function getDynamicWorkspace({
  ...
  // File tools and exec root at the workspace root; skills, LSP detection,
  // and the workspace identity stay project-scoped. Absent state defaults
  // the root to the project path — today's coupled behavior.
  const workspaceRoot = state?.workspaceRoot ? path.resolve(state.workspaceRoot) : projectPath;
