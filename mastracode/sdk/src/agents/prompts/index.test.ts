...
describe('buildFullPrompt workspace-root split', () => {
  ...
  it('advertises the workspace root and names the repo subdir when roots differ', () => {
    const prompt = buildFullPrompt({ ...baseCtx, workspaceRoot: '/workspace' });
  ...
  it('behaves exactly as today when no distinct workspaceRoot is set', () => {
    ...
    const withEqualRoot = buildFullPrompt({ ...baseCtx, workspaceRoot: projectDir });
