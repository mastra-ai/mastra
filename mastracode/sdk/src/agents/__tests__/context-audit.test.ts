...
function findGroup(audit: Awaited<ReturnType<typeof buildContextAudit>>, id: string) {
  return [...audit.startup.groups, ...audit.accumulated.groups].find(group => group.id === id);
}

describe('buildContextAudit', () => {
  it('returns an empty audit with no divide-by-zero percentages', async () => {
  ...
  it('measures each prompt section separately with its provenance', async () => {
    const audit = await buildContextAudit({ promptSections });
    ...
    expect(group.entries[1]!.tokens).toBe(await tokenEstimate(promptSections[1]!.content));
    expect(group.tokens).toBe(group.entries.reduce((sum, entry) => sum + entry.tokens, 0));
  });

  it('skips empty sections so they do not clutter the report', async () => {
    const audit = await buildContextAudit({
  ...
  it('omits groups that have nothing to report', async () => {
    const audit = await buildContextAudit({ promptSections });

    expect(audit.startup.groups.map(group => group.id)).toEqual(['system-prompt']);
  });

  it('attributes tools to the server that provides them', async () => {
    const audit = await buildContextAudit({
      tools: [
  ...
  it('percentages are shares of the audited total and sum to 100', async () => {
    const audit = await buildContextAudit({
      promptSections,
  ...
  it('reports conversation tokens net of startup context and injected observations', async () => {
    ...
    const expected = 10_000 - audit.startup.tokens - (await tokenEstimate(injectedObservations));
    expect(findGroup(audit, 'conversation')!.entries[0]!.tokens).toBe(expected);
    expect(audit.totalTokens).toBe(10_000);
  });

  it('clamps conversation tokens when the provider count predates the current context', async () => {
    const audit = await buildContextAudit({ promptSections, conversation: { promptTokens: 1 } });

    expect(findGroup(audit, 'conversation')!.entries[0]!.tokens).toBe(0);
  });

  it('omits the conversation group before the first request', async () => {
    const audit = await buildContextAudit({ promptSections });
  ...
  it('says plainly that stored-only observations are excluded', async () => {
    const audit = await buildContextAudit({ injectedObservations: 'User prefers pnpm.' });
  ...
  // rendered text would.
  it('accepts an observation token count reported by the memory subsystem', async () => {
    const audit = await buildContextAudit({ injectedObservations: { tokens: 1_200 } });

    expect(findGroup(audit, 'observations')!.tokens).toBe(1_200);
  });

  it('omits the observation group when nothing is injected', async () => {
    const audit = await buildContextAudit({ promptSections, injectedObservations: { tokens: 0 } });

    expect(findGroup(audit, 'observations')).toBeUndefined();
  });

  it('counts injected observations against the context window', async () => {
    const audit = await buildContextAudit({ injectedObservations: 'User prefers pnpm.' });
  ...
  it('never carries the audited content itself, so a report is safe to share', async () => {
    const audit = await buildContextAudit({
