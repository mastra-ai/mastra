...
export interface RegisterWorkflowBuilderPrimitivesOptions {
   ...
   * parent directory pass that parent here.
   */
...
export async function registerWorkflowBuilderPrimitives(
  ...
  const { projectPath, allowedPaths = [], workingDirectory = projectPath, codeAgent, mcpManager } = options;

  // 1. Agents workflows can compose.
  mastra.addAgent(workflowBuilderAgent, 'workflow-builder');
  mastra.addAgent(codeAgent, 'code-agent');

  // 2. Workspace tools: file tools and exec root at the working directory
  ...
  const workspace = new Workspace({
    ...
    filesystem: new LocalFilesystem({
      ...
      allowedPaths: [workingDirectory, ...allowedPaths],
    }),
    sandbox: new LocalSandbox({ workingDirectory: workingDirectory }),
