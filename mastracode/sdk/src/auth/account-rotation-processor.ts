...
export type RotationClassification =
  { kind: 'rotate'; reason: 'rate-limit' | 'quota-exhausted' | 'auth-failed' } | { kind: 'hop' } | { kind: 'never' };
...
async function emitAccountSwitchPart(
  ...
  const controllerContext = args.requestContext?.get('controller') as
    { emitEvent?: (event: { type: 'info'; message: string }) => void } | undefined;
...
function loadRoutingSettings(args: Pick<RoutingProcessorArgs, 'requestContext'>, settingsPath?: string) {
  const configDirName = getRoutingController(args)?.getState?.().configDir;
  return typeof configDirName === 'string' ? loadSettings(settingsPath, configDirName) : loadSettings(settingsPath);
}
 ...
 * one.
 */
function getRouteTargetAccountId(
  args: Pick<RoutingProcessorArgs, 'requestContext'>,
  settingsPath: string | undefined,
  route: AccountRoute | null,
): string | undefined {
  if (!route) return undefined;
  return loadRoutingSettings(args, settingsPath).models.packAccountPreferences?.[route.packId]?.[route.modelId];
}
...
function resolveAccountRoute(
  ...
  const pending = state?.mastracodePendingPackFallback as
    { toPackId?: unknown; toModelId?: unknown; threadId?: unknown } | null | undefined;
  ...
  const settings = loadRoutingSettings(args, settingsPath);
...
async function applyPreferredAccountRoute(
  ...
  const preferredId = getRouteTargetAccountId(args, settingsPath, route);
...
export class AccountRotationProcessor implements Processor {
  ...
  async processAPIError(args: ProcessAPIErrorArgs): Promise<{ retry: boolean }> {
    ...
    const targetAccountId =
      route?.providerId === providerId ? getRouteTargetAccountId(args, this.options.settingsPath, route) : undefined;
  ...
  private async getPackCascade(args: ProcessAPIErrorArgs): Promise<PackCascade | null> {
    ...
    const modeId =
      typeof controller?.session?.modeId === 'string' && controller.session.modeId.length > 0
        ? controller.session.modeId
        : 'build';
    const settings = loadRoutingSettings(args, this.options.settingsPath);
    const packs = listResolvableModePacks(settings);
