...
The age thresholds are server-side, per-project config (seconds), served by `GET /web/factory/projects/:id/health/thresholds` and stored in the `queue-health` factory storage domain (`queue_health_settings` table) keyed by `(org_id, github_project_id)`. Defaults are `[14400, 86400, 259200]` (4h / 24h / 72h); `saveConfig` rejects a non-ascending or empty `thresholdsSeconds`. Thresholds live in seconds so fast-moving automated flows can represent sub-minute buckets.
