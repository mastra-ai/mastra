...
See `.env.schema` (package root; varlock validates `.env` against it). For auth-less local development, set `MASTRACODE_AUTH_DISABLED=1`; non-local deployments require `APP_DATABASE_URL`. Auth provider precedence is Mastra platform (`MASTRA_SHARED_API_URL`), WorkOS (`WORKOS_API_KEY` plus `WORKOS_CLIENT_ID`), then Better Auth (`BETTER_AUTH_SECRET`). Better Auth allows public email/password registration by default; set `MASTRACODE_AUTH_SIGNUP_DISABLED=1` to permit sign-in without new registrations. GitHub needs the complete `GITHUB_APP_*` group plus auth; Linear needs `LINEAR_CLIENT_ID` and `LINEAR_CLIENT_SECRET`; Railway sandboxes need `RAILWAY_API_TOKEN`. `MASTRACODE_PUBLIC_URL` controls the WorkOS (`/auth/callback`), GitHub App (`/auth/github/callback`), and Linear callback URLs.
