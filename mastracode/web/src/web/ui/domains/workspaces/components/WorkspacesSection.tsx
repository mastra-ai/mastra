...
export function WorkspacesSection({ children }: { children?: ReactNode }) {
  ...
  const openingWorktreePath = useRef<string | undefined>(undefined);
  ...
  const openWorktreeThread = async (worktreePath: string) => {
    if (location.pathname.startsWith('/factory') || openingWorktreePath.current === worktreePath) return;
    openingWorktreePath.current = worktreePath;
    try {
      ...
      if (openingWorktreePath.current === worktreePath) openingWorktreePath.current = undefined;
    }
  };
