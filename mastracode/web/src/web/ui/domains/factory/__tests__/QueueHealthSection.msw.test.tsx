 ...
 * stack, so the specs exercise what a user sees at /factories/:id/metrics: the
...
const FACTORY_ID = 'fp-github-project-1';
const PROJECT_REPOSITORY_ID = 'repo-link-1';
// The factory workspace is a feature worktree; its sessionId is the path
// work-item sessions point at for the active-work signal.
...
/** A factory workspace session row on WORKTREE, as `/sessions` returns it. */
function worktreeSession() {
  return {
    ...
    orgId: 'org-1',
    userId: 'user-1',
    branch: 'feat/queue-health',
    baseBranch: 'main',
    sandboxId: 'sbx-test',
    sandboxWorkdir: WORKTREE,
    materializedAt: ago(HOUR_S),
    createdAt: ago(HOUR_S),
    updatedAt: ago(HOUR_S),
  };
}
...
function useQueueHealthHandlers({ workItems, activityThreads = [], thresholds }: QueueHealthHandlers) {
  server.use(
    ...
    http.get(`${TEST_BASE_URL}/web/github/projects/${PROJECT_REPOSITORY_ID}/sessions`, () =>
      HttpResponse.json({ sessions: [worktreeSession()] }),
    ),
...
function renderAt() {
  ...
  const router = createMemoryRouter(createAppRoutes(), { initialEntries: [`/factories/${FACTORY_ID}/metrics`] });
