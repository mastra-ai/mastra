...
import { useProvidersQuery } from '#shared/hooks/use-providers';
...
export function SettingsPanel({ initialTab = 'general', onClose }: SettingsPanelProps) {
  ...
  const providerSources = new Map(
    (providersQuery.data?.providers ?? []).map(provider => [provider.provider, provider.source]),
  );
  const models = providersQuery.data
    ? (modelsQuery.data ?? []).flatMap(model => {
        const source = providerSources.get(model.provider);
        if (source === undefined) return model.hasApiKey ? [model] : [];
        ...
        >
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-5 pb-5">
            ...
            <TabContent value="model">
              <ModelTab
                models={models}
                loading={modelsQuery.isPending || providersQuery.isPending}
