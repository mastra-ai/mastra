...
const ATTACHMENT_OPTIONS: ReadonlyArray<{ value: AttachmentChoice; label: string }> = [
...
export function OMSection({ resourceId, models }: { resourceId?: string; models: AgentControllerAvailableModel[] }) {
  ...
  const mutationError = mutationFailure instanceof Error ? mutationFailure.message : null;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Field label="Observer model" hint="Summarizes the conversation into observations">
        {modelSelect('Observer model', config?.observerModelId ?? '', v => switchModel('observer', v))}
      </Field>

      <Field label="Reflector model" hint="Distills observations into longer-term memory">
        {modelSelect('Reflector model', config?.reflectorModelId ?? '', v => switchModel('reflector', v))}
      </Field>
