...
function installAgentControllerHandlers(
  providerSources: ProviderSources = { anthropic: 'oauth', openai: 'oauth' },
): CapturedRequests {
  ...
  server.use(
    ...
    http.get(`${TEST_BASE_URL}/web/config/providers`, () =>
      HttpResponse.json({
        ...
        providers: [
          { provider: 'openai', displayName: 'ChatGPT Plus/Pro', oauthSupported: true, source: providerSources.openai },
          {
            provider: 'anthropic',
...
function renderSettingsPanel(providerSources?: ProviderSources) {
  seedProject();
  const captured = installAgentControllerHandlers(providerSources);
  renderWithProviders(
    <Harness>
...
describe('SettingsPanel', () => {
  ...
  describe('when changing model preferences', () => {
    ...
    it('does not offer a model whose stored OAuth credential failed live refresh', async () => {
      const user = userEvent.setup();
      renderSettingsPanel({ anthropic: 'none', openai: 'oauth' });

      await user.click(screen.getByRole('tab', { name: /model/i }));
      await user.click(await screen.findByRole('button', { name: /openai \/ gpt-4o-mini/i }));

      expect(screen.queryByRole('option', { name: /claude-sonnet anthropic/i })).not.toBeInTheDocument();
      expect(screen.getByRole('option', { name: /gpt-4o-mini openai/i })).toBeEnabled();
    });
  });
