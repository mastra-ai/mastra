...
function ModelSettingsProbe() {
  ...
  return <output aria-label="Model settings state">{overlays.isOpen('model-settings') ? 'open' : 'closed'}</output>;
}
...
describe('StatusLine', () => {
  ...
  describe('when the session reports its model', () => {
    ...
    it('opens model settings from the active model control', async () => {
      seedProject();
      useAgentControllerHandlers();
      const user = userEvent.setup();
      renderStatusLine();

      await user.click(await screen.findByRole('button', { name: 'Change model from gpt-4o-mini' }));

      expect(screen.getByLabelText('Model settings state')).toHaveTextContent('open');
    });
