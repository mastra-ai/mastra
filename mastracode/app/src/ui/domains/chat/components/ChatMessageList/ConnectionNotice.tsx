...
export function ConnectionNotice() {
  ...
  const selectProjectDirectory = host.kind === 'desktop' ? host.selectProjectDirectory : undefined;
  ...
  const projectAccessRequired =
    ...
    Boolean(selectProjectDirectory && activeProject?.path);

  const recover = async () => {
    setActionError(undefined);
    setRecovering(true);
    try {
      if (projectAccessRequired && selectProjectDirectory && activeProject?.path) {
        const selection = await selectProjectDirectory({ defaultPath: activeProject.path });
