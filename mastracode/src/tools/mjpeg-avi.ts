...
function buildHdrl(args: {
  ...
  const hdrlPayloadSize =
    LIST_TYPE_SIZE +
    CHUNK_HEADER_SIZE + AVIH_SIZE +
    CHUNK_HEADER_SIZE + strlPayloadSize;
  const buf = Buffer.alloc(hdrlPayloadSize);

  let p = 0;
  FOURCC_HDRL.copy(buf, p); p += 4;

  // avih chunk
  FOURCC_AVIH.copy(buf, p); p += 4;
  buf.writeUInt32LE(AVIH_SIZE, p); p += 4;
  // dwMicroSecPerFrame
  ...
  // dwFlags
  buf.writeUInt32LE(AVIF_HASINDEX, p); p += 4;
  // dwTotalFrames
  buf.writeUInt32LE(totalFrames, p); p += 4;
  // dwInitialFrames
  buf.writeUInt32LE(0, p); p += 4;
  // dwStreams
  buf.writeUInt32LE(1, p); p += 4;
  // dwSuggestedBufferSize
  buf.writeUInt32LE(maxFrameLen, p); p += 4;
  // dwWidth, dwHeight
  buf.writeUInt32LE(width, p); p += 4;
  buf.writeUInt32LE(height, p); p += 4;
  // dwReserved[4]
  p += 16;

  // strl LIST
  FOURCC_LIST.copy(buf, p); p += 4;
  buf.writeUInt32LE(strlPayloadSize, p); p += 4;
  FOURCC_STRL.copy(buf, p); p += 4;

  // strh chunk
  FOURCC_STRH.copy(buf, p); p += 4;
  buf.writeUInt32LE(STRH_SIZE, p); p += 4;
  FOURCC_VIDS.copy(buf, p); p += 4; // fccType
  FOURCC_MJPG.copy(buf, p); p += 4; // fccHandler
  ...
  buf.writeUInt32LE(microSecPerFrame, p); p += 4;
  buf.writeUInt32LE(1_000_000, p); p += 4;
  // dwStart
  buf.writeUInt32LE(0, p); p += 4;
  // dwLength (frames)
  buf.writeUInt32LE(totalFrames, p); p += 4;
  ...
  buf.writeInt16LE(0, p); p += 2;
  buf.writeInt16LE(0, p); p += 2;
  buf.writeInt16LE(width, p); p += 2;
  buf.writeInt16LE(height, p); p += 2;

  // strf chunk (BITMAPINFOHEADER for MJPG)
  FOURCC_STRF.copy(buf, p); p += 4;
  buf.writeUInt32LE(STRF_SIZE, p); p += 4;
  // biSize
  buf.writeUInt32LE(STRF_SIZE, p); p += 4;
  // biWidth, biHeight
  buf.writeInt32LE(width, p); p += 4;
  buf.writeInt32LE(height, p); p += 4;
  // biPlanes, biBitCount
