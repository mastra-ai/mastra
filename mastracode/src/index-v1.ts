...
import { HarnessCompat, v1ModeToLegacy } from './HarnessCompat.js';
import type { HarnessCompatConfig } from './HarnessCompat.js';
...
type MastraCodeHarnessModeV1 = HarnessModeV1 & { agentId?: string };

function hash(value: string): string {
  return createHash('sha256').update(value).digest('hex').slice(0, 32);
}

function legacyModeToV1(mode: HarnessMode<MastraCodeState>, fallbackAgent: Agent): MastraCodeHarnessModeV1 {
...
function applyEffectiveDefaultsToV1Modes(
  modes: MastraCodeHarnessModeV1[],
  effectiveDefaults: Record<string, string>,
): MastraCodeHarnessModeV1[] {
...
function subagentsToV1Registry(subagents: HarnessSubagent[]): SubagentRegistryConfig {
  return {
    maxDepth: 1,
    types: Object.fromEntries(
      subagents.map(subagent => [
        subagent.id,
        {
          ...
          allowedHarnessTools: subagent.allowedHarnessTools,
...
export interface MastraCodeConfig {
  ...
  workspace?: HarnessCompatConfig<MastraCodeState>['workspace'];
...
export async function createMastraCode(config?: MastraCodeConfig) {
  ...
  const defaultModesV1: MastraCodeHarnessModeV1[] = [
    {
      id: 'build',
      description: 'Build',
    ...
    threads.map(thread => {
      ...
      return harnessStorage.saveSession({
        ...
        state: {
          ...
          tokenUsage: {
            promptTokens: 7535601,
            completionTokens: 16929,
            totalTokens: 7552530,
            cachedInputTokens: 5773312,
  ...
  const workspace: HarnessCompatConfig<MastraCodeState>['workspace'] = config?.workspace ?? getDynamicWorkspace;

  const mastraV1 = new Mastra({
  ...
  const adapterConfig: HarnessCompatConfig<MastraCodeState> = {
  ...
  const harnessV1 = new HarnessV1({
    ...
    subagents: subagentsToV1Registry(subagents),
    toolCategoryResolver: getToolCategory,
  });
  const harness = new HarnessCompat<MastraCodeState>(adapterConfig, harnessV1);
  ...
  const omThreadStateHarness = harness as never;
