...
describe('createApiClient', () => {
  describe('when the request succeeds', () => {
    it('prefixes the base url and returns the parsed JSON body', async () => {
      server.use(
        http.get(`${BASE_URL}/api/web/config/providers`, () =>
          HttpResponse.json({ providers: [{ provider: 'openai' }] }),
        ),
      );

      const client = createApiClient({ baseUrl: BASE_URL });
      const body = await client.get<{ providers: Array<{ provider: string }> }>('/api/web/config/providers');
    ...
    it('sends the JSON body on a mutation and returns the response', async () => {
      server.use(
        http.put(`${BASE_URL}/api/web/config/providers/openai/key`, async ({ request }) => {
      ...
      const client = createApiClient({ baseUrl: BASE_URL });
      const body = await client.put<{ ok: boolean; key: string }>('/api/web/config/providers/openai/key', {
        key: 'sk-test',
      });
  ...
  describe('when the server returns an error envelope', () => {
    it('throws with the server-provided error message', async () => {
      ...
      await expect(client.get('/api/web/config/providers')).rejects.toThrow('Credential storage is not available');
    });
  });

  describe('when the server returns a non-JSON error', () => {
    it('throws a status-based fallback message', async () => {
      server.use(http.get(`${BASE_URL}/api/web/config/providers`, () => new HttpResponse('nope', { status: 500 })));

      const client = createApiClient({ baseUrl: BASE_URL });

      await expect(client.get('/api/web/config/providers')).rejects.toThrow('500');
    });
  });
  ...
  describe('when a custom fetch implementation is injected', () => {
    it('uses the injected fetch instead of the global one', async () => {
      ...
      await client.get('/api/web/config/providers');

      expect(calledUrl).toBe(`${BASE_URL}/api/web/config/providers`);
    });
  });
});
