...
describe('GoalManager adapter', () => {
  ...
  it('resumes a paused goal when judge defaults are updated', async () => {
    const agent = createAgent();
    agent.updateObjectiveOptions.mockResolvedValue(
      makeRecord({ judgeModelId: 'anthropic/claude-sonnet-4-5', maxRuns: 25, runsUsed: 3, status: 'active' }),
    );
    const state = createState(agent);
    const manager = new GoalManager();
    await manager.setGoal(state, 'finish the task', '__GATEWAY_OPENAI_MODEL__', 50);

    // Simulate the goal being paused by a judge failure (e.g. rate limit)
    manager.applyEvaluation({ runsUsed: 3, status: 'paused' });
    expect(manager.getGoal()).toMatchObject({ status: 'paused' });

    // Changing judge settings should resume the paused goal
    const goal = await manager.updateJudgeDefaults(state, 'anthropic/claude-sonnet-4-5', 25);

    expect(agent.updateObjectiveOptions).toHaveBeenCalledWith(expect.objectContaining({ status: 'active' }));
    expect(goal).toMatchObject({ status: 'active', turnsUsed: 3 });
    expect(manager.isActive()).toBe(true);
  });
