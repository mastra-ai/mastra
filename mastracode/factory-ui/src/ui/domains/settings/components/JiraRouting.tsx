...
/**
 * Routing for the selected Jira projects. A Jira project feeds exactly one
 * Factory; until it is routed its issues are not picked up by any board.
 */
export function JiraRouting({
  sourceIds,
  projects,
  factories,
}: {
  ...
  const route = (sourceId: string, value: string) => {
    saveBinding.mutate(
      { integrationId: 'jira', sourceId, factoryProjectId: value === UNROUTED ? null : value },
      {
        onSuccess: () => toast.success('Jira routing updated'),
        onError: error => toast.error(error instanceof Error ? error.message : 'Failed to save Jira routing'),
      },
    );
  };
    ...
    <div className="flex flex-col">
      {sourceIds.map(sourceId => {
        ...
        const boundFactoryId = bindings.find(
          binding => binding.integrationId === 'jira' && binding.sourceId === sourceId,
        )?.factoryProjectId;
