...
describe('sessionsRefetchInterval', () => {
  ...
  it('does not poll when every session is materialized', () => {
    expect(
      sessionsRefetchInterval(data({ workspaces: [session({})], userSessions: [session({ sessionId: 'sess-2' })] })),
    ).toBe(false);
  });

  it('polls while a workspace session is un-materialized', () => {
    expect(sessionsRefetchInterval(data({ workspaces: [session({ materializedAt: null })] }))).toBe(15_000);
  });

  it('polls while a user session is un-materialized', () => {
    expect(sessionsRefetchInterval(data({ userSessions: [session({ materializedAt: null })] }))).toBe(15_000);
  });
});
