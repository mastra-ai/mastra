...
The plan must be executable by someone with no access to this conversation beyond this message. Write it to `<repo>/.artifacts/plans/issue-<number>.md` (the `.artifacts` directory inside the repository checkout) and include the same plan in the conversation.
