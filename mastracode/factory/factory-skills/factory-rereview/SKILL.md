...
description: Re-review a GitHub pull request or GitLab merge request after a push — reconcile the previous review against the new commits, look for new defects the push introduced, then a fresh pass over the whole change, and finish with a verdict on it
---
...
**Provider note:** the commands below are GitHub's. On a GitLab merge request — `$ARGUMENTS` names a "GitLab merge request" — `gh` cannot read it at all, so use `gitlab_get_merge_request` for the change and its notes, and `gitlab_create_merge_request_comment` to publish the verdict. Your previous pass is a note from the connected account in that same result, not a `reviews` entry. Read the pushed commits with `git` in the checkout, which already has the merge request head. Where a phase names a GitHub-only signal with no GitLab counterpart — CI checks, review threads, approval state — record it as an uncollected signal in the handoff rather than treating it as clean.
...
On a GitLab merge request, publish with `gitlab_create_merge_request_comment` instead, writing the body to `.artifacts/factory-rereview/mr-<iid>.md` first. State the verdict as the first line: a GitLab approval is a separate permissioned action the connected account may not hold, so the note body is the verdict of record.
