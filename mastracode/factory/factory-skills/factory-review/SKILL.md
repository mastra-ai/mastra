...
For `mastra-ai/mastra`, reconcile the verdict label after publishing: approve adds `status:auto-approved` and removes `status:changes-requested`; request changes adds `status:changes-requested` and removes `status:auto-approved`.
