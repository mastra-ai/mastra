...
6. Now read the change: `gh pr diff <number>` and `gh pr view <number> --json commits,files`. Compare it against the design you just recorded. Agreement is valid when independently justified; disagreement needs evidence, not merely a different preference.
