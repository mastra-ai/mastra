...
Next, publish the review on the PR itself — this is part of every pass, not something to wait to be asked for. Write the handoff body to `<repo>/.artifacts/factory-review/pr-<number>.md` (the `.artifacts` directory inside the repository checkout) and submit a PR review matching the verdict:
...
4. Write the follow-up body to `<repo>/.artifacts/factory-review/follow-up-pr-<number>.md`; it links the review and lists each finding it addresses, and the handoff links the follow-up PR.
