...
2. Compare `reviewTarget.url` with the `url` you resolve for the PR under review (typically `https://github.com/<owner>/<repo>/pull/<number>` from the Phase 1 PR). If `reviewTarget.url` is `null`, fall back to comparing the trailing PR number in `reviewTarget.externalId` (`github-pr:<number>`) — but only after confirming the Phase 1 lookup ran against the intake-bound repository: the externalId scopes the number to that repository, so first check that the session checkout's `git remote get-url origin` names the same `<owner>/<repo>` as the Phase 1 PR URL. A trailing-number match against any other repository proves nothing; if the repositories differ, treat it as a mismatch. A mismatch on the bound-repository number is proof of a wrong-target review.
