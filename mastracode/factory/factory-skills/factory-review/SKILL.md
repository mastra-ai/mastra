...
**Write the body so the repository's automation accepts it.** Many repositories run triage bots that auto-close PRs whose body doesn't reference an issue or skips the PR template. Check for a PR template (`.github/PULL_REQUEST_TEMPLATE*`) and fill it out; always reference the reviewed PR explicitly (`Follow-up to #<number>`), and if the reviewed PR references an issue, mention that issue too — with a non-closing reference (`Relates to #<issue>`), never a closing keyword like `Fixes`, so merging the follow-up cannot close an issue the reviewed PR is meant to resolve. If a bot closes the follow-up anyway, reopen it once with the missing reference added; if it closes again, record that in the handoff and move on.
...
`rationale` — one or two sentences: review complete, verdict, and the headline reason. Keep it brief; overly long rationales are truncated.
