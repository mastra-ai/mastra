  ...
  async prepare(): Promise<MastraArgs> {
    ...
    const sessionRetirement =
      machine && storage.isDomainReady('source-control')
        ...
        : undefined;
    // Terminal sessions use the same retirement path as explicit deletion:
    // teardown first, then local destruction or remote scrub/pooling, followed
    // by runtime workspace/token-cache invalidation. Session rows remain so a
    // reopened work item can rematerialize and rerun setup.
