...
export interface PlatformJiraApiClientConfig {
  client: PlatformApiClient;
  connectionId: string;
}

export type PlatformJiraApiErrorCode = 'jira_auth_failed' | 'jira_request_failed';

export class PlatformJiraApiError extends Error {
  readonly status: number | null;
  readonly code: PlatformJiraApiErrorCode;

  constructor(message: string, status: number | null) {
    super(message);
    this.name = 'PlatformJiraApiError';
    this.status = status;
...
export class PlatformJiraApiClient {
  readonly #client: PlatformApiClient;
  readonly #connectionId: string;

  constructor(config: PlatformJiraApiClientConfig) {
    if (!config.connectionId) throw new Error('PlatformJiraApiClient is missing required config: connectionId.');
  ...
  async #request<T>(
    ...
    try {
      ...
      if (error instanceof PlatformApiError) throw new PlatformJiraApiError(error.message, error.status);
      throw new PlatformJiraApiError(error instanceof Error ? error.message : String(error), null);
    }
  }
