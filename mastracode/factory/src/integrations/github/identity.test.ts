...
function makeHost(overrides: {
  ...
  membersByOrg?: Record<string, Array<{ login: string; id: number }>>;
  membersError?: Record<string, Error>;
} = {}): { host: GithubIdentityHost; paginate: ReturnType<typeof vi.fn> } {
  ...
  const octokit = { paginate, orgs: { listMembers: {} as unknown } } as unknown as Octokit;
  return {
    paginate,
    host: {
...
describe('buildGithubIdentity', () => {
  it('paginates org members for every Organization-typed installation', async () => {
  ...
  it('skips user-account installations (no org members exist)', async () => {
    const { host, paginate } = makeHost({
      installations: [
        makeInstallation({ accountType: 'User', accountName: 'someuser' }),
      ],
    });
    const identity = buildGithubIdentity(host);
