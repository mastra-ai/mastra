...
describe('title generation routes with a tenant', () => {
  let seed: FactoryStorageTestSeed;

  function buildApp({ withStorage = true }: { withStorage?: boolean } = {}) {
    const app = new Hono();
    app.use('*', async (c, next) => {
      c.set('factoryAuthUser' as never, { workosId: 'user-a', organizationId: 'org1' } as never);
      await next();
    ...
    mountApiRoutes(
      ...
      new ConfigRoutes({
        ...
        controller: makeAgentController([{ provider: 'anthropic', hasApiKey: true }]),
        ...(withStorage ? { titleSettings: seed.titleSettings } : {}),
      }).routes(),
    );
    return app;
  }

  const getJson = (app: Hono) => app.request('/web/config/title-generation');
  const putJson = (app: Hono, body: unknown) =>
    app.request('/web/config/title-generation', {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(body),
    });
  ...
  it('reads the defaults when no row exists', async () => {
    const res = await getJson(buildApp());
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ config: { enabled: true, modelId: null, thinkingLevel: null } });
    expect(await seed.titleSettings.get({ orgId: 'org1' })).toBeNull();
  });

  it('persists a patch and returns the effective config', async () => {
    const app = buildApp();
    const res = await putJson(app, { enabled: false, modelId: 'google/gemini-2.5-flash', thinkingLevel: 'low' });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({
      ok: true,
      config: { enabled: false, modelId: 'google/gemini-2.5-flash', thinkingLevel: 'low' },
    });
    await expect(seed.titleSettings.get({ orgId: 'org1' })).resolves.toMatchObject({
      orgId: 'org1',
      enabled: false,
  ...
  it('clears a knob with null back to its default', async () => {
    const app = buildApp();
    await putJson(app, { modelId: 'google/gemini-2.5-flash', thinkingLevel: 'low' });
    const res = await putJson(app, { modelId: null, thinkingLevel: null });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({
      ok: true,
      config: { enabled: true, modelId: null, thinkingLevel: null },
    });
  });

  it('rejects invalid knobs without writing', async () => {
    const app = buildApp();
    expect((await putJson(app, { enabled: 'yes' })).status).toBe(400);
    expect((await putJson(app, { modelId: '' })).status).toBe(400);
    expect((await putJson(app, { thinkingLevel: 'ultra' })).status).toBe(400);
    expect((await putJson(app, {})).status).toBe(400);
    expect(await seed.titleSettings.get({ orgId: 'org1' })).toBeNull();
  });

  it('reports storage as unavailable when the domain is missing', async () => {
    const app = buildApp({ withStorage: false });
    expect((await getJson(app)).status).toBe(503);
    expect((await putJson(app, { enabled: true })).status).toBe(503);
  });
});
