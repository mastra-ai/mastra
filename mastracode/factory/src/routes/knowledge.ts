    ...
    return [
      ...
      registerApiRoute('/web/factory/projects/:id/knowledge/subgraph', {
        ...
        handler: async raw => {
          ...
          const last = eligible.length > members.length ? members.at(-1) : undefined;
          ...
          if (structuralLens) {
            for (const member of members) {
              const key = `${selected.id}\u0000${member.id}\u0000contains`;
              edgeSeen.add(key);
              edges.push({
          ...
          const emptyScopeCounts = directScopeMemberCounts([], this.#limits.maxNodes);
          const graphNodes = await Promise.all(
            [
              ...structuralNodes.map(node => ({ node, boundaryScope: undefined, isScope: true })),
              ...nodes.map(node => ({ node, boundaryScope: undefined, isScope: false })),
              ...[...boundaryNodes.values()].map(value => ({
                node: value.node,
                boundaryScope: value.scope,
                isScope: false,
              })),
            ].map(async ({ node, boundaryScope, isScope }) => {
              ...
              return {
                ...
                ...(isScope ? { isScope: true, ...(scopeCounts.get(node.id) ?? emptyScopeCounts) } : {}),
                ...
                ? {
                    nextCursor: this.#mintHandle(
                      ...
                      JSON.stringify([
                        selected.id,
                        limit,
                        createKnowledgeNodeCursor(last, structuralLens ? {} : { isScope: false }),
                      ]),
                    ),
                  }
                : {}),
