...
export const loginPreservesModelPackScenario = {
  ...
  async run({ terminal, runtime }) {
    ...
    terminal.submit(
      `!node -e 'const fs=require("fs"); const app=process.env.MASTRA_APP_DATA_DIR; const s={...JSON.parse(fs.readFileSync(app+"/config.json","utf8")),...JSON.parse(fs.readFileSync(app+"/state.json","utf8"))}; const a=JSON.parse(fs.readFileSync(app+"/auth.json","utf8")); const reg=Object.keys(a).find(k=>k.startsWith("accounts:anthropic:")); console.log("LOGIN_PRESERVE_AUTH_OK="+(a.anthropic?.type==="oauth")); console.log("LOGIN_PRESERVE_LABEL="+(reg?a[reg].label:"missing")); console.log("LOGIN_PRESERVE_PACK="+s.models.activeModelPackId); console.log("LOGIN_PRESERVE_OM="+s.onboarding.omPackId+":"+s.models.activeOmPackId+":"+s.models.omModelOverride); console.log("LOGIN_PRESERVE_DEFAULTS="+Object.keys(s.models.modeDefaults||{}).length);'`,
    );
