...
const packName = 'Auto Memory Pack E2E';
...
export const modelsPackMemoryAutoScenario = {
  name: 'models-pack-memory-auto',
  description:
    'Creates a custom pack whose memory (OM) model is Auto through /models and verifies persistence and detail rendering.',
  testName: 'creates a custom pack with an Auto memory model and persists it',
  env: () => ({
    ANTHROPIC_API_KEY: '',
    OPENAI_API_KEY: '',
  ...
  async run({ terminal, runtime }) {
    ...
    await runtime.waitForScreenText(/Auto Memory Pack E2E/i, terminal, 8_000);
    terminal.write('\r');
    ...
    await runtime.waitForScreenText(/Auto — follow the main model/i, terminal, 8_000);
    await runtime.waitForScreenText(/Skip — use \/om settings/i, terminal, 8_000);
    // Second row is "Auto — follow the main model".
    terminal.write('\x1b[B');
    terminal.write('\r');

    await runtime.waitForScreenText(/Switched to Auto Memory Pack E2E pack/i, terminal, 12_000);
    ...
    await runtime.waitForScreenText(/MEMORY_PACK_ACTIVE=custom:Auto Memory Pack E2E/i, terminal, 8_000);
    await runtime.waitForScreenText(/MEMORY_PACK_MEMORY=auto/i, terminal, 8_000);
    await runtime.waitForScreenText(/MEMORY_PACK_DEFAULT_MEMORY=auto/i, terminal, 8_000);

    // Reopen /models and confirm the pack detail renders the memory row.
    terminal.submit('/models');
    await runtime.waitForScreenText(/Switch model pack/i, terminal, 8_000);
    await runtime.waitForScreenText(/memory\s+→\s+auto/i, terminal, 8_000);
