...
export const agentConnectionsToolFlowScenario = {
  ...
  description: 'Discover and connect a peer agent through core pubsub discovery and real tools/state signals.',
  testName: 'connects a pubsub-discovered peer agent and exposes connected-agent state to the model',
  useOpenAIModel: true,
  aimockFixture: 'agent-connections-tool-flow.json',
  async inProcessApp({ startMastraCodeApp }) {
    let peerAdvertisement: { unsubscribe: () => void } | undefined;
    ...
    const app = await startMastraCodeApp({
      onCreated: result => {
        ...
        timer = setInterval(() => {
          ...
          if (advertised || !threadId || !result.session.stream.isActive()) return;
          advertised = true;
          void (
  ...
  async run({ terminal, runtime }) {
    ...
    await runtime.waitForScreenText(/Agent connection tool flow completed/i, terminal, 20_000);
    await expect(
      terminal.getByText(
        /agent_connections_list ✗|agent_connect .*✗|Failed to list agent connections|Unknown agent peer id/i,
