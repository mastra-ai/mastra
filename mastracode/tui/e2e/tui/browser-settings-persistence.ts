...
export const browserSettingsPersistenceScenario = {
  ...
  async run({ terminal, runtime }) {
    ...
    terminal.submit(
      `!node -e 'const fs=require("fs"); const s=JSON.parse(fs.readFileSync(process.env.MASTRA_APP_DATA_DIR+"/config.json","utf8")); const b=s.browser||{}; console.log("BROWSER_AFTER_PROFILE_CDP="+(b.cdpUrl||"missing")); console.log("BROWSER_AFTER_PROFILE_SET="+(b.profile&&b.profile.endsWith("browser-profile-e2e"))); console.log("BROWSER_AFTER_PROFILE_PRESERVE="+(b.stagehand&&b.stagehand.preserveUserDataDir));'`,
    );
    ...
    terminal.submit(
      `!node -e 'const fs=require("fs"); const s=JSON.parse(fs.readFileSync(process.env.MASTRA_APP_DATA_DIR+"/config.json","utf8")); const b=s.browser||{}; console.log("BROWSER_CDP="+(b.cdpUrl||"missing")); console.log("BROWSER_PROFILE="+(b.profile||"missing")); console.log("BROWSER_EXEC="+(b.executablePath||"missing")); console.log("BROWSER_STAGEHAND_ENV="+(b.stagehand&&b.stagehand.env)); console.log("BROWSER_PRESERVE="+(b.stagehand&&Object.prototype.hasOwnProperty.call(b.stagehand,"preserveUserDataDir")));'`,
    );
    ...
    terminal.submit(
      `!node -e 'const fs=require("fs"); const s=JSON.parse(fs.readFileSync(process.env.MASTRA_APP_DATA_DIR+"/config.json","utf8")); const b=s.browser||{}; console.log("BROWSER_CLEAR_ENABLED="+b.enabled); console.log("BROWSER_CLEAR_PROVIDER="+b.provider); console.log("BROWSER_CLEAR_HEADLESS="+b.headless); console.log("BROWSER_CLEAR_VIEWPORT="+(b.viewport&&b.viewport.width)+"x"+(b.viewport&&b.viewport.height)); console.log("BROWSER_CLEAR_CDP="+(b.cdpUrl||"missing")); console.log("BROWSER_CLEAR_PROFILE="+(b.profile||"missing")); console.log("BROWSER_CLEAR_EXEC="+(b.executablePath||"missing")); console.log("BROWSER_CLEAR_AGENT="+(b.agentBrowser?"kept":"missing"));'`,
    );
    ...
    terminal.submit(
      `!node -e 'const fs=require("fs"); const s=JSON.parse(fs.readFileSync(process.env.MASTRA_APP_DATA_DIR+"/config.json","utf8")); const b=s.browser||{}; console.log("BROWSER_MODEL="+(b.stagehand&&b.stagehand.model||"missing"));'`,
    );
    ...
    terminal.submit(
      `!node -e 'const fs=require("fs"); const s=JSON.parse(fs.readFileSync(process.env.MASTRA_APP_DATA_DIR+"/config.json","utf8")); const b=s.browser||{}; console.log("BROWSER_MODEL_CLEARED="+(b.stagehand&&b.stagehand.model||"missing")); console.log("BROWSER_MODEL_ENV="+(b.stagehand&&b.stagehand.env||"missing"));'`,
    );
