 ...
 */
type VoiceEventType =
  | 'speaking' // Emitted when starting to speak
  | 'writing' // Emitted while speaking with audio data
  | 'transcriber' // Emitted when the transcriber stream is created
  | 'speaker' // Emitted when the speaker stream is created
  | 'writer' // Emitted when the writer stream is created
...
export class OpenAIRealtimeVoice extends MastraVoice {
  ...
  private setupEventListeners(): void {
    const inputStreams = {
      transcriber: new PassThrough(),
    };
    const responseStreams = new Map<string, { writer: StreamWithId; speaker: StreamWithId }>();
    ...
    this.client.on('session.created', ev => {
      ...
      this.emit('transcriber', inputStreams.transcriber);
    });
    ...
    this.client.on('response.created', ev => {
      ...
      const writerStream = new PassThrough() as StreamWithId;

      speakerStream.id = ev.response.id;
      writerStream.id = ev.response.id;

      responseStreams.set(ev.response.id, { speaker: speakerStream, writer: writerStream });
      this.emit('speaker', speakerStream);
      this.emit('writer', writerStream);
    });
    this.client.on('conversation.item.input_audio_transcription.delta', ev => {
      this.emit('transcription', { text: ev.delta, response_id: ev.response_id, role: 'user' });

      if (inputStreams.transcriber) {
        inputStreams.transcriber.write(ev.delta);
      }
    });
    this.client.on('conversation.item.input_audio_transcription.done', ev => {
      if (inputStreams.transcriber) {
        inputStreams.transcriber.end('\n');
      }
    });
    this.client.on('response.audio.delta', ev => {
      ...
      const stream = responseStreams.get(ev.response_id);
      if (stream?.speaker) {
        stream?.speaker.write(audio);
      }
    });
    ...
    this.client.on('response.text.done', ev => {
      this.emit('writing.done', { text: ev.text, response_id: ev.response_id });
    ...
    this.client.on('response.done', ev => {
      ...
      responseStreams.delete(ev.response.id);
    });
  }
