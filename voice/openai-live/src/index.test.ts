...
describe('OpenAILiveVoice', () => {
  ...
  describe('connect', () => {
    ...
    it('sends a session.update after connecting', async () => {
      await voice.connect();
      const updates = sentEvents(voice).filter((ev: any) => ev.type === LIVE_EVENTS.sessionUpdate);
      expect(updates).toHaveLength(1);
      expect(updates[0].session.model).toBe('gpt-live-1');
      expect(updates[0].session.voice).toBe('marin');
    });

    it('forwards delegation config in the session', async () => {
      ...
      const update = sentEvents(delegated).find((ev: any) => ev.type === LIVE_EVENTS.sessionUpdate);
      expect(update.session.delegation).toEqual({ type: 'responses', responses: { model: 'gpt-4o' } });
    });
  });
  ...
  describe('send', () => {
    it('appends base64 audio for an Int16Array', async () => {
      ...
      const appends = ws.send.mock.calls
        ...
        .filter((ev: any) => ev.type === LIVE_EVENTS.inputAudioAppend);
  ...
  describe('speak', () => {
    it('sends a response.create for text input', async () => {
      const ws = attachOpenSocket(voice);
      await voice.speak('Hello there');
      const responses = ws.send.mock.calls
        .map(([raw]: [string]) => JSON.parse(raw))
        .filter((ev: any) => ev.type === LIVE_EVENTS.responseCreate);
  ...
  describe('inbound events', () => {
    ...
    it('emits speaker + speaking on output audio delta', () => {
      ...
      (voice as any).client.emit(LIVE_EVENTS.outputAudioDelta, {
        type: LIVE_EVENTS.outputAudioDelta,
        audio: payload,
      });
    ...
    it('emits writing on transcript delta', () => {
      ...
      (voice as any).client.emit(LIVE_EVENTS.outputTranscriptDelta, {
        type: LIVE_EVENTS.outputTranscriptDelta,
        delta: 'partial text',
      });
    ...
    it('propagates error events', () => {
      ...
      (voice as any).client.emit(LIVE_EVENTS.error, { type: LIVE_EVENTS.error, error: { message: 'boom' } });
