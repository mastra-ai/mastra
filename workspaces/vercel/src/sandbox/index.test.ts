...
function createDeploymentResponse(id: string, url: string, readyState = 'BUILDING') {
  return {
    ok: true,
    json: async () => ({ id, url, readyState }),
    text: async () => '',
  };
}
...
describe('VercelSandbox', () => {
  ...
  describe('executeCommand()', () => {
    ...
    it('should throw if not started', async () => {
      ...
      await expect(freshSandbox.executeCommand('echo', ['hi'])).rejects.toThrow();
    });
