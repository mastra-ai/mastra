...
describe('DaytonaSandbox', () => {
  ...
  describe('Mount Configuration', () => {
    ...
    it('GCS prefix mount passes --only-dir to gcsfuse', async () => {
      mockSandbox.process.executeCommand.mockImplementation(async (command: string) => {
        if (command.includes('storage.googleapis.com')) {
          return { exitCode: 0, result: '' };
        }
        if (command.includes('which gcsfuse')) {
          return { exitCode: 0, result: '/usr/bin/gcsfuse' };
        }
        if (command.includes('id -u && id -g')) {
          return { exitCode: 0, result: '1000\n1000' };
        }
      ...
      await sandbox.mount(mockFilesystem, '/data/gcs-prefix');

      const mountCall = mockSandbox.process.executeCommand.mock.calls.find((call: any[]) => {
        const command = call[0] || '';
        return (
          command.includes('gcsfuse') && command.includes('/data/gcs-prefix') && !command.includes('which gcsfuse')
        );
      });

      expect(mountCall).toBeDefined();
      if (mountCall) {
        expect(mountCall[0]).toContain('--only-dir=workspace/data');
        expect(mountCall[0]).not.toContain('--only-dir=workspace/data/');
      }
    });

    it('GCS mount without prefix omits --only-dir', async () => {
      mockSandbox.process.executeCommand.mockImplementation(async (command: string) => {
        if (command.includes('storage.googleapis.com')) {
          return { exitCode: 0, result: '' };
        }
      ...
      await sandbox.mount(mockFilesystem, '/data/gcs-no-prefix');

      const mountCall = mockSandbox.process.executeCommand.mock.calls.find((call: any[]) => {
        const command = call[0] || '';
        return (
          command.includes('gcsfuse') && command.includes('/data/gcs-no-prefix') && !command.includes('which gcsfuse')
        );
      });

      expect(mountCall).toBeDefined();
      if (mountCall) {
        expect(mountCall[0]).not.toContain('--only-dir');
      }
    });
  });
