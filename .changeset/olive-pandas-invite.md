...
adapters. Older custom adapters remain compatible through process-local backoff, which is not durable
across evaluator or process restarts. The prior record-config state existed only on the unmerged
experimental branch and is intentionally not copied into the new storage record.
