...
- `@mastra/core`:
  ...
  - Mapping templates now accept `${stepResults.<stepId>}` with no subpath when the referenced step's `outputSchema` is a primitive (`z.string()` / `z.number()` / `z.boolean()`). The runtime still throws if the resolved value is an object or array, so `${stepResults.<stepId>.<field>}` remains required for structured outputs.
