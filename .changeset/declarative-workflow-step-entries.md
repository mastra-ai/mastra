...
- `@mastra/core`:
  ...
  - Mapping templates now accept `${stepResults.<stepId>}` with no subpath. Primitive step outputs render via `String(v)`; object and array outputs are JSON-encoded via `JSON.stringify` and inlined into the template. This makes patterns like `foreach(agent) → mapping → synthesis-agent` work naturally — the mapping can hand the full `{ text: string }[]` output of a foreach to a downstream agent as one JSON blob, instead of forcing callers to fake indexed access (`${stepResults.<id>.0.text}`) up to some fixed slot count. `null`/`undefined` still render as empty strings; unrepresentable values (circular references, BigInt) throw with a hint pointing at the placeholder.
