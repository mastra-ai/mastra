...
- `handlers.onAction` — override the default action (button click) handler. Receives `defaultHandler()` which runs the built-in `tool_approve:*` / `tool_deny:*` routing and returns `{ kind: 'approved' | 'denied' | 'unknown' }`.
