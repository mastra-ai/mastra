...
Which ring is working comes from the display state the server sends continuously, so a background pass shows on the budget it acts on rather than as one shared word.
