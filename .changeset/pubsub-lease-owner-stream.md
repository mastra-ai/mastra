...
Added a lease API to `PubSub` (`acquireLease` / `releaseLease` / `renewLease` / `getLeaseOwner`) and returned an `outcome` promise from `agent.sendSignal` / `sendMessage` / `queueMessage` / `sendStateSignal` / `sendNotificationSignal` that resolves once Mastra has settled what it did with the signal (for a `persist` signal, after it is written to memory).
