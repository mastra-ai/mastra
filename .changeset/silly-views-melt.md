...
`runEvals` also warns when two scorers or gates share an id. Results are keyed by id, so duplicates are combined into a single result (a later scorer overwrites the earlier one's score for each item; gate scores are pooled) and a failure can't be attributed to one of them.
