...
For anyone using `GoalManager` from `@mastra/code-sdk` directly: `saveToThread` no longer deletes when there is no goal in memory, and it only clears the older thread-metadata copy on a save that actually wrote. Use the new `deleteFromThread` to delete.
