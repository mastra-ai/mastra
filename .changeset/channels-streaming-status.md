...
- Adds optional `streaming` to `ChannelConfig` (default `false`). When enabled, the agent's text deltas are pushed into the platform message progressively via the Chat SDK's `StreamingPlan`.
