...
A chat session used to keep the exact text you first typed ("Tell me what have been done in the factory since…"), and a work session showed its branch ("factory/pr-22160"), even though Mastra had already named the underlying thread "PR review approval". The session row now mirrors that title when the thread is named, and reconciles against the stored thread title whenever the session is reopened, so sessions started before this also get named. The row only changes when the thread is first named or when you regenerate the title by hand — it never renames itself while you read it.
