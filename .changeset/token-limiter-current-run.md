...
Those messages are kept whole, so they are measured whole. When the messages carrying that tool traffic exceed the budget left after system messages and conversation overhead, nothing remains that can safely be removed, so the processor now fails with a non-retryable `TripWire` naming that as the cause. Previously `best-fit` removed the tool messages instead, leaving the model to re-issue the same tool call, and `contiguous` stopped but blamed it on "No messages fit within the remaining token budget".

**Behavior change to note**

Protection adds up across steps. A long multi-step run whose protected messages exceed that remaining budget now stops with that error, rather than quietly dropping its earlier tool traffic.
