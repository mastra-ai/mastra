...
- New `AgentChannels.close()` tears down all live thread subscriptions and approval card state — useful for tests and graceful shutdown:

```ts
for (const channels of Object.values(mastra.getChannels())) {
  channels.close();
}
```
