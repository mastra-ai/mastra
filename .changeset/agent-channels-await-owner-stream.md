...
When `sendMessage` returns an `ownerStream` (i.e. the call woke a new run from idle), the handler now awaits `ownerStream.consumeStream()` before returning, keeping the host alive for the duration of the run. Calls that lose the wake-path lease race resolve `ownerStream` to `undefined` and skip consumption — another invocation owns the run. Calls that join an in-flight run, queue, persist, or discard are unaffected.
