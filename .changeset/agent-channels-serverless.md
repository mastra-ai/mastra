...
**Await the agent run when waking a new run.** Previously `processChatMessage` fired `agent.sendMessage(...)` and returned immediately, relying on a side-effect subscription to drive the stream. That works in long-lived Node processes but breaks on serverless where the function exits as soon as it returns and kills the run mid-flight. The handler now awaits `result.outcome`; when it resolves to `{ action: 'wake', output }` (this invocation woke and owns the run), it awaits `output.consumeStream()` before returning. Any other outcome (`deliver`/`persist`/`discard`) means the signal was handed off — another invocation owns the run — so consumption is skipped.
