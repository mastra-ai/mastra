...
Added a `visibility: 'all' | 'llm'` flag to chunks and message parts so processors can hide content from the user-facing stream while keeping it in the agent loop, message list, and persisted memory.

A processor's `processOutputStream` can now return a chunk with `visibility: 'llm'` instead of returning `null`. The chunk continues through the pipeline so tools still execute and message-building still sees it, but it is filtered out of `fullStream`/`textStream`/`objectStream` (and any other user-facing readers built from them). The flag is propagated onto the resulting `MastraMessagePart` so UI-facing memory retrieval can filter it later via the new `filterMessagesByVisibility` helper.

The flag is optional and defaults to `'all'` — existing processors and chunks behave exactly as before.
