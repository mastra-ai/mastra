...
```ts
import { webSearchAction, webSearchLinks, webSearchTarget } from '@mastra/core/tools/provider-web-search';

const action = webSearchAction(result); // { type: 'openPage', url: 'https://mastra.ai/docs' }
webSearchTarget(action); // 'https://mastra.ai/docs' — the query, pattern or page, whichever the action carries
