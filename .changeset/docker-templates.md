...
- Build-time secrets via `runWithSecrets(command, { secrets, output })`: the step runs in a throwaway build stage forked from the steps before it, secret values are passed by value (`{ secrets }` on the template or `build()`), and only `output` is copied into the image, so values never land in the image's layers, config, or history, nor in the template identity.
