...
Channel tools (`send_message`, `add_reaction`, and friends) are no longer auto-injected into an agent's resolved toolset. If you want your agent to act on a channel run — send a message, add a reaction — pass the chat tools to your agent explicitly. `AgentChannels.getTools()` is now `@deprecated` but remains functional.
