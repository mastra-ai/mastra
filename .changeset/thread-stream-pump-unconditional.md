...
Also fixed `consumeStream()` resolving early for callers after the first. Once consumption had started, later `consumeStream()` calls returned immediately instead of waiting for the stream to finish, so a caller awaiting `consumeStream()` could continue while the run was still in flight. All callers now share a single drain promise and resolve together when the stream actually completes.

Also fixed `consumeStream()` resolving early for callers after the first. Consumption can be started from several places, and once it began, later `consumeStream()` calls returned immediately instead of waiting for the stream to finish. Callers reasonably expect `await consumeStream()` to mean the run is fully consumed, so the run could still be mid-flight when the promise resolved. All callers now share a single drain promise and resolve together when the stream actually completes.
