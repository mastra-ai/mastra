...
- Steering a running reply no longer clears the view: the steer slides in under the stream instead of parking at the top with an empty screen of room beneath it.
...
- Sending a message parks it at the top of the view with a full screen of room beneath, so the answer grows into empty space and nothing moves while it fits the window.
