...
The attachment is recorded as unavailable on its stored message (`metadata.mastra.unavailableAttachments`), so later turns show the same placeholder without trying to download it again. The record is per message: a new message that sends the same URL is downloaded again. The attachment itself stays in history.
