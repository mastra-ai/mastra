...
- Each item result includes a `toolReplay` divergence report (replayed count, misses, unconsumed events, argument mismatches) — useful for studying how agent behavior drifts between runs.
- Per-item recordings can also be pinned explicitly with `replayTraceId` on inline data items or `metadata.replayTraceId` on stored items.
