...
A run that fails after it started rejects the waiting caller immediately with the failure. Each lane turn also has a deadline (120 seconds) as the terminal backstop for a genuinely hung run: the waiting caller gets an error while the turn itself continues and its transcript still persists in order. A blocking ask also stops waiting when its calling turn aborts, without cancelling the lane turn.
