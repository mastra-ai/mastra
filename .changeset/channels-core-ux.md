...
- **Adaptive typing indicator** — the platform's typing status now reflects what the agent is doing (`is typing…` while generating text, `is calling {toolName}…` while a tool runs, `is waiting for approval…` while a tool is suspended), coalesced so the platform API isn't called on every delta.
