...
Tool-call results now carry `_meta`, emitting both the canonical nested `ui.resourceUri` and its legacy flat alias exactly as `tools/list` does. `_meta` returned by `execute()` is preserved and merged over the statically declared tool metadata, with the `ui` namespace merged key by key so sibling keys such as `ui.visibility` are not lost. Tools that declare an `outputSchema` are the exception: they validate their own output before returning and the schema strips unknown keys, so those tools can only supply `_meta` through the declared `mcp._meta` on the tool definition.
