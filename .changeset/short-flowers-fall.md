...
Added `dataset.purgeItem()` to permanently redact item content from every stored dataset version and linked experiment results while preserving version history and review status. Purged items reject later dataset updates, later experiment-result writes remain redacted, and MongoDB purges require transaction support.
