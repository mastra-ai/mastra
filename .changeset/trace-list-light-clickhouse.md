...
`listTracesLight` now projects only the columns a trace list renders, in both page and delta mode, and spans store a short `inputPreview` alongside `input`. Deriving that preview at read time forced ClickHouse to decompress the whole `input` column, so listing cost scaled with prompt size rather than page size.
