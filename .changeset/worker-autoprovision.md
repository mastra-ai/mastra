...
Added deploy-scoped worker provisioning for Mastra Cloud. Deployment builds now emit a nullable, versioned `workers.json` manifest when a Mastra instance explicitly configures shared storage and PubSub. The manifest describes orchestration, scheduler, background task, and registered custom workers using the target deployment environment, and `mastra deploy` uses it to coordinate dedicated worker services behind the `platform-workers` rollout flag.
