...
The default input is now a compact rounded field beside the filters, without an enclosing bar. Each selection morphs the current input capsule into a tag while the next input appears, preserving focus through quick selections. Suggestions stay anchored to the filter row. Completion, removal, and wrapped rows move together, with a selected-value preview and completion shimmer. Search and keyboard selection stay inline. Opt into the button with the same children:
