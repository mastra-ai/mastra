...
- **Tri-state waiting.** The default goal scorer emits `done`/`continue`/`waiting`; a `waiting` decision (only when the goal text explicitly asks to stop for the user) parks the objective as `paused` instead of looping.
...
The goal evaluation chunk now carries `pausedReason` and `judgeFailed`.
