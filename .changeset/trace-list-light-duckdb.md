...
Page-mode rows now carry an `inputPreview` derived from the stored `input` inside the query — without returning the blob to the caller — plus a computed `status` and the span `metadata` so Studio's configurable trace columns work on the lightweight list. Page responses include a `deltaCursor` so polling can switch to delta mode.
