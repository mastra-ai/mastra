...
Added platform-managed Jira intake for Factory deployments. Set `MASTRA_JIRA_CONNECTION_ID` together with `MASTRA_PLATFORM_ACCESS_TOKEN` or `MASTRA_PLATFORM_SECRET_KEY` to route Jira Cloud requests through one explicit Platform connection without runtime connection discovery. The connection applies to the entire Factory deployment, so deploy a separate Factory instance for each Jira tenant.
