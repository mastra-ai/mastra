...
The previous `agent.setHeartbeat()` (deterministic one-per-agent-or-thread id) is removed; use `mastra.heartbeats.create()` or `agent.heartbeats.create()` instead. Heartbeats survive process restarts: any persisted heartbeat row automatically starts the scheduler on boot, with no per-process registration step.
