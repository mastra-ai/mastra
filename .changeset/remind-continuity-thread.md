...
The session owns its derived reminder conversation. `Memory.deleteThread()` on a session thread now cascades to `subconscious:<threadId>:remind`, deleting its messages, its observational memory records, and its vectors through the same path as any other thread, so the retained reminder data does not outlive the session it belongs to.
