...
```sh
# Delete an experiment and the traces it produced
curl -X DELETE 'http://localhost:4111/api/experiments/exp_123'

# Delete the experiment but keep its traces
curl -X DELETE 'http://localhost:4111/api/datasets/ds_1/experiments/exp_123?deleteTraces=false'
```

Both routes respond `501` when the installed `@mastra/core` predates experiment deletion.
