...
Fixed Cloudflare Workers AI authentication returning 401 errors. Previously, `CLOUDFLARE_ACCOUNT_ID` was incorrectly used as the Bearer token in the Authorization header. Now `CLOUDFLARE_API_TOKEN` is used for authentication, while `CLOUDFLARE_ACCOUNT_ID` is correctly substituted into the API endpoint URL. Both environment variables are required.
