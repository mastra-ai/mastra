...
- **`init()` hook with request-context state** — plugins can declare `init(context)` to create long-lived state (like an authenticated API client) once per load. Tools read it at execution time via `requestContext.get(PLUGIN_STATE_KEY)[pluginName]`.
