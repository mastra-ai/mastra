...
Cadence is event-driven accounting evaluated after a successful commit: it is not a timer, poller, cron, or durable scheduler, it does not wake an idle process, and the counter is persisted but updated non-atomically, so it is advisory rather than exact - deployments using cadence with concurrent buffering may curate somewhat earlier or later than the configured interval. It remains off by default.
