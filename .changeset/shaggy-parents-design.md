...
**Live context usage in the composer.** A new ring indicator next to the model settings shows how full the model's context window is, measured from the real token usage streamed back on each run. Its popover breaks down the conversation: context used vs the model's context window (resolved from models.dev), cached input, output and reasoning tokens, conversation totals, plus what every request carries — estimated system prompt size, tool/workflow/skill counts and memory status — even before the first message.
