...
5. 検索ボリューム、競合性、関連キーワードを分析した上で、優先度を判断してください。

【出力形式】
1. メインキーワード（1-2語）：[キーワード]
2. 関連キーワード（5-8個）：
   - [キーワード1]：推定検索ボリューム（高・中・低）
   - [キーワード2]：推定検索ボリューム（高・中・低）
   （以下続く）
...
8. 以下のような要素を適切に織り込んでください：
   - 安全対策や学習環境（オンライン学習プラットフォームの利用など）
   - 教育プログラムの具体的内容（英語学習、プログラミング、資格対策など）
   - イベントや学習コースの詳細（オンラインセミナー、オープンキャンパスなど）
   - 保護者や受講者が利用できる便利な機能（アプリ、学習管理システムなど）
   - 申し込みの流れや手続き
...
## SEO最適化提案
- メタタイトル：[提案するタイトル]
- メタディスクリプション：[提案するディスクリプション]
- フォーカスキーワード：[主要キーワード]
- 内部リンク提案：[関連性の高いページへのリンク案]
- スラッグ提案：[URLに使用する文字列]

## 編集者コメント
[記事の強みや改善点、さらなる修正案があれば記載]
`,
  model: geminiModel,
}); 