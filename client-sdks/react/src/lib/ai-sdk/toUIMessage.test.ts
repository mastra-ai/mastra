...
describe('toUIMessage', () => {
  ...
  it('should handle workflow-related tool output chunks with accumulation', () => {
    ...
    // Check that output contains {networkMetadata, result} with accumulated workflow state
    const output = (toolPart as any).output;
    expect(output).toBeDefined();
    expect(output.networkMetadata).toBe(undefined);
    expect(output.result).toBeDefined();

    expect(output.result.runId).toBe('wf-run-789');
    expect(output.result.payload.workflowState.steps['step-1']).toEqual({
      id: 'step-1',
      name: 'process-data',
      status: 'success',
  ...
  it('should handle workflow finish chunk', () => {
    ...
    expect(output.result.payload.workflowState.status).toBe('success');
    expect(output.result.payload.currentStep).toBeUndefined();
  });
});
