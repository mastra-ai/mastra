  ...
  it.each([
    {
      ...
      primitiveId: 'recipe-workflow',
      finalResult: {
        runId: 'workflow-run-1',
        runResult: { status: 'success', steps: { input: { ingredient: 'tomato' } }, result: { ok: true } },
        runSuccess: true,
      },
      expectedCallId: 'workflow-run-1',
      expectedFrom: 'WORKFLOW',
      expectedOutput: {
        status: 'success',
        steps: { input: { ingredient: 'tomato' } },
        result: { ok: true },
        runId: 'workflow-run-1',
      },
    },
    {
      ...
      finalResult: { text: 'done', messages: [{ role: 'assistant', content: 'done' }] },
      expectedCallId: 'network-agent-result',
      expectedFrom: 'AGENT',
      expectedOutput: { text: 'done', messages: [{ role: 'assistant', content: 'done' }] },
    },
    {
      primitiveType: 'tool',
      primitiveId: 'weather-tool',
      finalResult: { result: { temperature: 21 }, toolCallId: 'weather-call-1' },
      expectedCallId: 'weather-call-1',
      expectedFrom: 'TOOL',
      expectedOutput: { temperature: 21 },
    },
  ])(
    'restores a persisted $primitiveType network result as a dynamic tool',
    async ({ primitiveType, primitiveId, finalResult, expectedCallId, expectedFrom, expectedOutput }) => {
      const initialMessages = [
        {
          id: 'network-agent-result',
          role: 'assistant',
          createdAt: new Date(),
          content: {
            format: 2,
            parts: [
              {
                ...
                text: JSON.stringify({
                  ...
                  primitiveType,
                  primitiveId,
                  selectionReason: 'best primitive',
      ...
      await waitFor(() => {
        ...
        const part = message?.content.parts[0] as unknown as Record<string, unknown>;
        const metadata = message?.content.metadata as MastraDBMessageMetadata | undefined;

        expect(message?.content.parts).toHaveLength(1);
        expect(part.type).toBe('dynamic-tool');
        expect(part.toolName).toBe(primitiveId);
        expect(part.toolCallId).toBe(expectedCallId);
        expect(part.state).toBe('output-available');
        expect(part.input).toEqual({ ingredient: 'tomato' });
        expect(part.output).toEqual(expectedOutput);
        expect(metadata?.from).toBe(expectedFrom);
        expect(metadata?.agentInput).toEqual({ ingredient: 'tomato' });
