...
describe('Scores Methods', () => {
  ...
  describe('getScoresBySpan()', () => {
    it('should construct correct URL and make GET request', async () => {
      const mockResponse = { pagination: { total: 0, page: 0, perPage: 10, hasMore: false }, scores: [] };
      mockFetchResponse(mockResponse);

      const result = await client.getScoresBySpan({
        traceId: 'trace-123',
        spanId: 'span-456',
      });

      // Test that response is passed through
      expect(result).toEqual(mockResponse);
    ...
    it('should construct correct URL with pagination parameters', async () => {
      const mockResponse = { pagination: { total: 15, page: 1, perPage: 5, hasMore: true }, scores: [] };
      mockFetchResponse(mockResponse);
