...
describe('Agent vNext', () => {
  ...
  it('stream: handles parallel client tool calls in single cycle', async () => {
    const toolCallId1 = 'call_1';
    const toolCallId2 = 'call_2';

    // First cycle: emit both tool calls
    const firstCycle = [
      { type: 'step-start', payload: { messageId: 'm1' } },
      {
        type: 'tool-call',
        payload: { toolCallId: toolCallId1, toolName: 'weatherTool', args: { location: 'NYC' } },
      },
      {
        type: 'tool-call',
        payload: { toolCallId: toolCallId2, toolName: 'newsTool', args: { topic: 'weather' } },
      },
      { type: 'step-finish', payload: { stepResult: { isContinued: false } } },
      { type: 'finish', payload: { stepResult: { reason: 'tool-calls' }, usage: { totalTokens: 3 } } },
    ];

    // Second cycle: final response
    const secondCycle = [
      { type: 'step-start', payload: { messageId: 'm2' } },
      { type: 'text-delta', payload: { text: 'Combined results' } },
      { type: 'step-finish', payload: { stepResult: { isContinued: false } } },
      { type: 'finish', payload: { stepResult: { reason: 'stop' }, usage: { totalTokens: 6 } } },
    ];
    ...
    const resp = await agent.stream({
      messages: 'Weather and news together',
      clientTools: { weatherTool, newsTool },
    });
    ...
    expect(toolCallChunks[0].payload.toolName).toBe('weatherTool');
    expect(toolCallChunks[1].payload.toolName).toBe('newsTool');

    // Verify final finish reason is stop
