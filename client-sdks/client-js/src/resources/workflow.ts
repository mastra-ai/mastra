...
export class Workflow extends BaseResource {
   ...
   * @returns Promise containing the workflow execution results
   */
  async streamFullVNext(params: { runId?: string; inputData: Record<string, any>; runtimeContext?: RuntimeContext }) {
    const searchParams = new URLSearchParams();
    ...
    const runtimeContext = parseClientRuntimeContext(params.runtimeContext);
    const response: Response = await this.request(
      `/api/workflows/${this.workflowId}/streamFullVNext?${searchParams.toString()}`,
      {
        method: 'POST',
        body: { inputData: params.inputData, runtimeContext },
        stream: true,
