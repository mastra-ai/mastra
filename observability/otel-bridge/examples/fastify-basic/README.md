# Fastify Basic Example - OtelBridge

> **Note**: This is a minimal example demonstrating the OtelBridge plugin integration with Fastify. The Mastra agent configuration may need adjustments based on your specific setup and API version.

Minimal example demonstrating OpenTelemetry context propagation with Mastra using Fastify.

## What This Demonstrates

**Scenario A**: HTTP service receiving W3C trace context headers

- Fastify server with OtelBridge plugin
...
```bash
curl -X POST http://localhost:3457/chat \
...
1. **Plugin Registration**: `fastifyPlugin` is registered with Fastify to extract OTEL headers and store them in `request.requestContext`
