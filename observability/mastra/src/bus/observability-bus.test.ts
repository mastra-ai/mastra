...
describe('cleanEvent — correlationContext preservation (issue: nulls in storage)', () => {
  it('preserves a plain-object correlationContext through deepClean', async () => {
    ...
    const event: LogEvent = {
      ...
      log: {
        ...
        correlationContext: {
          traceId: 'trace-1',
          spanId: 'span-1',
          entityType: 'AGENT' as any,
          entityId: 'agent-1',
          entityName: 'chef',
          parentEntityType: 'AGENT' as any,
          parentEntityId: 'agent-0',
          parentEntityName: 'parent',
          rootEntityType: 'AGENT' as any,
          rootEntityId: 'root-1',
          rootEntityName: 'root',
          userId: 'user-1',
          runId: 'run-1',
    ...
    const call = (exporter.onLogEvent as any).mock.calls[0][0] as LogEvent;
    expect(call.log.correlationContext).toBeDefined();
    expect(call.log.correlationContext).toMatchObject({
      traceId: 'trace-1',
      spanId: 'span-1',
      entityId: 'agent-1',
      parentEntityId: 'agent-0',
  ...
  it('drops correlationContext when it is on the prototype (getter or class field)', async () => {
    const exporter = createMockExporter({
      onLogEvent: vi.fn().mockResolvedValue(undefined),
    });
    const bus = new ObservabilityBus();
    ...
    // Simulate an emitter that exposes correlationContext via a prototype getter
    // (or a class with it on the prototype chain rather than as own enumerable).
    class LogWithProtoContext {
      timestamp = new Date();
      level = 'info' as const;
      message = 'hello';
    ...
    const event: LogEvent = { type: 'log', log: new LogWithProtoContext() as any };
    bus.emit(event);
    await bus.flush();

    const call = (exporter.onLogEvent as any).mock.calls[0][0] as LogEvent;
    // This documents the suspected bug: deepClean only copies own enumerable
    // keys, so a prototype-defined correlationContext is silently dropped.
    expect(call.log.correlationContext).toBeUndefined();
  });

  it('captures a snapshot — later mutations to the original do not reach exporters', async () => {
    const exporter = createMockExporter({
      onLogEvent: vi.fn().mockResolvedValue(undefined),
    });
    const bus = new ObservabilityBus();
    ...
    const log: any = {
      ...
      level: 'info',
      message: 'hello',
      // correlationContext intentionally absent at emit time
    };
    const event: LogEvent = { type: 'log', log };

    bus.emit(event);
    // Caller back-fills correlationContext after emit (e.g. span finishes later)
    log.correlationContext = { traceId: 'trace-late', spanId: 'span-late' };
