...
export class DefaultSpan<TType extends SpanType> extends BaseSpan<TType> {
  ...
  constructor(options: CreateSpanOptions<TType>, observabilityInstance: ObservabilityInstance) {
    ...
    if (options?.attributes) {
      this.attributes = { ...this.attributes, ...this.cleanForSpan(options.attributes) };
    }
    if (options?.metadata) {
      this.metadata = { ...this.metadata, ...this.cleanForSpan(options.metadata) };
    }
  ...
  error(options: ErrorSpanOptions<TType>): void {
    ...
    this.errorInfo = this.cleanForSpan(
    ...
    if (attributes) {
      this.attributes = { ...this.attributes, ...this.cleanForSpan(attributes) };
    }
    if (metadata) {
      this.metadata = { ...this.metadata, ...this.cleanForSpan(metadata) };
    }
