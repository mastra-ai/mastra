...
describe('DatadogExporter', () => {
  ...
  describe('span type mapping', () => {
    ...
    it('maps MODEL_CHUNK to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
    ...
    it('maps MCP_TOOL_CALL to tool kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.MCP_TOOL_CALL });
    ...
    it('maps WORKFLOW_STEP to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.WORKFLOW_STEP });
    ...
    it('maps WORKFLOW_CONDITIONAL to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.WORKFLOW_CONDITIONAL });
    ...
    it('maps WORKFLOW_CONDITIONAL_EVAL to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.WORKFLOW_CONDITIONAL_EVAL });
    ...
    it('maps WORKFLOW_PARALLEL to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.WORKFLOW_PARALLEL });
    ...
    it('maps WORKFLOW_LOOP to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.WORKFLOW_LOOP });
    ...
    it('maps WORKFLOW_SLEEP to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.WORKFLOW_SLEEP });
    ...
    it('maps WORKFLOW_WAIT_EVENT to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.WORKFLOW_WAIT_EVENT });
    ...
    it('maps PROCESSOR_RUN to task kind', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({ type: SpanType.PROCESSOR_RUN });
  ...
  describe('token usage transformation', () => {
    it('transforms AI SDK v5 usage metrics format (inputTokens/outputTokens) to datadog metrics format', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
    ...
    it('prefers v5 fields over v4 fields when both present', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({
        type: SpanType.MODEL_GENERATION,
        attributes: {
          usage: {
            inputTokens: 200, // v5 - should be used
            outputTokens: 100, // v5 - should be used
            promptTokens: 50, // v4 - should be ignored
            completionTokens: 25, // v4 - should be ignored
          },
        },
      });
    ...
    it('forwards cachedInputTokens from v5 format', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({
        type: SpanType.MODEL_GENERATION,
        attributes: {
  ...
  describe('input/output formatting', () => {
    ...
    it('stringifies object input for LLM spans', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({
        type: SpanType.MODEL_GENERATION,
        input: { query: 'search term', filters: { date: '2024' } },
      });
      ...
      expect(mockAnnotate).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({
          inputData: [{ role: 'user', content: '{"query":"search term","filters":{"date":"2024"}}' }],
        }),
      );
    });

    it('extracts text property from object output for LLM spans', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({
        type: SpanType.MODEL_GENERATION,
        output: { text: 'Hello world', metadata: { model: 'gpt-4' } },
      });
      ...
      expect(mockAnnotate).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({
          outputData: [{ role: 'assistant', content: 'Hello world' }],
        }),
      );
    });
  });
