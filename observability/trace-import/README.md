...
The initial importer reads completed traces from the Langfuse Observations API v2, preserves their source timestamps and metadata, validates complete trace trees, uploads deterministic Mastra spans through the existing Platform observability collector, and verifies a deterministic sample through the Platform query API.
