...
describe("MCPClient", () => {
	...
	describe("add", () => {
		...
		it("should throw for invalid configuration", async () => {
			...
			await expect(
				...
				'Command "invalid-cmd" not found in server configuration',
			)
		})

		it("should validate runtime arguments", async () => {
			// Get a server with runtime args validation
			const everythingServer = await registry.getServerDefinition({
				id: "everything-ref",
			})
		...
		it("should validate required environment variables", async () => {
			...
			await expect(
				...
				"Missing required environment variables: STRIPE_API_KEY",
			)
	...
	describe("edit", () => {
		...
		it("should validate environment variables", async () => {
			await expect(
				...
				'Environment variable "INVALID_KEY" not defined in server configuration',
			)
		})
		...
		it("should throw for non-existent server", async () => {
			await expect(
				...
				'RegistryClient: Server "non-existent" not found in registry at',
			)
		})
	})
	...
	describe("remove", () => {
		...
		it("should throw for non-existent server", async () => {
			await expect(config.remove("non-existent")).rejects.toThrow(
				"Configuration for server non-existent not found",
			)
		})
	})
