...
export class RegistryClient {
	...
	private getMockRegistry(): MockRegistry {
		const mockRegistry =
			MOCK_REGISTRIES[this.directoryUrl as keyof typeof MOCK_REGISTRIES]
		if (!mockRegistry) {
			throw new Error(
				`RegistryClient: Registry not found at ${this.directoryUrl}`,
			)
		}
		return mockRegistry
	}
	...
	public async listServers(): Promise<ServerDefinition[]> {
		...
		return mockRegistry.servers.map(
			(serverData: ServerData) => new ServerDefinition(serverData),
		)
	}

	public async getServerDefinition(input: {
		id: string
		...
		const mockRegistry = this.getMockRegistry()
		const serverData = mockRegistry.servers.find(
			(server) => server.id === input.id,
		)

		if (!serverData) {
			throw new Error(
				`RegistryClient: Server "${input.id}" not found in registry at ${this.directoryUrl}`,
			)
		}
