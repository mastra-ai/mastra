...
- **Number**: 13873
- **Title**: fix(server): only bypass auth in dev mode when no auth provider is configured
- **Author**: rphansen91
- **Created**: 2026-03-05T21:15:26Z
- **Updated**: 2026-03-07T00:04:23Z
- **Draft**: False
- **Review Decision**: APPROVED
- **Additions**: 44, **Deletions**: 14, **Changed Files**: 4
- **Branch**: fix/dev-auth-bypass -> main
- **Labels**: none
- **URL**: https://github.com/mastra-ai/mastra/pull/13873

## Steps

1. Run these commands to gather full PR details:
   ```
   gh pr view 13873 --json body,comments,reviews,commits,statusCheckRollup,mergeable,reviewRequests
   gh pr checks 13873
   gh pr diff 13873 | head -500
   ```
...
5. Write the JSON file to `maintainers/prs/13873.json` with this exact structure:
...
**IMPORTANT**: The file MUST be valid JSON. Write it to `maintainers/prs/13873.json`. Be concise but thorough in `summary` and `maintainerNotes`.
