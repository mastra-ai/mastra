...
./lima-cluster.sh sync --dry-run   # Preview what would sync
./lima-cluster.sh sync             # Actually sync
...
# Get results back
./lima-cluster.sh sync-back --dry-run 1  # Preview
./lima-cluster.sh sync-back 1            # Actually sync
...
| `sync [--dry-run] [workers...]`  | Sync repo TO workers (rsync)                   |
| `sync-back [--dry-run] <worker>` | Sync changes FROM worker back to repo          |
| `shell [n\|name]`                | Shell into a worker (default: worker-1)        |
| `list`                           | List all cluster VMs                           |
| `status [workers...]`            | Show git state of workers                      |
...
### 8. SIGPIPE with pipefail and grep -q

**Problem**: When using `set -o pipefail` with `grep -q` in a pipeline, SIGPIPE (exit code 141) causes failures. This happens because `grep -q` exits immediately when it finds a match, closing the pipe while the upstream command is still writing.

**Error**:

```bash
# This fails with exit code 141 under set -o pipefail
limactl list --json | grep -q '"name":"mastra-golden"'
```

**Solution**: Capture the output to a variable first, then grep:
...
### 9. Tilde Expansion in bash -c Strings

**Problem**: When using `limactl shell vm -- bash -c "..."`, the `~` character doesn't expand to the home directory because it's inside double quotes when the outer shell processes the command.

**Error**:

```
bash: line 1: /home/user/repo/.git-state: No such file or directory
```

**Solution**: Use `\$HOME` instead of `~` in bash -c commands:

```bash
# Wrong - ~ won't expand:
limactl shell "$vm" -- bash -c "mkdir -p ~/$REPO_NAME"

# Correct - $HOME expands in the remote shell:
limactl shell "$vm" -- bash -c "mkdir -p \$HOME/$REPO_NAME"
```

### 10. rsync Exclusions with String Concatenation

**Problem**: When building rsync options as a string with single quotes, the quotes become literal characters when the variable is expanded.

**Error**: Exclusions don't work - all files get synced including excluded paths.

**Solution**: Use bash arrays for rsync options:

```bash
# Wrong - single quotes become literal:
local rsync_opts="-avz --progress"
rsync_opts+=" --exclude='node_modules/'"
rsync $rsync_opts ...  # rsync sees --exclude='node_modules/' with literal quotes

# Correct - use arrays:
local -a rsync_opts=(-avz --progress)
rsync_opts+=(--exclude="node_modules/")
rsync "${rsync_opts[@]}" ...  # rsync sees --exclude=node_modules/ correctly
```

### 11. limactl JSON Output Parsing

**Problem**: `limactl list --json` outputs one JSON object per line (not a JSON array). Parsing with `tr ',' '\n'` mangles the JSON.

**Solution**: Process line-by-line since each line is a complete JSON object:

```bash
# Wrong - breaks JSON structure:
json=$(limactl list --json)
echo "$json" | tr ',' '\n' | grep ...

# Correct - read each JSON object as a line:
while IFS= read -r line; do
  name=$(echo "$line" | grep -o '"name":"[^"]*"' | head -1 | cut -d'"' -f4)
  status=$(echo "$line" | grep -o '"status":"[^"]*"' | head -1 | cut -d'"' -f4)
done < <(limactl list --json 2>/dev/null)
```
