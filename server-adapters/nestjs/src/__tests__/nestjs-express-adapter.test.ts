...
describe('NestJS Server Adapter (Express Platform)', () => {
  createRouteAdapterTestSuite({
    suiteName: 'NestJS Express Adapter Integration Tests',

    setupAdapter: async (context: AdapterTestContext) => {
      // Create NestJS app with default Express platform
      ...
      // Verify we're using Express
      expect(adapter.getPlatformType()).toBe('express');
  ...
  describe('Express Stream Data Redaction', () => {
  ...
  createMultipartTestSuite({
    suiteName: 'NestJS Express Multipart FormData',
