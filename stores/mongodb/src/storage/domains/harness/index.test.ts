...
import type { HarnessMongoDB } from './index';

const TEST_CONFIG = {
  id: 'test-mongodb-harness-store',
  uri: process.env.MONGODB_URI || 'mongodb://localhost:27017',
  dbName: process.env.MONGODB_DB || 'mastra_test',
};
...
describe('HarnessMongoDB', () => {
  let store: HarnessMongoDB;
  let composite: MongoDBStore;

  beforeEach(async () => {
    composite = new MongoDBStore(TEST_CONFIG);
    const harness = await composite.getStore('harness');
    store = harness as HarnessMongoDB;
    await store.init();
  ...
  it('exposes harness through MongoDBStore', async () => {
    const harness = await composite.getStore('harness');
