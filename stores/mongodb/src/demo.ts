...
async function demo() {
  ...
  try {
    ...
    const queryEmbedding = Array.from({ length: 128 }, () => Math.random()); // Example query embedding
    ...
    await store.insert({
      ...
      record: {
        name: 'example',
        value: 42,
        embedding: Array.from({ length: 128 }, () => Math.random()), // Example embedding
      },
    });
    // sleep for 1 second to ensure the document is inserted
