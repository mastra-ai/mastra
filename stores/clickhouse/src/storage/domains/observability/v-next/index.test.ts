  ...
  describe('shared normalization rules', () => {
    ...
    it('score promotes organizationId from metadata', async () => {
      await storage.createScore({
        score: {
          timestamp: new Date(),
          traceId: 'trace-meta-org',
          spanId: null,
          scorerId: 'quality',
          score: 0.9,
          reason: null,
          experimentId: null,
          metadata: { organizationId: 'org-from-meta' },
        },
      });

      const result = await storage.listScores({});
      const match = result.scores.find(s => s.traceId === 'trace-meta-org');
      expect(match).toBeDefined();
      expect(match!.organizationId).toBe('org-from-meta');
    });

    it('feedback promotes userId from metadata', async () => {
      await storage.createFeedback({
        feedback: {
          timestamp: new Date(),
      ...
      const result = await storage.listFeedback({});
      const match = result.feedback.find((f: any) => f.traceId === 'trace-meta-uid');
      expect(match).toBeDefined();
      expect(match!.userId).toBe('user-from-meta');
    });

    it('feedback promotes organizationId from metadata', async () => {
      await storage.createFeedback({
        feedback: {
          timestamp: new Date(),
          ...
          experimentId: null,
          sourceId: null,
          metadata: { organizationId: 'org-from-fb-meta' },
        },
      });

      const result = await storage.listFeedback({});
      const match = result.feedback.find((f: any) => f.traceId === 'trace-meta-fborg');
      expect(match).toBeDefined();
      expect(match!.organizationId).toBe('org-from-fb-meta');
    });

    it('top-level field takes priority over metadata promotion', async () => {
      await storage.createScore({
        score: {
          timestamp: new Date(),
          traceId: 'trace-meta-priority',
          spanId: null,
          scorerId: 'quality',
          score: 0.9,
          reason: null,
      ...
      const result = await storage.listScores({});
      const match = result.scores.find(s => s.traceId === 'trace-meta-priority');
      expect(match).toBeDefined();
      expect(match!.organizationId).toBe('org-top-level');
    });
