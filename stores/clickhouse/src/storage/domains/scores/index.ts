...
export class ScoresStorageClickhouse extends ScoresStorage {
  ...
  async listScoresByBatchId({
    ...
    try {
      // Get total count
      const countResult = await this.client.query({
        query: `SELECT COUNT(*) as count FROM ${TABLE_SCORERS} WHERE batchId = {var_batchId:String}${tenancy.sql}`,
        query_params: { var_batchId: batchId, ...tenancy.params },
        format: 'JSONEachRow',
      });
      const countRows = await countResult.json();
      let total = 0;
      ...
      // Get paginated results
      const result = await this.client.query({
        query: `SELECT * FROM ${TABLE_SCORERS} WHERE batchId = {var_batchId:String}${tenancy.sql} ORDER BY createdAt DESC LIMIT {var_limit:Int64} OFFSET {var_offset:Int64}`,
        query_params: {
          var_batchId: batchId,
          var_limit: limitValue,
          var_offset: start,
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('CLICKHOUSE', 'LIST_SCORES_BY_BATCH_ID', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
          details: { batchId },
        },
