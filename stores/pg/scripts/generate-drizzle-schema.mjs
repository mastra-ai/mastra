...
import { generateDrizzleSchema, extractImports } from '../../../scripts/drizzle-schema-generator/index.mjs';
...
await generateDrizzleSchema({
  ...
  postProcess: content => {
    ...
    content = content.replace(/public_mastra_/g, 'mastra_').replace(/\.op\(['"][^'"]+['"]\)/g, '');

    const tableNames = [...content.matchAll(/export const (\w+) = pgTable\(/g)].map(m => m[1]);
    if (!tableNames.length) return content;

    // Add pgSchema import
    content = content.replace(/import \{([^}]*)pgTable([^}]*)\}/, (m, a, b) =>
      m.includes('pgSchema') ? m : `import {${a}pgTable, pgSchema${b}}`,
    );

    let { imports, body } = extractImports(content);
    body = body.replace(/export const (\w+) = pgTable\(/g, 'const $1 = pgTable(').replace(/\bpgTable\(/g, 'table(');
