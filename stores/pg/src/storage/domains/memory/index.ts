...
export class MemoryPG extends MemoryStorage {
  ...
  public async listMessages(args: StorageListMessagesInput): Promise<StorageListMessagesOutput> {
    ...
    try {
      ...
      const conditions: string[] = [`thread_id IN (${inPlaceholders(allThreadIdsList.length)})`];
      const queryParams: any[] = [...allThreadIdsList];
      let paramIndex = allThreadIdsList.length + 1;
  ...
  async branchThread(args: StorageBranchThreadInput): Promise<StorageBranchThreadOutput> {
    ...
    if (!sourceThread) {
      throw new MastraError({
        id: createStorageErrorId('PG', 'BRANCH_THREAD', 'SOURCE_NOT_FOUND'),
        domain: ErrorDomain.STORAGE,
        category: ErrorCategory.USER,
        text: `Source thread with id ${sourceThreadId} not found`,
        details: { sourceThreadId },
    ...
    if (existingThread) {
      throw new MastraError({
        id: createStorageErrorId('PG', 'BRANCH_THREAD', 'THREAD_EXISTS'),
        domain: ErrorDomain.STORAGE,
        category: ErrorCategory.USER,
        text: `Thread with id ${newThreadId} already exists`,
        details: { newThreadId },
    ...
    try {
      return await this.#db.client.tx(async t => {
        ...
        const sourceMessages = await t.manyOrNone<{ id: string; createdAt: Date }>(
          `SELECT id, "createdAt" FROM ${messageTableName} WHERE thread_id = $1 ORDER BY "createdAt" ASC`,
          [sourceThreadId],
        );
        ...
        if (branchPoint && !sourceMessages.some(m => m.id === branchPoint)) {
          throw new MastraError({
            id: createStorageErrorId('PG', 'BRANCH_THREAD', 'BRANCH_POINT_NOT_FOUND'),
            domain: ErrorDomain.STORAGE,
            category: ErrorCategory.USER,
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('PG', 'BRANCH_THREAD', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
          details: { sourceThreadId, newThreadId },
        },
  ...
  async promoteBranch(args: StoragePromoteBranchInput): Promise<StoragePromoteBranchOutput> {
    ...
    if (!branchThread) {
      throw new MastraError({
        id: createStorageErrorId('PG', 'PROMOTE_BRANCH', 'BRANCH_NOT_FOUND'),
        domain: ErrorDomain.STORAGE,
        category: ErrorCategory.USER,
    ...
    if (!branchMetadata) {
      throw new MastraError({
        id: createStorageErrorId('PG', 'PROMOTE_BRANCH', 'NOT_A_BRANCH'),
        domain: ErrorDomain.STORAGE,
        category: ErrorCategory.USER,
    ...
    if (!parentThread) {
      throw new MastraError({
        id: createStorageErrorId('PG', 'PROMOTE_BRANCH', 'PARENT_NOT_FOUND'),
        domain: ErrorDomain.STORAGE,
        category: ErrorCategory.USER,
    ...
    try {
      return await this.#db.client.tx(async t => {
        ...
        const branchPointMessage = await t.oneOrNone<{ createdAt: Date }>(
          `SELECT "createdAt" FROM ${messageTableName} WHERE id = $1`,
          [branchPointMessageId],
        );

        let messagesAfterBranchPoint: { id: string }[] = [];
        if (branchPointMessage) {
          messagesAfterBranchPoint = await t.manyOrNone<{ id: string }>(
            `SELECT id FROM ${messageTableName}
             WHERE thread_id = $1 AND "createdAt" > $2
             ORDER BY "createdAt" ASC`,
            [parentThreadId, branchPointMessage.createdAt],
          );
        }
        ...
        if (messagesAfterBranchPoint.length > 0) {
          if (deleteParentMessages) {
            ...
            const messageIds = messagesAfterBranchPoint.map(m => m.id);
            await t.none(
              `DELETE FROM ${messageTableName} WHERE id IN (${inPlaceholders(messageIds.length)})`,
              messageIds,
            );
          } else {
            ...
            const messageIds = messagesAfterBranchPoint.map(m => m.id);
            await t.none(
              `UPDATE ${messageTableName} SET thread_id = $1 WHERE id IN (${inPlaceholders(messageIds.length, 2)})`,
              [archiveThreadId, ...messageIds],
            );
          }
        }

        // Move branch's own messages to the parent thread
        await t.none(`UPDATE ${messageTableName} SET thread_id = $1 WHERE thread_id = $2`, [
          parentThreadId,
          branchThreadId,
        ]);
        ...
        await t.none(`UPDATE ${threadTableName} SET metadata = $1, "updatedAt" = $2, "updatedAtZ" = $3 WHERE id = $4`, [
          JSON.stringify(updatedParentMetadata),
          now,
          now,
          parentThreadId,
        ]);
        ...
        await t.none(`DELETE FROM ${threadTableName} WHERE id = $1`, [branchThreadId]);
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('PG', 'PROMOTE_BRANCH', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
