...
export class MemoryPG extends MemoryStorage {
  ...
  async getThreadById({ threadId }: { threadId: string }): Promise<StorageThreadType | null> {
    try {
      const tableName = getTableName({ indexName: TABLE_THREADS, schemaName: getSchemaName(this.schema), tableMap: this.tableMap });
  ...
  async updateThread({
    ...
    const threadTableName = getTableName({ indexName: TABLE_THREADS, schemaName: getSchemaName(this.schema), tableMap: this.tableMap });
  ...
  async deleteThread({ threadId }: { threadId: string }): Promise<void> {
    try {
      const tableName = getTableName({ indexName: TABLE_MESSAGES, schemaName: getSchemaName(this.schema), tableMap: this.tableMap });
      const threadTableName = getTableName({ indexName: TABLE_THREADS, schemaName: getSchemaName(this.schema), tableMap: this.tableMap });
  ...
  async updateMessages({
    ...
    const selectQuery = `SELECT id, content, role, type, "createdAt", thread_id AS "threadId", "resourceId" FROM ${getTableName({ indexName: TABLE_MESSAGES, schemaName: getSchemaName(this.schema), tableMap: this.tableMap })} WHERE id IN ($1:list)`;
    ...
    await this.client.tx(async t => {
      ...
      for (const existingMessage of existingMessages) {
        ...
        if (setClauses.length > 0) {
          values.push(id);
          const sql = `UPDATE ${getTableName({ indexName: TABLE_MESSAGES, schemaName: getSchemaName(this.schema), tableMap: this.tableMap })} SET ${setClauses.join(', ')} WHERE id = $${paramIndex}`;
      ...
      if (threadIdsToUpdate.size > 0) {
        queries.push(
          t.none(
            `UPDATE ${getTableName({ indexName: TABLE_THREADS, schemaName: getSchemaName(this.schema), tableMap: this.tableMap })} SET "updatedAt" = NOW(), "updatedAtZ" = NOW() WHERE id IN ($1:list)`,
  ...
  async deleteMessages(messageIds: string[]): Promise<void> {
    ...
    try {
      const messageTableName = getTableName({ indexName: TABLE_MESSAGES, schemaName: getSchemaName(this.schema), tableMap: this.tableMap });
      const threadTableName = getTableName({ indexName: TABLE_THREADS, schemaName: getSchemaName(this.schema), tableMap: this.tableMap });
  ...
  async getResourceById({ resourceId }: { resourceId: string }): Promise<StorageResourceType | null> {
    const tableName = getTableName({ indexName: TABLE_RESOURCES, schemaName: getSchemaName(this.schema), tableMap: this.tableMap });
