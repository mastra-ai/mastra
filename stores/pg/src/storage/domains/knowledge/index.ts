  ...
  async listActivity(input: {
    ...
    for (const row of result.rows) {
      ...
      if (targetType === 'node') {
        ...
        if (!node || !isKnowledgeScopeVisible(await this.#getNodeScopeIds(this.#executor, targetId), scopeIds))
          continue;
      } else {
        const record = await this.#getRecord(this.#executor, targetId, true);
        if (!record || !(await this.#isRecordVisible(this.#executor, record, scopeIds))) continue;
      }
      events.push({
        id: String(row.id),
