...
describe('ObservabilityStoragePostgresVNext — integration', () => {
  describe.skipIf(!integrationEnabled)('init() — TimescaleDB path', () => {
    ...
    it('inserts a span end-to-end and reads it back via getTrace', async () => {
      const harness = await createHarness({
        connection: parseConnectionString(TIMESCALE_URL),
        schemaPrefix: 'obs_vnext_timescale_trace',
        setupDatabase: ensureTimescale,
      });

      try {
        const span = makeSpan({
          traceId: 'timescale-trace',
          spanId: 'timescale-root',
          startedAt: dayAt(0, 9),
        ...
        const trace = await harness.domain.getTrace({ traceId: 'timescale-trace' });
        expect(trace?.spans.map(entry => entry.spanId)).toEqual(['timescale-root']);
      } finally {
        await harness.close();
      }
    });

    it('serves listTraces page mode after inserts on a hypertable', async () => {
      const harness = await createHarness({
        connection: parseConnectionString(TIMESCALE_URL),
        schemaPrefix: 'obs_vnext_timescale_page',
        setupDatabase: ensureTimescale,
      });
      ...
      try {
        await harness.domain.createSpan({
          span: makeSpan({
            traceId: 'timescale-page-trace',
            spanId: 'timescale-page-root',
            startedAt: dayAt(0, 10),
            endedAt: dayAt(0, 10, 0, 1),
          }),
        });

        const result = await harness.domain.listTraces({ filters: { traceId: 'timescale-page-trace' } });
        expect(result.spans.map(span => span.spanId)).toEqual(['timescale-page-root']);
    ...
    it('serves listTraces delta mode after inserts on a hypertable', async () => {
      const harness = await createHarness({
        connection: parseConnectionString(TIMESCALE_URL),
        schemaPrefix: 'obs_vnext_timescale_delta',
        setupDatabase: ensureTimescale,
      });
      ...
      try {
        await withDeltaPolling(async () => {
          await harness.domain.createSpan({
            span: makeSpan({
              traceId: 'timescale-delta-trace',
              spanId: 'timescale-delta-root',
              startedAt: dayAt(0, 11),
              endedAt: dayAt(0, 11, 0, 1),
            }),
          });

          const delta = await harness.domain.listTraces({
            mode: 'delta',
            after: '0',
            limit: 10,
            filters: { traceId: 'timescale-delta-trace' },
          });

          expect(delta.spans.map(span => span.spanId)).toEqual(['timescale-delta-root']);
          expect(Number(delta.deltaCursor)).toBeGreaterThan(0);
        });
      } finally {
        await harness.close();
      }
