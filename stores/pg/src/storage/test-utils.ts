...
import type { MemoryStorageBase, StorageColumn, TABLE_NAMES } from '@mastra/core/storage';
import pgPromise from 'pg-promise';
import { afterAll, afterEach, beforeAll, beforeEach, describe, expect, it } from 'vitest';
import type { PostgresStoreConfig } from '../shared/config';
import { PGDomainBase } from './domains/base';
...
export function pgTests() {
  ...
  let domainBase: PGDomainBase;

  describe('Public Fields Access', () => {
    beforeAll(async () => {
      domainBase = new PGDomainBase({ config: TEST_CONFIG });
    });
    afterAll(async () => {
      try {
        await domainBase.close();
      } catch {}
    });
    it('should expose db field as public', () => {
      expect(domainBase.client).toBeDefined();
      expect(typeof domainBase.client).toBe('object');
      expect(domainBase.client.query).toBeDefined();
      expect(typeof domainBase.client.query).toBe('function');
    });

    it('should expose pgp field as public', () => {
      expect(domainBase.pgp).toBeDefined();
      expect(typeof domainBase.pgp).toBe('function');
      expect(domainBase.pgp?.end).toBeDefined();
      expect(typeof domainBase.pgp?.end).toBe('function');
    });

    it('should allow direct database queries via public db field', async () => {
      const result = await domainBase.client.one('SELECT 1 as test');
      expect(result.test).toBe(1);
    });

    it('should allow access to pgp utilities via public pgp field', () => {
      const helpers = domainBase.pgp?.helpers;
    ...
    it('should maintain connection state through public db field', async () => {
      ...
      const result1 = await domainBase.client.one('SELECT NOW() as timestamp1');
      const result2 = await domainBase.client.one('SELECT NOW() as timestamp2');
    ...
    it('should throw error when pool is used after disconnect', async () => {
      await domainBase.close();
      await expect(domainBase.client.connect()).rejects.toThrow();
    });
  });
  ...
  describe('PgStorage Table Name Quoting', () => {
    ...
    beforeEach(async () => {
      ...
      try {
        ...
        await domainBase.operations.clearTable({ tableName: camelCaseTable as TABLE_NAMES });
        await domainBase.operations.clearTable({ tableName: snakeCaseTable as TABLE_NAMES });
      } catch (error) {
    ...
    it('should create and upsert to a camelCase table without quoting errors', async () => {
      await expect(
        domainBase.operations.createTable({
          tableName: camelCaseTable as TABLE_NAMES,
          schema: BASE_SCHEMA,
        }),
      ).resolves.not.toThrow();

      await domainBase.operations.insert({
        tableName: camelCaseTable as TABLE_NAMES,
        record: { id: '1', name: 'Alice', createdAt: new Date(), updatedAt: new Date() },
      });

      const row: any = await domainBase.operations.load({
        tableName: camelCaseTable as TABLE_NAMES,
        keys: { id: '1' },
      });
  ...
  describe('Function Namespace in Schema', () => {
    ...
    let testDomainBase: PGDomainBase;
    ...
    beforeAll(async () => {
      ...
      testDomainBase = new PGDomainBase({
        config: {
          ...TEST_CONFIG,
          id: 'schema-fn-test-store',
          schemaName: testSchema,
    ...
    it('should create trigger function in the correct schema namespace', async () => {
      ...
      await testDomainBase.operations.createTable({
  ...
  describe('Timestamp Fallback Handling', () => {
    ...
    let memoryStore: MemoryStorageBase | undefined;
    ...
    it('should use createdAtZ over createdAt for messages when both exist', async () => {
      ...
      const messagesByIdResult = await memoryStore?.listMessagesById({ messageIds: [testMessageId] });
    ...
    it('should have consistent timestamp handling between threads and messages', async () => {
      ...
      // Get thread
      const retrievedThread = await memoryStore?.getThreadById({ threadId: testThreadId });
      expect(retrievedThread).toBeTruthy();
      expect(retrievedThread?.createdAt).toBeInstanceOf(Date);
      expect(retrievedThread?.createdAt?.getTime()).toBe(threadCreatedAt.getTime());

      // Get messages
      const messagesResult = await memoryStore?.listMessages({ threadId: testThreadId });
      expect(messagesResult?.messages.length).toBe(1);
      expect(messagesResult?.messages[0]?.createdAt).toBeInstanceOf(Date);
