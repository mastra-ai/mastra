...
describe('PgVector bit and sparsevec Type Support', () => {
  const connectionString = process.env.DB_URL || 'postgresql://postgres:postgres@localhost:5434/mastra';
  let vectorDB: PgVector;
  let pgVersionSupported = false;

  beforeAll(async () => {
    vectorDB = new PgVector({ connectionString, id: 'pg-vector-bit-sparse-test' });

    // Check if pgvector >= 0.7.0 is installed (required for bit and sparsevec)
    const client = await vectorDB.pool.connect();
    try {
      const result = await client.query(`
        SELECT extversion FROM pg_extension WHERE extname = 'vector'
      ...
      if (result.rows.length > 0) {
        ...
        pgVersionSupported = major > 0 || (major === 0 && minor >= 7);
      }
    } catch {
      pgVersionSupported = false;
    } finally {
      client.release();
    }
  });
  ...
  describe('bit Vector Type', () => {
    ...
    it('should create index with bit type', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: bit type requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should create bit index with jaccard metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: bit type requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should support bit type with HNSW index', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: bit type requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should support bit type with IVFFlat index (only hamming)', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: bit type requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should reject IVFFlat with bit and jaccard', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: bit type requires pgvector >= 0.7.0');
        return;
      }
      ...
      ).rejects.toThrow(/IVFFlat index only supports 'bit' vector type with 'hamming' metric/);
    });

    it('should upsert and query binary vectors using bit type', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: bit type requires pgvector >= 0.7.0');
        return;
      }
      ...
      // Create binary vectors (0s and 1s)
      const testVector1 = Array.from({ length: dimension }, () => Math.round(Math.random()));
      const testVector2 = Array.from({ length: dimension }, () => Math.round(Math.random()));

      const ids = await vectorDB.upsert({
        indexName: testIndexName,
        vectors: [testVector1, testVector2],
        metadata: [{ test: 'bit1' }, { test: 'bit2' }],
      });
    ...
    it('should verify bit column type in database', async () => {
      ...
      // Verify the table was created with bit type
      const client = await vectorDB.pool.connect();
      try {
        const result = await client.query(
          `
    ...
    it('should support up to 64000 dimensions for indexed bit vectors', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: bit type requires pgvector >= 0.7.0');
        return;
      }

      // bit type supports up to 64000 dimensions for indexed vectors
      const largeDimension = 64000;
    ...
    it('should throw error when exceeding dimension limit for bit type', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: bit type requires pgvector >= 0.7.0');
        return;
      }

      // bit type max is 64000 dimensions
      const tooLargeDimension = 64001;

      await expect(
        vectorDB.createIndex({
          indexName: testIndexName,
          dimension: tooLargeDimension,
          metric: 'hamming',
          vectorType: 'bit',
        }),
      ).rejects.toThrow(/bit indexed vectors can have at most 64000 dimensions/);
    });
  });

  // Sparse vector type tests
  describe('sparsevec Vector Type', () => {
    ...
    it('should create index with sparsevec type', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: sparsevec type requires pgvector >= 0.7.0');
        return;
      }

      await vectorDB.createIndex({
        indexName: testIndexName,
        dimension: 10000, // High dimension for sparse vectors
        metric: 'cosine',
        vectorType: 'sparsevec',
      });

      const stats = await vectorDB.describeIndex({ indexName: testIndexName });
      expect(stats.dimension).toBe(10000);
      expect(stats.vectorType).toBe('sparsevec');
      expect(stats.metric).toBe('cosine');
    });

    it('should create sparsevec index with l2 metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: sparsevec type requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should create sparsevec index with inner_product metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: sparsevec type requires pgvector >= 0.7.0');
        return;
      }
      ...
      const stats = await vectorDB.describeIndex({ indexName: testIndexName });
      expect(stats.vectorType).toBe('sparsevec');
      expect(stats.metric).toBe('inner_product');
    });

    it('should support sparsevec type with HNSW index', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: sparsevec type requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should support sparsevec type with flat index', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: sparsevec type requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should reject IVFFlat with sparsevec', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: sparsevec type requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should upsert and query sparse vectors', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: sparsevec type requires pgvector >= 0.7.0');
        return;
      }
      ...
      const nonZeroCount = 100; // Only 100 non-zero elements

      // Create sparse vectors
      const createSparseVector = (offset: number) => {
        const vector = new Array(dimension).fill(0);
        for (let i = 0; i < nonZeroCount; i++) {
          vector[(i + offset) % dimension] = Math.random();
        }
        return vector;
      };

      const testVector1 = createSparseVector(0);
      const testVector2 = createSparseVector(50); // Some overlap
      ...
      const ids = await vectorDB.upsert({
        indexName: testIndexName,
        vectors: [testVector1, testVector2],
        metadata: [{ test: 'sparse1' }, { test: 'sparse2' }],
      });
    ...
    it('should verify sparsevec column type in database', async () => {
      ...
      // Verify the table was created with sparsevec type
      const client = await vectorDB.pool.connect();
      try {
        const result = await client.query(
          `
    ...
    it('should throw error when exceeding non-zero element limit for sparsevec', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: sparsevec type requires pgvector >= 0.7.0');
        return;
      }
      ...
      const dimension = 10000;
      const tooManyElements = 1001; // sparsevec max is 1000 non-zero elements

      // Create a vector with too many non-zero elements
      const createSparseVector = (count: number) => {
        const vector = new Array(dimension).fill(0);
      ...
      const sparseVector = createSparseVector(tooManyElements);

      // Upsert should fail because the element count exceeds the limit
      await expect(
        vectorDB.upsert({
          indexName: testIndexName,
          vectors: [sparseVector],
        }),
      ).rejects.toThrow(/sparsevec indexed vectors can have at most 1000 non-zero elements/);
    });
  });

  // Index compatibility tests
  describe('Index Compatibility for bit and sparsevec', () => {
    const testIndexName = 'test_compat_index';
    ...
    it('should allow bit with HNSW and hamming metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should allow bit with HNSW and jaccard metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should allow sparsevec with HNSW and l2 metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should allow sparsevec with HNSW and cosine metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should allow sparsevec with HNSW and inner_product metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should reject bit with IVFFlat and cosine metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
    ...
    it('should reject sparsevec with HNSW and dotproduct metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
      ...
      await expect(
        ...
        /HNSW index with 'sparsevec' vector type only supports 'l2', 'cosine', or 'inner_product' metrics/,
      );
    });

    it('should reject bit with HNSW and euclidean metric', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
      ...
      ).rejects.toThrow(/HNSW index with 'bit' vector type only supports 'hamming' or 'jaccard' metrics/);
    });
  });

  // Type detection tests
  describe('Type Detection in describeIndex', () => {
    it('should detect bit vector type', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }

      const testIndexName = 'test_detect_bit';
    ...
    it('should detect sparsevec vector type', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }

      const testIndexName = 'test_detect_sparsevec';
  ...
  // Default metric tests
  describe('Default Metrics for bit and sparsevec', () => {
    const testIndexName = 'test_default_metric';
    ...
    it('should default to hamming for bit type', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }

      // When creating bit index without specifying metric, it should default to hamming
      // This is handled in createIndex validation
      await vectorDB.createIndex({
        indexName: testIndexName,
        dimension: 64,
        vectorType: 'bit',
      ...
      const stats = await vectorDB.describeIndex({ indexName: testIndexName });
      expect(stats.vectorType).toBe('bit');
      // The metric should be set appropriately
    });

    it('should default to cosine for sparsevec type', async () => {
      if (!pgVersionSupported) {
        console.log('Skipping test: requires pgvector >= 0.7.0');
        return;
      }
