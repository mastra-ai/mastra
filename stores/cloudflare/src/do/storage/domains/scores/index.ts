...
export class ScoresStorageDO extends ScoresStorage {
  ...
  async listScoresByBatchId({
    ...
    try {
      ...
      // Get total count
      const countQuery = createSqlBuilder().count().from(fullTableName).where('batchId = ?', batchId);
      applyTenancyFilters(countQuery, filters);
      const countResult = await this.#db.executeQuery(countQuery.build());
      const total = Array.isArray(countResult)
        ? Number(countResult?.[0]?.count ?? 0)
      ...
      // Get paginated results
      const selectQuery = createSqlBuilder().select('*').from(fullTableName).where('batchId = ?', batchId);
      applyTenancyFilters(selectQuery, filters);
      selectQuery.limit(limitValue).offset(start);
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('CLOUDFLARE_DO', 'GET_SCORES_BY_BATCH_ID', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
        },
        error,
