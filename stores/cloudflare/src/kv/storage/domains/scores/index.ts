...
export class ScoresStorageCloudflare extends ScoresStorage {
  ...
  async listScoresByDatasetId({
    ...
    try {
      ...
      for (const { name: key } of keys) {
        const score = await this.#db.getKV(TABLE_SCORERS, key);
        if (score && score.datasetId === datasetId && matchesTenancy(score, filters)) {
          scores.push(transformScoreRow(score));
        }
      }
      ...
      const mastraError = new MastraError(
        {
          id: createStorageErrorId('CLOUDFLARE', 'GET_SCORES_BY_DATASET_ID', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
          text: `Failed to get scores by dataset id: ${datasetId}`,
        },
        error,
      );
      this.logger?.trackException(mastraError);
