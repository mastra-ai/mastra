...
import type { StorageColumn, StoragePagination } from '@mastra/core/storage';
...
export class ScoresOracle extends ScoresStorage {
  ...
  async listScoresByRunId({
    ...
    try {
      return await this.listScores({
        whereClause: `WHERE ${SCORE_RUN_ID} = :runId`,
        binds: { runId },
        pagination,
  ...
  async listScoresByEntityId({
    ...
    try {
      return await this.listScores({
        whereClause: `WHERE ${SCORE_ENTITY_ID} = :entityId AND ${SCORE_ENTITY_TYPE} = :entityType`,
        binds: { entityId, entityType },
        pagination,
  ...
  async listScoresBySpan({
    ...
    try {
      return await this.listScores({
        whereClause: `WHERE ${SCORE_TRACE_ID} = :traceId AND ${SCORE_SPAN_ID} = :spanId`,
        binds: { traceId, spanId },
        pagination,
