...
function rowToItem(row: Record<string, any>): DatasetItem {
  ...
  return {
    ...
    timeout: t.timeout == null ? undefined : Number(t.timeout),
...
export class DatasetsSpanner extends DatasetsStorage {
  ...
  async init(): Promise<void> {
    ...
    await this.db.alterTable({
      ...
      ifNotExists: ['organizationId', 'projectId', 'externalId', 'unmockedToolPolicy', 'scorerIds', 'timeout'],
    });
  ...
  protected async _doUpdateItem(args: UpdateDatasetItemInput): Promise<DatasetItem> {
    try {
      ...
      const merged = {
        ...
        timeout: args.timeout !== undefined ? args.timeout : existing.timeout,
