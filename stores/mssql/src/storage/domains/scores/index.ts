...
export class ScoresMSSQL extends ScoresStorage {
  ...
  async listScoresByDatasetId({
    ...
    try {
      ...
      const request = this.pool.request();
      request.input('p1', datasetId);
      Object.entries(tenancy.params).forEach(([key, value]) => request.input(key, value));

      const totalResult = await request.query(
        `SELECT COUNT(*) as count FROM ${getTableName({ indexName: TABLE_SCORERS, schemaName: getSchemaName(this.schema) })} WHERE [datasetId] = @p1${tenancySql}`,
      );
      ...
      const dataRequest = this.pool.request();
      dataRequest.input('p1', datasetId);
      dataRequest.input('p2', limitValue);
      dataRequest.input('p3', start);
      Object.entries(tenancy.params).forEach(([key, value]) => dataRequest.input(key, value));

      const result = await dataRequest.query(
        `SELECT * FROM ${getTableName({ indexName: TABLE_SCORERS, schemaName: getSchemaName(this.schema) })} WHERE [datasetId] = @p1${tenancySql} ORDER BY [createdAt] DESC OFFSET @p3 ROWS FETCH NEXT @p2 ROWS ONLY`,
      );
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('MSSQL', 'LIST_SCORES_BY_DATASET_ID', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
          details: { datasetId },
        },
