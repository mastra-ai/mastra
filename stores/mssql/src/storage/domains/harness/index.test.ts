...
import { HarnessMSSQL } from './index';

const TEST_CONFIG: MSSQLConfig = {
  id: process.env.MSSQL_STORE_ID || 'test-mssql-harness-store',
...
const describeIfMSSQL = process.env.ENABLE_TESTS === 'true' ? describe : describe.skip;

describeIfMSSQL('HarnessMSSQL', () => {
  let pool: sql.ConnectionPool;
  let store: HarnessMSSQL;

  beforeEach(async () => {
    pool = createTestPool();
    await pool.connect();
    store = new HarnessMSSQL({ pool });
    await store.init();
  ...
  it('exposes harness through MSSQLStore', async () => {
    const composite = new MSSQLStore({ ...TEST_CONFIG, id: 'mssql-harness-composite' });
    try {
