...
describe('CloudflareVector', () => {
  ...
  const testIndexName = `default-${globalThis.crypto.randomUUID()}`;
  const testIndexName2 = `default-${globalThis.crypto.randomUUID()}`;
  ...
  describe('Index Operations', () => {
    ...
    const tempIndexNameCreateDescribeDelete = `create-describe-delete-${globalThis.crypto.randomUUID().slice(0, 8)}`;
  ...
  describe('Error Handling', () => {
    it('should handle duplicate index creation gracefully', async () => {
      const duplicateIndexName = `duplicate-test-${globalThis.crypto.randomUUID()}`;
      const dimension = 768;
