...
export function createSpan(overrides: Partial<CreateSpanRecord> = {}): CreateSpanRecord {
  ...
  const traceId = overrides.traceId || `trace-${globalThis.crypto.randomUUID()}`;
  const spanId = overrides.spanId || `span-${globalThis.crypto.randomUUID()}`;
...
export function createFeedbackRecord(overrides: Partial<CreateFeedbackRecord> = {}): CreateFeedbackRecord {
  return {
    feedbackId: overrides.feedbackId ?? `feedback-${globalThis.crypto.randomUUID()}`,
