...
export function createDatasetsTests({ storage }: { storage: MastraStorage }) {
  ...
  describe('Dataset CRUD', () => {
    ...
    it('updateDataset updates fields and returns merged record', async () => {
      ...
      const updated = await datasetsStorage.updateDataset({ id: ds.id, name: 'updated-name', description: 'new desc' });
  ...
  describe('Bulk Operations', () => {
    ...
    it('bulkAddItems uses single version bump for all items', async () => {
      ...
      const dv = await datasetsStorage.listDatasetVersions({ datasetId: ds.id, pagination: { page: 0, perPage: 10 } });
