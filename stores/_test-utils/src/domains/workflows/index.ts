...
export function createWorkflowsTests({ storage }: { storage: MastraStorage }) {
  ...
  describe('listWorkflowRuns', () => {
    ...
    it('should handle Unicode characters including null character in snapshot when filtering by status', async () => {
      // Test for GitHub issue #11563: "Unsupported unicode escape sequence" when listing messages
      // The issue occurs when snapshot contains null characters (\u0000) and we filter by status,
      // which triggers a TEXT to JSONB cast in PostgreSQL that fails on null characters.
      const workflowName = 'unicode_null_test';
      const runId = `run-${randomUUID()}`;

      // Create a snapshot with content containing:
      // 1. Accented characters (Portuguese: "Ótimo, já...")
      // 2. A null character (\u0000) which causes the PostgreSQL jsonb cast to fail
      const snapshotWithUnicode = {
        runId,
        ...
        waitingPaths: {},
        timestamp: Date.now(),
        // Include Unicode content that can trigger the issue
        // The null character \u0000 triggers PostgreSQL error 22P05 when casting TEXT to JSONB
      ...
      await workflowsStorage.persistWorkflowSnapshot({
        workflowName,
        runId,
        snapshot: snapshotWithUnicode,
      });

      // This should NOT throw "unsupported Unicode escape sequence" error
      // The issue is that PostgreSQL's ::jsonb cast fails on \u0000 (null character)
      const { runs } = await workflowsStorage.listWorkflowRuns({ status: 'success' });

      expect(runs.length).toBeGreaterThanOrEqual(1);
      const foundRun = runs.find(r => r.workflowName === workflowName);
      expect(foundRun).toBeDefined();
      const snapshot = foundRun!.snapshot as any;
      expect(snapshot.userMessage).toBe('Ótimo, já entendi! Vamos lá então.');
      // Verify the null character is preserved in the returned data (no data loss)
      expect(snapshot.problematicContent).toBe('Text with null char: \u0000 and accented: áéíóú');
      expect(snapshot.problematicContent.includes('\u0000')).toBe(true);
    });

    it('should handle unpaired Unicode surrogates in snapshot when filtering by status', async () => {
      // Test for GitHub issue #11563: Also handles unpaired surrogates (\uD800-\uDFFF)
      // PostgreSQL's jsonb cast fails on unpaired surrogates with:
      // "Unicode low surrogate must follow a high surrogate"
      const workflowName = 'unicode_surrogate_test';
      const runId = `run-${randomUUID()}`;

      // Create a snapshot with unpaired surrogates
      // JavaScript's JSON.stringify can produce these from strings with lone surrogates
      const snapshotWithSurrogate = {
        ...
        waitingPaths: {},
        timestamp: Date.now(),
        // Unpaired high surrogate (\uD800) and low surrogate (\uDC00)
        // These trigger PostgreSQL error when casting TEXT to JSONB
