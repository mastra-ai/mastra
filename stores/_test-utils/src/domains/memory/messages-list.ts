...
export function createMessagesListTest({ storage }: { storage: MastraStorage }) {
  ...
  describe('listMessages', () => {
    ...
    it('should list messages with pagination', async () => {
      ...
      const page3 = await memoryStore.listMessages({
        threadId: thread.id,
        perPage: 2,
        page: 2,
      });
    ...
    it('should filter by resourceId', async () => {
      ...
      await memoryStore.saveMessages({ messages: [differentResourceMessage] });
    ...
    it('should filter by date range', async () => {
      ...
      await memoryStore.saveThread({ thread: dateThread });
      ...
      await memoryStore.saveMessages({ messages: dateMessages });
    ...
    it('should handle empty thread', async () => {
      ...
      await memoryStore.saveThread({ thread: emptyThread });
    ...
    describe('perPage and page parameters', () => {
      ...
      it('should return ALL messages when perPage is false', async () => {
        ...
        await memoryStore.saveMessages({ messages: manyMessages });
      ...
      it('should handle page with perPage for pagination', async () => {
        // Page 0 (messages 0-1)
        const page0 = await memoryStore.listMessages({ threadId: thread.id, perPage: 2, page: 0 });
        expect(page0.messages).toHaveLength(2);
        expect(page0.hasMore).toBe(true);

        // Page 1 (messages 2-3)
        const page1 = await memoryStore.listMessages({ threadId: thread.id, perPage: 2, page: 1 });
        expect(page1.messages).toHaveLength(2);
        expect(page1.hasMore).toBe(true);

        // Page 2 (message 4)
        const page2 = await memoryStore.listMessages({ threadId: thread.id, perPage: 2, page: 2 });
      ...
      it('should handle perPage with resourceId filter', async () => {
        ...
        await memoryStore.saveMessages({ messages: otherMessages });
      ...
      it('should handle invalid perPage values gracefully', async () => {
        ...
        const result0 = await memoryStore.listMessages({
          threadId: thread.id,
          perPage: 0,
        });
        ...
        const resultNeg = await memoryStore.listMessages({
          threadId: thread.id,
          perPage: -5,
        });
