...
export function createThreadsTest({ storage }: { storage: MastraStorage }) {
  describe('Threads', () => {
    it('should create and retrieve a thread', async () => {
      ...
      // Retrieve thread
      const retrievedThread = await storage.getStore('memory')?.getThreadById({ threadId: thread.id });
    ...
    it('should return null for non-existent thread', async () => {
      const result = await storage.getStore('memory')?.getThreadById({ threadId: 'non-existent' });
    ...
    it('should get threads by resource ID', async () => {
      ...
      const { threads } = await storage.getStore('memory')?.listThreadsByResourceId({
    ...
    it('should update thread title and metadata', async () => {
      ...
      const updatedThread = await storage.getStore('memory')?.updateThread({
        id: thread.id,
        title: 'Updated Title',
    ...
    it('should delete thread', async () => {
      ...
      await storage.getStore('memory')?.deleteThread({ threadId: thread.id });
    ...
    it('should return paginated threads with total count', async () => {
      ...
      const page1 = await storage.getStore('memory')?.listThreadsByResourceId({ resourceId, page: 0, perPage: 7 });
      ...
      const page3 = await storage.getStore('memory')?.listThreadsByResourceId({ resourceId, page: 2, perPage: 7 });
    ...
    it('should return paginated results when no pagination params for listThreadsByResourceId', async () => {
      ...
      await storage.getStore('memory')?.saveThread({ thread: { ...createSampleThread(), resourceId } });

      const results = await storage.getStore('memory')?.listThreadsByResourceId({ resourceId, page: 0, perPage: 100 });
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle large metadata objects', async () => {
      ...
      await storage.getStore('memory')?.saveThread({ thread: threadWithLargeMetadata });
      const retrieved = await storage.getStore('memory')?.getThreadById({ threadId: thread.id });
    ...
    it('should handle concurrent thread updates', async () => {
      ...
      // Verify final state
      const finalThread = await storage.getStore('memory')?.getThreadById({ threadId: thread.id });
  ...
  //     const { threads: retrievedThreads } = await storage.getStore('memory')?.listThreadsByResourceId({
  ...
  describeSorting('Thread Sorting', () => {
    ...
    beforeEach(async () => {
      ...
      for (const threadInfo of threadData) {
        const savedThread = await storage.getStore('memory')?.saveThread({ thread: threadInfo });
    ...
    describe('listThreadsByResourceId sorting', () => {
      it('should sort paginated threads by createdAt DESC by default', async () => {
        const result = await storage.getStore('memory')?.listThreadsByResourceId({
          resourceId,
          page: 0,
          perPage: 3,
        });
      ...
      it('should maintain sort order consistency across pages', async () => {
        ...
        const { threads: allThreads } = await storage.getStore('memory')?.listThreadsByResourceId({
          resourceId,
          orderBy: { field: 'updatedAt', direction: 'DESC' },
          page: 0,
          perPage: 10,
        ...
        const page1 = await storage.getStore('memory')?.listThreadsByResourceId({
          resourceId,
          page: 0,
          perPage: 2,
          orderBy: { field: 'updatedAt', direction: 'DESC' },
        ...
        const page2 = await storage.getStore('memory')?.listThreadsByResourceId({
          resourceId,
          page: 1,
          perPage: 2,
          orderBy: { field: 'updatedAt', direction: 'DESC' },
        ...
        const page3 = await storage.getStore('memory')?.listThreadsByResourceId({
          resourceId,
          page: 2,
          perPage: 2,
          orderBy: { field: 'updatedAt', direction: 'DESC' },
      ...
      it('should handle single thread with sorting parameters', async () => {
        ...
        const singleThread = await storage.getStore('memory')?.saveThread({
    ...
    describe('Thread sorting edge cases', () => {
      it('should handle threads with identical timestamps', async () => {
        ...
        const { threads: result } = await storage.getStore('memory')?.listThreadsByResourceId({
          resourceId: identicalResourceId,
          page: 0,
          perPage: 3,
          orderBy: { field: 'createdAt', direction: 'ASC' },
