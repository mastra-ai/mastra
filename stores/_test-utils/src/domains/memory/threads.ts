...
export function createThreadsTest({ storage }: { storage: MastraStorage }) {
  ...
  describe('Threads', () => {
    ...
    it('should return null for non-existent thread', async () => {
      const result = await memoryStore.getThreadById({ threadId: 'non-existent' });
    ...
    it('should return paginated threads with total count', async () => {
      ...
      const threadPromises = Array.from({ length: 17 }, () =>
        memoryStore.saveThread({ thread: { ...createSampleThread(), resourceId } }),
      );
  ...
  describeSorting('Thread Sorting', () => {
    ...
    beforeEach(async () => {
      ...
      for (const threadInfo of threadData) {
        const savedThread = await memoryStore.saveThread({ thread: threadInfo });
    ...
    describe('listThreadsByResourceId sorting', () => {
      ...
      it('should maintain sort order consistency across pages', async () => {
        ...
        const { threads: allThreads } = await memoryStore.listThreadsByResourceId({
          resourceId,
          orderBy: { field: 'updatedAt', direction: 'DESC' },
          page: 0,
          perPage: 10,
        ...
        const page1 = await memoryStore.listThreadsByResourceId({
          resourceId,
          page: 0,
          perPage: 2,
          orderBy: { field: 'updatedAt', direction: 'DESC' },
        ...
        const page2 = await memoryStore.listThreadsByResourceId({
          resourceId,
          page: 1,
          perPage: 2,
          orderBy: { field: 'updatedAt', direction: 'DESC' },
        ...
        const page3 = await memoryStore.listThreadsByResourceId({
          resourceId,
          page: 2,
          perPage: 2,
          orderBy: { field: 'updatedAt', direction: 'DESC' },
      ...
      it('should handle single thread with sorting parameters', async () => {
        ...
        const singleThread = await memoryStore.saveThread({
    ...
    describe('Thread sorting edge cases', () => {
      it('should handle threads with identical timestamps', async () => {
        ...
        const { threads: result } = await memoryStore.listThreadsByResourceId({
          resourceId: identicalResourceId,
          page: 0,
          perPage: 3,
          orderBy: { field: 'createdAt', direction: 'ASC' },
