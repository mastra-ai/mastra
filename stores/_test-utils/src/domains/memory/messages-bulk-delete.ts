...
export function createMessagesBulkDeleteTest({ storage }: { storage: MastraStorage }) {
  describe('Messages Bulk Delete', () => {
    ...
    it('should delete multiple messages successfully', async () => {
      ...
      const { messages: remainingMessages } = await storage.getMessages({ threadId: thread.id });
    ...
    it('should handle messages from different threads', async () => {
      ...
      const { messages: thread1Messages } = await storage.getMessages({ threadId: 'bulk-thread-1' });
      ...
      const { messages: thread2Messages } = await storage.getMessages({ threadId: 'bulk-thread-2' });
    ...
    it('should handle large batches of message deletions', async () => {
      ...
      const { messages: allMessages } = await storage.getMessages({ threadId: thread.id, selectBy: { last: 100 } });
      ...
      const { messages: remainingMessages } = await storage.getMessages({ threadId: thread.id, selectBy: { last: 100 } });
