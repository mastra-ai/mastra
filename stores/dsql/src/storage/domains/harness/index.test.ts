...
const describeIfDSQL = canRunDSQLTests() ? describe : describe.skip;

describeIfDSQL('HarnessDSQL', () => {
  let pool: Pool;
  let store: HarnessDSQL;

  beforeEach(async () => {
    pool = createTestPool();
    store = new HarnessDSQL({ pool });
    await store.init();
  ...
  it('exposes harness through DSQLStore', async () => {
    const composite = new DSQLStore({ ...TEST_CONFIG, id: 'dsql-harness-composite' });
    try {
