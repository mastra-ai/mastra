    ...
    try {
      ...
      const results = await this.#client.batch(
        [
          ...
          {
            sql: `INSERT INTO ${TABLE_DATASET_ITEMS} (id, datasetId, datasetVersion, externalId, organizationId, projectId, validTo, isDeleted, input, groundTruth, expectedTrajectory, toolMocks, unmockedToolPolicy, timeout, requestContext, metadata, source, createdAt, updatedAt) VALUES (?, ?, (SELECT version FROM ${TABLE_DATASETS} WHERE id = ?), ?, (SELECT organizationId FROM ${TABLE_DATASETS} WHERE id = ?), (SELECT projectId FROM ${TABLE_DATASETS} WHERE id = ?), NULL, 0, jsonb(?), jsonb(?), jsonb(?), jsonb(?), ?, ?, jsonb(?), jsonb(?), jsonb(?), ?, ?)`,
            args: [
              id,
              args.datasetId,
              args.datasetId,
    ...
    try {
      ...
      await this.#client.batch(
        [
          ...
          {
            sql: `INSERT INTO ${TABLE_DATASET_ITEMS} (id, datasetId, datasetVersion, externalId, organizationId, projectId, validTo, isDeleted, input, groundTruth, expectedTrajectory, toolMocks, unmockedToolPolicy, timeout, requestContext, metadata, source, createdAt, updatedAt) VALUES (?, ?, (SELECT version FROM ${TABLE_DATASETS} WHERE id = ?), ?, (SELECT organizationId FROM ${TABLE_DATASETS} WHERE id = ?), (SELECT projectId FROM ${TABLE_DATASETS} WHERE id = ?), NULL, 1, jsonb(?), jsonb(?), jsonb(?), jsonb(?), ?, ?, jsonb(?), jsonb(?), jsonb(?), ?, ?)`,
            args: [
              id,
              datasetId,
              datasetId,
        ...
        try {
          ...
          if (plan.inserts.length > 0) {
            ...
            for (const { id, item } of plan.inserts) {
              await tx.execute({
                sql: `INSERT INTO ${TABLE_DATASET_ITEMS} (id, datasetId, datasetVersion, externalId, organizationId, projectId, validTo, isDeleted, input, groundTruth, expectedTrajectory, toolMocks, unmockedToolPolicy, timeout, requestContext, metadata, source, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, NULL, 0, jsonb(?), jsonb(?), jsonb(?), jsonb(?), ?, ?, jsonb(?), jsonb(?), jsonb(?), ?, ?)`,
                args: [
                  id,
                  input.datasetId,
                  newVersion,
  ...
  protected async _doBatchDeleteItems(input: BatchDeleteItemsInput): Promise<void> {
    try {
      ...
      for (const item of currentItems) {
        ...
        // Insert tombstone
        statements.push({
          sql: `INSERT INTO ${TABLE_DATASET_ITEMS} (id, datasetId, datasetVersion, externalId, organizationId, projectId, validTo, isDeleted, input, groundTruth, expectedTrajectory, toolMocks, unmockedToolPolicy, timeout, requestContext, metadata, source, createdAt, updatedAt) VALUES (?, ?, (SELECT version FROM ${TABLE_DATASETS} WHERE id = ?), ?, ?, ?, NULL, 1, jsonb(?), jsonb(?), jsonb(?), jsonb(?), ?, ?, jsonb(?), jsonb(?), jsonb(?), ?, ?)`,
          args: [
            item.id,
            input.datasetId,
            input.datasetId,
