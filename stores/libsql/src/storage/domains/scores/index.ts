...
export class ScoresLibSQL extends ScoresStorage {
  ...
  async listScoresByBatchId({
    ...
    try {
      ...
      const conditions: string[] = [`batchId = ?`];
      const queryParams: InValue[] = [batchId];
      appendTenancyConditions(conditions, queryParams, filters);
      const whereClause = `WHERE ${conditions.join(' AND ')}`;
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('LIBSQL', 'LIST_SCORES_BY_BATCH_ID', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
        },
        error,
  ...
  async listScoresByDatasetId({
    ...
    try {
      ...
      throw new MastraError(
        {
          id: createStorageErrorId('LIBSQL', 'LIST_SCORES_BY_DATASET_ID', 'FAILED'),
          domain: ErrorDomain.STORAGE,
          category: ErrorCategory.THIRD_PARTY,
        },
        error,
