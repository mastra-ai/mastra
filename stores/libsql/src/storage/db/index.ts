...
import { createExecuteWriteOperationWithRetry, prepareDeleteStatement, prepareStatement, prepareUpdateStatement } from "./utils";
...
export class LibSQLDB extends MastraBase {
    ...
    constructor({ client, maxRetries, initialBackoffMs }: { client: Client, maxRetries?: number, initialBackoffMs?: number }) {
        super({
            component: 'STORAGE',
     ...
     */
    public async batchDelete({ tableName, keys }: { tableName: TABLE_NAMES; keys: Array<Record<string, any>> }): Promise<void> {
        return this.executeWriteOperationWithRetry(
    ...
    public async delete(args: { tableName: TABLE_NAMES; keys: Record<string, any> }): Promise<void> {
        return this.executeWriteOperationWithRetry(
            () => this.doDelete(args),
            `delete from table ${args.tableName}`,
        ).catch(error => {
            throw new MastraError(
                {
    ...
    private getSqlType(type: StorageColumn['type']): string {
        switch (type) {
            ...
            case 'timestamp':
                return 'TEXT'; // Store timestamps as ISO strings
            case 'uuid':
                return 'TEXT';
            default:
                return 'TEXT';
