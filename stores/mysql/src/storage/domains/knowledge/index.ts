...
export class KnowledgeMySQL extends KnowledgeStorage {
  ...
  override async listScopeGrants(
    ...
    const scopeFilter = input.scopeNodeId ? ' AND g.scopeNodeId=?' : '';
    const result = await this.#executor.execute({
      sql: input.includeDeleted
        ? `SELECT g.scopeNodeId,g.scopeRefId,g.role,g.canSuggest FROM "${TABLE_KNOWLEDGE_SCOPE_GRANTS}" g WHERE 1=1${scopeFilter} ORDER BY g.scopeNodeId,g.scopeRefId`
        : `SELECT g.scopeNodeId,g.scopeRefId,g.role,g.canSuggest FROM "${TABLE_KNOWLEDGE_SCOPE_GRANTS}" g JOIN "${TABLE_KNOWLEDGE_NODES}" target ON target.id=g.scopeNodeId JOIN "${TABLE_KNOWLEDGE_NODES}" ref ON ref.id=g.scopeRefId WHERE target.deletedAt IS NULL AND ref.deletedAt IS NULL${scopeFilter} ORDER BY g.scopeNodeId,g.scopeRefId`,
      args: input.scopeNodeId ? [input.scopeNodeId] : [],
    });
    return result.rows.map(row => ({
      scopeNodeId: String(row.scopeNodeId),
      scopeRefId: String(row.scopeRefId),
      role: String(row.role) as KnowledgeScopeGrant['role'],
      ...(row.canSuggest === null ? {} : { canSuggest: Boolean(row.canSuggest) }),
    }));
  }
  ...
  async #mergeNodes(tx: Executor, input: Parameters<KnowledgeStorage['mergeNodes']>[0]): Promise<KnowledgeNode> {
    await this.#assertExpectedAccessEpoch(tx, input.expectedAccessEpoch);
    ...
    while (results.length < limit) {
      ...
      for (const row of records.rows) {
        ...
        results.push({
          ...
          name: parent!.name,
          text: record.text,
          scopeIds: await this.#getRecordScopeIds(this.#executor, record.id),
        });
        if (results.length >= limit) return results;
      }
    ...
    const currentTargetScopeIntersects = (ids: string[]) => `(
      (target.type='node' AND EXISTS (
        ...
        WHERE targetNode.id=target.id
          AND ((COALESCE(target.expectedDeleted,0)=1 AND targetNode.deletedAt IS NOT NULL)
            OR (COALESCE(target.expectedDeleted,0)=0 AND targetNode.deletedAt IS NULL))
          AND ((COALESCE(target.expectedDeleted,0)=1 AND EXISTS (
              SELECT 1 FROM JSON_TABLE(target.scopeIds, '$[*]' COLUMNS (scopeId VARCHAR(255) PATH '$')) retainedTargetScope
              WHERE ${inScope('retainedTargetScope.scopeId', ids)}
            ))
            OR (COALESCE(target.expectedDeleted,0)=0 AND targetNode.isScope=1 AND ${inScope('targetNode.id', ids)})
            OR (COALESCE(target.expectedDeleted,0)=0 AND targetNode.isScope=0 AND EXISTS (
  ...
  async listActivity(input: {
    ...
    const retainedVisible = (ids: string[]) => `EXISTS (
      SELECT 1 FROM JSON_TABLE(JSON_EXTRACT(activity.details, '$.${ACTIVITY_VISIBILITY_SCOPE_IDS}'), '$[*]' COLUMNS (scopeId VARCHAR(255) PATH '$')) retainedScope
      WHERE ${inScope('retainedScope.scopeId', ids)}
    )`;
    const targetVisible = `(
    ...
    clauses.push(`(
      JSON_UNQUOTE(JSON_EXTRACT(activity.details, '$.proposalId')) IS NULL OR EXISTS (
        SELECT 1 FROM "${TABLE_KNOWLEDGE_PROPOSALS}" proposal
        WHERE proposal.id=JSON_UNQUOTE(JSON_EXTRACT(activity.details, '$.proposalId')) AND ${proposalVisibility}
      )
    )`);
  ...
  async #activity(
    ...
    if (importRunId) {
      const importRun = await executor.execute({
        sql: `SELECT id FROM "${TABLE_KNOWLEDGE_IMPORT_RUNS}" WHERE id=? LIMIT 1`,
        args: [importRunId],
      });
      if (!importRun.rows.length) throw new KnowledgeNotFoundError('import run', importRunId);
    }
    await executor.execute({
