...
export class ScoresUpstash extends ScoresStorage {
  ...
  async listScoresByBatchId({
    ...
    keys.forEach(key => pipeline.get(key));
    const results = await pipeline.exec();
    // Filter out nulls and by batchId
    const filtered = results
      .map((raw: any) => {
        if (!raw) return null;
        if (typeof raw === 'string') {
      ...
      .filter(
        (row): row is Record<string, any> =>
          !!row && typeof row === 'object' && row.batchId === batchId && matchesTenancy(row, filters),
      );
    const total = filtered.length;
    const perPage = normalizePerPage(perPageInput, 100); // false → MAX_SAFE_INTEGER
    const { offset: start, perPage: perPageForResponse } = calculatePagination(page, perPageInput, perPage);
  ...
  async listScoresByDatasetId({
    ...
    keys.forEach(key => pipeline.get(key));
    const results = await pipeline.exec();
    // Filter out nulls and by datasetId
    const filtered = results
      .map((raw: any) => {
        if (!raw) return null;
        if (typeof raw === 'string') {
      ...
      .filter(
        (row): row is Record<string, any> =>
          !!row && typeof row === 'object' && row.datasetId === datasetId && matchesTenancy(row, filters),
      );
    const total = filtered.length;
    const perPage = normalizePerPage(perPageInput, 100); // false → MAX_SAFE_INTEGER
    const { offset: start, perPage: perPageForResponse } = calculatePagination(page, perPageInput, perPage);
