...
async function insertSpanEvents(db: DuckDB, rows: SpanEventRow[]): Promise<void> {
...
export async function createSpan(db: DuckDB, args: CreateSpanArgs): Promise<void> {
  await insertSpanEvents(db, [createSpanRow(args.span)]);
}

/** Insert an 'update' or 'end' event for an existing span. */
export async function updateSpan(db: DuckDB, args: UpdateSpanArgs): Promise<void> {
  await insertSpanEvents(db, [updateSpanRow(args)]);
}

/** Insert 'start' events for multiple spans in a single statement. */
export async function batchCreateSpans(db: DuckDB, args: BatchCreateSpansArgs): Promise<void> {
...
export async function batchUpdateSpans(db: DuckDB, args: BatchUpdateSpansArgs): Promise<void> {
  if (args.records.length === 0) return;
...
export async function batchDeleteTraces(db: DuckDB, args: BatchDeleteTracesArgs): Promise<void> {
...
export async function getSpan(db: DuckDB, args: GetSpanArgs): Promise<GetSpanResponse | null> {
...
export async function getRootSpan(db: DuckDB, args: GetRootSpanArgs): Promise<GetRootSpanResponse | null> {
...
export async function getTrace(db: DuckDB, args: GetTraceArgs): Promise<GetTraceResponse | null> {
...
export async function listTraces(db: DuckDB, args: ListTracesArgs): Promise<ListTracesResponse> {
