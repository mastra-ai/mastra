...
/**
 * Step that ingests knowledge documents into the knowledge base.
 * This demonstrates how to programmatically add content to Knowledge.
 */
const ingestStep = createStep({
  ...
  execute: async ({ mastra }) => {
    ...
    if (!supportKnowledge) {
      throw new Error('Knowledge not found - make sure knowledge is registered with Mastra');
    }
    ...
    const documentsAdded = sampleFAQDocuments.length + 1;
    console.log(`Added ${documentsAdded} documents to knowledge base`);
...
/**
 * Workflow that ingests sample FAQ documents into the knowledge base.
 *
 * This workflow demonstrates:
 * 1. Using mastra.getKnowledge() to access the registered knowledge instance
 * 2. Adding documents with different key patterns (static/* vs regular keys)
 * 3. Building a knowledge base programmatically through a workflow
 *
 * Usage:
 *   const result = await mastra.getWorkflow('ingest-knowledge').execute({});
 */
