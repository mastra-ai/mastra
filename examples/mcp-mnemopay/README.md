...
The example uses the published [`@mnemopay/sdk`](https://www.npmjs.com/package/@mnemopay/sdk) package from npm automatically via `npx` — no manual clone or build needed.

## Setup

1. **Install dependencies** from the mastra repo root:
...
Mastra Agent (GPT-4o)
    ...
    |       |
    |       +-- MnemoPay MCP Server (npx @mnemopay/sdk)
    |               |
