...
async function seedThread(threadId: string, resourceId: string, turns: Array<['user' | 'assistant', string]>, baseTime = Date.now()) {
  await callCenterMemory.saveThread({
    thread: { id: threadId, resourceId, title: threadId, metadata: {}, createdAt: new Date(baseTime), updatedAt: new Date(baseTime) },
  });
  const messages = turns.map(([role, text], i) => msg(threadId, resourceId, role, text, new Date(baseTime + i * 4_000)));
...
console.log('\n=== A. Short call (one exchange) → summary record produced ===');
{
  ...
  await seedThread(thread, resource, [
    ['user', "Hi, this is Dana Smith. I think I need a roof inspection — I saw a water stain on the ceiling after the storm. The house is at 815 Oak Street, zip 94103. Best number for me is 415-555-0142. Oh, and do you folks also do gutter cleaning?"],
    ['assistant', "Thanks Dana — I've got you down for a roof inspection at 815 Oak Street in 94103, and yes, gutter cleaning is something the team offers; I'll note your interest. Someone will call you back at 415-555-0142 to schedule. Is it okay if we keep a short summary of this call on file?"],
  ...
  check('sentiment is a valid enum value', !!record && ['positive', 'neutral', 'negative'].includes(record.sentiment), record?.sentiment);
  check('requestedServices captured roof inspection', !!record && record.requestedServices.join(' ').toLowerCase().includes('roof'), record?.requestedServices);
...
console.log('\n=== C. Degenerate calls: greeting-only and empty thread ===');
{
  ...
  await seedThread(thread, resource, [
    ['assistant', "Hi, you've reached Meridian Trades — quick heads up that I'm an AI assistant. How can I help today?"],
  ]);
  let crashed = false;
  try {
  ...
  console.log(`  [INFO] greeting-only produced a record: ${callRecords.has(thread)}`, callRecords.get(thread)?.summary ?? '');

  // Empty thread: summarizeThread must no-op without a model call.
  const emptyThread = `smoke-call-empty-${RUN}`;
  await callCenterMemory.saveThread({
    thread: { id: emptyThread, resourceId: resource, title: emptyThread, metadata: {}, createdAt: new Date(), updatedAt: new Date() },
  });
  const empty = await callCenterMemory.summarizeThread({ model: 'openai/gpt-4.1-mini', threadId: emptyThread, resourceId: resource });
...
console.log('\n=== D. Returning caller: sync OM distills call 1 at threshold; call 2 sees the facts ===');
{
  ...
  await seedThread(thread1, resource, [
    ['user', "Hi, my name is Marco Reyes and I'm looking for a general contractor for a kitchen remodel — new cabinets, countertops, and probably rewiring one wall for the oven."],
    ['assistant', 'Thanks Marco, a kitchen remodel with cabinets, countertops, and some electrical work — got it. What area are you in?'],
    ...
    ['user', 'Hoping to start in September, budget around forty thousand dollars, and I would prefer weekday work only since I work from home on weekends.'],
    ['assistant', 'September start, about $40k, weekdays only — noted. What is the best number to reach you for the estimator?'],
    ['user', 'You can reach me at 415-555-0177. Mornings are best. One more thing — for the cabinets I was thinking shaker style in a sage green, and my partner wants a quartz countertop, maybe with a waterfall edge on the island if the budget allows.'],
    ['assistant', 'Shaker cabinets in sage green and quartz countertops with a possible waterfall-edge island — lovely choices, noted. Do you already have appliances picked out, or should the estimator plan for appliance recommendations too?'],
    ['user', "We're keeping the fridge, but the range is being replaced with a thirty-six inch dual-fuel one, which is why that wall needs rewiring — the old range was gas only. The dishwasher is on its last legs too, so add that."],
    ['assistant', 'Understood — new thirty-six inch dual-fuel range with the electrical work to support it, plus a new dishwasher, keeping the existing fridge. Anything about flooring or are you keeping what you have?'],
    ['user', 'The floor is original fir and we want to keep and protect it during the work. Also, heads up: there is street parking only, and we have a very friendly but very loud beagle named Waffles who will be home during the day.'],
    ['assistant', "We'll plan floor protection for the original fir, note the street-parking situation for the crew, and warn everyone about Waffles the beagle — thanks for the heads up. Do you know if the building has any permit history we should look at?"],
    ['user', 'The upstairs unit did a similar remodel two years ago and their permits went through the city fine, so I can share their paperwork as a reference. Do you handle the permit filing or do I?'],
    ['assistant', 'We handle permit filing as part of the project — having the upstairs unit\'s paperwork as a reference will speed that up. The estimator will confirm the permit scope during the visit.'],
    ['user', 'Great. Next question: is your work insured and do you offer any warranty on the remodel?'],
    ['assistant', "Yes — we're fully licensed and insured, and remodel work carries a two-year workmanship warranty. I'll have the estimator bring the full warranty terms to the visit so you can read them before signing anything."],
    ['user', 'Good to know. How long does a remodel like this usually take once it starts? We are hosting family for the holidays in late November, so the kitchen really needs to be usable again by the third week of November at the absolute latest.'],
    ['assistant', 'A remodel of this scope typically runs eight to ten weeks. A September start puts completion in early-to-mid November, which fits your holiday deadline, but the estimator will build a schedule with buffer so a permit delay does not push you past the third week of November.'],
    ['user', 'Perfect. And while the wall is open for the range rewiring, could your electrician also add two extra outlets on the island and move the light switch that is currently hidden behind where the new fridge cabinet panel will go?'],
    ['assistant', 'Absolutely — two additional island outlets and relocating the switch away from the fridge panel are easy to fold into the same electrical scope while the wall is open. I have added both to the notes for the estimator.'],
  ]);
  ...
  if (om) {
    ...
    check('observer distilled call 1 into active observations', observed.observed && !!observed.record.activeObservations);
    check('observations mention the kitchen remodel', (observed.record.activeObservations ?? '').toLowerCase().includes('kitchen'));
