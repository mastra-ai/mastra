...
- Test project is at `_test-project`
  ...
  - The `pnpm dev` command doesn't exit automatically, it watches for changes and rebuilds on the fly. So you have to close it yourself after a while.
...
The type for `externals` should be `boolean | string[]`, where:

- `true` means all dependencies are external (except workspace dependencies)
- `string[]` means only the dependencies in the array are external
- `false` or `undefined` means no dependencies are external (current behavior)
...
- The code that needs to change is inside `packages/deployer`
- This is what defines something as "workspace":
  - `workspaceMap`
  - `isWorkspace`
  - `getWorkspaceInformation()`
- The `.mastra/.build` folder is an intermediate build folder. The final output is in `.mastra/output`
