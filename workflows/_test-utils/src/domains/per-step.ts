...
export function createPerStepWorkflows(ctx: WorkflowCreatorContext) {
  ...
  {
    mockRegistry.register('perstep-basic:step1', () =>
      vi.fn().mockResolvedValue({ value: 'step1-done' }),
    );
    mockRegistry.register('perstep-basic:step2', () =>
      vi.fn().mockResolvedValue({ value: 'step2-done' }),
    );
  ...
  {
    mockRegistry.register('perstep-parallel:stepA', () =>
      vi.fn().mockResolvedValue({ value: 'A-done' }),
    );
    mockRegistry.register('perstep-parallel:stepB', () =>
      vi.fn().mockResolvedValue({ value: 'B-done' }),
    );
    mockRegistry.register('perstep-parallel:stepC', () =>
      vi.fn().mockResolvedValue({ value: 'C-done' }),
    );
  ...
  {
    mockRegistry.register('perstep-conditional:check', () =>
      vi.fn().mockResolvedValue({ shouldBranch: true }),
    );
    mockRegistry.register('perstep-conditional:branchA', () =>
      vi.fn().mockResolvedValue({ result: 'took-A' }),
    );
    mockRegistry.register('perstep-conditional:branchB', () =>
      vi.fn().mockResolvedValue({ result: 'took-B' }),
    );
  ...
  {
    mockRegistry.register('perstep-nested:outer', () =>
      vi.fn().mockResolvedValue({ value: 'outer-done' }),
    );
    mockRegistry.register('perstep-nested:inner', () =>
      vi.fn().mockResolvedValue({ value: 'inner-done' }),
    );
    mockRegistry.register('perstep-nested:final', () =>
      vi.fn().mockResolvedValue({ value: 'final-done' }),
    );
...
export function createPerStepTests(ctx: WorkflowTestContext, registry?: WorkflowRegistry) {
  ...
  describe('perStep execution', () => {
    ...
    it.skipIf(skipTests.perStepParallel)('should execute only one step in parallel workflow when perStep is true', async () => {
    ...
    it.skipIf(skipTests.perStepConditional)('should execute only check step in conditional when perStep is true', async () => {
    ...
    it.skipIf(skipTests.perStepNested)('should execute only outer step in nested workflow when perStep is true', async () => {
    ...
    it.skipIf(skipTests.perStepContinue)('should continue execution step by step with multiple perStep calls', async () => {
      const { workflow, mocks } = registry!['perstep-basic'];
