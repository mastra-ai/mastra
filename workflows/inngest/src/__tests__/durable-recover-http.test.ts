/**
 * Regression test for issue #25160: `POST /api/agents/:agentId/recover` on a `createInngestAgent()`
 * agent must answer with a clear 400 "not supported" error, not a 500 telling the caller to set
 * `durable: true`.
 *
 * Runs against a real Inngest dev server, with the durable loop on a separate connect() worker, and
 * drives the real HTTP route through `createHonoServer`. The run is left suspended on a tool approval
 * so it is still live in Inngest when recovery is requested.
 */
...
const AGENT_ID = 'recover-http-agent';
const DB_PATH = `/tmp/mastra-recover-http-${Date.now()}.db`;
const DB_URL = `file:${DB_PATH}`;
const OUT_DIR = mkdtempSync(path.join(tmpdir(), 'recover-http-out-'));
...
describe('Inngest durable agent recover over HTTP (real Inngest dev server)', () => {
  beforeAll(async () => {
  ...
  it('answers /recover with a 400 "not supported" error instead of a 500', async () => {
    const threadId = `thread-${Date.now()}`;
    const resourceId = `user-a-${Date.now()}`;
    const auth = new SimpleAuth({
      tokens: { 'token-a': { id: resourceId } },
    ...
    // Wait until the worker has persisted the suspended loop snapshot, so the run is live in Inngest.
    let snapshot: any;
    for (let i = 0; i < 60 && snapshot?.status !== 'suspended'; i++) {
      const wf = await new DefaultStorage({ id: `recover-http-reader-${i}`, url: DB_URL }).getStore('workflows');
    ...
    const recoverRes = await app.request(`/api/agents/${AGENT_ID}/recover`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', authorization: 'Bearer token-a' },
      body: JSON.stringify({ runId }),
    });
    const body = await recoverRes.text();
    expect(recoverRes.status, body).toBe(400);
    expect(body).toContain('InngestAgent.recover() is not supported');
    expect(body).not.toContain('durable: true');

    // Boot-time recovery skips the Inngest agent instead of failing on it.
    await expect(mastra.recoverAllDurableAgents()).resolves.toMatchObject({ failed: 0, recovered: 0 });
  });
});
