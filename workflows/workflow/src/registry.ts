...
import type { ExecutionGraph, SerializedStepFlowEntry } from '@mastra/core/workflows';
 ...
 */
const REGISTRY_KEY = Symbol.for('@mastra/workflow.registry.v1');
...
export function requireRegisteredWorkflow(workflowId: string): RegisteredMastraWorkflow {
  ...
  if (!workflow) {
    ...
    throw new Error(
      `Workflow "${workflowId}" is not registered with @mastra/workflow. ` +
        `Registered workflows: ${known.length ? known.join(', ') : '(none)'}. ` +
        `Make sure the file in your "workflows/" directory that re-exports ` +
        `"@mastra/workflow/workflows" also imports your Mastra entrypoint for its side effects, ` +
