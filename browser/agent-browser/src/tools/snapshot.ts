 ...
 */
export function createSnapshotTool(getBrowser: () => Promise<BrowserManagerLike>) {
  return createTool({
    id: 'browser_snapshot',
    description:
    ...
    execute: async input => {
      try {
        ...
        const headerParts = [
          `Page: ${title}`,
          `URL: ${url}`,
          `Elements: ${startElement}-${endElement} of ${filteredElementCount} (options filtered)`,
        ];

        // Add scroll position info
        const scrollPercent =
          Math.round((scrollInfo.scrollY / (scrollInfo.scrollHeight - scrollInfo.viewportHeight)) * 100) || 0;
        ...
        return createError(
          ...
          'Ensure the browser is launched and the page has loaded.',
        );
      }
    },
  });
