...
must_haves:
  truths:
    - "BrowserToolset types are importable from the package"
    - "Config interface enforces type safety for headless and timeout options"
    - "Zod schemas validate navigate tool inputs and outputs"
  artifacts:
...
- Package builds with pnpm build (TypeScript compiles without errors)
