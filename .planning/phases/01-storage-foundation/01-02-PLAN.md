...
must_haves:
  truths:
    ...
    - "Adding/updating/deleting items auto-increments dataset version"
...
export class DatasetsInMemory extends DatasetsStorage {
  ...
  // Item CRUD with auto-versioning
  ...
  async deleteItem({ id, datasetId }: { id: string; datasetId: string }): Promise<void> {
    ...
    // Auto-increment version on delete
    const newVersion = dataset.version + 1;
  ...
  async listItems(args: ListDatasetItemsInput): Promise<ListDatasetItemsOutput> {
    ...
    // Filter by version if specified (snapshot semantics: items at or before this version)
    if (args.version !== undefined) {
      items = items.filter(item => item.version <= args.version!);
    }
  ...
  async getItemsByVersion({ datasetId, version }: { datasetId: string; version: number }): Promise<DatasetItem[]> {
    // Snapshot semantics: return items that existed at or before this version
    const items = Array.from(this.db.datasetItems.values()).filter(
      item => item.datasetId === datasetId && item.version <= version
    );
    items.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    return items;
  }
