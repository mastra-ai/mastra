...
<task type="auto">
  <name>Task 2: Add workflow schema route and update client SDK</name>
  <files>
    packages/server/src/server/handlers/workflows.ts
    packages/server/src/server/schemas/workflows.ts
    packages/client-js/src/resources/datasets.ts
    packages/client-js/src/types.ts
  </files>
  <action>
...
Add to route exports (check where routes are registered).

2. Update packages/client-js/src/types.ts to add schema fields to Dataset:
...
3. Add getWorkflowSchema method to client SDK (if not in datasets, add to workflows resource):
   ...
   <verify> - `pnpm typecheck` passes in packages/server and packages/client-js - GET /workflows/:workflowId/schema returns schema objects - Client SDK Dataset type includes inputSchema and outputSchema
   </verify>
   <done>Workflow schema route added, client SDK types and methods updated</done>
   </task>
