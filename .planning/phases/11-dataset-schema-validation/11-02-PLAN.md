...
must_haves:
  ...
  artifacts:
    ...
    - path: 'packages/core/src/datasets/validation/errors.ts'
      provides: 'SchemaUpdateValidationError class'
      contains: 'SchemaUpdateValidationError'
  key_links:
...
<task type="auto">
  <name>Task 2: Add SchemaUpdateValidationError and schema change validation with tests</name>
  <files>
    packages/core/src/datasets/validation/errors.ts
    packages/core/src/storage/domains/datasets/inmemory.ts
    packages/core/src/storage/domains/datasets/__tests__/datasets.test.ts
  ...
  <action>
1. Add SchemaUpdateValidationError to packages/core/src/datasets/validation/errors.ts:
...
2. Export SchemaUpdateValidationError from packages/core/src/datasets/validation/index.ts:
...
3. Update updateDataset in DatasetsInMemory to validate existing items when schema changes:

   ```typescript
   import { SchemaUpdateValidationError } from '../../../datasets/validation';
