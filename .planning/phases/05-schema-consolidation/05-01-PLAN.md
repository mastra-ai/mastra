...
must_haves:
  truths:
    - "types.ts has all 5 output schemas updated with error handling fields"
    ...
    - "No local Zod schema definitions in snapshot.ts or click.ts"
...
Output: types.ts updated with error handling fields; snapshot.ts and click.ts import from types.ts.
</objective>
