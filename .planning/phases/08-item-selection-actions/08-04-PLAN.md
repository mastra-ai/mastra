...
must_haves:
  truths:
    ...
    - 'Three-dot menu appears when dataset has ≥1 item'
    - 'Clicking Export/Create/Delete enters selection mode (checkbox column appears, header has select-all)'
    - 'Export to CSV downloads selected items immediately and clears selection'
    - 'Create Dataset opens modal with selected items; selection clears after dialog closes'
    - 'Delete Items shows confirmation; selection clears after deletion completes'
  artifacts:
  ...
  key_links:
    ...
    - from: 'dataset-detail.tsx'
      to: 'items-list.tsx'
      via: 'clearSelectionTrigger prop'
      pattern: 'clearSelectionTrigger'
---
...
clearSelectionTrigger?: number;  // Parent increments to clear selection
```
...
```tsx
{
  selectionMode !== 'idle' ? (
    <div className="flex items-center gap-2">
      <span className="text-sm text-neutral3">{selection.selectedCount} selected</span>
      <Button variant="primary" size="sm" disabled={selection.selectedCount === 0} onClick={handleExecuteAction}>
...
Add useEffect to watch clearSelectionTrigger prop from parent:

```typescript
// When parent increments clearSelectionTrigger, clear selection and exit selection mode
// This allows DatasetDetail to signal when dialogs close or actions complete
useEffect(() => {
...
const handleBulkDeleteConfirm = async () => {
  ...
  setItemIdsToDelete([]);
  setClearSelectionTrigger(prev => prev + 1); // Clear selection after delete
};

const handleCreateSuccess = (newDatasetId: string) => {
  setCreateDialogOpen(false);
  setItemsForCreate([]);
  setClearSelectionTrigger(prev => prev + 1); // Clear selection after create
  // Optionally navigate to new dataset
  onNavigateToDataset?.(newDatasetId);
};

// Also clear selection when create dialog is closed without success
const handleCreateDialogOpenChange = (open: boolean) => {
  setCreateDialogOpen(open);
  if (!open) {
    setItemsForCreate([]);
    setClearSelectionTrigger(prev => prev + 1); // Clear selection when dialog closes
  }
};
```
