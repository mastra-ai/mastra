...
6. Run `pnpm install` in auth/cloud to install jose
  ...
  <verify>
    ...
    - `! grep -q 'roles:' auth/cloud/src/client.ts` returns 0 (roles field REMOVED from CloudUser)
    - `grep -q 'parseUser(data.user, data.jwt)' auth/cloud/src/client.ts` returns 0 (jwt passed to parseUser)
  ...
  <done>
    ...
    - JWTClaims interface exists and is exported
    - exchangeCode() extracts `data.jwt` and returns { user, session, jwt }
    - parseUser() accepts jwt param and populates sessionToken field
    - parseUser() call in exchangeCode passes data.jwt as second argument
  </done>
</task>
