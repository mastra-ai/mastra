...
must_haves:
  truths:
    ...
    - 'client.getDatasetRun(datasetId, runId) returns single run'
    - 'client.getDatasetRunResults(datasetId, runId) returns results'
    - 'client.compareRuns(datasetId, params) returns comparison result'
  artifacts:
...
public getDatasetRun(datasetId: string, runId: string): Promise<DatasetRun> {
  return this.request(`/api/datasets/${datasetId}/runs/${runId}`);
}

public listDatasetRunResults(datasetId: string, runId: string, pagination?: { page?: number; perPage?: number }): Promise<ListDatasetRunResultsResponse> {
  const params = new URLSearchParams();
  if (pagination?.page !== undefined) params.set('page', String(pagination.page));
  if (pagination?.perPage !== undefined) params.set('perPage', String(pagination.perPage));
  const query = params.toString();
  return this.request(`/api/datasets/${datasetId}/runs/${runId}/results${query ? `?${query}` : ''}`);
}

// Analytics (nested under datasets)
public compareRuns(params: CompareRunsParams): Promise<CompareRunsResponse> {
  const { datasetId, ...body } = params;
  return this.request(`/api/datasets/${datasetId}/compare`, { method: 'POST', body: JSON.stringify(body) });
}
```
