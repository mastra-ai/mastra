...
must_haves:
  ...
  key_links:
    - from: 'packages/playground-ui/src/domains/datasets/components/comparison/comparison-view.tsx'
      to: 'useCompareRuns(datasetId, runIdA, runIdB)'
      via: 'hook call'
...
- Use useCompareRuns(datasetId, runIdA, runIdB) hook
...
- Props: datasetId: string, runIdA: string, runIdB: string
...
- Get datasetId and runId from params
- Use useDatasetRun(datasetId, runId) and useDatasetRunResults(datasetId, runId)
...
function DatasetRun() {
  ...
  const { data: run, isLoading: runLoading } = useDatasetRun(datasetId!, runId!);
...
- Get datasetId from params and runIdA/runIdB from query params (e.g., ?runA=xxx&runB=yyy)
- Render ComparisonView with datasetId, runIdA and runIdB
