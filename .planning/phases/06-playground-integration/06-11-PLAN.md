...
<task type="auto">
  <name>Task 2: Refactor useDatasetRunResults to object params and add polling</name>
  <files>
    packages/playground-ui/src/domains/datasets/hooks/use-dataset-runs.ts
    packages/playground/src/pages/datasets/dataset/run/index.tsx
  </files>
  <action>
Update useDatasetRunResults hook to use object params and poll while run is active.

**Step A: Update the hook signature to use object params (use-dataset-runs.ts)**

Change from positional args to object pattern for cleaner API:
...
**Step B: Update the consumer (run/index.tsx)**

Update the call site to use object syntax:
...
This is cleaner than positional args with `undefined` placeholders.
  </action>
  <verify>Run `pnpm build` from packages/playground-ui and packages/playground - both build successfully</verify>
  <done>useDatasetRunResults uses object params with runStatus, polls every 2s while run is pending/running</done>
</task>
