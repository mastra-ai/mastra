...
must_haves:
  truths:
    ...
    - "useDatasetRun(datasetId, runId) returns single run with polling"
    - "useDatasetRunResults(datasetId, runId) returns run results"
    - "useCompareRuns(datasetId, runIdA, runIdB) returns comparison"
...
export const useCompareRuns = (datasetId: string, runIdA: string, runIdB: string, options?: Omit<CompareRunsParams, 'datasetId' | 'runIdA' | 'runIdB'>) => {
  const client = useMastraClient();
  return useQuery({
