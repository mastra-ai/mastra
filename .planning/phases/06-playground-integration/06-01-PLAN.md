...
must_haves:
  truths:
    ...
    - 'GET /api/datasets/:datasetId returns dataset with items'
    - 'POST /api/datasets/:datasetId/items adds item to dataset'
    - 'GET /api/datasets/:datasetId/runs returns run history'
    - 'POST /api/datasets/:datasetId/runs triggers a new run'
    - 'GET /api/datasets/:datasetId/runs/:runId returns single run'
    - 'GET /api/datasets/:datasetId/runs/:runId/results returns run results'
    - 'POST /api/datasets/:datasetId/compare returns comparison between two runs'
  artifacts:
...
3. Run operations (nested under datasets):
   ...
   - getRunHandler: GET /api/datasets/:datasetId/runs/:runId - returns single run
   - listResultsHandler: GET /api/datasets/:datasetId/runs/:runId/results - returns paginated results

4. Analytics (nested under datasets):
   - compareRunsHandler: POST /api/datasets/:datasetId/compare - calls compareRuns() from analytics
   - Validates both runIdA and runIdB belong to the dataset (or warns if cross-version)
...
Route paths (all runs nested under datasets):
...
- GET /api/datasets/:datasetId/runs/:runId/results
- POST /api/datasets/:datasetId/compare
...
- POST /api/datasets/:datasetId/runs triggers runDataset execution
- POST /api/datasets/:datasetId/compare returns ComparisonResult
- All run routes nested under /api/datasets/:datasetId/runs/...
